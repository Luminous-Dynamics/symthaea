...
//!
//! The UI is physically constrained by its substrate. Low battery dims the
//! amber glow and slows animations. High DHT latency makes nodes sluggish.
//! No error popups — the organism enters *torpor* to conserve Joules.
//!
...
//! We catch the Promise rejection gracefully and default to `--device-energy: 1`.
//! The organism survives even when its sensory organs are blinded.
...
/// Torpor threshold: below this battery level (0.15 = 15%), the UI
/// enters torpor mode with reduced animations and dimmed visuals.
...
///
/// Attempts to read Battery Status API; silently falls back to
/// `device_energy = 1.0` if the browser blocks the API.
pub fn provide_thermodynamic_context() -> ThermodynamicState {
    let (device_energy, set_device_energy) = signal(1.0_f64);
    let (network_health, set_network_health) = signal(1.0_f64);
    let (torpor_level, set_torpor_level) = signal(0.0_f64);
    let (battery_available, set_battery_available) = signal(false);

    // Attempt Battery API — graceful fallback on rejection
    spawn_local(async move {
        match try_get_battery().await {
            Some(battery) => {
                set_battery_available.set(true);
                // Initial read
                let level = get_battery_level(&battery);
                set_device_energy.set(level);
                update_torpor(level, &set_torpor_level);

                // Poll periodically (Battery events aren't reliable in all browsers)
                let battery_clone = battery.clone();
                let _interval = Interval::new(BATTERY_POLL_MS, move || {
                    let level = get_battery_level(&battery_clone);
                    set_device_energy.set(level);
                    update_torpor(level, &set_torpor_level);
                });
                // Leak the interval to keep it alive for the app's lifetime
                std::mem::forget(_interval);
            }
            None => {
                // Battery API blocked (Firefox, Safari). Organism survives blind.
                web_sys::console::log_1(
                    &"[Hearth:Thermo] Battery API unavailable. Defaulting to full energy.".into(),
                );
    ...
    // Network health starts at 1.0, updated by DHT latency probes (Phase 6)
    let _ = set_network_health; // will be wired when Holochain is connected
...
fn update_torpor(battery_level: f64, set_torpor: &WriteSignal<f64>) {
    if battery_level < TORPOR_BATTERY_THRESHOLD {
        // Progressive torpor: 0% battery → torpor 1.0, threshold → torpor 0.0
...
// ---------------------------------------------------------------------------
// Battery API access (graceful fallback)
// ---------------------------------------------------------------------------

/// Try to get the BatteryManager. Returns None if the API is blocked.
async fn try_get_battery() -> Option<web_sys::BatteryManager> {
    let window = web_sys::window()?;
    let navigator = window.navigator();

    // navigator.getBattery() returns a Promise<BatteryManager>.
    // In Firefox/Safari this either doesn't exist or rejects.
...
fn get_battery_level(battery: &web_sys::BatteryManager) -> f64 {
    battery.level()
}
