...
//! Subject-aware content generator for South African CAPS Matric curriculum.
//!
//! Unlike the `MockGenerator` (which always returns Grade 3 content), this
//! generator inspects the standard code and description in the context string
//! to produce grade-appropriate, subject-specific content for CAPS Mathematics
//! and Physical Sciences (Grades 10-12).
//!
//! Content is curated from actual Matric exam-style problems and explanations.
//! No external LLM required — all content is embedded.

use crate::channels::{ContentChannels, ContentIntent};
use crate::pipeline::{ContentGenerator, GenerationOutput};

/// Content generator for SA CAPS Matric curriculum.
///
/// Produces subject-accurate content for Mathematics and Physical Sciences
/// by parsing the CAPS standard code from the context string.
...
// ============================================================
// Topic detection from context string
// ============================================================

fn detect_topic(context: &str) -> Box<dyn TopicContent> {
    // Match CAPS standard codes in the context
    if context.contains("P1.CALC") || context.contains("Differential Calculus") || context.contains("first principles") {
        Box::new(CalculusTopic)
    } else if context.contains("P1.ALG") || (context.contains("Algebra") && context.contains("Gr12")) {
        Box::new(AlgebraTopic { grade: 12 })
    } else if context.contains("P1.SEQ") || context.contains("Sequences and Series") || context.contains("Sigma notation") {
        Box::new(SequencesTopic)
    } else if context.contains("P1.FIN") || context.contains("Annuities") || context.contains("annuit") || context.contains("sinking fund") {
        Box::new(FinanceTopic)
    } else if context.contains("P1.FN") || context.contains("Functions and Graphs") {
        Box::new(FunctionsTopic)
    } else if context.contains("P1.COUNT") || context.contains("Counting Principles") || context.contains("permutation") {
        Box::new(CountingTopic)
    } else if context.contains("P2.TRIG") || context.contains("Trigonometry") || context.contains("compound angle") {
        Box::new(TrigonometryTopic)
    } else if context.contains("P2.GEOM") || context.contains("Euclidean Geometry") || context.contains("circle theorem") {
        Box::new(GeometryTopic)
    } else if context.contains("P2.ANAG") || context.contains("Analytical Geometry") {
        Box::new(AnalyticalGeomTopic)
    } else if context.contains("P2.STAT") || context.contains("Statistics") || context.contains("standard deviation") {
        Box::new(StatisticsTopic)
    // Physics
    } else if context.contains("P1.MECH") || context.contains("Momentum") || context.contains("momentum") || context.contains("Work, Energy") {
        Box::new(MechanicsTopic)
    } else if context.contains("P1.DOP") || context.contains("Doppler") {
        Box::new(DopplerTopic)
    } else if context.contains("P1.ELEC") || context.contains("Electric Circuits") || context.contains("Electrodynamics") {
        Box::new(ElectricityTopic)
    } else if context.contains("P1.MOD") || context.contains("Photoelectric") || context.contains("Emission") || context.contains("spectr") {
        Box::new(ModernPhysicsTopic)
    // Chemistry
    } else if context.contains("P2.ORG") || context.contains("Organic Chemistry") || context.contains("IUPAC") {
        Box::new(OrganicChemTopic)
    } else if context.contains("P2.RATE") || context.contains("Reaction Rate") || context.contains("collision theory") {
        Box::new(ReactionRateTopic)
    } else if context.contains("P2.EQUIL") || context.contains("Equilibrium") || context.contains("Le Chatelier") {
        Box::new(EquilibriumTopic)
    } else if context.contains("P2.ACID") || context.contains("Acids and Bases") || context.contains("titration") {
        Box::new(AcidBaseTopic)
    } else if context.contains("P2.ECHEM") || context.contains("Electrochemistry") || context.contains("Galvanic") {
        Box::new(ElectrochemTopic)
    } else if context.contains("P2.FERT") || context.contains("Fertiliser") {
        Box::new(FertiliserTopic)
    // Fallback for Gr10/11 topics
    } else if context.contains("ALG") || context.contains("Algebra") || context.contains("equation") {
        let grade = if context.contains("Gr11") { 11 } else { 10 };
        Box::new(AlgebraTopic { grade })
    } else if context.contains("FN.") || context.contains("Functions") || context.contains("parabola") {
        Box::new(FunctionsTopic)
    } else if context.contains("TRIG") || context.contains("sin") || context.contains("cos") {
        Box::new(TrigonometryTopic)
    } else if context.contains("GEOM") || context.contains("Geometry") {
        Box::new(GeometryTopic)
    } else if context.contains("PHY") || context.contains("Newton") || context.contains("kinematics") {
        Box::new(MechanicsTopic)
    } else if context.contains("CHM") || context.contains("Chemistry") || context.contains("bonding") || context.contains("mole") {
        Box::new(ReactionRateTopic) // fallback chemistry
    } else {
        Box::new(AlgebraTopic { grade: 12 }) // safe fallback
    }
}

fn detect_example_index(context: &str) -> usize {
    if context.contains("Example #3") || context.contains("index: 2") {
        2
    } else if context.contains("Example #2") || context.contains("index: 1") {
        1
    } else {
        0
    }
...
fn detect_difficulty(context: &str) -> u16 {
    if context.contains("advanced") || context.contains("complex") {
        875
    } else if context.contains("challenging") || context.contains("multi-step") {
        625
    } else if context.contains("medium") || context.contains("two steps") {
        375
    } else {
        125
    }
...
fn detect_hint_level(context: &str) -> u8 {
    if context.contains("level-3") || context.contains("very specific") {
        3
...
// ============================================================
// Topic trait and implementations
// ============================================================
...
mod tests {
    ...
    #[test]
    fn test_caps_generator_calculus() {
        ...
        assert!(out.text.contains("derivative") || out.text.contains("rate of change"));
        assert!(!out.text.contains("crayons")); // NOT mock content
        assert!(out.coherence >= 0.85);
    }

    #[test]
    fn test_caps_generator_physics() {
        let gen = CapsContentGenerator;
        let ch = ContentChannels::teaching_factual();
        let out = gen.generate(&ch, "Standard: CAPS.PhysicalSciences.Gr12.P1.MOD1 - Photoelectric Effect", 500);
        assert!(out.text.contains("photon") || out.text.contains("photoelectric"));
    }

    #[test]
    fn test_caps_generator_chemistry() {
        let gen = CapsContentGenerator;
        let ch = ContentChannels::teaching_factual();
        let out = gen.generate(&ch, "Standard: CAPS.PhysicalSciences.Gr12.P2.ACID - Acids and Bases", 500);
        assert!(out.text.contains("acid") || out.text.contains("pH") || out.text.contains("Brønsted"));
    }

    #[test]
    fn test_caps_generator_worked_example() {
        let gen = CapsContentGenerator;
        let ch = ContentChannels::worked_example();
        let out = gen.generate(&ch, "Standard: CAPS.Mathematics.Gr12.P1.CALC - Example #1 - Differential Calculus", 500);
        assert!(out.text.contains("Step 1"));
        assert!(out.text.contains("Answer:"));
    }

    #[test]
    fn test_caps_generator_practice_problem() {
        let gen = CapsContentGenerator;
        let ch = ContentChannels::practice_problem();
        let out = gen.generate(&ch, "Standard: CAPS.Mathematics.Gr12.P1.ALG - easy practice problem", 500);
        // Practice problems have answer\nexplanation\ndistractors format
        assert!(!out.text.is_empty());
    }

    #[test]
    fn test_caps_generator_flashcard() {
        let gen = CapsContentGenerator;
        let ch = ContentChannels::practice_problem();
        let out = gen.generate(&ch, "flashcard for CAPS.Mathematics.Gr12.P2.TRIG - Trigonometry", 200);
        assert!(out.text.contains("|")); // front | back format
    }

    #[test]
    fn test_caps_generator_misconception() {
        let gen = CapsContentGenerator;
        let ch = ContentChannels::misconception_correction();
        let out = gen.generate(&ch, "misconception about CAPS.Mathematics.Gr12.P1.CALC - calculus", 500);
        assert!(out.text.contains("WRONG:") || out.text.contains("RIGHT:"));
    }

    #[test]
    fn test_caps_generator_vocabulary() {
        let gen = CapsContentGenerator;
        let ch = ContentChannels::teaching_factual();
        let out = gen.generate(&ch, "Vocabulary for CAPS.PhysicalSciences.Gr12.P2.ECHEM - electrochemistry", 500);
        assert!(out.text.contains("anode") || out.text.contains("cathode"));
    }

    #[test]
    fn test_all_topics_produce_content() {
        let gen = CapsContentGenerator;
        let topics = [
            "P1.CALC", "P1.ALG Gr12", "P1.SEQ", "P1.FIN", "P1.FN Functions and Graphs",
            "P1.COUNT", "P2.TRIG", "P2.GEOM", "P2.ANAG", "P2.STAT",
            "P1.MECH", "P1.DOP Doppler", "P1.ELEC", "P1.MOD Photoelectric",
            "P2.ORG Organic", "P2.RATE", "P2.EQUIL", "P2.ACID", "P2.ECHEM", "P2.FERT Fertiliser",
        ];
        for topic in &topics {
            let ch = ContentChannels::teaching_factual();
            let out = gen.generate(&ch, topic, 500);
            assert!(!out.text.is_empty(), "Topic '{}' produced empty content", topic);
            assert!(!out.text.contains("crayons"), "Topic '{}' fell back to mock content", topic);
        }
    }
}
