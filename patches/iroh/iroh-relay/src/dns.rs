...
impl DnsProtocol {
    fn connection_config(self, addr: SocketAddr) -> ConnectionConfig {
        let server_name: Arc<str> = Arc::from(addr.ip().to_string());
        let mut config = match self {
            DnsProtocol::Udp => ConnectionConfig::udp(),
            DnsProtocol::Tcp => ConnectionConfig::tcp(),
            DnsProtocol::Tls => ConnectionConfig::tls(server_name),
            DnsProtocol::Https => ConnectionConfig::https(server_name, None),
        };
...
impl HickoryResolver {
    ...
    fn build_resolver(builder: &Builder) -> TokioResolver {
        ...
        for (addr, proto) in builder.nameservers.iter() {
            config.name_servers.push(NameServerConfig::new(
                addr.ip(),
                true,
                vec![proto.connection_config(*addr)],
            ));
        }
        ...
        *hickory_builder.options_mut() = options;
        ...
        hickory_builder
            .build()
            .expect("valid Hickory DNS resolver configuration")
    }
    ...
    fn system_config() -> Result<(ResolverConfig, ResolverOpts), hickory_resolver::net::NetError> {
        ...
        let (domain, search, name_servers) = system_config.into_parts();
        let name_servers = name_servers
            .into_iter()
            .filter(|nameserver| !WINDOWS_BAD_SITE_LOCAL_DNS_SERVERS.contains(&nameserver.ip))
            .collect();
        Ok((
    ...
    fn lookup_ipv6(&self, host: String) -> BoxFuture<Result<BoxIter<Ipv6Addr>, DnsError>> {
        ...
        Box::pin(async move {
            ...
            let addresses = lookup
                ...
                .iter()
                .filter_map(|record| match &record.data {
                    RData::AAAA(address) => Some(Ipv6Addr::from(*address)),
                    _ => None,
                })
                .collect::<Vec<_>>();
