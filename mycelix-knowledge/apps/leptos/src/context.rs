...
pub fn provide_knowledge_context() {
    let state = KnowledgeCtx {
        claims: RwSignal::new(mock_claims()),
        fact_checks: RwSignal::new(mock_fact_checks()),
        inferences: RwSignal::new(mock_inferences()),
        graph_stats: RwSignal::new(GraphStatsView { relationship_count: 47 }),
    };
...
fn mock_fact_checks() -> Vec<FactCheckResultView> {
    vec![
...
fn mock_inferences() -> Vec<InferenceView> {
    vec![
