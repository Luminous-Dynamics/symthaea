...
impl TemplateService {
    ...
    pub async fn create_template(
        ...
        let body_text = input
            .body_text
            .unwrap_or_else(|| html2text::from_read(input.body_html.as_bytes(), 80));
        let id = Uuid::new_v4();
        let fields_json = serde_json::to_value(&fields)
            .map_err(|e| TemplateError::InvalidSyntax(e.to_string()))?;
    ...
    pub async fn list_templates(
        ...
        user_id: Uuid,
        category: Option<&str>,
    ) -> Result<Vec<EmailTemplate>, TemplateError> {
        let rows = sqlx::query!(
    ...
    pub async fn render_template(
        ...
        if let Some(ref r) = context.recipient {
            ...
            if let Some(ref v) = r.first_name {
                data.insert("recipient.firstName".to_string(), v.clone());
            }
            if let Some(ref v) = r.last_name {
                data.insert("recipient.lastName".to_string(), v.clone());
            }
            if let Some(ref v) = r.company {
                data.insert("recipient.company".to_string(), v.clone());
            }
        }
        if let Some(ref s) = context.sender {
            data.insert("sender.email".to_string(), s.email.clone());
            if let Some(ref v) = s.first_name {
                data.insert("sender.firstName".to_string(), v.clone());
            }
            if let Some(ref v) = s.last_name {
                data.insert("sender.lastName".to_string(), v.clone());
            }
            if let Some(ref v) = s.signature {
                data.insert("sender.signature".to_string(), v.clone());
            }
        }
    ...
    fn extract_fields(
        ...
        subject: &str,
        body: &str,
    ) -> Result<Vec<TemplateField>, TemplateError> {
        ...
        for cap in re.captures_iter(&format!("{} {}", subject, body)) {
            ...
            if fields.iter().any(|f: &TemplateField| f.name == name) {
                continue;
            }
            let field_type = if name.contains("email") {
                FieldType::Email
...
impl SnippetService {
    ...
    pub async fn create_snippet(
        ...
        user_id: Uuid,
        input: SnippetInput,
    ) -> Result<TextSnippet, TemplateError> {
        if input.trigger.len() > 20 {
            return Err(TemplateError::InvalidSyntax(
                "Trigger must be 20 chars or less".to_string(),
            ));
        }

        let exists: bool = sqlx::query_scalar!(
            "SELECT EXISTS(SELECT 1 FROM text_snippets WHERE user_id = $1 AND trigger = $2)",
            user_id,
            input.trigger
        )
        .fetch_one(&self.pool)
        ...
        .unwrap_or(false);
        if exists {
            return Err(TemplateError::DuplicateTrigger);
        }
    ...
    pub async fn expand_trigger(
        &self,
        user_id: Uuid,
        trigger: &str,
    ) -> Result<Option<String>, TemplateError> {
        let snippet = sqlx::query!(
            "SELECT id, expansion FROM text_snippets WHERE user_id = $1 AND trigger = $2",
            user_id,
            trigger
        )
        ...
        if let Some(s) = snippet {
            sqlx::query!(
                "UPDATE text_snippets SET use_count = use_count + 1 WHERE id = $1",
                s.id
            )
            .execute(&self.pool)
            .await?;
    ...
    pub async fn delete_snippet(
        ...
        let result = sqlx::query!(
            "DELETE FROM text_snippets WHERE id = $1 AND user_id = $2",
            snippet_id,
            user_id
        )
        .execute(&self.pool)
    ...
    pub async fn get_triggers(
        ...
        let rows = sqlx::query!(
            "SELECT trigger, expansion FROM text_snippets WHERE user_id = $1",
            user_id
        )
        .fetch_all(&self.pool)
        .await?;
...
impl SignatureService {
    ...
    pub async fn create_signature(
        ...
        content_html: String,
        is_default: bool,
    ) -> Result<Signature, TemplateError> {
        let id = Uuid::new_v4();
    ...
    pub async fn get_default_signature(
        &self,
        user_id: Uuid,
    ) -> Result<Option<Signature>, TemplateError> {
