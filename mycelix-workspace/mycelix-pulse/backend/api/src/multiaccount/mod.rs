...
impl MultiAccountService {
    ...
    pub async fn add_account(
    ...
    ) -> Result<EmailAccount, AccountError> {
        // Check account limit
        ...
        let exists: bool = sqlx::query_scalar!(
            "SELECT EXISTS(SELECT 1 FROM email_accounts WHERE user_id = $1 AND email_address = $2)",
            user_id,
            input.email_address
        )
        .fetch_one(&self.pool)
        ...
        // Test connection
        self.test_connection(&input.account_type, &input.credentials)
            .await?;
    ...
    pub async fn get_account(
        ...
        let sync_settings: SyncSettings =
            serde_json::from_value(row.sync_settings).unwrap_or_default();
    ...
    pub async fn list_accounts(&self, user_id: Uuid) -> Result<Vec<EmailAccount>, AccountError> {
        ...
        Ok(rows
            ...
            .map(|r| {
                let sync_settings: SyncSettings =
                    serde_json::from_value(r.sync_settings).unwrap_or_default();
                EmailAccount {
                    id: r.id,
                    user_id: r.user_id,
    ...
    pub async fn toggle_account(
        ...
        let result = sqlx::query!(
            "UPDATE email_accounts SET is_enabled = $3 WHERE id = $1 AND user_id = $2",
            account_id,
            user_id,
            is_enabled
        )
    ...
    pub async fn update_sync_settings(
        ...
        let result = sqlx::query!(
            "UPDATE email_accounts SET sync_settings = $3 WHERE id = $1 AND user_id = $2",
            account_id,
            user_id,
            json
        )
    ...
    pub async fn delete_account(
        ...
        sqlx::query!(
            "DELETE FROM emails WHERE account_id = $1 AND user_id = $2",
            account_id,
            user_id
        )
        .execute(&self.pool)
    ...
    pub async fn get_account_stats(
        &self,
        user_id: Uuid,
    ) -> Result<Vec<AccountStats>, AccountError> {
        let rows = sqlx::query!(
        ...
        Ok(rows
            ...
            .map(|r| AccountStats {
                account_id: r.id,
                email_address: r.email_address,
                total_emails: r.total_emails.unwrap_or(0),
                unread_count: r.unread_count.unwrap_or(0),
                storage_used_bytes: r.storage_used.unwrap_or(0),
                last_sync_at: r.last_sync_at,
                sync_status: r.sync_status,
            })
            .collect())
    ...
    async fn test_connection(
        ...
        match account_type {
            ...
            AccountType::Gmail | AccountType::Outlook => {
                if credentials.oauth_token.is_none() {
                    return Err(AccountError::InvalidCredentials);
                }
                Ok(())
            }
    ...
    pub async fn mark_sync_completed(&self, account_id: Uuid) -> Result<(), AccountError> {
        sqlx::query!(
            "UPDATE email_accounts SET sync_status = 'active', last_sync_at = NOW() WHERE id = $1",
            account_id
        )
        .execute(&self.pool)
        .await?;
...
impl UnifiedInboxService {
    ...
    pub async fn get_unified_inbox(
        ...
        offset: i32,
        folder: Option<&str>,
    ) -> Result<Vec<UnifiedEmail>, AccountError> {
        let rows = sqlx::query!(
        ...
        Ok(rows
            ...
            .map(|r| UnifiedEmail {
                ...
                has_attachments: r.has_attachments.unwrap_or(false),
                folder: r.folder,
                snippet: r.snippet,
            })
            .collect())
    ...
    pub async fn get_unread_counts(
        &self,
        user_id: Uuid,
    ) -> Result<Vec<(Uuid, String, i64)>, AccountError> {
        let rows = sqlx::query!(
        ...
        Ok(rows
            .into_iter()
            .map(|r| (r.id, r.email_address, r.unread_count.unwrap_or(0)))
            .collect())
    }
...
impl SendAsService {
    ...
    pub async fn add_identity(
    ...
    ) -> Result<SendAsIdentity, AccountError> {
        // Verify account ownership
        let account_exists: bool = sqlx::query_scalar!(
            "SELECT EXISTS(SELECT 1 FROM email_accounts WHERE id = $1 AND user_id = $2)",
            account_id,
            user_id
        )
        .fetch_one(&self.pool)
        ...
        let is_default = sqlx::query_scalar!(
            "SELECT COUNT(*) = 0 FROM send_as_identities WHERE account_id = $1",
            account_id
        )
        .fetch_one(&self.pool)
        .await?
    ...
    pub async fn list_identities(
        &self,
        user_id: Uuid,
    ) -> Result<Vec<SendAsIdentity>, AccountError> {
        let rows = sqlx::query!(
    ...
    pub async fn set_default(
        ...
        sqlx::query!(
            "UPDATE send_as_identities SET is_default = false WHERE account_id = $1",
            account_id
        )
        .execute(&self.pool)
        .await?;
        sqlx::query!(
            "UPDATE send_as_identities SET is_default = true WHERE id = $1",
            identity_id
        )
        .execute(&self.pool)
        .await?;
    ...
    pub async fn get_default_identity(
        &self,
        user_id: Uuid,
        account_id: Uuid,
    ) -> Result<Option<SendAsIdentity>, AccountError> {
        let row = sqlx::query!(
            r#"SELECT i.id, i.account_id, i.email_address, i.display_name, i.signature_id, i.is_default, i.reply_to, i.created_at
               FROM send_as_identities i
               JOIN email_accounts a ON i.account_id = a.id
