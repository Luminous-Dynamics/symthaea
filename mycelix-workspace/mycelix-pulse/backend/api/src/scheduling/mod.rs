...
impl SchedulingService {
    ...
    pub async fn schedule_email(
        ...
        let draft_exists: bool = sqlx::query_scalar!(
            "SELECT EXISTS(SELECT 1 FROM drafts WHERE id = $1 AND user_id = $2)",
            draft_id,
            user_id
        )
        .fetch_one(&self.pool)
    ...
    pub async fn get_scheduled_email(
        &self,
        user_id: Uuid,
        id: Uuid,
    ) -> Result<ScheduledEmail, SchedulingError> {
    ...
    pub async fn list_scheduled(
        &self,
        user_id: Uuid,
    ) -> Result<Vec<ScheduledEmail>, SchedulingError> {
...
impl AutoResponderService {
    ...
    ) -> Result<AutoResponder, SchedulingError> {
        let id = Uuid::new_v4();
    ...
    pub async fn get(&self, user_id: Uuid, id: Uuid) -> Result<AutoResponder, SchedulingError> {
        ...
        Ok(AutoResponder {
            ...
            exclude_contacts: row.exclude_contacts,
    ...
    pub async fn list(&self, user_id: Uuid) -> Result<Vec<AutoResponder>, SchedulingError> {
        ...
        Ok(rows
            ...
            .map(|r| AutoResponder {
                ...
                exclude_contacts: r.exclude_contacts,
    ...
    pub async fn toggle(
        ...
        let result = sqlx::query!(
            "UPDATE auto_responders SET is_active = $3 WHERE id = $1 AND user_id = $2",
            id,
            user_id,
            is_active
        )
    ...
    pub async fn should_respond(
        ...
        user_id: Uuid,
        sender_email: &str,
    ) -> Result<Option<AutoResponder>, SchedulingError> {
        ...
        for r in responders {
            ...
            if r.exclude_domains.iter().any(|d| d == domain) {
                continue;
            }

            if r.exclude_contacts {
                let is_contact: bool = sqlx::query_scalar!(
                    "SELECT EXISTS(SELECT 1 FROM contacts WHERE user_id = $1 AND email = $2)",
                    user_id,
                    sender_email
                )
                .fetch_one(&self.pool)
    ...
    pub async fn log_response(
        ...
        sqlx::query!("INSERT INTO auto_response_log (user_id, responder_id, sender_email, responded_at) VALUES ($1, $2, $3, NOW())", user_id, responder_id, sender_email)
            .execute(&self.pool).await?;
        sqlx::query!(
            "UPDATE auto_responders SET response_count = response_count + 1 WHERE id = $1",
            responder_id
        )
        .execute(&self.pool)
        .await?;
    ...
    pub async fn delete(&self, user_id: Uuid, id: Uuid) -> Result<(), SchedulingError> {
        let result = sqlx::query!(
            "DELETE FROM auto_responders WHERE id = $1 AND user_id = $2",
            id,
            user_id
        )
        .execute(&self.pool)
...
impl RulesEngine {
    ...
    pub async fn create_rule(
        ...
        priority: i32,
    ) -> Result<EmailRule, SchedulingError> {
        let count: i64 = sqlx::query_scalar!(
            "SELECT COUNT(*) FROM email_rules WHERE user_id = $1",
            user_id
        )
        .fetch_one(&self.pool)
        .await?
        .unwrap_or(0);
        if count >= 100 {
            return Err(SchedulingError::RuleLimitExceeded);
        }

        let id = Uuid::new_v4();
        let conditions_json = serde_json::to_value(&conditions)
            .map_err(|e| SchedulingError::InvalidCondition(e.to_string()))?;
    ...
    pub async fn get_rule(&self, user_id: Uuid, id: Uuid) -> Result<EmailRule, SchedulingError> {
        ...
        let conditions: Vec<RuleCondition> =
            serde_json::from_value(row.conditions).unwrap_or_default();
        let actions: Vec<RuleAction> = serde_json::from_value(row.actions).unwrap_or_default();
        let condition_logic = if row.condition_logic == "Or" {
            ConditionLogic::Or
        } else {
            ConditionLogic::And
        };
    ...
    pub async fn list_rules(&self, user_id: Uuid) -> Result<Vec<EmailRule>, SchedulingError> {
        ...
        Ok(rows
            ...
            .map(|r| {
                let conditions: Vec<RuleCondition> =
                    serde_json::from_value(r.conditions).unwrap_or_default();
                let actions: Vec<RuleAction> =
                    serde_json::from_value(r.actions).unwrap_or_default();
                let condition_logic = if r.condition_logic == "Or" {
                    ConditionLogic::Or
                } else {
                    ConditionLogic::And
                };
    ...
    pub async fn apply_rules(
        ...
        user_id: Uuid,
        email: &EmailForRules,
    ) -> Result<Vec<RuleAction>, SchedulingError> {
        ...
        for rule in rules {
            ...
            if self.matches_rule(&rule, email) {
                sqlx::query!(
                    "UPDATE email_rules SET match_count = match_count + 1 WHERE id = $1",
                    rule.id
                )
                .execute(&self.pool)
                .await?;
                if rule
                    .actions
                    .iter()
                    .any(|a| matches!(a.action_type, ActionType::Skip))
                {
                    break;
                }
    ...
    fn evaluate_condition(&self, condition: &RuleCondition, email: &EmailForRules) -> bool {
        let value = match &condition.field {
            ...
            ConditionField::HasAttachment => {
                return email.has_attachments == (condition.value == "true");
            }
    ...
    pub async fn toggle_rule(
        ...
        let result = sqlx::query!(
            "UPDATE email_rules SET is_active = $3 WHERE id = $1 AND user_id = $2",
            id,
            user_id,
            is_active
        )
...
impl ReminderService {
    ...
    pub async fn create(
        ...
        note: Option<String>,
    ) -> Result<FollowUpReminder, SchedulingError> {
        let id = Uuid::new_v4();
    ...
    pub async fn complete(&self, user_id: Uuid, id: Uuid) -> Result<(), SchedulingError> {
        let result = sqlx::query!(
            "UPDATE follow_up_reminders SET is_completed = true WHERE id = $1 AND user_id = $2",
            id,
            user_id
        )
        .execute(&self.pool)
    ...
    pub async fn snooze(
        ...
        let result = sqlx::query!(
            "UPDATE follow_up_reminders SET remind_at = $3 WHERE id = $1 AND user_id = $2",
            id,
            user_id,
            new_time
        )
