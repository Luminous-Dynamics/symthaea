...
pub fn provide_keyboard_context() {
    ...
    let closure =
        Closure::<dyn Fn(web_sys::KeyboardEvent)>::new(move |ev: web_sys::KeyboardEvent| {
            ...
            if email_count == 0 && ev.key() != "/" && ev.key() != "Escape" {
                return;
            }
            ...
            match ev.key().as_str() {
                "j" => {
                    ...
                    state.focused_index.update(|idx| {
                        *idx = Some(
                            idx.map(|i| (i + 1).min(email_count.saturating_sub(1)))
                                .unwrap_or(0),
                        );
                    });
                }
                ...
                "r" => {
                    ...
                    if let Some(idx) = state.focused_index.get_untracked() {
                        ...
                        if let Some(email) = emails.get(idx) {
                            mail.compose_mode
                                .set(mail_leptos_types::ComposeMode::Reply {
                ...
                "f" => {
                    ...
                    if let Some(idx) = state.focused_index.get_untracked() {
                        ...
                        if let Some(email) = emails.get(idx) {
                            mail.compose_mode
                                .set(mail_leptos_types::ComposeMode::Forward {
                                    subject: format!(
                                        "Fwd: {}",
                ...
                "n" | "p" => {
                    ...
                    state.focused_index.update(|idx| {
                        ...
                        let next = (current + delta)
                            .max(0)
                            .min(email_count.saturating_sub(1) as i32)
                            as usize;
                        *idx = Some(next);
                    });
                }
