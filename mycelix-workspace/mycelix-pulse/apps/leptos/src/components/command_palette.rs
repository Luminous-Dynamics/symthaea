...
fn default_actions() -> Vec<PaletteAction> {
    vec![
        ...
        PaletteAction {
            id: "density-compact".into(),
            label: "Compact Density".into(),
            shortcut: "".into(),
            category: "Appearance".into(),
            icon: "\u{2B1C}".into(),
            pinned: false,
        ...
        PaletteAction {
            id: "density-default".into(),
            label: "Default Density".into(),
            shortcut: "".into(),
            category: "Appearance".into(),
            icon: "\u{2B1C}".into(),
            pinned: false,
        ...
        PaletteAction {
            id: "density-comfortable".into(),
            label: "Comfortable Density".into(),
            shortcut: "".into(),
            category: "Appearance".into(),
            icon: "\u{2B1C}".into(),
            pinned: false,
        ...
        PaletteAction {
            id: "new-event".into(),
            label: "Create Calendar Event".into(),
            shortcut: "".into(),
            category: "Actions".into(),
            icon: "\u{1F4C5}".into(),
            pinned: false,
        ...
        PaletteAction {
            id: "change-shortcut".into(),
            label: "Change Palette Shortcut".into(),
            shortcut: "".into(),
...
pub fn CommandPalette() -> impl IntoView {
    ...
    let filtered = move || {
        ...
        if q.is_empty() {
            ...
            all.into_iter()
                .filter(|a| {
                    a.label.to_lowercase().contains(&q) || a.category.to_lowercase().contains(&q)
                })
                .collect()
        }
    };
    ...
    let execute = move |id: &str| {
        ...
        match id {
            ...
            "density-compact" => {
                crate::theme::use_theme()
                    .density
                    .set(mail_leptos_types::Density::Compact);
            }
            "density-default" => {
                crate::theme::use_theme()
                    .density
                    .set(mail_leptos_types::Density::Default);
            }
            "density-comfortable" => {
                crate::theme::use_theme()
                    .density
                    .set(mail_leptos_types::Density::Comfortable);
            }
            "mark-all-read" => {
                let mail = use_mail();
    ...
    let on_keydown = move |ev: web_sys::KeyboardEvent| {
        ...
        match ev.key().as_str() {
            "ArrowDown" => {
                ev.prevent_default();
                selected_idx.update(|i| *i = (*i + 1).min(items.len().saturating_sub(1)));
            }
            "ArrowUp" => {
                ev.prevent_default();
                selected_idx.update(|i| *i = i.saturating_sub(1));
            }
            "Enter" => {
