...
pub fn EventForm(
    ...
    let source = RwSignal::new(
        editing
            ...
            .unwrap_or(CalendarSource::Personal),
    );
    ...
    let start_str = RwSignal::new(
        editing
            .as_ref()
            .map(|e| ts_to_datetime_local(e.start_time))
            .unwrap_or(default_start),
    );
    let end_str = RwSignal::new(
        editing
            .as_ref()
            .map(|e| ts_to_datetime_local(e.end_time))
    ...
    let on_submit = move |_| {
        ...
        let event = CalendarEventView {
            id: editing
                .as_ref()
                .map(|e| e.id.clone())
                .unwrap_or_else(|| format!("cal-{}", js_sys::Date::now() as u64)),
            title: t,
            description: {
