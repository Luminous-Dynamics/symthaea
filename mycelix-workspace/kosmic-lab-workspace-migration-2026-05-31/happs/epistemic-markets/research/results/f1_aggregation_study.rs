...
// Commercial licensing: see COMMERCIAL_LICENSE.md at repository root//! F1 Aggregation Effectiveness Study Runner
//!
...
use crate::{
    aggregation_study::{
        ...
        QuestionType, StudyCondition, aggregate_predictions,
    },
...
impl F1AggregationStudy {
        ...
        for &agent_count in &self.config.agent_counts.clone() {
            for &diversity in &self.config.diversity_levels.clone() {
                for &correlation in &self.config.correlation_levels.clone() {
                    ...
                    // Run this condition
                    let condition_result = self.run_condition(agent_count, diversity, correlation);
    ...
    fn run_condition(
        ...
        for _ in 0..self.config.iterations_per_condition {
            ...
            for _ in 0..self.config.questions_per_iteration {
                ...
                for method in &self.config.aggregation_methods {
                    ...
                    method_briers.get_mut(&method_name).unwrap().push(agg_brier);
        ...
        for method in &self.config.aggregation_methods {
            ...
            let variance = briers.iter().map(|b| (b - mean_brier).powi(2)).sum::<f64>()
    ...
    fn find_min_effective_agents(&self, results: &[ConditionResult]) -> usize {
        ...
        for &count in &self.config.agent_counts {
            let relevant: Vec<_> = results.iter().filter(|r| r.agent_count == count).collect();

            let success_rate = relevant
                .iter()
                .filter(|r| r.method_results.values().any(|m| m.wisdom_ratio > 1.0))
                .count() as f64
                / relevant.len() as f64;
    ...
    fn analyze_expert_comparison(
        ...
        let best_method_beats = results
            .iter()
            .filter(|r| r.beats_expert.get(best_method).copied().unwrap_or(false))
            .count();
...
fn format_timestamp(millis: u64) -> String {
    ...
    let seconds = secs % 60;
    format!("2026-01-30T{:02}:{:02}:{:02}Z", hours, minutes, seconds)
}
