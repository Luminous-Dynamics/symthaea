...
// Commercial licensing: see COMMERCIAL_LICENSE.md at repository root//! F1 Aggregation Study Runner Binary
//!
...
use f1_aggregation_study::{F1AggregationStudy, F1StudyConfig, F1StudyResults};
...
fn print_summary(results: &F1StudyResults) {
    ...
    println!(
        "  High diversity: {:?}",
        summary.method_rankings.by_high_diversity
    );
    println!(
        "  Low diversity: {:?}",
        summary.method_rankings.by_low_diversity
    );
    println!(
        "  Large crowds: {:?}",
        summary.method_rankings.by_large_crowd
    );
    println!(
        "  Small crowds: {:?}",
        summary.method_rankings.by_small_crowd
    );
}
