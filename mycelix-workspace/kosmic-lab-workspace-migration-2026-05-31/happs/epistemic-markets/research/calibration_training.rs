...
// Commercial licensing: see COMMERCIAL_LICENSE.md at repository root//! F2: Calibration Training Research
//!
...
use crate::metrics::{
    BrierScore, ExpectedCalibrationError, PredictorDomainPerformance, TimeSeriesMetrics,
    TransferAnalysis,
};
use crate::simulation::{AgentType, SimpleRng, SimulatedAgent};
...
impl TrainingParticipant {
    ...
    fn generate_feedback(
        ...
        match &self.training_method {
            ...
            TrainingMethod::ImmediateFeedback => Some(TrainingFeedback {
                ...
                message: format!("Your Brier score for this prediction: {:.3}", brier),
            ...
            TrainingMethod::Combined(methods) => {
                ...
                for method in methods {
                    ...
                    if let Some(fb) = temp_participant
                        .generate_feedback(step, predicted, outcome, brier, domain, rng)
                    {
    ...
    fn calculate_recent_brier(&self, count: usize) -> f64 {
        let recent: Vec<&TrainingPrediction> =
            self.training_predictions.iter().rev().take(count).collect();
...
impl CalibrationTrainingExperiment {
    ...
    pub fn run_followups(&mut self) {
        for &followup_time in &self.config.followup_assessments.clone() {
            ...
            for participant in &mut self.participants {
                ...
                participant.current_ece =
                    (participant.current_ece + decay).min(participant.initial_ece);
            }
    ...
    fn generate_recommendations(&self) -> Vec<String> {
        ...
        if let Some((best_method, effectiveness)) =
            self.results
                .method_effectiveness
                .iter()
                .max_by(|(_, a), (_, b)| {
        ...
        if !self.results.transfer_analyses.is_empty() {
            let best_transfer = self.results.transfer_analyses.iter().max_by(|a, b| {
