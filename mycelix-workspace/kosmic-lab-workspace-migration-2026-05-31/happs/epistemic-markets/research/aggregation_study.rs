...
// Commercial licensing: see COMMERCIAL_LICENSE.md at repository root//! F1: Aggregation Effectiveness Research
//!
...
use crate::simulation::{
    AgentPopulationConfig, AgentType, SimpleRng, SimulatedAgent, SimulatedMarket, SimulationConfig,
    SimulationEngine,
};
...
pub fn aggregate_predictions(
    ...
    match method {
        ...
        AggregationMethod::GeometricMean => {
            let epsilon = 1e-10;
            let log_sum: f64 = predictions.iter().map(|(p, _)| p.max(epsilon).ln()).sum();
...
impl AggregationStudy {
            ...
            for &diversity in &self.config.diversity_levels.clone() {
                for &independence in &self.config.independence_levels.clone() {
                    for question_type in &self.config.question_types.clone() {
                        self.run_condition(agent_count, diversity, independence, *question_type);
                    }
                }
            }
        }
    ...
    fn run_condition(
        ...
        let condition =
            StudyCondition::from_floats(agent_count, diversity, independence, question_type);
        ...
        if !all_individual_briers.is_empty() {
            let mean =
                all_individual_briers.iter().sum::<f64>() / all_individual_briers.len() as f64;
    ...
    fn generate_agents(&mut self, count: usize, diversity: f64) -> Vec<SimulatedAgent> {
        ...
        let type_probs = if diversity < 0.3 {
            ...
            vec![(AgentType::Noise, 0.8), (AgentType::Informed, 0.2)]
        ...
        for i in 0..count {
            ...
            let info_quality =
                base_info + (self.rng.next_u64() as f64 / u64::MAX as f64 - 0.5) * info_var * 2.0;
    ...
    fn collect_predictions(
        ...
        for agent in agents {
            ...
            let private_signal =
                if (self.rng.next_u64() as f64 / u64::MAX as f64) < agent.information_quality {
            ...
            let base_estimate =
                independence * private_signal + (1.0 - independence) * common_belief;
...
mod tests {
    ...
    fn test_extremized_aggregation() {
        ...
        let result =
            aggregate_predictions(&predictions, AggregationMethod::Extremized { factor: 2 });
