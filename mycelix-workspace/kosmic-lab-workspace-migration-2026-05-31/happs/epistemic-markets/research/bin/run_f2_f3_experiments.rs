...
// Commercial licensing: see COMMERCIAL_LICENSE.md at repository root//! F2 and F3 Research Experiment Runner
//!
//! Runs:
...
use epistemic_markets_research::{
    ...
    TrainingMethod, WisdomSeedFormat, WisdomStudyConfig, WisdomStudyResults,
    WisdomTransmissionStudy,
};
...
fn main() {
    ...
    println!(
        ...
        f2_results.summary.overall_improvement_rate * 100.0
    );
    println!(
        "  Calibration Trainable: {}",
        if f2_results.summary.calibration_is_trainable {
            "YES"
        } else {
            "NO"
        }
    ...
    println!(
        "  Transmission Rate: {:.1}%",
        f3_results.summary.transmission_rate * 100.0
    );
    println!(
        "  Wisdom Transmissible: {}",
        if f3_results.summary.wisdom_is_transmissible {
            "YES"
        } else {
            "NO"
        }
...
fn generate_f2_report(results: &TrainingExperimentResults) -> String {
    ...
    report
        .push_str("**What training methods most effectively improve forecaster calibration?**\n\n");
    ...
    for (method, improvements) in &results.improvements_by_method {
        ...
        let avg_initial =
            improvements.iter().map(|i| i.initial_brier).sum::<f64>() / improvements.len() as f64;
        let avg_final =
            improvements.iter().map(|i| i.final_brier).sum::<f64>() / improvements.len() as f64;
    ...
    report.push_str(
        "This suggests that without ongoing practice, calibration improvements decay over time, ",
    );
    ...
    if !results.transfer_analyses.is_empty() {
        report
            .push_str("| Source Domain | Target Domain | Transfer Coef. | Positive Transfer? |\n");
        report
            .push_str("|---------------|---------------|----------------|--------------------|\n");
    ...
    if results.summary.calibration_is_trainable {
        report.push_str(
            "**Calibration IS trainable.** More than 50% of participants showed improvement.\n\n",
        );
    } else {
    ...
    if results.summary.transfer_coefficient_avg > 0.3 {
        ...
        report.push_str(
            "**Moderate transfer** observed. Some domain-specific training may be beneficial.\n\n",
        );
    } else {
        report.push_str(
...
fn generate_f3_report(results: &WisdomStudyResults) -> String {
    ...
    report.push_str(
        "This indicates the proportion of seed views that resulted in improved predictions. ",
    );
    if results.summary.transmission_rate > 0.5 {
        ...
        report.push_str(
            "A rate below 50% suggests room for improvement in seed design or presentation.\n\n",
        );
    }
    ...
    let best_eff = results
        ...
        .get(&results.summary.best_format);
    report.push_str(&format!(
    ...
    if avg_citation_rate > 0.3 {
        ...
        report
            .push_str("**Moderate engagement** with seeds. Some predictors find them useful.\n\n");
    } else {
        report.push_str(
            "**Low citation rate**. Consider prompting users to engage more with seeds.\n\n",
        );
    }
    ...
    if results.summary.network_depth > 2 {
        report.push_str(
            "**Multi-generational propagation** observed. Wisdom spreads through the network.\n\n",
        );
    } else {
        report
            .push_str("**Limited propagation depth**. Seeds mostly influence direct viewers.\n\n");
    }
    ...
    report
        .push_str("- Seed influence was measured by comparing pre/post-seed prediction accuracy\n");
...
fn create_f2_json(results: &TrainingExperimentResults) -> String {
    ...
    let method_effectiveness: Map<String, Value> = results
    ...
    let improvements_by_method: Map<String, Value> = results
        .improvements_by_method
        .iter()
        .map(|(method, improvements)| {
            let imps: Vec<Value> = improvements
                .iter()
                .map(|i| {
                    json!({
    ...
    let persistence_data: Map<String, Value> = results
        .persistence_data
        .iter()
        .map(|(method, data)| (format!("{:?}", method), json!(data)))
        .collect();

    // Convert snapshots
    let snapshots: Vec<Value> = results
        .snapshots
        .iter()
        .map(|s| {
            let method_ece: Map<String, Value> = s
                .method_avg_ece
                .iter()
    ...
    let transfer_analyses: Vec<Value> = results
        .transfer_analyses
        .iter()
        .map(|t| {
            json!({
    ...
    let retention: Map<String, Value> = results
        .summary
        .avg_retention_at_followup
        .iter()
...
fn create_f3_json(results: &WisdomStudyResults) -> String {
    ...
    let format_effectiveness: Map<String, Value> = results
        .format_effectiveness
        .iter()
        .map(|(format, eff)| {
            (
