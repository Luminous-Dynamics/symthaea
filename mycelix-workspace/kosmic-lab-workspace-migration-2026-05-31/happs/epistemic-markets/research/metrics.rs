...
// Commercial licensing: see COMMERCIAL_LICENSE.md at repository root//! Common Research Metrics for Epistemic Markets
//!
...
impl BrierScore {
    ...
    pub fn calculate(predictions: &[(f64, f64)]) -> Self {
        ...
        let sum_squared_error: f64 = predictions.iter().map(|(p, o)| (p - o).powi(2)).sum();
...
mod tests {
    ...
    fn test_ece_perfect_calibration() {
        ...
        let predictions = vec![(0.1, false), (0.1, false), (0.9, true), (0.9, true)];
        let ece = ExpectedCalibrationError::calculate(&predictions, 10);
    ...
    fn test_statistical_comparison() {
        ...
        let comparison =
            StatisticalComparison::compare_brier_scores(&predictions_a, &predictions_b);
