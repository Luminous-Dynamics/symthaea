...
// Commercial licensing: see COMMERCIAL_LICENSE.md at repository root//! F4: Mechanism Design Research
//!
...
impl MechanismComparisonStudy {
    ...
    fn generate_summary(&self) -> MechanismSummary {
        ...
        let lmsr_best = lmsr_metrics.iter().max_by(|(_, a), (_, b)| {
            a.overall_score()
                .partial_cmp(&b.overall_score())
                .unwrap_or(std::cmp::Ordering::Equal)
        });
    ...
    fn generate_recommendations(&self) -> Vec<String> {
        ...
        if !low_liq_metrics.is_empty() {
            let avg_slippage: f64 = low_liq_metrics
                .iter()
                .map(|(_, m)| m.avg_slippage)
                .sum::<f64>()
        ...
        if let Some(ob) = self.results.mechanism_metrics.get("OrderBook") {
            if ob.failed_trades > 0 {
                let failure_rate =
                    ob.failed_trades as f64 / (ob.total_trades + ob.failed_trades) as f64 * 100.0;
                recommendations.push(format!(
