...
// Commercial licensing: see COMMERCIAL_LICENSE.md at repository root//! Simulation Framework for Epistemic Markets Research
//!
...
impl SimulatedMarket {
    ...
    pub fn add_information_event(
        ...
        self.true_probability = (self.true_probability + impact * direction).clamp(0.01, 0.99);
    }
        ...
        for market in &self.markets {
            if let Some(outcome) = market.outcome {
                for pred in &market.predictions {
                    self.results
                        .all_predictions
                        .push((pred.predicted_probability, if outcome { 1.0 } else { 0.0 }));
                }
            }
        }
...
impl ABTestAnalysis {
    ...
    pub fn analyze(results: &SimulationResults) -> Self {
        ...
        for (name, predictions) in &results.group_predictions {
            ...
            let brier_scores: Vec<f64> = predictions.iter().map(|(p, o)| (p - o).powi(2)).collect();
...
mod tests {
    ...
    fn test_agent_creation() {
        let agent = SimulatedAgent::new("test_agent".to_string(), AgentType::Informed, 0.1, 0.8);
