...
// Commercial licensing: see COMMERCIAL_LICENSE.md at repository root//! F5: Anti-Manipulation Research
//!
...
impl ManipulationSimulation {
    ...
    fn simulate_attack(&mut self, attack: &ManipulationAttack) {
        match attack {
            ...
            ManipulationAttack::Collusion { group_size } => self.simulate_collusion(*group_size),
    ...
    fn simulate_gradient_poisoning(&mut self) {
        ...
        for market_idx in 0..self.config.num_markets {
            ...
            for i in 0..(self.config.trades_per_market / 20) {
                ...
                let true_direction = if self.rng.next_u64() % 2 == 0 {
                    1.0
                } else {
                    -1.0
                };
    ...
    fn simulate_honest_traders(&mut self) {
        for market_idx in 0..self.config.num_markets {
            ...
            for i in 0..self.config.trades_per_market {
                ...
                let random_component = if self.rng.next_u64() % 2 == 0 {
                    1.0
                } else {
                    -1.0
                };
    ...
    fn run_detection(&self, method: DetectionMethod) -> DetectionResults {
        ...
        let accuracy = (true_positives + true_negatives) as f64 / predictions.len().max(1) as f64;
...
pub fn run_sybil_test(config: &SybilTestConfig) -> SybilTestResults {
    ...
    for &count in &config.sybil_counts {
        for &cost in &config.sybil_costs {
            ...
            results
                .success_rates
                .insert((count, cost_key), success_rate);
...
mod tests {
    ...
    fn test_attack_types() {
        ...
        for attack in attacks {
            assert_ne!(
                attack,
                ManipulationAttack::PriceManipulation { target_price: 0 }
            );
        }
    }
}
