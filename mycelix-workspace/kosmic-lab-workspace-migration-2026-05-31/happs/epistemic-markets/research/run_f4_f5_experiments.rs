...
// Commercial licensing: see COMMERCIAL_LICENSE.md at repository root//! F4 and F5 Research Experiment Runner
//!
...
impl Default for F4ExperimentConfig {
    fn default() -> Self {
        Self {
            liquidity_conditions: vec![("low", 10), ("medium", 100), ("high", 1000)],
...
fn run_f4_experiment(config: &F4ExperimentConfig) -> F4Results {
    ...
    for (liquidity_name, num_participants) in &config.liquidity_conditions {
        for &info_pattern in &config.information_patterns {
            ...
            // Run LMSR simulation
            let lmsr_perf =
                run_lmsr_simulation(*num_participants, lmsr_liquidity, info_rate, config.seed);

            // Run Order Book simulation
            let ob_perf = run_orderbook_simulation(*num_participants, info_rate, config.seed);
...
fn run_lmsr_simulation(
    ...
    let mut market = LMSRMarket::new("lmsr_test".to_string(), liquidity, 2, 10000.0);
    ...
    for step in 0..num_trades {
        ...
        let direction = if trader_signal > current_price {
            1.0
        } else {
            -1.0
        };
        ...
        let result = market.trade(
            ...
            &format!("trader_{}", step % num_traders),
        );
...
fn run_orderbook_simulation(num_traders: usize, info_rate: f64, seed: u64) -> MechanismPerformance {
...
fn generate_f4_summary(conditions: &[ConditionResults]) -> F4Summary {
    ...
    let best_overall = if lmsr_wins > ob_wins {
        "LMSR"
    } else {
        "OrderBook"
    };
    ...
    key_findings.push(format!(
        "OrderBook won {}/{} conditions",
        ob_wins,
        conditions.len()
    ));

    // Analyze patterns
    let low_liq: Vec<_> = conditions
        .iter()
        .filter(|c| c.condition_name.contains("low"))
        .collect();
    if !low_liq.is_empty() {
        let avg_ob_success: f64 = low_liq
            .iter()
            .map(|c| c.orderbook_metrics.trade_success_rate)
            .sum::<f64>()
            / low_liq.len() as f64;
        key_findings.push(format!(
            "Order book trade success rate in low liquidity: {:.1}%",
            avg_ob_success * 100.0
        ));
    }

    let high_liq: Vec<_> = conditions
        .iter()
        .filter(|c| c.condition_name.contains("high"))
        .collect();
    if !high_liq.is_empty() {
        let avg_ob_spread: f64 = high_liq
            .iter()
            .map(|c| c.orderbook_metrics.spread)
            .sum::<f64>()
            / high_liq.len() as f64;
        key_findings.push(format!(
            "Order book average spread in high liquidity: {:.4}",
            avg_ob_spread
        ));
    }
...
fn generate_f4_recommendations(
    ...
    if summary.best_for_low_liquidity == "LMSR" {
        recommendations.push(
            "Use LMSR for thin/new markets where liquidity providers may be scarce".to_string(),
        );
    }

    if summary.best_for_high_liquidity == "OrderBook" {
        recommendations
            .push("Consider order book for mature markets with active market makers".to_string());
    }
    ...
    let avg_ob_failure: f64 = conditions
        .iter()
    ...
    recommendations.push(
        "Implement hybrid approach: start with LMSR, transition to order book as liquidity grows"
            .to_string(),
    );
...
fn run_f5_experiment(config: &F5ExperimentConfig) -> F5Results {
    ...
    for (attack_name, attack) in &attack_types {
        ...
        for &intensity in &config.attack_intensities {
            ...
            let best_detection = sim.detections.iter().max_by(|a, b| {
                a.1.recall
        ...
        let avg_detection: f64 = intensities
            .iter()
            .map(|i| i.true_positive_rate)
            .sum::<f64>()
            / intensities.len() as f64;
        let avg_success: f64 = intensities
            .iter()
...
fn generate_detection_comparison(
    ...
    for (method, data) in scores {
        ...
        let worst_against: Vec<String> = sorted
            .iter()
            .rev()
            .take(2)
            .map(|(a, _)| a.clone())
...
fn generate_f5_summary(
    ...
    let most_dangerous = attack_results
        .iter()
        .max_by(|a, b| {
    ...
    let most_effective = detection_comparison
        .first()
        .map(|d| d.method_name.clone())
        .unwrap_or_else(|| "Ensemble".to_string());

    let avg_detection: f64 = attack_results
        .iter()
        .map(|a| a.avg_detection_rate)
        .sum::<f64>()
        / attack_results.len().max(1) as f64;

    let avg_fpr: f64 = detection_comparison
        .iter()
        .map(|d| d.overall_fpr)
        .sum::<f64>()
    ...
    let attacks_requiring_attention: Vec<String> = attack_results
        .iter()
...
fn generate_security_recommendations(
    ...
    for attack in attack_results {
        if attack.avg_detection_rate < 0.6 {
            let rec = match attack.attack_type.as_str() {
                ...
                "SybilAttack" => {
                    "Strengthen identity verification and implement stake-based reputation"
                }
                ...
                "GradientPoisoning" => {
                    "Use Byzantine-robust aggregation for federated learning updates"
                }
    ...
    recommendations
        .push("Implement layered defense: combine multiple detection methods".to_string());
    recommendations
        .push("Establish monitoring dashboards for real-time manipulation detection".to_string());
...
fn generate_f4_report(results: &F4Results) -> String {
    ...
    report.push_str(&format!(
        "**Best for Volatile Information:** {}\n",
        results.summary.best_for_volatile_info
    ));
    report.push_str(&format!(
        "**Optimal LMSR Liquidity Parameter:** {:.0}\n\n",
    ...
    for cond in &results.conditions {
        ...
        report.push_str("|--------|------|------------|\n");
        report.push_str(&format!(
            "| Price Discovery Speed | {:.3} | {:.3} |\n",
            cond.lmsr_metrics.price_discovery_speed, cond.orderbook_metrics.price_discovery_speed
        ));
        report.push_str(&format!(
            "| Avg Slippage | {:.4} | {:.4} |\n",
            cond.lmsr_metrics.avg_slippage, cond.orderbook_metrics.avg_slippage
        ));
        report.push_str(&format!(
            "| Max Slippage | {:.4} | {:.4} |\n",
            cond.lmsr_metrics.max_slippage, cond.orderbook_metrics.max_slippage
        ));
        report.push_str(&format!(
            "| Spread | N/A | {:.4} |\n",
...
fn generate_f5_report(results: &F5Results) -> String {
    ...
    for attack in &results.attack_results {
        ...
        report.push_str(&format!(
            "**Hardest Intensity to Detect:** {:.1}\n",
            attack.hardest_intensity_to_detect
        ));
        report.push_str(&format!(
            "**Avg Success if Undetected:** {:.1}%\n\n",
...
fn main() {
    ...
    println!(
        "F4 Best Overall Mechanism: {}",
        f4_results.summary.best_mechanism_overall
    );
    println!(
        "F4 completed with {} conditions tested\n",
    ...
    println!(
        "F5 Most Dangerous Attack: {}",
        f5_results.summary.most_dangerous_attack
    );
    println!(
        "F5 Most Effective Detection: {}",
        f5_results.summary.most_effective_detection
    );
    println!(
        "F5 completed with {} attack types tested\n",
        f5_results.attack_results.len()
    );

    // Write output files
    let results_dir =
        "/srv/luminous-dynamics/mycelix-workspace/happs/epistemic-markets/research/results";

    fs::write(
        format!("{}/f4_mechanism_results.json", results_dir),
        &f4_json,
    )
    .expect("Failed to write F4 JSON results");
    fs::write(
        format!("{}/f4_mechanism_report.md", results_dir),
        &f4_report,
    )
    .expect("Failed to write F4 report");

    fs::write(
        format!("{}/f5_manipulation_results.json", results_dir),
        &f5_json,
    )
    .expect("Failed to write F5 JSON results");
    fs::write(
        format!("{}/f5_manipulation_report.md", results_dir),
        &f5_report,
    )
