...
// Commercial licensing: see COMMERCIAL_LICENSE.md at repository root//! Scoring Zome - Calibration, Brier Scores, and Collective Intelligence Metrics
//!
...
pub fn analyze_cascade(market_id: EntryHash) -> ExternResult<CascadeAnalysis> {
    ...
    .map_err(|e| {
        wasm_error!(WasmErrorInner::Guest(format!(
            "Failed to fetch information events: {:?}",
            e
        )))
    })?;
