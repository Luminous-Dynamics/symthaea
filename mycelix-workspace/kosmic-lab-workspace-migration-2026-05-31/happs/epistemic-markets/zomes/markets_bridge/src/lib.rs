...
// Commercial licensing: see COMMERCIAL_LICENSE.md at repository root//! Markets Bridge - Cross-hApp Prediction Market Integration
//!
...
pub fn submit_prediction_request(input: SubmitRequestInput) -> ExternResult<EntryHash> {
    ...
    let request = PredictionRequest {
        ...
        expires_at: input
            .expires_at
            .unwrap_or(now + 7 * 24 * 60 * 60 * 1_000_000),
    ...
    for subscription in subscriptions {
        ...
        let outcome = match &subscription.outcome_mapping {
            OutcomeMapping::Direct { field_path } => {
                extract_field_value(&event, field_path).unwrap_or_else(|| "unknown".to_string())
            }
            OutcomeMapping::Conditional {
                conditions,
                default_outcome,
            } => {
            ...
            OutcomeMapping::Threshold {
                ...
                if let Some(value) =
                    extract_field_value(&event, field_path).and_then(|v| v.parse::<f64>().ok())
                {
                    if value >= *threshold {
                        above_outcome.clone()
                    } else {
        ...
        let _result: Result<EntryHash, _> = match response {
            ZomeCallResponse::Ok(extern_io) => extern_io.decode().map_err(|e| {
                wasm_error!(WasmErrorInner::Guest(format!(
                    "Failed to decode resolution result: {:?}",
                    e
                )))
            }),
            _ => Err(wasm_error!(WasmErrorInner::Guest(
                "Resolution call failed".into()
            ))),
        };
        ...
        if let Some(hash) = link.target.into_entry_hash() {
            if let Some(record) = get(hash, GetOptions::default())? {
                if let Some(sub) = record
                    .entry()
                    .to_app_option::<EventSubscription>()
                    .ok()
                    .flatten()
                {
                    if sub.active {
...
pub fn list_pending_requests(_: ()) -> ExternResult<Vec<PredictionRequest>> {
    let anchor = anchor_hash("pending_requests")?;
    let links = get_links(
        LinkQuery::try_new(anchor, LinkTypes::AnchorToRequest)?,
        GetStrategy::default(),
    )?;
