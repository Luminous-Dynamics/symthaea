...
// Commercial licensing: see COMMERCIAL_LICENSE.md at repository root//! Predictions Zome - Foundations for Long-Term Epistemic Infrastructure
//!
...
pub enum TemporalCommitment {
    ...
    Vesting { schedule: Vec<VestingMilestone> },
...
pub enum SuccessorPolicy {
    ...
    Auction { minimum_matl: f64 },
    /// Community election
    Election { eligible_voters: VoterEligibility },
    /// Automatic to mentee
    Mentorship {
        mentee: AgentPubKey,
...
pub fn submit_prediction(input: SubmitPredictionInput) -> ExternResult<EntryHash> {
    ...
    for ancestor in &prediction.epistemic_lineage {
        create_link(ancestor.clone(), hash.clone(), LinkTypes::LineageLink, ())?;
    }
...
fn calculate_vesting_status(
    ...
    for milestone in &sorted_schedule {
        if now >= milestone.timestamp {
            // Milestone completed
            let amount_unlocked =
                (total_stake as f64 * (milestone.percentage_unlocked - current_unlock_pct)) as u64;
    ...
    let early_penalty_rate = if locked_amount > 0 {
        ...
        let time_factor = if let Some(ref next) = next_milestone {
            ...
            let total_period = next
                .timestamp
                .saturating_sub(completed.last().map(|c| c.completed_at).unwrap_or(0));
            if total_period > 0 {
...
pub fn early_withdraw(input: EarlyWithdrawInput) -> ExternResult<WithdrawalResult> {
    ...
    execute_early_withdrawal(&caller, withdrawal_amount, penalty, &input.prediction_id)?;
...
#[hdk_extern]
pub fn check_wisdom_activations(
    input: CheckActivationsInput,
) -> ExternResult<Vec<WisdomActivation>> {
    let now = sys_time()?.as_micros() as u64;
        ...
        if let Some(prediction) = get_prediction(prediction_id.clone())? {
            if let Some(ref wisdom_seed) = prediction.wisdom_seed {
                ...
                if let Some(ref domain) = input.domain {
                    let matches_domain = wisdom_seed
                        .if_correct
                        .applicability
                        .domains
                        .contains(domain)
                ...
                for condition in &wisdom_seed.activation_conditions {
                    if let Some(activation) =
                        check_condition_met(condition, &prediction, wisdom_seed, now, &input)?
                    {
...
pub fn activate_wisdom_seed(prediction_id: EntryHash) -> ExternResult<ActivatedWisdom> {
    ...
    let wisdom_seed = prediction.wisdom_seed.clone().ok_or_else(|| {
        wasm_error!(WasmErrorInner::Guest(
            "Prediction has no wisdom seed".into()
        ))
    })?;
...
pub fn get_top_wisdom(input: GetTopWisdomInput) -> ExternResult<Vec<RankedWisdom>> {
    ...
    for link in links {
        if let Some(record) = get(
            ...
            if let Some(activated) = record
                .entry()
                .to_app_option::<ActivatedWisdom>()
                .ok()
                .flatten()
            {
...
fn apply_early_withdrawal_reputation_penalty(
    ...
    let penalty = ReputationPenaltyInput {
        ...
        reason: format!(
            "Early vesting withdrawal with {:.1}% penalty",
            penalty_rate * 100.0
        ),
    };
