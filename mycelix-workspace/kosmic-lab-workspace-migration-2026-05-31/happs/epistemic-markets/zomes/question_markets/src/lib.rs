...
// Commercial licensing: see COMMERCIAL_LICENSE.md at repository root//! Question Markets - Trade on What's Worth Knowing
//!
...
impl QuestionValue {
    ...
    pub fn estimate_initial(question: &str, proposer_matl: f64, initial_curiosity: u64) -> Self {
...
pub fn propose_question(input: ProposeQuestionInput) -> ExternResult<EntryHash> {
    ...
    let question_market = QuestionMarket {
        ...
        expires_at: input
            .expires_at
            .unwrap_or(now + 30 * 24 * 60 * 60 * 1_000_000), // 30 days default
...
pub fn signal_curiosity(input: SignalCuriosityInput) -> ExternResult<EntryHash> {
    ...
    if let Some(record) = get(input.question_market.clone(), GetOptions::default())? {
        if let Some(mut market) = record
            .entry()
            .to_app_option::<QuestionMarket>()
            .ok()
            .flatten()
        {
...
pub fn buy_question_shares(input: BuySharesInput) -> ExternResult<EntryHash> {
    ...
    let mut market: QuestionMarket = record
        ...
        .ok_or_else(|| {
            wasm_error!(WasmErrorInner::Guest(
                "Question market entry not found".into()
            ))
        })?;
    ...
    create_link(
        buyer,
        position_entry_hash.clone(),
        LinkTypes::HolderToPosition,
        (),
    )?;
...
pub fn list_question_markets(_: ()) -> ExternResult<Vec<QuestionMarket>> {
    let anchor = anchor_hash("all_questions")?;
    let links = get_links(
        LinkQuery::try_new(anchor, LinkTypes::AnchorToQuestion)?,
        GetStrategy::default(),
    )?;
    ...
    if should_spawn && market.spawned_market.is_none() {
        ...
        let spawned_hash: EntryHash = match response {
            ZomeCallResponse::Ok(extern_io) => extern_io.decode().map_err(|e| {
                wasm_error!(WasmErrorInner::Guest(format!(
                    "Failed to decode spawned market hash: {:?}",
                    e
                )))
            })?,
            ZomeCallResponse::Unauthorized(_, _, _, _) => {
                return Err(wasm_error!(WasmErrorInner::Guest(
                    "Unauthorized to spawn market".into()
                )));
            }
...
fn get_matl_score(agent: &AgentPubKey) -> ExternResult<f64> {
    ...
    Ok(matl_result.map(|score| score.composite).unwrap_or(0.5)) // Conservative default for unknown agents
}
