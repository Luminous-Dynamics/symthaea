...
// Commercial licensing: see COMMERCIAL_LICENSE.md at repository root//! RB-BFT Integrity Zome
//!
...
impl KVector {
    ...
    pub fn is_valid(&self) -> bool {
        is_valid_reputation(self.k_r)
            && is_valid_reputation(self.k_a)
            && is_valid_reputation(self.k_i)
            && is_valid_reputation(self.k_p)
            && is_valid_reputation(self.k_m)
            && is_valid_reputation(self.k_s)
            && is_valid_reputation(self.k_h)
            && is_valid_reputation(self.k_topo)
    }
}
...
pub fn validate(op: Op) -> ExternResult<ValidateCallbackResult> {
    match op.flattened::<EntryTypes, LinkTypes>()? {
        ...
        FlatOp::RegisterDeleteLink { link_type, .. } => validate_delete_link(link_type),
    ...
    match entry {
        EntryTypes::ValidatorRegistration(reg) => {
            ...
            if reg.kvector.k_r < MIN_VALIDATOR_REPUTATION {
                return Ok(ValidateCallbackResult::Invalid(format!(
                    "Minimum reputation {} required to register as validator",
        ...
        EntryTypes::CommittedBlock(block) => {
            ...
            if quorum < QUORUM_THRESHOLD {
                return Ok(ValidateCallbackResult::Invalid(format!(
                    "Block did not reach quorum ({:.1}% < {:.1}%)",
                    quorum * 100.0,
...
fn validate_update_entry(
    ...
    match entry {
        ...
        EntryTypes::CommittedBlock(_) => Ok(ValidateCallbackResult::Invalid(
            "Committed blocks cannot be updated".into(),
        )),
        // Challenge evidence is immutable
        EntryTypes::ChallengeEvidence(_) => Ok(ValidateCallbackResult::Invalid(
        ...
        EntryTypes::BlockProposal(_) => Ok(ValidateCallbackResult::Invalid(
