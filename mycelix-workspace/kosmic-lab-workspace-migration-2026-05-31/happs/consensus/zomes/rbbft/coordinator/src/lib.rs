...
// Commercial licensing: see COMMERCIAL_LICENSE.md at repository root//! RB-BFT Coordinator Zome
//!
...
pub fn get_proposal_votes(proposal_hash: ActionHash) -> ExternResult<Vec<Vote>> {
    ...
    for link in links {
        if let Some(target) = link.target.into_action_hash() {
            if let Some(record) = get(target, GetOptions::default())? {
                if let Some(vote) = record.entry().to_app_option::<Vote>().map_err(|e| {
                    wasm_error!(WasmErrorInner::Guest(format!("Deserialize error: {:?}", e)))
                })? {
                    votes.push(vote);
                }
...
pub fn calculate_consensus(proposal_hash: ActionHash) -> ExternResult<ConsensusResult> {
    ...
    let quorum_reached =
        weighted_total > 0.0 && (weighted_for / weighted_total) >= QUORUM_THRESHOLD;
...
pub fn update_kvector(kvector: KVector) -> ExternResult<ActionHash> {
    ...
    let original = get(original_hash.clone(), GetOptions::default())?
        .ok_or_else(|| wasm_error!(WasmErrorInner::Guest("Registration not found".into())))?;

    let mut reg = original
        .entry()
        .to_app_option::<ValidatorRegistration>()
        .map_err(|e| wasm_error!(WasmErrorInner::Guest(format!("Deserialize error: {:?}", e))))?
        .ok_or_else(|| wasm_error!(WasmErrorInner::Guest("Invalid registration entry".into())))?;
