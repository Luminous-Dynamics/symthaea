...
// Commercial licensing: see COMMERCIAL_LICENSE.md at repository root//! Collective Sensemaking Coordinator Zome
//!
...
pub fn share_belief(input: ShareBeliefInput) -> ExternResult<Record> {
    ...
    for tag in &input.tags {
        ...
        create_link(
            tag_anchor,
            action_hash.clone(),
            LinkTypes::TagToBeliefShare,
            (),
        )?;
    }

    get(action_hash, GetOptions::default())?.ok_or(wasm_error!(WasmErrorInner::Guest(
        "Failed to get created belief share".into()
    )))
}
...
pub fn cast_vote(input: CastVoteInput) -> ExternResult<Record> {
    ...
    get(action_hash, GetOptions::default())?.ok_or(wasm_error!(WasmErrorInner::Guest(
        "Failed to get created vote".into()
    )))
}
...
fn get_agent_weight(agent: AgentPubKey) -> ExternResult<f64> {
    ...
    if let Some(link) = links.first() {
        if let Some(record) = get(
            ActionHash::try_from(link.target.clone()).unwrap(),
            GetOptions::default(),
        )? {
            if let Some(rep) = record
                .entry()
                .to_app_option::<EpistemicReputation>()
...
fn check_and_create_consensus(belief_hash: ActionHash) -> ExternResult<Option<ActionHash>> {
    ...
    create_link(
        belief_hash,
        action_hash.clone(),
        LinkTypes::BeliefShareToConsensus,
        (),
    )?;
...
fn get_all_beliefs_with_embeddings() -> ExternResult<Vec<BeliefWithEmbedding>> {
    ...
    for link in links.into_iter().take(500) {
        // Limit for performance
        let hash = ActionHash::try_from(link.target)
            .map_err(|_| wasm_error!(WasmErrorInner::Guest("Invalid target".into())))?;
        if let Some(record) = get(hash.clone(), GetOptions::default())? {
            if let Some(share) = record
                .entry()
                .to_app_option::<BeliefShare>()
                .map_err(|e| wasm_error!(WasmErrorInner::Guest(e.to_string())))?
            {
...
fn classify_cluster_pattern(beliefs: &[BeliefWithEmbedding]) -> PatternType {
    ...
    for belief in beliefs {
        match belief.stance {
            ...
            Some(BeliefStance::StronglyDisagree) | Some(BeliefStance::Disagree) => {
                disagree_count += 1
            }
            _ => {}
        }
...
pub fn record_pattern(input: RecordPatternInput) -> ExternResult<Record> {
    ...
    for belief_hash in input.belief_hashes {
        create_link(
            action_hash.clone(),
            belief_hash,
            LinkTypes::PatternToBeliefs,
            (),
        )?;
    }

    get(action_hash, GetOptions::default())?.ok_or(wasm_error!(WasmErrorInner::Guest(
        "Failed to get created pattern".into()
    )))
}
...
pub fn update_reputation(input: UpdateReputationInput) -> ExternResult<Record> {
    ...
    get(action_hash, GetOptions::default())?.ok_or(wasm_error!(WasmErrorInner::Guest(
        "Failed to get created reputation".into()
    )))
}
...
pub fn get_domain_expertise(input: DomainExpertiseInput) -> ExternResult<DomainExpertiseResult> {
    ...
    Ok(DomainExpertiseResult {
        ...
        last_activity: if domain_votes.is_empty() {
            None
        } else {
            Some(last_activity)
        },
    ...
    for link in links {
        ...
        if let Some(record) = get(hash, GetOptions::default())? {
            if let Some(rel) = record
                .entry()
                .to_app_option::<AgentRelationship>()
                .map_err(|e| wasm_error!(WasmErrorInner::Guest(e.to_string())))?
            {
...
pub fn update_relationship(input: UpdateRelationshipInput) -> ExternResult<Record> {
    ...
    let relationship = if let Some(existing_rel) = existing {
        ...
        AgentRelationship {
            ...
            relationship_stage: input
                .relationship_stage
                .unwrap_or(existing_rel.relationship_stage),
            shared_domains: merge_domains(
                existing_rel.shared_domains,
                input.new_domains.unwrap_or_default(),
            ),
            agreement_ratio: input
                .agreement_ratio
                .unwrap_or(existing_rel.agreement_ratio),
        }
    } else {
        ...
        AgentRelationship {
            ...
            relationship_stage: input
                .relationship_stage
                .unwrap_or(RelationshipStage::NoRelation),
    ...
    get(action_hash, GetOptions::default())?.ok_or(wasm_error!(WasmErrorInner::Guest(
        "Failed to get created relationship".into()
    )))
}
...
fn get_relationship_internal(
    my_agent: AgentPubKey,
    other_agent: AgentPubKey,
) -> ExternResult<Option<AgentRelationship>> {
    let links = get_links(
...
pub fn calculate_weighted_consensus(
    belief_hash: ActionHash,
) -> ExternResult<WeightedConsensusResult> {
    let my_agent = agent_info()?.agent_initial_pubkey;
...
pub fn analyze_belief_trends(input: TrendAnalysisInput) -> ExternResult<TrendAnalysisResult> {
    ...
    let velocity_alerts =
        detect_belief_velocity(&beliefs_in_window, input.velocity_threshold.unwrap_or(3));
...
fn detect_emerging_consensus(
    beliefs: &[&BeliefWithTimestamp],
) -> ExternResult<Vec<EmergingConsensusAlert>> {
    if beliefs.len() < 3 {
        return Ok(Vec::new());
    }

    // Group beliefs by semantic similarity (using embedding clustering)
    let beliefs_with_embeddings: Vec<_> =
        beliefs.iter().filter(|b| !b.embedding.is_empty()).collect();
    ...
    for (tag, confidences) in tag_confidences {
        ...
        if variance < 0.02 && mean > 0.7 {
            warnings.push(HerdingWarning {
                ...
                suggestion:
                    "Consider seeking diverse perspectives or questioning shared assumptions."
                        .to_string(),
            });
        }
        ...
        if confidences.len() >= 5 && mean > 0.6 {
            warnings.push(HerdingWarning {
                ...
                suggestion: "Ensure independent verification rather than following the crowd."
                    .to_string(),
            });
        }
    }
...
fn calculate_overall_trend(
    window: &[&BeliefWithTimestamp],
    all: &[BeliefWithTimestamp],
) -> TrendDirection {
    if all.is_empty() {
        ...
        if embeddings.len() >= 2 {
            ...
            for i in 0..embeddings.len().min(10) {
                for j in (i + 1)..embeddings.len().min(10) {
                    total_sim +=
                        cosine_similarity(&embeddings[i].embedding, &embeddings[j].embedding);
                    count += 1;
                }
            }
            if count > 0 {
