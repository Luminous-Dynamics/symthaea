...
// Commercial licensing: see COMMERCIAL_LICENSE.md at repository root//! LUCID Bridge Coordinator Zome
//!
...
pub fn record_federation(input: RecordFederationInput) -> ExternResult<Record> {
    ...
    get(action_hash, GetOptions::default())?.ok_or(wasm_error!(WasmErrorInner::Guest(
        "Federation record not found".into()
    )))
}
...
pub fn store_coherence_analysis(input: StoreCoherenceInput) -> ExternResult<Record> {
    ...
    let analysis = bridge_integrity::CoherenceAnalysis {
        ...
        contradictions: input
            .contradictions
            .into_iter()
            .map(|c| bridge_integrity::DetectedContradiction {
    ...
    get(action_hash, GetOptions::default())?.ok_or(wasm_error!(WasmErrorInner::Guest(
        "Coherence analysis not found".into()
    )))
}
...
#[hdk_extern]
pub fn get_coherence_candidates(
    input: GetCoherenceCandidatesInput,
) -> ExternResult<Vec<CoherenceCandidate>> {
...
#[hdk_extern]
pub fn get_thought_coherence_status(
    thought_id: String,
) -> ExternResult<Option<CoherenceCheckResult>> {
    ...
    if let Some(record) = analyses.last() {
        if let Some(analysis) = record
            ...
            .flatten()
        {
            let contradictions: Vec<PotentialContradiction> = analysis
                .contradictions
                .iter()
...
pub fn get_contradicting_thoughts(_: ()) -> ExternResult<Vec<CoherenceCheckResult>> {
    ...
    let links = get_links(
        LinkQuery::try_new(
            anchor_hash(all_coherence_anchor)?,
            LinkTypes::ThoughtsToCoherence,
        )?,
        GetStrategy::default(),
    )?;
                    ...
                    for thought_id in &analysis.thought_ids {
                        ...
                        if !contradictions.is_empty() {
                            results.push(CoherenceCheckResult {
                                ...
                                potential_contradictions: contradictions,
                                suggestions: vec![
                                    "Review and resolve this contradiction".to_string(),
                                ],
                            });
                        }
                    }
