...
// Commercial licensing: see COMMERCIAL_LICENSE.md at repository root//! Gradient Quality Proof Host
//!
...
use zk_gradient_core::{GradientConstraints, GradientProofInput, GradientProofOutput};
...
fn main() {
    ...
    let output: GradientProofOutput = receipt.journal.decode().expect("Failed to decode journal");
    ...
    println!(
        "  - Computed for model hash {:02x?}...",
        &output.global_model_hash[..4]
    );
    println!(
        "  - Training: {} epochs, lr={}",
        output.epochs, output.learning_rate
    );
