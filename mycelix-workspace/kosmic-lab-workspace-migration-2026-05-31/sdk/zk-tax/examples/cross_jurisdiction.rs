...
// Commercial licensing: see COMMERCIAL_LICENSE.md at repository root//! Example: Cross-Jurisdiction Tax Bracket Proofs
//!
...
fn main() {
    ...
    for info in &proof.jurisdiction_brackets {
        ...
        println!(
            "    Range: {} - {}",
            info.bracket_lower,
            if info.bracket_upper == u64::MAX {
                "+".to_string()
            } else {
                info.bracket_upper.to_string()
            }
        );
    }
    ...
    println!(
        "\nProof generated for {} countries!",
        oecd_proof.jurisdiction_count()
    );
    ...
    println!(
        "    Average: {:.1}%",
        oecd_proof.average_rate_bps() as f64 / 100.0
    );
    println!(
        "    Rates within 5%: {}",
        oecd_proof.rates_within_tolerance(500)
    );
    ...
    println!(
        "\nProof generated for {} countries!",
        americas_proof.jurisdiction_count()
    );
    ...
    println!("Income: ${} USD equivalent (NEVER REVEALED)", usd_income);
    println!(
        "\nProof generated for {} jurisdictions!",
        high_income_proof.jurisdiction_count()
    );
    ...
    println!(
        "  Rates within 10%: {}",
        consistent_proof.rates_within_tolerance(1000)
    );
