...
// Commercial licensing: see COMMERCIAL_LICENSE.md at repository root//! Subnational (state/province/canton) tax examples.
//!
...
use mycelix_zk_tax::{
    ...
    subnational::{SwissCanton, USState, get_cantonal_brackets, get_state_brackets},
};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
        ...
        match get_state_brackets(state, 2024, FilingStatus::Single) {
            Ok(brackets) => {
                ...
                let bracket = brackets
                    .iter()
                    .find(|b| 100_000 >= b.lower && 100_000 <= b.upper);
    ...
    for (canton, name) in cantons {
        ...
        let bracket = brackets
            .iter()
            .find(|b| 150_000 >= b.lower && 150_000 <= b.upper);
        if let Some(b) = bracket {
    ...
    println!(
        "   Combined marginal: {}%",
        (combined_proof.federal_proof.rate_bps + combined_proof.state_proof.rate_bps) as f64
            / 100.0
    );
    ...
    println!(
        "     Federal marginal: {}%",
        ca_federal.rate_bps as f64 / 100.0
    );
    println!("     State marginal: {}%", ca_state_rate as f64 / 100.0);
    println!(
        "     Combined: {}%",
        (ca_federal.rate_bps + ca_state_rate) as f64 / 100.0
    );

    println!("   Texas:");
    println!(
        "     Federal marginal: {}%",
        tx_federal.rate_bps as f64 / 100.0
    );
