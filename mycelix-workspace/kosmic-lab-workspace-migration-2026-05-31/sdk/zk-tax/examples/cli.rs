...
// Commercial licensing: see COMMERCIAL_LICENSE.md at repository root//! CLI: Generate ZK proofs for tax compliance.
//!
...
use mycelix_zk_tax::subnational::{
    SwissCanton, USState, get_cantonal_brackets, get_state_brackets,
};
use mycelix_zk_tax::{
    BatchProofBuilder, CompositeProofBuilder, CrossJurisdictionProofBuilder, DeductionCategory,
    DeductionProofBuilder, EffectiveTaxRateProof, FilingStatus, Jurisdiction, ProofChain,
    RangeProofBuilder, TaxBracketProver,
};
use std::env;
...
fn cmd_bracket(args: &[String]) {
    ...
    let proof = prover
        .prove(income, jurisdiction, filing_status, year)
    ...
    println!(
        "  Range: ${} - {}",
        format_number(proof.bracket_lower),
        if proof.bracket_upper == u64::MAX {
            "+".to_string()
        } else {
            format!("${}", format_number(proof.bracket_upper))
        }
    );
    println!("  Commitment: {}", proof.commitment);
    println!("\nGenerated in {:?}", elapsed);
    println!(
        "\nThis proves: 'My income falls in the {}% bracket'",
        proof.rate_bps as f64 / 100.0
    );
...
fn cmd_range(args: &[String]) {
    ...
    if min > 0 && max == u64::MAX {
        println!(
            "\nUse case: Rental application proving income >= ${}",
            format_number(min)
        );
    } else if max < u64::MAX && min == 0 {
        println!(
            "\nUse case: Benefits eligibility proving income <= ${}",
            format_number(max)
        );
    }
}
...
fn cmd_effective(args: &[String]) {
    ...
    println!("Output (PUBLIC):");
    println!(
        "  Marginal Rate: {:.0}% (bracket {})",
        proof.marginal_rate_percent(),
        proof.marginal_bracket
    );
    ...
    println!(
        "  Progressive Savings: {:.1}% lower than marginal!",
...
fn cmd_batch(args: &[String]) {
    ...
    let years: Vec<u32> = years_str
        .split(',')
        .map(|s| s.trim().parse().unwrap())
        .collect();
    let incomes: Vec<u64> = incomes_str
        .split(',')
        .map(|s| s.trim().parse().unwrap())
        .collect();
    ...
    println!(
        ...
        proof
            .consistent_bracket()
            .map_or("No".to_string(), |b| format!("Yes ({})", b))
    );
    println!("  Commitment: {}", proof.batch_commitment);
    println!("\nGenerated in {:?}", elapsed);
    println!(
        "\nUse case: Mortgage application proving {}-year income history",
        years.len()
    );
}
...
fn cmd_cross(args: &[String]) {
    ...
    let jurisdictions: Vec<Jurisdiction> = jurisdictions_str
        .split(',')
        .map(|s| s.trim().parse().unwrap())
        .collect();
    ...
    println!(
        "  Countries: {:?}",
        jurisdictions.iter().map(|j| j.name()).collect::<Vec<_>>()
    );
    println!("  Filing Status: {}", filing_status.name());
    println!("  Tax Year: {}\n", year);
    ...
    let proof = builder
        .build_dev()
        .expect("Cross-jurisdiction proof failed");
    let elapsed = start.elapsed();
    ...
    println!(
        "  Average Rate: {:.1}%",
        proof.average_rate_bps() as f64 / 100.0
    );
...
fn cmd_oecd(args: &[String]) {
    ...
    println!(
        "  Spread: {:.0} percentage points",
        (max - min) as f64 / 100.0
    );
    println!("\nGenerated in {:?}", elapsed);
}
...
fn cmd_deduction(args: &[String]) {
    ...
    if charitable > 0 {
        println!("  Charitable: ${}", format_number(charitable));
    }
    if mortgage > 0 {
        println!("  Mortgage Interest: ${}", format_number(mortgage));
    }
    if salt > 0 {
        println!("  State & Local Taxes: ${}", format_number(salt));
    }
    if medical > 0 {
        println!("  Medical: ${}", format_number(medical));
    }
    if retirement > 0 {
        println!("  Retirement: ${}", format_number(retirement));
    }
    println!("  Filing Status: {}", filing_status.name());
    println!("  Tax Year: {}\n", year);
    ...
    if charitable > 0 {
        builder = builder.add(DeductionCategory::Charitable, charitable);
    }
    if mortgage > 0 {
        builder = builder.add(DeductionCategory::MortgageInterest, mortgage);
    }
    if salt > 0 {
        builder = builder.add(DeductionCategory::StateLocalTaxes, salt);
    }
    if medical > 0 {
        builder = builder.add(DeductionCategory::Medical, medical);
    }
    if retirement > 0 {
        builder = builder.add(DeductionCategory::Retirement, retirement);
    }

    let proof = builder.build().expect("Deduction proof failed");
    let elapsed = start.elapsed();

    let standard_deduction = if filing_status == FilingStatus::MarriedFilingJointly {
        29_200
    } else {
        14_600
    };
    ...
    println!(
        "  Total deductions: ${} - ${}",
        format_number(proof.total_lower),
        format_number(proof.total_upper)
    );
    println!("  Categories claimed: {}", proof.category_count);
    println!(
        "  Standard deduction ({}): ${}",
        year,
        format_number(standard_deduction)
    );
    println!(
        "  Should itemize? {}",
        if proof.total_lower > standard_deduction {
            "YES"
        } else {
            "NO"
        }
    ...
    println!("\nGenerated in {:?}", elapsed);
    println!(
        "\nThis proves: 'I have at least ${} in deductions'",
        format_number(proof.total_lower)
    );
...
fn cmd_composite(args: &[String]) {
    ...
    if proof.has_effective_rate() {
        if let Some(ref ep) = proof.effective_rate_proof {
            println!(
                "  [Effective] {:.1}% (marginal: {:.0}%)",
                ep.effective_rate_percent(),
                ep.marginal_rate_percent()
            );
        }
    }
    if proof.has_range() {
        if let Some(ref rp) = proof.range_proof {
            println!(
                "  [Range] ${} - ${}",
                format_number(rp.range_lower),
                format_number(rp.range_upper)
            );
        }
    }
...
fn cmd_chain(args: &[String]) {
    ...
    println!(
        "\n  Final chain hash: {}...",
        &final_hash[..32.min(final_hash.len())]
    );
    println!("\nGenerated in {:?}", elapsed);
...
fn cmd_state(args: &[String]) {
    ...
    let start = Instant::now();
    let brackets =
        get_state_brackets(state, year, filing_status).expect("Failed to get state brackets");
    ...
    println!(
        ...
        if bracket.upper == u64::MAX {
            ...
            format!("${}", format_number(bracket.upper))
        }
    );
    println!(
        "  Tax type: {}",
        if state.is_flat_tax() {
            "Flat"
        } else {
            "Progressive"
        }
...
fn cmd_canton(args: &[String]) {
    ...
    println!(
        ...
        if bracket.upper == u64::MAX {
            ...
            format!("CHF {}", format_number(bracket.upper))
        }
    );
    println!("\nGenerated in {:?}", elapsed);
