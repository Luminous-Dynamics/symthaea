...
// Commercial licensing: see COMMERCIAL_LICENSE.md at repository root//! Proof caching example - improve performance with cached proofs.
//!
//! Run with: `cargo run --example caching --features prover`

use mycelix_zk_tax::{
    CacheConfig, CacheStats, FilingStatus, Jurisdiction, ProofCache, TaxBracketProver, cache_proof,
    clear_global_cache, get_cached, global_cache, global_stats,
};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "   Speedup: {:.0}x",
        generate_time.as_nanos() as f64 / cache_time.as_nanos() as f64
    );
