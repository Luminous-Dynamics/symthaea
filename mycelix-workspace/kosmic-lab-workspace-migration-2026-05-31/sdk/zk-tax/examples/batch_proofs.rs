...
// Commercial licensing: see COMMERCIAL_LICENSE.md at repository root//! Example: Multi-Year Batch Proofs
//!
...
fn main() {
    ...
    let incomes = [(2022, 78_000u64), (2023, 82_000), (2024, 85_000)];
    ...
    let mortgage_proof = prove_three_year_history(incomes, Jurisdiction::US, FilingStatus::Single)
    ...
    println!(
        "  Consistent bracket: {:?}",
        mortgage_proof.consistent_bracket()
    );
    println!("  Shows growth: {}", mortgage_proof.shows_growth());
    println!(
        "  Verification: {}",
        if mortgage_proof.verify().is_ok() {
            "VALID"
        } else {
            "INVALID"
        }
    ...
    println!(
        "  Verification: {}",
        if career_history.verify().is_ok() {
            "VALID"
        } else {
            "INVALID"
        }
    ...
    println!(
        "  All in same bracket: {:?}",
        visa_proof.consistent_bracket()
    );
    println!(
        "  Verification: {}",
        if visa_proof.verify().is_ok() {
            "VALID"
        } else {
            "INVALID"
        }
    ...
    println!(
        "  Shows growth: {} (expected: false)",
        declining.shows_growth()
    );
