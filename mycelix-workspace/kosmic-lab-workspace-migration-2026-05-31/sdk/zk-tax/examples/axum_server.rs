...
// Commercial licensing: see COMMERCIAL_LICENSE.md at repository root//! Production-ready Axum HTTP server for ZK tax proofs.
//!
...
async fn prove_bracket(
    ...
    let proof = state
        .prover
        .prove(income, jurisdiction, filing_status, req.tax_year)?;

    let response = BracketProofResponse::from_proof(proof);
    let elapsed = start.elapsed().as_millis() as u64;

    Ok(Json(
        ApiResponse::success(response).with_processing_time(elapsed),
    ))
}
...
async fn prove_range(
    ...
    Ok(Json(
        ApiResponse::success(serde_json::to_value(&proof)?).with_processing_time(elapsed),
    ))
}
...
async fn verify_proof(
    ...
    Ok(Json(
        ApiResponse::success(VerifyResponse { valid, details }).with_processing_time(elapsed),
    ))
}
...
async fn list_jurisdictions(
    ...
    Json(ApiResponse::success(JurisdictionListResponse {
        jurisdictions,
    }))
}
...
async fn get_brackets(
    ...
    let jurisdiction = Jurisdiction::from_code(&query.jurisdiction).ok_or_else(|| {
        AppError::BadRequest(format!("Unknown jurisdiction: {}", query.jurisdiction))
    })?;
