...
// Commercial licensing: see COMMERCIAL_LICENSE.md at repository root//! Business entity taxation example.
//!
//! Run with: `cargo run --example business_entities --features "prover,entity"`

use mycelix_zk_tax::entity::{BusinessProver, EntityTaxInfo, EntityType};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "   Federal rate: {}%",
        c_corp.federal_rate_bps as f64 / 100.0
    );
    ...
    println!(
        "   As S-Corp: FICA on salary only ~${}",
        llc_scorp.fica_on_salary
    );
    println!(
        "   Potential savings: ${}\n",
    ...
    let proof = prover.prove_entity(EntityType::CCorp, 500_000, 2024)?;
