...
// Commercial licensing: see COMMERCIAL_LICENSE.md at repository root//! Benchmarks for ZK Tax proof generation.
//!
    ...
    for income in incomes {
        group.bench_with_input(
            ...
            |b, &inc| {
                b.iter(|| {
                    prover
                        .prove(black_box(inc), Jurisdiction::US, FilingStatus::Single, 2024)
                        .unwrap()
                })
            },
        );
    ...
    for jurisdiction in jurisdictions {
        group.bench_with_input(
            ...
            |b, &j| {
                b.iter(|| {
                    prover
                        .prove(100_000, black_box(j), FilingStatus::Single, 2024)
                        .unwrap()
                })
            },
        );
...
fn bench_batch_proof(c: &mut Criterion) {
    let years_and_incomes = [(2022, 75_000u64), (2023, 80_000), (2024, 85_000)];
    ...
    group.bench_function("5_year_history", |b| {
        b.iter(|| {
            let mut builder = BatchProofBuilder::new(Jurisdiction::US, FilingStatus::Single);
    ...
    group.bench_function("3_link_chain", |b| {
        b.iter(|| {
            ...
            for (year, income) in &years_incomes {
                let proof = prover
                    .prove(*income, Jurisdiction::US, FilingStatus::Single, *year)
                    .unwrap();
...
fn bench_verification(c: &mut Criterion) {
    ...
    group.bench_function("range_verify", |b| {
        b.iter(|| black_box(&range_proof).verify().unwrap())
    });
    ...
    group.bench_function("effective_rate_verify", |b| {
        b.iter(|| black_box(&eff_proof).verify().unwrap())
    });
...
fn bench_compression(c: &mut Criterion) {
    ...
    group.bench_function("compress_rle_base64", |b| {
        b.iter(|| {
            CompressedProof::compress(black_box(&proof), black_box(CompressedProofType::RleBase64))
                .unwrap()
        })
    });
    ...
    group.bench_function("compress_zstd", |b| {
        b.iter(|| {
            CompressedProof::compress(black_box(&proof), black_box(CompressedProofType::Zstd))
                .unwrap()
        })
    });
    ...
    group.bench_function("decompress_rle_base64", |b| {
        b.iter(|| black_box(&compressed_rle).decompress().unwrap())
    });

    group.bench_function("decompress_zstd", |b| {
        b.iter(|| black_box(&compressed_zstd).decompress().unwrap())
    });
...
fn bench_global_cache(c: &mut Criterion) {
    ...
    group.bench_function("global_cache_stats", |b| b.iter(|| global_cache().stats()));
