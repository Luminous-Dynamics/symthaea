...
// Commercial licensing: see COMMERCIAL_LICENSE.md at repository root//! ZK Tax Bracket Guest Code
//!
...
fn main() {
    ...
    let in_range =
        input.gross_income >= input.bracket_lower && input.gross_income < input.bracket_upper;

    // Compute commitment to bracket parameters
    // Using a simple but deterministic hash
    let commitment = compute_commitment(input.bracket_lower, input.bracket_upper, input.tax_year);
