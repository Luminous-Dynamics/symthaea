...
// Commercial licensing: see COMMERCIAL_LICENSE.md at repository root//! Integration tests verifying SDK compatibility with Holochain zome.
//!
//! These tests ensure:
...
fn test_commitment_matches_all_g20_jurisdictions() {
    ...
    for j in jurisdictions {
        ...
        for bracket in brackets {
            ...
            let zome_commitment = compute_commitment_zome_style(bracket.lower, bracket.upper, 2024);
...
fn test_jurisdiction_codes_match_zome_validation() {
    ...
    for code in zome_valid_jurisdictions {
        ...
        assert!(result.is_ok(), "Zome code '{}' should parse in SDK", code);
    }
...
fn test_tax_year_in_valid_range() {
    ...
    for year in [2020, 2021, 2022, 2023, 2024, 2025] {
        ...
        assert!(result.is_ok(), "Year {} should be valid", year);
    }

    // Year outside range should fail
    for year in [2019, 2026] {
        let result = get_brackets(Jurisdiction::US, year, FilingStatus::Single);
        assert!(result.is_err(), "Year {} should be invalid", year);
    }
}
...
fn test_commitment_hex_format() {
    ...
    // Should be valid hex
    assert!(hex::decode(&hex).is_ok(), "Commitment should be valid hex");
}
