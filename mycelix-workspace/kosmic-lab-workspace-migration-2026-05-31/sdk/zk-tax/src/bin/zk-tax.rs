...
// Commercial licensing: see COMMERCIAL_LICENSE.md at repository root//! ZK Tax CLI - Command-line tool for generating and verifying tax proofs.
//!
//! # Usage
//!
//! ```bash
...
use mycelix_zk_tax::{
    FilingStatus, Jurisdiction, TaxBracketProof, TaxBracketProver, proof::CompressedProof,
};
...
fn cmd_prove(
    ...
    if verbose {
        eprintln!(
            "Generating proof for income ${} in {} ({})...",
            income, jurisdiction, year
        );
    }
...
fn cmd_verify(
    ...
    match proof.verify() {
        Ok(_) => {
            ...
            println!(
                "  Range: ${} - ${}",
                proof.bracket_lower, proof.bracket_upper
            );

            if let Some(expected) = expected_bracket {
                if proof.bracket_index != expected {
                    eprintln!(
                        "WARNING: Expected bracket {}, got {}",
                        expected, proof.bracket_index
                    );
                    std::process::exit(1);
                }
            }
...
fn parse_filing_status(s: &str) -> Result<FilingStatus, Box<dyn std::error::Error>> {
    match s.to_lowercase().as_str() {
        ...
        "mfs" | "married_filing_separately" | "separate" => {
            Ok(FilingStatus::MarriedFilingSeparately)
        }
