...
// Commercial licensing: see COMMERCIAL_LICENSE.md at repository root//! Tax bracket definitions for all supported jurisdictions.
...
pub fn get_brackets(
    ...
    match (jurisdiction, year, status) {
        ...
        (_, year, _) if !(2020..=2025).contains(&year) => Err(Error::unsupported_year(year)),

        // Invalid filing status for jurisdiction
        (j, _, status) if !j.is_valid_filing_status(status) => {
            let valid: Vec<&str> = j.valid_filing_statuses().iter().map(|s| s.code()).collect();
...
mod tests {
    ...
    fn test_japan_bracket_lookup() {
        ...
        let bracket =
            find_bracket(5_000_000, Jurisdiction::JP, 2024, FilingStatus::Single).unwrap();
        assert_eq!(bracket.index, 2);
    ...
    fn test_all_g20_jurisdictions_2024() {
        ...
        for (j, income, expected_index) in jurisdictions {
            ...
            assert_eq!(
                bracket.index, expected_index,
                "Failed for {:?} with income {}",
                j, income
            );
        }
    }
    ...
    fn test_argentina_bracket_lookup() {
        ...
        let bracket =
            find_bracket(5_000_000, Jurisdiction::AR, 2024, FilingStatus::Single).unwrap();
        assert_eq!(bracket.index, 2);
    ...
    fn test_korea_bracket_lookup() {
        ...
        let bracket =
            find_bracket(30_000_000, Jurisdiction::KR, 2024, FilingStatus::Single).unwrap();
        assert_eq!(bracket.index, 1);
        assert_eq!(bracket.rate_bps, 1500);
    }
    ...
    fn test_saudi_no_income_tax() {
        ...
        let bracket =
            find_bracket(1_000_000, Jurisdiction::SA, 2024, FilingStatus::Single).unwrap();
    ...
    fn test_us_historical_years() {
        ...
        for year in [2020, 2021, 2022, 2023, 2024, 2025] {
            let bracket =
                find_bracket(85_000, Jurisdiction::US, year, FilingStatus::Single).unwrap();
            assert_eq!(
                bracket.index, 2,
                "Year {} should have bracket index 2",
                year
            );
    ...
    fn test_uk_historical_years() {
        ...
        for year in [2020, 2021, 2022, 2023, 2024, 2025] {
            let bracket =
                find_bracket(60_000, Jurisdiction::UK, year, FilingStatus::Single).unwrap();
            assert_eq!(
                bracket.index, 2,
                "Year {} should have bracket index 2",
                year
    ...
    fn test_us_mfj_historical() {
        ...
        for year in [2020, 2021, 2022, 2023] {
            ...
            assert_eq!(
                bracket.index, 2,
                "Year {} MFJ should have bracket index 2",
                year
            );
            assert_eq!(
                bracket.rate_bps, 2200,
                "Year {} MFJ should have 22% rate",
                year
            );
        }
    }
