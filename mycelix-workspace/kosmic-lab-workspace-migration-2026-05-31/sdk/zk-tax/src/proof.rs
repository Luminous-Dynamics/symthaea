...
// Commercial licensing: see COMMERCIAL_LICENSE.md at repository root//! Proof types for ZK tax verification.
        ...
        if self.is_simulation_proof() {
            return Err(Error::invalid_proof(
                "SECURITY ERROR: Simulation proof rejected. \
                 ...
                 Enable 'prover' feature for real proofs."
                    .to_string(),
            ));
        }
        Ok(())
...
impl BatchProof {
    ...
    pub fn summary(&self) -> String {
        let years: Vec<String> = self
            .year_proofs
            .iter()
            .map(|p| p.tax_year.to_string())
            .collect();
        let brackets: Vec<String> = self
            .year_proofs
            .iter()
            .map(|p| p.bracket_index.to_string())
            .collect();
    ...
    pub fn shows_growth(&self) -> bool {
        ...
        sorted
            .windows(2)
            .all(|w| w[1].bracket_index >= w[0].bracket_index)
    }
...
impl BatchProofBuilder {
    ...
    pub fn build_dev(self) -> Result<BatchProof> {
        if self.year_incomes.is_empty() {
            return Err(Error::invalid_proof(
                "No years specified for batch proof".to_string(),
            ));
        }
...
impl EffectiveTaxRateProof {
    ...
    fn calculate_us_effective_rate(
        income: u64,
        filing_status: FilingStatus,
        year: TaxYear,
    ) -> Result<(u16, u8)> {
    ...
    pub fn progressive_tax_savings_bps(&self) -> u16 {
        self.marginal_rate_bps
            .saturating_sub(self.actual_rate_bps.unwrap_or(0))
    }

    /// Verify the proof commitment.
    pub fn verify(&self) -> Result<()> {
        let expected = compute_effective_rate_commitment(
            self.actual_rate_bps
                .unwrap_or(self.effective_rate_lower_bps),
            self.jurisdiction,
            self.filing_status,
            self.tax_year,
        );
...
impl CrossJurisdictionProof {
    ...
    pub fn verify(&self) -> Result<()> {
        ...
        for info in &self.jurisdiction_brackets {
            let expected =
                compute_commitment(info.bracket_lower, info.bracket_upper, self.tax_year);
        ...
        let expected_combined =
            compute_cross_jurisdiction_commitment(&self.jurisdiction_brackets, self.tax_year);
...
impl CrossJurisdictionProofBuilder {
    ...
    pub fn build_dev(self) -> Result<CrossJurisdictionProof> {
        if self.jurisdictions.is_empty() {
            return Err(Error::invalid_proof(
                "No jurisdictions specified".to_string(),
            ));
        }
        ...
        let combined_commitment =
            compute_cross_jurisdiction_commitment(&jurisdiction_brackets, self.tax_year);
...
fn convert_usd_to_local(usd: u64, jurisdiction: Jurisdiction) -> u64 {
    let rate = match jurisdiction {
        ...
        | Jurisdiction::IE
        | Jurisdiction::FI
        | Jurisdiction::GR => 0.92, // EUR per USD
...
impl DeductionProof {
    ...
    pub fn summary(&self) -> String {
        let comparison = if self.itemizing_beneficial {
            format!(
                "Itemizing saves ${}",
                format_currency(self.total_lower.saturating_sub(self.standard_deduction))
            )
        } else {
            format!(
                "Standard deduction better (${} vs ${})",
        ...
        if self.has_range() {
            if let Some(ref p) = self.range_proof {
                parts.push(format!(
                    ...
                    if p.range_upper == u64::MAX {
                        "+".to_string()
                    } else {
                        format!("${}", format_currency(p.range_upper))
                    }
                ));
            }
        }
...
impl CompositeProofBuilder {
    ...
    pub fn build_dev(self) -> Result<CompositeProof> {
        ...
        let range_proof = if self.range_lower.is_some() || self.range_upper.is_some() {
            ...
            Some(IncomeRangeProof::prove_dev(
                self.income,
                lower,
                upper,
                self.tax_year,
...
impl ProofChain {
    ...
    pub fn summary(&self) -> String {
        ...
        let years: Vec<String> = self.links.iter().map(|l| l.tax_year.to_string()).collect();
        let types: Vec<&str> = self.links.iter().map(|l| l.proof_type.as_str()).collect();
...
impl CombinedFederalStateProof {
    ...
    pub fn prove_dev(
        ...
        let state_info = if state.has_income_tax() {
            let state_brackets =
                crate::subnational::get_state_brackets(state, tax_year, filing_status)
    ...
    pub fn verify(&self) -> Result<()> {
        if self.is_dev_mode {
            ...
            Err(Error::verification_failed(
                "Real verification not yet implemented",
            ))
        }
    }
...
impl IncomeStabilityProof {
    ...
    pub fn prove_dev(year_incomes: Vec<(TaxYear, u64)>, max_variation_percent: u8) -> Result<Self> {
        ...
        // Compute commitment
        let commitment =
            Self::compute_stability_commitment(&years, actual_variation_rounded, is_stable);
    ...
    fn compute_stability_commitment(
        years: &[TaxYear],
        variation: u8,
        stable: bool,
    ) -> BracketCommitment {
...
impl CompressedProof {
    ...
    pub fn from_range_proof(proof: &IncomeRangeProof) -> Result<Self> {
        let original =
            bincode::serialize(proof).map_err(|e| Error::serialization(e.to_string()))?;
        let compressed = Self::compress(&original);
...
mod tests {
    ...
    fn test_builder_prove_multiple_fails_if_below() {
        ...
        let result = RangeProofBuilder::new(60_000, 2024).prove_multiple_of(annual_rent, 3);
    ...
    fn test_range_proof_different_years() {
        ...
        assert_ne!(
            proof_2024.commitment.to_hex(),
            proof_2023.commitment.to_hex()
        );
    }
    ...
    fn test_batch_proof_empty_fails() {
        ...
        let result = BatchProofBuilder::new(Jurisdiction::US, FilingStatus::Single).build_dev();
    ...
    fn test_effective_rate_basic() {
        ...
        let proof =
            EffectiveTaxRateProof::prove_dev(85_000, Jurisdiction::US, FilingStatus::Single, 2024)
                .unwrap();
    ...
    fn test_effective_rate_low_income() {
        ...
        let proof =
            EffectiveTaxRateProof::prove_dev(20_000, Jurisdiction::US, FilingStatus::Single, 2024)
                .unwrap();
    ...
    fn test_effective_rate_high_income() {
        ...
        let proof =
            EffectiveTaxRateProof::prove_dev(500_000, Jurisdiction::US, FilingStatus::Single, 2024)
                .unwrap();
    ...
    fn test_effective_rate_progressive_savings() {
        ...
        let proof =
            EffectiveTaxRateProof::prove_dev(150_000, Jurisdiction::US, FilingStatus::Single, 2024)
                .unwrap();
    ...
    fn test_effective_rate_builder() {
        ...
        let proof =
            EffectiveTaxRateBuilder::new(85_000, Jurisdiction::US, FilingStatus::Single, 2024)
                .build()
                .unwrap();
    ...
    fn test_effective_rate_zero_income() {
        ...
        let proof =
            EffectiveTaxRateProof::prove_dev(0, Jurisdiction::US, FilingStatus::Single, 2024)
                .unwrap();
    ...
    fn test_effective_rate_within_first_bracket() {
        ...
        let proof =
            EffectiveTaxRateProof::prove_dev(10_000, Jurisdiction::US, FilingStatus::Single, 2024)
                .unwrap();
    ...
    fn test_effective_rate_non_us_approximation() {
        ...
        let proof =
            EffectiveTaxRateProof::prove_dev(50_000, Jurisdiction::UK, FilingStatus::Single, 2024)
                .unwrap();
    ...
    fn test_cross_jurisdiction_empty_fails() {
        let result = CrossJurisdictionProofBuilder::new(100_000, 2024).build_dev();
    ...
    fn test_deduction_category_names() {
        ...
        assert_eq!(
            DeductionCategory::StateLocalTaxes.name(),
            "State & Local Taxes"
        );
        assert_eq!(
            DeductionCategory::Retirement.name(),
    ...
    fn test_deduction_category_limits() {
        assert_eq!(
            DeductionCategory::StateLocalTaxes.us_limit_2024(),
            Some(10_000)
        );
    ...
    fn test_proof_chain_multiple_proofs() {
        ...
        // Add 2022 proof
        let proof_2022 = prover
            .prove(75_000, Jurisdiction::US, FilingStatus::Single, 2022)
            .unwrap();
        chain.add_bracket_proof(&proof_2022);

        // Add 2023 proof
        let proof_2023 = prover
            .prove(80_000, Jurisdiction::US, FilingStatus::Single, 2023)
            .unwrap();
        chain.add_bracket_proof(&proof_2023);

        // Add 2024 proof
        let proof_2024 = prover
            .prove(85_000, Jurisdiction::US, FilingStatus::Single, 2024)
            .unwrap();
    ...
    fn test_proof_chain_mixed_types() {
        ...
        assert_eq!(
            types,
            vec!["bracket", "effective_rate", "range", "deduction"]
        );
    }
