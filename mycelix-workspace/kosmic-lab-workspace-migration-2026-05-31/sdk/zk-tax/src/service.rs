...
// Commercial licensing: see COMMERCIAL_LICENSE.md at repository root//! Production service layer with rate limiting, caching, and observability.
//!
...
use crate::{Error, FilingStatus, Jurisdiction, Result, TaxBracketProof, TaxYear};
    ...
    pub fn prove_bracket(&self, request: ProveRequest) -> Result<ProveResponse> {
        ...
        if !self.rate_limiter.check(&request.client_id) {
            ...
            return Err(Error::proof_generation_with_context(
                ...
                format!(
                    "Wait and retry. Limit: {} requests per {:?}",
...
mod tests {
    ...
    fn test_cache() {
        ...
        let key =
            ProofCacheService::cache_key(85_000, Jurisdiction::US, FilingStatus::Single, 2024);
