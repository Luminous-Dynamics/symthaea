...
// Commercial licensing: see COMMERCIAL_LICENSE.md at repository root//! WebAssembly bindings for browser-based tax proof generation.
//!
...
use crate::{
    FilingStatus, Jurisdiction, TaxBracketProof,
    brackets::get_brackets as sdk_get_brackets,
    proof::{BatchProofBuilder, CompressedProof, CompressedProofType, RangeProofBuilder},
};
...
pub fn verify_proof(proof: JsValue) -> Result<bool, JsError> {
    ...
    proof.verify().map_err(|e| JsError::new(&e.to_string()))?;
...
fn parse_jurisdiction(s: &str) -> Result<Jurisdiction, JsError> {
    Jurisdiction::from_code(s).ok_or_else(|| JsError::new(&format!("Unknown jurisdiction: {}", s)))
}
