...
// Commercial licensing: see COMMERCIAL_LICENSE.md at repository root//! Proof aggregation for efficient batch verification.
//!
...
use crate::{BracketCommitment, Error, Result, TaxBracketProof};
...
fn verify_merkle_proof(leaf: &[u8; 32], proof: &[[u8; 32]], index: usize, root: &[u8; 32]) -> bool {
    ...
    fn create_test_proofs(count: usize) -> Vec<TaxBracketProof> {
        ...
        (0..count)
            .map(|i| {
                ...
                prover
                    .prove(income, Jurisdiction::US, FilingStatus::Single, 2024)
                    .unwrap()
            })
            .collect()
    }
