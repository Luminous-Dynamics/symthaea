...
// Commercial licensing: see COMMERCIAL_LICENSE.md at repository root//! Ethereum anchoring implementation.
//!
...
impl Anchorer for EthereumAnchor {
    async fn anchor(proof: &TaxBracketProof, config: &AnchorConfig) -> Result<ProofAnchor> {
    ...
    async fn verify(_anchor: &ProofAnchor, _config: &AnchorConfig) -> Result<bool> {
        // Would query the blockchain to verify the anchor exists
        Ok(true)
    }

    async fn status(anchor: &ProofAnchor, _config: &AnchorConfig) -> Result<AnchorStatus> {
...
mod tests {
    use super::*;
    use crate::{FilingStatus, Jurisdiction, TaxBracketProver};
