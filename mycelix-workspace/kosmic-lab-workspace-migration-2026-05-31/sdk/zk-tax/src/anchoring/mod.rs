...
// Commercial licensing: see COMMERCIAL_LICENSE.md at repository root//! Blockchain anchoring for tax proofs.
//!
...
impl ProofAnchor {
    ...
    pub fn explorer_url(&self) -> Option<String> {
        ...
        match self.network {
            ...
            Network::EthereumSepolia => {
                Some(format!("https://sepolia.etherscan.io/tx/0x{}", tx_hex))
            }
            ...
            Network::SolanaDevnet => {
                Some(format!("https://solscan.io/tx/{}?cluster=devnet", tx_hex))
            }
            Network::BitcoinMainnet => Some(format!("https://blockstream.info/tx/{}", tx_hex)),
            Network::BitcoinTestnet => {
                Some(format!("https://blockstream.info/testnet/tx/{}", tx_hex))
            }
            _ => None,
        }
    }
...
impl AnchorMerkleTree {
    ...
    pub fn from_proofs(proofs: &[TaxBracketProof]) -> Self {
        let leaves: Vec<[u8; 32]> = proofs.iter().map(|p| p.commitment.to_bytes()).collect();
