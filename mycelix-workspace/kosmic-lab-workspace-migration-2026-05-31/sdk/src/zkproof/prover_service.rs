...
// Commercial licensing: see COMMERCIAL_LICENSE.md at repository root//! RISC-0 Prover Service Client
//!
...
use super::trust_risc0::{
    SCALE, TrustProverError, ZkKVector, ZkTrustParams, ZkTrustProofInput, ZkTrustProofOutput,
    ZkTrustReceipt, ZkTrustStatement, compute_commitment, evaluate_statement,
};
...
impl ProverServiceClient {
        ...
        #[cfg(feature = "reqwest")]
        {
            ...
            for attempt in 0..=self.config.retry_count {
                ...
                match client.post(&url).json(&request).send() {
                    Ok(response) => {
                        if response.status().is_success() {
                            return response.json::<ProveResponse>().map(|r| r.receipt).map_err(
                                |e| ProverServiceError::DeserializationFailed(e.to_string()),
                            );
                        } else {
                            let error = response.json::<ServiceError>().unwrap_or(ServiceError {
    ...
    fn prove_locally_simulation(
        &self,
        input: ZkTrustProofInput,
    ) -> Result<ZkTrustReceipt, ProverServiceError> {
        eprintln!(
