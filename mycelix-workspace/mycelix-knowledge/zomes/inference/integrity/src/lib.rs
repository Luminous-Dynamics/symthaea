...
// Commercial licensing: see COMMERCIAL_LICENSE.md at repository root//! Inference Integrity Zome
    ...
    match op.flattened::<EntryTypes, LinkTypes>()? {
        FlatOp::StoreEntry(store_entry) => match store_entry {
            ...
            OpEntry::UpdateEntry {
                ...
                EntryTypes::CredibilityScore(score) => {
                    validate_update_credibility_score(action, score)
                }
                EntryTypes::Pattern(_) => Ok(ValidateCallbackResult::Valid),
                EntryTypes::EnhancedCredibilityScore(score) => {
                    validate_update_enhanced_credibility(action, score)
                }
...
fn validate_update_credibility_score(
    _action: Update,
    score: CredibilityScore,
) -> ExternResult<ValidateCallbackResult> {
...
fn validate_update_enhanced_credibility(
    _action: Update,
    score: EnhancedCredibilityScore,
) -> ExternResult<ValidateCallbackResult> {
    if score.overall_score < 0.0 || score.overall_score > 1.0 {
