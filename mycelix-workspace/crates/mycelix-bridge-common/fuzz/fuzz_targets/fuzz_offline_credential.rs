...
fuzz_target!(|input: FuzzOfflineInput| {
    ...
    if input.has_attestation {
        let attester_did = String::from_utf8_lossy(
            &input.attester_did_bytes[..input.attester_did_bytes.len().min(64)],
        )
        .to_string();
    ...
    if input.degradation_enabled
        && offline.attestation.is_none()
        && input.custom_grace_hours.is_none()
    {
