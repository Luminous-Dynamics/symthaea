...
use mycelix_bridge_common::routing::{
    BridgeDomain, CIVIC_DOMAINS, COMMONS_DOMAINS, resolve_civic_zome, resolve_commons_zome,
};
...
fuzz_target!(|input: FuzzRoutingInput| {
    ...
    for d in COMMONS_DOMAINS {
        assert!(
            d.is_commons(),
            "{:?} in COMMONS_DOMAINS but is_commons() false",
            d
        );
        assert!(
            !d.is_civic(),
            "{:?} in COMMONS_DOMAINS but is_civic() true",
            d
        );
    }
    for d in CIVIC_DOMAINS {
        assert!(
            d.is_civic(),
            "{:?} in CIVIC_DOMAINS but is_civic() false",
            d
        );
        assert!(
            !d.is_commons(),
            "{:?} in CIVIC_DOMAINS but is_commons() true",
            d
        );
    }
