...
pub fn vec_too_short(entry_type: &str, field: &str, min: usize) -> String {
    format!("{} {} must have at least {} items", entry_type, field, min)
}
...
pub fn invalid_domain(domain: &str, valid: &[&str]) -> String {
    format!("Invalid domain '{}'. Must be one of: {:?}", domain, valid)
}
...
mod tests {
    ...
    #[test]
    fn test_field_empty() {
        assert_eq!(field_empty("Property", "id"), "Property id cannot be empty");
    }
