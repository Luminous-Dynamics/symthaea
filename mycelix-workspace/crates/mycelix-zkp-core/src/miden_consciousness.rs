...
pub fn prove_consciousness_tier_miden(
    ...
    use miden_core::Felt;
    use miden_vm::{
        Assembler, DefaultHost, ExecutionOptions, ProvingOptions, StackInputs, advice::AdviceInputs,
    };
    ...
    let threshold_felt = Felt::new(threshold_permille as u64)
        .map_err(|e| ZkpError::ProvingError(format!("Felt conversion error: {}", e)))?;
    let stack_inputs = StackInputs::new(&[threshold_felt])
...
pub fn verify_consciousness_tier_miden(
    ...
    let threshold_felt = Felt::new(threshold_permille as u64)
        .map_err(|e| ZkpError::VerificationFailed(format!("Felt conversion error: {}", e)))?;
    let stack_inputs = StackInputs::new(&[threshold_felt])
    ...
    let hash_felts = program_hash
        ...
        .collect::<MidenResult<Vec<Felt>>>()?;
    let hash_word: Word = Word::new([hash_felts[0], hash_felts[1], hash_felts[2], hash_felts[3]]);
