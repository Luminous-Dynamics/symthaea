...
impl MockDHT {
    ...
    fn create_project(&mut self, project: MockClimateProject) {
        self.events.push(MockBridgeEvent {
            ...
            payload: format!(
                r#"{{"name":"{}","expected_credits":{}}}"#,
                project.name, project.expected_credits
            ),
    ...
    fn complete_milestone(
        ...
        if let Some(milestone) = self
            .milestones
            .iter_mut()
            .find(|m| m.project_id == project_id && m.title == milestone_title)
        {
    ...
    fn issue_credits_for_milestone(&mut self, project_id: &str, tonnes: f64, _verifier_did: &str) {
    ...
    fn get_credits_by_owner(&self, owner_did: &str) -> Vec<&MockCarbonCredit> {
        self.credits
            .iter()
            .filter(|c| c.owner_did == owner_did)
            .collect()
    }
    ...
    fn transfer_credit(&mut self, credit_id: &str, new_owner_did: &str) -> bool {
        if let Some(credit) = self.get_credit_mut(credit_id) {
            ...
            self.events.push(MockBridgeEvent {
                ...
                payload: format!(r#"{{"from":"{}","to":"{}"}}"#, old_owner, new_owner_did),
...
mod tests {
    ...
    fn test_verify_active_credit() {
        ...
        let query = dht
            .queries
            .iter()
            .find(|q| q.query_id == "query:verify:001")
            .unwrap();
    ...
    fn test_verify_retired_credit_fails() {
        ...
        let query = dht
            .queries
            .iter()
            .find(|q| q.query_id == "query:retired")
            .unwrap();
    ...
    fn test_verify_nonexistent_credit() {
        ...
        let query = dht
            .queries
            .iter()
            .find(|q| q.query_id == "query:notfound")
            .unwrap();
    ...
    fn test_full_climate_lifecycle() {
        ...
        let query = dht
            .queries
            .iter()
            .find(|q| q.query_id == "query:lifecycle")
            .unwrap();
        assert_eq!(query.result, Some("Verified".to_string()));
    ...
    fn test_offsetting_workflow() {
        ...
        let credit_ids: Vec<String> = dht
            .get_credits_by_project("project:offset")
            .iter()
            .map(|c| c.id.clone())
            .collect();
        ...
        let org_credit_data: Vec<(String, f64)> = dht
            .get_credits_by_owner("did:mycelix:organization")
            .iter()
    ...
    fn test_concurrent_operations_same_credit() {
        ...
        let query = dht
            .queries
            .iter()
            .find(|q| q.query_id == "query:concurrent")
            .unwrap();
        assert_eq!(query.result, Some("Verified".to_string()));
