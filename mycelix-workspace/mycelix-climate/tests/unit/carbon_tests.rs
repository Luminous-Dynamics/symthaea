...
// Commercial licensing: see COMMERCIAL_LICENSE.md at repository root//! Carbon Zome Unit Tests
//!
...
mod tests {
    ...
    fn test_active_credit_with_retired_at_rejected() {
        ...
        assert!(
            result
                .unwrap_err()
                .contains("Non-retired credits cannot have retired_at")
        );
    }
    ...
    fn test_retired_credit_without_retired_at_rejected() {
        ...
        assert!(
            result
                .unwrap_err()
                .contains("Retired credits must have retired_at")
        );
    }
    ...
    fn test_transfer_credit_to_self_rejected() {
        ...
        assert!(
            result
                .unwrap_err()
                .contains("Cannot transfer to current owner")
        );
    }
    ...
    fn verify_footprint(
        footprint: &mut TestCarbonFootprint,
        verifier_did: &str,
    ) -> Result<(), String> {
    ...
    fn test_multiple_footprints_same_entity() {
        ...
        let total: f64 = footprints
            .iter()
            .map(|f| calculate_total_emissions(f))
            .sum();
