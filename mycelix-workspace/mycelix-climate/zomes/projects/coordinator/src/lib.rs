...
// Commercial licensing: see COMMERCIAL_LICENSE.md at repository root//! Projects Coordinator Zome
//!
...
pub fn create_climate_project(input: CreateProjectInput) -> ExternResult<Record> {
    ...
    get(action_hash, GetOptions::default())?.ok_or(wasm_error!(WasmErrorInner::Guest(
        "Failed to get created project".into()
    )))
}
...
pub fn verify_project(input: VerifyProjectInput) -> ExternResult<Record> {
    let record = get(input.project_action_hash.clone(), GetOptions::default())?.ok_or(
        wasm_error!(WasmErrorInner::Guest("Project not found".into())),
    )?;
    ...
    let action_hash = update_entry(
        input.project_action_hash.clone(),
        &EntryTypes::ClimateProject(updated),
    )?;
    ...
    get(action_hash, GetOptions::default())?.ok_or(wasm_error!(WasmErrorInner::Guest(
        "Failed to get verified project".into()
    )))
}
...
pub fn activate_project(input: ActivateProjectInput) -> ExternResult<Record> {
    ...
    get(action_hash, GetOptions::default())?.ok_or(wasm_error!(WasmErrorInner::Guest(
        "Failed to get activated project".into()
    )))
}
...
pub fn complete_project(input: CompleteProjectInput) -> ExternResult<Record> {
    ...
    get(action_hash, GetOptions::default())?.ok_or(wasm_error!(WasmErrorInner::Guest(
        "Failed to get completed project".into()
    )))
}
...
pub fn create_milestone(input: CreateMilestoneInput) -> ExternResult<Record> {
    ...
    let _project_record = get(input.project_action_hash.clone(), GetOptions::default())?.ok_or(
        wasm_error!(WasmErrorInner::Guest("Project not found".into())),
    )?;
    ...
    get(action_hash, GetOptions::default())?.ok_or(wasm_error!(WasmErrorInner::Guest(
        "Failed to get created milestone".into()
    )))
}
...
pub fn complete_milestone(input: CompleteMilestoneInput) -> ExternResult<Record> {
    let record = get(input.milestone_action_hash.clone(), GetOptions::default())?.ok_or(
        wasm_error!(WasmErrorInner::Guest("Milestone not found".into())),
    )?;
    ...
    let action_hash = update_entry(
        input.milestone_action_hash,
        &EntryTypes::ProjectMilestone(updated),
    )?;

    get(action_hash, GetOptions::default())?.ok_or(wasm_error!(WasmErrorInner::Guest(
        "Failed to get completed milestone".into()
    )))
}
