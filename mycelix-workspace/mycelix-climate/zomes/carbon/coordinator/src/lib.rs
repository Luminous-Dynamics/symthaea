...
// Commercial licensing: see COMMERCIAL_LICENSE.md at repository root//! Carbon Coordinator Zome
//!
...
pub fn create_carbon_footprint(input: CreateFootprintInput) -> ExternResult<Record> {
    ...
    get(action_hash, GetOptions::default())?.ok_or(wasm_error!(WasmErrorInner::Guest(
        "Failed to get created footprint".into()
    )))
}
...
pub fn verify_carbon_footprint(input: VerifyFootprintInput) -> ExternResult<Record> {
    let record = get(input.original_action_hash.clone(), GetOptions::default())?.ok_or(
        wasm_error!(WasmErrorInner::Guest("Footprint not found".into())),
    )?;
    ...
    let action_hash = update_entry(
        ...
        &EntryTypes::CarbonFootprint(updated),
    )?;
    ...
    get(action_hash, GetOptions::default())?.ok_or(wasm_error!(WasmErrorInner::Guest(
        "Failed to get updated footprint".into()
    )))
}
...
pub fn create_carbon_credit(input: CreateCreditInput) -> ExternResult<Record> {
    ...
    get(action_hash, GetOptions::default())?.ok_or(wasm_error!(WasmErrorInner::Guest(
        "Failed to get created credit".into()
    )))
}
...
pub fn transfer_credit(input: TransferCreditInput) -> ExternResult<Record> {
    let record = get(input.credit_action_hash.clone(), GetOptions::default())?.ok_or(
        wasm_error!(WasmErrorInner::Guest("Credit not found".into())),
    )?;
    ...
    let action_hash = update_entry(
        input.credit_action_hash.clone(),
        &EntryTypes::CarbonCredit(final_credit),
    )?;
    ...
    get(action_hash, GetOptions::default())?.ok_or(wasm_error!(WasmErrorInner::Guest(
        "Failed to get transferred credit".into()
    )))
}
...
pub fn retire_credit(input: RetireCreditInput) -> ExternResult<Record> {
    ...
    get(action_hash, GetOptions::default())?.ok_or(wasm_error!(WasmErrorInner::Guest(
        "Failed to get retired credit".into()
    )))
}

/// Get the transfer history for a credit
#[hdk_extern]
pub fn get_credit_transfer_history(
