...
// Commercial licensing: see COMMERCIAL_LICENSE.md at repository root//! P2P Grid Trading Integrity Zome
use hdi::prelude::*;
    ...
    match op.flattened::<EntryTypes, LinkTypes>()? {
        FlatOp::StoreEntry(store_entry) => match store_entry {
            OpEntry::CreateEntry { app_entry, action } => match app_entry {
                ...
                EntryTypes::EnergyProduction(production) => validate_create_energy_production(
                    EntryCreationAction::Create(action),
                    production,
                ),
                EntryTypes::EnergyConsumption(consumption) => validate_create_energy_consumption(
                    EntryCreationAction::Create(action),
                    consumption,
                ),
...
fn validate_create_energy_production(
    ...
    if !production.producer_did.starts_with("did:") {
        return Ok(ValidateCallbackResult::Invalid(
            "Producer must be a valid DID".into(),
        ));
    }
    if production.amount_kwh <= 0.0 {
        return Ok(ValidateCallbackResult::Invalid(
            "Production amount must be positive".into(),
        ));
    }
    Ok(ValidateCallbackResult::Valid)
}
...
fn validate_create_energy_consumption(
    ...
    if !consumption.consumer_did.starts_with("did:") {
        return Ok(ValidateCallbackResult::Invalid(
            "Consumer must be a valid DID".into(),
        ));
    }
    if consumption.amount_kwh <= 0.0 {
        return Ok(ValidateCallbackResult::Invalid(
            "Consumption amount must be positive".into(),
        ));
    }
    Ok(ValidateCallbackResult::Valid)
}
...
fn validate_update_trade(_action: Update, _trade: Trade) -> ExternResult<ValidateCallbackResult> {
