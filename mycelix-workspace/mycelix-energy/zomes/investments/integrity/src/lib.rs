...
// Commercial licensing: see COMMERCIAL_LICENSE.md at repository root//! Energy Investments Integrity Zome
use hdi::prelude::*;
    ...
    match op.flattened::<EntryTypes, LinkTypes>()? {
        FlatOp::StoreEntry(store_entry) => match store_entry {
            ...
            OpEntry::UpdateEntry {
                ...
                EntryTypes::Dividend(_) => Ok(ValidateCallbackResult::Invalid(
                    "Dividends cannot be updated".into(),
                )),
                EntryTypes::Portfolio(portfolio) => validate_update_portfolio(action, portfolio),
            },
...
fn validate_create_investment(
    ...
    if !investment.investor_did.starts_with("did:") {
        return Ok(ValidateCallbackResult::Invalid(
            "Investor must be a valid DID".into(),
        ));
    }
    ...
    if investment.share_percentage < 0.0 || investment.share_percentage > 100.0 {
        return Ok(ValidateCallbackResult::Invalid(
            "Share percentage must be 0-100".into(),
        ));
    }
    Ok(ValidateCallbackResult::Valid)
}
...
fn validate_create_dividend(
    ...
    if dividend.amount <= 0.0 {
        return Ok(ValidateCallbackResult::Invalid(
            "Dividend amount must be positive".into(),
        ));
    }
    Ok(ValidateCallbackResult::Valid)
}
