...
// Commercial licensing: see COMMERCIAL_LICENSE.md at repository root//! Regenerative Exit Integrity Zome
use hdi::prelude::*;
        ...
        FlatOp::StoreEntry(store_entry) => match store_entry {
            OpEntry::CreateEntry { app_entry, action } => match app_entry {
                ...
                EntryTypes::RegenerativeContract(contract) => {
                    validate_create_regenerative_contract(
                        EntryCreationAction::Create(action),
                        contract,
                    )
                }
                EntryTypes::ReadinessAssessment(assessment) => {
                    validate_create_readiness_assessment(
                        EntryCreationAction::Create(action),
                        assessment,
                    )
                }
                EntryTypes::OwnershipTransfer(transfer) => validate_create_ownership_transfer(
                    EntryCreationAction::Create(action),
                    transfer,
                ),
            },
            OpEntry::UpdateEntry {
                ...
                EntryTypes::ReadinessAssessment(_) => Ok(ValidateCallbackResult::Invalid(
                    "Assessments cannot be updated".into(),
                )),
                EntryTypes::OwnershipTransfer(_) => Ok(ValidateCallbackResult::Invalid(
...
fn validate_create_regenerative_contract(
    ...
    if !contract.community_did.starts_with("did:") {
        return Ok(ValidateCallbackResult::Invalid(
            "Community must be a valid DID".into(),
        ));
    }
    if contract.target_ownership_percentage <= contract.current_ownership_percentage {
        return Ok(ValidateCallbackResult::Invalid(
            "Target must exceed current ownership".into(),
        ));
    }
    if contract.conditions.is_empty() {
        return Ok(ValidateCallbackResult::Invalid(
            "Must have at least one condition".into(),
        ));
    }
    Ok(ValidateCallbackResult::Valid)
}
...
fn validate_create_ownership_transfer(
    ...
    if transfer.to_percentage <= transfer.from_percentage {
        return Ok(ValidateCallbackResult::Invalid(
            "Transfer must increase ownership".into(),
        ));
    }
    Ok(ValidateCallbackResult::Valid)
}
