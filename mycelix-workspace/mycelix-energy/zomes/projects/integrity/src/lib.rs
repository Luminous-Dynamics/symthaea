...
// Commercial licensing: see COMMERCIAL_LICENSE.md at repository root//! Energy Projects Integrity Zome
use hdi::prelude::*;
    ...
    match op.flattened::<EntryTypes, LinkTypes>()? {
        FlatOp::StoreEntry(store_entry) => match store_entry {
            OpEntry::CreateEntry { app_entry, action } => match app_entry {
                ...
                EntryTypes::ProjectMilestone(milestone) => validate_create_project_milestone(
...
fn validate_create_energy_project(
    ...
    if !project.developer_did.starts_with("did:") {
        return Ok(ValidateCallbackResult::Invalid(
            "Developer must be a valid DID".into(),
        ));
    }
