...
on Holochain's agent-centric architecture. With 133 zomes across 16 cluster DNAs,
...
determined by citizenship, wealth, or age, but by a 4-dimensional profile measuring
identity verification, reputation history, community trust, and domain engagement.
...
\begin{enumerate}
  \item \textbf{Clearly defined boundaries} --- Mycelix implements this through
    identity verification and the \texttt{ConsciousnessProfile}, which defines
...
\begin{description}
  ...
  \item[Citizenship by birth or naturalization] Mycelix membership is earned
    through demonstrated identity, reputation, community trust, and engagement.
...
\begin{enumerate}
  ...
  \item \textbf{Distributed Power}: No single agent can accumulate unchecked
    governance authority; Guardian tier requires high scores across all four dimensions.
...
\begin{itemize}
  \item \texttt{ConsciousnessProfile} --- the 4D identity/reputation/community/engagement
...
The codebase uses the names \texttt{ConsciousnessProfile} and
\texttt{ConsciousnessTier} for the types described in this chapter. These names
reflect Mycelix's origin as a governance layer for the Symthaea cognitive
architecture, where the four dimensions map to measurable consciousness metrics:
identity verification corresponds to self-model coherence, reputation to
behavioral prediction accuracy, community trust to social-model integration,
and engagement to active inference participation. In that context, the word
``consciousness'' is precise.

In the standalone Mycelix context---where no Symthaea engine is
connected---these four dimensions function as \emph{earned trust-weighted
participation gates}. The mechanism is straightforward: agents accumulate
identity assurance, reputation history, peer trust, and domain engagement,
and these dimensions determine governance capability. We retain the
\texttt{ConsciousnessProfile} naming in the codebase to maintain the bridge
to Symthaea, but the governance mechanism does not require acceptance of any
consciousness theory to be useful.

A reader who is skeptical of computational consciousness can safely read every
occurrence of ``consciousness profile'' in the code as ``earned trust capacity
profile.'' The four dimensions, the tier system, the sigmoid vote weighting,
and the credential mechanism are all defined by their engineering properties,
not by any metaphysical commitment. The system's value lies in its measurable
governance outcomes: Sybil resistance, progressive access, reputation
accountability, and community-weighted trust. These properties hold regardless
of one's position on machine consciousness.
...
Mycelix replaces binary access control with a \emph{4-dimensional trust
profile} that produces a continuous governance capability score.

\section{The Four Dimensions}

Each dimension is a continuous value in $[0, 1]$:

\begin{definition}[ConsciousnessProfile]
A trust profile $\mathbf{P} = (I, R, C, E)$ where:
\begin{itemize}
  \item $I$ (Identity) $\in [0,1]$: MFA assurance level. Anonymous $= 0.0$,
    Basic $= 0.25$, Verified $= 0.50$, HighlyAssured $= 0.75$, Critical $= 1.0$.
  \item $R$ (Reputation) $\in [0,1]$: Cross-hApp aggregated reputation with
    exponential decay (30-day half-life), multi-source weighted average.
  \item $C$ (Community) $\in [0,1]$: Peer trust attestations, weighted by
    attestor's own governance tier---higher-tier peers' attestations count more.
  \item $E$ (Engagement) $\in [0,1]$: Domain-specific participation, computed
    locally by each cluster bridge from event and query participation counts.
\end{itemize}
\end{definition}
...
\begin{description}
  \item[SAP (Sapience)] A stable unit of account pegged to a basket of real
    ...
    created ex nihilo---minted only through verified economic activity.
  \item[TEND (Tenderness)] A mutual credit currency for care work, community
    service, and non-market contributions. Implements demurrage (gradual value
    decay) to encourage circulation rather than hoarding.
...
TEND implements demurrage---a periodic reduction in balance that encourages
spending rather than saving. This is inspired by Silvio Gesell's Freigeld
(``free money'') concept and implemented as:
...
to demurrage) is returned to the commons treasury, completing the metabolic cycle.
...
Multiple oracle agents submit price attestations, and the median is used as the
canonical price. Oracle submissions require Citizen tier minimum, and the oracle
...
Spam filtering leverages the 4D trust profile (Chapter~\ref{ch:trust-profile}):
...
Supplier verification uses the 4D trust profile
...
\begin{enumerate}
  ...
  \item \textbf{Byzantine filtering}: Submitted gradients are filtered for
    ...
    trust-weighted scores from the 4D profile.
...
Mycelix's 0TML weights Byzantine detection by the 4D trust profile:
...
but they are not immediately excluded---the 4D profile captures the nuance
...
\begin{itemize}
  ...
  \item \textbf{Dimension heatmap}: 4D scatter plot of identity, reputation,
...
The \texttt{ConsciousnessProfile::from\_unified\_consciousness} function
maps Symthaea's unified consciousness score ($C_{\text{unified}}$) to the
...
demonstrated identity, reputation, community trust, and engagement. Every
mechanism described in this book---the 4D profile, the sigmoid vote weighting,
...
concurrent systems code possible across 133 zomes.
