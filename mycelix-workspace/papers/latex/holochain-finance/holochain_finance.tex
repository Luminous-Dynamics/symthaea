...
and non-inflationary; TEND implements Gesellian demurrage to encourage circulation;
...
\section{TEND: Mutual Credit with Demurrage}
...
\subsection{Demurrage}

TEND implements Gesellian demurrage~\cite{gesell1916}: balances decay
over time to encourage circulation:

\begin{equation}
  B(t) = B(t_0) \cdot e^{-\lambda(t - t_0)}
\end{equation}

where $\lambda$ is the demurrage rate. The ``composted'' value (the decay)
returns to the commons treasury:

\begin{equation}
  D(t_0, t) = B(t_0) - B(t) = B(t_0)(1 - e^{-\lambda(t - t_0)})
\end{equation}

\begin{proposition}[Demurrage zero-sum invariant]
With demurrage, the total TEND supply (positive + negative balances +
composted treasury) remains zero:
\begin{equation}
  \sum_{i} B_i(t) + T_{\text{compost}}(t) = 0
\end{equation}
\end{proposition}
...
\begin{itemize}[nosep]
  \item TEND: Demurrage automatically composts value over time
  \item SAP: Manual composting when backing goods are consumed
