    ...
    move || {
        ...
        match current_view {
            ...
            PageView::Page { html } => {
                ...
                //
                // SECURITY: `html` originates from externally-fetched, proxy-relayed
                // pages. The CORS proxy's SSRF guard only restricts *where* content
                // is fetched from, not *what* it contains, so it must never reach
                // `inner_html` unsanitized. `annotate_html` sanitizes its own output,
                // but it can be skipped here (overlay disabled, or no search engine
                // available yet) — sanitize explicitly on every path so this doesn't
                // depend on an upstream sanitization invariant holding forever.
