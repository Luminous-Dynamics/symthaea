...
///
/// `pub(crate)` so other modules that build `inner_html` from externally-fetched
/// content (e.g. `components::sentient_overlay`) can reuse the same allowlist
/// instead of re-implementing sanitization or trusting proxy-relayed HTML as-is.
pub(crate) fn sanitize_html(html: &str, base_url: Option<&url::Url>) -> String {
    let mut builder = ammonia::Builder::new();

    // Allow the inline `style` attribute on elements for layout fidelity.
    // Note: this is the *attribute* (`style="..."`), not the `<style>` *tag*.
    // `<style>` blocks are intentionally left out of `add_tags` below —
    // ammonia's `Builder::new()` defaults `clean_content_tags` to
    // `{"script", "style"}`, which strips `<style>` tags (and their content)
    // entirely. Explicitly allowlisting the `<style>` *tag* via `add_tags`
    // conflicts with that default blacklist and panics
    // (`ammonia::Builder::clean_dom` asserts a tag can't be in both
    // `clean_content_tags` and `tags`), so we rely on the default blacklist
    // to drop page-supplied `<style>` blocks (and any dangerous CSS therein)
    // rather than trying to allow and sanitize them.
