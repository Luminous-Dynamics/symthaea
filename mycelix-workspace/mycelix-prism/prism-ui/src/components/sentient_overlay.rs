...
///
/// SECURITY: the input `html` originates from externally-fetched, proxy-relayed
/// pages. The proxy's SSRF guard (`prism_common::ssrf`) only restricts *where*
/// content is fetched from — it says nothing about *what* the fetched page
/// contains. The result of this function is injected into the DOM via Leptos's
/// `inner_html` (see `AnnotatedContent` below and `pages::content_router`), so
/// it must never carry live markup (`<script>`, `onerror=`, etc.) through to
/// the caller. We therefore run the same allowlist sanitizer used for the
/// initial page render (`engine::sanitize_html`) over the final output as a
/// second, independent pass — this holds even if the caller's own
/// sanitization is ever bypassed, changed, or forgotten upstream.
pub fn annotate_html(html: &str, engine: &SearchEngine) -> String {
    ...
    let combined = format!("{}\n{}", html, overlay_html);
    crate::engine::sanitize_html(&combined, None)
}
...
/// Escape the five HTML-significant characters. Applied to any dynamic text
/// (page sentences, claim content, source strings) interpolated into the
/// overlay fragment above — these values can originate from externally
/// fetched, proxy-relayed pages and must not be able to break out of the
/// surrounding markup. The final `annotate_html` output is additionally run
/// through the allowlist sanitizer (`engine::sanitize_html`) as a second,
