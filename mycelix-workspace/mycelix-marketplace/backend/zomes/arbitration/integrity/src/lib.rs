...
/// Arbitrator registration - an agent opting into the arbitrator pool
///
/// Agents self-register to be eligible for arbitrator assignment. Eligibility
/// (MATL score > 0.7) is re-checked live against the reputation zome at
/// assignment time, not trusted from this entry, since scores drift over time.
#[hdk_entry_helper]
#[derive(Clone, PartialEq)]
pub struct ArbitratorRegistration {
    /// Agent registering as an available arbitrator
...
pub enum LinkTypes {
    ...
    /// All registered arbitrators (pool anchor -> ArbitratorRegistration)
    AllArbitrators,
}
...
pub enum EntryTypes {
    ...
    ArbitratorRegistration(ArbitratorRegistration),
}
    ...
    match op.flattened::<EntryTypes, LinkTypes>()? {
        FlatOp::StoreEntry(store_entry) => match store_entry {
            OpEntry::CreateEntry { app_entry, .. } => match app_entry {
                ...
                EntryTypes::ArbitratorRegistration(reg) => validate_arbitrator_registration(&reg),
            },
...
fn validate_vote(vote: &ArbitrationVote) -> ExternResult<ValidateCallbackResult> {
    // Self-contained field checks (reasoning, MATL score range).
    if let ValidateCallbackResult::Invalid(reason) = validate_vote_fields(vote) {
        return Ok(ValidateCallbackResult::Invalid(reason));
    }

    // Self-dealing exclusion: an arbitrator must not be a party to (or the
    // filer of) the dispute they are voting on. This must be enforced here
    // (integrity validation) rather than only in coordinator-side assignment
    // logic, since assignment can be bypassed or stale by the time a vote
    // lands on the DHT. Fetching the dispute is a deterministic dependency
    // lookup (must_get_valid_record), so it's safe inside validation.
    ...
    let dispute: Dispute = dispute_record
        ...
        .map_err(|e| {
            wasm_error!(WasmErrorInner::Guest(format!(
                "Failed to deserialize dispute for vote validation: {:?}",
                e
            )))
        })?
        .ok_or(wasm_error!(WasmErrorInner::Guest(
            "Dispute entry not found for vote validation".into()
        )))?;

    Ok(validate_vote_not_self_dealing(vote, &dispute))
}

/// Pure field-shape checks for a vote (reasoning length, MATL score range).
/// Split out from `validate_vote` so it can be unit tested without an
/// HDI/conductor context (`must_get_valid_record`, used for the self-dealing
/// check, requires a live host and cannot run in plain `#[test]`s).
fn validate_vote_fields(vote: &ArbitrationVote) -> ValidateCallbackResult {
    // Reasoning must not be empty
    if vote.reasoning.trim().is_empty() {
        return ValidateCallbackResult::Invalid("Vote reasoning cannot be empty".into());
    }
    ...
    if vote.arbitrator_matl_score < 0.0 || vote.arbitrator_matl_score > 1.0 {
        return ValidateCallbackResult::Invalid("Invalid MATL score (must be 0.0-1.0)".into());
    }

    ValidateCallbackResult::Valid
}

/// Pure check that an arbitrator is not a party to (or filer of) the dispute
/// they are voting on.
fn validate_vote_not_self_dealing(
    ...
    {
        return ValidateCallbackResult::Invalid(
            "Arbitrator cannot be a party to (or the filer of) the dispute they are voting on"
                .into(),
        );
    }
...
fn validate_arbitrator_registration(
    reg: &ArbitratorRegistration,
) -> ExternResult<ValidateCallbackResult> {
    // Nothing to validate beyond entry shape today: registration merely
    // signals willingness to arbitrate. Live MATL-score eligibility (>0.7)
    // and self-dealing exclusion are enforced at assignment time (coordinator)
    // and at vote time (this zome's validate_vote), not here, since the
    // registration entry has no dispute context and MATL scores drift.
    let _ = reg;
    Ok(ValidateCallbackResult::Valid)
}
...
mod tests {
    ...
    fn test_validate_vote_valid() {
        let vote = valid_vote();
        let result = validate_vote_fields(&vote);
        assert_eq!(result, ValidateCallbackResult::Valid);
    }
    ...
    // ===== Self-Dealing Exclusion Tests =====

    #[test]
    fn test_vote_not_self_dealing_valid_third_party() {
        let dispute = valid_dispute(); // buyer=2, seller=3, filed_by=2
        let vote = ArbitrationVote {
            arbitrator: mock_agent(4),
            ..valid_vote()
        };
        let result = validate_vote_not_self_dealing(&vote, &dispute);
        assert_eq!(result, ValidateCallbackResult::Valid);
    }

    #[test]
    fn test_vote_rejects_buyer_as_arbitrator() {
        let dispute = valid_dispute(); // buyer=2
        let vote = ArbitrationVote {
            arbitrator: mock_agent(2),
            ..valid_vote()
        };
        let result = validate_vote_not_self_dealing(&vote, &dispute);
        assert!(matches!(result, ValidateCallbackResult::Invalid(_)));
    ...
    #[test]
    fn test_vote_rejects_seller_as_arbitrator() {
        let dispute = valid_dispute(); // seller=3
        let vote = ArbitrationVote {
    ...
    #[test]
    fn test_vote_rejects_filer_as_arbitrator() {
        // Filer distinct from buyer/seller identity check (seller filed here)
        let dispute = Dispute {
            filed_by: mock_agent(3),
            ..valid_dispute()
        };
    ...
    #[test]
    fn test_validate_arbitrator_registration_valid() {
        let reg = ArbitratorRegistration {
            agent: mock_agent(9),
            registered_at: Timestamp::from_micros(1000000),
        };
        let result = validate_arbitrator_registration(&reg).unwrap();
        assert_eq!(result, ValidateCallbackResult::Valid);
    }

    #[test]
    fn test_arbitrator_registration_serde_roundtrip() {
        let reg = ArbitratorRegistration {
            agent: mock_agent(9),
            registered_at: Timestamp::from_micros(1000000),
        };
        let json = serde_json::to_string(&reg).unwrap();
        let parsed: ArbitratorRegistration = serde_json::from_str(&json).unwrap();
        assert_eq!(parsed.agent, reg.agent);
    }
}
