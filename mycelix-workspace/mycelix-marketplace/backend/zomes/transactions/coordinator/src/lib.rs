...
/// Only the seller may confirm (they are the one agreeing to fulfill the order).
#[hdk_extern]
...
/// Only the seller may mark as shipped (they are the one dispatching the goods).
#[hdk_extern]
...
/// Only the buyer may confirm delivery (they are the one receiving the goods;
/// this is the buyer's attestation that the order actually arrived).
#[hdk_extern]
...
/// Which party to a transaction must be the caller for a given state transition.
...
///
/// Verifies both that the state transition is legal (`allowed_from_states`)
/// and that the caller is the correct party for that transition
/// (`required_caller`), mirroring the ownership checks already performed by
/// `dispute_transaction` and `cancel_transaction`.
fn update_transaction_status(
    transaction_hash: ActionHash,
    new_status: TransactionStatus,
    tracking_info: Option<String>,
    allowed_from_states: Vec<TransactionStatus>,
    required_caller: TransactionParty,
    ...
    // Verify caller is the correct party for this transition
    let agent_info = agent_info()?;
    let caller = agent_info.agent_initial_pubkey;
    let expected = match required_caller {
        TransactionParty::Buyer => &current.transaction.buyer,
        TransactionParty::Seller => &current.transaction.seller,
    };
    if caller != *expected {
        return Err(wasm_error!(WasmErrorInner::Guest(format!(
            "Only the {} can perform this transition",
            match required_caller {
                TransactionParty::Buyer => "buyer",
                TransactionParty::Seller => "seller",
            }
        ))));
    }
