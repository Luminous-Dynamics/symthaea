...
# Commercial licensing: see COMMERCIAL_LICENSE.md at repository root# Mycelix Emergency - Holochain Development Environment
# Offline-first emergency coordination hApp
#
# Usage:
#   nix develop              # Enter dev shell
#   nix develop .#ci         # CI environment (minimal)
{
  description = "Mycelix Emergency - Offline-first disaster coordination on Holochain";
      ...
      in {
        devShells = {
          default = holochainBase.mkHolochainShell {
            ...
            extraShellHook = ''
              echo "Zomes:"
              echo "  incidents/       - Disaster declaration & lifecycle"
              echo "  triage/          - Mass casualty triage (START)"
              echo "  resources/       - Resource tracking & deployment"
              echo "  coordination/    - Teams, zones, SITREPs"
              echo "  shelters/        - Shelter registration & occupancy"
              echo "  comms/           - Offline-first emergency messaging"
