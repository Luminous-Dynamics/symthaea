...
// Commercial licensing: see COMMERCIAL_LICENSE.md at repository root//! # Comprehensive Identity Module Integration Tests
//!
...
mod did_registry_tests {
    ...
    async fn test_create_did_success() {
        ...
        assert_eq!(
            did_doc.id, expected_did,
            "DID should be deterministic from agent key"
        );
        assert_eq!(
            did_doc.controller, alice_agent,
            "Controller must be the creating agent"
        );
        assert!(
    ...
    async fn test_resolve_did() {
        ...
        assert!(
            resolved_alice.is_some(),
            "Should resolve from creator's cell"
        );
    ...
    async fn test_invalid_did_format_rejected() {
        ...
        for (invalid_did, description) in invalid_dids {
            ...
            match result {
                ...
                Ok(Some(_)) => {
                    panic!(
                        "Should not resolve invalid DID: {} ({})",
                        invalid_did, description
                    );
                }
            }
        }
    ...
    async fn test_get_registered_happs() {
        ...
        let happs_to_register = vec![
            (
                "finance-happ",
                "Finance Module",
                vec!["payments", "lending"],
            ),
            (
                "governance-happ",
                "Governance Module",
                vec!["voting", "proposals"],
            ),
            (
                "energy-happ",
                "Energy Module",
                vec!["projects", "investments"],
            ),
        ];
        ...
        for (id, name, caps) in &happs_to_register {
            ...
            let _: Record = conductor
                .call(&alice_cell.zome("identity_bridge"), "register_happ", input)
                .await;
        }
...
mod unit_tests {
    ...
    fn test_bridge_event_type_serialization() {
        ...
        for event_type in event_types {
            ...
            let deserialized: BridgeEventType =
                serde_json::from_str(&json).expect("Deserialize failed");
    ...
    fn test_schema_category_serialization() {
        ...
        for category in categories {
            ...
            let deserialized: SchemaCategory =
                serde_json::from_str(&json).expect("Deserialize failed");
