...
pub fn verify_commitment_cli(credential_path: &Path, nonce_b64: &str) -> Result<()> {
    ...
    let expected =
        generate_zk_commitment(&credential.id, &credential.credential_subject.id, &nonce);
...
pub async fn verify_credential_cli(
    ...
    println!();
    println!("{}", style("=== Credential Verification ===").bold().cyan());
    println!();
            ...
            .contains(&"AcademicCredential".to_string())
    {
        ...
        println!("  {} Missing required credential types", style("✗").red());
        checks_failed += 1;
    }
    ...
    if credential.proof.proof_value.starts_with('z') {
        ...
        println!("  {} Proof value not multibase-encoded", style("✗").red());
        checks_failed += 1;
    }
    ...
    match BASE64.decode(&credential.zk_commitment) {
        ...
        Err(_) => {
            println!("  {} Failed to decode ZK commitment", style("✗").red());
            checks_failed += 1;
        }
    }
    ...
    println!("  Domain:     {}", credential.dns_did.domain);
        ...
        .await
        {
            Ok(verification) => {
                if verification.verified {
                    println!("  {} DNS-DID verified live", style("✓").green());
                    checks_passed += 1;
                } else {
                    println!(
            ...
            Err(e) => {
                println!("  {} DNS lookup failed: {}", style("⚠").yellow(), e);
                warnings += 1;
            }
        }
    } else {
    ...
    if warnings > 0 {
        println!("  Warnings:       {}", style(warnings).yellow().bold());
    }
    if checks_failed > 0 {
        println!("  Checks failed:  {}", style(checks_failed).red().bold());
    }
    println!();
    ...
    if checks_failed == 0 {
        ...
        println!("  {}", style("Credential has ERRORS").red().bold());
    }
...
mod tests {
    ...
    fn test_zk_commitment() {
        ...
        let commitment = generate_zk_commitment("urn:uuid:123", "did:student:test", nonce);
        assert_eq!(commitment.len(), 32); // SHA-256

        // Same inputs should produce same output
        let commitment2 = generate_zk_commitment("urn:uuid:123", "did:student:test", nonce);
        assert_eq!(commitment, commitment2);

        // Different inputs should produce different output
        let commitment3 = generate_zk_commitment("urn:uuid:456", "did:student:test", nonce);
