...
// Commercial licensing: see COMMERCIAL_LICENSE.md at repository root//! CSV Parser for Legacy Academic Records
//!
...
pub fn parse_csv(path: &Path, mapping: &FieldMapping) -> Result<ParseResult> {
    ...
    for (col, name) in &required {
        if !header_map.contains_key(col.as_str()) {
            anyhow::bail!(
                "Required column '{}' (mapped to {}) not found in CSV",
                col,
                name
            );
        }
...
mod tests {
    ...
    fn test_parse_valid_csv() {
        ...
        writeln!(
            file,
            "student_id,first_name,last_name,degree_name,major,conferral_date,gpa"
        )
        .unwrap();
        writeln!(
            file,
            "STU001,Jane,Doe,Bachelor of Science,CS,2024-05-15,3.85"
        )
        .unwrap();
    ...
    fn test_parse_missing_required() {
        ...
        writeln!(
            file,
            "student_id,first_name,last_name,degree_name,major,conferral_date"
        )
        .unwrap();
