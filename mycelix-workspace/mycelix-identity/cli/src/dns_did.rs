...
const RESOLVER_CHAIN: &[(&str, fn() -> ResolverConfig)] = &[
    ...
    ("9.9.9.9 (Quad9)", || {
        ...
        let quad9_v6 = NameServerConfig::new(
            SocketAddr::new(IpAddr::V6("2620:fe::fe".parse().unwrap()), 853),
            Protocol::Tls,
        );
...
pub async fn verify_dns_did(
    ...
    let mut resolver_used = RESOLVER_CHAIN[0].0;

    for (name, config_fn) in RESOLVER_CHAIN {
        let resolver = TokioAsyncResolver::tokio(config_fn(), opts.clone());
        ...
        match resolver.txt_lookup(&txt_name).await {
            Ok(response) => {
                ...
                if verbose {
                    println!("  {} Resolved via {}", style("✓").green(), name);
                }
                break;
            }
            Err(e) => {
    ...
    if verbose {
        ...
        match dnssec_status {
            DnssecStatus::Validated => {
                println!("  {} DNS response validated via DNSSEC", style("✓").green());
            }
            DnssecStatus::Insecure => {
                println!("  {} DNSSEC present but not validated", style("⚠").yellow());
            }
            DnssecStatus::Invalid => {
                println!("  {} DNSSEC validation failed", style("✗").red());
            }
...
pub fn print_setup_instructions(domain: &str, did: &str) {
    ...
    println!(
        "{}",
        style("=== DNS-DID Setup Instructions ===").bold().cyan()
    );
    println!();
    ...
    println!("  For maximum security, enable DNSSEC for your domain.");
    println!("  This provides cryptographic proof that DNS records are authentic.");
    println!();
