...
// Commercial licensing: see COMMERCIAL_LICENSE.md at repository root//! CSV Import Orchestration
//!
...
use crate::credential::{AcademicCredential, CredentialConfig, build_credential};
...
pub async fn run_import(config: ImportConfig) -> Result<()> {
    ...
    if config.dry_run {
        println!(
            "{}",
            style("  ⚠ DRY RUN MODE - No credentials will be created")
                .yellow()
                .bold()
        );
        println!();
    ...
    println!();
    println!("{} Loading configuration...", style("[1/6]").bold().dim());
    ...
    if !parse_result.errors.is_empty() {
        println!("  Parse errors: {}", style(parse_result.errors.len()).red());
    }
        ...
        match dns_did::verify_dns_did(domain, Some(&config.institution_did), false).await {
            Ok(verification) => {
                if verification.verified {
                    println!("  DNS-DID verified: {}", style("✓").green().bold());
                } else {
                    println!(
    ...
    println!();
    println!("{} Generating credentials...", style("[5/6]").bold().dim());
    ...
    println!();
    println!("{} Finalizing import...", style("[6/6]").bold().dim());
    ...
    println!();
    println!("  Batch ID:          {}", style(&batch_id).green().bold());
    println!("  Institution:       {}", config.institution_name);
    ...
    println!("  Successful:        {}", style(successful).green().bold());
    println!("  Failed:            {}", style(batch.failed).red().bold());
    println!();
...
pub async fn show_batch_status(batch_id: &str) -> Result<()> {
    ...
    println!("To check on-chain status, use the Holochain admin API or Observatory.");
