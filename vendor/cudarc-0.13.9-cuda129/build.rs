...
fn main() {
    ...
    let (major, minor): (usize, usize) = if cfg!(feature = "cuda-12080") {
        ...
        panic!(
            "Must specify one of the following features: [cuda-version-from-build-system, cuda-12080, cuda-12060, cuda-12050, cuda-12040, cuda-12030, cuda-12020, cuda-12010, cuda-12000, cuda-11080, cuda-11070, cuda-11060, cuda-11050, cuda-11040]"
        );
...
fn cuda_version_from_build_system() -> (usize, usize) {
    ...
    match version_number {
        // Luminous-Dynamics local patch: alias CUDA 12.9 to (12, 8).
        //
        // cudarc 0.13.9 has per-CUDA-version FFI bindings in src/*/sys/
        // and no sys_12090.rs module exists (the hand-maintained list
        // stopped at 12.8). Returning (12, 9) would activate the
        // `cuda-12090` cfg feature and break compilation at 425
        // `undeclared type Lib` errors.
        //
        // CUDA 12.x is ABI-backward-compatible, so telling cudarc the
        // toolkit is 12.8 makes it reuse the sys_12080 bindings against
        // the 12.9 runtime — which works in practice (the CUDA Runtime
        // API is additive within a major version).
        //
        // Cleanly unblocked when we bump candle-core to a release that
        // takes cudarc 0.18+, which has proper 12.9 support.
        "12.9" => (12, 8),
        "12.8" => (12, 8),
        "12.6" => (12, 6),
        "12.5" => (12, 5),
