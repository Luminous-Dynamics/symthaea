#!/usr/bin/env python3
"""Independently qualify a prepared Matter Reference Capsule environment.

This verifier is intentionally post-bootstrap and read-only. It establishes only
execution-environment identity/readiness. It does not install a pseudopotential,
submit an AiiDA process, invoke a Quantum ESPRESSO scientific calculation, or
establish any materials-science result.
"""
...
def nix_store_output_for_executable(path: Path) -> Path:
    resolved = path.resolve()
    require(str(resolved).startswith("/nix/store/"), f"executable is not from the Nix store: {resolved}")
    require(resolved.parent.name == "bin", f"unexpected Nix executable layout: {resolved}")
    output = resolved.parent.parent
    require(output.parent == Path("/nix/store"), f"unexpected Nix store output: {output}")
    return output
...
def nix_output_evidence(output: Path) -> dict[str, str]:
    ...
    require(deriver.endswith(".drv") and deriver.startswith("/nix/store/"), f"invalid Nix deriver: {deriver}")
...
def main() -> int:
    ...
    environment_manifest_path = state_root / "environment-manifest.json"
    ...
    require(environment_manifest_path.is_file(), "bootstrap environment manifest is missing")
    ...
    require(lock_source["candidate_uv_lock_sha256"] == lock_digest, "source orchestration lock digest mismatch")
    ...
    generator_tooling = lock_source.get("tooling", {})
    lock_generator = {"python": generator_tooling.get("python"), "uv": generator_tooling.get("uv")}
    require(lock_generator == EXPECTED_LOCK_GENERATOR, f"lock generator identity drift: {lock_generator}")

    # Prove the committed lock is accepted read-only by the actual Nix-supplied
    # consumer. This intentionally distinguishes lock generation from lock use.
    uv_path_text = shutil.which("uv")
    require(uv_path_text is not None, "uv is unavailable after bootstrap")
    uv_path = Path(uv_path_text).resolve()
    uv_version = run(str(uv_path), "--version")
    require(uv_version == EXPECTED_UV_CONSUMER_VERSION, f"unexpected uv consumer version: {uv_version}")
    uv_store = nix_store_output_for_executable(uv_path)
    uv_evidence = nix_output_evidence(uv_store)
    uv_evidence.update({"executable": str(uv_path), "executable_sha256": sha256(uv_path), "version": uv_version})

    run(str(uv_path), "lock", "--check", cwd=capsule)
    require(sha256(lock) == lock_digest, "uv lock --check changed the committed lock")

    root_node = flake_lock["nodes"][flake_lock["root"]]
    nixpkgs_node_name = root_node["inputs"]["nixpkgs"]
    nixpkgs_locked = flake_lock["nodes"][nixpkgs_node_name]["locked"]
    ...
    require(
        os.environ.get("INSTALL_NIX_ACTION_SHA") == EXPECTED_INSTALL_NIX_ACTION,
        "install-nix action identity changed",
    )

    qe_store = Path(os.environ["SYMTHAEA_QE_STORE"]).resolve()
    python_executable = Path(os.environ["SYMTHAEA_PYTHON"]).resolve()
    require(qe_store.is_dir() and str(qe_store).startswith("/nix/store/"), "QE is not a Nix store output")
    require(python_executable.is_file() and str(python_executable).startswith("/nix/store/"), "Python is not from Nix")

    python_version_output = run(str(python_executable), "--version")
    require(
        python_version_output == f"Python {EXPECTED_PYTHON_VERSION}",
        f"unexpected Nix Python version: {python_version_output}",
    )
    resolved_verifier_python = Path(sys.executable).resolve()
    require(resolved_verifier_python == python_executable, "venv Python does not resolve to the pinned Nix interpreter")
    require(sys.version.split()[0] == EXPECTED_PYTHON_VERSION, "verifier is running under an unexpected Python version")

    python_store = nix_store_output_for_executable(python_executable)
    python_evidence = nix_output_evidence(python_store)
    ...
    qe_store_nar_hash = run("nix-store", "--query", "--hash", str(qe_store))
    qe_deriver = run("nix-store", "--query", "--deriver", str(qe_store))
    require(qe_deriver.endswith(".drv") and qe_deriver.startswith("/nix/store/"), "QE deriver is not fixed in the store")

    python_distributions = {name: importlib.metadata.version(name) for name in EXPECTED_PYTHON_DISTRIBUTIONS}
    require(
        python_distributions == EXPECTED_PYTHON_DISTRIBUTIONS,
        f"frozen Python distribution identity drift: {python_distributions}",
    )

    require(environment_manifest["authority"] == "ExecutionEnvironmentPreparedOnly", "bootstrap authority drift")
    require(environment_manifest["profile"] == args.profile, "bootstrap profile mismatch")
    require(Path(environment_manifest["state_root"]).resolve() == state_root, "bootstrap state-root mismatch")
    require(environment_manifest["quantum_espresso_store"] == str(qe_store), "bootstrap QE store mismatch")
    require(environment_manifest["python"]["version"] == EXPECTED_PYTHON_VERSION, "bootstrap Python version mismatch")
    require(
        Path(environment_manifest["python"]["executable"]).resolve() == python_executable,
        "bootstrap Python executable does not resolve to the pinned Nix interpreter",
    )
    for distribution in ("aiida-core", "aiida-quantumespresso"):
        require(
            environment_manifest["python_distributions"][distribution] == python_distributions[distribution],
            f"bootstrap distribution mismatch: {distribution}",
        )
    require(environment_manifest["caching"] == "disabled", "bootstrap caching claim changed")
    require(environment_manifest["broker"] == "none", "bootstrap broker claim changed")

    executable_evidence: dict[str, dict[str, Any]] = {}
    for executable_name in ("pw.x", "ph.x", "q2r.x", "matdyn.x"):
        evidence = environment_manifest["executables"][executable_name]
        path = Path(evidence["path"]).resolve()
        require(path.is_file(), f"missing QE executable: {executable_name}")
        require(inside(path, qe_store), f"{executable_name} escaped the QE store output")
        digest = sha256(path)
        require(digest == evidence["sha256"], f"{executable_name} byte digest mismatch")
        require(os.access(path, os.X_OK), f"{executable_name} is not executable")
        executable_evidence[executable_name] = {"path": str(path), "sha256": digest}
    ...
    # Import only after all source/runtime identity checks above have passed.
    ...
    qualification = {
        ...
        "source": {
            ...
            "nixpkgs_node": nixpkgs_node_name,
        ...
        "runtime": {
            ...
            "lock_consumer": {
                ...
                "uv_lock_sha256_after_check": sha256(lock),
            },
            "shell_driver": shell_driver,
            "qe_store": str(qe_store),
            "qe_store_name": qe_store.name,
            "qe_store_nar_hash": qe_store_nar_hash,
            "qe_deriver": qe_deriver,
    ...
    args.output.write_text(
        json.dumps(qualification, sort_keys=True, separators=(",", ":"), allow_nan=False) + "\n",
        encoding="utf-8",
    )
    reparsed = load_json(args.output)
    require(reparsed["authority"] == "ExecutionEnvironmentPreparedQualifiedV1", "qualification receipt self-check failed")
    require(reparsed["aiida"]["process_node_count"] == 0, "qualification receipt process-count self-check failed")
    require(
        reparsed["runtime"]["lock_consumer"]["python_distributions"] == EXPECTED_PYTHON_DISTRIBUTIONS,
        "qualification receipt distribution self-check failed",
    )
    require(reparsed["source"]["lock_generator"] == EXPECTED_LOCK_GENERATOR, "qualification receipt generator self-check failed")
