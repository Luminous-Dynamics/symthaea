...
fn test_da_d1_d2_balance_after_warmup() {
    ...
    assert!(
        m.dopamine_effective >= 0.0 && m.dopamine_effective <= 2.0,
    ...
    assert!(
        m.neuromod_gradient_scale >= 0.5 && m.neuromod_gradient_scale <= 2.0,
        "neuromod_gradient_scale out of range: {}",
        m.neuromod_gradient_scale
    );

    // threshold_gate derives from ACh: should be in [0.5, 1.5]
    assert!(
        m.neuromod_threshold_gate >= 0.5 && m.neuromod_threshold_gate <= 1.5,
        "neuromod_threshold_gate out of range: {}",
        m.neuromod_threshold_gate
    );
    ...
    for i in 0..20 {
        ...
        gradient_scales.push(r.metadata.neuromod_gradient_scale);
        threshold_gates.push(r.metadata.neuromod_threshold_gate);
    }
...
fn test_ne_phasic_on_surprise() {
    ...
    for input in &novel_inputs {
        let r = service.cycle(input);
        ne_phasic_values.push(r.metadata.neuromod_ne_phasic);
    }
...
fn test_ach_plasticity_gate_varies() {
    ...
    for _ in 0..10 {
        for input in &inputs {
            let r = service.cycle(input);
            plasticity_values.push(r.metadata.neuromod_plasticity_gate);
        }
    }
...
fn test_serotonin_and_behavioral_flexibility() {
    ...
    for i in 0..30 {
        ...
        let r = service.cycle(input);
        sht_values.push(r.metadata.serotonin_effective);
        flexibility_values.push(r.metadata.neuromod_behavioral_flexibility);
    }
...
fn test_neuromod_snapshot_fires_every_10_cycles() {
    ...
    for i in 1..=25 {
        ...
        if r.metadata.neuromod_snapshot.is_some() {
...
fn test_personality_drift_populated() {
    ...
    assert!(
        m.neuromod_personality_drift.is_finite(),
        "neuromod_personality_drift must be finite, got {}",
        m.neuromod_personality_drift
    );
    assert!(
        m.neuromod_personality_drift >= 0.0,
    ...
    assert!(
        !m.neuromod_personality.is_empty(),
    ...
    let _anomalous = m.neuromod_personality_drift_anomalous;
}
...
fn test_mcts_exploration_modulation_varies() {
    ...
    for _ in 0..8 {
        for input in &inputs {
            let r = service.cycle(input);
            mcts_mods.push(r.metadata.neuromod_mcts_exploration_mod);
        }
    }
...
fn test_attention_allocation_populated() {
    ...
    for _ in 0..20 {
        ...
        attention_values.push(r.metadata.neuromod_attention_allocation);
    }
...
fn test_receptor_subtypes_all_finite() {
    ...
    assert!(
        m.neuromod_da_d1.is_finite() && m.neuromod_da_d1 >= 0.0 && m.neuromod_da_d1 <= 2.0,
    ...
    assert!(
        m.neuromod_da_d2.is_finite() && m.neuromod_da_d2 >= 0.0 && m.neuromod_da_d2 <= 2.0,
    ...
    assert!(
        m.neuromod_ne_alpha.is_finite() && m.neuromod_ne_alpha >= 0.0 && m.neuromod_ne_alpha <= 2.0,
    ...
    assert!(
        m.neuromod_ne_beta.is_finite() && m.neuromod_ne_beta >= 0.0 && m.neuromod_ne_beta <= 2.0,
...
fn test_consciousness_modulation_populated() {
    ...
    for _ in 0..20 {
        ...
        consciousness_mods.push(r.metadata.neuromod_consciousness_mod);
    }
...
fn test_da_phasic_burst_telemetry() {
    ...
    for _ in 0..30 {
        ...
        da_phasic_values.push(r.metadata.neuromod_da_phasic);
    }
...
fn test_exocortex_query_suggestion() {
    ...
    for _ in 0..30 {
        ...
        if r.metadata.exocortex_query_suggested {
            any_true = true;
        } else {
            any_false = true;
        }
...
fn test_sleep_consolidation_boost_range() {
    ...
    for _ in 0..20 {
        ...
        boost_values.push(r.metadata.neuromod_sleep_consolidation_boost);
    }
...
fn test_neuromod_100_cycle_stability() {
    ...
    for i in 0..100 {
        ...
        assert!(
            m.noradrenaline_effective >= 0.0 && m.noradrenaline_effective <= 2.0,
            "cycle {i}: noradrenaline_effective={} out of [0,2]",
            m.noradrenaline_effective
        );
        assert!(
            m.serotonin_effective >= 0.0 && m.serotonin_effective <= 2.0,
        ...
        assert!(
            m.neuromod_plasticity_gate >= 0.2 && m.neuromod_plasticity_gate <= 1.0,
            "cycle {i}: plasticity_gate={} out of [0.2,1.0]",
            m.neuromod_plasticity_gate
        );
        assert!(
            m.neuromod_consciousness_mod >= 0.6 && m.neuromod_consciousness_mod <= 1.2,
            "cycle {i}: consciousness_mod={} out of [0.6,1.2]",
            m.neuromod_consciousness_mod
        );
        assert!(
            m.neuromod_mcts_exploration_mod >= 0.6 && m.neuromod_mcts_exploration_mod <= 1.8,
            "cycle {i}: mcts_exploration_mod={} out of [0.6,1.8]",
            m.neuromod_mcts_exploration_mod
        );
        assert!(
            m.neuromod_behavioral_flexibility >= 0.7 && m.neuromod_behavioral_flexibility <= 1.5,
            "cycle {i}: behavioral_flexibility={} out of [0.7,1.5]",
            m.neuromod_behavioral_flexibility
        );
        assert!(
            m.neuromod_attention_allocation >= 0.8 && m.neuromod_attention_allocation <= 1.5,
            "cycle {i}: attention_allocation={} out of [0.8,1.5]",
            m.neuromod_attention_allocation
        );
        assert!(
            m.neuromod_threshold_gate >= 0.5 && m.neuromod_threshold_gate <= 1.5,
        ...
        assert!(
            m.neuromod_sleep_consolidation_boost >= 1.0
                && m.neuromod_sleep_consolidation_boost <= 3.0,
            "cycle {i}: sleep_consolidation_boost={} out of [1.0,3.0]",
            m.neuromod_sleep_consolidation_boost
        );
    }
}
...
fn test_neuromod_snapshot_internal_consistency() {
    ...
    let da_diff = (snap.da_effective - r.metadata.dopamine_effective).abs();
    let ne_diff = (snap.ne_effective - r.metadata.noradrenaline_effective).abs();
    let sht_diff = (snap.sht_effective - r.metadata.serotonin_effective).abs();
    let ach_diff = (snap.ach_effective - r.metadata.acetylcholine_effective).abs();

    assert!(
        da_diff < 0.001,
        "Snapshot DA mismatch: snap={}, meta={}",
        snap.da_effective,
        r.metadata.dopamine_effective
    );
    assert!(
        ne_diff < 0.001,
        "Snapshot NE mismatch: snap={}, meta={}",
        snap.ne_effective,
        r.metadata.noradrenaline_effective
    );
    assert!(
        sht_diff < 0.001,
        "Snapshot 5-HT mismatch: snap={}, meta={}",
        snap.sht_effective,
        r.metadata.serotonin_effective
    );
    assert!(
        ach_diff < 0.001,
        "Snapshot ACh mismatch: snap={}, meta={}",
        snap.ach_effective,
        r.metadata.acetylcholine_effective
    );

    // Snapshot derived signals should match metadata
    let gs_diff = (snap.gradient_scale - r.metadata.neuromod_gradient_scale).abs();
    let tg_diff = (snap.threshold_gate - r.metadata.neuromod_threshold_gate).abs();
    let pg_diff = (snap.plasticity_gate - r.metadata.neuromod_plasticity_gate).abs();
    let cm_diff = (snap.consciousness_mod - r.metadata.neuromod_consciousness_mod).abs();
    ...
    let d1_diff = (expected_d1 - r.metadata.neuromod_da_d1).abs();
    let expected_d2 = (snap.da_effective * snap.da_d2).clamp(0.0, 2.0);
    let d2_diff = (expected_d2 - r.metadata.neuromod_da_d2).abs();

    assert!(
        d1_diff < 0.001,
        "DA D1 consistency: expected={expected_d1}, got={}",
        r.metadata.neuromod_da_d1
    );
    assert!(
        d2_diff < 0.001,
        "DA D2 consistency: expected={expected_d2}, got={}",
        r.metadata.neuromod_da_d2
    );
...
fn test_circadian_hour_populated() {
    ...
    let hour = r.metadata.circadian_hour;
