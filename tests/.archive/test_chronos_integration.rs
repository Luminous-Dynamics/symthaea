...
use symthaea::{ChronosActor, HormoneState, SymthaeaHLB, TimeQuality};
use std::thread::sleep;
use std::time::Duration;

#[tokio::test]
async fn test_chronos_initialization() {
    println!("🧪 Test: Chronos is initialized in SymthaeaHLB");

    let _symthaea = SymthaeaHLB::new(10_000, 1_000)
        .await
        .expect("Failed to initialize Symthaea");

    println!("✅ SymthaeaHLB initialized with Chronos Lobe");
}
...
#[tokio::test]
async fn test_symthaea_with_chronos() {
    println!("🧪 Test: Symthaea processes queries with time perception");
    ...
    println!("✅ Symthaea integrates with Chronos");
}

#[tokio::test]
async fn test_circadian_affects_hearth_capacity() {
    println!("🧪 Test: Circadian rhythm modulates Hearth capacity in Symthaea");
