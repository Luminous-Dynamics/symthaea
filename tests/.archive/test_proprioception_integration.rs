...
use symthaea::{
    SymthaeaHLB, ProprioceptionActor, BodySensation,
};
...
async fn test_proprioception_initialization() {
    println!("🧪 Test: Proprioception is initialized in SymthaeaHLB");

    let _symthaea = SymthaeaHLB::new(10_000, 1_000)
        .await
        .expect("Failed to initialize Symthaea");

    println!("✅ SymthaeaHLB initialized with Proprioception");
}
...
#[tokio::test]
async fn test_symthaea_with_proprioception() {
    println!("🧪 Test: Symthaea processes queries with hardware awareness");
    ...
    let response = symthaea.process("What is NixOS?").await;
    ...
    let response2 = symthaea.process("Install firefox").await;
    assert!(response2.is_ok(), "Second query should process successfully");

    println!("✅ Symthaea integrates with Proprioception");
}

#[tokio::test]
async fn test_hardware_affects_hearth_capacity() {
    println!("🧪 Test: Hardware state modulates Hearth capacity in Symthaea");
    ...
    let _response = symthaea.process("test query").await;
...
async fn test_the_awakening_with_hardware_awareness() {
    ...
    println!("\n🤖 Initial state:");
    let response1 = symthaea.process("test").await.unwrap();
    println!("  Response: {}", response1.content);

    sleep(Duration::from_millis(200));

    println!("\n🤖 After 200ms:");
    let response2 = symthaea.process("test again").await.unwrap();
    println!("  Response: {}", response2.content);

    println!("\n🎉 The body awakens with HARDWARE AWARENESS! 🤖✨");
    println!("Symthaea now FEELS the silicon she lives in!");
}
