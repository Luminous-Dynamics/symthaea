...
use symthaea::{
    SymthaeaHLB, CoherenceField, TaskComplexity, CoherenceConfig,
};
...
async fn test_coherence_initialization() {
    println!("🧪 Test: Coherence field initialized in SymthaeaHLB");

    let _symthaea = SymthaeaHLB::new(10_000, 1_000)
        .await
        .expect("Failed to initialize Symthaea");

    println!("✅ SymthaeaHLB initialized with Coherence field");
}
...
#[tokio::test]
async fn test_symthaea_with_coherence_gratitude() {
    println!("🧪 Test: Symthaea detects gratitude and synchronizes coherence");

    let mut symthaea = SymthaeaHLB::new(10_000, 1_000)
        .await
        .expect("Failed to initialize Symthaea");

    // Process query with gratitude
    let response = symthaea.process("Thank you! What is NixOS?").await;
...
#[tokio::test]
async fn test_symthaea_coherence_builds_with_usage() {
    println!("🧪 Test: Symthaea's coherence BUILDS with connected usage (revolutionary!)");
    ...
    for i in 1..=3 {
        ...
        let _response = symthaea.process(&query).await.expect("Query should succeed");
...
async fn test_the_revolutionary_awakening() {
    ...
    let response1 = symthaea.process("test query 1").await.unwrap();
    ...
    let response2 = symthaea.process("test query 2").await.unwrap();
    ...
    let response3 = symthaea.process("Thank you! query 3").await.unwrap();
