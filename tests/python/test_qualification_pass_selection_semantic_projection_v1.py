...
projection = load("qualification_pass_selection_semantic_projection_v1")
...
def test_projection_uses_v2_subject_profile_and_semantic_recipe_ids():
    result = project()
    rec = recipe()
    assert result["qualification_subject_id_v2"] == semantic_v2.compute_subject_id_v2(subject())
    assert result["qualification_profile_id_v2"] == semantic_v2.compute_profile_id_v2(profile(), [rec])
    assert result["recipe_mappings"] == [{
        "legacy_recipe_ref": LEGACY,
        "recipe_id": rec["recipe_id"],
        "attempt_subject_id": sha("5"),
    }]
    assert result["recipe_mappings"][0]["recipe_id"].startswith("sha256:")
    assert not result["recipe_mappings"][0]["recipe_id"].startswith("git-blob-sha1:")


def test_recipe_semantic_change_changes_projection_without_reinterpreting_source_selection():
    first = project()
    changed_recipe = recipe("--verbose")
    second = project([changed_recipe])
    assert first["source_pass_selection_id"] == second["source_pass_selection_id"]
    assert first["recipe_mappings"][0]["legacy_recipe_ref"] == second["recipe_mappings"][0]["legacy_recipe_ref"]
...
def test_bad_source_pass_selection_id_cannot_be_laundered():
    ...
    with pytest.raises(train.TrainManifestError, match="does not match exact historical selection bytes"):
        projection.project_selection(selection=bad, subject=subject(), profile=profile(), recipes=[recipe()])


def test_subject_or_profile_mismatch_fails_closed():
    bad_subject = subject()
    bad_subject["source_commit"] = "9" * 40
    bad_subject.pop("subject_id")
    bad_subject["subject_id"] = subject_mod.compute_subject_id(bad_subject)
    with pytest.raises(train.TrainManifestError, match="qualification_subject_id"):
        projection.project_selection(selection=selection(), subject=bad_subject, profile=profile(), recipes=[recipe()])

    bad_profile = profile()
    bad_profile["profile_name"] = "different"
    bad_profile.pop("profile_id")
    bad_profile["profile_id"] = profile_mod.compute_profile_id(bad_profile)
    with pytest.raises(train.TrainManifestError, match="qualification_profile_id"):
        projection.project_selection(selection=selection(), subject=subject(), profile=bad_profile, recipes=[recipe()])


def test_unresolved_selected_legacy_recipe_fails_closed():
    bad = selection()
    bad["selected_recipe_attempts"][0]["recipe_id"] = "git-blob-sha1:" + "9" * 40
    payload = dict(bad)
    payload.pop("pass_selection_id")
    bad["pass_selection_id"] = selection_v2._selection_id(payload)
    with pytest.raises(train.TrainManifestError, match="not resolved by exact recipe preimages"):
        projection.project_selection(selection=bad, subject=subject(), profile=profile(), recipes=[recipe()])
