...
def test_steps_container_is_typed_shape_not_whole_subtree_selection():
    p=load(POLICY); profile=next(x for x in p["resource_profiles"] if x["resource_kind"]=="WorkflowJobStepsObservation")
    assert profile["required_containers"]==[{"path":"steps","container_type":"array"}]
    assert "steps" not in profile["required_paths"] and "steps" not in profile["optional_paths"]
    assert p["path_semantics"]["required_containers"]=="RequireTypedContainerAndSelectShapeOnly"; hak.validate_policy(p)
def test_selector_grammar_is_content_bound():
    p=load(POLICY); assert p["selector_grammar"]=={"id":"hak.selector-path","version":1}; hak.validate_policy(p)
def test_unsupported_selector_grammar_rejected():
    p=load(POLICY); p["selector_grammar"]={"id":"hak.selector-path","version":2}; redigest_policy(p)
...
def test_container_prefix_of_selected_descendant_is_allowed():
    p=load(POLICY); profile=next(x for x in p["resource_profiles"] if x["resource_kind"]=="WorkflowJobStepsObservation")
    assert {"path":"steps","container_type":"array"} in profile["required_containers"] and "steps[*].name" in profile["optional_paths"]; hak.validate_policy(p)
