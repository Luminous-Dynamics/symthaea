...
use symthaea::consciousness::recursive_improvement::{
    ...
    OutcomeCategory,
    // Resolution System
    resolution::{ExitCodeResolver, ResourceStateResolver, Resolver},
...
fn test_live_fire_file_creation() {
    ...
    println!("  Average prediction error: {:?}", stats.average_prediction_error);
    ...
    assert_eq!(stats.total_observations, 1, "Bridge should have 1 observation");
...
fn test_live_fire_predicted_failure() {
    ...
    assert!(prediction_correct, "Prediction about non-existence should be correct");
...
fn test_live_fire_calibration_drift() {
    ...
    for (filename, confidence) in &predictions {
        ...
        println!("\nPredicting: touch {} will succeed (confidence: {})", filename, confidence);
    ...
    if let Some(avg_outcome) = stats.average_outcome {
        assert!(avg_outcome > 0.8, "Average outcome should be high (all succeeded)");
    }
...
fn test_live_fire_overconfidence_penalty() {
    ...
    println!("Predicting: touch will FAIL (confidence in failure: {})", wrong_confidence);
    ...
    println!("Actual result: {}", if actually_succeeded { "SUCCESS" } else { "FAILURE" });
    ...
    println!("  Prediction correct: {} (we predicted failure, got success)", prediction_correct);
    println!("  Average prediction error: {:?}", stats.average_prediction_error);
    ...
    if let Some(avg_error) = stats.average_prediction_error {
        assert!(avg_error > 0.7, "Prediction error should be high after confident wrong prediction");
    }
...
fn test_magi_full_loop_closure() {
    ...
    let test_scenarios: Vec<(&str, &str, Vec<String>, f64)> = vec![
        ("Create marker file", "touch", vec![marker_path.to_string_lossy().to_string()], 0.90),
        ("Write to file", "sh", vec!["-c".to_string(), format!("echo 'MAGI WAS HERE' > {}", marker_path.display())], 0.85),
        ("Verify content", "sh", vec!["-c".to_string(), format!("grep 'MAGI' {}", marker_path.display())], 0.80),
        ("Create subdirectory", "mkdir", vec![evidence_path.to_string_lossy().to_string()], 0.88),
        ("Move file", "mv", vec![marker_path.to_string_lossy().to_string(), evidence_path.to_string_lossy().to_string()], 0.82),
    ];
    ...
    for (description, command, args, confidence) in &test_scenarios {
        ...
        println!("\n┌─────────────────────────────────────────────────────────────────┐");
        println!("│ LOOP ITERATION {} - {}                    ", loop_iterations, description);
        println!("└─────────────────────────────────────────────────────────────────┘");
    ...
    println!("  Successful predictions: {}/{}", successful_predictions, loop_iterations);
    println!("  Accuracy: {:.1}%", (successful_predictions as f64 / loop_iterations as f64) * 100.0);
    println!("\n  Bridge Statistics:");
    println!("    Total observations: {}", final_stats.total_observations);
    println!("    Average prediction error: {:?}", final_stats.average_prediction_error);
    ...
    assert!(successful_predictions >= 4, "At least 4/5 predictions should succeed");

    // Verify the world state changed
    println!("\n  World State Verification:");
    println!("    evidence/ directory exists: {}", evidence_path.exists());
    println!("    marker.txt moved to evidence/: {}", evidence_marker_path.exists());
    ...
    assert!(evidence_marker_path.exists(), "The moved file must exist - proving world state was modified");
...
fn test_live_fire_signal_evolution() {
    ...
    if let Some(avg_error) = final_stats.average_prediction_error {
        assert!(avg_error < 0.2, "Prediction error should be low after many successes");
    }

    if let Some(avg_outcome) = final_stats.average_outcome {
        assert!(avg_outcome > 0.9, "Average outcome should be high (all succeeded)");
    }
