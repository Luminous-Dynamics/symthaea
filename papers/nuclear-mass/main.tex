...
correctly identifies all four tested known proton emitters as unbound:
...
$\beta\beta$ experiments show a systematic bias of $-962$~keV (RMS =
1012~keV). This under-prediction of $Q_{\beta\beta}$ reflects the model's
slight under-binding of the $Z{+}2$ daughter relative to the parent, and
serves as a calibration point for future improvements. The bias is
consistent across the chart, suggesting a systematic Coulomb-correction
issue rather than random scatter.
...
rather than exceptional ML performance. With only 8 training epochs and
32,770 parameters on 2,535 samples, the correction layer has not converged
far from zero. This is actually desirable: the model avoids harming the
DZ10 extrapolation while providing a framework for future improvement with
more training data or deeper architectures.
...
\item The Th-229 nuclear clock $\alpha$-decay Q-value is reproduced to
      51~keV;
\item A systematic $-962$~keV bias in $Q_{\beta\beta}$ predictions
      identifies a reproducible Coulomb-correction deficit.
\end{itemize}

The complete system (8,340 lines of Rust, 101 tests) is open source at
\url{https://github.com/Luminous-Dynamics/symthaea}.
