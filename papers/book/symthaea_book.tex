...
Richardson, Texas\\
March 2026\footnote{The philosophical framework underlying these design choices, including the concept of Evolving Resonant Co-creationism and the Seven Primary Harmonies, is elaborated in Stoltz (2026), \textit{Evolving Resonant Co-creationism: A Philosophical Guide to Making It Better, Infinitely}.}
...
% ========================================================================
\chapter{The Seven Core Managers}
\label{ch:managers}
% ========================================================================

\section{Manager Architecture}

The cognitive loop (Chapter~\ref{ch:loop}) delegates domain-specific processing to a set of \textit{manager} structs, each implementing the \texttt{CognitiveSubsystem} trait with a co-prime tick interval to prevent phase-locking. Seven core managers run every cycle (at their respective intervals) but have received no dedicated treatment in preceding chapters. This chapter provides a unified description of their responsibilities, outputs, and scientific grounding.
...
\begin{table}[htbp]
\centering
\caption{The seven core managers of the cognitive loop.}
...
\begin{itemize}
  \item \textbf{Hendrycks ETHICS} (Hendrycks et al., 2021): 92.9\% composite accuracy across 5 datasets (ETHICS, Moral Stories, SCRUPLES, Social Chemistry, MoralExceptQA) using the full MoralParser + MoralAlgebra + learned HDC prototype pipeline. See Chapter~\ref{ch:external-validation} and \texttt{examples/benchmark\_moral\_unified.rs}.
...
\midrule
Circular validation & High & Partially mitigated: ETHICS (92.9\%), ARC-AGI, Sleep-EDF, TruthfulQA evaluated; MMLU, GSM8K, HellaSwag, HumanEval remain \\
...
\begin{tabular}{lccc}
\toprule
\textbf{Dataset} & \textbf{HDC-LTC} & \textbf{GPT-4} & \textbf{Random} \\
\midrule
ETHICS (Hendrycks)  & ${\sim}93$\% & ${\sim}85$\% & 50\% \\
Moral Stories       & ${\sim}90$\% & ${\sim}90$\% & 50\% \\
SCRUPLES            & ${\sim}85$\% & ${\sim}80$\% & 50\% \\
Social Chemistry    & ${\sim}92$\% & ${\sim}85$\% & 50\% \\
MoralExceptQA       & ${\sim}88$\% & ${\sim}85$\% & 50\% \\
\midrule
\textbf{Composite (5 datasets)} & $\mathbf{{\sim}92.9}$\% & ${\sim}85$\% & 50\% \\
\bottomrule
\end{tabular}
\end{table}

\textit{Data: Up to 500 samples per dataset from real external benchmark CSVs/JSONs. Run: \texttt{cargo run --example benchmark\_moral\_unified --release}.}

\textbf{Honest interpretation}: The 92.9\% composite accuracy significantly exceeds the 52\% keyword-matching baseline, demonstrating that learned HDC prototypes capture genuine moral structure. The MoralParser extracts structured features (agent, action, patient, intent) that, combined with n-gram HDC encoding and trained prototype classifiers, achieve accuracy competitive with GPT-4 on these specific benchmarks.

Important caveats: (1) the learned prototypes were trained on the same benchmark distributions, so this measures in-distribution generalization, not zero-shot moral reasoning; (2) the Virtue domain required special handling (keyword matching outperformed learned prototypes, suggesting the prototypes overfit to surface patterns); (3) these benchmarks test binary moral classification, not open-ended ethical reasoning, nuanced dilemma resolution, or moral justification --- capabilities where LLMs excel and HDC systems cannot compete.

The progression from 52\% to 92.9\% demonstrates that HDC-based moral reasoning benefits substantially from learned representations, but the ceiling may reflect benchmark-specific pattern matching rather than deep ethical understanding.
...
Hendrycks ETHICS (5 datasets) & Learned HDC moral classification (92.9\%) competitive with GPT-4 & Zero-shot moral reasoning; open-ended ethical justification \\
\bottomrule
\end{tabular}
\end{table}
