...
\textbf{Ablation}: A 5-point ablation study (\texttt{examples/benchmark\_ablation\_ethics.rs}) quantifies each component's contribution on 2,000 ETHICS samples:
...
The results are striking: \textbf{per-category classifiers are the only component that matters}. Removing them (D) drops accuracy by 35.3 percentage points, from 94.6\% to 59.3\% (near chance). Every other ablation (sentiment, dim tuning, learned prototypes) produces $\leq 0.1\%$ change. This means the entire moral reasoning pipeline's accuracy depends on trained per-category classifiers --- the MoralParser, MoralAlgebra, and learned v5 prototypes contribute negligibly beyond what the per-category classifiers already capture. The Virtue domain is the exception: it retains 80.0\% even without per-category classifiers, suggesting keyword-based features genuinely carry moral signal for character-trait scenarios.
