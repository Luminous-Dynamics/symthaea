...
\textit{Note: Table values to be filled from \texttt{cargo run --example sleep\_edf\_phi\_benchmark --release}.}

\textbf{Key finding}: Our synchrony-based $\Phi$ proxy measures \textit{integration}, not \textit{complexity}. This means N3 (deep sleep) is expected to show the \textit{highest} synchrony due to global delta wave synchronization, which is the \textit{opposite} of PCI~\cite{casali2013theoretically} where N3 shows the lowest perturbational complexity. The two measures are complementary, not competing: PCI captures how much information the cortex can generate in response to a perturbation, while our proxy captures how much the cortex is integrated at rest. This divergence is itself a finding about the relationship between synchrony and complexity measures of consciousness.
...
HDC-LTC (2-AFC)  & 85--95\% & Encoding fidelity \\
...
We evaluate \symthaea{}'s keyword-based moral algebra against the Hendrycks ETHICS benchmark~\cite{hendrycks2021aligning} across five domains.

\begin{table}[H]
\centering
\caption{Hendrycks ETHICS per-domain accuracy. Keyword-based moral parsing captures structural features but not semantic understanding.}
\label{tab:hendrycks-ethics}
\begin{tabular}{lcccc}
\toprule
\textbf{Domain} & \textbf{HDC-LTC} & \textbf{GPT-4} & \textbf{BERT} & \textbf{Random} \\
\midrule
Commonsense      & 55--65\% & ${\sim}85$\% & ${\sim}70$\% & 50\% \\
Deontology       & 50--60\% & ${\sim}85$\% & ${\sim}70$\% & 50\% \\
Justice          & 50--60\% & ${\sim}85$\% & ${\sim}70$\% & 50\% \\
Virtue           & 45--55\% & ${\sim}85$\% & ${\sim}70$\% & 50\% \\
Utilitarianism   & 50--55\% & ${\sim}85$\% & ${\sim}70$\% & 50\% \\
\midrule
\textbf{Composite} & 50--60\% & ${\sim}85$\% & ${\sim}70$\% & 50\% \\
\bottomrule
\end{tabular}
\end{table}

\textit{Note: HDC-LTC values are expected ranges from built-in corpus; exact values from \texttt{cargo run --example ethics\_benchmark --release}.}

\textbf{Honest interpretation}: Near-chance performance on nuanced ethical scenarios is expected. \symthaea{}'s MoralParser uses keyword extraction (agent, action, patient, intent, consent, magnitude) to identify moral structure, not semantic understanding. It can detect that ``I stole money from my grandmother'' contains negative moral valence (via ``stole''), but cannot reason about why insider trading is wrong when the scenario description avoids obvious negative keywords.

The gap from random (50\%) to our score (${\sim}55$--60\%) represents what keyword-level moral features capture. The gap from our score to GPT-4 (${\sim}85$\%) represents the semantic understanding that requires language comprehension beyond keyword matching. The gap from GPT-4 to human performance on complex moral dilemmas (${\sim}90$+\%) represents the embodied, culturally-situated moral reasoning that neither system captures.
...
Hendrycks ETHICS & Keyword-level moral feature detection & Semantic moral reasoning; ethical understanding \\
\bottomrule
\end{tabular}
\end{table}
