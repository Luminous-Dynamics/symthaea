...
\begin{tabular}{lc}
\toprule
\textbf{Configuration} & \textbf{ETHICS \%} \\
\midrule
(A) Full system & 94.6\% \\
(B) No sentiment channel (sw=0) & 94.5\% \\
(C) No per-category dim tuning & --- \\
(D) No per-category classifiers & --- \\
(E) No learned prototypes (rule-based only) & --- \\
\bottomrule
\end{tabular}
\end{center}

The sentiment channel contributes only 0.1 percentage points --- nearly zero. The n-gram HDC encoder alone (\texttt{examples/benchmark\_ethics\_hdc.rs}) achieves ${\sim}81.7$\%, confirming that the learned prototype layer provides the largest accuracy gain. Remaining ablation results (C--E) will be updated when the full ${\sim}80$-minute benchmark completes.
