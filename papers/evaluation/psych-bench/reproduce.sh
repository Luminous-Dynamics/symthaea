...
# Remove only the exact CSVs owned by the current paper generator. Other tracked
# CSVs in the historical directory (for example legacy derived artifacts) are not
# silently promoted into this receipt and are not claimed as reproduced here.
for name in "${PAPER_CSV_NAMES[@]}"; do
  rm -f -- "$STAGED_PAPER_DATA/$name"
done

export CARGO_TARGET_DIR="$REPO/target/psych-bench-reproduction-build/$SOURCE_SHA"

is_expected_paper_csv_path() {
  local path="$1"
  local name
  for name in "${PAPER_CSV_NAMES[@]}"; do
    if [[ "$path" == "papers/data/psych_bench/$name" ]]; then
      return 0
    fi
  done
  return 1
...
validate_execution_worktree() {
  if [[ "$(git -C "$WORKTREE" rev-parse HEAD)" != "$SOURCE_SHA" ]]; then
    printf '%s\n' 'detached execution subject changed during reproduction' >&2
    return 1
  fi

  local path
  while IFS= read -r -d '' path; do
    if ! is_expected_paper_csv_path "$path"; then
      printf 'unexpected tracked mutation in detached execution worktree: %s\n' "$path" >&2
      return 1
    fi
  done < <(git -C "$WORKTREE" diff HEAD --name-only -z --)

  while IFS= read -r -d '' path; do
    if ! is_expected_paper_csv_path "$path"; then
      printf 'unexpected untracked path in detached execution worktree: %s\n' "$path" >&2
      return 1
    fi
...
run_synthetic_self_test() {
  ...
  if [[ "${PSYCH_BENCH_REPRO_SELF_TEST_MUTATE_SOURCE:-0}" == 1 ]]; then
...
# The execution worktree may differ from the source tree only at the exact CSV
# outputs owned by the paper generator. This check happens before and after receipt
# construction so build scripts invoked by Cargo cannot silently rewrite source,
# Cargo.lock, or the reproduction controller and still yield a visible receipt.
validate_execution_worktree

# Copy only the exact generated CSV census after all four producer steps succeed.
# Non-generator-owned historical CSVs remain outside this receipt by construction.
for name in "${PAPER_CSV_NAMES[@]}"; do
  cp -- "$STAGED_PAPER_DATA/$name" "$PAPER_ARTIFACTS/$name"
done
