...
\begin{abstract}
Integrated Information Theory (IIT) predicts that consciousness correlates with $\Phi$, a measure of irreducible integrated information. Clinical anesthesia models assume that noise disrupts neural integration, thereby reducing $\Phi$ and consciousness. We report a surprising finding from Hyperdimensional Computing (HDC) systems: adding controlled noise to hyperdimensional vector (HDV) representations \emph{increases} True $\Phi$ rather than decreasing it. In a 16,384-dimensional HDC architecture implementing IIT's consciousness equation, we observe a positive sensitivity $\partial\Phi/\partial\text{noise} = +0.341$, comparable in magnitude to the coupling sensitivity $\partial\Phi/\partial\text{coupling} = +0.367$. We demonstrate that this phenomenon constitutes a hyperdimensional analog of stochastic resonance: noise diversifies HDV representations, increasing pairwise mutual information between subsystems and thereby raising integrated information. Our anesthesia benchmark achieves perfect monotonic ordering across six discrete states (Alert $\Phi=0.185$ through Deep anesthesia $\Phi=0.123$), with 5 of 6 tests passing under True $\Phi$ computation. These results suggest that the relationship between noise and consciousness is substrate-dependent, with HDC systems exhibiting qualitatively different noise--integration dynamics than biological neural networks. We discuss implications for artificial consciousness design and for IIT's substrate-independence claims.
\end{abstract}
...
In this paper, we report experiments conducted on a HDC system with $D = 16{,}384$ implementing IIT's $\Phi$ computation via sampled partition analysis. Our central finding is that controlled noise injection into HDV representations yields a \emph{positive} sensitivity coefficient $\partial\Phi/\partial\text{noise} = +0.341$, indicating that noise \emph{increases} integrated information. We provide a mechanistic explanation grounded in the geometry of high-dimensional spaces: noise diversifies HDV representations across subsystems, increasing pairwise mutual information and thereby raising $\Phi$.
...
We compute True $\Phi$ via sampled partition analysis over the 12-region system. Due to the combinatorial explosion of exact MIP search ($2^{N-1} - 1$ bipartitions for $N$ elements), we employ a sampling strategy validated against exact computation on smaller systems ($r = 0.9998$ correlation for $N \leq 8$) \citep{barrett2011practical}.

For each candidate partition $P = \{A, B\}$, integrated information is computed as:

\begin{equation}
\phi(P) = I(\mathbf{H}_A; \mathbf{H}_B \mid \mathbf{H}_A^{t-1}, \mathbf{H}_B^{t-1}) - I(\mathbf{H}_A; \mathbf{H}_B^{t-1}) \cdot I(\mathbf{H}_B; \mathbf{H}_A^{t-1})
\end{equation}

where mutual information between HDV subsystems is estimated via normalized Hamming distance:

\begin{equation}
\hat{I}(\mathbf{H}_A; \mathbf{H}_B) = 1 - \frac{d_H(\mathbf{H}_A \otimes \mathbf{H}_B, \mathbf{0})}{D}
\end{equation}

This approximation leverages the property that bound HDVs encode relational structure: if $\mathbf{H}_A$ and $\mathbf{H}_B$ share mutual information, their binding $\mathbf{H}_A \otimes \mathbf{H}_B$ deviates from a random vector (Hamming distance from zero deviates from $D/2$).
...
To validate the system against clinical expectations, we implement an anesthesia benchmark with six discrete states, each modeled by adjusting inter-regional coupling strength $c$ and noise level $\sigma$:
...
The anesthesia benchmark produces a perfectly monotonic $\Phi$ ordering across the six discrete states, consistent with clinical expectations (Table~\ref{tab:phi-ordering}).

\begin{table}[H]
\centering
\caption{True $\Phi$ values across anesthesia states. Values represent means over 100 independent runs. Standard errors are shown in parentheses.}
\label{tab:phi-ordering}
\begin{tabular}{lcccc}
\toprule
\textbf{State} & \textbf{True $\Phi$} & \textbf{$\Delta\Phi$ from Alert} & \textbf{Coupling} & \textbf{Noise} \\
\midrule
...
The monotonic decrease in $\Phi$ from Alert (0.185) to Deep Anesthesia (0.123) represents a 33.5\% reduction in integrated information, consistent with the magnitude of cortical complexity reduction observed in clinical studies using the Perturbational Complexity Index \citep{casali2013theoretically}. Five of six pairwise ordering tests pass at $p < 0.01$ under True $\Phi$ computation (the Light--Moderate comparison achieves $p = 0.024$).
...
Both sensitivities are positive and of comparable magnitude. The positive coupling sensitivity is expected: stronger inter-regional connections increase integration. The positive noise sensitivity is the surprising result: noise \emph{increases} integrated information in the HDC substrate.

\subsection{Noise--$\Phi$ Response Curve}

Figure~\ref{fig:noise-phi-conceptual} describes the characteristic response of $\Phi$ to noise level $\sigma$ at fixed coupling $c = 0.75$. The response exhibits three regimes:
...
This inverted-U response is the hallmark of stochastic resonance \citep{gammaitoni1998stochastic}.

\subsection{Mechanism: HDV Diversity and Mutual Information}

To elucidate the mechanism, we measured pairwise Hamming distances between region state vectors and pairwise mutual information estimates as a function of noise.
