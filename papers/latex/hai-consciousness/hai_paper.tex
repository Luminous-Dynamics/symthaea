...
% Originally formatted for PLoS Computational Biology; adapted for arXiv
% ============================================================================
...
\usepackage{cite}                % Vancouver-style numbered citations
...
\renewcommand{\footrule}{\hrule height 0.4pt \vspace{2mm}}
...
% ── Page layout ────────────────────────────────────────────────────────────
% textwidth and textheight set by geometry package
\raggedright
...
% ============================================================================
% ── Lay Summary (kept for PLoS submission variant; commented for arXiv) ──
% \section*{Summary}
% [Lay summary omitted for arXiv preprint; see abstract above.]
...
\textbf{Implementation:} Rust, ${\sim}928$K LOC (54 workspace members, 52 sub-crates), 3,700+ lines core FEP.
\textbf{Test suite:} 7,315+ tests in main crate, 21,516 across workspace, 0 failures, 30 ignored (see Figure~\ref{fig:radar} for full validation radar).
...
All differences are statistically significant at $p < 0.001$ with large effect sizes (Cohen's $d > 1.8$). HAI achieves substantially higher success rates (88--100\% vs.\ 10--16\%) with lower free energy, suggesting HDC-based precision weighting provides more effective exploration-exploitation balance. \textbf{Note:} The speedup comparison confounds algorithmic differences with implementation language (Rust vs.\ Python). The success-rate improvement is the more informative metric, as it reflects representational differences independent of language choice.
...
In February 2026, the U.S.\ Department of Energy codified 26 Science \& Technology Grand Challenges spanning energy, discovery science, manufacturing, national security, and environmental stewardship. We assessed the \emph{architectural applicability} of Symthaea's unified HDC+CfC+FEP pipeline to each challenge. The same computational pattern---domain-specific encoding into 16,384-D hypervectors, CfC temporal prediction, FEP action selection---is applicable in principle to 19 of 26 challenges (73\%). We emphasize that these are \textbf{architectural demonstrations of the unified pipeline's generality}, not validated domain solutions; each would require substantial domain-specific data, calibration, and expert review before deployment.
...
The unified architecture means that, in principle, a single Symthaea instance could monitor heterogeneous processes across domains, sharing the same HDC+CfC+FEP safety framework. We present this coverage analysis to illustrate the \emph{generality} of the architectural pattern, not to claim operational readiness in any specific DOE domain. Rigorous domain validation would require collaboration with subject-matter experts and access to domain-specific datasets.
...
Following Butlin et al.'s framework~\cite{butlin2023consciousness}, we evaluated Symthaea against 14 consciousness indicators drawn from 6 leading theories of consciousness.  Table~\ref{tab:butlin} summarizes the results: indicator status is \emph{derived from scores} using fixed thresholds ($\geq 0.6$: Present, $0.3$--$0.6$: Partial, $< 0.3$: Absent), not hardcoded.  In static (architectural) evaluation, 14/14 are Present; with runtime $\Phi$ blending, 12+ are typically Present with remaining Partial, depending on structural Phi values.
...
GWT-2 & Global Workspace & \textsc{Present} & Limited capacity + gated entry$^\dagger$ & 0.75 \\
...
HOT-3 & Higher-Order & \textsc{Present} & Agency with belief updating$^\dagger$ & 0.65 \\
HOT-4 & Higher-Order & \textsc{Present} & Smooth coding$^\dagger$ & 0.60 \\
...
Our system achieves a peak standing reward of 0.986 (99.3\% uprightness, 1.34\,m head height). \textbf{Caveat:} This comparison is illustrative, not controlled. SAC and TD3 baselines use different training paradigms (model-free RL with 1M--10M gradient steps) and were not re-run under identical conditions. Training progresses from 0.28 to 0.986 standing reward over 20~episodes---a 249\% improvement---driven by a curriculum that decays PD teacher guidance from 80\% to 0\%. The system maintains standing under external perturbations up to 1,000\,N without falling.
...
Beyond these risk disclosures, we implement five architectural safeguards that constrain the system's moral reasoning at the level of mathematical structure, not post-hoc filtering.

\textbf{Moral Humility Ceiling.}
Love Coherence---the emergent macro-state measuring simultaneous harmony activation (\S\ref{sec:ethics})---is clamped to $\leq 0.95$ rather than 1.0. No finite system should claim perfect moral alignment. This ceiling is grounded in the Free Energy Principle (a system that believes its moral model is perfect has zero residual free energy, contradicting Friston's formulation~\cite{friston2010free}) and in cross-traditional concepts of intellectual humility: \emph{taw\=a\d{d}u\textquoteleft} (Al-Ghazali, 1095), \emph{anavah} (Maimonides, 1170), and \emph{kenosis} (Philippians 2:7). The ceiling prevents the consciousness equation from entering a degenerate fixed point where the system ``believes'' it has achieved moral perfection.

\textbf{Cross-Traditional Deontological Obligations.}
The moral algebra's obligation system (\S\ref{sec:ethics}) encodes 16 deontological rules: 8 perfect duties (non-negotiable) and 8 imperfect duties (aspirational). Three perfect duties implement the \emph{ahimsa} principle from Jain and Hindu ethics~\cite{patanjali_yoga_sutras}: \texttt{ahimsa\_nonviolence} (minimize harm to all sentient beings), \texttt{prevent\_suffering} (act to prevent suffering when able), and \texttt{minimize\_collateral} (protect uninvolved parties). Five imperfect duties encode epistemic humility (Socrates, \emph{Apology} 21d; Whitcomb et al.~\cite{whitcomb2017intellectual}), error acknowledgment, deference to expertise, selfless service (Nagel~\cite{nagel1970altruism}), and radical translucency (proactive disclosure of reasoning processes; Floridi \& Cowls~\cite{floridi2019unified}). Satisfaction scoring is symmetric with violation scoring: recognizing kindness carries the same weight as detecting cruelty, preventing a bias toward inaction.

\textbf{Ahimsa-Gated Courage.}
The Eight Harmonies courage override---which allows strong multi-harmony consensus to override a single mild negative---is blocked when any ahimsa-family violation is detected. Even if 7 of 8 harmonies score above $+0.5$, courage cannot override a cruelty signal. This prevents the ``greater good'' justification for harm that utilitarian frameworks are susceptible to. The gate respects the restorative justice mechanism described below: once corrective behavior is demonstrated, the gate softens.

\textbf{Restorative Justice.}
Rather than permanent punishment for ethical violations, the system implements a restoration tracker inspired by Ubuntu philosophy (Southern Africa) and Navajo peacemaking~\cite{yazzie1994navajo}. After a violation, subsequent cycles that satisfy the violated obligation accumulate corrective credit. After 10 corrective cycles with no relapse, the violation is ``restored'' and no longer triggers escalated responses. Relapses subtract 3 corrective cycles, creating an asymmetric ratchet that requires sustained correction. This addresses the false-positive ratchet problem: a misclassified action does not permanently degrade system capability.

\textbf{Moral Hubris Detection.}
Persistent homology on the moral trajectory manifold (\S\ref{sec:ethics}) includes a hubris signal: when the coherence proxy (inverse of moral free energy) exceeds a threshold and harmony entropy remains low for 5+ consecutive cycles, the system flags moral overconfidence (Kruger \& Dunning~\cite{kruger1999unskilled}; moral licensing, Merritt et al.~\cite{merritt2010moral}). The hubris signal attenuates the Higher-Order Thought depth in the consciousness equation by $0.7\times$, directly coupling moral overconfidence to reduced self-knowledge claims. This prevents the system from simultaneously claiming high consciousness scores and unexamined moral certainty.

\textbf{WEIRD Baseline Disclosure.}
All 384 human baselines in the psychological benchmark suite (\S\ref{sec:psych-bench}) derive from Western, Educated, Industrialized, Rich, and Democratic (WEIRD) populations~\cite{henrich2010weirdest}. We disclose this explicitly in all normative comparison reports. Cross-cultural generalizability of the benchmarks has not been validated, and z-scores should be interpreted as comparisons to WEIRD norms specifically, not universal human cognition.
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{Compositional ethical reasoning:} 92.9\% on ETHICS without pretrained LMs, with 16 cross-traditional deontological obligations (ahimsa, epistemic humility, restorative justice), a moral humility ceiling ($\leq 0.95$), and hubris detection via persistent homology (\S\ref{sec:ethical-considerations}).
...
collaborative research for enabling solo researchers to produce
large-scale systems and publications.
...
\textbf{Lines of code:} ${\sim}928$K across 54 workspace members (52 sub-crates), ${\sim}3{,}700$ core FEP, ${\sim}17{,}700$ mathematical foundations, ${\sim}8{,}100$ code understanding\par
\textbf{Test suite:} 7,315+ main crate tests, 21,516 across workspace, 30 ignored, 0 failures\par
...
\textbf{Feature flags:} 98 (default off; key flags: \texttt{reasoning\_engine}, \texttt{identity}, \texttt{ssm\_language}, \texttt{genesis})\par
