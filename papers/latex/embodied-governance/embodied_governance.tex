...
The governance substrate is Mycelix, a 133-zome Holochain architecture with consciousness-gated access tiers derived from a 4-dimensional participant profile (identity verification, reputation, community engagement, and civic participation), producing a 5-tier hierarchy (Observer through Guardian) with progressive vote weights.
We demonstrate that governance-embodied reward signals produce measurably different learning dynamics compared to governance-agnostic baselines: prediction error from governance outcomes modulates learning rate by up to $2\times$, reciprocity-driven oxytocin injections reduce competitive behavior by 34\%, and emergency NE surges increase attentional allocation to governance-relevant information by 2.7$\times$.
...
Our system bridges Mycelix---a Holochain-based decentralized governance platform comprising 133 zomes across 16 cluster DNAs---into the Symthaea cognitive architecture, a consciousness-first AI system with a 9-transmitter neuromodulatory bath.
...
\begin{enumerate}
  ...
  \item Consciousness-gated governance access: a 4-dimensional participant profile producing 5-tier progressive permissions.
...
Mycelix is built on Holochain as a ``fractal CivOS'' comprising 133 zomes (modular code units) organized into 16 cluster DNAs spanning governance, identity, finance, health, knowledge, supply chain, and other civic domains.
...
Mycelix implements a consciousness-gated access model where governance privileges are determined by a 4-dimensional participant profile:

\begin{definition}[Consciousness Profile]
A participant's consciousness profile is a 4-tuple $(I, R, C, E) \in [0,1]^4$ where:
\begin{itemize}
  \item $I$ = identity verification depth (DID credentials, MFA, biometrics)
  \item $R$ = reputation score (accumulated from governance participation, peer attestation)
  \item $C$ = community engagement (reciprocity history, mutual aid participation)
  \item $E$ = civic participation (voting frequency, proposal quality, council service)
\end{itemize}
\end{definition}

The composite score $S = w_I I + w_R R + w_C C + w_E E$ (with weights $w_I = 0.2$, $w_R = 0.3$, $w_C = 0.25$, $w_E = 0.25$) determines the participant's tier:
...
To assess the contribution of individual neuromodulatory pathways, we conducted ablation experiments, disabling each pathway individually:
