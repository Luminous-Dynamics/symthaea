...
\node[font=\scriptsize, hlbyellow!80!black, anchor=south] at (axis cs:-1.5,77) {$n=6$};
\node[font=\scriptsize, hlbgreen!80!black, anchor=south] at (axis cs:0,77) {$n=30$};
\node[font=\scriptsize, hlbyellow!80!black, anchor=south] at (axis cs:1.5,77) {$n=10$};
\node[font=\scriptsize, hlbred, anchor=south] at (axis cs:2.75,77) {$n=19$};
\end{axis}
\end{tikzpicture}
\caption{Forest plot of normative z-scores for 76 benchmarks with matched human baselines (z-scores clamped to $\pm 3$ for readability; 30 benchmarks exceed this range in raw values). Background shading indicates clinical interpretation bands \citep{cicchetti1994guidelines}. Band counts: 30 Average ($|z| \leq 1$), 10 Above Average, 6 Below Average, 19 Superior ($z > 2$), 11 Impaired ($z < -2$). Clamped benchmarks include 19 at $+3$ (many with raw $z > 10$, reflecting mismatched metric scales) and 11 at $-3$. Data loaded from \texttt{normative\_zscores.csv}.}
\label{fig:forest}
\end{figure}
...
\emph{Normative z-score interpretation.} Several benchmarks produce extreme normative z-scores ($|z| > 5$) because the human baseline standard deviations for those paradigms are very small. These extremes reflect the ratio $(\text{HLB} - \mu_\text{human}) / \sigma_\text{human}$ and should be interpreted as ``far from human typical'' rather than as meaningful psychometric distances. The forest plot (Figure~\ref{fig:forest}) clips z-scores to $[-3, +3]$ for visual clarity; unclipped values are available in the supplementary CSV data.
