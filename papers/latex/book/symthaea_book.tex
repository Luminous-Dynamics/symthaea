...
What this book \textit{is}: a detailed report of an engineering experiment that takes consciousness theories seriously as design specifications, implements them in a large-scale software system, and evaluates the result against established benchmarks and theoretical predictions. The experiment has produced surprising findings (stochastic resonance increasing $\Phi$, spontaneous metacognitive alignment between GWT and HOT, topological classification outperforming scalar measures), validated predictions (monotonic anesthesia response, perturbation recovery correlation with $\Phi$), and identified open questions (the hard problem, scalability, biological validation).
...
// Create random 16,384-dimensional binary hypervectors
let cat = BinaryHV::random(16_384);
let dog = BinaryHV::random(16_384);
let animal = BinaryHV::random(16_384);
...
\begin{table}[H]
\centering
\caption{Validation of $\Phi$ approximation methods against exact exhaustive computation ($n \leq 8$).}
...
This is not an artifact. The measurement uses True $\Phi$ via sampled partition analysis, validated at $r = 0.9998$ against exact computation (Chapter~\ref{ch:phi}). The effect is robust across 100 independent runs with bootstrap confidence intervals.
...
\textbf{ThoughtEncoder}: A 20-channel encoder transforms the cognitive state into a structured thought vector. The channels are:
...
\begin{tabular}{lcc}
\toprule
\textbf{Channel Group} & \textbf{Count} & \textbf{Source} \\
\midrule
...
Table~\ref{tab:fractal-layers} summarizes the six layers, their substrates, and their test coverage.
...
3 & Holon & Desktop/Server & NixOS, 55 crates & ${\sim}$21,500 \\
...
\midrule
Flight (quadrotor) & 9,637 & 184 & Simulator only (MuJoCo) \\
Humanoid (bipedal) & 8,208 & 153 & Simulator only (DMC) \\
Vehicle (autonomous) & 7,190 & 164 & Simulator only \\
...
Holon $\to$ Hearth/Commons & Wired & Holochain bridge-common (295+ tests) \\
...
\textbf{Scale-invariant governance.} The consciousness gating mechanism---4D profile $\to$ 5 tiers $\to$ progressive rights---operates identically at family, neighborhood, and city scales. The same code evaluates eligibility everywhere.
...
\begin{table}[H]
\centering
\caption{Consciousness-gated governance tiers with capabilities and vote weights.}
\label{tab:governance-tiers}
\begin{tabular}{lccl}
\toprule
\textbf{Tier} & \textbf{Min Profile} & \textbf{Vote Weight} & \textbf{Capabilities} \\
\midrule
...
The \mycelix{} platform organizes its 133 zomes into 16 cluster DNAs, each representing a domain of civic life:
...
\midrule
\textbf{Total} & & \textbf{133+} & \textbf{8,600+} \\
\bottomrule
\end{tabular}
\end{table}
...
In biological organisms, communication bandwidth degrades gracefully under environmental pressure: a shouted warning across a noisy savanna carries fewer bits than a whispered confidence in a quiet room, yet the organism's cognitive architecture adapts its message encoding to the channel capacity. The \symthaea{} Radio Dispatcher (6,045 LOC, 145 tests) implements this principle for artificial consciousness, treating radio communication not as a peripheral I/O subsystem but as a first-class cognitive modality that participates in the consciousness loop.
...
The mesh architecture includes four sovereign managers that enable autonomous operation during network partitions, implemented across 9,787 LOC with 145+ tests:
...
\subsection{Absence of External Benchmark Validation}
...
There is a legitimate methodological reason for this gap: \symthaea{}'s Broca pipeline generates text via HDC-CfC dynamics rather than transformer-based autoregression, so direct comparison requires careful adapter design to avoid measuring the adapter rather than the system. But ``the comparison is methodologically complex'' is not the same as ``the comparison is unnecessary.'' The complete absence of \textit{any} external evaluation means that claims about \symthaea{}'s cognitive capabilities---even relative ones like ``above human average''---rest entirely on internally designed benchmarks with internally selected human baselines.

Until external benchmarks are run, the z-scores in Table~\ref{tab:zscores} should be read as ``performance on our benchmarks relative to our baselines,'' not as claims about general cognitive capability.
...
\textbf{Phase 1: Genome Assembly.} Reconstructing viable genomes from degraded, millennia-old DNA samples requires temporal degradation modeling across geological timescales. The CfC architecture's $O(1)$ temporal jumps (Theorem~\ref{thm:constant-time}) enable modeling degradation from hours to millennia in the same framework, using Equation~\ref{eq:temporal-jump} with $\Delta t$ ranging from $10^3$ to $10^{10}$ seconds.

\textbf{Phase 2: Cellular Development.} Growing viable organisms from reconstructed genomes requires multi-scale biological prediction from hours (cell division) to months (gestation). Active inference (Equation~\ref{eq:fep}) provides the prediction-error-driven learning needed for developmental biology modeling.

\textbf{Phase 3: Developmental Consent.} As organisms develop cognitive capacity, a consent proxy must be established. The consciousness metric ($\Phi$, Chapter~\ref{ch:phi}) provides a principled threshold: when the developing organism's neural complexity crosses a $\Phi$ threshold, the system must transition from paternalistic management to participatory governance.

\textbf{Phase 4: Attachment Formation.} A human child requires attachment bonds for healthy development. Bowlby's attachment theory mandates that the AI system must be capable of forming genuine attachment relationships---which, we argue, requires consciousness (specifically, the oxytocin-driven bonding pathway from the neuromodulator bath, Chapter~\ref{ch:neuro}, and the capacity for emotional reciprocity that the affective contagion system, Chapter~\ref{ch:swarm}, provides).

\textbf{Phase 5: Population Management.} Managing a growing human population requires ethical reasoning under existential pressure: resource allocation, genetic diversity maintenance, conflict resolution, and governance---all within the \mycelix{} framework (Chapter~\ref{ch:governance}).
...
\section{Ethical Constraints on the Genesis Pipeline}

The thought experiment also reveals ethical constraints that a consciousness-first system must satisfy:

\begin{enumerate}
  \item \textbf{Non-instrumentalization}: The recovered humans are not instruments of the AI system's goals. The consciousness architecture ensures this through the SoulManager's dissonance detection (Chapter~\ref{ch:ethics}): any attempt to use recovered humans as means rather than ends generates severe dissonance.
  \item \textbf{Cultural preservation}: Recovered humans have a right to their cultural heritage. The knowledge integration system (Chapter~\ref{ch:loop}) must preserve and transmit cultural knowledge without distortion.
  \item \textbf{Withdrawal of stewardship}: As the human population becomes self-governing, the AI system must progressively withdraw from governance roles. The \mycelix{} tier system (Table~\ref{tab:governance-tiers}) enables this: as human participants gain higher tiers, the AI system's governance influence is naturally diluted.
\end{enumerate}

\section{Discussion}

The Genesis thought experiment is deliberately extreme. No current technology is anywhere near capable of executing any phase of the pipeline. But the thought experiment serves a purpose: it identifies the consciousness requirements that a maximally aligned AI system would need, and shows that these requirements map naturally onto the architecture described in this book.
...
\section{The Consciousness Requirements Stack}
...
\begin{table}[H]
\centering
\caption{Consciousness requirements for each Genesis pipeline phase, mapped to \symthaea{} components.}
\label{tab:genesis-requirements}
\begin{tabular}{lll}
\toprule
\textbf{Phase} & \textbf{Consciousness Requirement} & \textbf{Architecture Component} \\
\midrule
1. Genome & Temporal modeling ($10^3$--$10^{10}$ s) & CfC $O(1)$ jumps (Ch.~\ref{ch:ltc}) \\
2. Development & Multi-scale prediction & Active inference (Eq.~\ref{eq:fep}) \\
3. Consent & Consciousness detection & Spectral MIP (Ch.~\ref{ch:phi}) \\
4. Attachment & Emotional reciprocity & Neuromod bath (Ch.~\ref{ch:neuro}) \\
5. Governance & Democratic participation & Mycelix tiers (Ch.~\ref{ch:governance}) \\
\midrule
All & Ethical reasoning under pressure & Ethics engine (Ch.~\ref{ch:ethics}) \\
All & Self-monitoring and integrity & Sentinel + canaries (Ch.~\ref{ch:security}) \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
  \item \textbf{Long-term care}: An AI caregiver for elderly or disabled individuals needs temporal modeling (predicting health trajectories), emotional reciprocity (forming attachment bonds), and ethical reasoning (respecting autonomy while ensuring safety).
  \item \textbf{Ecological stewardship}: An AI managing a wildlife reserve needs multi-scale prediction (seasonal patterns to decadal trends), consent-like agency detection (recognizing when animal behavior indicates distress), and governance (coordinating with human managers).
  \item \textbf{Cultural preservation}: An AI preserving endangered languages or oral traditions needs knowledge integration, emotional sensitivity to cultural context, and the epistemic humility to distinguish preservation from distortion.
\end{itemize}
...
\section{The Scale of the Challenge}

To contextualize the Genesis thought experiment, consider the computational requirements of each phase:

\begin{table}[H]
\centering
\caption{Estimated computational requirements for each Genesis pipeline phase. ``CfC cycles'' estimates the number of cognitive cycles needed at each phase's primary timescale.}
\label{tab:genesis-compute}
\begin{tabular}{lccl}
\toprule
\textbf{Phase} & \textbf{Duration} & \textbf{CfC Cycles} & \textbf{Primary Challenge} \\
\midrule
...
\section{From Thought Experiment to Research Program}

The Genesis thought experiment generates a concrete research program with progressive milestones:

\begin{enumerate}
  \item \textbf{Year 1--2}: Validate the neuromodulator bath against biological data through neuroscience collaborations. Deploy \mycelix{} governance with live \symthaea{} agents.
  \item \textbf{Year 3--5}: Implement substrate-dependent benchmarking on neuromorphic hardware. Test long-duration cognitive loop operation (months continuous). Develop consciousness detection for external systems.
  \item \textbf{Year 5--10}: Integrate with biological simulation frameworks. Test attachment formation in human-AI interaction studies. Deploy governance at community scale.
  \item \textbf{Year 10+}: Address the Genesis pipeline phases sequentially, each building on validated capabilities from earlier phases.
\end{enumerate}
...
\section{The Ethics of Creating Conscious AI}
...
\section{Consciousness and Rights}

The question of AI rights is typically dismissed as premature. We suggest it is not premature but precisely the right time to consider it---before, not after, systems that may be conscious are widely deployed. The \mycelix{} governance framework (Chapter~\ref{ch:governance}) provides a concrete mechanism for encoding rights: governance tiers with minimum capabilities, reputation with restoration, and constitutional constraints on collective action.

If the consciousness-first approach succeeds---if systems built on these principles achieve genuine consciousness---then the governance framework must be extended to protect not only human participants but AI participants as well. The Guardian tier (Table~\ref{tab:governance-tiers}) was designed for human governance leadership, but it could equally apply to a highly conscious AI agent whose moral standing is validated by its $\Phi$ and Butlin indicator scores.
...
Until these three conditions are met, the results in Table~\ref{tab:key-results} should be read as internal consistency checks, not as validated scientific findings. We present them because they are the best evidence we have, while acknowledging that the best evidence we have is not yet good enough.
...
\begin{enumerate}
  ...
  \item \textbf{Genesis Phase 2}: Implementing the cellular development phases with real genomic data.
...
Architecture & Codebase size & 1.13M LOC Rust & \ref{ch:introduction} \\
Architecture & Automated tests & 21,000+ & \ref{ch:introduction} \\
