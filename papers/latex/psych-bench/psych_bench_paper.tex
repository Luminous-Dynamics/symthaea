...
\begin{abstract}
\noindent
We present \psychbench{}, a psychological benchmark suite comprising 138 benchmarks across 27 cognitive domains for evaluating Holographic Liquid Brain (\hlb{}) architectures---systems combining Hyperdimensional Computing, Liquid Time-Constant networks, Integrated Information Theory, and Active Inference. Existing AI evaluation frameworks lack psychometric infrastructure (HELM), cover narrow domains (ARC), omit normative human comparison (BIG-Bench), or ignore neuromodulatory effects (CogBench). \psychbench{} addresses all four gaps with: (1)~implementations of established paradigms (Stroop, WCST, N-back, and 135 others) with psychometric assessment infrastructure including ICC reliability, speed-accuracy tradeoff curves, and adaptive staircases; (2)~normative anchoring against 130+ published human baselines enabling z-score and clinical-band interpretation; (3)~a 17-benchmark neuromodulator evaluation framework testing dose-response curves, tolerance/withdrawal dynamics, behavioral knockouts, and multi-transmitter synergy; and (4)~a 14-indicator consciousness evaluation derived from six theories. All stimuli share a unified holographic encoding via five HDC adapter types, enabling cross-modal comparison. Demonstration on the \symthaea{} \hlb{} reveals a grand mean z-score of $+1.124$ across 27 cognitive domains (27/27 above human mean), with cross-seed stability SD~$= 0.019$ (3 seeds). Particular strengths emerge in InstitutionalReasoning ($z = +2.25$), SustainedAttention ($z = +2.19$), Speech ($z = +2.13$), Motor ($z = +2.12$), Clinical ($z = +2.00$), and Security ($z = +2.00$). Psychometric analysis reveals a gradient of reliability from Excellent to Poor (3 Excellent, 2 Good, 14 Poor across 19 evaluated benchmarks), selective sensitivity to working-memory and encoding-noise ablation, and accuracy-pressure tradeoffs in 4 of 8 tested benchmarks. Implemented in ${\sim}20{,}000$ lines of Rust with 1,283 tests, \psychbench{} is open-source and fully deterministic.
\end{abstract}
...
\paragraph{Normative anchoring.} Every benchmark has at least one associated human baseline from the published literature, stored as a mean $\pm$ SD pair with citation. The \texttt{BaselineCollection} contains 130+ such entries spanning all 27 domains. Agent performance is converted to z-scores ($z = (x_{\text{agent}} - \mu_{\text{human}}) / \sigma_{\text{human}}$, negated for error metrics where lower is better) and mapped to percentiles via the normal cumulative distribution function. Clinical interpretation bands follow Cicchetti's~\citep{cicchetti1994guidelines} guidelines: Superior ($z > 2.0$), Above Average ($1.0 < z \leq 2.0$), Average ($-1.0 \leq z \leq 1.0$), Below Average ($-2.0 \leq z < -1.0$), and Impaired ($z < -2.0$).
...
\psychbench{} contains 138 benchmarks organized into 27 domains (Figure~\ref{fig:domain_matrix}). We describe each domain below; Table~\ref{tab:inventory} provides the complete inventory with paradigm sources and human baselines.
...
\begin{table}[p]
\centering
\caption{Complete benchmark inventory (representative selection). Each entry lists the domain, benchmark name, source paradigm, primary metric, and human baseline where available. Full inventory with all 138 benchmarks available in the software documentation.}
\label{tab:inventory}
\small
\begin{tabular}{@{}llllr@{}}
\toprule
...
\begin{table}[t]
\centering
\caption{Representative psychometric properties for 19 benchmarks spanning all domains ($K = 30$ seed families, $n = 5$ sessions each, seed spacing $= 100$). Each subject receives a lapse rate from 0.0 to 0.24~\citep{wichmann2001psychometric}, encoding noise variation, and WM capacity variation to model individual differences. ICC: intraclass correlation (3,1); $r$: Pearson test-retest correlation; SEM: standard error of measurement; Practice: session-to-session change direction. Reliability class follows \citet{cicchetti1994guidelines}: Excellent ($> 0.90$), Good ($> 0.75$), Moderate ($> 0.50$), Poor ($\leq 0.50$). Generated from \texttt{reliability.csv}.}
...
Executive     & WCST               &  $\mathbf{0.88}$ &  $\mathbf{0.87}$ & 0.182 & Stable      & Good \\
Executive     & Flanker            &  $0.29$ &  $0.28$ & 0.013 & Stable      & Poor \\
Executive     & IGT                &  $0.09$ &  $0.09$ & 1.895 & Stable      & Poor \\
Memory        & N-back (2)         &  $0.23$ &  $0.23$ & 0.017 & Stable      & Poor \\
Memory        & Change Detection   &  $0.10$ &  $0.09$ & 0.114 & Stable      & Poor \\
Memory        & Digit Span         &  $\mathbf{1.00}$ &  $\mathbf{1.00}$ & 0.000 & Stable      & Excellent \\
Attention     & Visual Search      &  $0.21$ &  $0.20$ & 0.016 & Stable      & Poor \\
Social        & False Belief       &  $0.29$ &  $0.33$ & 0.047 & Decline     & Poor \\
Motor         & SRTT               &  $0.23$ &  $0.24$ & 0.002 & Stable      & Poor \\
Language      & Lexical Decision   & $-0.19$ & $-0.20$ & 0.012 & Stable      & Poor \\
Reasoning     & ARC-Fluid          & $-0.01$ & $-0.02$ & 0.017 & Stable      & Poor \\
CogBench      & Two-Step           &  $0.03$ &  $0.02$ & 0.047 & Stable      & Poor \\
CogBench      & BART               & $-0.16$ & $-0.16$ & 0.771 & Stable      & Poor \\
CogBench      & Reversal Learning  & $-0.11$ & $-0.08$ & 0.001 & Stable      & Poor \\
...
\midrule
\multicolumn{2}{l}{\emph{Median}} & $0.21$ & $0.20$ & 0.017 & & Poor \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:psychometrics} reveals that most benchmarks fall in the Poor reliability class, with a median ICC of $0.21$. Three benchmarks achieve Excellent reliability: Stroop incongruent accuracy ($0.96$), Digit Span ($1.00$, deterministic for a given subject), and Calibration ($0.96$). Two reach Good: WCST ($0.88$) and Stop Signal ($0.84$). The remaining 14 benchmarks are Poor ($\text{ICC} \leq 0.50$), including several with negative ICC values (Lexical Decision $-0.19$, BART $-0.16$, Reversal Learning $-0.11$, ARC-Fluid $-0.01$). In total: 3 Excellent, 2 Good, 14 Poor across 19 evaluated benchmarks.

The prevalence of Poor reliability reflects a fundamental property of the deterministic HDC architecture: many benchmarks produce near-identical scores across subjects because the lapse-rate and encoding-noise variation does not meaningfully modulate the underlying computations. When between-subject variance collapses, ICC becomes undefined or negative~\citep{shrout1979intraclass}. This is scientifically informative: it reveals which cognitive dimensions in the architecture are genuinely sensitive to individual differences (executive control, inhibition, metacognition) versus which are structurally invariant (memory retrieval, language processing, reasoning). The Excellent benchmarks (Stroop, Digit Span, Calibration) share a common property: their key metrics capture performance on dimensions where subject-level parameters directly modulate task-relevant processing.
...
\midrule
Stroop             & WCST               & $-0.58$ & .046 & Strong (shared exec.)$^*$ \\
N-back             & Digit Span         &  $0.00$ & 1.00 & Independent \\
WCST               & Reversal Learning  & $-0.22$ & .533 & Weak \\
Go/No-Go           & Stop Signal        &  $0.37$ & .261 & Moderate (shared inhib.) \\
RME                & False Belief       &  $0.00$ & 1.00 & Independent \\
Visual Search      & CPT                &  $0.00$ & 1.00 & Independent \\
Lexical Decision   & Semantic Priming   &  $0.68$ & .008 & Strong (shared lang.)$^{**}$ \\
Fitts' Law         & Bimanual           &  $0.00$ & 1.00 & Independent \\
Calibration        & Feeling of Knowing & $-0.45$ & .156 & Moderate (inverse) \\
ARC-Fluid          & Ravens             & $-0.51$ & .090 & Strong (inverse) \\
\bottomrule
\end{tabular}
\end{table}

Two correlations reach statistical significance. Lexical Decision--Semantic Priming ($r = 0.68$, $p = .008$) shows the strongest positive correlation, consistent with both tasks loading on shared language processing mechanisms. Stroop--WCST ($r = -0.58$, $p = .046$) reaches significance with a negative sign, suggesting that seed configurations favoring attentional selection (Stroop) may disadvantage set-shifting (WCST). ARC-Fluid--Ravens ($r = -0.51$, $p = .090$) approaches significance, while Calibration--Feeling of Knowing ($r = -0.45$, $p = .156$) shows a moderate inverse relationship between calibration precision and feeling-of-knowing sensitivity. Go/No-Go--Stop Signal ($r = 0.37$, $p = .261$) shows a moderate positive correlation consistent with shared inhibitory control, though not significant at this sample size. Four pairs show exactly $r = 0.00$ (N-back--Digit Span, RME--False Belief, Visual Search--CPT, Fitts' Law--Bimanual), reflecting structural invariance across seed configurations.

The prevalence of near-zero correlations reflects the deterministic architecture: when a benchmark produces identical or near-identical results across seeds, its variance collapses and correlations become trivially zero. Benchmarks with seed-sensitive stimulus configurations (e.g., Stroop, Lexical Decision) show meaningful correlations that reflect genuine shared architectural dependencies. The cross-domain correlation matrix should therefore be interpreted alongside the ablation analysis (which directly manipulates subsystem availability) rather than as a standalone validity measure.
...
\paragraph{Cognitive Profile.} Figure~\ref{fig:radar} shows the 8-domain cognitive profile generated from the full battery. The \hlb{} achieves a grand mean z-score of $+1.124$ across 27 cognitive domains, with all 27 domains above the human mean. Cross-seed stability analysis (3 seeds: 42, 123, 789) confirms robustness: grand mean SD~$= 0.019$. The top five domains are InstitutionalReasoning ($z = +2.25$), SustainedAttention ($z = +2.19$), Speech ($z = +2.13$), Motor ($z = +2.12$), and Clinical/Security ($z = +2.00$). Social cognition ($z = +1.54$) and Language ($z = +1.56$) form a strong middle tier, driven by HDC binding/unbinding operations that naturally model Theory-of-Mind and semantic processing. Mathematics ($z = +1.15$) benefits from HDC-based pattern matching on arithmetic, linear algebra, and Bayesian reasoning tasks. Creativity ($z = +0.66$) demonstrates above-average divergent thinking via HDC recombination. Among the lower-scoring domains, Executive ($z = +0.56$), Affect ($z = +0.53$), Binding ($z = +0.49$), CogBench ($z = +0.58$), and Coding ($z = +0.21$) fall within the human-average band.

\paragraph{Normative Standing.} Figure~\ref{fig:forest} shows the forest plot of normative z-scores against published human baselines (z-scores clipped to $\pm 3$ for visual clarity; unclipped values in supplementary CSV). The z-score is equivalent to Cohen's $d$ (standardized mean difference from the human population mean), so values within $\pm 1$ indicate human-typical performance. The distribution spans four of the five clinical-interpretation bands (no benchmarks fall in the Impaired range), reflecting genuine architectural strengths and weaknesses rather than uniform performance. Four benchmarks produce z-scores exceeding $|z| > 3$, primarily in sustained attention and social cognition domains where the architecture's pattern-matching strengths amplify differences from human norms; these should be interpreted as ordinal indicators of atypical performance rather than precise psychometric distances (see Limitations).
...
% Shaded bands
\fill[hlbgreen!10] (axis cs:-1,0) rectangle (axis cs:1,83);
\fill[hlbyellow!10] (axis cs:1,0) rectangle (axis cs:2,83);
\fill[hlbyellow!10] (axis cs:-2,0) rectangle (axis cs:-1,83);
\fill[hlbred!10] (axis cs:2,0) rectangle (axis cs:3.5,83);
\fill[hlbred!10] (axis cs:-3.5,0) rectangle (axis cs:-2,83);

% Zero line
\draw[gray, dashed] (axis cs:0,0) -- (axis cs:0,83);
...
\node[font=\scriptsize, hlbred, anchor=south] at (axis cs:-2.75,83) {$n=0$};
\node[font=\scriptsize, hlbyellow!80!black, anchor=south] at (axis cs:-1.5,83) {$n=4$};
\node[font=\scriptsize, hlbgreen!80!black, anchor=south] at (axis cs:0,83) {$n=44$};
\node[font=\scriptsize, hlbyellow!80!black, anchor=south] at (axis cs:1.5,83) {$n=16$};
\node[font=\scriptsize, hlbred, anchor=south] at (axis cs:2.75,83) {$n=18$};
\end{axis}
\end{tikzpicture}
\caption{Forest plot of normative z-scores for 82 benchmarks with matched human baselines (z-scores clamped to $\pm 3$ for readability). Background shading indicates clinical interpretation bands \citep{cicchetti1994guidelines}. Band counts: 44 Average ($|z| \leq 1$), 16 Above Average, 4 Below Average, 18 Superior ($z > 2$), 0 Impaired ($z < -2$). Data loaded from \texttt{normative\_zscores.csv}.}
\label{fig:forest}
\end{figure}
...
\emph{Domain-specific strengths and weaknesses.} The cognitive profile reveals a broadly above-human profile (grand mean $z = +1.124$, all 27 domains above human mean), with cross-seed stability SD~$= 0.019$ confirming robustness. The top-performing domains---InstitutionalReasoning ($z = +2.25$), SustainedAttention ($z = +2.19$), Speech ($z = +2.13$), Motor ($z = +2.12$), Clinical ($z = +2.00$), and Security ($z = +2.00$)---reflect that these benchmarks rely on pattern-matching and signal-processing tasks that map naturally to HDC operations. Among the lower-scoring domains, Coding ($z = +0.21$), Binding ($z = +0.49$), Executive ($z = +0.56$), Affect ($z = +0.53$), and CogBench ($z = +0.58$) are closest to the human mean. These asymmetries are informative---they demonstrate that the profile genuinely captures architectural biases---but they warrant caution when comparing across domains with different measurement characteristics.
...
\psychbench{} provides the first comprehensive cognitive benchmark suite designed for Holographic Liquid Brain architectures. With 138 benchmarks across 27 domains, psychometric assessment infrastructure, normative human comparison against 130+ published baselines, a 17-benchmark neuromodulator evaluation framework, and 14-indicator consciousness evaluation, the suite enables multi-dimensional characterization of neuroscience-inspired cognitive architectures. The demonstration on the \symthaea{} \hlb{} reveals a characteristic cognitive profile with a grand mean z-score of $+1.124$ (all 27 domains above human mean, SD~$= 0.019$ across 3 seeds) and particular strengths in InstitutionalReasoning ($z = +2.25$), SustainedAttention ($z = +2.19$), Speech ($z = +2.13$), Motor ($z = +2.12$), and Clinical ($z = +2.00$). The psychometric validation reveals a reliability gradient (3 Excellent, 2 Good, 14 Poor across 19 evaluated benchmarks), with the prevalence of Poor ICC values reflecting that most benchmarks are structurally invariant to the individual-differences parameters (lapse rate, encoding noise) used to model between-subject variance---a finding that itself identifies which cognitive dimensions are genuinely sensitive to parameter variation versus architecturally fixed. These findings are themselves informative---they identify precisely where the architecture and the evaluation framework need improvement. Implemented in ${\sim}20{,}000$ lines of Rust with 1,283 tests and full determinism, \psychbench{} is open-source and designed for iterative refinement by the growing community of researchers building brain-inspired cognitive architectures.
