...
\paragraph{Cognitive Profile.} Figure~\ref{fig:radar} shows the 8-domain cognitive profile generated from the full battery. The \hlb{} achieves a grand mean z-score of $+1.156$ across 26 cognitive domains, with all 26 domains above the human mean. Cross-seed stability analysis (3 seeds: 42, 123, 789) confirms robustness: grand mean SD~$= 0.019$. The top five domains are InstitutionalReasoning ($z = +2.25$), SustainedAttention ($z = +2.19$), Speech ($z = +2.13$), Motor ($z = +2.12$), and Clinical/Security ($z = +2.00$). Social cognition ($z = +1.54$) remains strong, driven by similarity-based Theory-of-Mind tests and game-theoretic benchmarks where holographic pattern matching maps naturally to the task structure. Mathematics ($z = +1.15$) benefits from HDC-based pattern matching on arithmetic, linear algebra, and Bayesian reasoning tasks. Among the lower-scoring domains, Binding ($z = +0.49$), Executive ($z = +0.49$), Affect ($z = +0.53$), CogBench ($z = +0.58$), and Creativity ($z = +0.66$) all fall within the human-average band.
...
\emph{Domain-specific strengths and weaknesses.} The cognitive profile reveals a broadly above-human profile (grand mean $z = +1.156$, all 26 domains above human mean), with cross-seed stability SD~$= 0.019$ confirming robustness. The top-performing domains---InstitutionalReasoning ($z = +2.25$), SustainedAttention ($z = +2.19$), Speech ($z = +2.13$), Motor ($z = +2.12$), Clinical ($z = +2.00$), and Security ($z = +2.00$)---reflect that these benchmarks rely on pattern-matching and signal-processing tasks that map naturally to HDC operations. Among the lower-scoring domains, Binding ($z = +0.49$), Executive ($z = +0.49$), Affect ($z = +0.53$), CogBench ($z = +0.58$), and Creativity ($z = +0.66$) are closest to the human mean. These asymmetries are informative---they demonstrate that the profile genuinely captures architectural biases---but they warrant caution when comparing across domains with different measurement characteristics.
