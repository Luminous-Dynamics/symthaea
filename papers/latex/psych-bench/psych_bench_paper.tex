...
\psychbench{} contains 135+ benchmarks organized into 26 domains (Figure~\ref{fig:domain_matrix}). We describe each domain below; Table~\ref{tab:inventory} provides the complete inventory with paradigm sources and human baselines.
...
\begin{table}[p]
\centering
\caption{Complete benchmark inventory (representative selection). Each entry lists the domain, benchmark name, source paradigm, primary metric, and human baseline where available. Full inventory with all 135+ benchmarks available in the software documentation.}
\label{tab:inventory}
\small
\begin{tabular}{@{}llllr@{}}
\toprule
...
\paragraph{Cognitive Profile.} Figure~\ref{fig:radar} shows the 8-domain cognitive profile generated from the full battery. The \hlb{} achieves a grand mean z-score of $+1.150$ across 26 cognitive domains, with 26/26 domains above the human mean and 0 below. Cross-seed stability analysis (3 seeds: 42, 123, 789) confirms robustness: grand mean $= 1.150 \pm 0.019$ (SD). The top five domains are InstitutionalReasoning ($z = +2.25$), SustainedAttention ($z = +2.19$), Speech ($z = +2.13$), Motor ($z = +2.12$), and Clinical/Security ($z = +2.00$). Social cognition ($z = +1.54$) remains strong, driven by similarity-based Theory-of-Mind tests and game-theoretic benchmarks where holographic pattern matching maps naturally to the task structure. The lowest-scoring domains---Binding ($z = +0.38$), Executive ($z = +0.47$), Affect ($z = +0.53$), CogBench ($z = +0.58$), and Butlin ($z = +0.64$)---all remain positive, within the human-average or above-average bands.
...
\emph{Domain-specific strengths and weaknesses.} The cognitive profile reveals a broadly above-human profile (grand mean $z = +1.150$, 26/26 domains above human mean), with cross-seed stability SD~$= 0.019$ confirming robustness. The top-performing domains---InstitutionalReasoning ($z = +2.25$), SustainedAttention ($z = +2.19$), Speech ($z = +2.13$), Motor ($z = +2.12$), Clinical ($z = +2.00$), and Security ($z = +2.00$)---reflect that these benchmarks rely on pattern-matching and signal-processing tasks that map naturally to HDC operations. The lowest-scoring domains---Binding ($z = +0.38$), Executive ($z = +0.47$), Affect ($z = +0.53$), CogBench ($z = +0.58$), and Butlin ($z = +0.64$)---remain positive but closer to the human mean. These asymmetries are informative---they demonstrate that the profile genuinely captures architectural biases---but they warrant caution when comparing across domains with different measurement characteristics.
...
\psychbench{} provides the first comprehensive cognitive benchmark suite designed for Holographic Liquid Brain architectures. With 135+ benchmarks across 26 domains, psychometric assessment infrastructure, normative human comparison against 130+ published baselines, a 17-benchmark neuromodulator evaluation framework, and 14-indicator consciousness evaluation, the suite enables multi-dimensional characterization of neuroscience-inspired cognitive architectures. The demonstration on the \symthaea{} \hlb{} reveals both the suite's strengths and its current limitations: the architecture produces a characteristic cognitive profile with a grand mean z-score of $+1.150$ (26/26 domains above human mean, SD~$= 0.019$ across 3 seeds) and particular strengths in InstitutionalReasoning ($z = +2.25$), SustainedAttention ($z = +2.19$), Speech ($z = +2.13$), Motor ($z = +2.12$), and Clinical ($z = +2.00$) that is interpretable and reproducible. The psychometric validation reveals improved reliability after adopting a lapse-rate individual-differences model~\citep{wichmann2001psychometric} (7 Excellent, 2 Good, 6 Moderate, 3 Poor across 18 evaluated benchmarks), with the Poor benchmarks (Remote Associates, ICC $= 0.13$; Alternate Uses, ICC $\approx 0$; Feeling of Knowing, ICC $= 0.18$) reflecting fundamental HDC limitations in semantic distance encoding and gamma-correlation computation---scientifically honest limitations that themselves validate the evaluation framework's discriminative power. These findings are themselves informative---they identify precisely where the architecture and the evaluation framework need improvement. Implemented in ${\sim}20{,}000$ lines of Rust with 1,119+ unit tests and full determinism, \psychbench{} is open-source and designed for iterative refinement by the growing community of researchers building brain-inspired cognitive architectures.
...
\psychbench{} is open-source under AGPL-3.0. The complete source code, including all 135+ benchmark implementations, harness infrastructure, and data generation scripts, is available at:
