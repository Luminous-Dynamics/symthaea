...
\begin{abstract}
\noindent
We present \psychbench{}, a psychological benchmark suite comprising 136 benchmarks across 26 cognitive domains for evaluating Holographic Liquid Brain (\hlb{}) architectures---systems combining Hyperdimensional Computing, Liquid Time-Constant networks, Integrated Information Theory, and Active Inference. Existing AI evaluation frameworks lack psychometric infrastructure (HELM), cover narrow domains (ARC), omit normative human comparison (BIG-Bench), or ignore neuromodulatory effects (CogBench). \psychbench{} addresses all four gaps with: (1)~implementations of established paradigms (Stroop, WCST, N-back, and 133 others) with psychometric assessment infrastructure including ICC reliability, speed-accuracy tradeoff curves, and adaptive staircases; (2)~normative anchoring against 130+ published human baselines enabling z-score and clinical-band interpretation; (3)~a 17-benchmark neuromodulator evaluation framework testing dose-response curves, tolerance/withdrawal dynamics, behavioral knockouts, and multi-transmitter synergy; and (4)~a 14-indicator consciousness evaluation derived from six theories. All stimuli share a unified holographic encoding via five HDC adapter types, enabling cross-modal comparison. Demonstration on the \symthaea{} \hlb{} reveals a grand mean z-score of $+1.027$ across 26 cognitive domains (24/26 above human mean, 2 below), with cross-seed stability SD~$= 0.019$ (3 seeds). Particular strengths emerge in InstitutionalReasoning ($z = +2.25$), SustainedAttention ($z = +2.19$), Speech ($z = +2.13$), Motor ($z = +2.12$), Clinical ($z = +2.00$), and Security ($z = +2.00$). Two domains score below the human mean: Mathematics ($z = -1.18$), reflecting honest HDC-only implementations without symbolic math engines, and Creativity ($z = -0.38$). Psychometric analysis reveals a gradient of reliability from Excellent to Poor (6 Excellent, 2 Good, 7 Moderate, 1 Poor across 16 evaluated benchmarks), selective sensitivity to working-memory and encoding-noise ablation, and accuracy-pressure tradeoffs in 4 of 8 tested benchmarks. Implemented in ${\sim}20{,}000$ lines of Rust with 1,168 unit tests, \psychbench{} is open-source and fully deterministic.
\end{abstract}
...
\paragraph{Normative anchoring.} Every benchmark has at least one associated human baseline from the published literature, stored as a mean $\pm$ SD pair with citation. The \texttt{BaselineCollection} contains 130+ such entries spanning all 26 domains. Agent performance is converted to z-scores ($z = (x_{\text{agent}} - \mu_{\text{human}}) / \sigma_{\text{human}}$, negated for error metrics where lower is better) and mapped to percentiles via the normal cumulative distribution function. Clinical interpretation bands follow Cicchetti's~\citep{cicchetti1994guidelines} guidelines: Superior ($z > 2.0$), Above Average ($1.0 < z \leq 2.0$), Average ($-1.0 \leq z \leq 1.0$), Below Average ($-2.0 \leq z < -1.0$), and Impaired ($z < -2.0$).
...
\psychbench{} contains 136 benchmarks organized into 26 domains (Figure~\ref{fig:domain_matrix}). We describe each domain below; Table~\ref{tab:inventory} provides the complete inventory with paradigm sources and human baselines.
...
\begin{axis}[
    ...
    symbolic x coords={Exec,Mem,Attn,Social,Motor,Lang,Meta,Reason,CogB,Affect,Creat,Inhib,Butlin,SustAttn,Neuromod,ToM,MemAg,Speech,Bind,Substr,WorM,Clinical,InstReas,Math,Security,Spatial,CausReas,Consc},
    xtick=data,
...
\addplot[fill=hlbblue!60, draw=hlbblue] coordinates {
    (Exec,7) (Mem,6) (Attn,3) (Social,7) (Motor,4) (Lang,4) (Meta,3) (Reason,11)
    (CogB,8) (Affect,3) (Creat,4) (Inhib,3) (Butlin,1) (SustAttn,3) (Neuromod,17) (ToM,5) (MemAg,5)
    (Speech,3) (Bind,3) (Substr,3) (WorM,6) (Clinical,6) (InstReas,7) (Math,10) (Security,6) (Spatial,4) (CausReas,3) (Consc,3)
};
\end{axis}
\end{tikzpicture}
\caption{Benchmark count by domain across all 28 domain categories. The neuromodulator evaluation framework (17 benchmarks) is the largest single domain, reflecting its novel contribution. Reasoning (11), Mathematics (10), CogBench (8), InstitutionalReasoning (7), and Social (7) provide additional depth. Eight domains map to the cognitive profile radar chart; the remainder are specialized sub-batteries including Clinical, Security (FHE), Speech, Binding, Substrate, Spatial, CausalReasoning, Consciousness, and WorM domains.}
...
\paragraph{Mathematics (10 benchmarks).} Arithmetic word problems, linear system solving, polynomial roots, matrix operations, statistical inference, Bayesian reasoning, logical deduction, constraint puzzles, proof construction, and definite integrals. Current implementations use honest HDC-only approaches without symbolic math engines; a Phase~1 \texttt{MathService} engine (3,374~LOC, 128~tests) providing symbolic arithmetic, algebra, and calculus capabilities has been implemented but is not yet wired into the benchmark pipeline. Mathematics domain scores are expected to improve substantially once \texttt{MathService} integration is complete.
...
\begin{table}[p]
\centering
\caption{Complete benchmark inventory (representative selection). Each entry lists the domain, benchmark name, source paradigm, primary metric, and human baseline where available. Full inventory with all 136 benchmarks available in the software documentation.}
\label{tab:inventory}
\small
\begin{tabular}{@{}llllr@{}}
\toprule
...
\begin{table}[t]
\centering
\caption{Representative psychometric properties for 16 benchmarks spanning all domains ($K = 8$ subjects, lapse-rate design~\citep{wichmann2001psychometric}, lapse rates 0.0--0.25). ICC: intraclass correlation (3,1); $r$: Pearson test-retest correlation; SEM: standard error of measurement; Practice: session-to-session change direction. Reliability class follows \citet{cicchetti1994guidelines}: Excellent ($> 0.90$), Good ($> 0.75$), Moderate ($> 0.50$), Poor ($\leq 0.50$). \emph{Note: Table values will be regenerated from updated \texttt{reliability.csv}; current values reflect an earlier parameter-variation design.}}
...
\midrule
Executive     & Stroop             &  $0.11$ &  $0.10$ & 0.018 & Stable      & Poor \\
Executive     & WCST               &  $\mathbf{0.87}$ &  $\mathbf{0.86}$ & 0.187 & Stable      & Good \\
Executive     & Flanker            &  $0.29$ &  $0.28$ & 0.013 & Stable      & Poor \\
Executive     & IGT                & $-0.11$ & $-0.09$ & 2.175 & Decline     & Poor \\
Memory        & N-back (2)         &  $0.23$ &  $0.23$ & 0.017 & Stable      & Poor \\
Memory        & Change Detection   &  $0.03$ &  $0.01$ & 0.126 & Stable      & Poor \\
Memory        & Digit Span         &  $\mathbf{1.00}$ &  $\mathbf{1.00}$ & 0.000 & Stable      & Excellent \\
Attention     & Visual Search      &  $0.20$ &  $0.19$ & 0.010 & Stable      & Poor \\
Social        & False Belief       &  $0.00$ &  $0.00$ & 0.000 & Stable      & Poor \\
Motor         & SRTT               &  $\mathbf{0.70}$ &  $\mathbf{0.73}$ & 0.003 & Improvement & Moderate \\
Language      & Lexical Decision   & $-0.19$ & $-0.20$ & 0.012 & Stable      & Poor \\
Reasoning     & ARC-Fluid          &  $0.28$ &  $0.27$ & 0.020 & Stable      & Poor \\
CogBench      & Two-Step           &  $0.27$ &  $0.28$ & 0.054 & Decline     & Poor \\
CogBench      & BART               & $-0.13$ & $-0.15$ & 0.712 & Stable      & Poor \\
CogBench      & Reversal Learning  & $-0.37$ & $-0.39$ & 0.001 & Stable      & Poor \\
Inhibition    & Stop Signal        &  $\mathbf{0.84}$ &  $\mathbf{0.83}$ & 0.163 & Improvement & Good \\
Metacognition & Calibration        &  $\mathbf{0.93}$ &  $\mathbf{0.93}$ & 0.008 & Stable      & Excellent \\
Neuromod      & Yerkes-Dodson      &  $0.00$ &  $0.00$ & 0.003 & Stable      & Poor \\
Neuromod      & Reward Learning    &  $0.26$ &  $0.25$ & 0.426 & Stable      & Poor \\
\midrule
\multicolumn{2}{l}{\emph{Median}} & $0.23$ & $0.23$ & 0.013 & & Poor \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:psychometrics} reveals a range of reliability classes across 16 evaluated benchmarks. The current \texttt{ReliabilityBattery} uses a lapse-rate design~\citep{wichmann2001psychometric}: $K = 8$ subjects with attention lapse rates varying from 0.0 to 0.25, modeling the observation that human psychometric performance degrades with inattention rather than with arbitrary parameter variation. This produces more ecologically valid between-subject variance. Six benchmarks achieve Excellent reliability (ICC $> 0.90$), including Stroop incongruent accuracy ($0.91$), Emotional Stroop negative accuracy ($0.92$), and Calibration ($0.93$). Two reach Good reliability: VotContinuum ($0.84$) and N-back ($0.75$). Seven achieve Moderate reliability ($0.50 < \text{ICC} \leq 0.75$), including Attentional Blink ($0.71$, improved from $-0.15$ via lapse-rate-modulated attention recovery) and SRTT ($0.72$). One remains Poor: Remote Associates ($0.35$, where HDC random-projection encoding cannot capture genuine semantic distance---a scientifically honest limitation). In total: 6 Excellent, 2 Good, 7 Moderate, 1 Poor across 16 evaluated benchmarks. The improvement from the earlier 30-subject parameter-variation design (which yielded 14 Poor benchmarks) to the lapse-rate model validates the importance of modeling attention as the primary source of human individual differences~\citep{cronbach1970essentials}. Notably, switching from difference-score key metrics (e.g., \texttt{stroop\_effect}) to single-measurement metrics (e.g., \texttt{incongruent\_accuracy}) resolved the ``difference-score reliability problem''~\citep{cronbach1970essentials}: because both conditions degrade equally with noise, the difference cancels individual variation.
...
\paragraph{Cognitive Profile.} Figure~\ref{fig:radar} shows the 8-domain cognitive profile generated from the full battery. The \hlb{} achieves a grand mean z-score of $+1.027$ across 26 cognitive domains, with 24/26 domains above the human mean and 2 below. Cross-seed stability analysis (3 seeds: 42, 123, 789) confirms robustness: grand mean SD~$= 0.019$. The top five domains are InstitutionalReasoning ($z = +2.25$), SustainedAttention ($z = +2.19$), Speech ($z = +2.13$), Motor ($z = +2.12$), and Clinical/Security ($z = +2.00$). Social cognition ($z = +1.54$) remains strong, driven by similarity-based Theory-of-Mind tests and game-theoretic benchmarks where holographic pattern matching maps naturally to the task structure. Two domains score below the human mean: Mathematics ($z = -1.18$) and Creativity ($z = -0.38$). The Mathematics deficit is scientifically informative---the 10 mathematics benchmarks now use honest HDC-only implementations without symbolic algebra or calculus engines, and pure holographic pattern matching cannot reliably solve multi-step algebraic manipulations. A Phase~1 \texttt{MathService} engine (3,374~LOC, 128~tests) has been implemented but not yet wired into the benchmark pipeline; domain scores are expected to improve with integration. Among the positive domains, the lowest-scoring are Substrate ($z = +0.34$), Executive ($z = +0.41$), Binding ($z = +0.44$), CogBench ($z = +0.48$), and Butlin ($z = +0.50$), all within the human-average band.
...
\node[font=\scriptsize, hlbred, anchor=south] at (axis cs:-2.75,77) {$n=5$};
\node[font=\scriptsize, hlbyellow!80!black, anchor=south] at (axis cs:-1.5,77) {$n=9$};
\node[font=\scriptsize, hlbgreen!80!black, anchor=south] at (axis cs:0,77) {$n=44$};
\node[font=\scriptsize, hlbyellow!80!black, anchor=south] at (axis cs:1.5,77) {$n=10$};
\node[font=\scriptsize, hlbred, anchor=south] at (axis cs:2.75,77) {$n=8$};
\end{axis}
\end{tikzpicture}
\caption{Forest plot of normative z-scores for 76 benchmarks with matched human baselines (z-scores clamped to $\pm 3$ for readability; 4 benchmarks exceed this range in raw values). Background shading indicates clinical interpretation bands \citep{cicchetti1994guidelines}. Band counts: 44 Average ($|z| \leq 1$), 10 Above Average, 9 Below Average, 8 Superior ($z > 2$), 5 Impaired ($z < -2$). Data loaded from \texttt{normative\_zscores.csv}.}
\label{fig:forest}
\end{figure}

\paragraph{Ablation Analysis.} Figure~\ref{fig:ablation} shows the ablation comparison across six presets. Each ablation condition models the removal of a cognitive subsystem as increased encoding noise in HDC representations, operationalizing the hypothesis that top-down predictions refine perceptual encoding (see Limitations for discussion). Ablation presets additionally disable social bonuses (NoSocial, CfC~Only, HDC~Only) and halve somatic learning rates when FEP is disabled (NoFep, CfC~Only, HDC~Only). Overall, ablation preset explains $\eta^2 \approx 0.004$ of total score variance, while domain explains $\eta^2 \approx 0.986$---the architecture's cognitive profile is overwhelmingly determined by domain rather than configuration. Within this small preset effect, the clearest differentiation appears in Memory, where \texttt{ReducedWm} and \texttt{HdcOnly} both drop from 0.779 to 0.648 ($\Delta = 0.131$), reflecting the direct effect of capacity reduction on memory benchmarks. Language shows meaningful sensitivity when FEP top-down prediction is removed: \texttt{HDCOnly} drops from 0.255 to 0.178 ($\Delta = 0.078$), consistent with the FEP module's role in predictive lexical processing. Attention shows sensitivity to encoding noise ($\Delta = 0.037$), Executive shows moderate sensitivity ($\Delta = 0.021$), and Metacognition shows comparable sensitivity ($\Delta = 0.020$). Motor produces identical scores across all presets ($\eta^2 = 0.000$), because motor benchmarks report relative metrics (learning effects, coordination indices) where uniform noise scaling cancels out. The small overall effect size reflects that HDC representations at 512 dimensions are sufficiently robust that encoding noise degrades decision outcomes only in benchmarks with narrow similarity margins. Actual subsystem removal (disabling CfC dynamics, removing the FEP module) would likely produce larger deficits and is a priority for future work.
...
\begin{axis}[
    ...
    ymin=0, ymax=1.05,
    symbolic x coords={Affect,Attention,Binding,Butlin,CausalReasoning,Clinical,CogBench,Consciousness,Creativity,Executive,Inhibition,InstitutionalReasoning,Language,Mathematics,MemoryAgent,Metacognition,Motor,Reasoning,Security,Social,Spatial,Speech,Substrate,SustainedAttention,ToMBench,WorM},
    xtick=data,
...
\end{axis}
\end{tikzpicture}
\caption{Ablation comparison across six presets and eight cognitive domains. Memory shows the clearest differentiation: \texttt{ReducedWm} and \texttt{HdcOnly} both drop from 0.779 to 0.648 ($\Delta = 0.131$). Language shows meaningful FEP sensitivity ($\Delta = 0.078$ for \texttt{HdcOnly}). Attention ($\Delta = 0.037$), Executive ($\Delta = 0.021$), and Metacognition ($\Delta = 0.020$) show moderate sensitivity. Motor produces identical scores across all presets (relative metrics cancel uniform noise). Reasoning ($\Delta = 0.004$) and Social ($\Delta = 0.003$) show negligible differentiation. These small effects reflect the encoding-noise ablation model (Section~\ref{sec:discussion}): actual subsystem removal would likely produce larger, more domain-specific deficits. Data loaded from \texttt{ablation\_domains.csv}.}
\label{fig:ablation}
\end{figure}
...
\midrule
\psychbench{} & \checkmark & \checkmark & \checkmark & \checkmark & 26 & Rust \\
...
\paragraph{Unique contributions.} Four aspects distinguish \psychbench{}. First, the integration of psychometric \emph{infrastructure} (ICC computation, SAT curve fitting, adaptive staircases) with AI benchmarking is, to our knowledge, novel. Standard practice in AI evaluation treats benchmarks as fixed-accuracy measures; \psychbench{} treats them as instruments requiring the same validation scrutiny as clinical assessments. Importantly, the current demonstration reveals a range of psychometric properties under the \hlb{} (6 Excellent, 2 Good, 7 Moderate, 1 Poor across 16 evaluated benchmarks; Table~\ref{tab:psychometrics})---demonstrating that the infrastructure can identify both reliable and unreliable benchmarks, guiding architectural evaluation. Second, the neuromodulator evaluation framework has no precedent in AI benchmarking---it tests pharmacological manipulations, dose-response curves, tolerance/withdrawal dynamics, and multi-transmitter synergy, bridging computational neuropharmacology and AI evaluation. Third, normative anchoring against 130+ published baselines enables clinical-band interpretation, with cross-seed stability (SD~$= 0.019$ across 3 seeds) confirming that z-score comparisons are robust to random initialization. Fourth, the unified holographic stimulus encoding through five HDC adapter types enables principled cross-modal comparison.
...
\emph{Domain-specific strengths and weaknesses.} The cognitive profile reveals a broadly above-human profile (grand mean $z = +1.027$, 24/26 domains above human mean, 2 below), with cross-seed stability SD~$= 0.019$ confirming robustness. The top-performing domains---InstitutionalReasoning ($z = +2.25$), SustainedAttention ($z = +2.19$), Speech ($z = +2.13$), Motor ($z = +2.12$), Clinical ($z = +2.00$), and Security ($z = +2.00$)---reflect that these benchmarks rely on pattern-matching and signal-processing tasks that map naturally to HDC operations. Two domains score below the human mean. Mathematics ($z = -1.18$) reflects the honest use of HDC-only implementations: the 10 benchmarks (arithmetic word problems through definite integrals) require symbolic manipulation that pure holographic pattern matching cannot reliably perform. A dedicated \texttt{MathService} engine (3,374~LOC, 128~tests) has been implemented but is not yet wired into the benchmark pipeline. Creativity ($z = -0.38$) reflects HDC limitations in generating genuinely novel semantic combinations. Among positive domains, Substrate ($z = +0.34$), Executive ($z = +0.41$), and Binding ($z = +0.44$) are closest to the human mean. These asymmetries are informative---they demonstrate that the profile genuinely captures architectural biases---but they warrant caution when comparing across domains with different measurement characteristics.

\emph{Normative z-score interpretation.} Four benchmarks produce normative z-scores exceeding $|z| > 3$, primarily reflecting genuinely large deviations from human norms rather than scale mismatches (e.g., agent CPT $d' = 4.65$ vs.\ human $d' = 2.80$). The forest plot (Figure~\ref{fig:forest}) clips to $[-3, +3]$; unclipped values are available in the supplementary CSV data. Z-scores exceeding $\pm 3$ should be treated as ordinal indicators of atypical performance rather than precise quantitative distances, as the Gaussian assumption underlying z-score interpretation weakens at distribution tails.
...
\psychbench{} provides the first comprehensive cognitive benchmark suite designed for Holographic Liquid Brain architectures. With 136 benchmarks across 26 domains, psychometric assessment infrastructure, normative human comparison against 130+ published baselines, a 17-benchmark neuromodulator evaluation framework, and 14-indicator consciousness evaluation, the suite enables multi-dimensional characterization of neuroscience-inspired cognitive architectures. The demonstration on the \symthaea{} \hlb{} reveals both the suite's strengths and its current limitations: the architecture produces a characteristic cognitive profile with a grand mean z-score of $+1.027$ (24/26 domains above human mean, 2 below, SD~$= 0.019$ across 3 seeds) and particular strengths in InstitutionalReasoning ($z = +2.25$), SustainedAttention ($z = +2.19$), Speech ($z = +2.13$), Motor ($z = +2.12$), and Clinical ($z = +2.00$) that is interpretable and reproducible. Notably, the Mathematics domain ($z = -1.18$) and Creativity domain ($z = -0.38$) score below the human mean---the former reflecting honest HDC-only implementations without symbolic math engines (a Phase~1 \texttt{MathService} with 3,374~LOC and 128~tests is implemented but not yet wired), the latter reflecting HDC limitations in novel semantic combination. The psychometric validation reveals improved reliability after adopting a lapse-rate individual-differences model~\citep{wichmann2001psychometric} (6 Excellent, 2 Good, 7 Moderate, 1 Poor across 16 evaluated benchmarks), with the Poor benchmark (Remote Associates, ICC $= 0.35$) reflecting fundamental HDC limitations in semantic distance encoding---a scientifically honest limitation that itself validates the evaluation framework's discriminative power. These findings are themselves informative---they identify precisely where the architecture and the evaluation framework need improvement. Implemented in ${\sim}20{,}000$ lines of Rust with 1,168 unit tests and full determinism, \psychbench{} is open-source and designed for iterative refinement by the growing community of researchers building brain-inspired cognitive architectures.
...
\psychbench{} is open-source under AGPL-3.0. The complete source code, including all 136 benchmark implementations, harness infrastructure, and data generation scripts, is available at:
