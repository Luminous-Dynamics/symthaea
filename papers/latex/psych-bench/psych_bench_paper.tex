...
\paragraph{Cognitive Profile.} Figure~\ref{fig:radar} shows the 8-domain cognitive profile generated from the full battery. The \hlb{} achieves a grand mean z-score of $+1.115$ across 26 cognitive domains, with 26/26 domains above the human mean and 0 below. Cross-seed stability analysis (3 seeds: 42, 123, 789) confirms robustness: grand mean $= 1.090 \pm 0.014$ (SD). The top five domains are InstitutionalReasoning ($z = +2.25$), SustainedAttention ($z = +2.19$), Speech ($z = +2.13$), Motor ($z = +2.12$), and Clinical ($z = +2.00$). Social cognition ($z = +1.54$) remains strong, driven by similarity-based Theory-of-Mind tests and game-theoretic benchmarks where holographic pattern matching maps naturally to the task structure. The lowest-scoring domains---CogBench ($z = +0.47$), Substrate ($z = +0.34$), Metacognition ($z = +1.04$), Binding ($z = +0.44$), and Executive ($z = +0.37$)---all remain positive, within the human-average or above-average bands.
...
\emph{Domain-specific strengths and weaknesses.} The cognitive profile reveals a broadly above-human profile (grand mean $z = +1.115$, 26/26 domains above human mean), with cross-seed stability SD~$= 0.019$ confirming robustness. The top-performing domains---InstitutionalReasoning ($z = +2.25$), SustainedAttention ($z = +2.19$), Speech ($z = +2.13$), Motor ($z = +2.12$), Clinical ($z = +2.00$), and Security ($z = +2.00$)---reflect that these benchmarks rely on pattern-matching and signal-processing tasks that map naturally to HDC operations. The lowest-scoring domains---Metacognition ($z = +0.36$), Substrate ($z = +0.43$), Executive ($z = +0.41$), Binding ($z = +0.45$), and Butlin ($z = +0.50$)---remain positive but closer to the human mean. These asymmetries are informative---they demonstrate that the profile genuinely captures architectural biases---but they warrant caution when comparing across domains with different measurement characteristics.
