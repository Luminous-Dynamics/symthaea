...
\caption{EFE ablation: sweeping safety precision reveals a sharp decision boundary at $\pi_{\text{safety}} \approx 2.40$. Below the crossover, mission EFE dominates (red); above, the agent chooses interception (blue). Marker color indicates the winning rendezvous fraction: orange = 75\% of impact time (favored at low $\pi$), blue = 50\% (favored at high $\pi$). Note that $G(\text{intercept})$ plateaus at high precision because successful interception eliminates danger ($\hat{d} \approx 0$), making $\pi_\text{safety} \cdot \hat{d}^2 \approx 0$ regardless of $\pi$.}
...
NoHuman      & 0.00 & 131.8   & 155.3  & MISSION \\
LowDanger    & 0.05 & 181.8   & 260.7  & MISSION \\
\bottomrule
\end{tabular}
\end{table}
