...
The manager receives \texttt{SentinelEvent} instances from the governance and network layers:
...
Each threat detector produces a \texttt{ThreatSignal} with a kind and base severity:
...
Defensive actions are graduated across three safety levels:
...
The physical embodiment system maps safety levels to robot behavior, further gated by consciousness:
...
The security system passes 120+ automated tests:
...
Under simulated attack scenarios:
...
The coherence-adjusted severity formula produces the expected amplification:
...
The fundamental distinction is that our system treats security as a \emph{cognitive function} (part of the consciousness loop, mediated by neuromodulators) rather than as an external service. This enables security responses that are proportionate, context-sensitive, and subject to the same ethical constraints as all other system behaviors.
