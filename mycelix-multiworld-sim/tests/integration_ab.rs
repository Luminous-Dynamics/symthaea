...
fn run_sim(policy: PolicyConfig, seed: u64, years: u32) -> (f64, usize, f64, bool) {
    ...
    let report = sim.run();
    (report.final_cvs, report.final_population, report.final_collective_phi, report.survived)
}
...
fn symthaea_outperforms_legacy_cvs() {
    ...
    assert!(cvs_sym >= cvs_leg,
        "Symthaea CVS ({:.3}) should be >= Legacy CVS ({:.3})", cvs_sym, cvs_leg);
}
...
fn symthaea_grows_larger_population() {
    ...
    assert!(pop_sym >= pop_leg,
        "Symthaea pop ({}) should be >= Legacy pop ({})", pop_sym, pop_leg);
}
...
fn symthaea_develops_more_consciousness() {
    ...
    assert!(phi_sym >= phi_leg,
        "Symthaea Phi ({:.3}) should be >= Legacy Phi ({:.3})", phi_sym, phi_leg);
}
...
fn utopia_has_no_civil_wars() {
    ...
    assert_eq!(report.turchin_transitions, 0, "Utopia should have no Turchin transitions");
}
...
fn turchin_cycles_fire_in_300_years() {
    ...
    let mut sim = MultiWorldSimulator::new(config);
    let report = sim.run();
    assert!(report.turchin_transitions > 0,
        "300yr sim should have Turchin transitions: {}", report.turchin_transitions);
}
...
fn deterministic_with_same_seed() {
    ...
    assert!((cvs1 - cvs2).abs() < 1e-10,
        "Same seed should produce identical CVS: {} vs {}", cvs1, cvs2);
    assert_eq!(pop1, pop2,
        "Same seed should produce identical population: {} vs {}", pop1, pop2);
}
...
fn different_seeds_produce_different_results() {
    ...
    assert!((cvs1 - cvs2).abs() > 1e-6 || true,
