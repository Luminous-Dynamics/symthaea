...
fn main() {
    ...
    for &(seed, _cg_wins, delta_str) in &diagnostic_seeds {
        ...
        for trust_weighted in [true, false] {
            ...
            let mean_trauma: f64 = sim.worlds.iter()
                .flat_map(|w| w.agents.iter().filter(|a| a.is_alive()))
                .map(|a| a.trauma_level)
                .sum::<f64>() / total_pop.max(1) as f64;
            let mean_load: f64 = sim.worlds.iter()
                .flat_map(|w| w.agents.iter().filter(|a| a.is_alive()))
                .map(|a| a.needs.allostatic_load)
                .sum::<f64>() / total_pop.max(1) as f64;
            let mean_joy: f64 = sim.worlds.iter()
                .flat_map(|w| w.agents.iter().filter(|a| a.is_alive()))
                .map(|a| a.needs.affect.joy)
                .sum::<f64>() / total_pop.max(1) as f64;
            let mean_engagement: f64 = sim.worlds.iter()
                .flat_map(|w| w.agents.iter().filter(|a| a.is_alive()))
            ...
            println!("  [{label}] CVS={:.4} Pop={} Phi={:.3} Trauma={:.3} Load={:.3}",
                report.final_cvs, total_pop, mean_phi, mean_trauma, mean_load);
            println!("       Joy={:.3} Engage={:.3} Factions={} Cascades={} ViabilityViol={}",
                mean_joy, mean_engagement, faction_count, cascade_count, viability_violations);
            println!("       Disasters={} TechMilestones={}", report.total_disasters, report.tech_milestones_achieved);
        }
        println!();
    }
}
