...
fn main() {
    ...
    let delta_pct = if ew_mean > 0.0 { delta_mean / ew_mean * 100.0 } else { 0.0 };

    // Paired differences for t-test
    let diffs: Vec<f64> = cg_cvs.iter().zip(ew_cvs.iter()).map(|(a, b)| a - b).collect();
    let diff_mean = mean(&diffs);
    let diff_std = std_dev(&diffs);
    let t_stat = if diff_std > 0.0 { diff_mean / (diff_std / (n_seeds as f64).sqrt()) } else { 0.0 };
    ...
    println!("{}", "-".repeat(50));
    println!("{:<28} {:>10.4} {:>10.4}", "Consciousness-Gated", cg_mean, cg_std);
    ...
    println!("\n=== PER-SEED DETAIL ===\n");
    println!("{:>6} {:>10} {:>10} {:>10}", "Seed", "CG CVS", "EW CVS", "Delta");
    for i in 0..n_seeds as usize {
        println!("{:>6} {:>10.4} {:>10.4} {:>+10.4}",
            i * 137 + 42, cg_cvs[i], ew_cvs[i], cg_cvs[i] - ew_cvs[i]);
    }

    println!("\n=== VERDICT ===");
    if p_approx < 0.05 && delta_mean > 0.0 {
        println!("Consciousness gating SIGNIFICANTLY improves CVS (p < 0.05, Δ = {:+.1}%)", delta_pct);
        ...
        println!("Consciousness gating does NOT improve CVS (Δ = {:+.1}%)", delta_pct);
    }
    println!("Published claim: +6.9%. Observed: {:+.1}% ± {:.1}%", delta_pct, diff_std * 100.0 / ew_mean);

    println!("\nHONEST CAVEAT: This is a simulated comparison, not a real-world experiment.");
    println!("The consciousness-gating model was designed by the same team that built the simulator.");
}
