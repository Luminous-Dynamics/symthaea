...
//! production-shaped candidate set. An opt-in live entry point additionally probes V3 against the
//! processor's actual `ActivePrimitive` records. Neither shadow path changes the returned
//! historical result or learning/plasticity.

use super::reasoning_active_primitive_evidence::plan_active_primitive_evidence;
use super::reasoning_context_competition::{
    ContextCompetitionPolicy, ContextHypothesis, ContextResolution,
};
use super::reasoning_evidence_seeking::EvidenceSeekingOutcome;
...
pub struct ShadowMetaStats {
    ...
    pub v3_attempts: u64,
    pub v3_selected: u64,
    pub v3_need_evidence: u64,
    pub v3_abstained: u64,
    pub v3_errors: u64,
    pub v3_identity_mismatches: u64,
}
...
#[derive(Debug, Clone, PartialEq)]
pub struct V3ShadowObservation {
    pub shadow_version: String,
    pub attempt: u64,
    pub outcome: V3ShadowOutcomeKind,
    pub active_profiles: usize,
    pub identity_mismatches: usize,
    pub request_count: usize,
    pub top_request_id: Option<String>,
    pub selected_candidate: Option<String>,
    pub activation_min: Option<f64>,
    pub activation_max: Option<f64>,
...
pub struct ShadowQualifiedMetaReasoner {
    ...
    last_v3_observation: Option<V3ShadowObservation>,
}
...
impl ShadowQualifiedMetaReasoner {
    ...
    /// Live measurement-only entry point used when the cognitive loop can also provide its exact
    /// current `ActivePrimitive` state. Historical reasoning still executes first and remains the
    /// returned result. V2 and V3 observations are strictly side-channel telemetry.
    ...
    pub fn last_v3_shadow_observation(&self) -> Option<&V3ShadowObservation> {
        self.last_v3_observation.as_ref()
    }
    ...
    fn observe_v3_shadow(
        &mut self,
        legacy_result: &MetaReasoningResult,
        active_primitives: &[ActivePrimitive],
        candidate_ids: &[String],
    ) {
        let attempt = self.stats.v3_attempts;
        self.stats.v3_attempts = self.stats.v3_attempts.saturating_add(1);
        let hypotheses = context_hypotheses(legacy_result);

        match plan_active_primitive_evidence(
            &hypotheses,
            ...
            Ok((evidence_report, plan)) => {
                let mismatch_increment =
                    u64::try_from(evidence_report.immutable_identity_mismatches).unwrap_or(u64::MAX);
                self.stats.v3_identity_mismatches = self
                    .stats
                    .v3_identity_mismatches
                    .saturating_add(mismatch_increment);

                let activation_min = evidence_report
                    .profiles
                    .iter()
                    .map(|profile| profile.activation)
                    .reduce(f64::min);
                let activation_max = evidence_report
                    .profiles
                    .iter()
                    .map(|profile| profile.activation)
                    .reduce(f64::max);

                let (outcome, request_count, top_request_id, selected_candidate) =
                    match &plan.outcome {
                        EvidenceSeekingOutcome::Selected { candidate_id, .. } => {
                            self.stats.v3_selected = self.stats.v3_selected.saturating_add(1);
                            (
                                V3ShadowOutcomeKind::Selected,
                                0,
                                None,
                                Some(candidate_id.clone()),
                            )
                        }
                        EvidenceSeekingOutcome::NeedEvidence { requests, .. } => {
                            self.stats.v3_need_evidence =
                                self.stats.v3_need_evidence.saturating_add(1);
                            (
                                V3ShadowOutcomeKind::NeedEvidence,
                                requests.len(),
                                requests.first().map(|request| request.request_id.clone()),
                                None,
                            )
                        }
                        EvidenceSeekingOutcome::Abstained { .. } => {
                            self.stats.v3_abstained = self.stats.v3_abstained.saturating_add(1);
                            (V3ShadowOutcomeKind::Abstained, 0, None, None)
                        }
                    };

                tracing::debug!(
                    target: "symthaea::reasoning_shadow_v3",
                    attempt,
                    ?outcome,
                    active_profiles = evidence_report.profiles.len(),
                    identity_mismatches = evidence_report.immutable_identity_mismatches,
                    request_count,
                    top_request = top_request_id.as_deref().unwrap_or(""),
                    selected_candidate = selected_candidate.as_deref().unwrap_or(""),
                    activation_min = activation_min.unwrap_or(0.0),
                    activation_max = activation_max.unwrap_or(0.0),
                    "canonical V3 live ActivePrimitive shadow observed"
                );

                self.last_v3_observation = Some(V3ShadowObservation {
            ...
            Err(err) => {
                self.stats.v3_errors = self.stats.v3_errors.saturating_add(1);
                tracing::debug!(
                    target: "symthaea::reasoning_shadow_v3",
                    attempt,
                    error = %err,
                    "canonical V3 live ActivePrimitive shadow rejected measurement"
                );
                self.last_v3_observation = Some(V3ShadowObservation {
                    shadow_version: LIVE_META_SHADOW_VERSION.into(),
                    attempt,
                    outcome: V3ShadowOutcomeKind::Error,
                    active_profiles: 0,
...
mod tests {
    ...
    #[test]
    fn active_evidence_v3_shadow_preserves_legacy_result_and_requests_measurements() {
        let mut reasoner = reasoner();
        let mut chain = ReasoningChain::new(BinaryHV::random(1506));
        ...
        assert_eq!(reasoner.shadow_stats().v3_need_evidence, 1);
        assert_eq!(reasoner.shadow_stats().v3_errors, 0);
        let observation = reasoner.last_v3_shadow_observation().unwrap();
        assert_eq!(observation.outcome, V3ShadowOutcomeKind::NeedEvidence);
        assert_eq!(observation.active_profiles, 2);
        assert_eq!(observation.request_count, 6);
        assert_eq!(observation.activation_min, Some(0.6));
        assert_eq!(observation.activation_max, Some(0.8));
    }

    #[test]
    fn v3_shadow_failure_does_not_reject_legacy_result() {
        let mut reasoner = reasoner();
        let mut chain = ReasoningChain::new(BinaryHV::random(1507));
        let result = reasoner.meta_reason_with_active_evidence(
            "evidence experiment research theory scientific",
            vec![candidate("NSM_KNOW"), candidate("NSM_DO")],
            &[active("NSM_KNOW", 0.8)],
        ...
        assert!(result.is_ok());
        assert_eq!(reasoner.shadow_stats().v3_attempts, 1);
        assert_eq!(reasoner.shadow_stats().v3_errors, 1);
        assert_eq!(
            reasoner.last_v3_shadow_observation().unwrap().outcome,
            V3ShadowOutcomeKind::Error
        );
    }
}
