...
impl MycelixBridge {
    ...
    // ========================================================================
    // HOLOCHAIN BRIDGE EVENT ADAPTER
    // ========================================================================

    /// Convert a Holochain `GovernanceBridgeEvent` into Symthaea's `GovernanceEvent`.
    ///
    /// This is the type-mapping adapter that closes the Holochain→Symthaea gap.
    /// The Holochain governance zome emits `GovernanceBridgeEvent` with a JSON payload;
    /// this method parses the payload and maps the event type to Symthaea's enum.
    ///
    /// # Event Type Mapping
    ///
    /// | Holochain | Symthaea | Payload Fields |
    /// |-----------|----------|----------------|
    /// | `ProposalCreated` | `ProposalCreated` | — |
    /// | `VoteReceived` | `VoteCast` | `voter_phi`, `vote_value` |
    /// | `ProposalPassed` | `TallyCompleted(passed=true)` | `collective_phi` |
    /// | `ProposalFailed` | `TallyCompleted(passed=false)` | `collective_phi` |
    /// | `VotingStarted` | *skipped* | — |
    /// | `VotingEnded` | *skipped* | — |
    /// | `ProposalExecuted` | *skipped* | — |
    /// | `ConstitutionAmended` | *skipped* | — |
    ///
    /// Events without a Symthaea equivalent (VotingStarted, etc.) return `None`.
    /// Payload parsing failures fall back to safe defaults.
    #[cfg(feature = "mycelix")]
    pub fn convert_bridge_event(
        &mut self,
        event_type: &str,
        proposal_id: Option<String>,
        payload: &str,
        timestamp_secs: u64,
    ) -> Option<crate::cognitive_loop::managers::governance_manager::GovernanceEvent> {
        use crate::cognitive_loop::managers::governance_manager::{
            GovernanceEvent, GovernanceEventKind,
        };
        ...
        let kind = match event_type {
            ...
            "VoteReceived" => {
                // Parse voter_phi and vote_value from JSON payload
                let voter_phi = Self::extract_f64(payload, "voter_phi").unwrap_or(0.5);
                let vote_value = Self::extract_f64(payload, "vote_value").unwrap_or(0.0);
                GovernanceEventKind::VoteCast {
                    voter_phi: voter_phi.clamp(0.0, 1.0),
                    vote_value: vote_value.clamp(-1.0, 1.0),
                }
            }
            "ProposalPassed" => {
                let collective_phi =
                    Self::extract_f64(payload, "collective_phi").unwrap_or(0.5);
                GovernanceEventKind::TallyCompleted {
                    passed: true,
                    collective_phi: collective_phi.clamp(0.0, 1.0),
                }
            }
            "ProposalFailed" => {
                let collective_phi =
            ...
            // Events without Symthaea equivalents — skip
            "VotingStarted" | "VotingEnded" | "ProposalExecuted" | "ConstitutionAmended" => {
                return None;
            }
            _ => {
                tracing::warn!(event_type, "Unknown GovernanceBridgeEvent type, skipping");
                return None;
            }
        };
    ...
    /// Convert a Holochain governance signal into a `GovernanceOutcome`.
    ///
    /// Called when a tally result is available. Maps Holochain tally fields
    /// to Symthaea's outcome structure for learning feedback.
    #[cfg(feature = "mycelix")]
    pub fn convert_tally_outcome(
        &mut self,
        proposal_id: String,
        passed: bool,
        my_vote_aligned: Option<bool>,
        payload: &str,
    ) -> crate::cognitive_loop::managers::governance_manager::GovernanceOutcome {
        use crate::cognitive_loop::managers::governance_manager::GovernanceOutcome;

        let value_alignment =
            Self::extract_f64(payload, "value_alignment_score").unwrap_or(0.5);
        let harmonic_resonance =
            Self::extract_f64(payload, "harmonic_resonance").unwrap_or(0.0);

        let outcome = GovernanceOutcome {
            proposal_id,
            passed,
            my_vote_aligned,
            value_alignment_score: if value_alignment.is_finite() {
                value_alignment.clamp(0.0, 1.0)
            } else {
                0.5
            },
            harmonic_resonance: if harmonic_resonance.is_finite() {
                harmonic_resonance.clamp(-1.0, 1.0)
            } else {
                0.0
            },
        };
    ...
    /// Simple JSON field extraction — avoids pulling in a full JSON parser.
    ///
    /// Looks for `"key": <number>` patterns. Returns `None` if not found or not a number.
    fn extract_f64(json: &str, key: &str) -> Option<f64> {
        ...
        let num_str = value_str[..end].trim().trim_matches('"');
