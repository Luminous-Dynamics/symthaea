...
impl UnifiedConsciousnessPipeline {
    ...
    /// Run a minimal, deterministic consciousness loop for a fixed number of steps.
    ///
    /// This is a convenience helper that:
    /// - constructs a default pipeline,
    /// - feeds it a simple synthetic 64-dimensional input pattern at each step, and
    /// - returns the resulting stream of `ConsciousMoment` records.
    ///
    /// It is intended as the smallest useful "end-to-end" demonstration of the
    /// consciousness pipeline without requiring any external dependencies.
    pub fn run_minimal_demo(steps: usize) -> Result<Vec<ConsciousMoment>> {
        let mut pipeline = Self::default_pipeline()?;
        let mut moments = Vec::with_capacity(steps);

        for i in 0..steps {
            // Deterministic synthetic input, reused from tests
            let input: Vec<f64> = (0..64)
                .map(|j| ((i + j) as f64 / 100.0).sin().abs())
                .collect();

            let moment = pipeline.process(&input)?;
            moments.push(moment);
        }
...
mod tests {
    ...
    fn test_run_minimal_demo() {
        ...
        assert_eq!(moments.len(), steps);

        for (i, m) in moments.iter().enumerate() {
            assert_eq!(m.step, i, "step index should match iteration");
            assert!(
                m.consciousness >= 0.0 && m.consciousness <= 1.0,
