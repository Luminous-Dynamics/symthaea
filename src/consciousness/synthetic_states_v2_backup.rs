...
impl SyntheticStateGenerator {
    ...
    /// ALERT FOCUSED (Φ: 0.65-0.85): Maximum Integration via Shared Patterns
    ///
    /// **CORRECTED (Dec 26)**: Direct similarity encoding with 80% shared pattern ratio
    /// All components share 4 global patterns (4/5 = 80% shared)
    /// **Expected Φ**: 0.65-0.85 (maximum integration)
    ///
    /// **Why This Works**:
    /// - bundle([s1, s2, s3, s4, unique]) = 4/5 shared = 80%
    /// - All pairs share 4/5 patterns → similarity ≈ 0.80
    /// - High pairwise similarity → High system_info → High Φ ✅
    fn generate_high_integration(&mut self) -> Vec<HV16> {
        // 4 global shared patterns + 1 unique per component = 80% shared
        ...
        (0..self.num_components)
            .map(|_| {
                ...
                // 4 shared + 1 unique = 80% shared → HIGH similarity → HIGH Φ
                HV16::bundle(&[
                    global1.clone(),
                    global2.clone(),
                    global3.clone(),
    ...
    ///
    /// **CORRECTED (Dec 26)**: 75% shared pattern ratio
    /// All components share 3 global patterns (3/4 = 75% shared)
    /// **Expected Φ**: 0.55-0.65 (high integration)
    fn generate_moderate_high_integration(&mut self) -> Vec<HV16> {
        // 3 global shared patterns + 1 unique per component = 75% shared
        let global1 = HV16::random(self.next_seed());
        let global2 = HV16::random(self.next_seed());
        let global3 = HV16::random(self.next_seed());
    ...
    ///
    /// **CORRECTED (Dec 26)**: 67% shared pattern ratio (2/3)
    /// All components share 2 global patterns
    /// **Expected Φ**: 0.45-0.55 (moderate integration)
    fn generate_moderate_integration(&mut self) -> Vec<HV16> {
        // 2 global shared patterns + 1 unique per component = 67% shared
        let global1 = HV16::random(self.next_seed());
        let global2 = HV16::random(self.next_seed());
    ...
    ///
    /// **CORRECTED (Dec 26)**: 50% shared pattern ratio (1/2)
    /// All components share 1 global pattern
    /// **Expected Φ**: 0.35-0.45 (moderate-low integration)
    fn generate_moderate_low_integration(&mut self) -> Vec<HV16> {
        // 1 global shared pattern + 1 unique per component = 50% shared
    ...
    /// LIGHT SLEEP (Φ: 0.25-0.35): Modular Network
    ///
    /// **CORRECTED (Dec 26)**: 40% shared pattern ratio (2/5)
    /// All components share 2 global patterns
    /// **Expected Φ**: 0.25-0.35 (low integration)
    fn generate_low_integration(&mut self) -> Vec<HV16> {
        // 2 global shared patterns + 3 unique per component = 40% shared
        let global1 = HV16::random(self.next_seed());
        let global2 = HV16::random(self.next_seed());
        ...
        (0..self.num_components)
            .map(|_| {
                ...
                // 2 shared + 3 unique = 40% shared
                HV16::bundle(&[global1.clone(), global2.clone(), unique1, unique2, unique3])
            })
            .collect()
    }
    ...
    ///
    /// **CORRECTED (Dec 26)**: 33% shared pattern ratio (1/3)
    /// All components share 1 global pattern
    /// **Expected Φ**: 0.15-0.25 (very low integration)
    fn generate_isolated_state(&mut self) -> Vec<HV16> {
        // 1 global shared pattern + 2 unique per component = 33% shared
    ...
    ///
    /// **CORRECTED (Dec 26)**: 25% shared pattern ratio (1/4)
    /// All components share 1 global pattern
    /// **Expected Φ**: 0.05-0.15 (minimal integration)
    fn generate_fragmented_state(&mut self) -> Vec<HV16> {
        // 1 global shared pattern + 3 unique per component = 25% shared
