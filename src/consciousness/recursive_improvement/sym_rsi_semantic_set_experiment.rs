...
//!
//! This experiment is synthetic and deterministic. Candidate sets are search
//! objects only: membership grants no evidence, validation, or confidence authority.

use std::collections::{HashMap, HashSet};

use super::semantic_context::{ContextIdentity, SemanticContextEncoder, SemanticContextError};
use super::semantic_support::{
    SemanticSupportError, assess_semantic_context_support,
};
...
pub fn run_sym_rsi_sem_003(subject_sha: &str) -> Result<Sem3Receipt, Sem3ExperimentError> {
    ...
    let calibration_queries = calibration_queries(&candidates)?;
    let clean_queries = clean_queries(&candidates)?;
    let ambiguous_queries = ambiguous_queries(&candidates)?;
    let unrelated_queries = unrelated_queries();
    let ood_queries = ood_queries(&candidates)?;
    ...
    let thresholds = calibrate_thresholds(encoder, &candidates, &calibration_queries)?;
    ...
    let mut all_ood_rejected = Vec::new();
    ...
    for dimension in SYM_RSI_SEM_003_DIMENSIONS {
        ...
        all_ood_rejected.extend(ood.iter().map(|observation| observation.0));
    }

    let clean_target_coverage = rate(
        all_clean.iter().filter(|observation| observation.target_included).count(),
        all_clean.len(),
    );
    ...
    let ood_support_rejection_rate = rate(
        all_ood_rejected.iter().filter(|&&rejected| rejected).count(),
        all_ood_rejected.len(),
    );
    ...
    let all_non_ood_guards = clean_coverage_passed
    ...
    let mut receipt = Sem3Receipt {
        ...
        calibration_query_digest: labelled_query_digest("calibration", &calibration_queries),
        clean_query_digest: labelled_query_digest("clean", &clean_queries),
        ambiguous_query_digest: ambiguous_query_digest(&ambiguous_queries),
        unrelated_query_digest: unlabelled_query_digest("unrelated", &unrelated_queries),
        ood_query_digest: unlabelled_query_digest("ood", &ood_queries),
        thresholds,
        dimension_metrics,
...
fn calibration_queries(candidates: &[Candidate]) -> Result<Vec<LabelledQuery>, Sem3ExperimentError> {
    ...
    for dimension in SYM_RSI_SEM_003_DIMENSIONS {
        let bank = candidates
            ...
            .filter(|candidate| candidate.dimension == dimension)
            .collect::<Vec<_>>();
        for target in bank.iter().take(SYM_RSI_SEM_003_CALIBRATION_QUERIES_PER_DIMENSION) {
            let target_index = candidate_index(&bank, target.identity)?;
            queries.push(LabelledQuery {
                dimension,
                context: perturb_context(&target.context, target_index, 0.025, 3),
...
fn clean_queries(candidates: &[Candidate]) -> Result<Vec<LabelledQuery>, Sem3ExperimentError> {
    ...
    for dimension in SYM_RSI_SEM_003_DIMENSIONS {
        ...
        for target in bank
            .iter()
            .skip(SYM_RSI_SEM_003_CALIBRATION_QUERIES_PER_DIMENSION)
            .take(SYM_RSI_SEM_003_CLEAN_QUERIES_PER_DIMENSION)
        {
            let target_index = candidate_index(&bank, target.identity)?;
            queries.push(LabelledQuery {
                dimension,
                context: perturb_context(&target.context, target_index, 0.045, 11),
                target: target.identity,
            });
        }
    }
...
fn ambiguous_queries(candidates: &[Candidate]) -> Result<Vec<AmbiguousQuery>, Sem3ExperimentError> {
    ...
    for dimension in SYM_RSI_SEM_003_DIMENSIONS {
        ...
        for family in 0..SYM_RSI_SEM_003_AMBIGUOUS_QUERIES_PER_DIMENSION {
            let left = bank.get(family * 2).ok_or(Sem3ExperimentError::MissingCandidate)?;
            let right = bank
                .get(family * 2 + 1)
...
/// Returns `(support_rejected, max_raw_similarity)` for OOD diagnostics.
...
fn build_dimension_metrics(
    ...
    Sem3DimensionMetrics {
        ...
        unrelated_nonempty_set_rate: rate(
            unrelated.iter().filter(|observation| observation.set_size > 0).count(),
            unrelated.len(),
        ),
...
fn threshold_for(dimension: usize, thresholds: &[Sem3Threshold]) -> Result<f32, Sem3ExperimentError> {
    thresholds
        .iter()
...
fn candidate_index(bank: &[&Candidate], identity: ContextIdentity) -> Result<usize, Sem3ExperimentError> {
    bank.iter()
...
mod tests {
    ...
    fn sem3_tampered_metric_breaks_receipt_integrity() {
        ...
        receipt.clean_target_coverage = f64::from_bits(receipt.clean_target_coverage.to_bits() ^ 1);
        assert!(!receipt.validate_self());
    }

    #[test]
    fn sem3_thresholds_use_only_calibration_queries() {
        let encoder = SemanticContextEncoder::default();
