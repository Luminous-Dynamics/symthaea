        ...
        while i < words.len() {
            ...
            if self.is_negation_word(&clean_word) {
                ...
                while i < words.len() && scope_length < MAX_NEGATION_SCOPE {
                    ...
                    // Check for scope breakers, but NOT for the first word after negation
                    // "not want" means "want" is negated, not a new positive intent
                    if scope_length > 0 && self.is_scope_breaker(&clean_next) {
                        break;
                    }
