...
use symthaea_neuroevolution::{
    FepFitnessConfig, FitnessWeights, GenerationSnapshot, NeuralGenome, NeuroevolutionConfig,
...
impl NeuroevolutionManager {
    ...
    pub fn process(&mut self, snapshot: &CycleSnapshot) -> SubsystemOutput {
        ...
        // Return a subsystem output with learning rate suggestion from best genome
        let mut output = SubsystemOutput::default();
        if let Some((genome, _)) = self.engine.best_ever() {
            let phenotype = genome.decode();
            // Suggest learning rate from the best evolved genome
            output.learning_rate_delta = (phenotype.neuron_config.learning_rate - 0.01) * 0.1;
        }
