...
impl DomainPlugin for ResearchPlugin {
    ...
    fn extract_entities(&self, text: &str) -> Vec<Entity> {
        ...
        let methods = [
            "regression", "classification", "clustering", "meta-analysis",
            "rct", "randomized", "controlled trial", "cohort", "cross-sectional",
            "longitudinal", "qualitative", "quantitative", "mixed methods",
            "neural network", "deep learning", "machine learning",
            "statistical test", "p-value", "confidence interval",
            "bayesian", "frequentist", "anova", "t-test", "chi-square",
        ];
        ...
        let datasets = [
            "imagenet", "mnist", "cifar", "coco", "glue", "superglue",
            "squad", "wikitext", "openwebtext", "the pile",
            "common crawl", "laion", "c4",
        ];
        ...
        let claim_words = ["claim", "argue", "propose", "hypothesize", "suggest",
                          "demonstrate", "show that", "prove", "establish"];
        for word in &claim_words {
            if let Some(pos) = lower.find(word) {
                entities.push(
                    Entity::new("claim_marker", *word, pos, pos + word.len())
    ...
    fn intent_prototypes(&self) -> IntentPrototypes {
        ...
        protos.custom.insert("literature_review".to_string(), vec![
            "literature review".to_string(), "related work".to_string(),
            "prior work".to_string(), "previous research".to_string(),
            "state of the art".to_string(), "survey".to_string(),
        ]);
        protos.custom.insert("methodology".to_string(), vec![
            "methodology".to_string(), "method".to_string(),
            "approach".to_string(), "technique".to_string(),
            "experimental design".to_string(), "protocol".to_string(),
        ]);
        protos.custom.insert("critique".to_string(), vec![
            "critique".to_string(), "weakness".to_string(),
            "limitation".to_string(), "flaw".to_string(),
            "review".to_string(), "evaluate".to_string(),
        ]);
        protos.custom.insert("synthesize".to_string(), vec![
            "synthesize".to_string(), "combine".to_string(),
            "integrate".to_string(), "connect".to_string(),
            "relationship".to_string(), "gap".to_string(),
        ]);
        protos
    }
    ...
    fn is_in_domain(&self, topic: &str) -> f64 {
        ...
        let research_keywords = [
            "paper", "study", "research", "journal", "publication", "citation",
            "hypothesis", "experiment", "methodology", "dataset", "results",
            "findings", "literature", "review", "abstract", "doi", "arxiv",
            "peer review", "conference", "proceedings", "thesis", "dissertation",
            "p-value", "statistical", "sample size", "control group",
            "reproducibility", "replication", "meta-analysis",
        ];
        let matches = research_keywords.iter().filter(|k| lower.contains(*k)).count();
        if matches >= 3 {
            0.9
        } else if matches >= 2 {
            0.7
    ...
    fn vocabulary(&self) -> Vec<String> {
        vec![
            "hypothesis", "methodology", "experiment", "dataset", "results",
            "findings", "conclusion", "abstract", "introduction", "discussion",
            "literature review", "citation", "reference", "p-value",
            "statistical significance", "effect size", "sample size",
            "control group", "treatment group", "bias", "confound",
    ...
    fn validate_input(&self, input: &str) -> ValidationResult {
        ...
        if input.len() < 10 {
            result.warnings.push("Very short research query; consider providing more context.".to_string());
        }
        result
    }
}
...
mod tests {
    ...
    fn test_extract_citation() {
        let plugin = ResearchPlugin;
        let entities = plugin.extract_entities("According to (Tononi et al., 2016), IIT predicts...");
    ...
    fn test_extract_dataset() {
        ...
        assert!(entities.iter().any(|e| e.entity_type == "dataset" && e.value == "mnist"));
    }

    #[test]
    fn test_extract_claim_marker() {
        let plugin = ResearchPlugin;
        let entities = plugin.extract_entities("The authors claim that consciousness requires integration");
