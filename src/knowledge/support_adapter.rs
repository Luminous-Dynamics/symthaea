...
const GLOBAL_KNOWLEDGE_PROVIDER_ID: &str = "symthaea/global-knowledge";
...
impl SupportKnowledgeSourceV1 for GlobalKnowledgeSupportAdapterV1<'_> {
    fn search_support_knowledge(
        ...
        query.validate()?;
        let provider_id = KnowledgeProviderIdV1::try_new(GLOBAL_KNOWLEDGE_PROVIDER_ID)?;
        ...
        for fact in self.manager.graph().all_facts() {
            ...
            hits.push(SupportKnowledgeHitV1 {
                ...
                source_id: KnowledgeRecordIdV1::try_new(format!("global-fact:{}", fact.id))?,
...
mod tests {
    ...
    fn lookup_is_read_through_and_does_not_mint_authority() {
        ...
        assert!(hits.iter().all(|hit| {
            hit.provider_id.as_str() == GLOBAL_KNOWLEDGE_PROVIDER_ID
