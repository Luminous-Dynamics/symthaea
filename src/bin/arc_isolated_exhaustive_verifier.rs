...
//! Independent exhaustive-reference verifier for process-isolated ARC policy runs.
...
fn run() -> Result<(), String> {
    ...
    let rows = require_array(object(&report, "policy report")?, "tasks", "policy report")?;
    let candidates = canonical_candidates();

    let mut rows_by_problem: BTreeMap<String, Vec<&Map<String, Value>>> = BTreeMap::new();
    for row in rows {
        let row = object(row, "policy task result")?;
        let problem_id = require_str(row, "problem_id", "policy task result")?.to_string();
        rows_by_problem.entry(problem_id).or_default().push(row);
    }

    let mut verified = Vec::with_capacity(rows_by_problem.len());
    for (problem_id, policy_rows) in rows_by_problem {
        if policy_rows.len() != 2 {
            return Err(format!("{problem_id} must have exactly two policy rows"));
        }
        let policies = policy_rows
            .iter()
            .map(|row| require_str(row, "policy_id", "policy task result"))
            .collect::<Result<Vec<_>, _>>()?;
        if policies != ["canonical-order-v1", "uniform-random-without-replacement-v1"] {
            return Err(format!("unexpected policy ordering for {problem_id}: {policies:?}"));
        }
        ...
        for row in policy_rows {
            validate_published_exhaustive(row, &problem_id, &full)?;
        }
...
fn exhaustive_reference(
    ...
    ExhaustiveResult {
        prediction_digest: if predictions.len() == 1 {
...
mod tests {
    ...
    fn ambiguous_predictions_abstain() {
        let train = vec![GridPair {
            input: vec![vec![0, 0, 0], vec![0, 0, 0]],
            output: vec![vec![0, 0, 0], vec![0, 0, 0]],
        }];
