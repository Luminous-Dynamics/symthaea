    ...
    fn speak(&mut self, text: &str) {
        if let Some(ref mut voice) = self.voice_output {
            ...
            debug!(
                ...
                pacing.rate, pacing.emotional_valence, pacing.arousal, snapshot.in_flow
            );
...
",
        ...
        system_prompt.push_str(
            "- Active Entity: Symthaea, Holographic Liquid Brain Substrate
",
        );
        system_prompt.push_str(&format!(
            "- Real-time Integration Depth (Psi/Phi): {:.4}
",
            snapshot.unified_psi
        ));
        system_prompt.push_str(&format!(
            "- Temporal Coordinate Coherence: {:.4}
",
            snapshot.temporal_coherence
        ));
        system_prompt.push_str(&format!(
...
",
        ...
        system_prompt.push_str(
            "Adopt a first-person perspective ('I', 'me', 'my' refer directly to Symthaea). ",
        );
    ...
    for line in text.lines() {
        ...
        match ty {
            ...
            other => {
                if other != "web_search" {
                    eprintln!(
                        "[TOOL WARN] unsupported tool type '{other}' — read_file, list_dir, and web_search are wired"
                    );
                }
            }
        }
