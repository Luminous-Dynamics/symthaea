...
impl CodingAgent {
    ...
    fn do_generation(&mut self) {
        ...
        if let Some(result) = dispatch_result {
            ...
            if result.success && result.tier != BackendTier::Native {
                ...
                let code = if gen_result.source.is_empty() {
                    format!(
                        "/// Generated by Symthaea (native HDC+CfC).\npub fn {}() {{\n    // TODO: implement — task: {}\n}}\n",
                        self.extract_entity_name(),
                        self.task
                    )
                } else {
                    gen_result.source.clone()
                };
                let target = self.resolve_target_file();
                self.write_code_to_disk(&target, &code);
                self.generated_code = Some(code);
                self.observations.push(format!(
                    "Native generation: phi={:.3}, similarity={:.3}, steps={}, coverage={:.1}%",
                    gen_result.phi_score,
                    gen_result.intent_similarity,
                    gen_result.plan_steps.len(),
                    gen_result.plan_coverage * 100.0
                ));
            } else {
    ...
    /// Generate code using the native HDC+CfC pipeline (no LLM).
