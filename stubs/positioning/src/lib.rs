...
pub mod ranging {
    pub fn rssi_to_range(_rssi_dbm: f64, _tx_power: f64, _n: f64) -> f64 { 0.0 }
    pub fn uwb_tof_to_range(_tof_ns: f64) -> f64 { 0.0 }
    pub fn wifi_rtt_to_range(_rtt_ns: f64) -> f64 { 0.0 }
}

pub mod dead_reckoning {
    pub fn barometric_altitude(_pressure_hpa: f64, _ref_pressure: f64) -> f64 { 0.0 }
}
...
impl PartialEq for GaussianEstimate3D {
    fn eq(&self, other: &Self) -> bool { self.mean == other.mean && self.covariance_diag == other.covariance_diag }
}
...
pub struct PeerFusion3D { peers: Vec<PeerEstimate3D>, max_peers: usize }
...
impl PeerFusion3D {
    ...
    pub fn fuse(&self) -> Option<GaussianEstimate3D> {
        ...
        let mut mean = [0.0; 3];
        let mut w_total = 0.0;
        for p in &self.peers { let w = p.trust_weight; for i in 0..3 { mean[i] += w * p.estimate.mean[i]; } w_total += w; }
        if w_total > 0.0 { for i in 0..3 { mean[i] /= w_total; } }
...
pub enum MeasurementModality {
    Rssi, LoraToA, UwbToF, WifiRtt, Gps, Barometric, Imu, Visual, Acoustic,
