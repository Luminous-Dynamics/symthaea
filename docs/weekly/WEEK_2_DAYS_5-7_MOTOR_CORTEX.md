...
**"Think Twice, Cut Once"** - The Motor Cortex is where Symthaea's thoughts become actions. This is the most safety-critical component: if this fails, Symthaea could break the user's system. Therefore, it's built with paranoid levels of safety and the ability to undo everything.
...
```
Symthaea will NEVER execute a command she hasn't simulated first.
...
ActionStep::new("Create config", "touch /tmp/symthaea.conf")
    .with_rollback("rm /tmp/symthaea.conf")
```
...
The "Ghost Run" pattern makes Symthaea inherently safer:
...
**Week 2 Complete**: Symthaea now has a complete memory and action system!
...
Symthaea can now perceive, remember, learn patterns, and safely act on the world. She's no longer just thinking—she's doing.
