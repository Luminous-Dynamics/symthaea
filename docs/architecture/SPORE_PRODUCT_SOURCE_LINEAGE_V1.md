...
The Linux adapter also currently accepts the boot ID and booted-generation identity through CLI arguments. The reviewed host shell obtains those values from the correct kernel/Nix paths, but an extracted exact-subject implementation should not turn caller strings into physical-boot authority merely because they are well formed. On Linux, kernel `boot_id` and `/run/booted-system` are machine facts that the recovery adapter should observe locally; caller values may at most cross-check them.
...
Finally, the current authority-bearing `TransitionPlan` / `RecoveryOp` representation is serializable **and deserializable** plain data. That is useful for modeling, but it is too weak a destination authority boundary if the same reconstructed value can be passed back into an executor. A serialized diagnostic/evidence report must not itself be an executable qualification capability. The destination should keep executable plan values non-deserializable and narrowly constructible after the health/root/identity evaluator succeeds, with a separate serializable report projection.

Tracking issue:

`Luminous-Dynamics/luminous-dynamics#56`

This audit does **not** claim presentation can falsely promote an arbitrary generation. The demonstrated defects are a **presentation -> recovery veto authority edge plus incomplete local exact-subject/per-boot qualification binding and an overly data-like execution capability boundary**.

The repaired direction must be:

```text
locally observed physical boot identity
    -> recovery-native factual preparation subject
    + semantic recovery roots
    + local health
        -> non-deserializable executable qualification capability
        -> exact-subject bless of this healthy prepared boot
        -> semantic LKG commit when generation promotion is needed

executable qualification capability
        -> serialized evidence/report projection
        -/-> executable capability reconstruction
...
## 6. Authority finding: clean lifecycle evidence is generation-scoped

`CleanShutdownMarker` in the exact historical source records clean reboot/poweroff/suspend/hibernate evidence with:

- termination kind;
- uptime;
- optional generation;
- optional hardware fingerprint;
- wall-clock write time.

It does not carry the kernel `boot_id` or the prepared boot counter.

`BootStateStore::prepare()` loads that marker and, for a non-first boot, treats any parseable marker as the previous termination. It does not first prove that the marker belongs to the previously committed active prepared boot.

The reviewed host `markLifecycle` helper derives `/run/booted-system` and invokes `spore-boot-state shutdown`, but the CLI-supplied generation remains only generation-level evidence. A new boot of the same generation is a different physical subject and must not be allowed to inherit another boot's clean-termination claim once exact prepared subjects exist.

Tracking issue:

`Luminous-Dynamics/luminous-dynamics#61`

The defect is a provenance/authority mismatch, not a demonstrated false-promotion exploit. The required direction is:

```text
locally committed active prepared subject
    -> exact clean lifecycle marker
    -> next preparation verifies marker.subject == previous active subject
```

The CLI may provide diagnostic cross-checks, but it must not mint or choose `boot_id` or counter authority. Wall clock remains audit metadata, never subject identity.

Legacy markers also cannot be retroactively upgraded by inventing exact subject fields that historical bytes never contained.

## 7. Pre-extraction repair gates
...
2. **REPAIR-002 — Recovery authority is locally exact-subject and presentation-independent.** Remove morphology/presentation state from factual recovery preparation, qualification, blessing, and LKG-commit authority. Linux boot ID and booted generation derive from locally observed kernel/boot filesystem facts rather than caller-selected authority values. Preparation and replay use a recovery-native exact prepared-boot identity independent of `BootEcologyComposer`, `BootGenome`, `genome.seed`, visual seed availability, and morphology history. Executable qualification plans are non-deserializable capabilities distinct from serialized reports. Every healthy prepared boot, including `AlreadyKnownGood`, receives an exact-subject bless so per-boot qualification truth remains correct. Presentation mirroring occurs only downstream of committed recovery truth.
...
5. **REPAIR-005 — Exact lifecycle evidence.** Bind clean shutdown, reboot, suspend, and hibernate evidence to the exact locally committed prepared boot subject. Callers may cross-check but cannot mint or select boot ID/counter authority, and legacy lifecycle markers must never fabricate exact-subject provenance.
...
## 9. Extraction ownership theorem
...
Caller-supplied boot identity != physical boot subject authority
Serialized plan/report != executable qualification capability
Generation identity != exact lifecycle-subject identity
Caller-supplied lifecycle metadata != lifecycle subject authority
Visual replay identity != recovery transaction identity
Parseable evidence != current exact-subject evidence
Legacy evidence != fabricated stronger provenance
