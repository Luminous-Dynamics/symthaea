...
PIE models how an off-Earth settlement converts local resources, imported seed equipment, energy, utilities, labor/robotics, and recycled wastes into useful products and replacement productive capacity.

The program does **not** assume self-sufficiency, a particular ISRU process, or that the Moon and Mars share the same industrial optimum. Its job is to make resource/process dependencies explicit and evidence-bearing so competing industrial architectures can be compared.
...
- Local mass fraction is not equivalent to independence; critical imported items must remain visible.
- Recycling competes with mining and import on the same accounting basis.
- Waste and by-products never disappear; they terminate in a sink, inventory, recycle edge or explicit unknown.
- Process data derived from simulation/literature/vendor projections cannot become qualification evidence automatically.
