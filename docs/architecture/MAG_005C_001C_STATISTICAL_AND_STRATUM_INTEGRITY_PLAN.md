...
- exact two-sided definition: fixed-margin probability ordering, summing tables whose hypergeometric probability is less than or equal to the observed table probability;
- exact integer cell counts;
- fixed alpha declared before outcomes;
- no post-outcome choice of one-sided/two-sided alternative;
- no post-outcome choice among competing two-sided definitions;
...
The exact two-sided definition is authority-bearing because multiple two-sided Fisher conventions exist. A record that says only `FisherExactTwoSided` is underspecified.
...
For the exact probability-ordering profile above, the known answers are:

```text
top-K precision        = 0.80
random precision       = 0.20
absolute risk difference = 0.60
risk ratio             = 4
odds ratio             = 16
exact two-sided p      = 375911 / 1044291885
                       ≈ 0.0003599673667865378
```
...
- exact two-sided-definition identity where applicable;
- alternative;
- alpha;
- exact p-value representation where feasible plus rendered decimal;
- top-K precision;
- control precision;
...
1. statistically resolving 16/20 vs 4/20 positive control-difference case with exact rational p-value and effect-size known answers;
...
4. same counts but a different two-sided Fisher convention selected after reveal -> new lineage;
5. multiple primary endpoints with no preregistered multiplicity profile -> inferential decision invalid;
6. candidate appears in both TopK and RandomControl -> score-input refusal;
7. candidate appears twice inside TopK -> score-input refusal;
8. two labels resolve to the same canonical candidate before commitment -> allocation refusal until deduplicated;
9. alias discovered only after commitment -> preserve historical slot/disposition, no silent denominator repair;
10. OOD diagnostic tag overlaps TopK -> allowed because tag is not a primary stratum;
11. expected stratum size 20 but only 19 committed unique candidates -> allocation invalid;
12. random-control seed reproduces a different set than the stored membership -> allocation invalid;
13. stored p-value or effect size disagrees with exact recomputation -> reject stored summary;
14. significant exact test with contaminated Phase-A answer access -> no prospective credit despite the p-value;
15. significant exact test with incomplete terminal census -> evidence invalid;
16. significant scorecard result -> no synthesis, novelty, physical validation, manufacturing, or economic promotion.
