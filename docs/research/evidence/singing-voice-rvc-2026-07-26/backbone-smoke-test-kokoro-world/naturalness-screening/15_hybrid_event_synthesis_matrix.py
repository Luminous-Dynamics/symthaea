...
# Step 2 of the naturalness-improvement plan (synthetic-tumbling-raccoon):
# per-plan/F8c, pitch is measured exact to 1.2 cents median -- MORE precise
# than a human singer, which is itself unnatural. The existing vibrato+glide
# above were already present in every render the user judged "doesn't sound
# good," so this adds three elements that are NOT already present, on top
# of (not replacing) vibrato/glide. Toggled via NATURALIZE so A/B testing
# against the un-naturalized baseline is a one-line change.
NATURALIZE = False
NATURAL_DRIFT_RATE_HZ = 0.7        # slow wander, distinct from vibrato's 5.5Hz
NATURAL_DRIFT_DEPTH_CENTS = 10.0
NATURAL_JITTER_DEPTH_CENTS = 4.0   # frame-to-frame vocal-fold-instability noise
SCOOP_MS = 70.0                    # onset approach duration
SCOOP_DEPTH_CENTS = -60.0          # start this far BELOW target, ramp to 0
NATURALIZATION_SEED = 20260729     # deterministic, reproducible
...
def synthesize_word(f0_seg, sp_seg, ap_seg, f0_full, sp_full, ap_full, x_word, x_full, syll_specs,
    ...
    natural_mult = np.ones(len(word_f0_original))
    if NATURALIZE:
        rng = np.random.RandomState(NATURALIZATION_SEED)
        n_frames_word = len(word_f0_original)

        # Slow drift: sum of a few random-phase, randomized-frequency
        # low-frequency sinusoids (0.4-1.0 Hz band around
        # NATURAL_DRIFT_RATE_HZ), not filtered noise. A moving-average
        # convolution was tried first and had a real bug: at FRAME_DT=5ms,
        # a 0.7Hz smoothing window is 286 frames -- LONGER than many short
        # words' own frame count, so np.convolve(..., mode="same") returned
        # an array sized to the kernel, not the word, breaking multiplication
        # a few lines down. Sinusoid summation has no such failure mode --
        # its output length is exactly len(t_abs) by construction.
        t_local = t_abs  # already 0-based per-word; deterministic given the seed
        n_components = 3
        drift_cents = np.zeros(n_frames_word)
        for _ in range(n_components):
            freq = NATURAL_DRIFT_RATE_HZ * rng.uniform(0.6, 1.4)
            phase = rng.uniform(0, 2 * np.pi)
            drift_cents += np.sin(2 * np.pi * freq * t_local + phase)
        if drift_cents.std() > 1e-9:
            drift_cents = drift_cents / drift_cents.std() * NATURAL_DRIFT_DEPTH_CENTS
        drift_mult = 2.0 ** (drift_cents / 1200.0)

        # Micro-jitter: independent frame-to-frame perturbation.
        jitter_cents = rng.standard_normal(n_frames_word) * NATURAL_JITTER_DEPTH_CENTS
        jitter_mult = 2.0 ** (jitter_cents / 1200.0)

        # Onset scooping: each syllable's vowel approaches its target
        # pitch from below over the first SCOOP_MS, instead of starting
        # exactly on-target.
        scoop_mult = np.ones(n_frames_word)
        scoop_frames = max(1, round(SCOOP_MS / 1000.0 / FRAME_DT))
        for (s, e, vs, ve) in syll_frame_ranges:
            if vs is None or ve is None:
                continue
            n = min(scoop_frames, ve - vs)
            if n <= 1:
                continue
            ramp_cents = np.linspace(SCOOP_DEPTH_CENTS, 0.0, n)
            scoop_mult[vs:vs + n] = 2.0 ** (ramp_cents / 1200.0)

        natural_mult = drift_mult * jitter_mult * scoop_mult
    ...
    for e in entries:
        ...
        seg_natural = natural_mult[cursor:cursor + n]
        seg_orig_f0 = e["f0"]
        if e["vclass"] == "sonorant":
            eligible = seg_orig_f0 > 0
            e["final_f0"] = np.where(eligible, seg_target * seg_vibrato * seg_natural, 0.0)
...
if __name__ == "__main__":
    ...
    NATURALIZE = (sys.argv[6] == "1") if len(sys.argv) > 6 else False
