#!/usr/bin/env python3
"""Gate 3 (2026-07-28): word-level vs. phoneme-level retiming A/B.

This is a DELIBERATE ABLATION of 03_reshape_pyworld.py (the phoneme-aware
v2 pipeline) that changes ONLY the timing-method variable: every other
fix from the review round (per-word isolated synthesis, genuine silence
between words, RMS loudness-matching) is kept identical, so any
difference between this script's output and v2's is attributable to
word-level vs. phoneme-aware retiming alone, not confounded with the
click/loudness fixes. Per the reviewer's own Gate 3 spec: "Same phrases
and notes, with only the timing method changed." Output files use a
_wordlevel suffix so they don't clobber the phoneme-aware version.

Below (unmodified from 03_reshape_pyworld.py except where marked GATE3):
...
for phrase in config["phrases"]:
    ...
    align_path = BASE / "alignments" / f"{phrase['id']}.json"
    words = json.loads(align_path.read_text())
    melody = phrase["melody_hz"]
    assert len(words) == len(melody)
    ...
    assert len(ps_words) == len(words), (
        f"{phrase['id']}: {len(ps_words)} G2P word-tokens != {len(words)} aligned words "
        f"({ps_words!r} vs {word_texts!r})"
    )
    ...
    for word, target_hz, ps_word in zip(words, melody, ps_words):
        ...
        # GATE3: force a single whole-word span instead of real phoneme
        # classification -- the word-level ablation. `is_vowel=True` so
        # phoneme_sub_durations treats it as "the whole word is one vowel
        # nucleus" (uniform stretch), matching the original v1 behavior.
        phonemes = [("*", True)]
    ...
    out_path = BASE / "audio" / f"{phrase['id']}_sung_wordlevel.wav"
...
print("\nGate 3 word-level variant done.")
