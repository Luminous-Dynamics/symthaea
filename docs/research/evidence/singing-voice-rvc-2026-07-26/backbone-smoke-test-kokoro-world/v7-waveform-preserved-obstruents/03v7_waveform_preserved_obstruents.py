#!/usr/bin/env python3
"""v7 (2026-07-28): the reviewer's proposed 5th arm -- preserve the
...
def synthesize_word(f0_seg, sp_seg, ap_seg, x_word, syll_specs, fs):
    ...
    return crossfade_concat_fs(rendered_chunks, fs, CROSSFADE_MS)
...
def main(config_path, audio_dir, align_dir, out_suffix):
    ...
    for phrase in phrases:
        ...
        for word, sylls in zip(words, phrase["word_syllables"]):
            ...
            y_word = synthesize_word(f0_seg, sp_seg, ap_seg, x_word, syll_specs, fs)
    ...
    print("\nv7 (waveform-preserved obstruents) done.")
...
if __name__ == "__main__":
    ...
    out_suffix = sys.argv[4] if len(sys.argv) > 4 else "sung_v7"
