...
def crossfade_concat(chunks):
    """Concatenate waveform chunks with a short linear crossfade at
    every boundary, rather than a hard join."""
    if not chunks:
        return np.zeros(1)
    n_fade_default = 1  # placeholder, recomputed per call site with fs
    out = chunks[0].copy()
    for nxt in chunks[1:]:
...
def crossfade_concat_fs(chunks, fs, crossfade_ms):
    n_fade = max(1, int(fs * crossfade_ms / 1000.0))
    if not chunks:
        return np.zeros(1)
    out = chunks[0].copy()
    for nxt in chunks[1:]:
        n = min(len(out), len(nxt), n_fade)
        if n <= 1:
            out = np.concatenate([out, nxt])
            continue
        fade_out = np.linspace(1.0, 0.0, n)
...
def crossfade_concat_with_lineage(chunks, fs, crossfade_ms):
    """Same crossfade-concatenation as crossfade_concat_fs, but also
    returns, for each chunk, its exact (start, end) sample range in the
    FINAL output -- ground truth, not inferred, since this function
    performs the exact concatenation. Once a chunk's bounds are recorded,
    later merges only ever modify the TAIL of the growing array (the
    crossfade-overlap region, which falls exactly within that chunk's
    own recorded [end-n_fade, end) range), so recorded bounds remain
    valid positions in the final array."""
    n_fade = max(1, int(fs * crossfade_ms / 1000.0))
    if not chunks:
        return np.zeros(1), []
    out = chunks[0].copy()
    bounds = [{"start": 0, "end": len(chunks[0])}]
    for nxt in chunks[1:]:
        n = min(len(out), len(nxt), n_fade)
        if n <= 1:
            new_start = len(out)
...
def synthesize_word(f0_seg, sp_seg, ap_seg, x_word, syll_specs, fs):
    ...
    y_word, bounds = crossfade_concat_with_lineage(rendered_chunks, fs, CROSSFADE_MS)
    ...
    n_fade = max(1, int(fs * CROSSFADE_MS / 1000.0))
    lineage = []
    for i, ((method, group_entries), b) in enumerate(zip(groups, bounds)):
        has_entry_fade = i > 0
        has_exit_fade = i < len(groups) - 1
        core_start = b["start"] + (n_fade if has_entry_fade else 0)
        core_end = b["end"] - (n_fade if has_exit_fade else 0)
        ...
        record = {
            ...
            "entry_fade": [int(b["start"]), int(b["start"] + n_fade)] if has_entry_fade else None,
            "exit_fade": [int(b["end"] - n_fade), int(b["end"])] if has_exit_fade else None,
        }
        if method == "raw":
...
if __name__ == "__main__":
    ...
    out_suffix = sys.argv[4] if len(sys.argv) > 4 else "sung_v7b"
