#!/usr/bin/env python3
"""Gate 4 (generalization gate): evaluate one generalize01-training checkpoint.

Renders the HELD-OUT phrase ("won't you sing along with me",
heldout01.ds -- real ph_seq/ph_dur/f0, never included in training) through:
  1. the trained NSF-HiFiGAN vocoder (native infer.py acoustic)
  2. the raw predicted mel -> Griffin-Lim (same isolation method as Gates 2-3)
and transcribes both with Whisper. Every phoneme in this phrase was seen
during training in other contexts, but this exact sequence never was --
a genuine compositional-generalization test, unlike Gate 3's memorization.

Usage: python3 gate4_eval_checkpoint.py <step>
"""
import subprocess
import sys
import os
...
def render(step, mel_only):
    ...
    cmd = [
        ...
        f"{DS}/heldout01.ds", "--exp", "generalize01-gate4", "--ckpt", str(step),
