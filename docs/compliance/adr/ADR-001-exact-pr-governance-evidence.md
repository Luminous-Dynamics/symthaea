...
The remaining governance problem is two-dimensional:

1. bind the *change-set being classified* to the exact immutable event base/head pair; and
2. ensure the authoritative governance evaluator itself comes from trusted base-branch code and never executes untrusted PR-head code.
...
- treats safety-critical source paths, the governance charter, policy, validator, ordinary governance workflow, and base-controlled governance-root workflow as Class A surfaces;
...
- emits explicit nonclaims that structural validation does not establish scientific adequacy, test execution, full-CI success, or merge authorization.

Also stage `.github/workflows/pr-governance-root.yml` as the stronger future merge-authority context:

- trigger with `pull_request_target` lifecycle events so the workflow definition and validator come from trusted base-branch code;
- use only `contents: read` permissions;
- check out the exact event base SHA with credentials persistence disabled;
...
- run the trusted base validator over the exact event base/head Git objects;
- retain the same draft admission and stable concurrency/revocation model.

The ordinary `pull_request` Governance check remains useful integration evidence, but the staged `Governance Root Check` is the intended eventual required-status authority once it has been merged, behaviorally qualified, and bound into the trusted-root ruleset.

### Scientific Basis

The basis is empirical repository and platform evidence rather than a claim of scientific novelty. Historical #4960 execution showed that PR association and checkout subject can differ. Inspection of the existing governance helper showed a mutable `origin/$GITHUB_BASE_REF...HEAD` comparison, a silent `HEAD~1` fallback, and an always-zero CI exit path even when Class A changes were detected. Inspection of ordinary PR workflow semantics then exposed the self-modification boundary: PR-head workflow changes can affect the lower-authority integration check itself.

The stronger design follows the repository's established evidence discipline: bind authority to exact immutable identities, keep the trusted evaluator outside the untrusted subject it evaluates, fail closed when identity cannot be established, and keep structural evidence distinct from execution and scientific evidence.
...
No existing product-runtime risk entry is changed by this governance-only patch. The change reduces repository-process risk in three ways: a Class A detection can no longer silently produce a green governance status; changed-file classification no longer depends on a moving branch ref; and the staged authoritative root does not execute PR-head governance code.
...
- [x] The Python validator has embedded positive/negative `--self-test` coverage for path classification, authority-specific commit prefixes, required ADR structure, template-placeholder rejection, and attempted removal of the policy's own Class A root.
- [x] The policy JSON is validated against hard-coded minimum protected roots and expected ADR/prefix semantics.
- [x] Workflow Syntax selects PR workflow/script changes using exact event base/head SHAs rather than a mutable branch name.
- [x] The base-controlled workflow is structurally read-only and contains no PR-head checkout or PR-head execution step.
- [ ] The new `pull_request_target` Governance Root cannot behaviorally execute from this introduction PR because it is not yet present on the trusted base; qualification must occur only after merge into the base/default lineage.
...
Positive consequences: immutable change-set identity, fail-closed Class A structural policy, self-protection of the governance surface, a base-controlled read-only merge-authority path, stronger evidence for any future required Governance context, and clearer separation between structural compliance and execution/scientific evidence.

Negative consequences: Class A PRs now require an ADR and approved commit prefix before the strict governance gate can pass; the stronger root requires a two-stage activation sequence because it cannot govern the PR that first introduces it; hosted-runner qualification remains pending; and Class B remains advisory until #5363 resolves the documentation conflict.
