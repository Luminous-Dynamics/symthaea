...
7. **No monorepo CI — but remember to push to standalone** - Do NOT add GitHub Actions workflows to this private monorepo. CI runs on the public standalone repos only. **When you land work in `symthaea/` or `mycelix-workspace/mycelix-*/`, you are NOT done until the corresponding standalone is also pushed** — otherwise CI never sees it and the public artifact drifts from main. The sync scripts:
   ...
   - `mycelix-workspace/mycelix-pulse/scripts/sync-to-standalone.sh` → `github.com/Luminous-Dynamics/mycelix-mail` ⚠️ **ARCHIVED as of Apr 19 2026** — sync script points at a read-only target; needs the repo unarchived OR a replacement `mycelix-pulse` repo created before Phase 5A can be published for CI
   - `mycelix-praxis/scripts/sync-to-standalone.sh` → Praxis standalone
   - `mycelix-workspace/scripts/sync-to-standalone.sh` → broader mycelix workspace
   - `_infrastructure/ci-templates/sync-to-standalone.sh` → generic template
   Default workflow: `./scripts/sync-to-standalone.sh --dry-run` to preview, then without the flag to sync, then `cd $STANDALONE_DIR && git add -A && git commit -m "sync: update from monorepo" && git push origin main`.
