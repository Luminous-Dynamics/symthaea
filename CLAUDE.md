...
- **Architecture**: Predictive coding loop — HDC encode → CfC evolve → predict → learn (~31Hz measured, 20Hz budget). **Known gap**: the LLM-facing facade (`Symthaea::process()`) and the autonomous ~31Hz tick loop (`CognitiveLoopService::cycle()`) share almost no state/logic — treat "8-phase pipeline" as describing the facade only, not the full system. The facade itself has also grown ~9 undocumented sub-phases (3.5, 4.5, 5.5, 6.75...) beyond the original 8.
- **Key entry points**:
  - `src/symthaea.rs` — public facade (8-phase pipeline: perception → cognition → translation; see architecture gap note above)
...
- **CognitiveLoopService refactor** (Mar 2026) — **claim not currently accurate, left here for history**: a refactor to 56→38 fields via 3 sub-structs (`consciousness`/`memory`/`behavior`) plus an EthicsAndValuesManager→EthicsEngine merge was documented as done, but the 2026-07-01 review found the struct has grown back to ~135 fields and EthicsAndValuesManager and EthicsEngine both still exist and are both read in the cycle — the dual-throttle moral-evaluation problem the merge was meant to fix is still present. Needs re-investigation, not just a doc update.
...
- **CI**: `symthaea-ci.yml` (GREEN) — fmt, clippy, test, docs, 49 feature matrix, 139 sub-crates
