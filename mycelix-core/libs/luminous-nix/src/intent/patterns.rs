...
impl PatternMatcher {
    ...
    pub fn new() -> Self {
        Self {
            search_patterns: vec![
                Regex::new(r"^search\s+(.+)$").unwrap(),
                Regex::new(r"^find\s+(.+)$").unwrap(),
                Regex::new(r"^look\s+for\s+(.+)$").unwrap(),
                Regex::new(r"^looking\s+for\s+(.+)$").unwrap(),
                Regex::new(r"^query\s+(.+)$").unwrap(),
                Regex::new(r"^locate\s+(.+)$").unwrap(),
                Regex::new(r"^where\s+is\s+(.+)$").unwrap(),
            ],
            install_patterns: vec![
                Regex::new(r"^install\s+(.+)$").unwrap(),
                Regex::new(r"^add\s+(.+)$").unwrap(),
                Regex::new(r"^get\s+(.+)$").unwrap(),
                Regex::new(r"^setup\s+(.+)$").unwrap(),
                Regex::new(r"^enable\s+(.+)$").unwrap(),
            ],
            remove_patterns: vec![
                Regex::new(r"^remove\s+(.+)$").unwrap(),
                Regex::new(r"^uninstall\s+(.+)$").unwrap(),
                Regex::new(r"^delete\s+(.+)$").unwrap(),
                Regex::new(r"^rm\s+(.+)$").unwrap(),
                Regex::new(r"^disable\s+(.+)$").unwrap(),
                Regex::new(r"^purge\s+(.+)$").unwrap(),
            ],
            generations_patterns: vec![
                Regex::new(r"^(list\s+)?generations?$").unwrap(),
                Regex::new(r"^show\s+generations?$").unwrap(),
                Regex::new(r"^generation\s+history$").unwrap(),
                Regex::new(r"^system\s+history$").unwrap(),
                Regex::new(r"^list\s+system\s+versions?$").unwrap(),
            ],
            rebuild_patterns: vec![
                Regex::new(r"^rebuild(\s+system)?$").unwrap(),
                Regex::new(r"^nixos-rebuild$").unwrap(),
                Regex::new(r"^update\s+system$").unwrap(),
                Regex::new(r"^switch\s+configuration$").unwrap(),
                Regex::new(r"^apply\s+config(uration)?$").unwrap(),
            ],
            gc_patterns: vec![
                Regex::new(r"^gc$").unwrap(),
                Regex::new(r"^garbage\s+collect(ion)?$").unwrap(),
                Regex::new(r"^clean(\s+up)?$").unwrap(),
                Regex::new(r"^cleanup$").unwrap(),
                Regex::new(r"^nix-collect-garbage$").unwrap(),
                Regex::new(r"^delete\s+old\s+generations?$").unwrap(),
                Regex::new(r"^free\s+up\s+space$").unwrap(),
                Regex::new(r"^clear\s+cache$").unwrap(),
            ],
            status_patterns: vec![
                Regex::new(r"^status$").unwrap(),
                Regex::new(r"^system\s+(info|status)$").unwrap(),
                Regex::new(r"^show\s+status$").unwrap(),
                Regex::new(r"^(what|which)\s+version$").unwrap(),
                Regex::new(r"^nixos\s+version$").unwrap(),
                Regex::new(r"^current\s+generation$").unwrap(),
            ],
            help_patterns: vec![
                Regex::new(r"^help$").unwrap(),
                Regex::new(r"^how\s+to\s+use$").unwrap(),
                Regex::new(r"^what\s+can\s+you\s+do$").unwrap(),
                Regex::new(r"^commands?$").unwrap(),
                Regex::new(r"^usage$").unwrap(),
            ],
            argument_pattern: Regex::new(r"^\w+\s+(.+)$").unwrap(),
        }
    }
