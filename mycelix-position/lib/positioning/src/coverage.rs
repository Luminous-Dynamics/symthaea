...
pub fn gdop(anchors: &[[f64; 3]], point: &[f64; 3]) -> f64 {
    ...
    match invert_3x3(&jtj) {
        Some(inv) => {
            ...
            if trace > 0.0 { trace.sqrt() } else { f64::INFINITY }
        }
        None => f64::INFINITY,
    }
}
...
pub fn gdop_2d(anchors: &[[f64; 2]], point: &[f64; 2]) -> f64 {
    ...
    if trace_inv > 0.0 { trace_inv.sqrt() } else { f64::INFINITY }
}
...
pub fn coverage_grid_2d(
    ...
    while x <= x_range.1 {
        ...
        while y <= y_range.1 {
            ...
            points.push(CoveragePoint { x, y, z: 0.0, gdop: g, pdop: g });
            y += step;
        }
        x += step;
    }
...
mod tests {
    ...
    fn gdop_collinear_is_high() {
        ...
        let g = gdop(&anchors, &[0.0, 0.0, 0.0]);
        assert!(g > 10.0 || g.is_infinite(), "GDOP {} should be high for collinear", g);
    }
    ...
    fn gdop_2d_triangle() {
        ...
        assert!(g < 5.0, "2D GDOP {} should be reasonable for equilateral triangle", g);
    }
    ...
    fn coverage_grid_produces_points() {
        ...
        let center = grid.iter().find(|p| (p.x - 50.0).abs() < 13.0 && (p.y - 50.0).abs() < 13.0);
