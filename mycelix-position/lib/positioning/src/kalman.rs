...
impl PositionFilter {
    ...
    pub fn predict(&mut self, dt: f64) {
        ...
        self.state.covariance[STATE_DIM + 1] += q_pos + self.state.covariance[4 * STATE_DIM + 4] * dt * dt;
        self.state.covariance[2 * STATE_DIM + 2] += q_pos + self.state.covariance[5 * STATE_DIM + 5] * dt * dt;
    ...
    pub fn update(
        ...
        if measured_range <= 0.0 || range_sigma <= 0.0 { return; }
        ...
        if predicted_range < 1e-10 { return; }
...
mod tests {
    ...
    fn trust_weighting_affects_convergence() {
        ...
        assert!((pos[0] - 50.0).abs() < 10.0, "Position {} should be near 50", pos[0]);
    }
}
