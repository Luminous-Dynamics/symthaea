...
fn bench_trilateration_3d() {
    ...
    let anchors: Vec<[f64; 3]> = vec![
        ...
        [0.0, 0.0, 100.0], [100.0, 100.0, 0.0], [50.0, 50.0, 100.0],
    ];
    ...
    let error = ((est.position[0] - target[0]).powi(2)
        ...
        + (est.position[2] - target[2]).powi(2)).sqrt();
...
fn bench_trilateration_2d() {
    ...
    let anchors: Vec<[f64; 2]> = vec![
        [0.0, 0.0], [100.0, 0.0], [50.0, 86.6], [0.0, 100.0],
    ];
    let ranges: Vec<f64> = anchors.iter().map(|a| {
        let dx: f64 = a[0] - target[0];
        let dy: f64 = a[1] - target[1];
    ...
    let error = ((est.position[0] - target[0]).powi(2)
        + (est.position[1] - target[1]).powi(2)).sqrt();
...
fn bench_ranging_models() {
    ...
    let start = Instant::now();
    for _ in 0..n { rssi_to_range(-65.0, -50.0, 2.5); }
    println!("RSSI ranging:  {n} calls in {:?} ({:?}/call)",
        start.elapsed(), start.elapsed() / n);

    let start = Instant::now();
    for _ in 0..n { uwb_tof_to_range(33.36); }
    println!("UWB ranging:   {n} calls in {:?} ({:?}/call)",
        start.elapsed(), start.elapsed() / n);

    let start = Instant::now();
    for _ in 0..n { lora_toa_to_range(3336.0, 125_000.0); }
    println!("LoRa ranging:  {n} calls in {:?} ({:?}/call)",
        start.elapsed(), start.elapsed() / n);

    let start = Instant::now();
    for _ in 0..n { wifi_rtt_to_range(66.7); }
    println!("WiFi RTT:      {n} calls in {:?} ({:?}/call)\n",
        start.elapsed(), start.elapsed() / n);
}

fn bench_gdop() {
    let anchors = vec![
        [100.0, 0.0, 0.0], [-50.0, 86.6, 0.0],
        [-50.0, -86.6, 0.0], [0.0, 0.0, 100.0],
    ];
    let point = [0.0, 0.0, 0.0];

    let n = 100_000;
    let start = Instant::now();
    for _ in 0..n { gdop(&anchors, &point); }
    let elapsed = start.elapsed();
