...
impl MockDHT {
    ...
    fn complete_transfer(&mut self, transfer_id: &str, completion_time: i64) -> bool {
        ...
        let encumbrances = self.get_deed(&property_id)
            .map(|d| d.encumbrances.clone())
            .unwrap_or_default();

        // Get previous deed ID
        let previous_deed_id = self.get_deed(&property_id)
            .map(|d| d.id.clone())
            .unwrap_or_default();
    ...
    fn file_dispute(&mut self, dispute: MockDispute) {
        self.events.push(MockBridgeEvent {
            ...
            payload: format!(r#"{{"claimant":"{}","respondent":"{}"}}"#, dispute.claimant_did, dispute.respondent_did),
            timestamp: 1704067200000000,
        });
    ...
    fn get_events_for_property(&self, property_id: &str) -> Vec<&MockBridgeEvent> {
        self.events.iter().filter(|e| e.property_id == property_id).collect()
    }
}
...
mod tests {
    ...
    fn test_complete_transfer_updates_registry() {
        ...
        let transfer = MockTransfer {
            ...
            conditions: vec![
                MockCondition { condition_type: "PaymentReceived".to_string(), satisfied: false },
            ],
            completed: None,
        };
        ...
        let deeds: Vec<_> = dht.deeds.iter().filter(|d| d.property_id == "property:1").collect();
    ...
    fn test_deed_chain_integrity() {
        ...
        // 4. Verify deed chain
        let deeds: Vec<_> = dht.deeds.iter()
    ...
    fn test_dispute_escalation_to_justice() {
        ...
        assert_eq!(escalated_dispute.justice_case_id, Some("JUSTICE-CASE-001".to_string()));
    }
    ...
    fn test_dispute_creates_bridge_event() {
        ...
        let dispute_event = events.iter().find(|e| e.event_type == "DisputeFiled").unwrap();
    ...
    fn test_ownership_transfer_broadcasts_event() {
        ...
        let transfer_event = events.iter().find(|e| e.event_type == "OwnershipTransferred").unwrap();
    ...
    fn test_multiple_transfers_same_property() {
        ...
        for i in 0..(owners.len() - 1) {
            ...
            dht.initiate_transfer(transfer);
            dht.complete_transfer(&format!("transfer:multi:{}", i), 1704000000000000 + (i as i64) * 100000000);
        }
        ...
        let deeds: Vec<_> = dht.deeds.iter().filter(|d| d.property_id == "property:multi").collect();
    ...
    fn test_encumbrance_persistence_across_multiple_transfers() {
        ...
        for i in 1..=2 {
            ...
            dht.initiate_transfer(transfer);
            dht.complete_transfer(&format!("transfer:enc:persist:{}", i), 1704000000000000 + (i as i64) * 100000000);
        }

        // Verify encumbrance still present on latest deed
        let latest_deed = dht.deeds.iter()
