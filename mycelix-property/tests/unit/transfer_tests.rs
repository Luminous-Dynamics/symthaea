...
mod tests {
    ...
    fn test_all_transfer_types_supported() {
        ...
        for t in types {
            assert!(matches!(t, TestTransferType::Sale |
                TestTransferType::Gift | TestTransferType::Inheritance |
                TestTransferType::CourtOrder | TestTransferType::Exchange |
                TestTransferType::Other));
        }
    }
    ...
    fn test_transfer_condition_types_supported() {
        ...
        for t in types {
            assert!(matches!(t, TestConditionType::PaymentReceived |
                TestConditionType::InspectionComplete | TestConditionType::TitleClear |
                TestConditionType::DocumentsSigned | TestConditionType::TaxesPaid |
    ...
    fn test_condition_satisfaction_tracking() {
        ...
        assert!(validate_did(transfer.conditions[0].verified_by.as_ref().unwrap()));
    }
    ...
    fn test_transfer_status_transitions() {
        ...
        for status in statuses {
            assert!(matches!(status, TestTransferStatus::Initiated |
                TestTransferStatus::AwaitingAcceptance | TestTransferStatus::InEscrow |
                TestTransferStatus::ConditionsPending | TestTransferStatus::Completed |
                TestTransferStatus::Cancelled | TestTransferStatus::Disputed));
        }
    }
