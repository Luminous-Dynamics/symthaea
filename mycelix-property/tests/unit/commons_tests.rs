...
mod tests {
    ...
    fn test_all_resource_types_supported() {
        ...
        for t in types {
            assert!(matches!(t, TestResourceType::Land |
                TestResourceType::Water | TestResourceType::Forest |
                TestResourceType::Fishery | TestResourceType::Pasture |
                TestResourceType::Infrastructure | TestResourceType::Digital |
    ...
    fn test_governance_decision_methods_supported() {
        ...
        for method in methods {
            assert!(matches!(method, TestDecisionMethod::Consensus |
                TestDecisionMethod::Majority | TestDecisionMethod::SuperMajority |
                TestDecisionMethod::Stewards));
        }
    }
    ...
    fn test_all_right_types_supported() {
        ...
        for t in types {
            assert!(matches!(t, TestRightType::Access |
                TestRightType::Extraction | TestRightType::Management |
                TestRightType::Exclusion | TestRightType::Alienation));
        }
    }
    ...
    fn can_remove_steward(resource: &TestCommonResource, removed_by_did: &str) -> bool {
        resource.stewards.contains(&removed_by_did.to_string()) &&
        resource.stewards.len() > 1 // Cannot remove last steward
    }
