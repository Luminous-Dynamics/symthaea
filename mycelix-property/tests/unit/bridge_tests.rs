...
fn create_test_bridge_event() -> TestPropertyBridgeEvent {
    TestPropertyBridgeEvent {
        ...
        payload: r#"{"from_did":"did:mycelix:oldowner","to_did":"did:mycelix:newowner"}"#.to_string(),
...
mod tests {
    ...
    fn test_ownership_query_has_purpose() {
        let query = create_test_ownership_query();
        assert!(matches!(query.purpose, TestOwnershipQueryPurpose::CollateralVerification |
    ...
    fn test_all_query_purposes_supported() {
        ...
        for purpose in purposes {
            assert!(matches!(purpose, TestOwnershipQueryPurpose::CollateralVerification |
                TestOwnershipQueryPurpose::TransferVerification |
                TestOwnershipQueryPurpose::DisputeResolution |
                TestOwnershipQueryPurpose::TaxAssessment));
        }
    ...
    fn test_all_ownership_types_supported() {
        ...
        for t in types {
            assert!(matches!(t, TestOwnershipType::Sole |
                TestOwnershipType::Joint | TestOwnershipType::Fractional |
                TestOwnershipType::Corporate | TestOwnershipType::Trust |
                TestOwnershipType::Commons));
        }
    }
    ...
    fn test_all_pledge_statuses_supported() {
        ...
        for status in statuses {
            assert!(matches!(status, TestPledgeStatus::Active |
                TestPledgeStatus::Released | TestPledgeStatus::Foreclosed));
        }
    }
    ...
    fn test_all_event_types_supported() {
        ...
        for t in types {
            assert!(matches!(t, TestPropertyEventType::OwnershipTransferred |
                TestPropertyEventType::PropertyRegistered |
                TestPropertyEventType::CollateralPledged |
                TestPropertyEventType::CollateralReleased |
                TestPropertyEventType::EncumbranceAdded |
    ...
    fn test_ownership_transfer_event() {
        let event = create_test_bridge_event();
        assert_eq!(event.event_type, TestPropertyEventType::OwnershipTransferred);
    ...
    fn test_dispute_filed_event() {
        ...
        event.payload = r#"{"dispute_type":"Boundary","claimant":"did:mycelix:neighbor"}"#.to_string();
    ...
    fn test_all_encumbrance_types_supported() {
        ...
        for t in types {
            assert!(matches!(t, TestEncumbranceType::Mortgage |
                TestEncumbranceType::Lien | TestEncumbranceType::Easement |
                TestEncumbranceType::Lease | TestEncumbranceType::CollateralPledge));
        }
    }
    ...
    fn test_query_from_finance_happ() {
        ...
        assert_eq!(query.purpose, TestOwnershipQueryPurpose::CollateralVerification);
    }
    ...
    fn test_ownership_result_with_multiple_encumbrances() {
        ...
        let total_encumbered: u64 = result.encumbrances
            .iter()
    ...
    fn test_ownership_query_audit_trail() {
        ...
        let unique_happs: std::collections::HashSet<_> = queries.iter().map(|q| &q.source_happ).collect();
        assert_eq!(unique_happs.len(), 3);
        // In chronological order
        assert!(queries.windows(2).all(|w| w[0].queried_at < w[1].queried_at));
    }
