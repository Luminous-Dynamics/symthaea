...
fn create_test_dispute() -> TestPropertyDispute {
    TestPropertyDispute {
        ...
        description: "Fence encroaches 2 meters onto claimant's property according to new survey".to_string(),
        evidence_ids: vec![
...
mod tests {
    ...
    fn test_all_dispute_types_supported() {
        ...
        for t in types {
            assert!(matches!(t, TestDisputeType::Boundary |
                TestDisputeType::Ownership | TestDisputeType::Encumbrance |
                TestDisputeType::Easement | TestDisputeType::Trespass |
                TestDisputeType::Damage | TestDisputeType::Other(_)));
        }
    }
    ...
    fn test_all_dispute_statuses_supported() {
        ...
        for status in statuses {
            assert!(matches!(status, TestDisputeStatus::Filed |
                TestDisputeStatus::UnderReview | TestDisputeStatus::Mediation |
                TestDisputeStatus::Arbitration | TestDisputeStatus::Resolved |
                TestDisputeStatus::Dismissed));
        }
    }

    fn is_dispute_active(status: &TestDisputeStatus) -> bool {
        matches!(status, TestDisputeStatus::Filed |
            TestDisputeStatus::UnderReview | TestDisputeStatus::Mediation |
            TestDisputeStatus::Arbitration)
    }

    fn is_dispute_closed(status: &TestDisputeStatus) -> bool {
        matches!(status, TestDisputeStatus::Resolved | TestDisputeStatus::Dismissed)
    }
    ...
    fn test_all_claim_bases_supported() {
        ...
        for basis in bases {
            assert!(matches!(basis, TestClaimBasis::PriorOwnership |
                TestClaimBasis::Inheritance | TestClaimBasis::AdversePossession |
                TestClaimBasis::FraudulentTransfer | TestClaimBasis::DocumentaryEvidence |
    ...
    fn test_all_claim_statuses_supported() {
        ...
        for status in statuses {
            assert!(matches!(status, TestClaimStatus::Pending |
                TestClaimStatus::UnderInvestigation | TestClaimStatus::Validated |
                TestClaimStatus::Rejected | TestClaimStatus::Superseded));
        }
    }

    fn is_claim_active(status: &TestClaimStatus) -> bool {
        matches!(status, TestClaimStatus::Pending | TestClaimStatus::UnderInvestigation)
    }

    fn is_claim_resolved(status: &TestClaimStatus) -> bool {
        matches!(status, TestClaimStatus::Validated |
            TestClaimStatus::Rejected | TestClaimStatus::Superseded)
    }
    ...
    fn test_dispute_with_many_evidence_items() {
        let mut dispute = create_test_dispute();
        dispute.evidence_ids = (0..20)
            .map(|i| format!("evidence_{}.pdf", i))
            .collect();
    ...
    fn test_claim_with_many_supporting_documents() {
        let mut claim = create_test_ownership_claim();
        claim.supporting_documents = (0..15)
