//! # Mycelix Property - Sweettest Integration Tests
//!
...
mod registry_tests {
    ...
    async fn test_get_owner_properties() {
        ...
        let results: Vec<Record> = conductor
            .call(&cell.zome("registry"), "get_owner_properties", owner.clone())
            .await;

        assert!(results.len() >= 2, "Owner should have at least 2 properties");
    }
    ...
    async fn test_verify_ownership_and_clear_title() {
        ...
        let has_clear: bool = conductor
            .call(&cell.zome("registry"), "has_clear_title", property.id.clone())
            .await;

        assert!(has_clear, "Newly registered property should have clear title");
    }
}
...
mod transfer_tests {
    ...
    async fn test_initiate_and_complete_transfer() {
        ...
        let completed: Record = conductor
            .call(&cell.zome("transfer"), "complete_transfer", transfer.id.clone())
            .await;

        let completed_transfer: Transfer = decode_entry(&completed).expect("decode completed");
        assert!(completed_transfer.completed.is_some(), "Transfer should have completion time");
    }
}
...
mod disputes_tests {
    ...
    async fn test_file_and_get_dispute() {
        ...
        let disputes: Vec<Record> = conductor
            .call(&cell.zome("disputes"), "get_property_disputes", "prop:disputed-123".to_string())
            .await;
...
mod commons_tests {
    ...
    async fn test_create_common_resource_and_grant_right() {
        ...
        let record: Record = conductor
            .call(&cell.zome("commons"), "create_common_resource", resource_input)
            .await;
...
mod bridge_tests {
    ...
    async fn test_verify_ownership_via_bridge() {
        ...
        let result: OwnershipResult = conductor
            .call(&cell.zome("property_bridge"), "verify_ownership", verify_input)
            .await;
...
mod lifecycle_tests {
    ...
    async fn test_register_transfer_dispute_lifecycle() {
        ...
        let title_deed: Option<Record> = conductor
            .call(&cell.zome("registry"), "get_title_deed", property.id.clone())
            .await;
