//! Property Bridge Integrity Zome
//!
...
pub fn validate_create_entry_ownership_query(
    ...
    if query.property_id.is_empty() {
        return Ok(ValidateCallbackResult::Invalid("Property ID required".into()));
    }
...
pub fn validate_create_entry_collateral_pledge(
    ...
    if !pledge.owner_did.starts_with("did:mycelix:") {
        return Ok(ValidateCallbackResult::Invalid("Owner must have valid DID".into()));
    }
    Ok(ValidateCallbackResult::Valid)
}
