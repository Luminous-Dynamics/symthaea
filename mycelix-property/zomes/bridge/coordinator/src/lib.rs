//! Property Bridge Coordinator Zome
//!
...
pub fn verify_ownership(input: VerifyOwnershipInput) -> ExternResult<OwnershipResult> {
    ...
    let query = OwnershipQuery {
        id: format!("ownership:{}:{}:{}", input.source_happ, input.property_id, now.as_micros()),
    ...
    let (owner_did, ownership_type, share_percentage, encumbrances) = match registry_result {
        Some(data) => (
            data.owner_did,
            if data.co_owners.is_empty() { OwnershipType::Sole } else { OwnershipType::Joint },
            if data.co_owners.is_empty() { 100.0 } else {
                100.0 - data.co_owners.iter().map(|c| c.share_percentage).sum::<f64>()
            },
            data.encumbrances.iter().map(|e| EncumbranceInfo {
    ...
    create_link(anchor_hash(&input.property_id)?, hash, LinkTypes::PropertyToOwnership, ())?;

    // Verify against expected owner if provided
    if let Some(expected) = &input.expected_owner {
        if owner_did != *expected {
            return Err(wasm_error!(WasmErrorInner::Guest(
                format!("Owner mismatch: expected {} but found {}", expected, owner_did)
            )));
        }
    }
...
pub fn pledge_collateral(input: PledgeCollateralInput) -> ExternResult<Record> {
    ...
    let pledge = CollateralPledge {
        id: format!("pledge:{}:{}:{}", input.property_id, input.pledged_to_happ, now.as_micros()),
        property_id: input.property_id.clone(),
