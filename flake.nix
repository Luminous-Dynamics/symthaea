      ...
      eachSystem = flake-utils.lib.eachDefaultSystem (system:
        let
          overlays = [ (import rust-overlay) nix-ros-overlay.overlays.default
            (final: prev: {
              # sdl3's own test suite (pulled in transitively via
              # torchaudio -> torchcodec -> ffmpeg -> sdl2-compat -> sdl3)
              # includes a real-time-scheduling test (testrwlock) that
              # times out under this host's heavy concurrent-session CPU
              # oversubscription (load 30-60 on 12 cores is routine here,
              # see .claude/rules/CONCURRENT_SESSIONS.md) -- not a defect
              # in sdl3 itself, so skip checks rather than block builds on
              # host-load flakiness. Confirmed reproducible: the vanilla
              # (checked) sdl3 derivation failed this exact test twice.
              sdl3 = prev.sdl3.overrideAttrs (_: { doCheck = false; doInstallCheck = false; });

              # nixpkgs' python3Packages.torchaudio runs its own full pytest
              # suite as part of the build (observed: 35+ minutes at 100% CPU
              # and counting, not yet finished) -- irrelevant to us (we only
              # consume the library, not torchaudio's own upstream test
              # coverage) and a major, avoidable tax on every voice-vocoder
              # devShell (re)build, especially after nix-store GC evicts the
              # cached output (observed to happen more than once this
              # session from unrelated disk-pressure activity on this
              # shared host).
              ...
              # nixpkgs' top-level `python3Packages` isn't guaranteed to
              # re-derive from an overridden `python3` in every code path --
              # bind it explicitly so every consumer referencing
              # `pkgs.python3Packages` (vocosPkg, pythonVocoderEnv) sees the
              # same doCheck=false torchaudio, instead of two different
              # torchaudio derivations getting built (one overridden, one
              # vanilla) as happened on the first attempt at this fix.
        ...
        # Vocos (charactr/vocos-mel-24khz) -- not in nixpkgs, so packaged here
        # from the official PyPI wheel (pure-python, no build-backend needed)
        # instead of `pip install vocos` into an ad-hoc venv, per the
        # project's "immutable root, no pip" rule. All of its declared
        # runtime deps (torch, torchaudio, numpy, scipy, einops, pyyaml,
        # huggingface-hub, encodec==0.1.1 exact) are already in nixpkgs.
        vocosPkg = pkgs.python3Packages.buildPythonPackage rec {
          pname = "vocos";
          version = "0.1.0";
          format = "wheel";
          src = pkgs.fetchurl {
        ...
        # Vocos evaluation lane: Kokoro+WORLD singing-voice research (see
        # SYMTHAEA_VOCAL_IMPROVEMENT_PLAN_2026-07-28.md Step 4) needs a
        # neural-vocoder resynthesis pass. This env supplies torch/
        # torchaudio/vocos/soundfile via nix rather than pip; the rest of
        # that research pipeline (Kokoro TTS, pyworld, faster-whisper,
        # DNSMOS/UTMOS) stays in its existing pip venv under
        # /var/lib/symthaea/training-runs/ -- not re-migrated here, only
        # the new dependency this step introduces goes through the flake.
        pythonVocoderEnv = pkgs.python3.withPackages (ps: with ps; [
          torch
          torchaudio
          numpy
          scipy
      ...
      in {
        ...
        devShells.muse = pkgs.mkShell {
          ...
          shellHook = commonShellHook + ''
            ...
            # ABSOLUTE, not "./data/samples/vcsl": a relative path resolves
            # against each process's CWD, so anything launched from outside
            # symthaea/ silently rendered with NO samples while 14.8 GB sat on
            # disk. Measured 2026-07-30 (examples/renderer_ab_fair.rs, 5 pieces):
            # enabling VCSL lifts the native arm's crest factor by ~2.9 dB and
            # closes 45% of its gap to FluidSynth, and brings its spectral
            # balance to within 0.004 of FluidSynth's. Losing that to a CWD is
            # an expensive silent failure.
            for vcsl_candidate in "$PWD/data/samples/vcsl" "$PWD/symthaea/data/samples/vcsl"; do
              if [[ -d "$vcsl_candidate" ]]; then
                export SYMTHAEA_VCSL_DIR="$vcsl_candidate"
                break
              fi
            done
            echo ""
        ...
        # Usage: nix develop .#voice-vocoder
        devShells.voice-vocoder = pkgs.mkShell {
          buildInputs = [ pythonVocoderEnv ];

          shellHook = ''
            export LD_LIBRARY_PATH="${pkgs.stdenv.cc.cc.lib}/lib:${pkgs.zlib}/lib:$LD_LIBRARY_PATH"
            echo ""
            echo "Symthaea Vocos neural-vocoder shell (nix-managed, no pip)"
            echo "  python3 -c 'from vocos import Vocos; print(Vocos.from_pretrained(\"charactr/vocos-mel-24khz\"))'"
            echo ""
          '';
        };
