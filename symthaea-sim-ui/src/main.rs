...
fn App() -> impl IntoView {
    ...
    let toggle_count = Memo::new(move |_| {
        let mut count = 0;
        if consciousness_gating.get() { count += 1; }
        if turchin_cycles.get() { count += 1; }
        if fep_immiseration.get() { count += 1; }
        if sacred_stillness.get() { count += 1; }
        if disasters_enabled.get() { count += 1; }
        if faction_enabled.get() { count += 1; }
        if education_enabled.get() { count += 1; }
        if migration_enabled.get() { count += 1; }
        if stoichiometry.get() { count += 1; }
        if epistemic_decay.get() { count += 1; }
        count
    });
