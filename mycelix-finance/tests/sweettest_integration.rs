...
mod cgc_tests {
    ...
    async fn test_get_or_create_allocation() {
        ...
        let allocation: AllocationStatus = conductor
            .call(&cell.zome("cgc"), "get_or_create_allocation", member_did.clone())
            .await;

        assert_eq!(allocation.member_did, member_did, "Member DID must match");
        assert_eq!(allocation.total_allocated, 10, "Monthly allocation must be 10 CGC");
        assert_eq!(allocation.remaining, 10, "New allocation should have 10 remaining");
    ...
    async fn test_gift_cgc_and_query() {
        ...
        // Set up two agents
        let app1 = conductor.setup_app("giver-app", &[dna.clone()]).await.unwrap();
        ...
        let _: AllocationStatus = conductor
            .call(&cell1.zome("cgc"), "get_or_create_allocation", giver_did.clone())
            .await;
        ...
        assert_eq!(gift.contribution_type, ContributionType::Education, "Type must be Education");
        ...
        assert_eq!(updated_allocation.remaining, 7, "Should have 7 remaining after gifting 3");
        ...
        assert!(!gifts_given.is_empty(), "Should have at least one gift given");
        assert_eq!(gifts_given[0].receiver_did, receiver_did, "Gift receiver must match");
    }
    ...
    async fn test_recognition_summary() {
        ...
        let summary: RecognitionSummary = conductor
            .call(&cell1.zome("cgc"), "get_recognition_summary", receiver_did.clone())
            .await;

        assert_eq!(summary.member_did, receiver_did, "Member DID must match");
        assert!(summary.total_received >= 5, "Should have received at least 5");
        assert!(summary.unique_givers >= 1, "Should have at least 1 unique giver");
    }
    ...
    async fn test_register_cultural_alias() {
        ...
        let aliases: Vec<CulturalAlias> = conductor
            .call(&cell.zome("cgc"), "get_dao_aliases", "did:mycelix:dao-123".to_string())
            .await;
...
mod credit_scoring_tests {
    ...
    async fn test_create_and_get_credit_profile() {
        ...
        let profile: CreditProfile = decode_entry(&profile_record)
            .expect("Failed to decode credit profile");

        assert_eq!(profile.did, "did:mycelix:borrower-001", "DID must match");
        assert_eq!(profile.matl_score, 0.85, "MATL score must match");
        assert!(profile.computed_score > 0.0, "Computed score should be positive");
...
mod lending_tests {
    ...
    async fn test_request_loan() {
        ...
        assert_eq!(loan.borrower_did, "did:mycelix:borrower-001", "Borrower DID must match");
        ...
        assert_eq!(loan.status, LoanStatus::Requested, "Status must be Requested");
    ...
    async fn test_create_loan_offer() {
        ...
        assert!(offer_record.entry().as_option().is_some(), "Offer record must have entry");
    ...
    async fn test_loan_lifecycle_request_fund_repay() {
        ...
        assert_eq!(loan.status, LoanStatus::Requested, "Loan should be Requested");
        ...
        assert_eq!(funded_loan.status, LoanStatus::Funded, "Loan should be Funded");
        assert_eq!(funded_loan.lender_did, "did:mycelix:lender-lc", "Lender should be set");
        ...
        assert_eq!(repaid_loan.status, LoanStatus::Repaid, "Loan should be Repaid");
        assert!(repaid_loan.repaid.is_some(), "Repaid timestamp should be set");
    }
}
...
mod hearth_tests {
    ...
    async fn test_create_pool() {
        ...
        assert_eq!(pool.name, "Community Emergency Fund", "Pool name must match");
        assert_eq!(pool.dao_did, "did:mycelix:dao-test", "DAO DID must match");
        assert_eq!(pool.resource_type, ResourceType::Money, "Resource type must match");
        assert_eq!(pool.status, PoolStatus::Proposed, "New pool should be Proposed");
        assert_eq!(pool.total_resources, 0.0, "New pool should have no resources");

        // Get DAO pools
        let pools: Vec<PoolSummary> = conductor
            .call(&cell.zome("hearth"), "get_dao_pools", "did:mycelix:dao-test".to_string())
            .await;
    ...
    async fn test_contribute_to_pool() {
        ...
        let contributions: Vec<PoolContribution> = conductor
            .call(&cell.zome("hearth"), "get_pool_contributions", pool.id.clone())
            .await;

        assert!(!contributions.is_empty(), "Should have at least one contribution");
    }
    ...
    async fn test_request_allocation_and_vote() {
        ...
        assert_eq!(request.status, RequestStatus::Voting, "Status should be Voting");
        assert_eq!(request.need_category, NeedCategory::Healthcare, "Category must match");
...
mod tend_tests {
    ...
    async fn test_create_service_listing() {
        ...
        assert_eq!(listing.category, ServiceCategory::Technology, "Category must match");
