//! # HEARTH (Commons Pool) Integration Tests
//!
...
mod pool_creation {
    ...
    async fn test_create_pool() {
        ...
        assert!(matches!(pool.status, PoolStatus::Proposed), "Should start as Proposed");
        assert_eq!(pool.total_resources, 0.0, "Should start empty");

        println!("  - Pool created: {}", pool.name);
        println!("  - Status: {:?}", pool.status);
        println!("  - Governance threshold: {}%", pool.governance.allocation_threshold * 100.0);
    ...
    async fn test_pool_activation() {
        ...
        // Create pool
        let pool: CommonsPool = conductor
            .call(&alice_cell.zome("hearth"), "create_pool", CreatePoolInput {
        ...
        let _: PoolContribution = conductor
            .call(&alice_cell.zome("hearth"), "contribute", ContributeInput {
        ...
        // Second contribution
        let _: PoolContribution = conductor
            .call(&bob_cell.zome("hearth"), "contribute", ContributeInput {
                pool_id: pool_id.clone(),
        ...
        let _: PoolContribution = conductor
            .call(&charlie_cell.zome("hearth"), "contribute", ContributeInput {
                pool_id: pool_id.clone(),
        ...
        assert!(matches!(updated_pool.status, PoolStatus::Active), "Pool should be Active after 3 contributors");
        assert_eq!(updated_pool.contributor_count, 3, "Should have 3 contributors");
    ...
    async fn test_governance_validation() {
        ...
        let result: Result<CommonsPool, _> = conductor
            .call_fallible(&alice_cell.zome("hearth"), "create_pool", CreatePoolInput {
        ...
        match result {
            Err(e) => {
                ...
                assert!(
                    ...
                    "Should reject invalid threshold, got: {}", error_msg
                );
...
mod contribution_tests {
    ...
    async fn test_basic_contribution() {
        ...
        assert_eq!(contribution.contributor_did, alice_did, "Contributor mismatch");
    ...
    async fn test_invalid_contribution_amount() {
        ...
        let result: Result<PoolContribution, _> = conductor
            .call_fallible(&alice_cell.zome("hearth"), "contribute", ContributeInput {
                pool_id: pool.id.clone(),
                contributor_did: alice_did.clone(),
                amount: 0.0,
                message: None,
...
mod allocation_tests {
    ...
    async fn test_create_allocation_request() {
        ...
        for (cell, did) in [(&alice_cell, &alice_did), (&bob_cell, &bob_did), (&charlie_cell, &charlie_did)] {
            let _: PoolContribution = conductor
                .call(&cell.zome("hearth"), "contribute", ContributeInput {
                    pool_id: pool.id.clone(),
        ...
        let request: AllocationRequest = conductor
            .call(&dave_cell.zome("hearth"), "request_allocation", RequestAllocationInput {
                pool_id: pool.id.clone(),
        ...
        assert!(matches!(request.status, RequestStatus::Voting), "Should be in voting status");
    ...
    async fn test_max_allocation_percent() {
        ...
        let result: Result<AllocationRequest, _> = conductor
            .call_fallible(&dave_cell.zome("hearth"), "request_allocation", RequestAllocationInput {
                pool_id: pool.id.clone(),
                requester_did: dave_did.clone(),
                recipient_did: dave_did.clone(),
        ...
        match result {
            Err(e) => {
                let error_msg = format!("{:?}", e);
                assert!(
                    error_msg.contains("exceeds") || error_msg.contains("percent") || error_msg.contains("max"),
                    "Should reject exceeding max percent, got: {}", error_msg
                );
...
mod voting_tests {
    ...
    async fn test_cast_vote() {
        ...
        let vote: AllocationVote = conductor
            .call(&alice_cell.zome("hearth"), "vote_on_request", VoteInput {
...
mod need_category_tests {
    ...
    fn test_need_categories() {
        ...
        for category in &categories {
            ...
            let deserialized: NeedCategory = serde_json::from_str(&json).expect("Deserialize failed");
...
mod unit_tests {
    ...
    fn test_hearth_constants() {
        // Per Commons Charter
        assert_eq!(MIN_CONTRIBUTORS_TO_ACTIVATE, 3, "Min contributors should be 3");
        assert_eq!(MAX_ALLOCATION_PERCENT, 0.25, "Max allocation should be 25%");
        assert_eq!(DEFAULT_VOTING_PERIOD_HOURS, 72, "Default voting period should be 72 hours");
    }
    ...
    fn test_request_status_variants() {
        ...
        for status in statuses {
            let json = serde_json::to_string(&status).expect("Serialize failed");
            let deserialized: RequestStatus = serde_json::from_str(&json).expect("Deserialize failed");
            assert_eq!(status, deserialized, "Status round-trip failed");
        }
    }
    ...
    fn test_resource_types() {
        ...
        for resource_type in types {
            ...
            let deserialized: ResourceType = serde_json::from_str(&json).expect("Deserialize failed");
            assert_eq!(resource_type, deserialized, "Resource type round-trip failed");
        }
    }
    ...
    fn test_default_governance() {
        ...
        assert!(governance.allocation_threshold > 0.5, "Default threshold should be majority");
        assert!(governance.voting_period_hours > 0, "Should have voting period");
        assert!(governance.max_allocation_percent <= 0.5, "Max allocation should be reasonable");
    }
}
