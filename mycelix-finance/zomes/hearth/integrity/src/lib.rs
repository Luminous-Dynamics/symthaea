//! HEARTH (Commons Pool) Integrity Zome
//!
        ...
        FlatOp::StoreEntry(store_entry) => match store_entry {
            OpEntry::CreateEntry { app_entry, action } => {
                match app_entry {
                    ...
                    EntryTypes::PoolContribution(contribution) => {
                    ...
                    EntryTypes::Disbursement(disbursement) => {
                        validate_create_disbursement(EntryCreationAction::Create(action), disbursement)
                    }
            ...
            OpEntry::UpdateEntry { app_entry, action, .. } => {
                match app_entry {
                    EntryTypes::CommonsPool(pool) => {
                        validate_update_pool(action, pool)
                    }
...
fn validate_create_pool(
    ...
    if pool.name.is_empty() || pool.name.len() > 100 {
        return Ok(ValidateCallbackResult::Invalid("Pool name must be 1-100 chars".into()));
    }
    if pool.description.len() > 2000 {
        return Ok(ValidateCallbackResult::Invalid("Description too long (max 2000 chars)".into()));
    }
...
fn validate_create_contribution(
    ...
    if contribution.amount <= 0.0 {
        return Ok(ValidateCallbackResult::Invalid("Contribution must be positive".into()));
    }
    if let Some(msg) = &contribution.message {
        if msg.len() > 500 {
            return Ok(ValidateCallbackResult::Invalid("Message too long (max 500 chars)".into()));
        }
    }
    Ok(ValidateCallbackResult::Valid)
}
...
fn validate_create_request(
    ...
    if request.amount <= 0.0 {
        return Ok(ValidateCallbackResult::Invalid("Request amount must be positive".into()));
    }
    if request.justification.is_empty() {
        return Ok(ValidateCallbackResult::Invalid("Justification is required".into()));
    }
    if request.justification.len() > 2000 {
        return Ok(ValidateCallbackResult::Invalid("Justification too long (max 2000 chars)".into()));
    }
    Ok(ValidateCallbackResult::Valid)
}
...
fn validate_create_vote(
    ...
    if !vote.voter_did.starts_with("did:") {
        return Ok(ValidateCallbackResult::Invalid("Voter must be a valid DID".into()));
    }
    if vote.weight < 0.0 {
        return Ok(ValidateCallbackResult::Invalid("Vote weight cannot be negative".into()));
    }
    if let Some(comment) = &vote.comment {
        if comment.len() > 500 {
            return Ok(ValidateCallbackResult::Invalid("Comment too long (max 500 chars)".into()));
        }
    }
    Ok(ValidateCallbackResult::Valid)
}
...
fn validate_create_disbursement(
    ...
    if disbursement.amount <= 0.0 {
        return Ok(ValidateCallbackResult::Invalid("Disbursement must be positive".into()));
    }
    Ok(ValidateCallbackResult::Valid)
}
