//! HEARTH (Commons Pool) Coordinator Zome
//!
...
pub fn create_pool(input: CreatePoolInput) -> ExternResult<PoolSummary> {
    ...
    if input.name.is_empty() || input.name.len() > 256 {
        return Err(wasm_error!(WasmErrorInner::Guest("Pool name must be 1-256 characters".into())));
    }
    if input.description.is_empty() || input.description.len() > 4096 {
        return Err(wasm_error!(WasmErrorInner::Guest("Pool description must be 1-4096 characters".into())));
    }
    ...
    if let Some(ref alias) = input.cultural_alias {
        if alias.is_empty() || alias.len() > 256 {
            return Err(wasm_error!(WasmErrorInner::Guest("Cultural alias must be 1-256 characters".into())));
        }
    }
    if let Some(ref gov) = input.governance {
        if let Some(threshold) = gov.allocation_threshold {
            if !(0.0..=1.0).contains(&threshold) {
                return Err(wasm_error!(WasmErrorInner::Guest("Allocation threshold must be between 0.0 and 1.0".into())));
            }
        }
        if let Some(percent) = gov.max_allocation_percent {
            if !(0.0..=1.0).contains(&percent) {
                return Err(wasm_error!(WasmErrorInner::Guest("Max allocation percent must be between 0.0 and 1.0".into())));
            }
        }
    }
    ...
    let governance = if let Some(gov) = input.governance {
        PoolGovernance {
            ...
            voting_period_hours: gov.voting_period_hours.unwrap_or(DEFAULT_VOTING_PERIOD_HOURS),
...
pub fn get_pool(_pool_id: String) -> ExternResult<Option<CommonsPool>> {
    if _pool_id.is_empty() || _pool_id.len() > 256 {
        return Err(wasm_error!(WasmErrorInner::Guest("Pool ID must be 1-256 characters".into())));
    }
...
pub fn contribute(input: ContributeInput) -> ExternResult<PoolContribution> {
    ...
    if input.amount <= 0.0 || !input.amount.is_finite() {
        return Err(wasm_error!(WasmErrorInner::Guest("Amount must be a positive finite number".into())));
    }
    if let Some(ref message) = input.message {
        if message.len() > 4096 {
            return Err(wasm_error!(WasmErrorInner::Guest("Message must not exceed 4096 characters".into())));
        }
    }
    ...
    // Create contribution
    let contribution_id = format!("contrib:{}:{}:{}", input.pool_id, contributor_did, now.as_micros());
...
pub fn get_pool_contributions(pool_id: String) -> ExternResult<Vec<PoolContribution>> {
    ...
    for link in links {
        if let Some(record) = get(link.target.into_action_hash().ok_or_else(|| wasm_error!(WasmErrorInner::Guest("Invalid link target".to_string())))?, GetOptions::default())? {
            if let Some(contrib) = record.entry().to_app_option::<PoolContribution>().ok().flatten() {
...
pub fn request_allocation(input: RequestAllocationInput) -> ExternResult<RequestSummary> {
    ...
    if input.justification.is_empty() || input.justification.len() > 4096 {
        return Err(wasm_error!(WasmErrorInner::Guest("Justification must be 1-4096 characters".into())));
    }
    if let Some(ref recipient) = input.recipient_did {
        if recipient.is_empty() || recipient.len() > 256 {
            return Err(wasm_error!(WasmErrorInner::Guest("Recipient DID must be 1-256 characters".into())));
        }
    }
    ...
    let request_id = format!("request:{}:{}:{}", input.pool_id, requester_did, now.as_micros());
...
pub fn get_pool_requests(pool_id: String) -> ExternResult<Vec<RequestSummary>> {
    ...
    for link in links {
        if let Some(record) = get(link.target.into_action_hash().ok_or_else(|| wasm_error!(WasmErrorInner::Guest("Invalid link target".to_string())))?, GetOptions::default())? {
            if let Some(request) = record.entry().to_app_option::<AllocationRequest>().ok().flatten() {
...
pub fn get_request_votes(request_id: String) -> ExternResult<Vec<AllocationVote>> {
    ...
    for link in links {
        if let Some(record) = get(link.target.into_action_hash().ok_or_else(|| wasm_error!(WasmErrorInner::Guest("Invalid link target".to_string())))?, GetOptions::default())? {
            if let Some(vote) = record.entry().to_app_option::<AllocationVote>().ok().flatten() {
                votes.push(vote);
            }
        }
    }
...
pub fn finalize_request(_request_id: String) -> ExternResult<RequestSummary> {
    ...
    Err(wasm_error!(WasmErrorInner::Guest("Not yet implemented".into())))
}
