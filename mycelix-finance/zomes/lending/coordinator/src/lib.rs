...
use mycelix_finance_shared::batch::{links_to_records, filter_records_by};
...
pub fn request_loan(input: RequestLoanInput) -> ExternResult<Record> {
    if input.borrower_did.is_empty() || input.borrower_did.len() > 256 {
        return Err(wasm_error!(WasmErrorInner::Guest("Borrower DID must be 1-256 characters".into())));
    }
    if input.amount <= 0.0 {
        return Err(wasm_error!(WasmErrorInner::Guest("Amount must be greater than 0".into())));
    }
    if input.currency.is_empty() || input.currency.len() > 10 {
        return Err(wasm_error!(WasmErrorInner::Guest("Currency must be 1-10 characters".into())));
    }
    if input.term_days == 0 {
        return Err(wasm_error!(WasmErrorInner::Guest("Term must be at least 1 day".into())));
    }
    let now = sys_time()?;
    let loan = Loan {
    ...
    create_link(anchor_hash(&input.borrower_did)?, action_hash.clone(), LinkTypes::BorrowerToLoans, ())?;
...
pub fn create_loan_offer(input: CreateOfferInput) -> ExternResult<Record> {
    if input.lender_did.is_empty() || input.lender_did.len() > 256 {
        return Err(wasm_error!(WasmErrorInner::Guest("Lender DID must be 1-256 characters".into())));
    }
    if input.max_amount <= 0.0 || input.min_amount <= 0.0 {
        return Err(wasm_error!(WasmErrorInner::Guest("Amounts must be greater than 0".into())));
    }
    if input.min_amount > input.max_amount {
        return Err(wasm_error!(WasmErrorInner::Guest("min_amount must be <= max_amount".into())));
    }
    if input.currency.is_empty() || input.currency.len() > 10 {
        return Err(wasm_error!(WasmErrorInner::Guest("Currency must be 1-10 characters".into())));
    }
    if input.base_interest_rate < 0.0 || input.base_interest_rate > 1.0 {
        return Err(wasm_error!(WasmErrorInner::Guest("Interest rate must be between 0.0 and 1.0".into())));
    }
    if input.max_term_days == 0 {
        return Err(wasm_error!(WasmErrorInner::Guest("Max term must be at least 1 day".into())));
    }
    let now = sys_time()?;
    let offer = LoanOffer {
    ...
    create_link(anchor_hash(&input.lender_did)?, action_hash.clone(), LinkTypes::LenderToOffers, ())?;
...
pub fn fund_loan(input: FundLoanInput) -> ExternResult<Record> {
    if input.loan_id.is_empty() || input.loan_id.len() > 256 {
        return Err(wasm_error!(WasmErrorInner::Guest("Loan ID must be 1-256 characters".into())));
    }
    if input.lender_did.is_empty() || input.lender_did.len() > 256 {
        return Err(wasm_error!(WasmErrorInner::Guest("Lender DID must be 1-256 characters".into())));
    }
    ...
    let filter = ChainQueryFilter::new().entry_type(EntryType::App(AppEntryDef::try_from(UnitEntryTypes::Loan)?)).include_entries(true);
    for record in query(filter)? {
        if let Some(loan) = record.entry().to_app_option::<Loan>().ok().flatten() {
            if loan.id == input.loan_id {
                let now = sys_time()?;
                let maturity = Timestamp::from_micros(now.as_micros() as i64 + (loan.term_days as i64 * 24 * 3600 * 1_000_000));
                ...
                let action_hash = update_entry(record.action_address().clone(), &EntryTypes::Loan(funded_loan))?;
                create_link(anchor_hash(&input.lender_did)?, action_hash.clone(), LinkTypes::LenderToLoans, ())?;
...
pub fn repay_loan(loan_id: String) -> ExternResult<Record> {
    ...
    for record in query(filter)? {
        if let Some(loan) = record.entry().to_app_option::<Loan>().ok().flatten() {
            if loan.id == loan_id {
                let now = sys_time()?;
                let repaid_loan = Loan { status: LoanStatus::Repaid, repaid: Some(now), ..loan };
                let action_hash = update_entry(record.action_address().clone(), &EntryTypes::Loan(repaid_loan))?;
                return get(action_hash, GetOptions::default())?.ok_or(wasm_error!(WasmErrorInner::Guest("Not found".into())));
            }
        }
    }
...
pub fn get_active_offers(_: ()) -> ExternResult<Vec<Record>> {
    ...
    Ok(filter_records_by::<LoanOffer, _>(&all_records, |offer| offer.active))
}
        ...
        if let Some(loan) = record.entry().to_app_option::<Loan>().ok().flatten() {
            if loan.id == loan_id {
                if loan.status != LoanStatus::Active {
                    return Err(wasm_error!(WasmErrorInner::Guest("Can only default active loans".into())));
                }
                let defaulted = Loan {
                    status: LoanStatus::Defaulted,
                    ..loan
                };
                let action_hash = update_entry(record.action_address().clone(), &EntryTypes::Loan(defaulted))?;
                return get(action_hash, GetOptions::default())?.ok_or(wasm_error!(WasmErrorInner::Guest("Not found".into())));
            }
        }
    }
        ...
        if let Some(loan) = record.entry().to_app_option::<Loan>().ok().flatten() {
            if loan.id == loan_id {
                if loan.status != LoanStatus::Requested {
                    return Err(wasm_error!(WasmErrorInner::Guest("Can only cancel requested loans".into())));
                }
                let cancelled = Loan {
                    status: LoanStatus::Cancelled,
                    ..loan
                };
                let action_hash = update_entry(record.action_address().clone(), &EntryTypes::Loan(cancelled))?;
                return get(action_hash, GetOptions::default())?.ok_or(wasm_error!(WasmErrorInner::Guest("Not found".into())));
            }
        }
    }
...
pub fn deactivate_offer(offer_id: String) -> ExternResult<Record> {
    if offer_id.is_empty() || offer_id.len() > 256 {
        return Err(wasm_error!(WasmErrorInner::Guest("Offer ID must be 1-256 characters".into())));
    }
    let filter = ChainQueryFilter::new()
        .entry_type(EntryType::App(AppEntryDef::try_from(UnitEntryTypes::LoanOffer)?))
        .include_entries(true);
    ...
    for record in query(filter)? {
        if let Some(offer) = record.entry().to_app_option::<LoanOffer>().ok().flatten() {
            if offer.id == offer_id {
                ...
                let action_hash = update_entry(record.action_address().clone(), &EntryTypes::LoanOffer(deactivated))?;
                return get(action_hash, GetOptions::default())?.ok_or(wasm_error!(WasmErrorInner::Guest("Not found".into())));
            }
        }
    }
...
pub fn create_payment_schedule(input: CreateScheduleInput) -> ExternResult<Record> {
    ...
    if input.num_payments == 0 {
        return Err(wasm_error!(WasmErrorInner::Guest("Number of payments must be at least 1".into())));
    }
    // Find the loan
    let loan = get_loan(input.loan_id.clone())?
        .ok_or(wasm_error!(WasmErrorInner::Guest("Loan not found".into())))?;

    let loan_data = loan.entry().to_app_option::<Loan>().ok().flatten()
        .ok_or(wasm_error!(WasmErrorInner::Guest("Invalid loan data".into())))?;

    if loan_data.funded.is_none() {
        return Err(wasm_error!(WasmErrorInner::Guest("Loan not yet funded".into())));
    }

    let funded_time = loan_data.funded.ok_or_else(|| wasm_error!(WasmErrorInner::Guest(
        "Loan must be funded before generating payment schedule".to_string()
    )))?.as_micros() as i64;
    let payment_interval = (loan_data.term_days as i64 * 24 * 3600 * 1_000_000) / input.num_payments as i64;
    ...
    let payments: Vec<ScheduledPayment> = (1..=input.num_payments).map(|i| {
        ScheduledPayment {
            payment_number: i,
    ...
    create_link(anchor_hash(&input.loan_id)?, action_hash.clone(), LinkTypes::LoanToSchedule, ())?;
    get(action_hash, GetOptions::default())?.ok_or(wasm_error!(WasmErrorInner::Guest("Not found".into())))
}
...
#[hdk_extern]
pub fn calculate_credit_assessment(input: CreditAssessmentInput) -> ExternResult<CreditAssessmentResult> {
    if input.borrower_did.is_empty() || input.borrower_did.len() > 256 {
        return Err(wasm_error!(WasmErrorInner::Guest("Borrower DID must be 1-256 characters".into())));
    }
    let now = sys_time()?;
    ...
    let (payment_history, payment_details) = calculate_payment_history_with_decay_from_loans(
        &loans,
        now,
    )?;

    // 2. Collateral - verifiable assets pledged (30%)
    let (collateral_ratio, collateral_details) = calculate_collateral_with_transparency_from_loans(
        &loans,
    )?;

    // 3. Account age in lending - time since first loan (20%)
    let (account_age, age_details) = calculate_lending_age_with_decay_from_loans(
        &loans,
        now,
    )?;

    // Weighted sum - simpler, more transparent formula
    let base_score = payment_history * 0.50
...
fn calculate_payment_history_with_decay_from_loans(
    ...
    for record in loans {
        if let Some(loan) = record.entry().to_app_option::<Loan>().ok().flatten() {
            ...
            let age_days = (now.as_micros() as i64 - event_time.as_micros() as i64)
                / (24 * 3600 * 1_000_000);
...
fn calculate_collateral_with_transparency_from_loans(
    ...
    let details = format!(
        ...
        total_collateral_loans, total_loans, collateral_ratio * 100.0
    );
...
fn calculate_lending_age_with_decay_from_loans(
    ...
    if let Some(oldest) = oldest_created {
        ...
        let details = format!(
            "Account age: {} days (max credit at 730 days)",
            age_days
        );
...
pub fn request_fresh_start(borrower_did: String) -> ExternResult<FreshStartRequest> {
    ...
    let effective_at = Timestamp::from_micros(
        now.as_micros() as i64 + 90 * 24 * 3600 * 1_000_000
    );
...
pub fn get_adjusted_interest_rate(input: GetRateInput) -> ExternResult<AdjustedRateResult> {
    ...
    if input.base_rate < 0.0 || input.base_rate > 1.0 {
        return Err(wasm_error!(WasmErrorInner::Guest("Base rate must be between 0.0 and 1.0".into())));
    }
...
pub fn generate_threshold_proof(input: ThresholdProofInput) -> ExternResult<PrivacyCreditProof> {
    ...
    if input.threshold < 0.0 || input.threshold > 1.0 {
        return Err(wasm_error!(WasmErrorInner::Guest("Threshold must be between 0.0 and 1.0".into())));
    }
    let now = sys_time()?;
    ...
    let proof_data = format!(
        ...
        input.proof_type, input.threshold, result, now.as_micros()
    );
    ...
    let (reveals, conceals) = match &input.proof_type {
        ProofType::MeetsMinimum => (
            vec![
                format!("Score {} threshold {}", if result { ">=" } else { "<" }, input.threshold),
            ],
            vec![
        ...
        ProofType::WithinRange { low, high } => (
            vec![
                format!("Score {} in range [{}, {}]", if result { "is" } else { "is not" }, low, high),
            ],
            vec![
                "Exact credit score".to_string(),
        ...
        ProofType::TierMembership => (
            vec![
                format!("Tier membership: {}", if result { "confirmed" } else { "not in tier" }),
            ],
            vec![
                "Exact credit score".to_string(),
...
pub fn verify_threshold_proof(proof: PrivacyCreditProof) -> ExternResult<ProofVerificationResult> {
    ...
    Ok(ProofVerificationResult {
        proof_id: proof.proof_id,
        is_valid,
        reason: if is_valid { "Proof verified successfully".to_string() } else { "Invalid proof format".to_string() },
        verified_at: now,
    })
}
