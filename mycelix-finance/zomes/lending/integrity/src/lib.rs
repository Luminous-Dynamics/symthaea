    ...
    match op.flattened::<EntryTypes, LinkTypes>()? {
        FlatOp::StoreEntry(store_entry) => match store_entry {
            ...
            OpEntry::UpdateEntry { app_entry, action, .. } => {
                match app_entry {
                    EntryTypes::Loan(loan) => {
                        validate_update_loan(action, loan)
                    }
                    EntryTypes::LoanOffer(offer) => {
                        validate_update_loan_offer(action, offer)
                    }
                    EntryTypes::PaymentSchedule(_) => {
        ...
        FlatOp::RegisterCreateLink { link_type, base_address, target_address, .. } => {
            ...
            match link_type {
                LinkTypes::BorrowerToLoans | LinkTypes::LenderToLoans | LinkTypes::LenderToOffers => {
...
fn validate_create_loan(
    ...
    if !loan.borrower_did.starts_with("did:") {
        return Ok(ValidateCallbackResult::Invalid("Borrower must be a valid DID".into()));
    }
    if !loan.lender_did.starts_with("did:") {
        return Ok(ValidateCallbackResult::Invalid("Lender must be a valid DID".into()));
    }
    if loan.principal <= 0.0 {
        return Ok(ValidateCallbackResult::Invalid("Principal must be positive".into()));
    }
    if loan.interest_rate < 0.0 || loan.interest_rate > 1.0 {
        return Ok(ValidateCallbackResult::Invalid("Interest rate must be between 0 and 100%".into()));
    }
    if loan.borrower_did == loan.lender_did {
        return Ok(ValidateCallbackResult::Invalid("Cannot lend to yourself".into()));
    }
    Ok(ValidateCallbackResult::Valid)
}
...
fn validate_create_loan_offer(
    ...
    if offer.min_amount > offer.max_amount {
        return Ok(ValidateCallbackResult::Invalid("Min amount cannot exceed max amount".into()));
    }
    if offer.base_interest_rate < 0.0 {
        return Ok(ValidateCallbackResult::Invalid("Interest rate cannot be negative".into()));
    }
    Ok(ValidateCallbackResult::Valid)
}
...
fn validate_create_payment_schedule(
    ...
    if schedule.payments.is_empty() {
        return Ok(ValidateCallbackResult::Invalid("Schedule must have at least one payment".into()));
    }
    Ok(ValidateCallbackResult::Valid)
}
