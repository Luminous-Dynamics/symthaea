...
fn update_treasury_balance(treasury_id: &str, delta: f64) -> ExternResult<()> {
    let filter = ChainQueryFilter::new().entry_type(EntryType::App(AppEntryDef::try_from(UnitEntryTypes::Treasury)?)).include_entries(true);
    for record in query(filter)? {
        if let Some(treasury) = record.entry().to_app_option::<Treasury>().ok().flatten() {
            if treasury.id == treasury_id {
                let now = sys_time()?;
                let updated = Treasury { balance: treasury.balance + delta, last_updated: now, ..treasury };
...
pub fn execute_allocation(allocation_id: String) -> ExternResult<Record> {
    let filter = ChainQueryFilter::new().entry_type(EntryType::App(AppEntryDef::try_from(UnitEntryTypes::Allocation)?)).include_entries(true);
    for record in query(filter)? {
    ...
    Err(wasm_error!(WasmErrorInner::Guest("Allocation not found or not approved".into())))
}
...
pub fn get_treasury_contributions(treasury_id: String) -> ExternResult<Vec<Record>> {
    ...
    let query = LinkQuery::try_new(anchor_hash(&treasury_id)?, LinkTypes::TreasuryToContributions)?;
...
pub fn add_manager(input: AddManagerInput) -> ExternResult<Record> {
    let filter = ChainQueryFilter::new()
        .entry_type(EntryType::App(AppEntryDef::try_from(UnitEntryTypes::Treasury)?))
        .include_entries(true);
...
pub fn get_contributor_history(contributor_did: String) -> ExternResult<Vec<Record>> {
    let mut contributions = Vec::new();
    let query = LinkQuery::try_new(anchor_hash(&contributor_did)?, LinkTypes::ContributorToContributions)?;
    for link in get_links(query, GetStrategy::default())? {
        if let Some(record) = get(ActionHash::try_from(link.target).map_err(|_| wasm_error!(WasmErrorInner::Guest("Invalid".into())))?, GetOptions::default())? {
            contributions.push(record);
        }
