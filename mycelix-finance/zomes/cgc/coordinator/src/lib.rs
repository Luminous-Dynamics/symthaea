//! CGC (Civic Gifting Credits) Coordinator Zome
//!
...
pub fn gift_cgc(input: GiftCgcInput) -> ExternResult<GiftRecord> {
    ...
    if input.gratitude_message.is_empty() || input.gratitude_message.len() > 2048 {
        return Err(wasm_error!(WasmErrorInner::Guest("Gratitude message must be 1-2048 characters".into())));
    }
    ...
    // Create the gift
    let gift_id = format!("gift:{}:{}:{}", giver_did, input.receiver_did, now.as_micros());
    let gift = CgcGift {
        id: gift_id.clone(),
...
pub fn register_cultural_alias(input: RegisterAliasInput) -> ExternResult<CulturalAlias> {
    if input.alias.is_empty() || input.alias.len() > 256 {
        return Err(wasm_error!(WasmErrorInner::Guest("Alias must be 1-256 characters".into())));
    }
    ...
    if input.meaning.is_empty() || input.meaning.len() > 2048 {
        return Err(wasm_error!(WasmErrorInner::Guest("Meaning must be 1-2048 characters".into())));
    }
    let now = sys_time()?;
...
pub fn get_dao_aliases(dao_did: String) -> ExternResult<Vec<CulturalAlias>> {
    ...
    for link in links {
        if let Some(record) = get(link.target.into_action_hash().ok_or_else(|| wasm_error!(WasmErrorInner::Guest("Invalid link target".to_string())))?, GetOptions::default())? {
            if let Some(alias) = record.entry().to_app_option::<CulturalAlias>().ok().flatten() {
                aliases.push(alias);
            }
        }
    }
...
fn get_cycle_end_timestamp(cycle_id: &str) -> ExternResult<Timestamp> {
    ...
    if parts.len() != 2 {
        return Err(wasm_error!(WasmErrorInner::Guest("Invalid cycle ID".into())));
    }

    let year: i64 = parts[0].parse().map_err(|_| {
        wasm_error!(WasmErrorInner::Guest("Invalid year".into()))
    })?;
    let month: i64 = parts[1].parse().map_err(|_| {
        wasm_error!(WasmErrorInner::Guest("Invalid month".into()))
    })?;
...
fn find_allocation(member_did: &str, cycle_id: &str) -> ExternResult<Option<CgcAllocation>> {
    ...
    for link in links {
        if let Some(record) = get(link.target.into_action_hash().ok_or_else(|| wasm_error!(WasmErrorInner::Guest("Invalid link target".to_string())))?, GetOptions::default())? {
            if let Some(allocation) = record.entry().to_app_option::<CgcAllocation>().ok().flatten() {
...
fn update_allocation_after_gift(
    member_did: &str,
    cycle_id: &str,
    amount: u32,
) -> ExternResult<()> {
    ...
    for link in links {
        let action_hash = link.target.into_action_hash().ok_or_else(|| wasm_error!(WasmErrorInner::Guest("Invalid link target".to_string())))?;
        if let Some(record) = get(action_hash.clone(), GetOptions::default())? {
            if let Some(mut allocation) = record.entry().to_app_option::<CgcAllocation>().ok().flatten() {
                if allocation.cycle_id == cycle_id {
...
fn update_recognition_after_gift(
    ...
    for link in links {
        let action_hash = link.target.into_action_hash().ok_or_else(|| wasm_error!(WasmErrorInner::Guest("Invalid link target".to_string())))?;
        if let Some(record) = get(action_hash.clone(), GetOptions::default())? {
            if let Some(mut recognition) = record.entry().to_app_option::<CgcRecognition>().ok().flatten() {
