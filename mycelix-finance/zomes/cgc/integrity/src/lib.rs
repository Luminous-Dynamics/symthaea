//! CGC (Civic Gifting Credits) Integrity Zome
//!
        ...
        FlatOp::StoreEntry(store_entry) => match store_entry {
            OpEntry::CreateEntry { app_entry, action } => {
                match app_entry {
                    ...
                    EntryTypes::CgcRecognition(recognition) => {
                        validate_create_recognition(EntryCreationAction::Create(action), recognition)
                    }
            ...
            OpEntry::UpdateEntry { app_entry, action, .. } => {
                match app_entry {
                    ...
                    EntryTypes::CgcHighActivityFlag(flag) => {
                        validate_update_flag(action, flag)
                    }
                    EntryTypes::CulturalAlias(_) => {
                        Ok(ValidateCallbackResult::Invalid(
...
fn validate_create_gift(
    ...
    if !gift.giver_did.starts_with("did:") {
        return Ok(ValidateCallbackResult::Invalid("Giver must be a valid DID".into()));
    }
    ...
    if gift.amount == 0 {
        return Ok(ValidateCallbackResult::Invalid("Gift amount must be positive".into()));
    }
    if gift.amount > MONTHLY_CGC_ALLOCATION {
        return Ok(ValidateCallbackResult::Invalid(
            format!("Gift amount exceeds monthly allocation of {}", MONTHLY_CGC_ALLOCATION),
        ));
    }
