//! Credit Scoring Coordinator Zome
//!
//! # DEPRECATION NOTICE
//!
...
pub fn create_credit_profile(input: CreateProfileInput) -> ExternResult<Record> {
    ...
    if input.matl_score < 0.0 || input.matl_score > 1.0 {
        return Err(wasm_error!(WasmErrorInner::Guest("MATL score must be between 0.0 and 1.0".into())));
    }
    if input.collateral_ratio < 0.0 || input.collateral_ratio > 1.0 {
        return Err(wasm_error!(WasmErrorInner::Guest("Collateral ratio must be between 0.0 and 1.0".into())));
    }
    let now = sys_time()?;
    ...
    create_link(anchor_hash(&input.did)?, action_hash.clone(), LinkTypes::DidToProfile, ())?;
...
fn compute_credit_score(matl: f64, payment_history: f64, collateral: f64, age_days: u64, activity: f64) -> f64 {
    let age_factor = (age_days as f64 / 365.0).min(1.0);
    let raw = (matl * 0.40) + (payment_history * 0.30) + (collateral * 0.15) + (age_factor * 0.10) + (activity * 0.05);
    (raw * 1000.0).round()
}
...
pub fn get_credit_profile(did: String) -> ExternResult<Option<Record>> {
    ...
    if let Some(link) = links.first() {
        let action_hash = ActionHash::try_from(link.target.clone()).map_err(|_| wasm_error!(WasmErrorInner::Guest("Invalid".into())))?;
...
pub fn record_payment(input: RecordPaymentInput) -> ExternResult<Record> {
    if input.profile_did.is_empty() || input.profile_did.len() > 256 {
        return Err(wasm_error!(WasmErrorInner::Guest("Profile DID must be 1-256 characters".into())));
    }
    ...
    create_link(anchor_hash(&input.profile_did)?, action_hash.clone(), LinkTypes::ProfileToPayments, ())?;
    create_link(anchor_hash(&input.loan_id)?, action_hash.clone(), LinkTypes::LoanToPayments, ())?;
    get(action_hash, GetOptions::default())?.ok_or(wasm_error!(WasmErrorInner::Guest("Not found".into())))
}
...
pub fn register_collateral(input: RegisterCollateralInput) -> ExternResult<Record> {
    ...
    if input.asset_id.is_empty() || input.asset_id.len() > 256 {
        return Err(wasm_error!(WasmErrorInner::Guest("Asset ID must be 1-256 characters".into())));
    }
    if input.appraised_value <= 0.0 {
        return Err(wasm_error!(WasmErrorInner::Guest("Appraised value must be greater than 0".into())));
    }
    if input.currency.is_empty() || input.currency.len() > 10 {
        return Err(wasm_error!(WasmErrorInner::Guest("Currency must be 1-10 characters".into())));
    }
    ...
    create_link(anchor_hash(&input.owner_did)?, action_hash.clone(), LinkTypes::ProfileToCollateral, ())?;
    get(action_hash, GetOptions::default())?.ok_or(wasm_error!(WasmErrorInner::Guest("Not found".into())))
}
...
pub fn update_credit_score(did: String) -> ExternResult<Option<Record>> {
    ...
    let total_collateral: f64 = collateral_records.iter().filter_map(|r| {
        r.entry().to_app_option::<CollateralRecord>().ok().flatten().map(|c| c.appraised_value)
    }).sum();

    // Update the profile
    let filter = ChainQueryFilter::new()
        .entry_type(EntryType::App(AppEntryDef::try_from(UnitEntryTypes::CreditProfile)?))
        .include_entries(true);

    for record in query(filter)? {
        if let Some(profile) = record.entry().to_app_option::<CreditProfile>().ok().flatten() {
            if profile.did == did {
                let now = sys_time()?;
                ...
                let action_hash = update_entry(record.action_address().clone(), &EntryTypes::CreditProfile(updated))?;
                return get(action_hash, GetOptions::default());
            }
        }
    }
...
pub fn lock_collateral(input: LockCollateralInput) -> ExternResult<Record> {
    if input.collateral_id.is_empty() || input.collateral_id.len() > 256 {
        return Err(wasm_error!(WasmErrorInner::Guest("Collateral ID must be 1-256 characters".into())));
    }
    if input.loan_id.is_empty() || input.loan_id.len() > 256 {
        return Err(wasm_error!(WasmErrorInner::Guest("Loan ID must be 1-256 characters".into())));
    }
    let filter = ChainQueryFilter::new()
        .entry_type(EntryType::App(AppEntryDef::try_from(UnitEntryTypes::CollateralRecord)?))
        .include_entries(true);

    for record in query(filter)? {
        if let Some(collateral) = record.entry().to_app_option::<CollateralRecord>().ok().flatten() {
            if collateral.id == input.collateral_id {
                if collateral.locked_for_loan.is_some() {
                    return Err(wasm_error!(WasmErrorInner::Guest("Collateral already locked".into())));
                }
                let updated = CollateralRecord {
                    locked_for_loan: Some(input.loan_id),
                    ..collateral
                };
                let action_hash = update_entry(record.action_address().clone(), &EntryTypes::CollateralRecord(updated))?;
                return get(action_hash, GetOptions::default())?.ok_or(wasm_error!(WasmErrorInner::Guest("Not found".into())));
            }
        }
    }
    Err(wasm_error!(WasmErrorInner::Guest("Collateral not found".into())))
}
...
pub fn get_profiles_by_score_range(input: ScoreRangeInput) -> ExternResult<Vec<Record>> {
    if input.min_score > input.max_score {
        return Err(wasm_error!(WasmErrorInner::Guest("min_score must be <= max_score".into())));
    }
    let filter = ChainQueryFilter::new()
        .entry_type(EntryType::App(AppEntryDef::try_from(UnitEntryTypes::CreditProfile)?))
        .include_entries(true);
    ...
    for record in query(filter)? {
        if let Some(profile) = record.entry().to_app_option::<CreditProfile>().ok().flatten() {
            if profile.computed_score >= input.min_score && profile.computed_score <= input.max_score {
                results.push(record);
            }
        }
    }
