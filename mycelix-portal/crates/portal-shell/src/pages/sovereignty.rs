...
use leptos::prelude::*;
use portal_domain_trait::{DomainModule, DataSensitivity};
...
pub fn SovereigntyDashboard() -> impl IntoView {
    ...
    let inventory: Vec<InventoryEntry> = registry.domains.iter().map(|d| {
        InventoryEntry {
            domain_id: d.id(),
            domain_name: d.name(),
            bio_name: d.bio_name(),
...
use portal_domain_trait::EntryTypeInfo;
...
fn FlowGraph(domains: Vec<FlowNode>, flows: Vec<FlowInfo>) -> impl IntoView {
    ...
    let positions: Vec<(f64, f64)> = (0..domains.len()).map(|i| {
