...
enum KnowView { Claims, FactCheck, Markets }
...
pub fn KnowledgeOverview() -> impl IntoView {
    ...
    let markets: RwSignal<Vec<PredictionMarket>> = RwSignal::new(vec![
        PredictionMarket { id: "pm-1".into(), question: "Will the water stewardship protocol reduce waste by 20% in Q2?".into(), resolution_date: 1719792000, yes_probability: 0.72, volume: 4500, resolved: None },
        PredictionMarket { id: "pm-2".into(), question: "Will all Block 7 solar panels be installed by June?".into(), resolution_date: 1717200000, yes_probability: 0.85, volume: 8200, resolved: None },
        PredictionMarket { id: "pm-3".into(), question: "Will community garden yield exceed 500kg this season?".into(), resolution_date: 1725148800, yes_probability: 0.61, volume: 2100, resolved: None },
    ]);

    fn status_color(s: &ClaimStatus) -> &'static str {
        match s { ClaimStatus::Verified => "#22c55e", ClaimStatus::UnderReview => "#f59e0b", ClaimStatus::Disputed => "#ef4444", ClaimStatus::Proposed => "#60a5fa", ClaimStatus::Retracted => "#6b7280" }
    }
