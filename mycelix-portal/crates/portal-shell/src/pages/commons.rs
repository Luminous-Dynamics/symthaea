...
enum CommonsView { Overview, Housing, MutualAid, Water, Food }
...
pub fn CommonsOverview() -> impl IntoView {
    ...
    let aid_requests: RwSignal<Vec<AidRequest>> = RwSignal::new(vec![
        AidRequest { id: "ar-1".into(), requester_did: "did:maria".into(), request_type: AidRequestType::Childcare, description: "Need childcare coverage Tuesday-Thursday while recovering from surgery.".into(), urgency: Urgency::High, location: Some("Block 3".into()), amount_needed: None, fulfilled_amount: 0, status: AidRequestStatus::Open, created_at: 1711900800 },
        AidRequest { id: "ar-2".into(), requester_did: "did:james".into(), request_type: AidRequestType::Food, description: "Family of 5, temporary food insecurity after job transition.".into(), urgency: Urgency::Medium, location: Some("Block 7".into()), amount_needed: Some(200), fulfilled_amount: 120, status: AidRequestStatus::PartiallyFulfilled, created_at: 1711814400 },
        AidRequest { id: "ar-3".into(), requester_did: "did:community".into(), request_type: AidRequestType::Transportation, description: "Van needed for elderly transport to health center (3x/week).".into(), urgency: Urgency::Medium, location: None, amount_needed: Some(500), fulfilled_amount: 500, status: AidRequestStatus::Fulfilled, created_at: 1711728000 },
    ]);

    fn urgency_color(u: &Urgency) -> &'static str {
        match u { Urgency::Critical => "#ef4444", Urgency::High => "#f59e0b", Urgency::Medium => "#60a5fa", Urgency::Low => "#6b7280" }
    }
    fn status_label(s: &AidRequestStatus) -> &'static str {
        match s { AidRequestStatus::Open => "OPEN", AidRequestStatus::PartiallyFulfilled => "PARTIAL", AidRequestStatus::Fulfilled => "FULFILLED", AidRequestStatus::Cancelled => "CANCELLED" }
    }
