...
enum LucidView { Thoughts, Graph, Collective, Reasoning }
...
fn type_color(t: &ThoughtType) -> &'static str {
    match t { ThoughtType::Claim => "#22c55e", ThoughtType::Hypothesis => "#60a5fa", ThoughtType::Critique => "#f59e0b", ThoughtType::Observation => "#8b5cf6", ThoughtType::Question => "#ec4899", ThoughtType::Reflection => "#06b6d4", ThoughtType::Synthesis => "#e8c547" }
}
fn type_label(t: &ThoughtType) -> &'static str {
    match t { ThoughtType::Claim => "Claim", ThoughtType::Hypothesis => "Hypothesis", ThoughtType::Critique => "Critique", ThoughtType::Observation => "Observation", ThoughtType::Question => "Question", ThoughtType::Reflection => "Reflection", ThoughtType::Synthesis => "Synthesis" }
}
fn rel_color(r: &RelationshipType) -> &'static str {
    match r { RelationshipType::Supports => "rgba(34, 197, 94, 0.4)", RelationshipType::Contradicts => "rgba(239, 68, 68, 0.4)", RelationshipType::Extends | RelationshipType::Refines => "rgba(96, 165, 250, 0.4)", RelationshipType::Inspires => "rgba(232, 197, 71, 0.3)", RelationshipType::QuestionsAssumptionOf => "rgba(236, 72, 153, 0.4)", RelationshipType::Synthesizes => "rgba(139, 92, 246, 0.4)", RelationshipType::Contextualizes => "rgba(6, 182, 212, 0.3)" }
}
...
pub fn LucidOverview() -> impl IntoView {
    ...
    wasm_bindgen_futures::spawn_local(async move {
        let (thoughts, rels) = if identity_for_load.conductor_status.get() == ConductorStatus::Connected {
            let t = identity_for_load.call_zome::<(), Vec<Thought>>("lucid", "lucid", "get_my_thoughts", &()).await.unwrap_or_else(|_| mock_thoughts());
            let r = identity_for_load.call_zome::<(), Vec<Relationship>>("lucid", "lucid", "get_my_relationships", &()).await.unwrap_or_else(|_| mock_relationships());
            (t, r)
        } else { (mock_thoughts(), mock_relationships()) };
...
fn ThoughtDetail(thought: Thought) -> impl IntoView {
    ...
    let t = thought;
    let tcolor = type_color(&t.thought_type);
    let related: Vec<_> = state.relationships.get().iter()
...
fn GraphView() -> impl IntoView {
    ...
    let graph_nodes = move || state.thoughts.get().iter().map(|t| {
        GraphNode {
            id: t.id.clone(),
            label: if t.content.len() > 25 { format!("{}...", &t.content[..25]) } else { t.content.clone() },
    ...
    let graph_edges = move || state.relationships.get().iter().map(|r| {
        GraphEdge {
...
fn CreateThoughtInline() -> impl IntoView {
    ...
    let on_submit = move |ev: web_sys::SubmitEvent| {
        ...
        let tags: Vec<String> = tags_input.get().split(',').map(|t| t.trim().to_string()).filter(|t| !t.is_empty()).collect();
        let content_val = c.trim().to_string();

        wasm_bindgen_futures::spawn_local(async move {
            let input = CreateThoughtInput {
                content: content_val.clone(),
                thought_type: match tt.as_str() { "Claim" => ThoughtType::Claim, "Hypothesis" => ThoughtType::Hypothesis, "Critique" => ThoughtType::Critique, "Question" => ThoughtType::Question, "Reflection" => ThoughtType::Reflection, "Synthesis" => ThoughtType::Synthesis, _ => ThoughtType::Observation },
                confidence: conf, tags: tags.clone(), domain: None,
                epistemic: EpistemicProfile { empirical: 0.5, normative: 0.3, materiality: 0.5, harmonic: 0.5 },
            };
            let _ = identity.call_zome::<CreateThoughtInput, serde_json::Value>("lucid", "lucid", "create_thought", &input).await;

            // Optimistic: add to state
            let new_thought = Thought {
                id: format!("t-{:03}", state.thoughts.get().len() + 6),
                content: content_val, thought_type: input.thought_type, confidence: conf,
                tags, domain: None, epistemic: input.epistemic,
                author_did: "did:mycelix:uhCAkYou".into(), created_at: 0, updated_at: 0,
            };
