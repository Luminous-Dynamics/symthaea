...
use portal_viz::{PieChart, LineChart, BarChart, pie_chart::Segment, line_chart::Series, bar_chart::Bar};

// ── State Store ──

#[derive(Clone, Copy, PartialEq)]
enum HealthView { Home, Records, Consent, Privacy, Metabolism }
...
impl HealthState {
    fn new() -> Self {
        Self {
            ...
            privacy_budget: RwSignal::new(PrivacyBudget { patient_did: "did:me".into(), epsilon_total: 10.0, epsilon_spent: 7.6, delta: 1e-5, fl_contributions: 23, last_contribution_at: Some(1711900800) }),
            dividends: RwSignal::new(vec![
                DataDividend { id: "dd-1".into(), patient_did: "did:me".into(), cohort_id: "diabetes-2026".into(), cohort_name: "Diabetes Prevention Study".into(), amount: 42, currency: "SAP".into(), earned_at: 1711900800, claimed: true },
                DataDividend { id: "dd-2".into(), patient_did: "did:me".into(), cohort_id: "cardiac-ml".into(), cohort_name: "Cardiac ML Cohort".into(), amount: 86, currency: "SAP".into(), earned_at: 1711814400, claimed: false },
            ]),
        }
    }
}
...
fn mock_records() -> Vec<HealthRecord> {
    vec![
        HealthRecord { id: "hr-1".into(), patient_did: "did:me".into(), record_type: RecordType::LabResult, encrypted_payload: vec![0; 128], metadata: RecordMetadata { provider_did: Some("did:lab".into()), facility: Some("Community Health Center".into()), date_of_service: 1711900800, sensitivity: SensitivityLevel::Normal }, created_at: 1711900800 },
        HealthRecord { id: "hr-2".into(), patient_did: "did:me".into(), record_type: RecordType::Vitals, encrypted_payload: vec![0; 64], metadata: RecordMetadata { provider_did: Some("did:nurse".into()), facility: Some("Home Visit".into()), date_of_service: 1711814400, sensitivity: SensitivityLevel::Normal }, created_at: 1711814400 },
        HealthRecord { id: "hr-3".into(), patient_did: "did:me".into(), record_type: RecordType::Medication, encrypted_payload: vec![0; 96], metadata: RecordMetadata { provider_did: Some("did:pharm".into()), facility: Some("Community Pharmacy".into()), date_of_service: 1711728000, sensitivity: SensitivityLevel::Normal }, created_at: 1711728000 },
        HealthRecord { id: "hr-4".into(), patient_did: "did:me".into(), record_type: RecordType::LabResult, encrypted_payload: vec![0; 256], metadata: RecordMetadata { provider_did: Some("did:lab".into()), facility: None, date_of_service: 1711641600, sensitivity: SensitivityLevel::MentalHealth }, created_at: 1711641600 },
    ]
}

fn mock_consents() -> Vec<Consent> {
    vec![
        Consent { id: "c-1".into(), patient_did: "did:me".into(), recipient_did: "did:dr-chen".into(), categories: vec![ConsentCategory::LabResults, ConsentCategory::Vitals], purpose: "Primary care management".into(), granted_at: 1710000000, expires_at: Some(1741536000), revoked_at: None, no_further_disclosure: false },
        Consent { id: "c-2".into(), patient_did: "did:me".into(), recipient_did: "did:research-u".into(), categories: vec![ConsentCategory::LabResults], purpose: "Diabetes prevention study (FL only)".into(), granted_at: 1711000000, expires_at: Some(1742536000), revoked_at: None, no_further_disclosure: true },
        Consent { id: "c-3".into(), patient_did: "did:me".into(), recipient_did: "did:old-clinic".into(), categories: vec![ConsentCategory::All], purpose: "Former provider".into(), granted_at: 1700000000, expires_at: None, revoked_at: Some(1710000000), no_further_disclosure: false },
    ]
}

fn record_type_color(t: &RecordType) -> &'static str {
    match t { RecordType::LabResult => "#0D7377", RecordType::Vitals => "#06D6C8", RecordType::Medication => "#22c55e", RecordType::Immunization => "#60a5fa", RecordType::Encounter => "#8b5cf6", RecordType::Imaging => "#f59e0b", RecordType::Procedure => "#ec4899" }
}
fn record_type_label(t: &RecordType) -> &'static str {
    match t { RecordType::LabResult => "Lab", RecordType::Vitals => "Vitals", RecordType::Medication => "Meds", RecordType::Immunization => "Immun", RecordType::Encounter => "Visit", RecordType::Imaging => "Imaging", RecordType::Procedure => "Proc" }
}
    ...
    view! {
        <div class="health-content">
            ...
            <Show when=move || !state.vault_unlocked.get() && active_view.get() != HealthView::Home>
                <div class="thought-card" style="text-align: center; padding: 2rem;">
                    <h3 style="font-size: 1rem; margin-bottom: 0.5rem;">"Vault Locked"</h3>
                    <p style="font-size: 0.8rem; color: var(--text-muted); margin-bottom: 1rem;">"Unlock your health vault to view records."</p>
                    <button class="form-submit" on:click=move |_| state.vault_unlocked.set(true)>"Unlock Vault"</button>
                </div>
            </Show>
