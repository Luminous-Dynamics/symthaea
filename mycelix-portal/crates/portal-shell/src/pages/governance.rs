...
fn mock_councils() -> Vec<Council> {
    vec![
        Council { id: "c-root".into(), name: "Root Council".into(), purpose: "Top-level governance and constitutional stewardship".into(), council_type: CouncilType::Root, parent_council_id: None, phi_threshold: 0.4, quorum: 0.51, supermajority: 0.67, status: CouncilStatus::Active, created_at: 1700000000 },
        Council { id: "c-energy".into(), name: "Energy Working Group".into(), purpose: "Solar, wind, and geothermal project coordination".into(), council_type: CouncilType::WorkingGroup { focus: "renewable energy".into(), expires: Some(1743350400) }, parent_council_id: Some("c-root".into()), phi_threshold: 0.3, quorum: 0.4, supermajority: 0.6, status: CouncilStatus::Active, created_at: 1711000000 },
        Council { id: "c-care".into(), name: "Care Coordination Council".into(), purpose: "Coordinate care services across the community".into(), council_type: CouncilType::Domain { domain: "commons".into() }, parent_council_id: Some("c-root".into()), phi_threshold: 0.3, quorum: 0.4, supermajority: 0.6, status: CouncilStatus::Active, created_at: 1705000000 },
    ]
}
...
pub fn GovernanceOverview() -> impl IntoView {
    ...
    wasm_bindgen_futures::spawn_local(async move {
        ...
        let proposals = if identity_for_load.conductor_status.get() == ConductorStatus::Connected {
            identity_for_load.call_zome::<(), Vec<ProposalSummary>>(
                "governance", "proposals", "list_active_proposals", &()
            ).await.unwrap_or_else(|_| mock_proposals())
        } else {
            mock_proposals()
        };
        ...
        let councils = if identity_for_load.conductor_status.get() == ConductorStatus::Connected {
            identity_for_load.call_zome::<(), Vec<Council>>(
                "governance", "councils", "list_councils", &()
            ).await.unwrap_or_else(|_| mock_councils())
        } else {
            mock_councils()
        };
...
fn ProposalDetail(summary: ProposalSummary) -> impl IntoView {
    ...
    let cast_vote = move |choice: VoteChoice| {
        ...
        wasm_bindgen_futures::spawn_local(async move {
            let input = serde_json::json!({ "proposal_id": pid, "choice": format!("{:?}", choice_clone) });
            let _ = identity.call_zome::<serde_json::Value, serde_json::Value>(
                "governance", "voting", "cast_vote", &input
            ).await;
            ...
            tally.update(|t| {
                match choice_clone {
                    VoteChoice::For => { t.votes_for += 1; t.total_weight_for += 1.0; },
                    VoteChoice::Against => { t.votes_against += 1; t.total_weight_against += 1.0; },
                    VoteChoice::Abstain => { t.votes_abstain += 1; },
                }
            });
        ...
        wasm_bindgen_futures::spawn_local(async move {
            ...
            let new_proposal = ProposalSummary {
                proposal: Proposal {
                    ...
                    actions: "[]".into(),
                    discussion_url: None,
                    voting_starts: 0, voting_ends: 0, created: 0, updated: 0, version: 1,
                },
