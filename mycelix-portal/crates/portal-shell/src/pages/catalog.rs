...
pub fn CatalogOverview() -> impl IntoView {
    ...
    let identity = expect_context::<crate::identity::PortalIdentity>();
    let tier = identity.tier;

    // Build cluster cards from registry
    let clusters: Vec<ClusterCardData> = registry.domains.iter().map(|d| {
        let deps: Vec<DepInfo> = d.dependencies().iter().map(|dep| {
...
fn ClusterCard(data: ClusterCardData, accessible: bool) -> impl IntoView {
    ...
    let dep_summary = if data.dependencies.is_empty() {
        ...
        let deps: Vec<String> = data.dependencies.iter().map(|d| {
...
fn DependencyGraph(clusters: Vec<ClusterCardData>) -> impl IntoView {
    ...
    let positions: Vec<(f64, f64)> = (0..clusters.len()).map(|i| {
        let angle = (i as f64) * std::f64::consts::TAU / count;
        (radius * angle.cos(), radius * angle.sin())
    }).collect();

    // Pre-compute edge SVG data
    struct EdgeData { x1: f64, y1: f64, x2: f64, y2: f64, required: bool }
    ...
    for (i, cluster) in clusters.iter().enumerate() {
        for dep in &cluster.dependencies {
            if let Some(target_idx) = clusters.iter().position(|c| c.id == dep.id) {
                ...
                edges.push(EdgeData { x1, y1, x2, y2, required: dep.required });
            }
        }
    }

    // Pre-compute node SVG data
    struct NodeData { cx: f64, cy: f64, color: &'static str, glow: &'static str, bio_name: &'static str }
    let nodes: Vec<NodeData> = clusters.iter().enumerate().map(|(i, cluster)| {
        let (cx, cy) = positions[i];
        NodeData { cx, cy, color: cluster.color, glow: cluster.glow, bio_name: cluster.bio_name }
    }).collect();
