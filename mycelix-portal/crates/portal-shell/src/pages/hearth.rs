...
enum HearthView { Kinship, Gratitude, Stories, Milestones }
...
fn bond_type_label(b: &BondType) -> &'static str {
    match b { BondType::Parent => "Parent", BondType::Child => "Child", BondType::Sibling => "Sibling", BondType::Partner => "Partner", BondType::Chosen => "Chosen", BondType::Mentor => "Mentor", BondType::Elder => "Elder", BondType::Companion => "Companion" }
}
fn bond_color(b: &BondType) -> &'static str {
    match b { BondType::Partner => "#ec4899", BondType::Child => "#f59e0b", BondType::Parent => "#8b5cf6", BondType::Mentor => "#06b6d4", BondType::Companion => "#22c55e", _ => "#DB2777" }
}
...
fn GratitudeForm() -> impl IntoView {
    ...
    let on_submit = move |ev: web_sys::SubmitEvent| {
        ...
        let t = to.get();
        let m = message.get();
        if m.trim().is_empty() { return; }
        state.gratitude.update(|notes| {
            notes.insert(0, GratitudeNote {
                id: format!("g-{}", notes.len() + 4),
                from_did: "did:me".into(),
                to_did: if t.is_empty() { "did:community".into() } else { format!("did:{t}") },
