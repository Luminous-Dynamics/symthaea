...
//!
//! # Then run this test:
//! cargo test -p mycelix-portal --test conductor_integration -- --nocapture
//! ```
//!
//! # What it tests
//!
...
fn proposal_msgpack_wire_format() {
    ...
    assert!(bytes.len() > 50, "Proposal should encode to >50 bytes, got {}", bytes.len());
...
fn proposal_vec_wire_format() {
    let proposals = vec![
        Proposal {
            id: "MIP-001".into(), title: "First".into(), description: "".into(),
        ...
        Proposal {
            id: "MIP-002".into(), title: "Second".into(), description: "".into(),
            proposal_type: ProposalType::Emergency, author: "did:test".into(),
            status: ProposalStatus::Draft, actions: "[]".into(),
...
fn vote_phi_weight_wire_format() {
    ...
    assert_eq!(decoded.reason.as_deref(), Some("Strong support for community solar"));
...
fn council_nested_enum_wire_format() {
    let councils = vec![
        Council {
            id: "c-root".into(), name: "Root Council".into(),
            purpose: "Top-level governance".into(),
            council_type: CouncilType::Root,
            parent_council_id: None,
            phi_threshold: 0.4, quorum: 0.51, supermajority: 0.67,
            status: CouncilStatus::Active, created_at: 1711814400,
        },
        Council {
            id: "c-energy".into(), name: "Energy Working Group".into(),
            ...
            phi_threshold: 0.3, quorum: 0.4, supermajority: 0.6,
            status: CouncilStatus::Active, created_at: 1711900800,
        },
    ];
