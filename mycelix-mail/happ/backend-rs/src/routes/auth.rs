...
impl CredentialStore {
    ...
    fn derive_encryption_key() -> [u8; 32] {
        ...
        let secret = std::env::var("CREDENTIAL_ENCRYPTION_SECRET")
            .unwrap_or_else(|_| {
                tracing::warn!(
                    "CREDENTIAL_ENCRYPTION_SECRET not set. Using insecure default key. \
                     Set this environment variable for production use."
                );
                "mycelix-mail-default-credential-key-insecure".to_string()
            });
