...
use symthaea_mycelix_bridge::{
    pogq_from_quality_score, ConsciousnessBackend, ModelSnapshot, SymthaeaBackend,
};

use mycelix_sdk::{
    hyperfeel::{HyperGradient, HV16_BYTES},
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (node_eval, net_eval) =
        matl.evaluate_node(&hypergradient.node_id, &pogq, reputation);
