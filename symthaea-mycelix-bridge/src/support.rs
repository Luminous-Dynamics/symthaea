...
pub fn cognitive_update_to_memory_record(
    update: &CognitiveUpdatePackage,
) -> MemoryRecordData {
    MemoryRecordData {
...
mod tests {
    ...
    fn response_to_comment_with_confidence() {
        let comment = response_to_comment(
            "Try restarting the conductor",
            Some(0.9),
            Some("Probable"),
        );
    ...
    fn cognitive_update_round_trip() {
        ...
        let update = memory_to_cognitive_update(
            encoding.clone(), 0.75, "Network", "restart_fixes_dns",
        );
    ...
    fn cognitive_update_serializable() {
        let update = memory_to_cognitive_update(
            vec![42u8; 2048], 0.5, "Holochain", "clear_cache_pattern",
        );
