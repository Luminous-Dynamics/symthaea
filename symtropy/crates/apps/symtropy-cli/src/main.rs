...
//! symtropy lifesim <organism> --out <file.ppm>
//! ```
...
USAGE:
    ...
    symtropy lifesim <colony|mycelium> [--steps N] --out <file.ppm>
    symtropy --help
...
LIFESIM:
    colony              Export Ant Colony V0 pheromone/agent heatmap
    mycelium            Export Mycelial Network V0 biomass/nutrient/toxin heatmap
...
fn main() -> ExitCode {
    ...
    match cmd.as_str() {
        ...
        "lifesim" => match cmd_lifesim(args) {
            Ok(()) => ExitCode::SUCCESS,
            Err(e) => {
                eprintln!("error: {e}");
                ExitCode::FAILURE
...
fn cmd_lifesim(mut args: impl Iterator<Item = String>) -> Result<(), String> {
    let organism = args
        .next()
        .ok_or_else(|| "missing <organism>. Available: colony, mycelium".to_string())?;
    ...
    let out = out.ok_or_else(|| "missing --out <file.ppm>".to_string())?;
    let ppm = match organism.as_str() {
        "colony" => {
            let mut world = symtropy_colony::ColonyWorld::new(48, 24, (3, 12), 96);
            world.add_food_source(42, 12, 10_000);
            for y in 3..21 {
                if y != 6 {
                    world.set_obstacle(24, y, true);
                }
            }
            world.run_steps(steps);
            let metrics = world.debug_metrics();
            println!(
                "colony tick={} food_reserve={} trail_entropy={:.3} coherence={:.3} stress={:.3}",
                metrics.tick,
                metrics.food_reserve,
                metrics.trail_entropy,
                metrics.pheromone_coherence,
                metrics.stress
            );
            world.to_ppm_heatmap()
        }
        "mycelium" => {
            let mut world = symtropy_mycelium::MyceliumWorld::new(48, 24, (5, 12));
            world.add_dead_biomass(40, 12, 900.0);
            for y in 7..18 {
                world.add_toxin(23, y, 160.0);
            }
            world.run_steps(steps);
            let metrics = world.metrics();
            println!(
                "mycelium tick={} central_biomass={:.3} total_biomass={:.3} total_nutrient={:.3} total_toxin={:.3}",
                metrics.tick,
                metrics.central_biomass,
                metrics.total_biomass,
                metrics.total_nutrient,
                metrics.total_toxin
            );
            world.to_ppm_heatmap()
        }
        other => {
            return Err(format!(
                "unknown organism `{other}`. Available: colony, mycelium"
            ));
        }
    };
    ...
    std::fs::write(&out, ppm).map_err(|e| format!("write {}: {e}", out.display()))?;
