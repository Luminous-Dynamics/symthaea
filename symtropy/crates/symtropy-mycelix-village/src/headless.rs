...
fn main() {
    App::new()
        ...
        .insert_resource(ScenarioStats { ticks: 0, max_ticks: 1000 })
    ...
    for (body, brain) in &query {
        ...
        if body.handle.0 == 1 && stats.ticks > 500 {
            if profile.stewardship_care > 0.95 {
                error!("NPC-1: Stewardship Care should be dropping! Current: {:.4}", profile.stewardship_care);
            } else {
                info!("NPC-1: Stewardship Care is correctly dropping: {:.4}", profile.stewardship_care);
            }
        }
