...
fn run_mini_sim(seed: u64) -> SimState {
    ...
    for _tick in 0..TICKS {
        ...
        let wdata: Vec<_> = wells.iter().zip(well_remaining.iter())
            .filter(|(_, &r)| r > 0.0).map(|(&p, &r)| (p, (r / 500.0).min(1.0))).collect();
        for &h in &handles {
    ...
    let positions: Vec<[f64; 2]> = handles.iter().map(|&h| {
        world.body(h).map(|b| { let p = b.position().0; [p[0], p[1]] }).unwrap_or([0.0, 0.0])
    }).collect();
    let energies: Vec<f64> = handles.iter().map(|h| {
        consciousness.entities.get(h).map(|e| e.energy.available).unwrap_or(0.0)
    }).collect();

    SimState { positions, energies, cooperation: coop }
}
...
fn determinism_lockin_seed_42() {
    ...
    for i in 0..AGENTS {
        assert_eq!(a.positions[i][0].to_bits(), b.positions[i][0].to_bits(),
            "Agent {i} x-position diverged: {} vs {}", a.positions[i][0], b.positions[i][0]);
        assert_eq!(a.positions[i][1].to_bits(), b.positions[i][1].to_bits(),
            "Agent {i} y-position diverged");
        assert_eq!(a.energies[i].to_bits(), b.energies[i].to_bits(),
            "Agent {i} energy diverged: {} vs {}", a.energies[i], b.energies[i]);
    }
}
...
fn different_seeds_different_results() {
    ...
    let any_diff = a.positions.iter().zip(b.positions.iter())
