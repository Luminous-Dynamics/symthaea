...
const HARMONY_PROFILES: [[f64; 8]; 6] = [
    [0.9, 0.2, 0.1, 0.1, 0.2, 0.1, 0.1, 0.8],
    [0.8, 0.3, 0.2, 0.1, 0.3, 0.2, 0.2, 0.7],
    [0.1, 0.2, 0.8, 0.2, 0.8, 0.1, 0.2, 0.1],
    [0.2, 0.1, 0.7, 0.3, 0.7, 0.2, 0.1, 0.2],
    [0.1, 0.8, 0.1, 0.2, 0.1, 0.2, 0.8, 0.1],
    [0.2, 0.7, 0.2, 0.1, 0.2, 0.1, 0.7, 0.2],
];

#[derive(Clone, Copy)]
enum Ctrl { Fep, WellOnly, Greedy, Random, Stationary, PartnerOnly }
...
fn run_experiment(ctrl: Ctrl, seed: u64) -> ResOnlyResult {
    ...
    ResOnlyResult { controller: ctrl_name, alive, energy, cooperation: coop as f64,
...
fn main() {
    ...
    let tests = vec![
        ("FEP vs RANDOM", { let (_, _, p) = mann_whitney_u(&fep_alive, &random_alive); p }),
        ("FEP vs STATIONARY", { let (_, _, p) = mann_whitney_u(&fep_alive, &stationary_alive); p }),
        ("PARTNER vs RANDOM", { let (_, _, p) = mann_whitney_u(&partner_alive, &random_alive); p }),
        ("PARTNER vs STATIONARY", { let (_, _, p) = mann_whitney_u(&partner_alive, &stationary_alive); p }),
    ];
    let effects = [
    ...
    if social_better {
        ...
        eprintln!("  The FEP gradient transitions from liability to necessity when wells are removed.");
    } else {
