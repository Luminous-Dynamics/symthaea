...
const HARMONY_PROFILES: [[f64; 8]; 4] = [
    [0.7, 0.4, 0.2, 0.1, 0.3, 0.3, 0.2, 0.6],
    [0.3, 0.6, 0.3, 0.2, 0.2, 0.4, 0.6, 0.3],
...
fn run_experiment(use_learning: bool, seed: u64) -> LearnResult {
    ...
    // Weight analysis
    let mean_drift = agent_weights.iter().map(|w| w.drift_from_default()).sum::<f64>() / AGENTS as f64;
    ...
    LearnResult {
        condition: cond_name, alive, energy, clustering,
...
fn main() {
    ...
    println!("condition,seed,alive,energy,clustering,cooperation,weight_drift,coop_w,well_w,danger_w");
    ...
    for s in 0..SEEDS {
        ...
        println!("{},{seed},{:.1},{:.1},{:.2},{:.0},{:.4},{:.2},{:.2},{:.2}",
            r.condition, r.alive, r.energy, r.clustering, r.cooperation,
            r.mean_weight_drift, r.final_coop_weight, r.final_well_weight, r.final_danger_weight);
    ...
    let n = SEEDS as f64;
    eprintln!("\n── FIXED (baseline) ──");
    eprintln!("  Alive:       {:.1}/{AGENTS}", fixed_results.iter().map(|r| r.alive).sum::<f64>() / n);
    eprintln!("  Energy:      {:.1} J", fixed_results.iter().map(|r| r.energy).sum::<f64>() / n);
    eprintln!("  Clustering:  {:.2}", fixed_results.iter().map(|r| r.clustering).sum::<f64>() / n);
    eprintln!("  Cooperation: {:.0}", fixed_results.iter().map(|r| r.cooperation).sum::<f64>() / n);

    eprintln!("\n── LEARNING ──");
    eprintln!("  Alive:       {:.1}/{AGENTS}", learn_results.iter().map(|r| r.alive).sum::<f64>() / n);
    eprintln!("  Energy:      {:.1} J", learn_results.iter().map(|r| r.energy).sum::<f64>() / n);
    eprintln!("  Clustering:  {:.2}", learn_results.iter().map(|r| r.clustering).sum::<f64>() / n);
    eprintln!("  Cooperation: {:.0}", learn_results.iter().map(|r| r.cooperation).sum::<f64>() / n);
    eprintln!("  Weight drift: {:.4}", learn_results.iter().map(|r| r.mean_weight_drift).sum::<f64>() / n);
    eprintln!("  Learned coop:   {:.3} (default: 2.0)", learn_results.iter().map(|r| r.final_coop_weight).sum::<f64>() / n);
    eprintln!("  Learned well:   {:.3} (default: 4.0)", learn_results.iter().map(|r| r.final_well_weight).sum::<f64>() / n);
    eprintln!("  Learned danger: {:.3} (default: 3.0)", learn_results.iter().map(|r| r.final_danger_weight).sum::<f64>() / n);
    ...
    let tests = vec![("survival", p_alive), ("cooperation", p_coop), ("clustering", p_clust)];
    ...
    for (i, &(label, adj_p, sig)) in corrected.iter().enumerate() {
        ...
        eprintln!("  {label:12}: p_adj={adj_p:.4}, d={d:.3} ({size}) {}", if sig { "← SIGNIFICANT" } else { "" });
    }
