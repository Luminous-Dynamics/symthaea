...
#[derive(Clone, Copy)]
enum Ctrl { Fep, WellOnly, Greedy, Random }
...
fn run_experiment(ctrl: Ctrl, seed: u64) -> (f64, f64, f64) {
    ...
    let mut rng = seed;
    let wells: Vec<SVector<f64, 2>> = (0..3).map(|_| {
        SVector::from([
            ...
            let vel: SVector<f64, 2> = match ctrl {
                ...
                Ctrl::WellOnly => {
                    if let Some((wp, _)) = wdata.iter().min_by(|(a, _), (b, _)|
                        ...
                        let delta = wp - pos;
                        let d = delta.norm();
                        if d > 1.0 { delta / d * 20.0 } else { SVector::zeros() }
                    } else {
                    ...
                    for ai in 0..8 {
                        ...
                        for (ap, ah) in &adata {
                            ...
                            if d > 2.0 && d < consciousness.constants.harmony_range {
                                ...
                                if res > 0.5 { gain += consciousness.constants.harmony_resonance_regen_rate * (res - 0.5); }
                            }
                        }
...
fn main() {
    ...
    let ctrls = [(Ctrl::Fep,"FEP"), (Ctrl::WellOnly,"WELL"), (Ctrl::Greedy,"GREEDY"), (Ctrl::Random,"RANDOM")];
