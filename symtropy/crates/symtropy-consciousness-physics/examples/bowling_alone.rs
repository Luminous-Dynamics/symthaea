...
#[derive(Clone, Copy)]
enum ErosionMode { Stable, Eroding, Collapsed }
...
fn run_experiment(erosion: ErosionMode, seed: u64) -> BowlingResult {
    ...
    for tick in 0..TICKS {
        ...
        let wdata: Vec<_> = wells.iter().zip(well_remaining.iter())
            .filter(|(_, &r)| r > 0.0).map(|(&p, &r)| (p, (r / 5000.0).min(1.0))).collect();
        for &h in &handles {
            ...
            let near: Vec<_> = adata.iter().filter(|(p, _)| {
                let d = (p - pos).norm(); d > 2.0 && d < current_range
            }).cloned().collect();
    ...
    if decades.len() >= 2 {
        ...
        if last.cooperation_rate < first.cooperation_rate * 0.8 { putnam += 1.0; } // declining cooperation
        if last.clustering > first.clustering * 1.2 || last.clustering == 0.0 { putnam += 1.0; } // increasing isolation
        if last.alive > first.alive * 0.7 { putnam += 1.0; } // persistent survival (economy)
        if last.gini > first.gini + 0.05 { putnam += 1.0; } // rising inequality
    }

    BowlingResult { condition: cond_name, decades, final_alive: alive, putnam_score: putnam }
}
...
fn main() {
    ...
    for &(mode, name) in &modes {
        ...
        for s in 0..SEEDS {
            ...
            for d in &r.decades {
                println!("{name},{seed},{},{:.1},{:.1},{:.2},{:.4},{:.1},{:.4}",
                    d.decade, d.alive, d.cooperation_rate, d.clustering, d.gini,
                    d.current_range, d.current_regen);
            }
            results.push(r);
        }
    ...
    if let Some((_, results)) = all.iter().find(|(n, _)| *n == "ERODING") {
        ...
        for di in 0..n_decades {
            ...
            let coop = results.iter().map(|r| r.decades[di].cooperation_rate).sum::<f64>() / n;
            let clust = results.iter().filter_map(|r| {
                let c = r.decades[di].clustering;
                if c.is_finite() && c < 1e10 { Some(c) } else { None }
            }).sum::<f64>() / n;
            ...
            eprintln!("  {:5}   {:4.1}   {:.4}  {:5.1}  {:7.1}   {:6.2}   {:.3}",
