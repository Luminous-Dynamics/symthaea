...
fn main() {
    ...
    for tick in 0..TICKS {
        ...
        for &h in &handles {
            ...
            consciousness.ledger.record_phi_change(consciousness.phi(h) - old);
        }
        ...
        for &h in &handles {
            ...
            let near: Vec<_> = adata.iter().filter(|(p,_)| (p-pos).norm() > 2.0).cloned().collect();
            let dir = fep_gradient::free_energy_gradient(&pos, e.energy.fraction_remaining(), &e.harmony_activations, &near, &wdata, None, 0.0);
        ...
        let nw: Vec<bool> = handles.iter().map(|&h| world.body(h).map(|b| wells.iter().any(|&w| (b.position().0 - w).norm() < 35.0)).unwrap_or(false)).collect();
        for (i, &h) in handles.iter().enumerate() {
            ...
            if let Some(e) = consciousness.entities.get_mut(&h) {
                ...
                if nw[i] { e.energy.regenerate(wr); }
            }
        }
        ...
        if let Some(j) = bal.joules_per_phi { if j.is_finite() { jphi_detector.push(j); } }
        if consciousness.collective_phi > peak_phi { peak_phi = consciousness.collective_phi; peak_tick = tick; }

        if tick % 500 == 0 {
            let alive = handles.iter().filter(|h| consciousness.entities.get(h).map(|e| !e.energy.is_collapsed()).unwrap_or(false)).count();
            let ae: f64 = handles.iter().filter_map(|h| consciousness.entities.get(h).map(|e| e.energy.available)).sum::<f64>() / AGENTS as f64;
            let cl = avg_nn(&world, &handles);
            println!("{tick},{:.4},{alive},{ae:.1},{cl:.2},{coop_events}", consciousness.collective_phi);
        }
    }
    ...
    eprintln!("╠══════════════════════════════════════════╣");
    eprintln!("║ Alive:      {:>3}/{:<3}                      ║", alive, AGENTS);
    eprintln!("║ Peak Φ:     {:.4} (tick {:<5})            ║", peak_phi, peak_tick);
    eprintln!("║ Clustering: {:.2}                          ║", avg_nn(&world, &handles));
    eprintln!("║ Coop events:{:<10}                   ║", coop_events);
    eprintln!("║ J/Φ conv:   {}                    ║", if jphi_detector.is_converged() { "YES" } else { "NO " });
    eprintln!("╚══════════════════════════════════════════╝");
    eprintln!("  Survival: {}", if alive > AGENTS/2 { "PASS" } else { "FAIL" });
    eprintln!("  Cooperation: {}", if coop_events > 0 { "PASS" } else { "FAIL" });
}

fn avg_nn(w: &PhysicsWorld<2>, h: &[symtropy_physics::BodyHandle]) -> f64 {
    let p: Vec<_> = h.iter().filter_map(|h| w.body(*h).map(|b| b.position().0)).collect();
    if p.len() < 2 { return f64::MAX; }
    p.iter().enumerate().map(|(i,a)| p.iter().enumerate().filter(|(j,_)| *j!=i).map(|(_,b)| (a-b).norm()).fold(f64::MAX, f64::min)).sum::<f64>() / p.len() as f64
}
