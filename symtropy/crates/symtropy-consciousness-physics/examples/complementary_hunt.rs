...
#[derive(Clone, Copy)]
enum Ctrl { Fep, WellOnly, Greedy, PartnerOnly }
...
fn run_experiment(ctrl: Ctrl, require_complementary: bool, seed: u64) -> (f64, f64, f64) {
    ...
    for _ in 0..PREY_COUNT {
        ...
        prey_list.push(Prey::new(SVector::from([x, y]), PREY_ENERGY, 30.0, PREY_RESPAWN));
    }
    ...
    for _tick in 0..TICKS {
        ...
        let wdata: Vec<_> = prey_list.iter()
                ...
                Ctrl::PartnerOnly => {
                    ...
                    for (ap, _ah) in &adata {
                        ...
                        if d > 2.0 && d < consciousness.constants.harmony_range {
                            if d < 1.0 / best_score.max(0.01) { best_score = 1.0 / d; best_dir = delta / d; }
                        }
                    }
                    best_dir * 20.0
                },
        ...
        for prey in &mut prey_list {
            ...
            let agents_at_prey: Vec<(usize, [f64; 8])> = handles.iter().enumerate()
                ...
                .map(|(idx, &h)| (idx, consciousness.entities.get(&h).unwrap().harmony_activations))
                .collect();
            ...
            if hunt_success {
                ...
                for &(idx, _) in &agents_at_prey {
                    let h = handles[idx];
                    if let Some(e) = consciousness.entities.get_mut(&h) {
                        e.energy.regenerate(prey.energy_value / agents_at_prey.len() as f64);
                    }
                }
                prey.capture();
                hunts += 1;
...
fn main() {
    ...
    eprintln!("{AGENTS} agents (10 drivers + 10 ambushers), {PREY_COUNT} prey, {TICKS} ticks, {SEEDS} seeds");

    println!("controller,complementary,seed,alive,energy,hunts");

    let ctrls = [(Ctrl::Fep,"FEP"), (Ctrl::WellOnly,"WELL"), (Ctrl::Greedy,"GREEDY"), (Ctrl::PartnerOnly,"PARTNER")];
    ...
    for i in 0..ctrls.len() {
        ...
        let delta = comp - solo;
        eprintln!("  {:8}     {:5.1}       {:5.1}          {:+5.1}", name, solo, comp, delta);
    }
