...
enum NullCondition { Baseline, RandomWalk, Shuffled, NoRegen }
...
fn run_experiment(condition: NullCondition, seed: u64) -> NullResult {
    ...
    NullResult { condition: cond_name, alive, clustering, cooperation: coop as f64 }
}
...
fn main() {
    ...
    for &(cond, name) in &conditions {
        ...
        for s in 0..SEEDS {
            ...
            println!("{},{seed},{:.1},{:.2},{:.0}", r.condition, r.alive, r.clustering, r.cooperation);
            results.push(r);
        }
    ...
    for (i, &(label, adj_p, sig)) in corrected.iter().enumerate() {
        ...
        eprintln!("  vs {:13}: p_adj={adj_p:.4}, d={d:.3} ({size}) {}",
            label, if sig { "← SIGNIFICANT" } else { "" });
    }
