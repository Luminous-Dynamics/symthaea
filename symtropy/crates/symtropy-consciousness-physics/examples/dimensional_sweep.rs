...
fn run_dim<const D: usize>(with_drift: bool, seed: u64) -> DimResult {
    ...
    for _ in 0..DRIFTERS {
        ...
        if D > 1 { coords[1] = (nr(&mut rng) - 0.5) * 40.0; }
...
fn main() {
    ...
    for &d in &dimensions {
        for (name, drift) in &[("STATIC", false), ("DRIFT", true)] {
            ...
            let mut en=Vec::new(); let mut cl=Vec::new(); let mut de=Vec::new();

            for s in 0..SEEDS {
                let r = run_at_dimension(d, *drift, 42 + s as u64 * 997);
                pe.push(r.pred_error); mp.push(r.max_pe);
                al.push(r.alive); en.push(r.energy);
                cl.push(r.clustering); de.push(r.dark_energy);
            }

            let n = SEEDS as f64;
            println!("{:<6} {:<8} {:<10.4} {:<10.3} {:<8.1} {:<10.1} {:<10.2} {:<10.3}",
                format!("{}D", d), name,
                pe.iter().sum::<f64>()/n, mp.iter().sum::<f64>()/n,
                al.iter().sum::<f64>()/n, en.iter().sum::<f64>()/n,
                cl.iter().sum::<f64>()/n, de.iter().sum::<f64>()/n,
            );
        }
        println!();
    }
