...
fn main() {
    ...
    for ctrl_name in &["WELL", "FEP", "GREEDY"] {
        for &cap in &WELL_CAPACITIES {
            let regen_data = phase_data.iter().find(|(c, cp, r, _)| c == ctrl_name && (*cp - cap).abs() < 1.0 && *r);
            let noregen_data = phase_data.iter().find(|(c, cp, r, _)| c == ctrl_name && (*cp - cap).abs() < 1.0 && !*r);

            if let (Some((_, _, _, r_alive)), Some((_, _, _, nr_alive))) = (regen_data, noregen_data) {
                ...
                let marker = if necessary { " ← COOPERATION NECESSARY" } else { "" };
                eprintln!("  {:8}     {:5.0}    {:5.1}    {:5.1}     {:+5.1}   {:+.2}   {marker}",
