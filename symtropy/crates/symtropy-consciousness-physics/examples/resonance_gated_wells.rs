...
fn run_experiment(ctrl: Ctrl, res_threshold: f64, seed: u64) -> ResGateResult {
                            ...
                            if (pos - w).norm() < 35.0 && well_remaining[wi] > 0.0 {
                                ...
                                let has_resonant_partner = if res_threshold <= 0.0 {
                                    ...
                                    handles.iter().any(|&other| {
                                        ...
                                        let other_at_well = world.body(other)
                                            .map(|ob| (ob.position().0 - w).norm() < 35.0)
                                            .unwrap_or(false);
                                        if !other_at_well { return false; }
                                        let other_harmony = consciousness.entities.get(&other)
                                            .map(|oe| oe.harmony_activations);
                                        match other_harmony {
                                            Some(oh) => HarmonyField::<2>::resonance(&my_harmony, &oh) >= res_threshold,
                                            None => false,
                                        }
                                    })
                                };
    ...
    ResGateResult {
        controller: ctrl_name, res_threshold, alive, energy,
        cooperation: coop as f64, gated_access_ticks: gated_out as f64,
    }
}
...
fn main() {
    ...
    for &(ctrl, cname) in &ctrls {
        for &thresh in &RES_THRESHOLDS {
            ...
            for s in 0..SEEDS {
                ...
                println!("{},{:.1},{seed},{:.1},{:.1},{:.0},{:.0}",
                    r.controller, r.res_threshold, r.alive, r.energy, r.cooperation, r.gated_access_ticks);
                results.push(r);
            }
            let n = results.len() as f64;
            let mean_alive = results.iter().map(|r| r.alive).sum::<f64>() / n;
            let mean_gated = results.iter().map(|r| r.gated_access_ticks).sum::<f64>() / n;
            eprintln!("  → {cname} res≥{thresh:.1}: alive={mean_alive:.1}, gated_out={mean_gated:.0}");
    ...
    for &thresh in &RES_THRESHOLDS {
        let w = all.iter().find(|(c, t, _)| *c == "WELL" && (*t - thresh).abs() < 0.01)
            .map(|(_, _, r)| r.iter().map(|r| r.alive).sum::<f64>() / r.len() as f64).unwrap_or(0.0);
        let f = all.iter().find(|(c, t, _)| *c == "FEP" && (*t - thresh).abs() < 0.01)
            .map(|(_, _, r)| r.iter().map(|r| r.alive).sum::<f64>() / r.len() as f64).unwrap_or(0.0);
        let g = all.iter().find(|(c, t, _)| *c == "GREEDY" && (*t - thresh).abs() < 0.01)
            .map(|(_, _, r)| r.iter().map(|r| r.alive).sum::<f64>() / r.len() as f64).unwrap_or(0.0);
        let fep_wins = f > w + 1.0 && f > g + 1.0;
        eprintln!("  ≥{:.1}       {:5.1}    {:5.1}    {:5.1}    {}",
            thresh, w, f, g, if fep_wins { "← FEP WINS" } else { "" });
    }

    // Gated-out analysis
    eprintln!("\n── Gated-Out Events (at well but no resonant partner) ──");
    for &thresh in &RES_THRESHOLDS {
        if thresh < 0.01 { continue; } // no gate at 0
        let w = all.iter().find(|(c, t, _)| *c == "WELL" && (*t - thresh).abs() < 0.01)
            .map(|(_, _, r)| r.iter().map(|r| r.gated_access_ticks).sum::<f64>() / r.len() as f64).unwrap_or(0.0);
        let f = all.iter().find(|(c, t, _)| *c == "FEP" && (*t - thresh).abs() < 0.01)
            .map(|(_, _, r)| r.iter().map(|r| r.gated_access_ticks).sum::<f64>() / r.len() as f64).unwrap_or(0.0);
        let g = all.iter().find(|(c, t, _)| *c == "GREEDY" && (*t - thresh).abs() < 0.01)
            .map(|(_, _, r)| r.iter().map(|r| r.gated_access_ticks).sum::<f64>() / r.len() as f64).unwrap_or(0.0);
        eprintln!("  ≥{:.1}: WELL={w:.0}, FEP={f:.0}, GREEDY={g:.0} gated-out ticks", thresh);
    }

    // KEY TEST: At highest gate, does FEP outperform?
    let fep_07 = all.iter().find(|(c, t, _)| *c == "FEP" && (*t - 0.7).abs() < 0.01);
    let well_07 = all.iter().find(|(c, t, _)| *c == "WELL" && (*t - 0.7).abs() < 0.01);
    if let (Some((_, _, fep_r)), Some((_, _, well_r))) = (fep_07, well_07) {
        ...
        eprintln!("  FEP={:.1}, WELL={:.1}", f_alive.iter().sum::<f64>() / f_alive.len() as f64,
