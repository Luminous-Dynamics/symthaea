...
fn main() {
    ...
    for _ in 0..ADVERSARIES {
        ...
        consciousness.register(h, consciousness.constants.initial_energy, consciousness.constants.harmony_range);
        if let Some(e) = consciousness.entities.get_mut(&h) { e.harmony_activations = adv_harmony; }
        adv_handles.push(h);
    }

    let all: Vec<_> = coop_handles.iter().chain(adv_handles.iter()).cloned().collect();
    let mut generation = 0usize;

    // Track: mean cooperator-cooperator resonance AND cooperator-adversary resonance
    println!("tick,gen,coop_alive,adv_alive,coop_coop_res,coop_adv_res,clustering,coop_energy,harm_var");
    ...
    for tick in 0..TICKS {
        ...
        // FEP + gravity
        let adata: Vec<_> = all.iter().filter_map(|&h| {
            Some((world.body(h)?.position().0, consciousness.entities.get(&h)?.harmony_activations))
        }).collect();
        let phi_data: Vec<_> = all.iter().filter_map(|&h| Some((world.body(h)?.position().0, consciousness.phi(h)))).collect();
        let wdata: Vec<_> = wells.iter().zip(well_remaining.iter())
            .filter(|(_, &r)| r > 0.0).map(|(&p, &r)| (p, (r / 10000.0).min(1.0))).collect();
        for &h in &all {
            ...
            let near: Vec<_> = adata.iter().filter(|(p, _)| { let d = (p-pos).norm(); d > 2.0 && d < consciousness.constants.harmony_range }).cloned().collect();
            let fep = fep_gradient::free_energy_gradient(&pos, e.energy.fraction_remaining(), &e.harmony_activations, &near, &wdata, None, 0.0);
            let grav_near: Vec<_> = phi_data.iter().filter(|(p,_)| (p-pos).norm() > 2.0).cloned().collect();
            let grav = fep_gradient::phi_gravity(&pos, e.phi(), &grav_near, GRAVITY_G);
            if let Some(b) = world.body_mut(h) { b.linear_velocity = fep * 20.0 + grav * DT; }
        }
        ...
        let nw: Vec<Option<usize>> = all.iter().map(|&h| {
        ...
        for (idx, &h) in all.iter().enumerate() {
            ...
            if let Some(e) = consciousness.entities.get_mut(&h) {
                ...
                if let Some(wi) = nw[idx] { let d = wr.min(well_remaining[wi]); e.energy.regenerate(d); well_remaining[wi] -= d; }
            }
        }
        ...
        for i in 0..all.len() {
            for j in (i+1)..all.len() {
                ...
                let in_range = match (world.body(ha), world.body(hb)) {
                    (Some(a), Some(b)) => a.position().distance(b.position()) < consciousness.constants.harmony_range, _ => false };
        ...
        for age in &mut coop_age { *age += 1; }

        // Reproduction (cooperators only — adversaries are fixed)
        if tick > 0 && tick % REPRO_INTERVAL == 0 {
            generation += 1;
            let alive: Vec<usize> = (0..COOPERATORS).filter(|&i|
                consciousness.entities.get(&coop_handles[i]).map(|e| !e.energy.is_collapsed()).unwrap_or(false)).collect();
            let dead: Vec<usize> = (0..COOPERATORS).filter(|&i|
                consciousness.entities.get(&coop_handles[i]).map(|e| e.energy.is_collapsed()).unwrap_or(true)).collect();
            ...
            if !alive.is_empty() && !dead.is_empty() {
                ...
                for &di in &dead {
                    ...
                    for k in 0..8 { child[k] = (child[k] + (rng_f64(&mut rng) - 0.5) * MUTATION * 2.0).clamp(0.0, 1.0); }
        ...
        if tick % 1000 == 0 {
            let ca = coop_handles.iter().filter(|h| consciousness.entities.get(h).map(|e| !e.energy.is_collapsed()).unwrap_or(false)).count();
            let aa = adv_handles.iter().filter(|h| consciousness.entities.get(h).map(|e| !e.energy.is_collapsed()).unwrap_or(false)).count();
            ...
            let ce = coop_handles.iter().filter_map(|h| consciousness.entities.get(h).map(|e| e.energy.available)).sum::<f64>() / COOPERATORS as f64;
            let hv = harm_var(&consciousness, &coop_handles);
            println!("{tick},{generation},{ca},{aa},{cc_res:.4},{ca_res:.4},{cl:.2},{ce:.1},{hv:.4}");
        }
    }
    ...
    eprintln!("  Coop-Coop resonance: {:.4} → {:.4} ({})", initial_cc, final_cc,
        if final_cc > initial_cc + 0.02 { "INCREASED — evolved solidarity" }
        else if final_cc < initial_cc - 0.02 { "DECREASED — fragmented" }
        else { "STABLE" });
    eprintln!("  Coop-Adv resonance:  {:.4} → {:.4} ({})", initial_ca, final_ca,
        if final_ca < initial_ca - 0.02 { "DECREASED — evolved resistance!" }
        else if final_ca > initial_ca + 0.02 { "INCREASED — assimilated" }
        else { "STABLE" });
    eprintln!("  Harmony variance:    {:.4}", harm_var(&consciousness, &coop_handles));
...
fn mean_resonance_between(c: &ConsciousnessField<2>, group_a: &[symtropy_physics::BodyHandle], group_b: &[symtropy_physics::BodyHandle]) -> f64 {
    let mut total = 0.0; let mut count = 0;
    for &ha in group_a {
        for &hb in group_b {
            if ha == hb { continue; }
            if let (Some(a), Some(b)) = (c.entities.get(&ha), c.entities.get(&hb)) {
                total += HarmonyField::<2>::resonance(&a.harmony_activations, &b.harmony_activations);
                count += 1;
            }
        }
    }
