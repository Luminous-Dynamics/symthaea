...
fn run_experiment(use_bonds: bool, seed: u64) -> BondResult {
    ...
    for tick in 0..TICKS {
        ...
        for &h in &handles {
            ...
            let inputs = if collapsed {
                ...
                ConsciousnessInputs {
                    phi: ef, broadcast: 0.6, working_memory: (1.0 - pe).max(0.0),
                    attention: 0.6, recurrence: 1.0, embodiment: 0.7,
        ...
        let wdata: Vec<_> = wells.iter().zip(well_remaining.iter())
            .filter(|(_, &r)| r > 0.0).map(|(&p, &r)| (p, (r / 2000.0).min(1.0))).collect();
        for &h in &handles {
        ...
        if use_bonds && tick % 50 == 0 {
            for i in 0..handles.len() {
                for j in (i+1)..handles.len() {
                    if active_bonds.iter().any(|&(a, b, active)| active && ((a == i && b == j) || (a == j && b == i))) {
                        continue;
                    }
                    ...
                    // Both must be alive
                    let a_alive = consciousness.entities.get(&ha).map(|e| !e.energy.is_collapsed()).unwrap_or(false);
                    let b_alive = consciousness.entities.get(&hb).map(|e| !e.energy.is_collapsed()).unwrap_or(false);
                    if !a_alive || !b_alive { continue; }

                    let (harm_a, harm_b) = match (consciousness.entities.get(&ha), consciousness.entities.get(&hb)) {
        ...
        if any_sanctuary { sanctuary_ticks += 1; }
    ...
    let mean_phi = if phi_count > 0 { phi_sum / phi_count as f64 } else { 0.0 };

    BondResult { alive, energy, clustering, bonds_formed: total_bonds_formed as f64,
        bonds_active_end: bonds_active, sanctuary_ticks: sanctuary_ticks as f64, mean_phi }
}
...
fn main() {
    ...
    println!("condition,seed,alive,energy,clustering,bonds_formed,bonds_active,sanctuary_ticks,mean_phi");
    ...
    for s in 0..SEEDS {
        ...
        println!("NO_BONDS,{seed},{:.1},{:.1},{:.2},{:.0},{:.0},{:.0},{:.4}", r.alive, r.energy, r.clustering, r.bonds_formed, r.bonds_active_end, r.sanctuary_ticks, r.mean_phi);
    ...
    for s in 0..SEEDS {
        ...
        println!("BONDS,{seed},{:.1},{:.1},{:.2},{:.0},{:.0},{:.0},{:.4}", r.alive, r.energy, r.clustering, r.bonds_formed, r.bonds_active_end, r.sanctuary_ticks, r.mean_phi);
        bond_results.push(r);
    }

    let n = SEEDS as f64;
    eprintln!("\n── NO_BONDS ──");
    eprintln!("  Alive:      {:.1}/{AGENTS}", nobond_results.iter().map(|r| r.alive).sum::<f64>() / n);
    eprintln!("  Energy:     {:.1} J", nobond_results.iter().map(|r| r.energy).sum::<f64>() / n);
    eprintln!("  Clustering: {:.2}", nobond_results.iter().map(|r| r.clustering).sum::<f64>() / n);
    eprintln!("  Mean Φ:     {:.4}", nobond_results.iter().map(|r| r.mean_phi).sum::<f64>() / n);
    eprintln!("  Sanctuary:  {:.0} ticks", nobond_results.iter().map(|r| r.sanctuary_ticks).sum::<f64>() / n);

    eprintln!("\n── BONDS ──");
    eprintln!("  Alive:      {:.1}/{AGENTS}", bond_results.iter().map(|r| r.alive).sum::<f64>() / n);
    eprintln!("  Energy:     {:.1} J", bond_results.iter().map(|r| r.energy).sum::<f64>() / n);
    eprintln!("  Clustering: {:.2}", bond_results.iter().map(|r| r.clustering).sum::<f64>() / n);
    eprintln!("  Bonds formed: {:.1}", bond_results.iter().map(|r| r.bonds_formed).sum::<f64>() / n);
    eprintln!("  Bonds active: {:.1}", bond_results.iter().map(|r| r.bonds_active_end).sum::<f64>() / n);
    eprintln!("  Mean Φ:     {:.4}", bond_results.iter().map(|r| r.mean_phi).sum::<f64>() / n);
    eprintln!("  Sanctuary:  {:.0} ticks", bond_results.iter().map(|r| r.sanctuary_ticks).sum::<f64>() / n);
    ...
    if !bond_results.is_empty() {
        ...
        eprintln!("  Bonds formed:   {:.1} ({:.1}%)", mean_bonds, mean_bonds / max_possible * 100.0);
    }
...
fn effect_label(d: f64) -> &'static str {
    if d.abs() > 0.8 { "large" } else if d.abs() > 0.5 { "medium" } else { "small" }
}
