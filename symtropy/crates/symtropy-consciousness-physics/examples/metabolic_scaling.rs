...
fn run(n: usize, social: bool, seed: u64) -> (f64, f64) {
    ...
    consciousness.constants = ThermodynamicConstants {
        ...
        harmony_resonance_regen_rate: 0.12, energy_well_regen_rate: 0.0, // no wells — pure cooperation test
    ...
    let total_consumed: f64 = handles.iter()
        .filter_map(|h| consciousness.entities.get(h).map(|e| e.energy.lifetime_consumed))
        .sum();
...
fn main() {
    ...
    for &social in &[true, false] {
        ...
        for &n in &populations {
            ...
            println!("{},{},{:.2},{:.2},{:.4},{:.4}",
                cond, n, avg_total, avg_pc,
                (n as f64).ln(), avg_pc.ln());
        }
    }
    ...
    for &social in &[true, false] {
        ...
        let r_sq = {
            ...
            let ss_res: f64 = ln_n.iter().zip(&ln_pc).map(|(x,y)| {
