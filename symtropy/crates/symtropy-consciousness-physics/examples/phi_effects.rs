...
#[derive(Clone, Copy)]
enum Metric { Phi, Entropy, Random, Constant, Zero }
impl Metric {
    fn name(&self) -> &'static str {
        match self { Self::Phi=>"PHI", Self::Entropy=>"ENTROPY", Self::Random=>"RANDOM",
                     Self::Constant=>"CONST", Self::Zero=>"ZERO" }
    }
    fn value(&self, ef: f64, tick: u64, idx: usize) -> f64 {
        match self {
            Self::Phi => 0.5,
            Self::Entropy => { let p=ef.clamp(0.01,0.99); -(p*p.ln()+(1.0-p)*(1.0-p).ln())/(2.0f64).ln() }
            Self::Random => { let mut s=tick.wrapping_mul(6364136223846793005).wrapping_add(idx as u64*1442695040888963407);
                s=s.wrapping_mul(6364136223846793005).wrapping_add(1); (s>>11) as f64/(1u64<<53) as f64 }
            Self::Constant => 0.5,
            Self::Zero => 0.0,
        }
    }
...
fn run(metric: Metric, seed: u64) -> Results {
    ...
    let area = 120.0;
    for i in 0..AGENTS {
        let x = (nr(&mut rng)-0.5)*area; let y = (nr(&mut rng)-0.5)*area;
    ...
    for tick in 0..TICKS {
        ...
        for (idx, &h) in handles.iter().enumerate() {
            ...
            let inputs = ConsciousnessInputs { phi: phi_val, broadcast: 0.6, working_memory: 0.5,
                attention: 0.5, recurrence: 0.4, embodiment: 0.6, knowledge: 0.4, synchrony: 0.5 };
        ...
        for (idx, &h) in handles.iter().enumerate() {
            ...
            let ds = if danger > 0.1 { Some(&danger_pos) } else { None };
        ...
        // 1. Survival ticks
        for (idx, &h) in handles.iter().enumerate() {
            if !consciousness.entities.get(&h).map(|e| e.energy.is_collapsed()).unwrap_or(true) {
                alive_ticks[idx] += 1;
            }
        }
        ...
        if tick % 10 == 0 {
            let mut stable = 0;
            for i in 0..handles.len() {
                let mut nn = 0; let mut mnd = f64::MAX;
                for j in 0..handles.len() { if i==j {continue;}
                    if let (Some(a),Some(b)) = (world.body(handles[i]),world.body(handles[j])) {
                        let d = a.position().distance(b.position()); if d < mnd { mnd=d; nn=j; }
                    }
                }
                if nn == prev_nn[i] { stable += 1; }
                prev_nn[i] = nn;
            }
            if tick > 10 { cluster_stable_ticks += stable as u64; }
        }
            ...
            for i in 0..handles.len() {
                for j in (i+1)..handles.len() {
                    if let (Some(a), Some(b)) = (world.body(handles[i]), world.body(handles[j])) {
                        ...
                        if dist < 40.0 {
                            let va = a.linear_velocity; let vb = b.linear_velocity;
                            let na = va.norm(); let nb = vb.norm();
                            if na > 0.1 && nb > 0.1 {
    ...
    let avg_vv: f64 = velocity_variances.iter()
        .filter(|v| !v.is_empty())
        .map(|v| v.iter().sum::<f64>() / v.len() as f64)
        .sum::<f64>() / AGENTS as f64;
    let total_stability_samples = ((TICKS / 10) - 1) as f64 * AGENTS as f64;
    let cluster_pers = if total_stability_samples > 0.0 { cluster_stable_ticks as f64 / total_stability_samples } else { 0.0 };

    // 4. Post-danger survival
    let post_danger_alive = handles.iter().filter(|h| {
        consciousness.entities.get(h).map(|e| !e.energy.is_collapsed()).unwrap_or(false)
    }).count();

    let avg_corr = if correlation_samples > 0 { total_vel_correlation / correlation_samples as f64 } else { 0.0 };
...
fn main() {
    ...
    println!("╚════════════════════════════════════════════════════════════╝\n");
    println!("{} agents, {} ticks, {} seeds, danger injected at tick {}\n",
        AGENTS, TICKS, SEEDS, DANGER_START_TICK);

    let metrics = [Metric::Phi, Metric::Entropy, Metric::Random, Metric::Constant, Metric::Zero];
    ...
    for m in &metrics {
        let mut rs = Vec::new();
        for s in 0..SEEDS { rs.push(run(*m, 42 + s as u64 * 997)); }
        let n = SEEDS as f64;
        ...
        println!("{:<10} {:<12.0} {:<12.4} {:<12.3} {:<12.1}% {:<12.4}",
            m.name(), surv, vv, cs, pd * 100.0, vc);
    }
    ...
    println!("POST DANGER: % alive after danger injection at tick {}", DANGER_START_TICK);
