...
fn run_experiment(ctrl: Ctrl, threshold: usize, seed: u64) -> CoopWellResult {
    ...
    for _tick in 0..TICKS {
        ...
        let well_counts: Vec<usize> = wells.iter().map(|w| {
    ...
    CoopWellResult {
        controller: ctrl_name, threshold, alive, energy,
        cooperation: coop as f64, well_unlocks: well_unlock_ticks as f64,
    }
}
...
fn main() {
    ...
    for &thresh in &THRESHOLDS {
        let w = all.iter().find(|(c, t, _)| *c == "WELL" && *t == thresh)
            .map(|(_, _, r)| r.iter().map(|r| r.alive).sum::<f64>() / r.len() as f64).unwrap_or(0.0);
        let f = all.iter().find(|(c, t, _)| *c == "FEP" && *t == thresh)
            .map(|(_, _, r)| r.iter().map(|r| r.alive).sum::<f64>() / r.len() as f64).unwrap_or(0.0);
        let g = all.iter().find(|(c, t, _)| *c == "GREEDY" && *t == thresh)
            .map(|(_, _, r)| r.iter().map(|r| r.alive).sum::<f64>() / r.len() as f64).unwrap_or(0.0);
        let fep_wins = f > w + 1.0 && f > g + 1.0;
        eprintln!("  {:5}       {:5.1}    {:5.1}    {:5.1}    {}",
            thresh, w, f, g, if fep_wins { "← YES" } else { "" });
    }
    ...
    for &thresh in &THRESHOLDS {
        let w_unlocks = all.iter().find(|(c, t, _)| *c == "WELL" && *t == thresh)
            .map(|(_, _, r)| r.iter().map(|r| r.well_unlocks).sum::<f64>() / r.len() as f64).unwrap_or(0.0);
        let f_unlocks = all.iter().find(|(c, t, _)| *c == "FEP" && *t == thresh)
            .map(|(_, _, r)| r.iter().map(|r| r.well_unlocks).sum::<f64>() / r.len() as f64).unwrap_or(0.0);
        let g_unlocks = all.iter().find(|(c, t, _)| *c == "GREEDY" && *t == thresh)
            .map(|(_, _, r)| r.iter().map(|r| r.well_unlocks).sum::<f64>() / r.len() as f64).unwrap_or(0.0);
        eprintln!("  thresh={thresh}: WELL={w_unlocks:.0}, FEP={f_unlocks:.0}, GREEDY={g_unlocks:.0} unlock-ticks (of {TICKS})");
    }
