...
fn run(with_leakage: bool, seed: u64) -> Result {
    ...
    consciousness.constants = ThermodynamicConstants {
        ...
        ambient_regen_rate: 0.02, collapse_recovery_harmony_threshold: 0.5,
        harmony_range: 40.0,
    };
    ...
    for i in 0..AGENTS {
        ...
        if let Some(b) = world.body_mut(h) {
            ...
            b.linear_velocity = SVector::from([(nr(&mut rng)-0.5)*10.0, (nr(&mut rng)-0.5)*10.0]);
        }
    ...
    for _tick in 0..TICKS {
        ...
        let ad: Vec<_> = handles.iter().filter_map(|&h| {
    ...
    let avg_pe = handles.iter().filter_map(|h| consciousness.entities.get(h).map(|e| e.prediction_error)).sum::<f64>() / AGENTS as f64;
    ...
    Result {
        ...
        alive, avg_energy: ae, avg_pred_error: avg_pe,
...
fn main() {
    ...
    for (name, leakage) in &[("NORMAL", false), ("LEAKAGE", true)] {
        let mut cs=Vec::new(); let mut al=Vec::new(); let mut en=Vec::new();
        let mut pe=Vec::new(); let mut ns=Vec::new(); let mut nk=Vec::new(); let mut de=Vec::new();

        for s in 0..SEEDS {
            let r = run(*leakage, 42 + s as u64 * 997);
            cs.push(r.clustering); al.push(r.alive as f64); en.push(r.avg_energy);
            pe.push(r.avg_pred_error); ns.push(r.agents_near_source);
            nk.push(r.agents_near_sink); de.push(r.dark_energy);
        }

        let n = SEEDS as f64;
        println!("{:<12} {:<10.2} {:<8.1} {:<10.1} {:<10.4} {:<10.1}% {:<10.1}% {:<10.1}",
            name,
            cs.iter().sum::<f64>()/n,
            al.iter().sum::<f64>()/n,
            en.iter().sum::<f64>()/n,
