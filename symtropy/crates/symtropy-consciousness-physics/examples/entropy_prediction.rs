...
fn run_experiment(seed: u64) -> Vec<AgentSnapshot> {
        ...
        if tick % 100 == 0 {
            for (idx, &h) in handles.iter().enumerate() {
                if let Some(e) = consciousness.entities.get(&h) {
                    ...
                    snapshots_raw.push((tick, idx, e.energy.available, e.energy.entropy,
                        e.energy.temperature, e.energy.available_work(), e.phi(), alive));
                }
            }
        }
    ...
    snapshots_raw.iter().map(|&(tick, agent, energy, entropy, temp, aw, phi, alive)| {
        let will_collapse = collapse_tick[agent].map(|ct| ct > tick && ct <= tick + 500).unwrap_or(false);
        AgentSnapshot { tick, agent, energy, entropy, temperature: temp,
            available_work: aw, phi, alive, collapsed_within_500: will_collapse }
    }).collect()
}
...
fn main() {
    ...
    println!("seed,tick,agent,energy,entropy,temperature,available_work,phi,alive,collapse_within_500");
    ...
    for s in 0..NUM_SEEDS {
        ...
        let seed_doomed = snapshots.iter().filter(|s| s.collapsed_within_500 && s.alive).count();

        for snap in &snapshots {
            println!("{seed},{},{},{:.2},{:.6},{:.2},{:.2},{:.4},{},{}",
                snap.tick, snap.agent, snap.energy, snap.entropy, snap.temperature,
                snap.available_work, snap.phi, snap.alive, snap.collapsed_within_500);
    ...
    eprintln!("\n── Prediction Analysis ──");
    eprintln!("  Total snapshots: alive={}, doomed={}", all_alive_aw.len(), all_doomed_aw.len());
    eprintln!("  Collapse rate: {:.1}%", total_collapsed as f64 / (total_agents * (TICKS / 100)) as f64 * 100.0);
    ...
    if !all_doomed_aw.is_empty() && !all_alive_aw.is_empty() {
        ...
        eprintln!("  Separation: {:.2} J ({:.1}x)", mean_alive - mean_doomed, mean_alive / mean_doomed.max(0.01));
        ...
        for &thresh in &thresholds {
            ...
            eprintln!("  {thresh:>6.0} J    {sensitivity:.3}        {specificity:.3}        {accuracy:.3}");
        }
        ...
        if all_doomed_aw.is_empty() { eprintln!("  (No agents collapsed within prediction window)"); }
    }
