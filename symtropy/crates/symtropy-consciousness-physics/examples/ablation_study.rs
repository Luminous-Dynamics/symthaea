...
fn run(cond: &Condition, seed: u64) -> Result {
    ...
    for tick in 0..TICKS {
        ...
        if cond.fep_gradient {
            let agent_data: Vec<_> = handles.iter()
                .filter_map(|&h| {
            ...
            let well_data: Vec<_> = well_positions.iter()
                .map(|wp| (*wp, 1.0f64))
                .collect();
            ...
            for (idx, &h) in handles.iter().enumerate() {
                ...
                let dir = fep_gradient::free_energy_gradient(
                    &pos, energy_frac, &harmony, &nearby, &well_data, None, 0.0,
                );
                if let Some(body) = world.body_mut(h) {
        ...
        if cond.energy_costs {
            ...
            // Check well sharing
            for wp in &well_positions {
                let agents_at_well = handles.iter()
                    .filter(|&&h| {
                        world.body(h).map(|b| (b.position().0 - wp).norm() < 40.0).unwrap_or(false)
                    })
                    .count();
                if agents_at_well >= 2 { well_sharing += 1; }
            }
        }
...
fn main() {
    let conditions = vec![
        Condition { name: "FREE        ", energy_costs: false, offloading: false, fep_gradient: false },
        Condition { name: "ENERGY_ONLY ", energy_costs: true,  offloading: false, fep_gradient: false },
        Condition { name: "E+OFFLOAD   ", energy_costs: true,  offloading: true,  fep_gradient: false },
        Condition { name: "E+GRADIENT  ", energy_costs: true,  offloading: false, fep_gradient: true  },
        Condition { name: "E+OFF+RAND  ", energy_costs: true,  offloading: true,  fep_gradient: false },
        Condition { name: "FULL        ", energy_costs: true,  offloading: true,  fep_gradient: true  },
    ];

    println!("╔═══════════════════════════════════════════════════════════════════════════════════════╗");
    println!("║                        ABLATION STUDY: What Causes Cooperation?                      ║");
    println!("╚═══════════════════════════════════════════════════════════════════════════════════════╝");
    println!("\n{} agents, {} ticks, {} seeds, 2 energy wells\n", AGENTS, TICKS, SEEDS.len());
    ...
    for cond in &conditions {
        ...
        let avg_coop = results.iter().map(|r| r.cooperation_ticks as f64).sum::<f64>() / n;
        let avg_well = results.iter().map(|r| r.well_sharing_events as f64).sum::<f64>() / n;

        println!("{:<14} {:<10.1} {:<10.0} {:<10.1}% {:<10.1} {:<10.0} {:<10.0}",
            cond.name, avg_cluster, avg_survive, avg_collapse * 100.0, avg_energy, avg_coop, avg_well);
    }
