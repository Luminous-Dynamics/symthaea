...
fn run_experiment(curvature_scale: f64, seed: u64) -> CurvResult {
    ...
    let cond_name = if curvature_scale < 1e-6 { "FLAT" }
        else if curvature_scale < 0.02 { "LOW_CURV" }
        else { "HIGH_CURV" };
    ...
    for _tick in 0..TICKS {
        ...
        for &h in &handles {
            ...
            let mut vel = fep_gradient::free_energy_gradient(&pos, e.energy.fraction_remaining(),
            ...
            if curvature_scale > 1e-6 {
                ...
                let harmony_energy: f64 = near.iter().map(|(p, h)| {
    ...
    CurvResult { condition: cond_name, alive, energy, clustering, cooperation: coop as f64 }
}
...
fn main() {
    ...
    for &(scale, name) in &scales {
        #[cfg(not(feature = "consciousness-curvature"))]
        if scale > 0.0 { eprintln!("  Skipping {name} (no curvature feature)"); continue; }
        ...
        for s in 0..SEEDS {
            ...
            println!("{},{seed},{:.1},{:.1},{:.2},{:.0}", r.condition, r.alive, r.energy, r.clustering, r.cooperation);
            results.push(r);
        }
        let n = results.len() as f64;
        eprintln!("  → {name}: alive={:.1}, cluster={:.2}",
    ...
    if all_results.len() >= 2 {
        ...
        for (i, &(label, adj_p, sig)) in corrected.iter().enumerate() {
            ...
            eprintln!("  vs {:10}: p_adj={adj_p:.4}, d={d:.3} ({size}) {}", label, if sig { "← SIG" } else { "" });
        }
    }
