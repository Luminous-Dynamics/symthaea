...
fn run_benchmark(n_agents: usize, use_spatial_hash: bool) -> (f64, f64, usize) {
    ...
    let wells = vec![
        SVector::from([50.0, 0.0]), SVector::from([-50.0, 0.0]),
        SVector::from([0.0, 50.0]), SVector::from([0.0, -50.0]),
    ];
    ...
    for _tick in 0..TICKS {
        ...
        for (idx, &h) in handles.iter().enumerate() {
            ...
            let near: Vec<_> = if use_spatial_hash {
                spatial.query_neighbors(&pos).into_iter()
                    .filter(|&j| j != idx)
                    .filter_map(|j| {
                        let (p, h) = &adata[j];
                        let d = (p - pos).norm();
        ...
        if use_spatial_hash {
            for (idx, &h) in handles.iter().enumerate() {
                ...
                for &j in &spatial.query_neighbors(&pos_a) {
                    if j <= idx { continue; } // avoid double-counting
                    let hb = handles[j];
...
fn main() {
    ...
    for &n in &POPULATIONS {
        if n > 500 { eprintln!("  N={n} skipped (too slow for brute force)"); continue; }
        eprintln!("  N={n}...");
