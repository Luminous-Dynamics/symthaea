...
fn main() {
    ...
    println!("tick,generation,alive,mean_resonance,mean_phi,clustering,mean_energy,harmony_variance");
    ...
    for tick in 0..TICKS {
        ...
        let phi_data: Vec<_> = handles.iter().filter_map(|&h| {
            Some((world.body(h)?.position().0, consciousness.phi(h)))
        }).collect();
        let wdata: Vec<_> = wells.iter().zip(well_remaining.iter())
            .filter(|(_, &r)| r > 0.0).map(|(&p, &r)| (p, (r / 8000.0).min(1.0))).collect();
        ...
        for (idx, &h) in handles.iter().enumerate() {
            ...
            let fep_dir = fep_gradient::free_energy_gradient(&pos, e.energy.fraction_remaining(),
                &e.harmony_activations, &near, &wdata, None, 0.0);

            // Φ-gravity (gravitational attraction to high-Φ agents)
            let gravity_agents: Vec<_> = phi_data.iter()
                .filter(|(p, _)| (p - pos).norm() > 2.0)
                .cloned().collect();
            let gravity = fep_gradient::phi_gravity(&pos, self_phi, &gravity_agents, GRAVITY_CONSTANT);
        ...
        // Age all agents
        for age in &mut agent_age { *age += 1; }
        ...
        if tick > 0 && tick % REPRODUCTION_INTERVAL == 0 {
            ...
            if !alive_indices.is_empty() && !dead_indices.is_empty() {
                ...
                eprintln!("  Gen {generation} (tick {tick}): {}/{AGENTS} alive, {} reproduced",
                    alive_indices.len(), dead_indices.len());
            }
        }
        ...
        if tick % 500 == 0 {
            ...
            let mean_energy = handles.iter().filter_map(|h| consciousness.entities.get(h).map(|e| e.energy.available)).sum::<f64>() / AGENTS as f64;
    ...
    eprintln!("  Final mean resonance: {:.4} (started ~{:.1})", final_res, initial_expected_res);
    eprintln!("  Generations: {generation}");
    eprintln!("  Final harmony variance: {:.4}", compute_harmony_variance(&consciousness, &handles));
