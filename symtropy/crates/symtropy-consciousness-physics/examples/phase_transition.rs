...
fn run_experiment(maintenance: f64, seed: u64) -> PhaseResult {
    ...
    let mean_temp = if temp_count > 0 { temp_sum / temp_count as f64 } else { 310.0 };
    let total_entropy: f64 = handles.iter().filter_map(|h| consciousness.entities.get(h).map(|e| e.energy.entropy)).sum();
    let helmholtz: f64 = handles.iter().filter_map(|h| consciousness.entities.get(h).map(|e| e.energy.available_work())).sum::<f64>() / AGENTS as f64;

    PhaseResult {
        pressure: maintenance, alive, energy, clustering,
        cooperation_events: cooperation_events as f64,
        mean_temperature: mean_temp, total_entropy: total_entropy,
        helmholtz,
    }
}
...
fn main() {
    ...
    eprintln!("Sweeping {} maintenance pressures from {:.2} to {:.2} J/tick",
    ...
    for &pressure in &PRESSURES {
        ...
        for s in 0..SEEDS {
            ...
            println!("{:.2},{seed},{:.1},{:.1},{:.2},{:.0},{:.1},{:.2},{:.1}",
                r.pressure, r.alive, r.energy, r.clustering,
                r.cooperation_events, r.mean_temperature, r.total_entropy, r.helmholtz);
            results.push(r);
        }
    ...
    if all_results.len() >= 2 {
        ...
        eprintln!("  Cohen's d={d3:.3} ({})", if d3.abs() > 0.8 { "large" } else if d3.abs() > 0.5 { "medium" } else { "small" });
    }
