...
fn run_experiment(active_dims: usize, seed: u64) -> DimResult {
    ...
    let mean_res = if resonances.is_empty() { 0.0 } else { resonances.iter().sum::<f64>() / resonances.len() as f64 };
    let var_res = if resonances.len() < 2 { 0.0 } else {
        resonances.iter().map(|r| (r - mean_res).powi(2)).sum::<f64>() / (resonances.len() - 1) as f64
    };

    DimResult {
        dim: active_dims, alive,
