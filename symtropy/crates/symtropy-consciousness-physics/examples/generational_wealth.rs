...
fn run_experiment(seed: u64) -> WealthResult {
    ...
    let initial_well_proximity: Vec<f64> = handles.iter().map(|&h| {
        world.body(h).map(|b| {
            let pos = b.position().0;
            wells.iter().map(|w| (pos - w).norm()).fold(f64::MAX, f64::min)
        }).unwrap_or(f64::MAX)
    }).collect();
    let mut sorted_prox: Vec<(usize, f64)> = initial_well_proximity.iter().enumerate().map(|(i, &d)| (i, d)).collect();
    ...
    for tick in 0..TICKS {
        ...
        let wdata: Vec<_> = wells.iter().zip(well_remaining.iter())
            .filter(|(_, &r)| r > 0.0).map(|(&p, &r)| (p, (r / 6000.0).min(1.0))).collect();
        for &h in &handles {
        ...
        // Gini sampling
        if tick % 2000 == 0 {
            let energies: Vec<f64> = handles.iter().map(|h| {
                consciousness.entities.get(h).map(|e| e.energy.available).unwrap_or(0.0)
            }).collect();
        ...
        if tick > 0 && tick % REPRODUCTION_INTERVAL == 0 {
            ...
            let alive_idx: Vec<usize> = (0..handles.len())
                .filter(|&i| consciousness.entities.get(&handles[i]).map(|e| !e.energy.is_collapsed()).unwrap_or(false))
                .collect();
            let dead_idx: Vec<usize> = (0..handles.len())
                .filter(|&i| consciousness.entities.get(&handles[i]).map(|e| e.energy.is_collapsed()).unwrap_or(true))
                .collect();

            if !alive_idx.is_empty() && !dead_idx.is_empty() {
                for &di in &dead_idx {
                    // Fittest parent = most energy
                    let parent = *alive_idx.iter().max_by(|&&a, &&b| {
                        let ea = consciousness.entities.get(&handles[a]).map(|e| e.energy.available).unwrap_or(0.0);
                        let eb = consciousness.entities.get(&handles[b]).map(|e| e.energy.available).unwrap_or(0.0);
                    ...
                    if let Some(parent_body) = world.body(handles[parent]) {
                        ...
                        if let Some(b) = world.body_mut(h) {
                            let offset = SVector::from([(rng_f64(&mut rng) - 0.5) * 10.0, (rng_f64(&mut rng) - 0.5) * 10.0]);
    ...
    let final_alive = handles.iter().filter(|h| consciousness.entities.get(h).map(|e| !e.energy.is_collapsed()).unwrap_or(false)).count() as f64;

    // Dynasty fraction: what % of final population descends from initial top-25% (well-adjacent)?
    let dynasty_count = agent_lineage.iter().filter(|&&l| top_founders.contains(&l)).count() as f64;
    ...
    let mean_resonance = if res_count > 0 { res_total / res_count as f64 } else { 0.0 };

    let gini_early = if gini_samples.len() >= 3 { (gini_samples[1] + gini_samples[2]) / 2.0 } else { 0.0 };
    let gini_late = *gini_samples.last().unwrap_or(&0.0);

    WealthResult {
        generations: generation as f64,
        final_alive,
        gini_early, gini_late,
        dynasty_fraction,
        mean_resonance,
        lineage_diversity,
    }
...
fn main() {
    ...
    eprintln!("{AGENTS} agents, {TICKS} ticks, {SEEDS} seeds, reproduction every {REPRODUCTION_INTERVAL}");
    ...
    for s in 0..SEEDS {
        ...
        println!("{seed},{:.0},{:.1},{:.4},{:.4},{:.3},{:.4},{:.0}",
            r.generations, r.final_alive, r.gini_early, r.gini_late,
            r.dynasty_fraction, r.mean_resonance, r.lineage_diversity);
        results.push(r);
    }

    let n = results.len() as f64;
    eprintln!("\n── Generational Wealth ──");
    eprintln!("  Generations:    {:.0}", results.iter().map(|r| r.generations).sum::<f64>() / n);
    eprintln!("  Final alive:    {:.1}/{AGENTS}", results.iter().map(|r| r.final_alive).sum::<f64>() / n);
    eprintln!("  Gini (early):   {:.3}", results.iter().map(|r| r.gini_early).sum::<f64>() / n);
    eprintln!("  Gini (late):    {:.3}", results.iter().map(|r| r.gini_late).sum::<f64>() / n);
    eprintln!("  Dynasty frac:   {:.1}% (descendants of initial top-25%)", results.iter().map(|r| r.dynasty_fraction).sum::<f64>() / n * 100.0);
    eprintln!("  Lineage diversity: {:.1} unique lineages (of {AGENTS} original)", results.iter().map(|r| r.lineage_diversity).sum::<f64>() / n);
    eprintln!("  Mean resonance: {:.4}", results.iter().map(|r| r.mean_resonance).sum::<f64>() / n);
