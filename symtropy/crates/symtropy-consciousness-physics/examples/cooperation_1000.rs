...
fn run_experiment(n_agents: usize, seed: u64) -> ScaleResult {
    ...
    for _tick in 0..TICKS {
        ...
        for (idx, &h) in handles.iter().enumerate() {
            ...
            let near: Vec<_> = spatial.query_neighbors(&pos).into_iter()
                .filter(|&j| j != idx)
                .filter_map(|j| {
                    if j < adata.len() {
                        let (p, h) = &adata[j];
        ...
        for (idx, &h) in handles.iter().enumerate() {
            let Some(e_a) = consciousness.entities.get(&h) else { continue };
            let harm_a = e_a.harmony_activations;
            let pos_a = match world.body(h) { Some(b) => b.position().0, None => continue };

            for &j in &spatial.query_neighbors(&pos_a) {
                if j <= idx { continue; }
                let hb = handles[j];
                let in_range = match world.body(hb) {
                    Some(b) => (b.position().0 - pos_a).norm() < consciousness.constants.harmony_range,
                    None => false,
                };
                if !in_range { continue; }
                let harm_b = match consciousness.entities.get(&hb) {
                    Some(e) => e.harmony_activations, None => continue,
                };
                let res = HarmonyField::<2>::resonance(&harm_a, &harm_b);
                if res > 0.5 {
                    let rg = consciousness.constants.harmony_resonance_regen_rate * (res - 0.5) * 2.0;
                    if let Some(e) = consciousness.entities.get_mut(&h) { e.energy.regenerate(rg); }
    ...
    ScaleResult {
        n: n_agents, alive, survival_rate: alive / n_agents as f64,
        cooperation: coop as f64, coop_per_agent: coop as f64 / n_agents as f64,
        num_clusters: clusters.len() as f64,
        mean_cluster_size: if clusters.is_empty() { 0.0 } else {
...
fn find_clusters(positions: &[(usize, SVector<f64, 2>)], range: f64) -> Vec<Vec<usize>> {
    ...
    for &(i, ref pos) in positions { hash.insert(i, pos); }
...
fn main() {
    ...
    for &n in &POPULATIONS {
        ...
        for s in 0..SEEDS {
            ...
            println!("{},{seed},{:.0},{:.3},{:.0},{:.0},{:.0},{:.1},{:.0}",
                r.n, r.alive, r.survival_rate, r.cooperation, r.coop_per_agent,
                r.num_clusters, r.mean_cluster_size, r.wall_ms);
            eprintln!("    {:.0}% alive, {:.0} clusters (mean size {:.1}), {:.0}s",
                r.survival_rate * 100.0, r.num_clusters, r.mean_cluster_size, r.wall_ms / 1000.0);
            results.push(r);
        }
    ...
    for (n, results) in &all_results {
        ...
        eprintln!("  {:5}  {:5.0}%    {:5.1}     {:6.1}    {:8.0}    {:5.0}s",
            n,
    ...
    if all_results.len() >= 2 {
        ...
        eprintln!("  Cohen's d = {d:.3} ({})", if d.abs() > 0.8 { "large — scales with N" } else { "small — Dunbar limit holds" });
    }
