...
fn run_condition(
    maintenance: f64, range: f64, regen: f64, ambient: f64,
    well_dist: f64, well_cap: f64, arena: f64,
    seed: u64,
) -> FalsifyResult {
    ...
    for _tick in 0..TICKS {
        ...
        let wdata: Vec<_> = wells.iter().zip(well_remaining.iter())
            .filter(|(_, &r)| r > 0.0).map(|(&p, &r)| (p, (r / well_cap).min(1.0))).collect();
        for &h in &handles {
    ...
    FalsifyResult { condition: "", alive, cooperation: coop as f64, clustering }
}
...
fn main() {
    ...
    for &(maint, range, regen, ambient, well_d, well_c, arena, name) in &conditions {
        ...
        for s in 0..SEEDS {
            ...
            r.condition = name;
            println!("{name},{seed},{:.1},{:.0},{:.2}", r.alive, r.cooperation, r.clustering);
            results.push(r);
        }
        let n = results.len() as f64;
        eprintln!("  → {name}: alive={:.1}/{AGENTS}, coop={:.0}",
    ...
    for (name, results) in &all_results {
        ...
        let verdict = if alive < 1.0 { "DEAD" }
            else if coop < 100.0 { "NO COOP" }
            else if alive < AGENTS as f64 * 0.3 { "struggling" }
            else { "cooperating" };
        eprintln!("  {:10} {:5.1}    {:9.0}   {:6.2}      {verdict}", name, alive, coop, clust);
    }

    // Did we kill cooperation?
    let impossible = &all_results.last().unwrap().1;
    let impossible_alive: f64 = impossible.iter().map(|r| r.alive).sum::<f64>() / impossible.len() as f64;
    let impossible_coop: f64 = impossible.iter().map(|r| r.cooperation).sum::<f64>() / impossible.len() as f64;

    eprintln!("\n── Verdict ──");
    if impossible_alive < 1.0 && impossible_coop < 100.0 {
        eprintln!("  FALSIFIED: Cooperation completely fails under IMPOSSIBLE conditions.");
        eprintln!("  The engine's cooperation is NOT universal — it requires minimum range and regen.");
    } else if impossible_alive < AGENTS as f64 * 0.2 {
        eprintln!("  NEARLY FALSIFIED: Most agents die under extreme conditions ({:.0}% survive).", impossible_alive / AGENTS as f64 * 100.0);
