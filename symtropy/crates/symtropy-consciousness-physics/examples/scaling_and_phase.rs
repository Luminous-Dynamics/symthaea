...
struct RunResult { clustering: f64, alive_frac: f64, avg_energy: f64 }
...
fn run_sim(n_agents: usize, maint_cost: f64, area_size: f64, seed: u64) -> RunResult {
    ...
    consciousness.constants = ThermodynamicConstants {
        ...
        consciousness_maintenance_per_tick: maint_cost, movement_cost_per_unit: 0.008,
    ...
    let ae = handles.iter().filter_map(|h| consciousness.entities.get(h).map(|e| e.energy.available)).sum::<f64>() / n_agents as f64;
    let mut tn = 0.0;
    for i in 0..handles.len() {
        let mut md = f64::MAX;
    ...
    RunResult { clustering: tn / n_agents as f64, alive_frac: alive as f64 / n_agents as f64, avg_energy: ae }
}
...
fn main() {
    ...
    println!("{:<8} {:<12} {:<8} {:<12} {:<12}", "N", "Clustering", "±CI", "Alive%", "Energy");
    println!("{}", "─".repeat(52));
    ...
    for &n in &populations {
        ...
        let sc = (clusterings.iter().map(|x| (x-mc).powi(2)).sum::<f64>() / (seeds_per as f64 - 1.0)).sqrt();
        ...
        println!("{:<8} {:<12.2} ±{:<6.2} {:<12.1}% {:<12.1}", n, mc, ci, ma * 100.0, me);
    }
    ...
    println!("{:<10} {}", "Cost\\Dens", densities.iter().map(|d| format!("{:<10}", d)).collect::<String>());
    println!("{}", "─".repeat(50));
    ...
    for &cost in &costs {
        ...
        for &n in &densities {
            ...
            let phase = if avg_alive > 0.8 { "C" } else if avg_alive > 0.2 { "P" } else { "X" };
