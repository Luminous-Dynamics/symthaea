...
#[derive(Clone, Copy)]
enum CommMode { Silent, EnergyBroadcast, WellBroadcast }
...
fn run_experiment(mode: CommMode, seed: u64) -> CommResult {
    ...
    for _tick in 0..TICKS {
        ...
        if use_well_broadcast {
            ...
            let agent_wells: Vec<Vec<SVector<f64, 2>>> = handles.iter().map(|h| {
                consciousness.entities.get(h).map(|e| e.memory.known_wells.clone()).unwrap_or_default()
            }).collect();
            // Share with neighbors
            for i in 0..handles.len() {
                let pos_i = match world.body(handles[i]) { Some(b) => b.position().0, None => continue };
                for j in 0..handles.len() {
                    if i == j { continue; }
                    let pos_j = match world.body(handles[j]) { Some(b) => b.position().0, None => continue };
                    if (pos_i - pos_j).norm() > consciousness.constants.harmony_range { continue; }
        ...
        for (idx, &h) in handles.iter().enumerate() {
            ...
            let mut wdata: Vec<_> = wells.iter().zip(well_remaining.iter())
            ...
            let mut near: Vec<_> = adata.iter().filter(|(p, _)| {
            ...
            let mut dir = fep_gradient::free_energy_gradient(&pos, e.energy.fraction_remaining(),
            ...
            if use_energy_broadcast {
                ...
                for (other_pos, other_ef) in &energy_states {
                    ...
                    let dist = delta.norm();
                    if dist < 2.0 || dist > consciousness.constants.harmony_range { continue; }
                    if *other_ef < 0.3 { // help agents below 30% energy
    ...
    let wells_known = handles.iter().filter_map(|h| consciousness.entities.get(h).map(|e| e.memory.known_wells.len() as f64)).sum::<f64>() / AGENTS as f64;

    CommResult { condition: cond_name, alive, energy, clustering, cooperation: coop as f64, wells_known }
}
...
fn main() {
    ...
    for (name, results) in &all {
        let n = results.len() as f64;
        eprintln!("  {:10} {:5.1}  {:5.1}J  {:6.2}   {:8.0}   {:4.1}",
            name,
            results.iter().map(|r| r.alive).sum::<f64>() / n,
    ...
    let tests = vec![
        ("SILENT vs ENERGY_BC", { let (_, _, p) = mann_whitney_u(&s_alive, &e_alive); p }),
        ("SILENT vs WELL_BC", { let (_, _, p) = mann_whitney_u(&s_alive, &w_alive); p }),
    ];
    ...
    for (i, &(label, adj_p, sig)) in corrected.iter().enumerate() {
        ...
        eprintln!("  {label:22}: p_adj={adj_p:.4}, d={d:.3} ({size}) {}", if sig { "← SIG" } else { "" });
    }
