...
fn run_experiment(seed: u64) -> MigrationResult {
    ...
    let mut prev_positions: Vec<SVector<f64, 2>> = handles.iter().filter_map(|&h| {
        world.body(h).map(|b| b.position().0)
    }).collect();
    ...
    for tick in 0..TICKS {
        ...
        let wdata: Vec<_> = wells.iter().zip(well_remaining.iter())
            .filter(|(_, &r)| r > 0.0).map(|(&p, &r)| (p, (r / 4000.0).min(1.0))).collect();
        for &h in &handles {
        ...
        if tick > 0 && tick % 500 == 0 {
            let current_pos: Vec<SVector<f64, 2>> = handles.iter().filter_map(|&h| {
                world.body(h).map(|b| b.position().0)
            }).collect();
    ...
    let final_pos: Vec<SVector<f64, 2>> = handles.iter().filter_map(|&h| {
        world.body(h).map(|b| b.position().0)
    }).collect();
    let mean_dist = if final_pos.len() == start_positions.len() && !final_pos.is_empty() {
        final_pos.iter().zip(start_positions.iter()).map(|(f, s)| (f - s).norm()).sum::<f64>() / final_pos.len() as f64
    } else { 0.0 };

    // Collective migration: count 500-tick windows where 3+ agents migrated
    // (Simplified: just report total_migrations as proxy)
    let collective = if total_migrations > 3 { total_migrations / 3 } else { 0 };
...
fn main() {
    ...
    println!("seed,alive,migrations,agents_migrated,mean_distance,well1_tick,well2_tick,well3_tick,well4_tick,collective");
    ...
    for s in 0..SEEDS {
        ...
        println!("{seed},{:.1},{:.0},{:.0},{:.1},{:.0},{:.0},{:.0},{:.0},{:.0}",
            r.alive, r.total_migrations, r.agents_that_migrated, r.mean_distance_traveled,
            r.well_discovery_ticks[0], r.well_discovery_ticks[1],
            r.well_discovery_ticks[2], r.well_discovery_ticks[3], r.collective_migrations);
        results.push(r);
    }
    ...
    eprintln!("  Migrations:     {:.1} total ({:.1} agents migrated)",
        results.iter().map(|r| r.total_migrations).sum::<f64>() / n,
        results.iter().map(|r| r.agents_that_migrated).sum::<f64>() / n);
    eprintln!("  Mean distance:  {:.1} units", results.iter().map(|r| r.mean_distance_traveled).sum::<f64>() / n);

    eprintln!("\n── Well Discovery (mean tick) ──");
    for i in 0..4 {
        let mean_tick = results.iter().map(|r| r.well_discovery_ticks[i]).sum::<f64>() / n;
    ...
    eprintln!("  Correlation direction: {}", if d > 0.0 { "more migration = more survival" } else { "less migration = more survival" });
