...
fn run_at_cost(maint_cost: f64, seed: u64) -> f64 {
    ...
    consciousness.constants = ThermodynamicConstants {
        ...
        movement_cost_per_unit: 0.006, sprint_cost_multiplier: 2.5,
        collision_energy_drain: 0.05, harmony_resonance_regen_rate: 0.12,
        energy_well_regen_rate: 0.0, ambient_regen_rate: 0.02,
        collapse_recovery_harmony_threshold: 0.5, harmony_range: 40.0,
    };
    ...
    for i in 0..AGENTS {
        ...
        if let Some(b) = world.body_mut(h) {
            ...
            b.linear_velocity = SVector::from([(nr(&mut rng)-0.5)*8.0, (nr(&mut rng)-0.5)*8.0]);
        }
...
fn main() {
    ...
    for step in 0..COST_STEPS {
        ...
        let std = (fracs.iter().map(|f| (f - mean).powi(2)).sum::<f64>() / (SEEDS as f64 - 1.0)).sqrt();
    ...
    if !transition_costs.is_empty() {
        ...
        if per_seed_macs.len() > 1 {
            ...
            let std_mac = (per_seed_macs.iter().map(|m| (m - mean_mac).powi(2)).sum::<f64>()
                / (per_seed_macs.len() as f64 - 1.0)).sqrt();
            let cv = std_mac / mean_mac * 100.0;
            eprintln!("MAC mean: {:.3}, SD: {:.3}, CV: {:.1}%", mean_mac, std_mac, cv);
