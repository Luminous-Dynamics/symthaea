...
fn run_era(era: &Era, seed: u64) -> EraResult {
    ...
    for _tick in 0..TICKS {
        ...
        for &h in &handles {
            ...
            let dir = fep_gradient::free_energy_gradient(&pos, e.energy.fraction_remaining(),
                &e.harmony_activations, &near, &[], None, 0.0);
        ...
        for i in 0..handles.len() {
            for j in (i+1)..handles.len() {
                ...
                let in_range = match (world.body(ha), world.body(hb)) {
                    (Some(a), Some(b)) => a.position().distance(b.position()) < era.harmony_range,
                    _ => false,
                };
                ...
                if res > era.resonance_threshold {
                    let rg = consciousness.constants.harmony_resonance_regen_rate * (res - era.resonance_threshold) * 2.0;
                    ...
                    if dist < era.harmony_range && dist > 1.0 {
                        ...
                        if let Some(e) = consciousness.entities.get_mut(&h) {
                            for k in 0..8 {
                                ...
                                e.harmony_activations[k] += (inf_harm[k] - e.harmony_activations[k]) * influence * 0.01;
        ...
        // Track collapses
        for &h in &handles {
            if consciousness.entities.get(&h).map(|e| e.energy.is_collapsed()).unwrap_or(false) {
                total_collapses += 1;
            }
        }
    }
    ...
    let num_clusters = count_clusters(&world, &consciousness, &handles, era.resonance_threshold, era.harmony_range);
...
fn main() {
    ...
    let eras = vec![
        Era { name: "PRE-INTERNET",  harmony_range: 40.0,  maintenance: 0.15, resonance_threshold: 0.3, influencer_count: 0, influencer_strength: 0.0 },
        Era { name: "EARLY-WEB",     harmony_range: 100.0, maintenance: 0.20, resonance_threshold: 0.4, influencer_count: 2, influencer_strength: 5.0 },
        Era { name: "SOCIAL-MEDIA",  harmony_range: 300.0, maintenance: 0.30, resonance_threshold: 0.6, influencer_count: 6, influencer_strength: 15.0 },
        Era { name: "ALGO-FEEDS",    harmony_range: 400.0, maintenance: 0.40, resonance_threshold: 0.8, influencer_count: 10, influencer_strength: 25.0 },
        Era { name: "POST-ALGO",     harmony_range: 200.0, maintenance: 0.25, resonance_threshold: 0.4, influencer_count: 3, influencer_strength: 5.0 },
    ];
    ...
    for era in &eras {
        ...
        for s in 0..SEEDS {
            ...
            println!("{},{seed},{:.1},{:.1},{:.2},{:.1},{:.4},{:.0},{:.4}",
                era.name, r.alive, r.energy, r.clustering, r.num_clusters,
                r.polarization, r.cooperation_events, r.burnout_rate);
            results.push(r);
        }

        let n = results.len() as f64;
        eprintln!("\n── {} ──", era.name);
        eprintln!("  Alive:        {:.1}/{AGENTS}", results.iter().map(|r| r.alive).sum::<f64>() / n);
        eprintln!("  Energy:       {:.1} J", results.iter().map(|r| r.energy).sum::<f64>() / n);
        eprintln!("  Clustering:   {:.2}", results.iter().map(|r| r.clustering).sum::<f64>() / n);
        eprintln!("  Clusters:     {:.1}", results.iter().map(|r| r.num_clusters).sum::<f64>() / n);
        eprintln!("  Polarization: {:.4}", results.iter().map(|r| r.polarization).sum::<f64>() / n);
        eprintln!("  Cooperation:  {:.0}", results.iter().map(|r| r.cooperation_events).sum::<f64>() / n);
        eprintln!("  Burnout rate: {:.2}%", results.iter().map(|r| r.burnout_rate).sum::<f64>() / n * 100.0);
    ...
    if era_results.len() >= 4 {
        ...
        eprintln!("  Clustering: Mann-Whitney U={:.1}, z={:.3}, p={:.4}", u, z, p);
        ...
        eprintln!("  Polarization: Mann-Whitney U={:.1}, z={:.3}, p={:.4}", u2, z2, p2);
        eprintln!("  Cohen's d={:.3} ({})", d2, if d2.abs() > 0.8 { "large" } else { "medium/small" });
        ...
        eprintln!("  Burnout: Mann-Whitney U={:.1}, z={:.3}, p={:.4}", u3, z3, p3);
        eprintln!("  Cohen's d={:.3} ({})", d3, if d3.abs() > 0.8 { "large" } else { "medium/small" });
    }
...
fn count_clusters(world: &PhysicsWorld<2>, consciousness: &ConsciousnessField<2>,
    handles: &[symtropy_physics::BodyHandle], threshold: f64, range: f64) -> f64 {
    let n = handles.len();
    ...
    for start in 0..n {
        ...
        while let Some(idx) = stack.pop() {
            for other in 0..n {
                ...
                if !in_range { continue; }
                let res = match (consciousness.entities.get(&handles[idx]), consciousness.entities.get(&handles[other])) {
                    (Some(a), Some(b)) => HarmonyField::<2>::resonance(&a.harmony_activations, &b.harmony_activations),
                    _ => 0.0,
                };
                if res > threshold {
                    visited[other] = true;
...
fn compute_polarization(consciousness: &ConsciousnessField<2>,
    ...
    for &h in handles {
        if let Some(e) = consciousness.entities.get(&h) {
            let dist_sq: f64 = (0..8).map(|i| (e.harmony_activations[i] - mean[i]).powi(2)).sum();
            var += dist_sq;
        }
    }
    var / n
