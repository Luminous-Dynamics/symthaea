...
#[derive(Clone, Copy)]
enum Mode { Stateless, Memory, MemoryLearn }
...
fn run_experiment(mode: Mode, seed: u64) -> MemResult {
                ...
                if let Some(b) = world.body(h) {
                    ...
                    for (wi, &w) in wells.iter().enumerate() {
                        if (pos - w).norm() < 35.0 && well_remaining[wi] > 0.0 {
                            ...
                            well_remaining[wi] -= d;
                            if use_memory { e.memory.ticks_since_regen = 0; }
                            break;
                        }
                    }
                }
                if use_memory { e.memory.ticks_since_regen += 1; }
            }
        }
            ...
            for j in (i+1)..handles.len() {
                ...
                if res > 0.5 {
                    ...
                    if let Some(e) = consciousness.entities.get_mut(&ha) {
                        ...
                        if use_memory { e.memory.record_partner(hb, res); }
                    }
                    if let Some(e) = consciousness.entities.get_mut(&hb) {
                        e.energy.regenerate(rg);
                        if use_memory { e.memory.record_partner(ha, res); }
                    }
                    coop += 1;
                }
            }
    ...
    let wells_disc = handles.iter().filter_map(|h| consciousness.entities.get(h).map(|e| e.memory.known_wells.len() as f64)).sum::<f64>() / AGENTS as f64;
    let partners = handles.iter().filter_map(|h| consciousness.entities.get(h).map(|e| e.memory.partner_history.len() as f64)).sum::<f64>() / AGENTS as f64;
    let drift = agent_weights.iter().map(|w| w.drift_from_default()).sum::<f64>() / AGENTS as f64;
...
fn main() {
    ...
    for &(mode, name) in &modes {
        ...
        for s in 0..SEEDS {
            ...
            println!("{},{seed},{:.1},{:.1},{:.2},{:.0},{:.1},{:.1},{:.4}",
                r.condition, r.alive, r.energy, r.clustering, r.cooperation,
                r.wells_discovered, r.partners_known, r.weight_drift);
            results.push(r);
        }
        let n = results.len() as f64;
        eprintln!("  → {name}: alive={:.1}, wells={:.1}, partners={:.1}, drift={:.4}",
    ...
    for (name, results) in &all_results {
        let n = results.len() as f64;
        eprintln!("  {:10} {:5.1}  {:5.1}J  {:6.2}   {:8.0}  {:4.1}   {:5.1}     {:.4}",
            name,
            results.iter().map(|r| r.alive).sum::<f64>() / n,
