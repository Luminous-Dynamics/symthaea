...
#[derive(Clone, Copy)]
enum StartCondition { Equal, Random, Unequal }
...
fn run_experiment(start: StartCondition, seed: u64) -> InequalityResult {
    ...
    for i in 0..AGENTS {
        let (x, y) = match start {
            ...
            _ => ((rng_f64(&mut rng) - 0.5) * 120.0, (rng_f64(&mut rng) - 0.5) * 120.0),
        };
        ...
        let start_energy = match start {
            StartCondition::Unequal => {
                ...
                let min_dist = wells.iter().map(|w| {
                    ((x - w[0]).powi(2) + (y - w[1]).powi(2)).sqrt()
        ...
        consciousness.register(h, consciousness.constants.max_energy, consciousness.constants.harmony_range);
    ...
    let initial_energies: Vec<f64> = handles.iter().map(|h| {
    ...
    for tick in 0..TICKS {
        ...
        if tick == TICKS / 2 {
            for (idx, &h) in handles.iter().enumerate() {
                mid_energies[idx] = consciousness.entities.get(&h).map(|e| e.energy.available).unwrap_or(0.0);
            }
    ...
    let final_energies: Vec<f64> = handles.iter().map(|h| {
        consciousness.entities.get(h).map(|e| e.energy.available).unwrap_or(0.0)
    }).collect();
    ...
    let mobility = initial_quartiles.iter().zip(final_quartiles.iter())
        .filter(|(a, b)| a != b).count() as f64 / AGENTS as f64;
    ...
    InequalityResult {
        condition: cond_name, alive,
        gini_start, gini_mid, gini_end,
        energy_top25: top25, energy_bot25: bot25,
        mobility,
    }
}
...
fn main() {
    ...
    println!("condition,seed,alive,gini_start,gini_mid,gini_end,top25_energy,bot25_energy,mobility");
    ...
    for &(cond, name) in &conditions {
        ...
        for s in 0..SEEDS {
            ...
            println!("{},{seed},{:.1},{:.4},{:.4},{:.4},{:.1},{:.1},{:.3}",
                r.condition, r.alive, r.gini_start, r.gini_mid, r.gini_end,
                r.energy_top25, r.energy_bot25, r.mobility);
            results.push(r);
        }
        let n = results.len() as f64;
        eprintln!("  → {name}: Gini {:.3}→{:.3}→{:.3}, top25={:.0}J, bot25={:.0}J, mobility={:.1}%",
    ...
    for (name, results) in &all_results {
        let n = results.len() as f64;
        eprintln!("  {:10} {:8.3}    {:8.3}    {:8.3}    {:5.0}J   {:5.0}J   {:5.1}%",
            name,
            results.iter().map(|r| r.gini_start).sum::<f64>() / n,
            results.iter().map(|r| r.gini_mid).sum::<f64>() / n,
            results.iter().map(|r| r.gini_end).sum::<f64>() / n,
    ...
    if all_results.len() >= 2 {
        ...
        eprintln!("  {} initial conditions → {} final inequality",
            if d.abs() < 0.3 { "Different" } else { "Different" },
