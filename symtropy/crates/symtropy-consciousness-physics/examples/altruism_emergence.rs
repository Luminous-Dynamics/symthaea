...
enum RescueMode { None, Free, Costly }
...
fn run_experiment(mode: RescueMode, seed: u64) -> AltruismResult {
    ...
    for tick in 0..TICKS {
        ...
        if mode != RescueMode::None && tick % 100 == 0 {
            for i in 0..handles.len() {
                let hi = handles[i];
                let collapsed = consciousness.entities.get(&hi).map(|e| e.energy.is_collapsed()).unwrap_or(false);
                if !collapsed { continue; }
                if tick.saturating_sub(last_rescued_tick[i]) < RESCUE_COOLDOWN { continue; }
                ...
                for j in 0..handles.len() {
                    if i == j { continue; }
                    let hj = handles[j];
                    let alive = consciousness.entities.get(&hj).map(|e| !e.energy.is_collapsed()).unwrap_or(false);
                    if !alive { continue; }
                ...
                if let Some((donor_idx, _res)) = best_donor {
                    ...
                    let can_rescue = match mode {
                        ...
                        RescueMode::Costly => {
                            let donor_energy = consciousness.entities.get(&hd).map(|e| e.energy.available).unwrap_or(0.0);
                            donor_energy > donor_energy * RESCUE_COST_FRACTION + 20.0 // must have enough to survive after
                        },
                    ...
                    if can_rescue {
                        ...
                        if mode == RescueMode::Costly {
                            let cost = consciousness.entities.get(&hd).map(|e| e.energy.available * RESCUE_COST_FRACTION).unwrap_or(0.0);
        ...
        for i in 0..handles.len() {
            if was_rescued[i] {
                let collapsed = consciousness.entities.get(&handles[i]).map(|e| e.energy.is_collapsed()).unwrap_or(false);
                if collapsed {
                    re_collapse_count += 1;
    ...
    for i in 0..handles.len() {
        if was_donor[i] {
            let collapsed = consciousness.entities.get(&handles[i]).map(|e| e.energy.is_collapsed()).unwrap_or(true);
            if collapsed { donor_death_count += 1; }
        }
    }
...
fn main() {
    ...
    eprintln!("Rescue cost: {:.0}% of donor energy, gift: {RESCUE_GIFT}J", RESCUE_COST_FRACTION * 100.0);

    println!("condition,seed,alive,energy,rescues,re_collapses,donor_deaths,unique_rescued");

    let modes = [(RescueMode::None, "NO_RESCUE"), (RescueMode::Free, "FREE_RESCUE"), (RescueMode::Costly, "COSTLY_RESCUE")];
    ...
    for &(mode, name) in &modes {
        ...
        for s in 0..SEEDS {
            ...
            println!("{},{seed},{:.1},{:.1},{:.0},{:.0},{:.0},{:.0}",
                r.condition, r.alive, r.energy, r.rescues, r.re_collapses,
                r.donor_deaths, r.unique_rescued);
            results.push(r);
        }
        let n = results.len() as f64;
        eprintln!("  → {name}: alive={:.1}, rescues={:.0}, re-collapses={:.0}, donor_deaths={:.0}",
    ...
    if all_results.len() >= 3 {
        ...
        let rescue_rate: Vec<f64> = all_results[2].1.iter().map(|r| {
            if r.rescues > 0.0 { 1.0 - r.re_collapses / r.rescues } else { 0.0 }
        }).collect();
        let mean_success = rescue_rate.iter().sum::<f64>() / rescue_rate.len() as f64;
        eprintln!("\n── Rescue Effectiveness ──");
        eprintln!("  Mean rescue success rate: {:.1}% (rescued agents that survived)", mean_success * 100.0);
    }
