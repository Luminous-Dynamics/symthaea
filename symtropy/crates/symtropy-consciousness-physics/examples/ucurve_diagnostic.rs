...
fn run_diagnostic(willingness: f64, seed: u64) -> DiagResult {
    ...
    for _tick in 0..TICKS {
        ...
        for &h in &handles {
            ...
            if include_social { ticks_seeking += 1; }
            agent_tick_count += 1;
    ...
    DiagResult {
        willingness, alive,
        total_maintenance, total_well_regen, total_resonance_regen, total_ambient_regen,
        ticks_at_well: if agent_tick_count > 0 { ticks_at_well as f64 / agent_tick_count as f64 } else { 0.0 },
        ticks_seeking: if agent_tick_count > 0 { ticks_seeking as f64 / agent_tick_count as f64 } else { 0.0 },
        mean_displacement: total_displacement / (AGENTS as f64 * TICKS as f64),
        energy_efficiency: if total_maintenance > 0.0 { total_regen / total_maintenance } else { 0.0 },
    }
}
...
fn main() {
    ...
    for &w in &WILLINGNESS_LEVELS {
        ...
        for s in 0..SEEDS {
            ...
            println!("{w:.1},{seed},{:.1},{:.0},{:.0},{:.0},{:.0},{:.3},{:.3},{:.3},{:.3}",
                r.alive, r.total_maintenance, r.total_well_regen, r.total_resonance_regen,
                r.total_ambient_regen, r.ticks_at_well, r.ticks_seeking, r.mean_displacement,
                r.energy_efficiency);
            results.push(r);
        }
        all.push((w, results));
    ...
    eprintln!("  Will  Alive  Maint     Wells     Reson     Ambient   AtWell  Seeking  Displ  Efficiency");
    ...
    if well_00 > well_06 && res_10 > res_06 {
        ...
        eprintln!("  Partial cooperation pays the cost of both strategies but reaps neither benefit.");
    }
