...
fn run_at_dim(dim: usize, seed: u64) -> DimResult {
    match dim {
        ...
        3 => run::<3>(seed),
        4 => run::<4>(seed),
        _ => DimResult { pred_error_mean: 0.0, pred_error_max: 0.0, alive: 0.0,
            energy: 0.0, resources_visible: 0.0, dark_energy_frac: 0.0,
            clustering: 0.0, disappearance_tick: 0.0 },
    }
}
...
fn run<const D: usize>(seed: u64) -> DimResult {
    ...
    consciousness.constants = ThermodynamicConstants {
        ...
        harmony_resonance_regen_rate: 0.12, energy_well_regen_rate: 0.0,
    ...
    for i in 0..AGENTS {
        ...
        if D > 1 { coords[1] = (nr(&mut rng) - 0.5) * 80.0; }
    ...
    for _ in 0..RESOURCES {
        ...
        if D > 1 { coords[1] = (nr(&mut rng) - 0.5) * 60.0; }
    ...
    for tick in 0..TICKS {
        ...
        let ad: Vec<_> = agent_handles.iter().filter_map(|&h| {
        ...
        for (idx, &h) in agent_handles.iter().enumerate() {
            ...
            if let Some(b) = world.body_mut(h) {
                ...
                for d in 2..D { vel[d] = 0.0; } // locked to brane
                b.linear_velocity = vel;
            }
        }
        ...
        for i in 0..agent_handles.len() {
            for j in (i+1)..agent_handles.len() {
                ...
                let dist = match (world.body(ha),world.body(hb)) { (Some(a),Some(b)) => {
                    // Distance in BRANE dimensions only (first 2)
                    let pa = a.position().0; let pb = b.position().0;
                    let dx = pa[0]-pb[0]; let dy = if D > 1 { pa[1]-pb[1] } else { 0.0 };
                    (dx*dx+dy*dy).sqrt()
                }, _ => continue };
        ...
        let visible = resource_handles.iter().filter(|&&h| {
            if let Some(body) = world.body(h) {
                let pos = body.position().0;
                for d in 2..D {
                    if pos[d].abs() > BRANE_THRESHOLD { return false; }
                }
                true
            } else { false }
        }).count();
        ...
        if visible < prev_visible_count {
            ...
            for &h in &agent_handles {
                if let Some(e) = consciousness.entities.get_mut(&h) {
                    ...
                    if e.prediction_error > max_pe { max_pe = e.prediction_error; }
                }
            }
            if first_disappearance == TICKS { first_disappearance = tick; }
        }
    ...
    let final_visible = resource_handles.iter().filter(|&&h| {
        if let Some(body) = world.body(h) {
            let pos = body.position().0;
            for d in 2..D { if pos[d].abs() > BRANE_THRESHOLD { return false; } }
            true
        } else { false }
    }).count();
    ...
    for i in 0..agent_handles.len() {
        let mut md = f64::MAX;
        for j in 0..agent_handles.len() { if i==j {continue;} if let (Some(a),Some(b)) = (world.body(agent_handles[i]),world.body(agent_handles[j])) { let d=a.position().distance(b.position()); if d<md{md=d;} } }
        tn += md;
    }

    DimResult {
        pred_error_mean: if pe_samples > 0 { pe_sum / pe_samples as f64 } else { 0.0 },
        pred_error_max: max_pe,
        alive: alive as f64,
        energy: ae,
        resources_visible: final_visible as f64 / RESOURCES as f64,
        dark_energy_frac: if total_ke > 0.0 { 1.0 - visible_ke / total_ke } else { 0.0 },
...
fn main() {
    println!("dim,hidden,pred_error_mean,pred_error_max,alive,energy,visible_frac,dark_energy_frac,clustering,first_vanish");

    for d in [2, 3, 4] {
        let hidden = if d > 2 { d - 2 } else { 0 };
        let mut pe_m=Vec::new(); let mut pe_x=Vec::new(); let mut al=Vec::new();
        let mut en=Vec::new(); let mut vi=Vec::new(); let mut de=Vec::new();
        let mut cl=Vec::new(); let mut fv=Vec::new();

        for s in 0..SEEDS {
            let r = run_at_dim(d, 42 + s as u64 * 997);
            pe_m.push(r.pred_error_mean); pe_x.push(r.pred_error_max);
            al.push(r.alive); en.push(r.energy);
            vi.push(r.resources_visible); de.push(r.dark_energy_frac);
            cl.push(r.clustering); fv.push(r.disappearance_tick);
        }

        let n = SEEDS as f64;
        println!("{},{},{:.4},{:.3},{:.1},{:.1},{:.1},{:.3},{:.2},{:.0}",
            d, hidden,
            pe_m.iter().sum::<f64>()/n, pe_x.iter().sum::<f64>()/n,
            al.iter().sum::<f64>()/n, en.iter().sum::<f64>()/n,
            vi.iter().sum::<f64>()/n * 100.0, de.iter().sum::<f64>()/n,
            cl.iter().sum::<f64>()/n, fv.iter().sum::<f64>()/n,
        );
    }
