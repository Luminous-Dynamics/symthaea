...
The bistability above threshold---seeds either fully survive or fully collapse---is characteristic of first-order phase transitions in statistical mechanics. The spatial lottery (proximity to wells at time of pressure onset) determines the outcome (Figure~\ref{fig:phase}).

\end{multicols}
\begin{figure*}[t]
\centering
\includegraphics[width=0.85\textwidth]{fig5_phase_transition.pdf}
\caption{Phase transition in cooperation. (a) Survival drops sharply at 0.20~J/tick while cooperation events increase 3.5$\times$ under stress. (b) Temperature saturates at $\sim$331K as entropy production maximizes.}
\label{fig:phase}
\end{figure*}
\begin{multicols}{2}
...
\textbf{Finding 10: Higher-dimensional integration is more expensive.} HDC produces 4$\times$ higher $\Phi$ ($0.374 \pm 0.094$ vs $0.090 \pm 0.013$) but at a survival cost: $9.0/12$ alive vs $11.1/12$. HDC agents cluster less tightly ($17.18$ vs $8.15$), with higher per-agent behavioral divergence. Mean pairwise HDC vector similarity is $0.043$---agents produce diverse state representations from identical inputs.

The LTC network's $\Phi$ output crosses an activation threshold around tick 1000: $\Phi$ increases from $\sim 0$ to $\sim 0.56$ as network dynamics reach equilibrium. This threshold-crossing behavior arises from the closed-form LTC dynamics $x(t+\Delta t) = x_\infty + (x(t) - x_\infty) e^{-\Delta t/\tau}$, not from any consciousness-like mechanism.
...
\caption{Integration-parameterized curvature deflects trajectories. Trajectory deflection scales linearly with curvature $\kappa$ ($p=0.009$, $d=-35.5$).}
