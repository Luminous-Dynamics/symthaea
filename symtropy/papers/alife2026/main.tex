...
\section{Consciousness-Physics Coupling}
...
\section{Experiments (1,800+ Runs)}
...
\centering
\small
\caption{Ablation: Which mechanisms produce clustering?}
\label{tab:ablation}
\begin{tabular}{lrrl}
\toprule
...
\textbf{Finding 1:} The FEP gradient is the clustering mechanism (18.1$\to$1.8). Epistemic offloading is the survival mechanism (71.7\%$\to$56.7\% collapse). Neither alone is sufficient (Figure~\ref{fig:ablation_full}).
...
\centering
\small
\caption{Metric independence: all produce identical clustering.}
\label{tab:metric}
\begin{tabular}{lrl}
\toprule
...
\centering
\small
\caption{Causal test: $\Phi$=0 clusters 35\% less tightly.}
\label{tab:causal}
\begin{tabular}{lrrl}
\toprule
...
\centering
\small
\caption{Per-capita energy vs.\ population size (20 seeds each).}
\label{tab:scaling}
\begin{tabular}{rrrr}
\toprule
...
\centering
\small
\caption{Biological validation: emergent matches (not fitted).}
\label{tab:bio}
\begin{tabular}{lrrl}
\toprule
...
\textbf{Fix 4: Dimension-Correct Fields.} Replaced hardcoded $1/r^2$ harmony field falloff with $1/r^{D-1}$ (Plummer-softened: $(r^2 + \epsilon^2)^{(D-1)/2}$), matching real field theory in $D$ spatial dimensions.
...
\subsection{Consciousness-Sourced Space-Time Curvature}

We implemented conformal Riemannian geometry where consciousness curves gamespace:
\begin{equation}
...
\textbf{Finding 8: Consciousness bends space.} A test body launched past a stationary consciousness source ($\Phi = 0.8$) deflects proportionally to curvature scale (Table~\ref{tab:curvature}, Figure~\ref{fig:curvature_full}). At $\kappa = 0.05$, deflection is 12.4 units (62\% of closest approach). Mann-Whitney $U = 0.0$, $p = 0.009$, Cohen's $d = -35.5$.

\begin{table}[h]
\centering
\small
\caption{Curvature lensing: trajectory deflection.}
\label{tab:curvature}
\begin{tabular}{rrr}
\toprule
...
\textbf{Finding 10: Holographic consciousness is more expensive.} HDC produces 4$\times$ higher $\Phi$ ($0.374 \pm 0.094$ vs $0.090 \pm 0.013$) but at a survival cost: $9.0/12$ alive vs $11.1/12$. HDC agents cluster less tightly ($17.18$ vs $8.15$), suggesting higher individual agency reduces herd behavior. Mean pairwise thought similarity is $0.043$---agents develop unique cognitive signatures from identical inputs.

The LTC network ``awakens'' around tick 1000: $\Phi$ jumps from $0$ to $\sim 0.56$ as the network's internal state evolves past a critical threshold. This emergent activation is not programmed---it arises from the closed-form dynamics $x(t+\Delta t) = x_\infty + (x(t) - x_\infty) e^{-\Delta t/\tau}$.

\section{Discussion}

\subsection{Limitations}

$\Phi$ is a softmin bottleneck approximation, not full IIT partition analysis. The FEP gradient uses handcrafted heuristics, not learned policies. Agents do not learn or adapt strategies. The J/$\Phi$ metric has high variance across seeds (95\% CI spans $\pm 130$\% of mean at 10 seeds). The 16,384D HDC thought vector substrate (Section~2.5 of supplementary) produces dynamic $\Phi$ but requires $\sim$100 minutes per 10-seed experiment.
...
\caption{Ablation study (re-verified with overhauled engine). (a)~Only conditions with FEP gradient cluster tightly ($<4$). (b)~Gradient without offloading kills 72\% of agents. The FEP gradient is the clustering mechanism; epistemic offloading is the survival mechanism.}
...
\caption{Consciousness bends space. Trajectory deflection scales linearly with curvature $\kappa$ ($p=0.009$, $d=-35.5$).}
\label{fig:curvature_full}
\end{minipage}\hfill
\begin{minipage}{0.45\textwidth}
\includegraphics[width=\textwidth]{fig4_cooperation.pdf}
\caption{Cooperation emergence. Thermodynamic enforcement produces 81\% tighter clustering ($p<0.001$, $d=-1.94$).}
\label{fig:coop_full}
\end{minipage}
\end{figure*}
...
\bibitem[Carroll, 2004]{carroll2004} Carroll, S.M. (2004). \emph{Spacetime and Geometry: An Introduction to General Relativity}. Addison-Wesley.
\bibitem[Karras, 2012]{karras2012} Karras, T. (2012). Maximizing parallelism in the construction of BVHs, octrees, and k-d trees. \emph{High Performance Graphics}.
\end{thebibliography}
