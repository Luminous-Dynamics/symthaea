...
$\Phi$ is a softmin bottleneck approximation, not full IIT partition analysis. The FEP gradient uses handcrafted heuristics, not learned policies. The $O(n^2)$ broadphase limits scale to $\sim$500 bodies. Agents do not learn or adapt strategies.
...
Across 1,800+ runs, we show: (1)~clustering requires both gradient descent and cost reduction; (2)~the specific consciousness metric is interchangeable; (3)~the conscious/unconscious distinction produces a 35\% clustering difference when wired into behavioral pathways. Consciousness amplifies social drive at a survival cost---a trade-off that may be fundamental to systems where information integration is thermodynamically expensive.
