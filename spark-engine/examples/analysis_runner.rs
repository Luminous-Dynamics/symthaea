...
use spark_engine::rate_gap::{RateGapCalculator, ExperimentalConditions, HostMaterial, TriggerType};
use spark_engine::experimental_design::ExperimentDesigner;
use spark_engine::hypothesis_models::{HypothesisComparison, HotSpotModel, PhononCascadeModel, SuperScreeningModel};
...
fn main() {
    ...
    println!("  • 300 K (room temp):   {:.2e} cm³/s", gamow_300k.sigma_v_cm3_s);
    println!("  • 1,000 K:             {:.2e} cm³/s", gamow_1000k.sigma_v_cm3_s);
    println!("  • 10,000 K:            {:.2e} cm³/s", gamow_10000k.sigma_v_cm3_s);
    println!();

    println!("Gamow Peak Analysis (300 K, Ue = {} eV):", SCREENING_PD_EV);
    println!("  • Gamow peak energy:   {:.3} keV", gamow_300k.gamow_peak_kev);
    println!("  • Gamow width:         {:.3} keV", gamow_300k.gamow_width_kev);
    println!("  • Screening factor:    {:.2e}", gamow_300k.screening_enhancement);
    println!();
    ...
    println!("  • Energy gain achievable: {}", if achievable { "YES" } else { "NO" });
    println!("  • Verdict: {}", if q_factor < 1.0 {
    ...
    println!("  • Average credibility:   {:.1}/10", patterns.average_credibility);
    println!();

    println!("Observations with Neutron Detection:");
    for obs in db.with_neutrons() {
        println!("  • {} ({}) - {} [credibility: {:.1}]",
            obs.author, obs.year, obs.venue, obs.credibility.score);
        if let Some(effect) = obs.effects.iter().find(|e| matches!(e, spark_engine::literature::ObservedEffect::Neutrons { .. })) {
            if let spark_engine::literature::ObservedEffect::Neutrons { rate_per_s, energy_mev } = effect {
                println!("    Rate: {:.0} n/s, Energy: {} MeV",
                    rate_per_s,
                    energy_mev.map(|e| format!("{:.2}", e)).unwrap_or("unknown".to_string()));
            }
        }
    }
    println!();
    ...
    for obs in db.by_credibility(7.0) {
        println!("  • {} ({}) - credibility {:.1}: {}",
            obs.author, obs.year, obs.credibility.score,
    ...
    println!("  • Observed neutron rate:  {:.0} n/s", nasa_analysis.observed_rate);
    println!("  • Gamow predicted rate:   {:.2e} n/s", nasa_analysis.gamow_predicted_rate);
    println!();

    println!("THE GAP:");
    println!("  ╔═══════════════════════════════════════════════════════════╗");
    println!("  ║  Gap factor: {:.2e} ({:.0} orders of magnitude)        ║",
        nasa_analysis.gap_factor, nasa_analysis.gap_orders);
    println!("  ║  Classification: {:?}                          ║", nasa_analysis.assessment.classification);
    println!("  ╚═══════════════════════════════════════════════════════════╝");
    println!();

    println!("Gap Breakdown:");
    println!("  • Screening enhancement:  {:.2e}×", nasa_analysis.gap_breakdown.screening_enhancement);
    println!("  • Phonon enhancement:     {:.2e}×", nasa_analysis.gap_breakdown.phonon_enhancement);
    println!("  • Hot spot enhancement:   {:.2e}×", nasa_analysis.gap_breakdown.hot_spot_enhancement);
    println!("  • UNEXPLAINED:            {:.2e}× ({:.0} orders)",
    ...
    println!("  • Predicted peak T:      {:.0} K", hot_spot.peak_temperature_k);
    println!("  • Cooling time:          {:.2} ps", hot_spot.cooling_time_ps);
    println!("  • Predicted rate:        {:.2e} n/s", comparison.hot_spot.predicted_rate);
    println!("  • Gap from NASA:         {:.2e}×", comparison.hot_spot.gap_factor);
    println!("  • Required T for NASA:   {:.2e} K", comparison.hot_spot.required_temperature_k);
    println!("  • Plausible:             {}", if comparison.hot_spot.plausible { "YES" } else { "NO" });
    println!();
    ...
    println!("  • Energy boost (3 phonons): {:.1} meV", phonon.energy_boost_kev * 1000.0);
    println!("  • Enhancement factor:    {:.2e}×", phonon.enhancement_factor);
    println!("  • Predicted rate (3):    {:.2e} n/s", comparison.phonon_cascade.predicted_rate_n3);
    println!("  • Gap from NASA:         {:.2e}×", comparison.phonon_cascade.gap_factor);
    println!("  • Phonons needed:        {}", comparison.phonon_cascade.phonons_needed);
    println!("  • Plausible:             {}", if comparison.phonon_cascade.plausible { "YES" } else { "NO" });
    println!();
    ...
    println!("  • Base screening:        {:.0} eV", screening.base_screening_ev);
    println!("  • Predicted rate:        {:.2e} n/s", comparison.super_screening.predicted_rate);
    println!("  • Gap from NASA:         {:.2e}×", comparison.super_screening.gap_factor);
    println!("  • Screening needed:      {:.0} eV", comparison.super_screening.screening_needed_ev);
    println!("  • Plausible:             {}", if comparison.super_screening.plausible { "YES" } else { "NO" });
    println!();
    ...
    let required_screening = RateGapCalculator::required_screening_for_rate(&conditions, NASA_OBSERVED_RATE);
    let required_temp = RateGapCalculator::required_temperature_for_rate(&conditions, NASA_OBSERVED_RATE);

    println!("To explain {:.0} n/s from standard Gamow physics:", NASA_OBSERVED_RATE);
    println!();
    println!("  Option A: Increase screening energy");
    println!("    • Required Ue:  {:.0} eV (vs measured {:.0} eV)", required_screening, SCREENING_PD_EV);
    println!("    • Factor:       {:.0}× higher than measured", required_screening / SCREENING_PD_EV);
    println!("    • Assessment:   {}", if required_screening < 10000.0 { "Potentially achievable" } else { "Implausible" });
    println!();
    println!("  Option B: Increase temperature");
    println!("    • Required T:   {:.2e} K (vs room temp 300 K)", required_temp);
    println!("    • Factor:       {:.0}× higher", required_temp / 300.0);
    println!("    • Assessment:   {}", if required_temp < 1e6 { "Hot spots could achieve" } else { "Requires extreme conditions" });
    println!();
    ...
    println!("Total Duration: {:.0} months", program.total_duration_months);
    println!();

    for (i, phase) in program.phases.iter().enumerate() {
        println!("Phase {}: {} (${:.0}K, {} months)",
            i + 1, phase.name, phase.cost_usd / 1000.0, phase.duration_months);
    ...
    println!("   NASA observed ~10³ n/s, creating a {:.0}-order-of-magnitude gap.",
