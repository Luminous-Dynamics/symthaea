...
pub fn analyze_with_cfc(dynamics: &ScreeningDynamics) -> CfCAnalysisResult {
    ...
    let t_max = dynamics.screening_series.last().map(|(t, _)| *t).unwrap_or(1e-10);
    ...
    CfCAnalysisResult {
        ...
        enhancement_ratio: dynamics.enhancement_series.last().map(|(_, e)| *e).unwrap_or(1.0),
        learned_tau,
        dominant_time_scale,
        state_trajectory,
        temporal_features,
...
fn extract_temporal_features(
    ...
    let final_ue = dynamics.screening_series.last().map(|(_, ue)| *ue).unwrap_or(0.0);
    ...
    let phonon_correlation = if !states.is_empty() {
        let state_sum: Vec<f64> = states.iter().map(|s| s.iter().sum::<f32>() as f64).collect();
...
fn compute_correlations(results: &[CfCAnalysisResult]) -> CfCCorrelations {
    ...
    let rates: Vec<f64> = results.iter().map(|r| r.temporal_features.screening_rate).collect();
    let phonons: Vec<f64> = results.iter().map(|r| r.temporal_features.phonon_correlation).collect();
...
fn generate_insights(results: &[CfCAnalysisResult], corr: &CfCCorrelations) -> Vec<String> {
    ...
    if corr.tau_enhancement_corr.abs() > 0.5 {
        let direction = if corr.tau_enhancement_corr > 0.0 { "slower" } else { "faster" };
    ...
    if corr.rate_enhancement_corr.abs() > 0.3 {
        let direction = if corr.rate_enhancement_corr > 0.0 { "faster" } else { "slower" };
        insights.push(format!(
            "Initial screening buildup rate correlates with enhancement (r = {:.2}). \
             {} initial screening may be key to enhancement.",
            corr.rate_enhancement_corr, if corr.rate_enhancement_corr > 0.0 { "Faster" } else { "Slower" }
        ));
    }
    ...
    if let Some(highest) = results.iter().max_by(|a, b| {
        a.enhancement_ratio.partial_cmp(&b.enhancement_ratio).unwrap()
    }) {
        insights.push(format!(
