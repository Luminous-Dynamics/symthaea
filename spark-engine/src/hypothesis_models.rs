...
impl HotSpotModel {
    ...
    pub fn required_temperature_for_nasa(&self) -> f64 {
        ...
        for _ in 0..100 {
            ...
            let volume_cm3 = (4.0 / 3.0) * std::f64::consts::PI
                * (self.hot_spot_radius_nm * 1e-7).powi(3);
            let pd_atoms = volume_cm3 * 12.02 * 6.022e23 / 106.42;
...
impl SuperScreeningModel {
    ...
    pub fn compute(loading_ratio: f64, enhancement_factor: f64) -> Self {
        ...
        let effective_screening_ev = base_screening_ev * (1.0 + enhancement_factor * (loading_ratio - 0.7).max(0.0));
