...
impl NeutronSourceDesigner {
    ...
    pub fn design(spec: &SourceSpec, assume_anomaly: bool) -> NeutronSourceDesign {
        ...
        let core_radius_cm = (required_volume_cm3 * 3.0 / (4.0 * std::f64::consts::PI))
            .powf(1.0 / 3.0);
        let shielding_radius_cm = core_radius_cm + shielding_thickness_cm;
        let shielding_volume_cm3 = (4.0 / 3.0) * std::f64::consts::PI
        ...
        let total_mass_kg = pd_mass_g / 1000.0 + shielding_mass_kg + electronics_mass_kg + structure_mass_kg;
        let total_volume_l = (4.0 / 3.0) * std::f64::consts::PI * shielding_radius_cm.powi(3) / 1000.0
            + 5.0; // Electronics volume
        ...
        let limiting_factor = if !physics_ok {
            ...
            format!("Size/mass exceeds {} limit", match spec.portability {
        ...
        if q_factor < 1.0 {
            notes.push(format!("Q = {:.2e} << 1: net energy consumer, not generator", q_factor));
        }
    ...
    pub fn compare_to_competitors(_spec: &SourceSpec) -> Vec<CompetitorComparison> {
        vec![
            CompetitorComparison {
                ...
                cons: vec!["Decays (2.6 year half-life)".to_string(), "Expensive".to_string()],
            },
            CompetitorComparison {
                ...
                pros: vec!["No radioactive source".to_string(), "Controllable".to_string()],
                cons: vec!["High voltage (>100 kV)".to_string(), "Tritium buildup".to_string()],
            },
            CompetitorComparison {
                ...
                cons: vec!["Tritium source required".to_string(), "14.1 MeV neutrons".to_string()],
            },
            CompetitorComparison {
                ...
                cons: vec!["Fixed installation".to_string(), "Heavy regulation".to_string()],
            },
        ]
    }
}
