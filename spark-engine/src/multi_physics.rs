...
use crate::bridge::{
    LiteratureDataLoader, ScreeningMeasurement, NeutronMeasurement,
};
use crate::hypothesis_models::{
    HypothesisComparison, PhononCascadeModel, SuperScreeningModel,
};
...
impl MultiPhysicsEncoder {
    ...
    fn initialize_bases(&mut self) {
        ...
        for (i, domain) in [
            ...
            self.domain_bases.insert(*domain, self.random_vector(i as u64));
        }
    }
...
impl PhysicsKnowledgeBase {
    ...
    pub fn find_similar(&self, query: &PhysicsQuantity, threshold: f64) -> Vec<(&PhysicsQuantity, f64)> {
        let mut encoder = self.encoder.clone();
        let query_vec = encoder.encode(query);

        let mut results: Vec<_> = self.facts
            .iter()
...
impl PatternAnalyzer {
    ...
    pub fn analyze_neutron_rates(&self, data: &[NeutronMeasurement]) -> Vec<PatternFit> {
        ...
        if let Some((r2, slope)) = self.linear_fit(&inv_temps, &log_rates) {
            ...
            patterns.push(PatternFit {
                ...
                r_squared: r2,
                parameters: [("activation_energy_ev".to_string(), activation_ev)].into_iter().collect(),
                interpretation: format!(
                    "Rate follows Arrhenius with E_a = {:.3} eV. \
                    This is {} than Gamow barrier (~100 keV).",
                    activation_ev,
                    if activation_ev < 100.0 { "MUCH lower" } else { "consistent" }
                ),
        ...
        if let Some((r2, exponent)) = self.fit_power_law(&rates, &loadings) {
            patterns.push(PatternFit {
                pattern_type: PatternType::PowerLaw,
                r_squared: r2,
                parameters: [("loading_exponent".to_string(), exponent)].into_iter().collect(),
                interpretation: format!(
    ...
    fn fit_power_law(&self, y: &[f64], x: &[f64]) -> Option<(f64, f64)> {
        ...
        let log_x: Vec<f64> = x.iter().filter_map(|v| if *v > 0.0 { Some(v.ln()) } else { None }).collect();
        let log_y: Vec<f64> = y.iter().filter_map(|v| if *v > 0.0 { Some(v.ln()) } else { None }).collect();
...
impl PhysicsDiscoveryEngine {
    ...
    pub fn analyze(&self) -> DiscoveryReport {
        ...
        let screening_patterns = self.patterns.analyze_screening(&self.literature.screening_data);
        let neutron_patterns = self.patterns.analyze_neutron_rates(&self.literature.neutron_data);
    ...
    fn find_cross_domain_correlations(&self) -> Vec<CrossDomainCorrelation> {
        ...
        correlations.push(CrossDomainCorrelation {
            ...
            mechanism: "X-rays create hot spots or excite phonons that enhance tunneling".to_string(),
        ...
        correlations.push(CrossDomainCorrelation {
            ...
            evidence: "PdD has 56 meV optical phonon, could provide ~0.17 keV with 3 modes".to_string(),
        });
    ...
    fn identify_open_questions(&self) -> Vec<OpenQuestion> {
        vec![
            OpenQuestion {
                question: "What mechanism bridges the remaining 20+ orders of magnitude gap?".to_string(),
                domains: vec![PhysicsDomain::Particle, PhysicsDomain::Quantum],
    ...
    fn recommend_experiments(&self) -> Vec<RecommendedExperiment> {
        vec![
            RecommendedExperiment {
                ...
                expected_outcome: "Peak at 2.45 MeV if D-D, different spectrum if artifacts".to_string(),
                priority: 1,
                estimated_difficulty: "Moderate - requires time-of-flight or spectrometer".to_string(),
            },
            RecommendedExperiment {
                title: "Temperature dependence mapping".to_string(),
                purpose: "Measure rate vs T to extract effective activation energy".to_string(),
                expected_outcome: "E_a << Coulomb barrier would indicate tunneling enhancement".to_string(),
                priority: 2,
            ...
            RecommendedExperiment {
                ...
                expected_outcome: "If Ue(Er) >> Ue(Pd), explains part of rate difference".to_string(),
                priority: 3,
            ...
            RecommendedExperiment {
                ...
                expected_outcome: "Higher rate with pulsed = phonon coherence important".to_string(),
                priority: 4,
    ...
    pub fn summary(&self) -> String {
        ...
        for cond in &self.hypothesis_comparison.required_conditions {
            s.push_str(&format!(
                ...
                cond.required_value,
                cond.unit,
                if cond.physically_reasonable { "plausible" } else { "implausible" }
            ));
        }
        s.push_str("\n");
        ...
        for insight in &self.insights {
            s.push_str(&format!("  • {} (confidence: {:.0}%)\n", insight.title, insight.confidence * 100.0));
        }
        s.push_str("\n");
...
mod tests {
    ...
    fn test_pattern_analyzer() {
        ...
        assert!(gap.gap_orders > 30.0, "Gap should be > 30 orders: {}", gap.gap_orders);
