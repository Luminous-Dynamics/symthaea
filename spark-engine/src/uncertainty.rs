...
impl TornadoDiagram {
    ...
    pub fn build(params: &ParameterDistributions) -> Self {
        ...
        let gamow_t_low = GamowIntegration::dd_rate(t_low.max(1.0), params.screening_ev.0, params.phonon_modes);
        let gamow_t_high = GamowIntegration::dd_rate(t_high, params.screening_ev.0, params.phonon_modes);
        ...
        let gamow_ue_low = GamowIntegration::dd_rate(params.temperature.0, ue_low, params.phonon_modes);
        let gamow_ue_high = GamowIntegration::dd_rate(params.temperature.0, ue_high, params.phonon_modes);
...
impl MonteCarloEngine {
    ...
    pub fn morris_analysis(&mut self, params: &ParameterDistributions, n_trajectories: usize) -> MorrisAnalysis {
        ...
        for _ in 0..n_trajectories {
            ...
            let x0: Vec<f64> = (0..n_params).map(|_| self.rng.gen()).collect();
            ...
            let transform = |x: &[f64]| -> (f64, f64, f64, f64) {
                ...
                (t.max(1.0), ue.max(0.0), load.clamp(0.1, 1.0), vol.max(0.001))
            };
        ...
        // Sort and rank by μ*
        effects.sort_by(|a, b| b.mu_star.partial_cmp(&a.mu_star).unwrap_or(std::cmp::Ordering::Equal));
...
impl FeasibilityProbability {
    ...
    pub fn compute(
        ...
        let n_dose_ok = neutron_rates.iter().filter(|&&r| r < dose_threshold).count();
        ...
        let n_temp_ok = power_densities.iter().filter(|&&p| p < power_threshold).count();
        let p_temp_safe = n_temp_ok as f64 / n as f64;

        // Lifetime constraint
        let lifetime_threshold = 5.0; // years
        let n_life_ok = lifetime_years.iter().filter(|&&l| l > lifetime_threshold).count();
        ...
        let primary_failure_mode = failure_counts.iter()
...
impl BreakEvenAnalyzer {
    ...
    pub fn analyze(
        ...
        let lcoe = |cap: f64, om: f64, cf: f64, life: f64| -> f64 {
            ...
            if annual_generation_mwh <= 0.0 { return f64::INFINITY; }
            (cap * crf(life) + om) / annual_generation_mwh + fuel_usd_mwh
        };

        let current_lcoe = lcoe(capital_usd_w, om_usd_w_year, capacity_factor, lifetime_years);
        ...
        let achievable = capital_improvement_needed < 10.0 &&
            cf_breakeven < 0.95 &&
            life_breakeven < 50.0;
        ...
        if cf_breakeven > capacity_factor {
            recommendations.push(format!(
                ...
                capacity_factor * 100.0, cf_breakeven * 100.0
            ));
        }
...
mod tests {
    ...
    fn test_feasibility_probability() {
        ...
        let feasibility = FeasibilityProbability::compute(
            &neutron_rates,
            &power_densities,
            &lifetimes,
        );
