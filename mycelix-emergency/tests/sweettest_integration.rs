//! # Mycelix Emergency - Sweettest Integration Tests
//!
...
mod incident_tests {
    ...
    async fn test_update_disaster_status() {
        ...
        let updated_record: Record = conductor
            .call(&cell.zome("incidents"), "update_disaster_status", update_input)
            .await;
    ...
    async fn test_add_incident_update_and_timeline() {
        ...
        let timeline: Vec<Record> = conductor
            .call(&cell.zome("incidents"), "get_disaster_timeline", disaster_hash)
            .await;
...
mod integration_tests {
    ...
    async fn test_multi_agent_disaster_response_flow() {
        ...
        let _: Record = conductor
            .call(&resp_cell.zome("incidents"), "add_incident_update", update_input)
            .await;
        ...
        let _: Record = conductor
            .call(&coord_cell.zome("incidents"), "add_incident_update", evac_input)
            .await;
        println!("  3. Coordinator issued evacuation order");

        // 4. Verify timeline
        let timeline: Vec<Record> = conductor
            .call(&coord_cell.zome("incidents"), "get_disaster_timeline", disaster_hash.clone())
            .await;
        assert!(timeline.len() >= 2, "Should have at least 2 timeline entries");
        ...
        let updated: Record = conductor
            .call(&coord_cell.zome("incidents"), "update_disaster_status", close_input)
            .await;
