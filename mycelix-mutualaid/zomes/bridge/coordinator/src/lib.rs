//! Bridge Coordinator Zome - Cross-hApp verification and MATL integration
//!
...
use bridge_integrity::{
    Anchor as BridgeAnchor, AidQuery, AidResult, BridgeConfig, EntryTypes,
    LinkTypes, MatlReputation, QueryPurpose, QueryStatus, VerificationRecord,
...
pub fn create_query(input: CreateQueryInput) -> ExternResult<QueryWithHash> {
    ...
    let expires_at = Timestamp::from_micros(
        now.as_micros() + (input.ttl_seconds as i64 * 1_000_000)
    );
...
pub fn submit_result(input: SubmitResultInput) -> ExternResult<ResultWithHash> {
    ...
    if now > query.expires_at {
        ...
        return Err(wasm_error!(WasmErrorInner::Guest("Query has expired".to_string())));
    }
...
fn calculate_trust_score(reputation: &MatlReputation) -> f64 {
    ...
    let activity_score = if reputation.pools_count > 0 {
        let participation = (reputation.successful_offers as f64 + reputation.requests_made as f64) / 10.0;
    ...
    let weighted =
        (reputation.contribution_score * contribution_weight) +
...
#[hdk_extern]
pub fn check_verification(input: CheckVerificationInput) -> ExternResult<Option<VerificationWithHash>> {
