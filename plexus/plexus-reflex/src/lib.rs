...
use plexus_common::{ContentZone, SafetyLevel, ThreatType};
use plexus_dom::DomTree;
use url::Url;
...
mod tests {
    ...
    fn postparse_clean_page() {
        let dom = plexus_dom::parse_html("<html><body><p>Hello world</p></body></html>");
    ...
    fn postparse_password_field_forces_private() {
        let dom = plexus_dom::parse_html(r#"<form><input type="password" /></form>"#);
        let pre = PreFetchVerdict { zone: ContentZone::Local, blocked: false, reason: "test" };
        let v = arc().post_parse(&dom, &pre);
        assert_eq!(v.zone, ContentZone::Private);
    }
