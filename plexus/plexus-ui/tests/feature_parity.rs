...
use plexus_common::{ContentZone, EmpiricalLevel, SafetyLevel};
use plexus_dom::parse_html;
use plexus_privacy::{encoding_gate, ConsentStore, EncodingDecision};
...
fn security_detects_prompt_injection() {
    ...
    assert!(!post.threats.is_empty(), "Should detect adversarial content");
}
...
fn security_detects_credential_harvesting() {
    ...
    let pre = reflex.pre_fetch(&url, false, false);
    let dom = parse_html("<body><p>Enter your password and your credit card number now.</p></body>");
    let post = reflex.post_parse(&dom, &pre);
...
fn security_no_false_positive_on_legitimate_content() {
    ...
    let pre = reflex.pre_fetch(&url, false, false);
    let dom = parse_html("<body><p>Computer security involves protecting systems from attack vectors and exploits. Buffer overflow is a common vulnerability.</p></body>");
    let post = reflex.post_parse(&dom, &pre);
    assert!(post.threats.is_empty(), "Educational security content should not trigger: {:?}", post.threats);
}
...
fn security_classifies_known_domains() {
    ...
    let v = reflex.pre_fetch(&url::Url::parse("https://en.wikipedia.org/wiki/Rust").unwrap(), false, false);
    assert_eq!(v.zone, ContentZone::Public);

    // Chase = PRIVATE (known bank)
    let v = reflex.pre_fetch(&url::Url::parse("https://www.chase.com").unwrap(), false, false);
    assert_eq!(v.zone, ContentZone::Private);

    // docs.rs = PUBLIC (known safe)
    let v = reflex.pre_fetch(&url::Url::parse("https://docs.rs/vello").unwrap(), false, false);
    assert_eq!(v.zone, ContentZone::Public);

    // Unknown = LOCAL (default)
    let v = reflex.pre_fetch(&url::Url::parse("https://random.example.com").unwrap(), false, false);
...
fn security_detects_noindex_robots() {
    ...
    let url = url::Url::parse("https://example.com").unwrap();
    let pre = reflex.pre_fetch(&url, false, false);
    let dom = parse_html(r#"<html><head><meta name="robots" content="noindex"></head><body><p>Secret</p></body></html>"#);
    let post = reflex.post_parse(&dom, &pre);
    assert_eq!(post.zone, ContentZone::Private, "noindex should force Private");
}
...
fn security_detects_password_forms() {
    ...
    let post = reflex.post_parse(&dom, &pre);
    assert_eq!(post.zone, ContentZone::Private, "Password forms should force Private");
}
...
fn security_sanitizes_xss() {
    ...
    assert!(!clean.contains("onerror"), "Event handlers must be stripped");
...
fn search_returns_relevant_results() {
    ...
    let topics = vec![
        ...
        ("consciousness awareness", vec!["consciousness", "awareness", "iit", "global workspace"]),
        ("rust programming", vec!["rust", "memory", "programming"]),
        ("quantum entanglement", vec!["quantum", "entanglement", "particles"]),
        ("DNA double helix", vec!["dna", "helix", "gene"]),
        ("climate greenhouse gas", vec!["climate", "greenhouse", "co2", "temperature"]),
        ("solar energy photovoltaic", vec!["solar", "photovoltaic", "energy"]),
    ];
    ...
    for (query, keywords) in &topics {
        ...
        if !results.is_empty() {
            ...
            if relevant { passes += 1; }
            else { eprintln!("  WEAK: '{}' → '{}'", query, &results[0].content[..results[0].content.len().min(80)]); }
        }
    }
    assert!(passes >= 5, "At least 5/7 topics should return relevant top result, got {}/7", passes);
}
...
fn search_epistemic_levels_present() {
    ...
    let has_e4 = results.iter().any(|r| r.empirical_level == EmpiricalLevel::E4);
    let has_e3 = results.iter().any(|r| r.empirical_level == EmpiricalLevel::E3);
...
fn search_results_have_sources() {
    ...
    for r in &results {
        assert!(!r.sources.is_empty(), "Every result should have a source: {}", r.content);
    }
}
...
fn search_results_ranked_by_score() {
    ...
    for i in 1..results.len() {
        assert!(results[i].rank_score() <= results[i-1].rank_score() + 0.001,
...
fn privacy_consent_store() {
    ...
    assert_eq!(store.resolve_zone(ContentZone::Local, "example.com"), ContentZone::Local);

    // Approve: upgrades to public
    store.approve_domain("example.com".to_string());
    assert_eq!(store.resolve_zone(ContentZone::Local, "example.com"), ContentZone::Public);

    // Deny: forces private
    store.deny_domain("evil.com".to_string());
    assert_eq!(store.resolve_zone(ContentZone::Local, "evil.com"), ContentZone::Private);
    assert_eq!(store.resolve_zone(ContentZone::Public, "evil.com"), ContentZone::Private);

    // Private can never be overridden
    store.approve_domain("bank.com".to_string());
    assert_eq!(store.resolve_zone(ContentZone::Private, "bank.com"), ContentZone::Private);
}
...
fn reader_mode_preserves_structure() {
    let dom = parse_html("<body><h1>Title</h1><p>Paragraph 1</p><p>Paragraph 2</p><ul><li>Item</li></ul></body>");
    let text = dom.extract_visible_text();
...
fn ingest_loads_mycelix_claims() {
    let claims = plexus_ingest::load_all_claims();
    assert!(claims.len() >= 100, "Should load 100+ claims, got {}", claims.len());
}
...
fn ingest_no_coordinate_only_claims() {
    ...
    for c in &claims {
        ...
        assert!(!(low.contains("latitude") && low.contains("longitude")),
            "Coordinate-only claim leaked through: {}", c.content);
    }
}
...
fn ingest_claims_have_valid_levels() {
    let claims = plexus_ingest::load_all_claims();
    let has_e4 = claims.iter().any(|c| c.empirical_level == EmpiricalLevel::E4);
    let has_e3 = claims.iter().any(|c| c.empirical_level == EmpiricalLevel::E3);
