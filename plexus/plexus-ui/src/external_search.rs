...
pub async fn search_duckduckgo(query: &str) -> ExternalResult {
    ...
    match gloo_net::http::Request::get(&url).send().await {
        ...
        Err(_) => ExternalResult::error("DuckDuckGo", "CORS proxy not running. Start with: cargo run -p plexus-proxy"),
    }
}
...
pub async fn search_brave(query: &str) -> ExternalResult {
    ...
    match gloo_net::http::Request::get(&url).send().await {
        ...
        Err(_) => ExternalResult::error("Brave", "CORS proxy not running. Start with: cargo run -p plexus-proxy"),
    }
}
...
pub async fn query_perplexity(query: &str) -> ExternalResult {
        ...
        .await
    {
        ...
        Err(_) => ExternalResult::error("Perplexity", "CORS proxy not running. Start with: cargo run -p plexus-proxy"),
    }
}
