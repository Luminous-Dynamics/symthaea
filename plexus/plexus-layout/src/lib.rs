...
//! Layout engine for Symthaea Prism.
//!
...
use plexus_dom::{DomTree, NodeData, NodeId};
...
fn style_for_tag(tag: &str, parent: &TextStyle, config: &LayoutConfig, attrs: &HashMap<String, String>) -> TextStyle {
    ...
    match tag {
        "h1" => { s.font_size = base * 2.0; s.bold = true; s.is_block = true; s.color = [0.05, 0.05, 0.05, 1.0]; }
        "h2" => { s.font_size = base * 1.5; s.bold = true; s.is_block = true; s.color = [0.08, 0.08, 0.08, 1.0]; }
        "h3" => { s.font_size = base * 1.25; s.bold = true; s.is_block = true; }
        "h4" => { s.font_size = base * 1.1; s.bold = true; s.is_block = true; }
        "h5" | "h6" => { s.font_size = base; s.bold = true; s.is_block = true; }
        "p" | "div" | "section" | "article" | "main" | "nav" | "aside" | "header" | "footer" | "blockquote" => { s.is_block = true; }
        "strong" | "b" => { s.bold = true; s.is_block = false; }
        "em" | "i" => { s.italic = true; s.is_block = false; }
        "a" => {
            s.is_link = true;
            s.is_block = false;
        ...
        "code" => { s.is_code = true; s.is_block = false; s.color = [0.72, 0.14, 0.23, 1.0]; }
        "pre" => { s.is_code = true; s.is_block = true; }
        "li" => { s.is_block = true; s.list_marker = Some("• "); }
        "ul" | "ol" => { s.is_block = true; }
        "hr" => { s.is_block = true; }
        "br" => { s.is_block = true; }
        _ => { s.is_block = false; }
    }
...
#[derive(Debug)]
enum Block {
    Paragraph { runs: Vec<TextRun>, margin_top: f32, margin_bottom: f32 },
    Rule,
    #[allow(dead_code)]
    Spacer(f32),
}
...
fn flatten_dom(dom: &DomTree, config: &LayoutConfig) -> Vec<Block> {
    let Some(root) = dom.root() else { return vec![] };
...
fn flatten_node(
    ...
    match &node.data {
        ...
        NodeData::Element(el) => {
            ...
            if matches!(tag, "script" | "style" | "noscript" | "head" | "template" | "meta" | "link" | "title") {
                return;
            }
            ...
            if tag == "br" {
                ...
                current_runs.push(TextRun { text: "\n".to_string(), style: style.clone() });
                return;
            }
...
fn flush_paragraph(runs: &mut Vec<TextRun>, blocks: &mut Vec<Block>, mt: f32, mb: f32) {
    ...
    if has_content {
        blocks.push(Block::Paragraph { runs: drained, margin_top: mt, margin_bottom: mb });
    }
}
...
pub fn layout_dom(dom: &DomTree, viewport_width: f32, _viewport_height: f32) -> Vec<PaintRect> {
    layout_dom_with_config(dom, viewport_width, _viewport_height, &LayoutConfig::default())
}
...
pub fn layout_dom_with_config(
    ...
    for block in &blocks {
        match block {
            Block::Paragraph { runs, margin_top, margin_bottom } => {
                y += margin_top;
                ...
                let para_style = runs.first().map(|r| &r.style).cloned()
...
mod tests {
    ...
    fn layout_basic_page() {
        ...
        let has_title = rects.iter().any(|r| {
            matches!(&r.content, PaintContent::Text { text, .. } if text.contains("Title"))
        });
    ...
    fn layout_empty_page() {
        let dom = parse_html("");
        let rects = layout_dom(&dom, 800.0, 600.0);
        let text_count = rects.iter().filter(|r| matches!(r.content, PaintContent::Text { .. })).count();
    ...
    fn layout_h1_bold_and_large() {
        ...
        let rects = layout_dom(&dom, 800.0, 600.0);
        let h1 = rects.iter().find(|r| matches!(&r.content, PaintContent::Text { text, .. } if text == "Big"));
        assert!(h1.is_some());
        if let PaintContent::Text { font_size, bold, .. } = &h1.unwrap().content {
            assert!(*bold, "h1 must be bold");
            assert!(*font_size > 20.0, "h1 font must be large, got {}", font_size);
        }
    }
    ...
    fn layout_vertical_order() {
        ...
        let rects = layout_dom(&dom, 800.0, 600.0);
        let texts: Vec<_> = rects.iter().filter(|r| matches!(r.content, PaintContent::Text { .. })).collect();
        for i in 1..texts.len() {
            assert!(texts[i].y >= texts[i - 1].y, "text {} y={} should be >= text {} y={}", i, texts[i].y, i-1, texts[i-1].y);
        }
    }
    ...
    fn layout_text_wraps() {
        ...
        let text_rects: Vec<_> = rects.iter().filter(|r| matches!(r.content, PaintContent::Text { .. })).collect();
        assert!(text_rects.len() > 1, "Long text should wrap to multiple lines, got {}", text_rects.len());
    }
    ...
    fn layout_list_has_markers() {
        ...
        let rects = layout_dom(&dom, 800.0, 600.0);
        let has_bullet = rects.iter().any(|r| {
            matches!(&r.content, PaintContent::Text { text, .. } if text.contains("•"))
        });
    ...
    fn layout_reader_mode_centered() {
        ...
        let rects = layout_dom_with_config(&dom, 1200.0, 800.0, &LayoutConfig {
            reader_mode: true,
            max_content_width: 680.0,
            ..Default::default()
        });
        let text_rect = rects.iter().find(|r| matches!(&r.content, PaintContent::Text { .. }));
    ...
    fn layout_hr_produces_rule() {
        ...
        let rects = layout_dom(&dom, 800.0, 600.0);
        let has_rule = rects.iter().any(|r| matches!(r.content, PaintContent::Rule { .. }));
