...
use plexus_common::{ContentZone, SafetyLevel};
use plexus_layout::{LayoutConfig, PaintContent, PaintRect};
use plexus_privacy::ConsentStore;
...
fn navigate_url(
    ...
    // Fetch live URL
    let client = plexus_net::PrismClient::new();
...
fn navigate_search(
    ...
    let html = plexus_search::render_search_results_html(query, &results);
    let url = format!("prism://search?q={}", query);
    let dom = plexus_dom::parse_html(&html);
    let rects = plexus_layout::layout_dom_with_config(&dom, vw, vh, &LAYOUT_CONFIG);
...
fn process_page(
    reflex: &ReflexArc,
    dom: &plexus_dom::DomTree,
    url_str: &str,
    metadata: &plexus_net::FetchMetadata,
    vw: f32,
    vh: f32,
        ...
        match event {
            ...
            WindowEvent::Resized(size) => {
                if size.width > 0 && size.height > 0 {
                    ...
                    let dom = plexus_dom::parse_html(&state.current_html);
                    state.paint_rects = plexus_layout::layout_dom_with_config(
...
fn run_text_mode() {
    ...
    for query in &["ocean acidification", "consciousness", "rust programming", "quantum physics"] {
        ...
        for (i, r) in results.iter().enumerate() {
            let e = match r.empirical_level {
                plexus_common::EmpiricalLevel::E4 => "\x1b[32mE4\x1b[0m",
                plexus_common::EmpiricalLevel::E3 => "\x1b[33mE3\x1b[0m",
                _ => "E?",
            };
    ...
    println!("╠══════════════════════════════════════════════════════════════════╣");
    println!("║  Run with display:  cargo run -p plexus-shell                  ║");
    println!("╚══════════════════════════════════════════════════════════════════╝");
}
