...
SUBJECT_SHA="d5fc6185b471f091f098073a492d3ff42aeb0931"
...
missing_results="${TMPDIR:-/tmp}/melothaea-runner-noack-output.$$"
rm -rf "$missing_results"
if cargo run --quiet --locked -p symthaea-muse --features theory --example "$EXAMPLE" -- \
    --output-dir "$missing_results" >"$missing_out" 2>&1; then
...
if [[ -e "$missing_results" ]]; then
    echo 'error: missing-ack path created result filesystem state' >&2
    rm -rf "$missing_results"
    rm -f "$missing_out"
    exit 1
fi
...
if [[ -e "$fake_dir/results" ]]; then
    echo 'error: invalid receipt created result filesystem state before rejection' >&2
    rm -rf "$fake_dir"
    exit 1
fi
rm -rf "$fake_dir"
