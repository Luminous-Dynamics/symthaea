...
"""Closed-world verifier for the Melothaea lockbox-wrapper qualifier."""
...
EXPECTED_FILE = pathlib.Path(
    ".github/qualification/execute-melothaea-contextual-harmony-lockbox-v1.sh"
)
SCRIPT_PATH = pathlib.Path(
    ".github/qualification/qualify-melothaea-lockbox-wrapper-v1.sh"
)
...
def main() -> int:
    ...
    if not receipt.is_file():
        raise VerificationError(f"receipt missing: {receipt}")
    values, repeated = parse(receipt)

    for key, expected in [
        ("schema", "melothaea-lockbox-wrapper-qualification-v1"),
        ("qualifier_id", "melothaea-lockbox-wrapper-qualification-v1"),
        ("status", "PASS"),
        ("exit_code", "0"),
        ("terminal_stage", "none"),
        ("authority_scope", "engineering-execution-wrapper-contract-only"),
        ...
        ("expected_stream_qualifier_sha", EXPECTED_STREAM_QUALIFIER_SHA),
        ("expected_runner_qualifier_sha", EXPECTED_RUNNER_QUALIFIER_SHA),
        ("source_state", "clean-exact-subject-checkout-postflight"),
    ]:
        expect(values, key, expected)
    if repeated["subject_changed_file"] != [str(EXPECTED_FILE)]:
        raise VerificationError("wrapper changed-file sequence is noncanonical")
    for gate in GATES:
    ...
    for key in ("qualifier_script_sha256", "wrapper_source_sha256"):
    ...
    if values["wrapper_source_sha256"] != sha256(repo / EXPECTED_FILE):
        raise VerificationError("wrapper source digest mismatch")
    ...
    if values["subject_tree"] != actual_tree or actual_parent != BASE_SHA:
        raise VerificationError("wrapper subject tree/parent mismatch")
    actual_files = run(repo, "git", "diff", "--name-only", BASE_SHA, SUBJECT_SHA).splitlines()
    if actual_files != [str(EXPECTED_FILE)]:
        raise VerificationError("wrapper subject surface mismatch")

    print(
        "PASS: wrapper qualification is canonical and exact-head bound with zero "
        "lockbox subjects observed"
    )
    print(
        "NOTE: this proves only shell syntax and fail-closed pre-admission behavior; "
        "it does not execute the lockbox or establish scientific outcomes."
    )
    return 0
