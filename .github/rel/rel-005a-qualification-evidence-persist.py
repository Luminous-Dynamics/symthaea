...
def self_test() -> dict[str, Any]:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        contract.write_text(json.dumps({
            ...
            "relation": "REL-005A",
            "source_pipeline_head": "46" * 20,
            "source_pipeline_tree": "d8" * 20,
            "allowed_comparison_results": ["ALL_PREDICATES_PASS", "PREDICATE_FAILURES"],
            "selected_comparison_result": None,
            "outcome_independent": True,
            "durable_qualification_evidence": {
                "payload_files": names,
                "persistence_receipt": RECEIPT,
                "artifact_file_count": 9,
                "persistence_receipt_must_not_interpret_scientific_result": True,
            },
            "claims": {
        ...
        for i, name in enumerate(names):
            (payload / name).write_bytes(f"{name}\nsynthetic-{i}\n".encode())
        a = build(contract, payload, root / "a.json")
        b = build(contract, payload, root / "b.json")
        ...
        try:
            build(contract, payload, root / "missing.json")
        except ValueError as exc:
            require("payload census mismatch" in str(exc), "missing-file rejection failed for wrong reason")
        else:
            raise ValueError("missing payload file was accepted")
    return {
        "schema": "symthaea.rel.qualification-evidence-persistence-self-test.v1",
        "authority": AUTHORITY,
        "deterministic_commitment": True,
        "exact_payload_census": True,
