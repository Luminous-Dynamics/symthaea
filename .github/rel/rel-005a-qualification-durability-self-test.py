#!/usr/bin/env python3
"""Adversarial static self-test for REL-005A QualificationDurabilityContractOnly v4."""
...
def validate(c:dict[str,Any])->dict[str,Any]:
    ...
    require(forbidden==["qualification_mapping","per_result_policy","result_specific_artifact_policy","selected_result_policy"],"forbidden result-dependent fields changed")
    for key in forbidden: require(key not in c,f"forbidden result-dependent field present: {key}")
    j=c["job_boundaries"]
    ...
    require(pc=={"schema":DOMAIN,"algorithm":"sha256","domain_separator":DOMAIN,"canonical_json":{"sort_keys":True,"separators":[",",":"],"ensure_ascii":True},"bind_relation":True,"bind_source_pipeline_head":True,"bind_source_pipeline_tree":True,"bind_ordered_payload_entries":True},"payload commitment contract mismatch")
...
def main()->None:
    ...
    receipt["adversarial_mutations_rejected"]=6
