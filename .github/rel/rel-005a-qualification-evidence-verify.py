#!/usr/bin/env python3
"""Independently verify durable REL-005A qualification evidence before packaging.
...
CONTRACT_SCHEMA="symthaea.rel.qualification-durability-contract.v4"
...
def verify(contract_path:pathlib.Path,durable_dir:pathlib.Path)->dict[str,Any]:
    ...
    require(receipt.get("schema")=="symthaea.rel.qualification-evidence-persistence-receipt.v2","persistence receipt schema mismatch")
    ...
    return {"schema":"symthaea.rel.qualification-evidence-persistence-verification-receipt.v2","authority":"QualificationEvidencePersistenceVerificationOnly","source_pipeline_head":c["source_pipeline_head"],"source_pipeline_tree":c["source_pipeline_tree"],"payload_commitment_domain":DOMAIN,"payload_file_count":len(payload),"payload_set_commitment_sha256":commitment,"exact_file_census_verified":True,"byte_lengths_verified":True,"sha256_commitments_verified":True,"source_identity_bound":True,"scientific_payload_parsed":False,"scientific_result_interpreted":False,"claims":{"qualification_completed":False,"rel_005a_qualified":False,"scientific_pass":False,"scientific_fail":False}}
...
def self_test()->dict[str,Any]:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        c={"schema":CONTRACT_SCHEMA,"authority":"QualificationDurabilityContractOnly","relation":"REL-005A","source_pipeline_head":"46"*20,"source_pipeline_tree":"d8"*20,"allowed_comparison_results":["ALL_PREDICATES_PASS","PREDICATE_FAILURES"],"selected_comparison_result":None,"outcome_independent":True,"forbidden_result_dependent_fields":["qualification_mapping","per_result_policy"],"durable_qualification_evidence":{"payload_files":names,"persistence_receipt":RECEIPT_NAME,"artifact_file_count":9,"persistence_receipt_verification_required_before_packaging":True,"packaging_may_not_substitute_transport_digest_for_inner_verification":True,"payload_commitment":{"schema":DOMAIN,"domain_separator":DOMAIN}}}
    ...
    return {"schema":"symthaea.rel.qualification-evidence-persistence-verification-self-test.v2","authority":"QualificationEvidencePersistenceVerificationOnly","valid_artifact_accepted":True,"tampered_payload_rejected":True,"tampered_receipt_rejected":True,"source_identity_bound":True,"scientific_payload_parsed":False,"scientific_result_interpreted":False}
