            ...
            OpEntry::CreateEntry { app_entry, action } => {
                match app_entry {
                    ...
                    EntryTypes::ContentVerificationRequest(request) => {
                        validate_create_content_verification_request(EntryCreationAction::Create(action), request)
                    }
                    EntryTypes::ContentVerificationResult(_) => {
                        Ok(ValidateCallbackResult::Valid)
                    }
                    EntryTypes::AuthorReputationQuery(query) => {
                        validate_create_author_reputation_query(EntryCreationAction::Create(action), query)
                    }
                    EntryTypes::AuthorReputationResult(result) => {
                        validate_create_author_reputation_result(EntryCreationAction::Create(action), result)
                    }
                    EntryTypes::ContentReference(_) => {
                        Ok(ValidateCallbackResult::Valid)
                    }
...
fn validate_create_author_reputation_query(
    ...
    if !query.author_did.starts_with("did:mycelix:") {
        return Ok(ValidateCallbackResult::Invalid("Author must have valid DID".into()));
    }
    Ok(ValidateCallbackResult::Valid)
}
...
fn validate_create_author_reputation_result(
    ...
    if result.average_quality_score < 0.0 || result.average_quality_score > 1.0 {
        return Ok(ValidateCallbackResult::Invalid("Quality score must be 0.0-1.0".into()));
    }
    Ok(ValidateCallbackResult::Valid)
}
