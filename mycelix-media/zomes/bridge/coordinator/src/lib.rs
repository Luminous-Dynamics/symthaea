...
pub fn verify_content(input: VerifyContentInput) -> ExternResult<ContentVerificationResult> {
    ...
    let request = ContentVerificationRequest {
        id: format!("verify:{}:{}:{}", input.source_happ, input.content_hash, now.as_micros()),
    ...
    create_link(anchor_hash(&input.content_hash)?, hash, LinkTypes::ContentToVerification, ())?;
...
#[hdk_extern]
pub fn query_author_reputation(input: QueryAuthorReputationInput) -> ExternResult<AuthorReputationResult> {
    let now = sys_time()?;

    let query = AuthorReputationQuery {
        id: format!("author:{}:{}:{}", input.source_happ, input.author_did, now.as_micros()),
    ...
    create_link(anchor_hash(&input.author_did)?, hash, LinkTypes::AuthorToReputation, ())?;
...
pub fn register_content_reference(input: RegisterContentReferenceInput) -> ExternResult<Record> {
    ...
    let reference = ContentReference {
        id: format!("ref:{}:{}:{}", input.source_happ, input.content_hash, now.as_micros()),
        content_hash: input.content_hash.clone(),
        source_happ: input.source_happ.clone(),
        title: input.title,
    ...
    get(hash, GetOptions::default())?
        .ok_or(wasm_error!(WasmErrorInner::Guest("Reference not found".into())))
}
...
pub fn get_content_by_happ(source_happ: String) -> ExternResult<Vec<Record>> {
    let query = LinkQuery::new(anchor_hash(&source_happ)?, LinkTypeFilter::single_type(0.into(), (LinkTypes::HappToContent as u8).into()));
