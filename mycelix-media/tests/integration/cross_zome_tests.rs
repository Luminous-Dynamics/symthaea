...
fn create_test_attribution(publication: &TestPublication, contributor_did: &str, share: f64) -> TestAttribution {
    TestAttribution {
        id: format!("attr:{}:{}:1704067200000000", publication.id, contributor_did),
...
fn create_test_endorsement(publication: &TestPublication, endorser_did: &str) -> TestEndorsement {
    TestEndorsement {
        id: format!("endorse:{}:{}:1704067200000000", publication.id, endorser_did),
        publication_id: publication.id.clone(),
...
mod tests {
    ...
    fn test_attribution_verification_workflow() {
        let publication = create_test_publication("did:mycelix:author1");
        let mut attribution = create_test_attribution(&publication, "did:mycelix:contributor1", 25.0);
    ...
    fn test_collection_with_quality_filtered_publications() {
        ...
        let high_quality: Vec<&TestQualityScore> = quality_scores
            .iter()
    ...
    fn test_publication_fact_checks() {
        ...
        // Count verdicts
        let true_count = fact_checks.iter().filter(|fc| fc.verdict == "True" || fc.verdict == "MostlyTrue").count();
        let false_count = fact_checks.iter().filter(|fc| fc.verdict == "False").count();
    ...
    fn test_contributor_reputation_from_endorsements() {
        ...
        let reputation_boost = (endorsement_count as f64 / 50.0).min(1.0) * (attribution.share_percentage / 100.0);
    ...
    fn test_complete_publication_lifecycle() {
        ...
        let fact_check_score = if fact_check.verdict == "True" { 1.0 } else { 0.5 };
    ...
    fn test_attribution_chain_for_derivative_works() {
        ...
        let total: f64 = derivative_attributions.iter().map(|a| a.share_percentage).sum();
        assert_eq!(total, 100.0);

        // Original author gets attribution in derivative
        assert!(derivative_attributions.iter().any(|a| a.contributor_did == original.author_did));
    }
