...
mod tests {
    ...
    fn test_only_endorser_can_remove() {
        let endorsement = create_test_endorsement();
        assert!(can_remove_endorsement(&endorsement, &endorsement.endorser_did));
    ...
    fn test_private_collection_only_visible_to_curator() {
        ...
        assert!(can_view_collection(&collection, Some(&collection.curator_did)));
    }
    ...
    fn test_collection_with_many_publications() {
        let mut collection = create_test_collection();
        collection.publication_ids = (0..1000)
            .map(|i| format!("pub{}", i))
            .collect();
    ...
    fn test_award_endorsement_with_custom_name() {
        ...
        assert!(matches!(endorsement.endorsement_type, TestEndorsementType::Award(_)));
    }
