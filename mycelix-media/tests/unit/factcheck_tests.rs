...
fn create_test_fact_check() -> TestFactCheck {
    TestFactCheck {
        ...
        claim_text: "The global temperature has risen by 1.5 degrees Celsius since 1900".to_string(),
...
mod tests {
    ...
    fn filter_by_verdict<'a>(checks: &'a [TestFactCheck], verdict: &TestFactCheckVerdict) -> Vec<&'a TestFactCheck> {
    ...
    fn test_fact_check_many_evidence_items() {
        ...
        let supporting: Vec<&TestEvidenceItem> = check.evidence.iter().filter(|e| e.supports_claim).collect();
    ...
    fn test_epistemic_position_edge_values() {
        let positions = vec![
            TestEpistemicPosition { empirical: 0.0, normative: 0.0, mythic: 0.0 },
            TestEpistemicPosition { empirical: 1.0, normative: 1.0, mythic: 1.0 },
            TestEpistemicPosition { empirical: 0.0001, normative: 0.0001, mythic: 0.0001 },
        ];
        for ep in positions {
            assert!(validate_epistemic_position(&ep).is_ok());
        }
    ...
    fn test_multiple_fact_checks_same_publication() {
        ...
        let pub_checks: Vec<&TestFactCheck> = checks.iter().filter(|c| c.publication_id == pub_id).collect();
