        ...
        if contributor_pubs.contains(&&usage.publication_id) {
            if let Some(paid) = usage.royalty_paid {
                ...
                if let Some(attr) = attributions.iter().find(|a|
                    a.publication_id == usage.publication_id &&
                    a.contributor_did == contributor_did
                ) {
...
mod tests {
    ...
    fn test_usage_types_supported() {
        ...
        for usage_type in types {
            ...
            assert!(matches!(usage.usage_type, TestUsageType::View |
                TestUsageType::Share | TestUsageType::Download |
                TestUsageType::Derivative | TestUsageType::Citation));
        }
    }
    ...
    fn test_contributor_earnings_shared_royalties() {
        ...
        let author_earnings = calculate_contributor_earnings(
            &attributions,
            &usage_records,
            "did:mycelix:author",
        );
        ...
        let editor_earnings = calculate_contributor_earnings(
            &attributions,
            &usage_records,
            "did:mycelix:editor",
        );
    ...
    fn test_only_contributor_can_verify() {
        let attribution = create_test_attribution();
        assert!(can_verify_attribution(&attribution, &attribution.contributor_did));
    ...
    fn get_publication_shares(attributions: &[TestAttribution], publication_id: &str) -> PublicationShares {
