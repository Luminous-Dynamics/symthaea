...
mod tests {
    ...
    fn test_publication_has_unique_id() {
        ...
        assert!(publication.id.contains(&publication.author_did.replace("did:", "")));
    }
    ...
    fn test_custom_content_type() {
        ...
        assert!(matches!(publication.content_type, TestContentType::Other(_)));
    }
    ...
    fn test_license_types_supported() {
        ...
        for license_type in types {
            ...
            assert!(matches!(license.license_type, TestLicenseType::CC0 |
                TestLicenseType::CCBY | TestLicenseType::CCBYSA |
                TestLicenseType::CCBYNC | TestLicenseType::CCBYNCSA |
                TestLicenseType::AllRightsReserved | TestLicenseType::Custom(_)));
        }
    }
    ...
    fn test_only_author_can_update() {
        let publication = create_test_publication();
        assert!(can_update_publication(&publication, &publication.author_did));
    ...
    fn test_only_author_can_delete() {
        let publication = create_test_publication();
        assert!(can_delete_publication(&publication, &publication.author_did));
    ...
    fn test_co_author_cannot_update() {
        ...
        assert!(!can_update_publication(&publication, "did:mycelix:coauthor1"));
    }
    ...
    fn filter_by_content_type<'a>(publications: &'a [TestPublication], content_type: &TestContentType) -> Vec<&'a TestPublication> {
        publications.iter().filter(|p| &p.content_type == content_type).collect()
    }

    fn filter_by_language<'a>(publications: &'a [TestPublication], language: &str) -> Vec<&'a TestPublication> {
        publications.iter().filter(|p| p.language == language).collect()
    }

    fn filter_by_tag<'a>(publications: &'a [TestPublication], tag: &str) -> Vec<&'a TestPublication> {
        publications.iter().filter(|p| p.tags.contains(&tag.to_string())).collect()
    }
