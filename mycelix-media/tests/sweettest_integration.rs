//! # Mycelix Media - Sweettest Integration Tests
//!
...
mod publication_tests {
    ...
    async fn test_publish_and_retrieve() {
        ...
        let retrieved: Option<Record> = conductor
            .call(&cell.zome("publication"), "get_publication", pub_entry.id.clone())
            .await;
    ...
    async fn test_search_by_tag() {
        ...
        let results: Vec<Record> = conductor
            .call(&cell.zome("publication"), "search_by_tag", "climate".to_string())
            .await;
    ...
    async fn test_get_author_publications() {
        ...
        let results: Vec<Record> = conductor
            .call(&cell.zome("publication"), "get_author_publications", author.clone())
            .await;

        assert!(results.len() >= 3, "Should have at least 3 publications for author");
    }
}
...
mod factcheck_tests {
    ...
    async fn test_submit_and_retrieve_fact_check() {
        ...
        let checks: Vec<Record> = conductor
            .call(&cell.zome("factcheck"), "get_publication_fact_checks", publication.id.clone())
            .await;

        assert!(!checks.is_empty(), "Should have fact checks for publication");
    }
}
...
mod curation_tests {
    ...
    async fn test_endorse_publication() {
        ...
        let endorsements: Vec<Record> = conductor
            .call(&cell.zome("curation"), "get_publication_endorsements", publication.id.clone())
            .await;
    ...
    async fn test_quality_score_calculation() {
        ...
        let score: QualityScore = conductor
            .call(&cell.zome("curation"), "calculate_quality_score", publication.id.clone())
            .await;
...
mod attribution_tests {
    ...
    async fn test_add_and_get_attribution() {
        ...
        let attributions: Vec<Record> = conductor
            .call(&cell.zome("attribution"), "get_publication_attributions", publication.id.clone())
            .await;
...
mod lifecycle_tests {
    ...
    async fn test_publish_factcheck_endorse_lifecycle() {
        ...
        assert!(score.score > 0.0, "Quality score should be positive after fact-check and endorsement");
    }
}
