...
use crate::hdc::consciousness_verifier::{ConsciousnessVerifier, ConsciousnessVerdict};
...
fn test_text_to_consciousness_verdict() {
    ...
    let bridge = SemanticBridge::with_vocabulary(&[
        "light", "dark", "warm", "cold", "sound", "silence",
    ...
    assert!(rank >= 1, "Rich multi-modal input should produce at least some consciousness signal, got {:?}", report.verdict);

    // Verification should have run at least once (at cycle 5 and 10)
    let verification = pipeline.latest_verification();
    assert!(verification.is_some(), "Periodic verification should have produced a report");
}
...
fn test_semantic_similarity_affects_binding() {
    let bridge = SemanticBridge::with_vocabulary(&[
        "cat", "dog", "kitten", "puppy", "tree", "rock",
    ]);
...
fn test_phi_feedback_drives_adaptation() {
    ...
    for i in 0..20 {
        let inputs = vec![BinaryHV::random(300 + i as u64), BinaryHV::random(400 + i as u64)];
    ...
    assert!(modulation.is_some(), "Should have modulation after 20 cycles");
