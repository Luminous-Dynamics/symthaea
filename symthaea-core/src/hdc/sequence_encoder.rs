...
impl SequenceEncoder {
    ...
    pub fn encode_vectors(&self, vectors: &[Vec<i8>]) -> Vec<i8> {
        ...
        let permuted: Vec<Vec<i8>> = vectors
            .iter()
            .enumerate()
    ...
    pub fn encode_bytes(&self, bytes: &[u8]) -> Result<Vec<i8>> {
        let atoms = self.atom_vectors.as_ref().ok_or_else(|| {
            anyhow::anyhow!("Byte encoding requires atom vectors - use with_byte_atoms()")
        })?;

        let vectors: Vec<Vec<i8>> = bytes.iter().map(|&b| atoms[b as usize].clone()).collect();
...
mod tests {
    ...
    fn test_permute_unpermute_inverse() {
        ...
        for k in [1, 5, 100, 1000, 2047] {
            ...
            assert_eq!(
                hv, restored,
                "Unpermute should be inverse of permute for k={}",
                k
            );
        }
    }
    ...
    fn test_permute_creates_orthogonal() {
        ...
        assert!(
            sim_0_1 < 0.55,
            "permute(hv, 0) vs permute(hv, 1) should be ~orthogonal, got {}",
            sim_0_1
        );
        assert!(
            sim_0_100 < 0.55,
            "permute(hv, 0) vs permute(hv, 100) should be ~orthogonal, got {}",
            sim_0_100
        );
    }
    ...
    fn test_bind_commutative() {
        ...
        assert_eq!(
            ab, ba,
            "Binding should be commutative (this is WHY we need permute!)"
        );
    }
    ...
    fn test_bind_self_inverse() {
        ...
        assert_eq!(
            b, recovered_b,
            "Binding with same vector should recover original"
        );
    }
    ...
    fn test_sequence_order_matters() {
        ...
        assert!(
            sim < 0.9,
            "Different orderings should produce different sequences, got similarity {}",
            sim
        );
        ...
        assert_eq!(
            seq1, seq1_again,
            "Same ordering should produce same sequence"
        );
    }
    ...
    fn test_probe_position_recovery() {
        ...
        assert!(
            sim_0_a > sim_0_b,
            "Position 0 probe should be more similar to a than b"
        );
        assert!(
            sim_1_b > 0.6,
            "Position 1 probe should recover b, got {}",
            sim_1_b
        );
        assert!(
            sim_2_c > 0.6,
            "Position 2 probe should recover c, got {}",
            sim_2_c
        );
    }
    ...
    fn test_role_filler_encoding() {
        ...
        let frame =
            encoder.encode_role_fillers(&[(&agent, &cat), (&action, &chased), (&patient, &mouse)]);
        ...
        assert!(
            sim_cat > sim_mouse,
            "Agent query should return cat, not mouse"
        );
        assert!(
            sim_cat > sim_chased,
            "Agent query should return cat, not chased"
        );
        assert!(
            sim_cat > 0.6,
            "Agent query should strongly match cat, got {}",
            sim_cat
        );
    }
    ...
    fn test_string_encoding() {
        ...
        assert!(
            sim_hello_olleh < 0.7,
            "Reversed string should be different, got {}",
            sim_hello_olleh
        );
    }
    ...
    fn test_bundle_preserves_all() {
        ...
        assert!(
            sim_a > 0.55 && sim_a < 0.85,
            "Bundle should be moderately similar to a, got {}",
            sim_a
        );
        assert!(
            sim_b > 0.55 && sim_b < 0.85,
            "Bundle should be moderately similar to b, got {}",
            sim_b
        );
        assert!(
            sim_c > 0.55 && sim_c < 0.85,
            "Bundle should be moderately similar to c, got {}",
            sim_c
        );
    }
    ...
    fn test_sequence_with_statistical_retriever() {
        ...
        assert!(
            ...
            "Position probe should have significant z-score for original item, got {}",
            decision.z_score
        );
    }
    ...
    fn test_long_sequence_degradation() {
        ...
        let items: Vec<Vec<i8>> = (0..50).map(|_| encoder.random_hv()).collect();
        ...
        // Note: Long sequences may have marginal signal; threshold is relaxed
        assert!(
            sim_0 > 0.51,
            "Position 0 in long sequence should be recoverable, got {}",
            sim_0
        );
        assert!(
            sim_25 > 0.51,
            "Position 25 in long sequence should be recoverable, got {}",
            sim_25
        );
        assert!(
            sim_49 > 0.51,
            "Position 49 in long sequence should be recoverable, got {}",
            sim_49
        );

        println!(
            "Long sequence (50 items) recovery: pos0={:.3}, pos25={:.3}, pos49={:.3}",
            sim_0, sim_25, sim_49
        );
    }
}
