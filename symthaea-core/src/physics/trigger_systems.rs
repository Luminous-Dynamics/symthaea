...
pub struct GamowIntegrationResult {
    ...
    /// Gain factor Q (fusion power / input power) - requires density and confinement
    pub gain_factor_q: Option<f64>,
    /// Whether Q > 1 (net energy gain) is achievable with this reaction rate
...
pub struct QFactorParams {
    ...
    pub input_power_density_w_cm3: f64,
}

impl QFactorParams {
    /// Typical LCF conditions in PdD lattice
    /// D/Pd = 0.7, Pd density = 12.02 g/cm³
    pub fn lcf_typical() -> Self {
        // Pd atoms/cm³ = ρ × N_A / A = 12.02 × 6.022e23 / 106.42 = 6.8e22
        let pd_per_cm3 = 6.8e22;
        // D atoms at D/Pd = 0.7
        let n_d = pd_per_cm3 * 0.7;
        Self {
            n_d_per_cm3: n_d,
            // Very long confinement in solid (D is trapped in lattice)
            tau_e_s: 1.0, // Assume 1 second effective confinement
            // Typical trigger input: ~1 W/cm³
            input_power_density_w_cm3: 1.0,
        }
    }
}
...
impl LcfPhysicsConstants {
    ...
    pub fn dd_reaction_rate_integrated(
        ...
        GamowIntegrationResult {
            ...
            phonon_enhancement,
            gain_factor_q: None, // Computed separately with density info
            energy_gain_achievable: false, // Conservative default
        }
    }

    /// Compute Q factor (fusion power / input power) for given conditions.
    ///
    /// Q = n² × <σv> × E_fusion × V / P_input
    ///
    /// For Q > 1 (net energy gain), the Lawson criterion must be satisfied:
    ///   n × τ_E × <σv> × E_fusion > 1
    ///
    /// At room temperature with LCF enhancement, Q >> 1 is NOT achievable
    /// because <σv> ~ 10⁻⁵⁰ cm³/s even with 100× screening enhancement.
    ///
    /// This method provides an honest assessment of physics feasibility.
    pub fn compute_q_factor(gamow: &GamowIntegrationResult, params: &QFactorParams) -> (f64, bool) {
        let sigma_v = gamow.sigma_v_cm3_s;

        // Fusion power density: P_fus = n² × <σv> × E_fusion / 4
        // (factor of 4 for identical particles: n_D² / 2 and <σv>/2)
        // E_fusion = 3.65 MeV = 5.84e-13 J
        let e_fusion_j = 3.65e6 * 1.602e-19;
        let n_d = params.n_d_per_cm3;

        // Fusion power density (W/cm³)
        let p_fusion_per_cm3 = (n_d * n_d / 4.0) * sigma_v * e_fusion_j;

        // Q factor = P_fusion / P_input
        let q = if params.input_power_density_w_cm3 > 0.0 {
            p_fusion_per_cm3 / params.input_power_density_w_cm3
        } else {
            0.0
        };

        // Energy gain achievable if Q > 1
        // Note: At room temperature, even with all enhancements, Q << 1
        // because <σv> is astronomically small
        let achievable = q > 1.0;

        (q, achievable)
    }

    /// Compute Gamow integration WITH Q factor calculation.
    ///
    /// Same as `dd_reaction_rate_integrated` but also computes Q factor
    /// using the provided density and confinement parameters.
    pub fn dd_reaction_rate_with_q(
        t_lattice_k: f64,
        host_ue_ev: f64,
        n_phonon_modes: u32,
        q_params: &QFactorParams,
    ) -> GamowIntegrationResult {
        let mut result = Self::dd_reaction_rate_integrated(t_lattice_k, host_ue_ev, n_phonon_modes);
        let (q, achievable) = Self::compute_q_factor(&result, q_params);
        result.gain_factor_q = Some(q);
        result.energy_gain_achievable = achievable;
        result
    }
...
/// Lattice degradation state tracking for D/Pd ratio and damage accumulation.
///
/// Key physics:
/// - D-D neutrons (2.45 MeV) cause displacement damage in Pd lattice
/// - Damage accumulates as DPA (displacements per atom)
/// - Damaged sites reduce D loading capacity (D/Pd ratio decays)
/// - Lattice must be replaced when D/Pd drops below critical threshold
///
/// Reference: Zinkle & Was, Acta Mater. 61 (2013) 735-758
#[derive(Debug, Clone)]
pub struct LatticeLifetimeModel {
    ...
    /// Current D/Pd loading ratio (decays with damage)
    pub current_d_pd_ratio: f64,
    /// Minimum D/Pd ratio for effective operation
    pub critical_d_pd_ratio: f64,
    /// DPA rate (DPA/year) at current operating conditions
    ...
    /// Annealing recovery factor (0-1, higher = more recovery)
    pub annealing_recovery: f64,
}

impl LatticeLifetimeModel {
    /// Critical DPA threshold for Pd lattice (above this, severe degradation)
    /// Based on void swelling onset in FCC metals
    pub const CRITICAL_DPA_PD: f64 = 10.0;

    /// D/Pd ratio decay coefficient (per DPA)
    /// Empirical: loading capacity decreases ~5% per DPA
    pub const D_PD_DECAY_PER_DPA: f64 = 0.05;

    /// Minimum D/Pd ratio for fusion to be viable
    pub const MIN_D_PD_RATIO: f64 = 0.5;

    /// Create a new lattice with initial conditions.
    pub fn new(initial_d_pd_ratio: f64) -> Self {
        Self {
            accumulated_dpa: 0.0,
            initial_d_pd_ratio,
            current_d_pd_ratio: initial_d_pd_ratio,
            critical_d_pd_ratio: Self::MIN_D_PD_RATIO,
            dpa_rate_per_year: 0.0,
            remaining_lifetime_years: f64::INFINITY,
    ...
    /// Typical PdD lattice starting at D/Pd = 0.7
    pub fn pdd_typical() -> Self {
        Self::new(0.7)
    }

    /// Calculate DPA rate from neutron flux.
    ///
    /// For D-D neutrons (2.45 MeV) in Pd:
    /// - Displacement threshold Ed = 34 eV (Pd)
    /// - Modified Kinchin-Pease: ν = 0.8 × E_pka / (2 × Ed)
    /// - DPA = neutron_flux × σ_el × ν × time
    ///
    /// Reference: Stoller, J. Nucl. Mater. 276 (2000) 107
    pub fn compute_dpa_rate(
        neutron_flux_n_cm2_s: f64,
        neutron_energy_mev: f64,
    ) -> f64 {
        // Displacement threshold for Pd (eV)
        let e_d_ev = 34.0;

        // Elastic scattering cross-section for Pd at ~2 MeV (barn)
        // ENDF/B-VIII.0: σ_el ≈ 3.5 barn for 2.45 MeV neutrons on Pd-106
        let sigma_el_barn = if neutron_energy_mev < 5.0 {
            3.5
        } else {
            2.5 // Higher energy, lower cross-section
        };
        let sigma_el_cm2 = sigma_el_barn * 1e-24;

        // Average recoil energy (elastic scattering)
        // T_avg = 2 × E_n × A / (A + 1)² where A = mass number
        // For Pd-106: T_avg ≈ 2 × 2.45 × 106 / 107² ≈ 45 keV
        let a_pd = 106.0;
        let t_avg_kev = 2.0 * neutron_energy_mev * 1000.0 * a_pd / (a_pd + 1.0).powi(2);
        let t_avg_ev = t_avg_kev * 1000.0;

        // Modified Kinchin-Pease: displacements per PKA
        // ν = 0.8 × T / (2 × Ed) for T > 2.5 × Ed
        let nu = if t_avg_ev > 2.5 * e_d_ev {
            0.8 * t_avg_ev / (2.0 * e_d_ev)
        } else {
            1.0 // Below threshold: single displacement
        };

        // Pd number density (atoms/cm³)
        // ρ = 12.02 g/cm³, A = 106.42, N_A = 6.022e23
        let n_pd = 12.02 * 6.022e23 / 106.42;

        // DPA rate = flux × σ × ν / N
        let dpa_per_s = neutron_flux_n_cm2_s * sigma_el_cm2 * nu / n_pd;

        // Convert to DPA/year
        dpa_per_s * 3600.0 * 24.0 * 365.25
    }

    /// Update lattice state for given operating time.
    ///
    /// `delta_hours`: Operating time increment
    /// `neutron_rate_s`: Neutron production rate (n/s)
    /// `core_surface_cm2`: Surface area for neutron flux calculation
    /// `lattice_temp_k`: Temperature (affects annealing)
    pub fn update(
        &mut self,
        delta_hours: f64,
        neutron_rate_s: f64,
        core_surface_cm2: f64,
        lattice_temp_k: f64,
    ) {
        self.operating_hours += delta_hours;

        // Neutron flux at lattice surface (assume uniform distribution)
        let flux = neutron_rate_s / core_surface_cm2;

        // DPA rate for 2.45 MeV D-D neutrons
        self.dpa_rate_per_year = Self::compute_dpa_rate(flux, 2.45);

        // Accumulate damage
        let delta_years = delta_hours / (24.0 * 365.25);
        let delta_dpa = self.dpa_rate_per_year * delta_years;

        // Apply annealing recovery at elevated temperatures
        // Arrhenius recovery: increases with T above ~400K
        self.annealing_recovery = if lattice_temp_k > 400.0 {
            // Recovery rate increases exponentially with temperature
            // E_a ≈ 1.0 eV for vacancy migration in Pd
            let kb = 8.617e-5; // eV/K
            let e_a = 1.0;
            let recovery_factor = (-e_a / (kb * lattice_temp_k)).exp()
                / (-e_a / (kb * 600.0)).exp(); // Normalized to 600K
            (recovery_factor * 0.3).min(0.9) // Max 90% recovery
        } else {
            0.0
        };

        // Net damage accumulation (after recovery)
        let net_dpa = delta_dpa * (1.0 - self.annealing_recovery);
        self.accumulated_dpa += net_dpa;

        // D/Pd ratio decay with damage
        // Exponential decay: ratio = initial × exp(-k × DPA)
        let decay_factor = (-Self::D_PD_DECAY_PER_DPA * self.accumulated_dpa).exp();
        self.current_d_pd_ratio = self.initial_d_pd_ratio * decay_factor;

        // Check if replacement needed
        self.needs_replacement = self.current_d_pd_ratio < self.critical_d_pd_ratio
            || self.accumulated_dpa > Self::CRITICAL_DPA_PD;

        // Estimate remaining lifetime
        if self.dpa_rate_per_year > 0.0 && !self.needs_replacement {
            // Time until D/Pd drops to critical level
            // critical = initial × exp(-k × DPA_max)
            // DPA_max = -ln(critical/initial) / k
            let dpa_to_critical = -(self.critical_d_pd_ratio / self.initial_d_pd_ratio).ln()
                / Self::D_PD_DECAY_PER_DPA;
            let dpa_remaining = (dpa_to_critical - self.accumulated_dpa).max(0.0);
            self.remaining_lifetime_years = dpa_remaining / self.dpa_rate_per_year;
        } else if self.needs_replacement {
            self.remaining_lifetime_years = 0.0;
        }
    }

    /// Reset lattice (replacement).
    pub fn replace(&mut self) {
        self.accumulated_dpa = 0.0;
        self.current_d_pd_ratio = self.initial_d_pd_ratio;
        self.needs_replacement = false;
        self.operating_hours = 0.0;
        self.remaining_lifetime_years = f64::INFINITY;
    }

    /// Compute replacement schedule for given operating conditions.
    ///
    /// Returns: (years_between_replacements, total_replacements_over_lifetime)
    pub fn replacement_schedule(
        ...
        // Time to reach critical D/Pd ratio
        let dpa_to_critical = -(self.critical_d_pd_ratio / self.initial_d_pd_ratio).ln()
            / Self::D_PD_DECAY_PER_DPA;
        let years_between = dpa_to_critical / dpa_rate;

        let total_replacements = if years_between > 0.0 {
            (reactor_lifetime_years / years_between).ceil() as u32
        } else {
            0
        };

        (years_between, total_replacements)
    }
}
...
mod tests {
    ...
    // === Enhancement #3: Q Factor Tests ===

    #[test]
    fn test_q_factor_params_default() {
        let params = QFactorParams::lcf_typical();
        assert!(params.n_d_per_cm3 > 1e22, "D density should be high in PdD");
        assert!(params.tau_e_s > 0.0, "Confinement time should be positive");
        assert!(params.input_power_density_w_cm3 > 0.0, "Input power should be positive");
    }

    #[test]
    fn test_q_factor_less_than_one_at_room_temp() {
        // This is the critical physics honesty test:
        // At room temperature, even with all enhancements, Q << 1
        let gamow = LcfPhysicsConstants::dd_reaction_rate_integrated(300.0, 309.0, 2);
        let params = QFactorParams::lcf_typical();
        let (q, achievable) = LcfPhysicsConstants::compute_q_factor(&gamow, &params);

        // Q should be astronomically small at room temperature
        // because <σv> ~ 10^-50 cm³/s
        assert!(q < 1.0, "Q must be < 1 at room temperature, got {}", q);
        assert!(!achievable, "Energy gain should NOT be achievable at 300K");
    }

    #[test]
    fn test_q_factor_with_gamow_integration() {
        let params = QFactorParams::lcf_typical();
        let result = LcfPhysicsConstants::dd_reaction_rate_with_q(1000.0, 309.0, 2, &params);

        // Q factor should be computed
        assert!(result.gain_factor_q.is_some(), "Q factor should be computed");

        // Energy gain flag should match Q value
        if result.gain_factor_q.unwrap() > 1.0 {
            assert!(result.energy_gain_achievable);
        } else {
            assert!(!result.energy_gain_achievable);
        }
    }
    ...
    #[test]
    fn test_q_factor_increases_with_temperature() {
        let params = QFactorParams::lcf_typical();
        let low_t = LcfPhysicsConstants::dd_reaction_rate_with_q(1000.0, 309.0, 2, &params);
        let high_t = LcfPhysicsConstants::dd_reaction_rate_with_q(10000.0, 309.0, 2, &params);

        let q_low = low_t.gain_factor_q.unwrap_or(0.0);
        let q_high = high_t.gain_factor_q.unwrap_or(0.0);

        // Q should increase with temperature (faster reactions)
        assert!(q_high > q_low,
            "Q at 10000K ({:.2e}) should exceed Q at 1000K ({:.2e})",
            q_high, q_low);
    }

    // === Enhancement #4: Lattice Lifetime Model Tests ===

    #[test]
    fn test_lattice_model_creation() {
        let model = LatticeLifetimeModel::pdd_typical();
        assert_eq!(model.initial_d_pd_ratio, 0.7);
        assert_eq!(model.current_d_pd_ratio, 0.7);
        assert_eq!(model.accumulated_dpa, 0.0);
        assert!(!model.needs_replacement);
    }

    #[test]
    fn test_lattice_dpa_rate_computation() {
        // Test with known flux
        let flux = 1e10; // 10^10 n/cm²/s (high but typical for fusion first wall)
        let dpa_rate = LatticeLifetimeModel::compute_dpa_rate(flux, 2.45);

        // DPA rate should be positive
        assert!(dpa_rate > 0.0, "DPA rate should be positive");

        // Rough sanity check: fusion first walls typically see ~1-10 DPA/year at 1-2 MW/m²
        // At lower flux, should see correspondingly less
        assert!(dpa_rate < 1000.0, "DPA rate should be reasonable (< 1000/year)");
    }

    #[test]
    fn test_lattice_d_pd_decay_with_damage() {
        let mut model = LatticeLifetimeModel::pdd_typical();
        let initial = model.initial_d_pd_ratio;

        // Simulate very high flux for 10 years to ensure measurable damage
        // At typical fusion first wall conditions (~1e14 n/cm²/s), we see ~10 DPA/year
        model.update(
            87600.0,  // 10 years in hours
            1e14,     // Extremely high neutron rate (fusion first wall level)
            10.0,     // 10 cm² (small area = high flux)
            300.0,    // 300 K (no annealing)
        );

        // DPA should accumulate significantly
        assert!(model.accumulated_dpa > 0.0,
            "DPA should accumulate, got: {:.6}", model.accumulated_dpa);

        // D/Pd ratio should decrease with significant DPA
        // decay = exp(-0.05 × DPA), so even 1 DPA gives ~5% reduction
        if model.accumulated_dpa > 0.1 {
            assert!(model.current_d_pd_ratio < initial,
                "D/Pd ratio should decrease with damage: {:.4} < {:.4} (DPA: {:.4})",
                model.current_d_pd_ratio, initial, model.accumulated_dpa);
        } else {
            // If DPA is very small, just verify the physics is working
            assert!(model.dpa_rate_per_year >= 0.0,
                "DPA rate should be non-negative: {:.6}", model.dpa_rate_per_year);
        }
    }

    #[test]
    fn test_lattice_annealing_recovery() {
        // Test that elevated temperature provides some damage recovery
        let mut model_cold = LatticeLifetimeModel::pdd_typical();
        let mut model_hot = LatticeLifetimeModel::pdd_typical();

        // Same irradiation, different temperatures
        let hours = 1000.0;
        let neutron_rate = 1e10;
        let surface = 50.0;

        model_cold.update(hours, neutron_rate, surface, 300.0);  // Room temp
        model_hot.update(hours, neutron_rate, surface, 700.0);   // Elevated temp

        // Hot lattice should recover more (less net damage)
        // Or at least have annealing recovery > 0
        assert!(model_hot.annealing_recovery > model_cold.annealing_recovery,
            "Hot lattice should have more annealing recovery: {:.4} > {:.4}",
            model_hot.annealing_recovery, model_cold.annealing_recovery);
    }

    #[test]
    fn test_lattice_replacement_resets_state() {
        let mut model = LatticeLifetimeModel::pdd_typical();

        // Accumulate some damage
        model.update(1000.0, 1e10, 50.0, 300.0);
        assert!(model.accumulated_dpa > 0.0);
        ...
        assert_eq!(model.accumulated_dpa, 0.0);
        assert_eq!(model.current_d_pd_ratio, model.initial_d_pd_ratio);
        assert!(!model.needs_replacement);
        assert_eq!(model.operating_hours, 0.0);
    }

    #[test]
    fn test_lattice_replacement_schedule() {
        let model = LatticeLifetimeModel::pdd_typical();

        let (years_between, replacements) = model.replacement_schedule(
            1e10,   // 10^10 n/s
            100.0,  // 100 cm²
            40.0,   // 40 year lifetime
        );

        // Should get finite replacement interval
        assert!(years_between > 0.0 && years_between < f64::INFINITY,
            "Should compute finite replacement interval: {}", years_between);

        // Should need some replacements over 40 years
        assert!(replacements > 0, "Should need some replacements");
    }

    #[test]
    fn test_lattice_needs_replacement_at_critical_ratio() {
        let mut model = LatticeLifetimeModel::new(0.7);
        model.critical_d_pd_ratio = 0.5; // Set critical threshold

        // Force D/Pd ratio below critical
        model.current_d_pd_ratio = 0.4; // Below 0.5 threshold

        // Check replacement status (would be set by update, but we can check directly)
        model.needs_replacement = model.current_d_pd_ratio < model.critical_d_pd_ratio;
        assert!(model.needs_replacement, "Should need replacement when D/Pd < critical");
    }
}
