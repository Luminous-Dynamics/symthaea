...
impl ExperimentScorecard {
    ...
    pub fn new(
        ...
        let null_mean = mean(null_scores);
        let null_std = std_dev(null_scores);
        let effect_size = effect_size(primary_score, null_mean, null_std);
...
pub fn std_dev(values: &[f64]) -> f64 {
    ...
    let m = mean(values);
    let var = values.iter().map(|x| (x - m).powi(2)).sum::<f64>()
        / (values.len() - 1) as f64;
