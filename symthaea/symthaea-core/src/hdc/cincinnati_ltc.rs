...
impl CincinnatiLtcEngine {
    ...
    /// Set budding threshold (default 0.7, lower = more budding)
    pub fn set_budding_threshold(&mut self, threshold: f32) {
        self.budding.theta_bud = threshold;
    }

    /// Set pruning threshold (default 0.1, higher = more pruning)
    pub fn set_pruning_threshold(&mut self, threshold: f32) {
        self.budding.theta_prune = threshold;
    }

    /// Set learning rate (default 0.01)
    pub fn set_learning_rate(&mut self, rate: f32) {
        self.learning_rate = rate;
    }

    /// Set sustain steps for budding (default 10, lower = faster budding)
    pub fn set_sustain_steps(&mut self, steps: usize) {
        self.budding.sustain_steps = steps;
    }

    /// Get the budding system for diagnostics
    pub fn budding_system(&self) -> &PredictiveBudding {
        &self.budding
    }
}
