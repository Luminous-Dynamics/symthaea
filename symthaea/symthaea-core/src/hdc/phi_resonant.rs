...
impl ResonantPhiCalculator {
    ...
    /// Measure integration from resonance dynamics
    ///
    /// **FIXED**: The original implementation measured final energy, but all topologies
    /// converge to the same aligned state, giving Φ=1.0 for everything.
    ///
    /// **New approach**: Measure topology-specific integration through the *structure*
    /// of the similarity matrix itself. This captures how well the topology enables
    /// information integration without relying on convergence dynamics.
    ///
    /// ## Integration Metric
    ///
    /// Uses algebraic connectivity approximation similar to RealPhiCalculator:
    ...
    fn measure_integration(&self, _state: &[ContinuousHV], similarity_matrix: &[Vec<f64>]) -> f64 {
        ...
        // Build degree and Laplacian from similarity matrix
        // Degree: d_i = Σⱼ similarity(i,j) for j ≠ i
        ...
        // Check for disconnected graph (any node with zero degree)
        if degrees.iter().any(|&d| d < 1e-10) {
            return 0.0;
        }

        // Compute normalized Laplacian eigenvalue via power iteration
        // L_norm = D^(-1/2) L D^(-1/2) = I - D^(-1/2) A D^(-1/2)
        // For spectral gap, we find λ₂ via deflated power iteration

        // Step 1: Build normalized adjacency matrix
        ...
        for i in 0..n {
            for j in 0..n {
                if i != j {
                    ...
                    if d_i_sqrt > 1e-10 && d_j_sqrt > 1e-10 {
                        adj_norm[i][j] = similarity_matrix[i][j] / (d_i_sqrt * d_j_sqrt);
                    }
                }
            }
        }

        // Step 2: Find second largest eigenvalue of adj_norm via deflated power iteration
        // (First eigenvalue is 1.0 with eigenvector proportional to sqrt(degrees))
        ...
        for _ in 0..max_iter {
            // Matrix-vector multiply: w = A_norm * v
            let mut w = vec![0.0; n];
            for i in 0..n {
                for j in 0..n {
                    w[i] += adj_norm[i][j] * v[j];
                }
            }

            // Deflate: remove component along v1
            let dot_v1_w: f64 = w.iter().zip(&v1).map(|(a, b)| a * b).sum();
            for i in 0..n {
                w[i] -= dot_v1_w * v1[i];
            }

            // Compute eigenvalue estimate
            let dot_vw: f64 = v.iter().zip(&w).map(|(a, b)| a * b).sum();
            lambda2 = dot_vw;
        ...
        // Convert adjacency eigenvalue to Laplacian eigenvalue (spectral gap)
        // λ₂(L_norm) = 1 - λ₂(A_norm)
        let spectral_gap = 1.0 - lambda2;
