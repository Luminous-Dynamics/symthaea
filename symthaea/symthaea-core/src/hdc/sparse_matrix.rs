...
impl CsrMatrix {
    pub fn from_triplets(nrows: usize, ncols: usize, triplets: &[(usize, usize, f64)]) -> Self {
        ...
        for &(r, _, _) in triplets { row_counts[r] += 1; }
        let mut row_ptr = vec![0usize; nrows + 1];
        for i in 0..nrows { row_ptr[i + 1] = row_ptr[i] + row_counts[i]; }
        ...
        for i in 0..nrows {
            ...
            for j in 1..sv.len() {
                let mut k = j;
                while k > 0 && sc[k - 1] > sc[k] { sc.swap(k - 1, k); sv.swap(k - 1, k); k -= 1; }
            }
        }
        CsrMatrix { nrows, ncols, values, col_indices, row_ptr }
    }
    ...
    pub fn tridiagonal(n: usize, sub: f64, diag: f64, sup: f64) -> Self {
        ...
        for i in 0..n {
            t.push((i, i, diag));
            if i > 0 { t.push((i, i - 1, sub)); }
            if i + 1 < n { t.push((i, i + 1, sup)); }
        }
    ...
    pub fn get(&self, row: usize, col: usize) -> f64 {
        for k in self.row_ptr[row]..self.row_ptr[row + 1] {
            if self.col_indices[k] == col { return self.values[k]; }
            if self.col_indices[k] > col { break; }
        }
        0.0
    }
    ...
    pub fn cg_solve(&self, b: &[f64], tol: f64, max_iter: usize) -> (Vec<f64>, usize, f64) {
        ...
        if rs_old.sqrt() < tol { return (x, 0, rs_old.sqrt()); }
        for iter in 0..max_iter {
            ...
            if rs_new.sqrt() < tol { return (x, iter + 1, rs_new.sqrt()); }
    ...
    pub fn transpose(&self) -> CsrMatrix {
        let t: Vec<_> = (0..self.nrows).flat_map(|i| {
            (self.row_ptr[i]..self.row_ptr[i + 1]).map(move |k| (self.col_indices[k], i, self.values[k]))
        }).collect();
