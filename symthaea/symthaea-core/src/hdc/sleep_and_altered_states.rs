...
// ============================================================================
// DREAM CONTENT GENERATION (Revolutionary Enhancement)
// ============================================================================
//
// Dreams = brain's attempt to make sense of random activation during REM
...
//
// Key features:
// - Bizarre binding: concepts loosely associated ("flying cats")
// - Weak attention: no top-down filtering
// - Memory replay: episodic memories recombined
// - Narrative construction: attempt to make sense of fragments
// ============================================================================
...
pub struct DreamFragment {
    ...
    /// Bizarreness level (0.0 = realistic, 1.0 = highly bizarre)
    pub bizarreness: f64,

    /// Source type (memory replay, random activation, or narrative fill)
    pub source: DreamFragmentSource,
}

/// Source of dream fragment
#[derive(Debug, Clone, Copy, PartialEq, Serialize, Deserialize)]
pub enum DreamFragmentSource {
    /// Replay of episodic memory
    MemoryReplay,
    /// Random activation during REM
    RandomActivation,
    /// Narrative fill (brain trying to make sense)
    NarrativeFill,
    /// Bizarre binding (strange associations)
    BizarreBinding,
}

/// A complete dream scenario
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct DreamScenario {
    /// Dream fragments in narrative order
    pub fragments: Vec<DreamFragment>,

    /// Overall emotional tone
    pub emotional_valence: f64,

    /// Narrative coherence (0.0 = fragmented, 1.0 = coherent story)
    pub narrative_coherence: f64,

    /// Lucidity level (0.0 = no awareness, 1.0 = fully lucid)
    pub lucidity: f64,

    /// Dream themes detected
    pub themes: Vec<String>,

    /// Duration in subjective dream-time (minutes)
    pub subjective_duration: f64,
}

impl DreamScenario {
    /// Create empty dream scenario
    pub fn new() -> Self {
        Self {
            fragments: Vec::new(),
    ...
    /// Calculate overall bizarreness
    pub fn overall_bizarreness(&self) -> f64 {
        if self.fragments.is_empty() {
            return 0.0;
        }
        self.fragments.iter().map(|f| f.bizarreness).sum::<f64>() / self.fragments.len() as f64
    }

    /// Classify the dream type
    pub fn dream_type(&self) -> DreamType {
        let bizarreness = self.overall_bizarreness();

        if self.lucidity > 0.7 {
            DreamType::Lucid
        } else if self.emotional_valence < -0.5 && bizarreness > 0.5 {
            DreamType::Nightmare
        } else if self.narrative_coherence > 0.7 {
            DreamType::NarrativeEpic
        } else if bizarreness > 0.7 {
            DreamType::Surreal
...
impl Default for DreamScenario {
    fn default() -> Self {
        Self::new()
    }
}
...
#[derive(Debug, Clone, Copy, PartialEq, Serialize, Deserialize)]
pub enum DreamType {
    /// Lucid dream (aware of dreaming)
    Lucid,
    /// Nightmare (negative, bizarre)
    Nightmare,
    /// Narrative epic (coherent story)
    NarrativeEpic,
    /// Surreal (highly bizarre)
    Surreal,
    /// Pleasant dream
    Pleasant,
    /// Mundane dream (everyday scenarios)
    Mundane,
}
...
pub struct DreamGenerator {
    ...
    /// Current binding strength (affects bizarreness)
...
impl DreamGenerator {
    ...
    /// Set emotional bias for dream content
    pub fn set_emotional_bias(&mut self, valence: f64) {
        self.emotional_bias = valence.clamp(-1.0, 1.0);
    }

    /// Set binding strength (from ComponentModulation)
    pub fn set_binding_strength(&mut self, strength: f64) {
        self.binding_strength = strength.clamp(0.0, 1.0);
    }

    /// Generate a dream fragment with random activation
    pub fn generate_fragment(&mut self, fragment_seed: u64) -> DreamFragment {
        // Select random concepts to combine
        let idx1 = (fragment_seed % self.concept_seeds.len() as u64) as usize;
        let idx2 = ((fragment_seed / 7) % self.concept_seeds.len() as u64) as usize;

        let (name1, hv1) = &self.concept_seeds[idx1];
        let (name2, hv2) = &self.concept_seeds[idx2];

        // Bizarre binding: combine concepts loosely
        ...
        // Calculate bizarreness (inverse of binding strength + random factor)
        let base_bizarreness = 1.0 - self.binding_strength;
        let random_factor = ((fragment_seed % 100) as f64 / 100.0) * 0.3;
        let bizarreness = (base_bizarreness + random_factor).clamp(0.0, 1.0);

        // Generate description based on concepts
        let description = self.generate_description(name1, name2, bizarreness, fragment_seed);

        // Valence influenced by emotional bias + random variation
        let random_valence = ((fragment_seed % 200) as f64 / 100.0) - 1.0;
        let valence = (self.emotional_bias * 0.6 + random_valence * 0.4).clamp(-1.0, 1.0);
    ...
    fn generate_description(
        ...
        let templates_normal = [
            ...
            "A moment of {c1} and {c2}",
            "{c1} transforms into {c2}",
        ];

        let templates_bizarre = [
            "A {c1} that is also a {c2}, impossibly",
            "The {c1} speaks in the language of {c2}",
            "{c1} melts into {c2} and back again",
            "Infinite {c1} spiraling through {c2}",
    ...
    /// Generate a complete dream scenario
    pub fn generate_dream(&mut self, duration_minutes: f64, is_lucid: bool) -> DreamScenario {
        let fragment_count = (duration_minutes * 2.0) as usize; // ~2 fragments per minute
        let mut fragments = Vec::with_capacity(fragment_count);

        for i in 0..fragment_count {
            let fragment_seed = self.seed.wrapping_add(i as u64 * 13);
            fragments.push(self.generate_fragment(fragment_seed));
        }

        // Calculate overall emotional valence
        let emotional_valence = if fragments.is_empty() {
            0.0
        } else {
            fragments.iter().map(|f| f.valence).sum::<f64>() / fragments.len() as f64
        };

        // Narrative coherence inversely related to bizarreness
        let avg_bizarreness = if fragments.is_empty() {
            0.5
        } else {
            fragments.iter().map(|f| f.bizarreness).sum::<f64>() / fragments.len() as f64
        };
        let narrative_coherence = 1.0 - avg_bizarreness * 0.7;

        // Extract themes from concepts
        let mut themes = Vec::new();
        for (i, fragment) in fragments.iter().enumerate() {
            if i < 3 {
                themes.push(fragment.description.clone());
            }
        }

        let lucidity = if is_lucid { 0.85 } else { 0.1 };
    ...
    /// Generate a nightmare (negative emotional bias, high bizarreness)
    pub fn generate_nightmare(&mut self, duration_minutes: f64) -> DreamScenario {
        self.set_emotional_bias(-0.7);
        self.set_binding_strength(0.2); // Very bizarre
        ...
        self.set_binding_strength(0.4);

        nightmare
    }

    /// Generate a lucid dream (high lucidity, controllable)
    pub fn generate_lucid_dream(&mut self, duration_minutes: f64) -> DreamScenario {
        self.set_binding_strength(0.6); // Stronger binding in lucid dreams

        let mut lucid = self.generate_dream(duration_minutes, true);
        lucid.themes.insert(0, "'I know I am dreaming'".to_string());
...
// ============================================================================
// Integration with SleepAndAlteredStates
// ============================================================================

impl SleepAndAlteredStates {
    /// Generate dream content if currently in REM sleep
    pub fn generate_dream_content(&self, seed: u64) -> Option<DreamScenario> {
        match self.current_state {
            AlteredState::DreamNonLucid => {
                let mut generator = DreamGenerator::new(seed);
                generator.set_binding_strength(self.modulation.binding_strength);
                Some(generator.generate_dream(self.time_in_state.min(30.0), false))
            }
            AlteredState::DreamLucid => {
                let mut generator = DreamGenerator::new(seed);
                generator.set_binding_strength(self.modulation.binding_strength);
                Some(generator.generate_lucid_dream(self.time_in_state.min(30.0)))
            }
            _ => None,
        }
    }
