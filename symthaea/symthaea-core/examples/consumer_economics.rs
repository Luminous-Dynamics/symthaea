...
use symthaea_core::physics::{
    ...
    EconomicEngine, CapitalCosts, OmCosts, FuelCosts, EnergyComparison,
};
...
fn main() {
    ...
    println!("├────────────────────────────────────────────────────────────────────┤");
    println!("│  Power Output:        {:>8.0} kW                                  │", conditions.power_kw);
    println!("│  System Mass:         {:>8.0} kg                                  │", result.geometry_shielding.total_mass_kg);
    println!("│  Shielding:           {:>8.2} m                                   │", result.geometry_shielding.shielding.thickness_m);
    println!("│  Shell Material:      {}                              │", result.shell_material);
    println!("│  Feasible:            {}                                       │",
             if result.feasible { "YES" } else { "NO " });
    println!("├────────────────────────────────────────────────────────────────────┤");
    println!("│  ECONOMICS                                                         │");
    println!("├────────────────────────────────────────────────────────────────────┤");
    println!("│  Capital Cost:        ${:>9.0}                                │", capital.total());
    println!("│  Cost per kW:         ${:>9.0}/kW                             │", capital.cost_per_kw(conditions.power_kw));
    println!("│  Annual O&M:          ${:>9.0}/year                           │",
             om.annual_cost(conditions.power_kw, econ.capacity_factor));
    println!("│  LCOE:                ${:>9.1}/MWh (${:.3}/kWh)             │",
             baseline_lcoe.lcoe_usd_mwh, baseline_lcoe.lcoe_usd_kwh);
    println!("│  Payback Period:      {:>9.1} years                           │", baseline_lcoe.payback_years);
    println!("└────────────────────────────────────────────────────────────────────┘");
    ...
    println!("├────────────────────────────────────────────────────────────────────┤");
    println!("│  ★ Spark Fusion (5 kW)    {:>8.1}       {:>4.0}%   Yes       0     │",
             baseline_lcoe.lcoe_usd_mwh, econ.capacity_factor * 100.0);
    println!("│    Grid Electricity       {:>8.1}       {:>4.0}%   Yes      400    │",
             grid.lcoe_usd_mwh, grid.capacity_factor * 100.0);
    println!("│    Residential Solar      {:>8.1}       {:>4.0}%   No        40    │",
             solar.lcoe_usd_mwh * 3.0, solar.capacity_factor * 100.0); // Residential premium
    println!("│    Solar + Battery        {:>8.1}       {:>4.0}%   Yes       50    │",
             solar.lcoe_usd_mwh * 3.0 + 150.0, 50.0); // With storage
    println!("└────────────────────────────────────────────────────────────────────┘");

    let premium = baseline_lcoe.lcoe_usd_mwh / grid.lcoe_usd_mwh;
    println!("\n  Current premium over grid: {:.1}× (${:.2}/kWh vs ${:.2}/kWh)",
             premium, baseline_lcoe.lcoe_usd_kwh, grid.lcoe_usd_mwh / 1000.0);
    ...
    println!("├────────────────────────────────────────────────────────────────────┤");
    println!("│  Baseline (Today)      {:>10.0}     {:>8.1}       {:.1}×         │",
             capital.total(), baseline_lcoe.lcoe_usd_mwh,
             baseline_lcoe.lcoe_usd_mwh / grid.lcoe_usd_mwh);
    println!("│  Near-term (3-5 yr)    {:>10.0}     {:>8.1}       {:.1}×         │",
             near_term_capital.total(), near_term_lcoe.lcoe_usd_mwh,
             near_term_lcoe.lcoe_usd_mwh / grid.lcoe_usd_mwh);
    println!("│  Mid-term (5-10 yr)    {:>10.0}     {:>8.1}       {:.1}×         │",
             mid_term_capital.total(), mid_term_lcoe.lcoe_usd_mwh,
             mid_term_lcoe.lcoe_usd_mwh / grid.lcoe_usd_mwh);
    println!("│  Long-term (10+ yr)    {:>10.0}     {:>8.1}       {:.1}×         │",
             long_term_capital.total(), long_term_lcoe.lcoe_usd_mwh,
             long_term_lcoe.lcoe_usd_mwh / grid.lcoe_usd_mwh);
    println!("│  Grid parity target    {:>10}     {:>8.1}       1.0×         │",
             "-", grid.lcoe_usd_mwh);
    println!("└────────────────────────────────────────────────────────────────────┘");
    ...
    let effective_lcoe = (baseline_lcoe.lcoe_usd_mwh * annual_energy_kwh / 1000.0 - total_benefit_value)
        / (annual_energy_kwh / 1000.0);

    println!("\n┌────────────────────────────────────────────────────────────────────┐");
    println!("│  ADJUSTED ECONOMICS (Including Benefits)                          │");
    println!("├────────────────────────────────────────────────────────────────────┤");
    println!("│  Carbon credits:        ${:>6.0}/year                              │", carbon_credit_value);
    println!("│  Outage protection:     ${:>6.0}/year                              │", outage_value);
    println!("│  Inflation hedge:       ${:>6.0}/year                              │", inflation_hedge_value);
    println!("│  ─────────────────────────────────────────────                     │");
    println!("│  Total benefit value:   ${:>6.0}/year                              │", total_benefit_value);
    println!("│  Effective LCOE:        ${:>6.1}/MWh ({:.1}× vs grid)              │",
             effective_lcoe, effective_lcoe / grid.lcoe_usd_mwh);
    println!("└────────────────────────────────────────────────────────────────────┘");
    ...
    println!("│  SUMMARY                                                           │");
    println!("├────────────────────────────────────────────────────────────────────┤");
    println!("│  Current LCOE:          ${:.1}/MWh                              │", baseline_lcoe.lcoe_usd_mwh);
    println!("│  Grid parity target:    ${:.1}/MWh                               │", target_lcoe);
    println!("│  Required reduction:    {:.0}%                                     │", reduction_needed);
    println!("│  With benefits credit:  ${:.1}/MWh ({:.0}% reduction needed)       │",
             effective_lcoe, (1.0 - target_lcoe / effective_lcoe).max(0.0) * 100.0);
    println!("└────────────────────────────────────────────────────────────────────┘");
