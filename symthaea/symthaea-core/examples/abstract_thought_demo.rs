...
use symthaea_core::hdc::conjecture_engine::{
    ConjectureEngine, ConjectureStatus, MathDomain, ObservedSequence,
...
fn main() {
    ...
    for seq in &engine.observations {
        println!("    · {:<25} ({:?}, {} data points)",
            seq.name, seq.domain, seq.data.len());
    }
    ...
    println!("  Generated {} conjectures in {:.2}s", round1_count, round1_time.as_secs_f64());
    ...
    let verified_count = engine.conjectures.iter()
        .filter(|c| matches!(c.status, ConjectureStatus::FormallyVerified { .. }))
        .count();
    let tested_count = engine.conjectures.iter()
    ...
    println!("\n  ── Meta-HDC state ──");
    println!("    Concept vectors encoded: {}", at.meta_hdc.concepts.len());
    println!("    Clusters formed:         {}", at.meta_hdc.clusters.len());

    for (i, cluster) in at.meta_hdc.clusters.iter().enumerate() {
        println!("    Cluster #{}: {} members, {} distinct domains",
            i, cluster.members.len(), cluster.domain_diversity);
        // Show the source sequences in this cluster
        let sources: Vec<String> = cluster.members.iter()
            .filter_map(|&m| {
                let conj_idx = at.meta_hdc.concepts[m].conjecture_idx;
                engine.conjectures.get(conj_idx).map(|c| format!("{:?}:{}", c.domain, c.source))
            })
            .collect();
    ...
    println!("    Candidate subtrees tracked: {}", at.dynamic_grammar.candidates.len());
    println!("    Promoted macro-operators:   {}", at.dynamic_grammar.operators.len());
    ...
    if !at.dynamic_grammar.candidates.is_empty() {
        ...
        for cand in cands.iter().take(5) {
            println!("      · {} (appears in {} conjectures, {} sources, {} verified)",
                cand.canonical, cand.occurrences.len(), cand.sources.len(), cand.verified_count);
        }
    }
    ...
    if !at.dynamic_grammar.operators.is_empty() {
        ...
        for op in &at.dynamic_grammar.operators {
            println!("      · {} (arity {}, from {} conjectures)",
                op.canonical, op.arity, op.source_conjectures.len());
        }
    }

    println!("\n  ── Category Discovery state ──");
    println!("    Domain morphisms found: {}", at.category_discovery.morphisms.len());
    println!("    Functorial structures:  {}", at.category_discovery.functors.len());
    if let Some(cat) = &at.category_discovery.math_category {
        println!("    Category objects:       {} (math domains)", cat.num_objects());
    }

    if !at.category_discovery.functors.is_empty() {
        println!("\n    Discovered functors:");
        for f in &at.category_discovery.functors {
            println!("      · {} (confidence={:.2}, paths={})",
    ...
    println!("  Found {} cross-domain matches (mse_ratio < 3.0)", matches.len());
    for (i, m) in matches.iter().take(10).enumerate() {
        println!("    {}. {} ({:?}) → {} ({:?}): ratio={:.2}",
            i + 1, m.source_seq, m.source_domain,
            m.target_seq, m.target_domain, m.mse_ratio);
    }
    ...
    let cross_domain_clusters = at.meta_hdc.clusters.iter().filter(|c| c.domain_diversity >= 2).count();
    ...
    println!("  Generated {} conjectures in {:.2}s", round2_count, round2_time.as_secs_f64());

    // ── 7. Summary ──
    section("Summary");
    println!("  Round 1: {} conjectures in {:.2}s", round1_count, round1_time.as_secs_f64());
    println!("  Round 2: {} conjectures in {:.2}s", round2_count, round2_time.as_secs_f64());
    println!();
    println!("  Architecture performance on real sequences:");
    println!("    · Conjectures encoded as concept vectors: {}", concept_count);
    println!("    · Cross-domain clusters detected:         {}", cross_domain_clusters);
    println!("    · Recurring subtree candidates:           {}", candidate_count);
    println!("    · Promoted macro-operators:               {}", operator_count);
    println!("    · Functorial structures detected:         {}", functor_count);
    println!();
    ...
    if operator_count == 0 {
        ...
        println!("    ✓ Grammar promotion worked — {} macros available for future runs",
            operator_count);
    }
