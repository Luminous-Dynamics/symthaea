...
fn main() {
    ...
    println!("      LCOE: ${:.1}/MWh (${:.4}/kWh)", lcoe.lcoe_usd_mwh, lcoe.lcoe_usd_kwh);
    println!("      Capital cost: ${:.0}M", capital.total() / 1e6);
    println!("      Cost per kW: ${:.0}/kW", capital.cost_per_kw(conditions.power_kw));
    println!("      Payback vs wholesale: {:.1} years", lcoe.payback_years);
    ...
    println!("│  Mass                   ~2,800 kg          {:.0} tonnes             │",
             result.geometry_shielding.total_mass_kg / 1000.0);
    println!("│  Specific mass          560 kg/kW          {:.2} kg/kW              │",
             result.geometry_shielding.total_mass_kg / conditions.power_kw);
    println!("│  Dose rate              0.001 mSv/hr       {:.4} mSv/hr           │",
             result.geometry_shielding.shielding.final_dose);
    println!("│  LCOE                   $631/MWh           ${:.1}/MWh              │", lcoe.lcoe_usd_mwh);
    println!("│  Feasible               YES                {}                     │",
             if result.feasible { "YES" } else { "NO " });
    println!("└─────────────────────────────────────────────────────────────────────┘");

    let physics_status = if result.feasible { "🟢 PASS" } else { "🔴 FAIL" };
    let econ_status = if lcoe.lcoe_usd_mwh < 100.0 { "🟢 COMPETITIVE" }
        else if lcoe.lcoe_usd_mwh < 200.0 { "🟡 MARGINAL" }
        else { "🔴 UNVIABLE" };
    ...
    println!("├─────────────────────────────────────────────────────────────────────┤");
    println!("│  Physics Feasibility:   {:12}                                │", physics_status);
    println!("│  Economic Status:       {:12}                                │", econ_status);
