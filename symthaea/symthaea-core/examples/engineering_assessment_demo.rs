...
fn main() {
    ...
    println!("      Max temp: {:.0} K", physics_result.thermal_profile.t_max);
    println!("      Dose rate: {:.6} mSv/hr", physics_result.geometry_shielding.shielding.final_dose);
    println!("      Total mass: {:.0} kg", physics_result.geometry_shielding.total_mass_kg);
    println!("      Feasible: {}", if physics_result.feasible { "YES" } else { "NO" });
    ...
    println!("      Relative cost: {:.1}x baseline", mfg_result.total_cost);
    println!("      Lead time: {:.0} weeks", mfg_result.lead_time_weeks);
    println!("      Risk score: {:.0}%", mfg_result.risk_score * 100.0);
    println!("      Assembly steps: {}", mfg_result.assembly_sequence.len());
    ...
    println!("      LCOE: ${:.1}/MWh (${:.3}/kWh)", lcoe.lcoe_usd_mwh, lcoe.lcoe_usd_kwh);
    println!("      Capital cost: ${:.0}", capital.total());
    println!("      Cost per kW: ${:.0}/kW", capital.cost_per_kw(conditions.power_kw));
    ...
    let physics_status = if physics_result.feasible { "🟢 PASS" } else { "🔴 FAIL" };
    let uncertainty_status = if mc_results.feasibility_rate > 0.9 { "🟢 HIGH CONF" }
        else if mc_results.feasibility_rate > 0.7 { "🟡 MODERATE" }
        else { "🔴 LOW CONF" };
    let mfg_status = if mfg_result.risk_score < 0.3 { "🟢 LOW RISK" }
        else if mfg_result.risk_score < 0.5 { "🟡 MED RISK" }
        else { "🔴 HIGH RISK" };
    let econ_status = if lcoe.payback_years < 15.0 { "🟢 VIABLE" }
        else if lcoe.payback_years < 25.0 { "🟡 MARGINAL" }
        else { "🔴 UNVIABLE" };

    println!("┌─────────────────────────────────────────────────────────────────────┐");
    println!("│  CATEGORY              STATUS          KEY METRIC                   │");
    println!("├─────────────────────────────────────────────────────────────────────┤");
    println!("│  Physics Feasibility   {:12}    Max T = {:.0} K                │", physics_status, mc_results.temp_stats.p50);
    println!("│  Design Confidence     {:12}    {:.0}% feasible (MC)           │", uncertainty_status, mc_results.feasibility_rate * 100.0);
    println!("│  Manufacturing         {:12}    {:.0} weeks lead time          │", mfg_status, mfg_result.lead_time_weeks);
    println!("│  Economic Viability    {:12}    {:.1} year payback             │", econ_status, lcoe.payback_years);
    println!("└─────────────────────────────────────────────────────────────────────┘");
    ...
    println!("│  System mass:          {:.0} kg (±{:.0}%)                         │",
             mc_results.mass_stats.p50, mc_results.mass_stats.cv * 100.0);
    println!("│  Dose rate:            {:.4} mSv/hr (95th: {:.4})             │",
             mc_results.dose_stats.p50, mc_results.dose_stats.p95);
    println!("│  Design lifetime:      {:.0} years (95th: {:.0})                   │",
             mc_results.lifetime_stats.p50.min(100.0), mc_results.lifetime_stats.p5.min(100.0));
    println!("│  LCOE:                 ${:.1}/MWh ± ${}                         │",
             lcoe.lcoe_usd_mwh, (sensitivities[0].2 - lcoe.lcoe_usd_mwh).abs() as i64);
    println!("│  Total investment:     ${:.0}                                 │", capital.total());
    println!("└─────────────────────────────────────────────────────────────────────┘");
