...
use symthaea_core::physics::{
    DesignSpaceMapper, DesignEnvelope, FusionReaction,
};
...
fn main() {
    ...
    println!("{:>10} {:>15} {:>12} {:>10}", "Power (kW)", "Mass (kg)", "kg/kW", "Status");
    println!("{}", "─".repeat(50));
    for point in &dd_mass.points {
        let status = if point.feasible { "✓" } else { "✗" };
        println!("{:>10.1} {:>15.0} {:>12.0} {:>10}",
    ...
    println!("{:>10} {:>15} {:>10}", "Power (kW)", "Cost ($/kW)", "Status");
    println!("{}", "─".repeat(40));
    for point in &dd_cost.points {
        let status = if point.feasible { "✓" } else { "✗" };
        println!("{:>10.1} {:>15.0} {:>10}",
                 point.power_kw, point.param_value, status);
    }

    // Generate D-D envelope
    let dd_envelope = DesignEnvelope::from_sweeps(
        FusionReaction::DD,
        &dd_shielding,
        &dd_mass,
        &dd_cost,
    ...
    println!("│  Max tested power:      {:>8.1} kW               │", dd_envelope.max_power_kw);
    println!("│  Shielding range:       {:>5.0} - {:>5.0} cm          │",
             dd_envelope.shielding_range_cm.0, dd_envelope.shielding_range_cm.1);
    println!("│  Mass range:            {:>5.0} - {:>5.0} kg          │",
             dd_envelope.mass_range_kg.0, dd_envelope.mass_range_kg.1);
    println!("│  Cost range:          ${:>6.0} - ${:>6.0}/kW       │",
    ...
    println!("{:>10} {:>15} {:>15} {:>10}", "Power (kW)", "D-T Shield", "D-D Shield", "Ratio");
    println!("{}", "─".repeat(55));

    for i in 0..dt_shielding.points.len().min(dd_shielding.points.len()) {
        let dt_point = &dt_shielding.points[i];
        // Find closest D-D point
        let dd_point = dd_shielding.points.iter()
            .min_by(|a, b| {
                (a.power_kw - dt_point.power_kw).abs()
        ...
        println!("{:>10.0} {:>13.0} cm {:>13.0} cm {:>10.1}x",
    ...
    println!("{:>10} {:>12} {:>12} {:>15}", "Power", "Shielding", "Mass", "Cost/kW");
    println!("{}", "─".repeat(55));

    for &power in &powers {
        let shielding_point = dd_shielding.points.iter()
            .min_by(|a, b| (a.power_kw - power).abs().partial_cmp(&(b.power_kw - power).abs()).unwrap());
        let mass_point = dd_mass.points.iter()
            .min_by(|a, b| (a.power_kw - power).abs().partial_cmp(&(b.power_kw - power).abs()).unwrap());
        let cost_point = dd_cost.points.iter()
            .min_by(|a, b| (a.power_kw - power).abs().partial_cmp(&(b.power_kw - power).abs()).unwrap());

        if let (Some(s), Some(m), Some(c)) = (shielding_point, mass_point, cost_point) {
            println!("{:>8.0} kW {:>10.0} cm {:>10.0} kg {:>12.0} $/kW",
