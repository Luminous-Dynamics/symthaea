...
pub struct CudaRng {
    pub(crate) r#gen: sys::curandGenerator_t,
...
impl CudaRng {
    ...
    pub fn new(seed: u64, device: Arc<CudaDevice>) -> Result<Self, result::CurandError> {
        ...
        let r#gen = result::create_generator()?;
        let mut rng = Self { r#gen, device };
        rng.set_seed(seed)?;
        unsafe { result::set_stream(rng.r#gen, rng.device.stream as *mut _) }?;
        Ok(rng)
    }

    /// Re-seed the RNG.
    pub fn set_seed(&mut self, seed: u64) -> Result<(), result::CurandError> {
        unsafe { result::set_seed(self.r#gen, seed) }
    }

    pub fn set_offset(&mut self, offset: u64) -> Result<(), result::CurandError> {
        unsafe { result::set_offset(self.r#gen, offset) }
    }
    ...
    {
        unsafe { result::UniformFill::fill(self.r#gen, *dst.device_ptr_mut() as *mut T, dst.len()) }
    }
...
impl Drop for CudaRng {
    fn drop(&mut self) {
        let r#gen = std::mem::replace(&mut self.r#gen, std::ptr::null_mut());
        if !r#gen.is_null() {
            unsafe { result::destroy_generator(r#gen) }.unwrap();
        }
    }
}
