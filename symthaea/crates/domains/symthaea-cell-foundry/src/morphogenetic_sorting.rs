...
impl DiscoveryExperimentManager {
    ...
    pub fn run_research_sweep(
        &self,
        experiment_configs: Vec<(usize, usize, f32)>,
        base_run_id: &str,
    ) -> Vec<DiscoveryReport> {
        let mut batch_reports = Vec::new();
        for (idx, (gens, pop, mut_mag)) in experiment_configs.into_iter().enumerate() {
            let run_id = format!("{}_sweep_{}", base_run_id, idx);
            let training = vec![
                vec![3.0, 2.0, 1.0],
            ...
            let report = self.pipeline.run_discovery(
