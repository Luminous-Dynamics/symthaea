...
impl NeutronShielding {
    ...
    pub fn required_shielding(
        ...
        let shell_volume_cm3 =
            (4.0 / 3.0) * std::f64::consts::PI * (r_outer_cm.powi(3) - r_inner_cm.powi(3));
...
mod tests {
    ...
    fn test_shielding_calculation() {
        ...
        assert!(
            result.thickness_cm > 0.0,
            "Should need shielding for high flux"
        );
        assert!(
            result.dose_rate_msv_hr <= 0.0002,
            "Should achieve target dose"
        );
    }
