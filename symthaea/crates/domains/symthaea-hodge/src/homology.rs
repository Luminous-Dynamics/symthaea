...
pub(super) fn pairwise_similarities(window: &[symthaea_core::hdc::ContinuousHV]) -> Vec<f64> {
    let n = window.len();
    let mut sim = vec![0.0f64; n * n];
    for i in 0..n {
        sim[i * n + i] = 1.0;
        for j in (i + 1)..n {
            let s = window[i].similarity(&window[j]) as f64;
            sim[i * n + j] = s;
            sim[j * n + i] = s;
        }
    }
...
pub(super) fn compute_betti(sim: &[f64], n: usize, scale: f64) -> BettiNumbers {
    ...
    let beta_0 = count_components(&adj, n);
    let beta_1 = count_triangles(&adj, n) / 3;
    let beta_2 = count_tetrahedra(&adj, n) / 4;
...
pub(super) fn persistent_features(
    num_scales: usize,
    min_persistence: f64,
    sim: &[f64],
    n: usize,
    ...
    let betti_at_scale: Vec<BettiNumbers> =
        scales.iter().map(|&s| compute_betti(sim, n, s)).collect();

    let mut features = Vec::new();

    // Track β₀ feature births/deaths
    track_dimension_features(
        &scales,
        &betti_at_scale,
