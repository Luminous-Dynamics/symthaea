...
use symthaea_humanoid::{
    CfcCpg, GaitControlProfile, GaitGenome, HdcWorkspace, MachineState, Rng, execute_modular_gait,
};
...
fn run_fundamental_stance_check(act_count: usize, dt: f64) -> Result<(), String> {
    ...
    for tick in 0..100 {
        ...
        if state.root_height < 0.60 || state.root_height > 1.50 {
            return Err(format!(
                "Catastrophic postural drop detected at tick {} (Height: {:.2}m)",
                tick, state.root_height
            ));
        }
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    thread::spawn(move || {
        let mut vision_manifold =
            VisionManifold::new(VisionConfig::default(), cam_width, cam_height);
    ...
    while window.running() {
        let burst_modifier = if consecutive_failures > 3 {
            2.5f32
        } else {
            1.0f32
        };
        ...
        println!(
            "         [Generation {}] Launching Parallel Headless Rollouts...",
            generation
        );
        ...
        for worker_id in 0..POPULATION_SIZE {
            ...
            let mut worker_rng =
                LocalRng::new(master_rng.next_f32().to_bits() as u64 + worker_id as u64);
            ...
            let handle = thread::spawn(move || {
                ...
                let candidate_genome = GaitGenome {
                    walk_v: worker_rng.random_mutation(
                        base_champion.walk_v,
                        0.06 * variance_scale * mutation_scale,
                    ),
                    walk_s: worker_rng.random_mutation(
                        base_champion.walk_s,
                        0.03 * variance_scale * mutation_scale,
                    ),
                    walk_k: worker_rng.random_mutation(
                        base_champion.walk_k,
                        0.03 * variance_scale * mutation_scale,
                    ),
                    climb_v: worker_rng.random_mutation(
                        base_champion.climb_v,
                        0.06 * variance_scale * mutation_scale,
                    ),
                    climb_s: worker_rng.random_mutation(
                        base_champion.climb_s,
                        0.03 * variance_scale * mutation_scale,
                    ),
                    climb_k: worker_rng.random_mutation(
                        base_champion.climb_k,
                        0.03 * variance_scale * mutation_scale,
                    ),
                    vel_gain: worker_rng
                        .random_mutation(
                            base_champion.vel_gain,
                            0.05 * variance_scale * mutation_scale,
                        )
                        .max(0.0)
                        .min(1.5),
                    stride_gain: worker_rng
                        .random_mutation(
                            base_champion.stride_gain,
                            0.02 * variance_scale * mutation_scale,
                        )
                        .max(0.0)
                        .min(0.5),
                };
                ...
                for step in 0..SIMULATION_HORIZON_STEPS {
                    ...
                    if current_x > local_max_x {
                        local_max_x = current_x;
                    }
                    steps_active += 1;

                    if torso_z < 0.55 || torso_z > 1.60 {
                        break;
                    }
                    ...
                    if step % 24 == 0 && !mind_is_quiescent {
                        ...
                        for i in 0..32 {
                            semantic_surprise_variance +=
                                (current_hypervector[i] - thread_previous_hypervector[i]).powi(2);
                        }
                        thread_previous_hypervector = current_hypervector;

                        let modulated_temperature =
                            0.08f32 + (semantic_surprise_variance * 4.5f32).min(0.40f32);
                    ...
                    let profile = match current_action {
                        ...
                        1 => GaitControlProfile {
                            target_velocity: candidate_genome.walk_v,
                            stride_amplitude: candidate_genome.walk_s,
                            knee_clearance: candidate_genome.walk_k,
                            target_lean: 0.18,
                            kp: 42.0,
                            kd: 5.5,
                            scale: 40.0,
                        ...
                        2 => GaitControlProfile {
                            target_velocity: candidate_genome.walk_v * 0.6,
                            stride_amplitude: candidate_genome.walk_s * 1.2,
                            knee_clearance: candidate_genome.walk_k + 0.15,
                            target_lean: 0.14,
                            kp: 42.0,
                            kd: 5.5,
                            scale: 40.0,
                        ...
                        _ => GaitControlProfile {
                            target_velocity: candidate_genome.climb_v,
                            stride_amplitude: candidate_genome.climb_s,
                            knee_clearance: candidate_genome.climb_k,
                            target_lean: 0.24,
                            kp: 42.0,
                            kd: 5.5,
                            scale: 40.0,
                    ...
                    running_tracking_error +=
                        (profile.target_lean - state.joint_angles[0] as f32).powi(2);
        ...
        if let Some(winner) = batch_results.iter().max_by(|a, b| {
            let base_score_a = if a.steps_survived < SIMULATION_HORIZON_STEPS {
                a.distance - 5.0f32
            } else {
                a.distance + 5.0f32
            };
            let base_score_b = if b.steps_survived < SIMULATION_HORIZON_STEPS {
                b.distance - 5.0f32
            } else {
                b.distance + 5.0f32
            };
            ...
            metabolic_fitness_a
                .partial_cmp(&metabolic_fitness_b)
                .unwrap()
        }) {
            let base_score = if winner.steps_survived < SIMULATION_HORIZON_STEPS {
                winner.distance - 5.0f32
            } else {
                winner.distance + 5.0f32
            };
            ...
            if candidate_score > best_score + 0.01f32 {
                ...
                println!(
                    "\n📊 ────────── [GENERATION {} METRIC EVOLUTION] ──────────",
                    generation
                );
                println!(
                    "| Dimension           | Prior Historical Best | Current Gen Champion | Delta Status |"
                );
                println!(
                    "|---------------------|-----------------------|----------------------|--------------|"
                );
                println!(
                    "| Stride Distance     | {:>19.2}m | {:>18.2}m | {:>12} |",
                    diags.best_dist,
                    winner.distance,
                    if winner.distance > diags.best_dist {
                        "🔥 IMPROVED"
                    } else {
                        "● STABLE"
                    }
                ...
                println!(
                    "| Avg Control Effort  | {:>21.3} | {:>20.3} | {:>12} |",
                    diags.best_effort,
                    winner.accumulated_effort / winner.steps_survived as f32,
                    if (winner.accumulated_effort / winner.steps_survived as f32)
                        < diags.best_effort
                    {
                        "🍃 GREEN"
                    } else {
                ...
                println!(
                    "| Posture RMSE Error  | {:>21.3} | {:>20.3} | {:>12} |",
                    diags.best_tracking,
                    winner.tracking_error,
                    if winner.tracking_error < diags.best_tracking {
                        "🎯 SHARPER"
                    } else {
                        "● LOCKED"
                    }
                ...
                println!(
                    "| Survival Horizon    | {:>17} steps | {:>16} steps | {:>12} |",
                    diags.best_survival,
                    winner.steps_survived,
                    if winner.steps_survived > diags.best_survival {
                        "🚀 LONGER"
                    } else {
                        "● MATCHED"
                    }
                ...
                diags.best_effort = diags
                    .best_effort
                    .min(winner.accumulated_effort / winner.steps_survived as f32);
                ...
                println!(
                    "💀 [Generation {}] Landscape exploration active. Searching alternative topologies.",
                    generation
                );
            }
        }
        ...
        while visual_steps < SIMULATION_HORIZON_STEPS && window.running() {
            let current_visual_surprise = if let Ok(guard) = async_surprise.lock() {
                *guard
            } else {
                0.0f32
            };
            let cognitive_crouch = if current_visual_surprise > 15.0 {
                current_visual_surprise * 0.008f32
            } else {
                0.0f32
            };
            ...
            if visual_steps % 24 == 0 && !mind_is_quiescent {
                ...
                for i in 0..32 {
                    semantic_surprise_variance +=
                        (current_hypervector[i] - previous_hypervector[i]).powi(2);
                }
            ...
            let (profile, label) = match current_action {
                ...
                1 => (
                    GaitControlProfile {
                        target_velocity: champion_genome.walk_v,
                        stride_amplitude: champion_genome.walk_s,
                        knee_clearance: champion_genome.walk_k,
                        target_lean: 0.18,
                        kp: 42.0,
                        kd: 5.5,
                        scale: 40.0,
                ...
                2 => (
                    GaitControlProfile {
                        target_velocity: champion_genome.walk_v * 0.6,
                        stride_amplitude: champion_genome.walk_s * 1.2,
                        knee_clearance: champion_genome.walk_k + 0.15,
                        target_lean: 0.14,
                        kp: 42.0,
                        kd: 5.5,
                        scale: 40.0,
                ...
                _ => (
                    GaitControlProfile {
                        target_velocity: champion_genome.climb_v,
                        stride_amplitude: champion_genome.climb_s,
                        knee_clearance: champion_genome.climb_k,
                        target_lean: 0.24,
                        kp: 42.0,
                        kd: 5.5,
                        scale: 40.0,
            ...
            if visual_state == MachineState::Recovering {
                println!(
                    "💥 [Viewer] Posture validation breached. Resetting execution frame instantly."
                );
                break;
            }
            ...
            if visual_steps % 20 == 0 {
                ...
                println!(
                    ...
                    visual_x,
                    label,
                    visual_cmd.control_effort(),
                    visual_cpg.homeostatic_stress
                );
            }
            thread::sleep(Duration::from_millis(25));
        }
