...
impl SafeguardsPredictor {
    pub fn new() -> Self {
        ...
        Self { neuron: HdcLtcUnifiedNeuron::new(config, 0x5F6_10A0) }
    }
...
impl symthaea_core::temporal::TemporalPredictor for SafeguardsPredictor {
    ...
    fn domain(&self) -> &'static str { "safeguards" }
    fn tau_base(&self) -> f32 { 86_400.0 }
    fn default_horizons(&self) -> &'static [f32] { SAFEGUARDS_HORIZONS }
    fn horizon_labels(&self) -> &'static [&'static str] { SAFEGUARDS_HORIZON_LABELS }
}
...
impl SafeguardsFepAgent {
    pub fn new() -> Self {
        Self { reference_state: ContinuousHV::random(HDC_DIMENSION, 0x5F6_BEEF) }
    }
    ...
    pub fn select_action(&self, observed: &ContinuousHV) -> SafeguardsFepAction {
        ...
        if fe > 0.7 { SafeguardsFepAction::EmergencyAlert }
        else if fe > 0.5 { SafeguardsFepAction::EscalateToIAEA }
        else if fe > 0.3 { SafeguardsFepAction::RequestInspection }
        else if fe > 0.1 { SafeguardsFepAction::FlagForReview }
        else { SafeguardsFepAction::ContinueMonitoring }
    }
}
...
mod tests {
    ...
    fn healthy() -> SafeguardsReading {
        SafeguardsReading { inventory_discrepancy: 0.01, sensor_anomaly: 0.02, timeline_consistency: 0.95 }
    }

    #[test]
    fn test_horizons_ordered() {
        for i in 1..SAFEGUARDS_HORIZONS.len() { assert!(SAFEGUARDS_HORIZONS[i] > SAFEGUARDS_HORIZONS[i - 1]); }
    }

    #[test]
    fn test_horizons_labels_match() { assert_eq!(SAFEGUARDS_HORIZONS.len(), SAFEGUARDS_HORIZON_LABELS.len()); }

    #[test]
    fn test_encoder_dimension() {
        assert_eq!(SafeguardsHdcEncoder::new().encode(&healthy()).dim(), HDC_DIMENSION);
    }
