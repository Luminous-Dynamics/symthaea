...
fn word_to_events(word: &str, start: &mut f32) -> Vec<TemporalEvent> {
    let phonemes: Option<Vec<(&str, f32, f32)>> = match word.to_uppercase().as_str() {
        ...
        "DOG" => Some(vec![("D", 0.05, 0.6), ("AO", 0.12, 0.75), ("G", 0.06, 0.55)]),
        ...
        "BIG" => Some(vec![("B", 0.05, 0.6), ("IH", 0.08, 0.75), ("G", 0.06, 0.55)]),
        "PIG" => Some(vec![("P", 0.05, 0.6), ("IH", 0.08, 0.75), ("G", 0.06, 0.55)]),
        "DIG" => Some(vec![("D", 0.05, 0.6), ("IH", 0.08, 0.75), ("G", 0.06, 0.55)]),
        ...
        "STRONG" => Some(vec![("S", 0.08, 0.5), ("T", 0.04, 0.5), ("R", 0.05, 0.5), ("AO", 0.1, 0.7), ("NG", 0.06, 0.5)]),
        "STRING" => Some(vec![("S", 0.08, 0.5), ("T", 0.04, 0.5), ("R", 0.05, 0.5), ("IH", 0.08, 0.7), ("NG", 0.06, 0.5)]),
        "SPRING" => Some(vec![("S", 0.08, 0.5), ("P", 0.04, 0.5), ("R", 0.05, 0.5), ("IH", 0.08, 0.7), ("NG", 0.06, 0.5)]),
        "STREET" => Some(vec![("S", 0.08, 0.5), ("T", 0.04, 0.5), ("R", 0.05, 0.5), ("IY", 0.1, 0.75), ("T", 0.05, 0.5)]),
        _ => None,
    };
...
fn remap_events_baseline(events: &[TemporalEvent], grammar: &TemporalGrammar) -> Vec<TemporalEvent> {
    events.iter()
        .filter_map(|e| {
            grammar.category_id(&e.category).map(|cid| {
                TemporalEvent::new(&e.category, cid, e.start_time, e.duration, e.intensity)
...
fn main() {
    ...
    println!("\n    Unified:  {} clusters, {:.1}% cluster density",
             u_stats.num_clusters, u_stats.avg_cluster_density * 100.0);
    println!("    Baseline: {:.1}% density", b_stats.grammar_density * 100.0);
    ...
    let u_trained = u_scores.iter().find(|(l, _)| l == "trained").map(|(_, s)| *s).unwrap_or(0.0);
    let u_similar = u_scores.iter().find(|(l, _)| l == "similar").map(|(_, s)| *s).unwrap_or(0.0);
    let b_trained = b_scores.iter().find(|(l, _)| l == "trained").map(|(_, s)| *s).unwrap_or(0.0);
    let b_similar = b_scores.iter().find(|(l, _)| l == "similar").map(|(_, s)| *s).unwrap_or(0.0);
