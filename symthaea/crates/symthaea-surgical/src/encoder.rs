    ...
    pub fn new(g: &GenesisSeed, nl: usize) -> Self {
        Self {
            base: (0..NUM_STATE_CHANNELS)
                .map(|i| ContinuousHV::from_genesis(g, &format!("surg::ch::{i}"), DIM))
                .collect(),
            levels: (0..nl)
                .map(|l| ContinuousHV::from_genesis(g, &format!("surg::lv::{l}"), DIM))
                .collect(),
            prev: None,
            nl,
        }
...
mod tests {
    ...
    #[test]
    fn test_dim() {
        let mut e = SurgicalHdcEncoder::new(&GenesisSeed::from_phrase("t"), 32);
        assert_eq!(e.encode(&SurgicalState::home()).dim(), DIM);
    }
}
