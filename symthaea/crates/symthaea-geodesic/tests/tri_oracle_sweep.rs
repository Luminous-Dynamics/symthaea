...
fn test_cases() -> Vec<(&'static str, ProgramNode, ProgramNode)> {
    vec![
        ...
        ("iterate_cond",
            ProgramNode::iterate(
                ProgramNode::atom("i=0"), ProgramNode::atom("i+=1"),
...
fn test_weight_sweep() {
            ...
            for &trw in &trajectory_weights {
                ...
                for &steps in &trajectory_steps {
                    ...
                    if wins > best_accuracy || (wins == best_accuracy && mean_delta > best_delta) {
                        ...
                        println!("{:<8.2} {:<8.2} {:<8.2} {:<6} {:>4}/{} {:>6.0}% {:>+10.4} ← NEW BEST",
                            sw_n, tw_n, trw_n, steps, wins, total,
                            wins as f32 / total as f32 * 100.0, mean_delta);
                    }
                }
            }
        }
    ...
    println!("Accuracy: {}/10 ({:.0}%)", best_accuracy, best_accuracy as f32 / 10.0 * 100.0);
    ...
    for (name, correct, wrong) in &cases {
        ...
        println!("{:<20} {:>8.4} {:>8.4} {:>8.4} {:>8.4} {:>8.4} {:>8.4} {:>6}",
            name,
            c.structural, c.topological, c.trajectory,
            w.structural, w.topological, w.trajectory,
            if c.composite > w.composite { "YES" } else { "NO" });
    }
...
fn test_individual_oracle_accuracy() {
    ...
    let configs = [
        ("Structural only", TriOracleConfig {
            structural_weight: 1.0, topological_weight: 0.0, trajectory_weight: 0.0, trajectory_steps: 1 }),
        ("Topological only", TriOracleConfig {
            structural_weight: 0.0, topological_weight: 1.0, trajectory_weight: 0.0, trajectory_steps: 1 }),
        ("Trajectory only (5 steps)", TriOracleConfig {
            structural_weight: 0.0, topological_weight: 0.0, trajectory_weight: 1.0, trajectory_steps: 5 }),
        ("Trajectory only (20 steps)", TriOracleConfig {
            structural_weight: 0.0, topological_weight: 0.0, trajectory_weight: 1.0, trajectory_steps: 20 }),
        ("Default (0.5/0.2/0.3)", TriOracleConfig::default()),
    ];

    for (name, config) in &configs {
        let (wins, total, delta) = run_with_config(config);
        println!("{:<30} {}/{}  ({:.0}%)  mean_delta={:+.4}",
            name, wins, total, wins as f32 / total as f32 * 100.0, delta);
    }
}
