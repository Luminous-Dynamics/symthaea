...
fn emit_steps(combinator: &SkeletonCombinator, steps: &mut Vec<GeodesicPlanStep>, depth: usize) {
    match combinator {
        ...
        SkeletonCombinator::Branch { condition, then_branch, else_branch } => {
            steps.push(GeodesicPlanStep {
        ...
        SkeletonCombinator::Iterate { init, condition, body } => {
            steps.push(GeodesicPlanStep {
        ...
        SkeletonCombinator::Recurse { base_case, recursive_case } => {
            steps.push(GeodesicPlanStep {
        ...
        SkeletonCombinator::MapOver { transform, collection } => {
            steps.push(GeodesicPlanStep {
        ...
        SkeletonCombinator::FilterBy { predicate, collection } => {
            steps.push(GeodesicPlanStep {
                action: GeodesicPlanAction::AddIteratorChain,
                name: None,
                context: vec![
        ...
        SkeletonCombinator::Reduce { operation, initial, collection } => {
            steps.push(GeodesicPlanStep {
                action: GeodesicPlanAction::AddIteratorChain,
                name: None,
                context: vec![
        ...
        SkeletonCombinator::Leaf(slot) => {
            steps.push(GeodesicPlanStep {
                ...
                name: None,
                context: vec![
                    format!("expression:{}", slot_description(slot)),
                ],
                confidence: 0.8,
            });
        }
...
pub fn skeleton_to_code_spec(
    ...
    let purpose = if hints.is_empty() {
        format!("Generated by Geodesic Code Synthesis (β₁={})", target_betti.beta_1)
    } else {
        hints.join("; ")
    };
    ...
    if has_combinator(skeleton, |c| matches!(c, SkeletonCombinator::Recurse { .. })) {
        constraints.push("RECURSIVE".to_string());
    }
    if has_combinator(skeleton, |c| matches!(c, SkeletonCombinator::MapOver { .. })) {
        constraints.push("ITERATOR_MAP".to_string());
    }
    if has_combinator(skeleton, |c| matches!(c, SkeletonCombinator::FilterBy { .. })) {
    ...
    if hints.iter().any(|h| h.contains("error") || h.contains("Result") || h.contains("Option")) {
...
fn has_combinator(c: &SkeletonCombinator, pred: fn(&SkeletonCombinator) -> bool) -> bool {
    ...
    match c {
        ...
        SkeletonCombinator::Branch { then_branch, else_branch, .. } => {
...
fn emit_body(combinator: &SkeletonCombinator, indent: usize) -> Option<String> {
    ...
    match combinator {
        ...
        SkeletonCombinator::Branch { condition, then_branch, else_branch } => {
            let cond = condition.filled.as_deref().unwrap_or("todo!(\"condition\")");
...
mod tests {
    ...
    fn test_skeleton_to_plan_steps_linear() {
        let betti = BettiNumbers { beta_0: 1, beta_1: 0, beta_2: 0 };
        ...
        assert_eq!(steps.first().unwrap().action, GeodesicPlanAction::DefineFunction);
        assert_eq!(steps.last().unwrap().action, GeodesicPlanAction::Complete);

        // No loops or branches
        assert!(!steps.iter().any(|s| s.action == GeodesicPlanAction::AddLoop));
    }
    ...
    fn test_skeleton_to_plan_steps_loop() {
        ...
        let loop_count = steps.iter().filter(|s| s.action == GeodesicPlanAction::AddLoop).count();
    ...
    fn test_skeleton_to_plan_steps_recursive() {
        ...
        let recursion_count = steps.iter().filter(|s| s.action == GeodesicPlanAction::AddRecursion).count();
    ...
    fn test_skeleton_to_code_spec() {
        ...
        assert_eq!(spec.signature, Some("fn bubble_sort(arr: &mut [i32])".to_string()));
    }
    ...
    fn test_emit_rust_linear() {
        ...
        fn fill_leaves(c: &mut SkeletonCombinator, fills: &mut Vec<&str>) {
            match c {
                SkeletonCombinator::Sequence(steps) => {
                    for s in steps.iter_mut() { fill_leaves(s, fills); }
                }
                SkeletonCombinator::Leaf(slot) if !slot.is_filled() => {
                    if let Some(f) = fills.pop() { slot.fill(f); }
                }
                _ => {}
            }
        }
        ...
        let code = emit_rust_from_skeleton(&skeleton, "add", Some("fn add(a: i32, b: i32) -> i32"), &["add two numbers"]);
        assert!(code.is_some());
    ...
    fn test_emit_rust_with_loop() {
        ...
        fn fill_all(c: &mut SkeletonCombinator, fills: &mut Vec<&str>) {
            match c {
                SkeletonCombinator::Sequence(steps) => {
                    for s in steps.iter_mut() { fill_all(s, fills); }
                }
                SkeletonCombinator::Iterate { init, condition, body } => {
                    if !init.is_filled() { if let Some(f) = fills.pop() { init.fill(f); } }
                    if !condition.is_filled() { if let Some(f) = fills.pop() { condition.fill(f); } }
                    fill_all(body, fills);
                }
                SkeletonCombinator::Leaf(slot) if !slot.is_filled() => {
                    if let Some(f) = fills.pop() { slot.fill(f); }
        ...
        let mut fills = vec!["sum", "sum += arr[i]; i += 1;", "i < arr.len()", "let mut i = 0; let mut sum = 0;", "// sum function"];
        fills.reverse();
    ...
    fn test_emit_rust_filter() {
        ...
        fn fill_all(c: &mut SkeletonCombinator, fills: &mut Vec<&str>) {
            match c {
                ...
                SkeletonCombinator::FilterBy { predicate, collection } => {
                    if !predicate.is_filled() { if let Some(f) = fills.pop() { predicate.fill(f); } }
                    if !collection.is_filled() { if let Some(f) = fills.pop() { collection.fill(f); } }
                }
                SkeletonCombinator::Leaf(slot) if !slot.is_filled() => {
                    if let Some(f) = fills.pop() { slot.fill(f); }
                }
