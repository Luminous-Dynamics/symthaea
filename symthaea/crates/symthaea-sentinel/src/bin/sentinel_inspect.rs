...
use symthaea_sentinel::{
    AudioSentinel, AudioFeatures, AudioCategory, MEL_BANDS, FREQ_BINS, CONTROL_RATE,
    ...
    compute_ioi_variance, FileAudioPump, FileAudioConfig,
};
...
fn main() {
    ...
    for (name, pattern) in &sentinel.patterns {
        ...
        let (rhythm_max_idx, _rhythm_max) = pattern.rhythm_freq_signature.iter()
            .enumerate()
            .max_by(|(_, a), (_, b)| a.partial_cmp(b).unwrap())
            .unwrap();
        
        let (timbre_max_idx, _timbre_max) = pattern.timbre_freq_signature.iter()
            .enumerate()
            .max_by(|(_, a), (_, b)| a.partial_cmp(b).unwrap())
            .unwrap();
        ...
        println!("]");
        println!("           Peak at {:.2} Hz ({:.0} BPM equivalent)", 
            FREQ_BINS[rhythm_max_idx], FREQ_BINS[rhythm_max_idx] * 60.0);
        ...
        println!("  IOI Variance:  {:.4}  (0=regular, 1=irregular)", pattern.mean_burst_density);
