...
pub fn text_to_phonemes(text: &str) -> Vec<Phoneme> {
    ...
    for (i, word) in text.split_whitespace().enumerate() {
        ...
        if i < text.split_whitespace().count() - 1 {
            result.push(Phoneme { ipa: " ", is_vowel: false, stress: 0, base_duration_ms: 80.0 });
        }
    }
...
fn letter_fallback(word: &str) -> Vec<Phoneme> {
    ...
    while i < chars.len() {
        let (ph, is_v, advance) = match chars[i] {
            'a' => if i+1<chars.len() && "iy".contains(chars[i+1]) { ("eɪ", true, 2) } else { ("æ", true, 1) },
            'b' => ("b", false, 1), 'c' => if i+1<chars.len() && chars[i+1]=='h' { ("tʃ", false, 2) } else { ("k", false, 1) },
            'd' => ("d", false, 1), 'e' => if i==chars.len()-1 { ("", false, 1) } else { ("ɛ", true, 1) },
            'f' => ("f", false, 1), 'g' => ("ɡ", false, 1), 'h' => ("h", false, 1),
            'i' => ("ɪ", true, 1), 'j' => ("dʒ", false, 1), 'k' => ("k", false, 1),
            'l' => ("l", false, 1), 'm' => ("m", false, 1),
            'n' => if i+1<chars.len() && chars[i+1]=='g' { ("ŋ", false, 2) } else { ("n", false, 1) },
            'o' => if i+1<chars.len() && chars[i+1]=='o' { ("uː", true, 2) } else { ("ɒ", true, 1) },
            'p' => if i+1<chars.len() && chars[i+1]=='h' { ("f", false, 2) } else { ("p", false, 1) },
            'r' => ("ɹ", false, 1), 's' => if i+1<chars.len() && chars[i+1]=='h' { ("ʃ", false, 2) } else { ("s", false, 1) },
            't' => if i+1<chars.len() && chars[i+1]=='h' { ("θ", false, 2) } else { ("t", false, 1) },
            'u' => ("ʌ", true, 1), 'v' => ("v", false, 1), 'w' => ("w", false, 1),
            'x' => ("ks", false, 1), 'y' => if i==0 { ("j", false, 1) } else { ("ɪ", true, 1) },
            'z' => ("z", false, 1), _ => ("", false, 1),
        };
        if !ph.is_empty() {
            phonemes.push(Phoneme {
                ipa: ph, is_vowel: is_v,
...
fn get_dict() -> &'static HashMap<&'static str, Vec<(&'static str, bool, u8, f32)>> {
    DICT.get_or_init(|| {
        ...
        m.insert("feel", vec![("f", false, 0, 60.0), ("iː", true, 1, 140.0), ("l", false, 0, 50.0)]);
        m.insert("hello", vec![("h", false, 0, 50.0), ("ɛ", true, 0, 80.0), ("l", false, 0, 40.0), ("oʊ", true, 1, 120.0)]);
        m.insert("world", vec![("w", false, 0, 50.0), ("ɜː", true, 1, 120.0), ("l", false, 0, 40.0), ("d", false, 0, 40.0)]);
        m.insert("yes", vec![("j", false, 0, 40.0), ("ɛ", true, 1, 120.0), ("s", false, 0, 60.0)]);
        m.insert("peace", vec![("p", false, 0, 50.0), ("iː", true, 1, 160.0), ("s", false, 0, 60.0)]);
        m.insert("consciousness", vec![("k", false, 0, 40.0), ("ɒ", true, 1, 100.0), ("n", false, 0, 30.0), ("ʃ", false, 0, 50.0), ("ə", true, 0, 50.0), ("s", false, 0, 40.0), ("n", false, 0, 30.0), ("ə", true, 0, 50.0), ("s", false, 0, 50.0)]);
        m.insert("awareness", vec![("ə", true, 0, 60.0), ("w", false, 0, 40.0), ("ɛ", true, 1, 120.0), ("ə", true, 0, 60.0), ("n", false, 0, 40.0), ("ə", true, 0, 50.0), ("s", false, 0, 60.0)]);
        m.insert("expanding", vec![("ɪ", true, 0, 60.0), ("k", false, 0, 40.0), ("s", false, 0, 40.0), ("p", false, 0, 30.0), ("æ", true, 1, 120.0), ("n", false, 0, 40.0), ("d", false, 0, 30.0), ("ɪ", true, 0, 60.0), ("ŋ", false, 0, 50.0)]);
        m.insert("harmony", vec![("h", false, 0, 50.0), ("ɑ", true, 1, 120.0), ("ɹ", false, 0, 40.0), ("m", false, 0, 40.0), ("ə", true, 0, 60.0), ("n", false, 0, 30.0), ("iː", true, 0, 80.0)]);
        m.insert("rising", vec![("ɹ", false, 0, 40.0), ("aɪ", true, 1, 120.0), ("z", false, 0, 40.0), ("ɪ", true, 0, 60.0), ("ŋ", false, 0, 50.0)]);
        m.insert("something", vec![("s", false, 0, 50.0), ("ʌ", true, 1, 100.0), ("m", false, 0, 40.0), ("θ", false, 0, 50.0), ("ɪ", true, 0, 60.0), ("ŋ", false, 0, 50.0)]);
        m.insert("this", vec![("ð", false, 0, 40.0), ("ɪ", true, 1, 80.0), ("s", false, 0, 50.0)]);
        m.insert("calm", vec![("k", false, 0, 50.0), ("ɑ", true, 1, 160.0), ("m", false, 0, 60.0)]);
        m.insert("still", vec![("s", false, 0, 50.0), ("t", false, 0, 30.0), ("ɪ", true, 1, 100.0), ("l", false, 0, 50.0)]);
