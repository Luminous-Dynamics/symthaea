...
mod tests {
    ...
    fn produces_frames() {
        ...
        assert!(frames.len() > 10, "should produce many frames: {}", frames.len());
    }

    #[test]
    fn silence_has_zero_energy() {
        let ph = vec![crate::g2p::Phoneme { ipa: " ", is_vowel: false, stress: 0, base_duration_ms: 100.0 }];
        let frames = phonemes_to_frames(&ph, &VoiceProsody::default(), 44100);
        assert!(frames.iter().all(|f| f.energy < 0.01), "silence should have zero energy");
    }

    #[test]
    fn arousal_affects_frame_count() {
        let ph = g2p::text_to_phonemes("hello");
        let calm = phonemes_to_frames(&ph, &VoiceProsody { arousal: 0.1, ..Default::default() }, 44100);
        let excited = phonemes_to_frames(&ph, &VoiceProsody { arousal: 0.9, ..Default::default() }, 44100);
        assert!(excited.len() < calm.len(), "excited should be faster: {} vs {}", excited.len(), calm.len());
    }
}
