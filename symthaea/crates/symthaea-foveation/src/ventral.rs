...
//! When real perception models are available (future), routes through
//! SigLIP, OCR, and/or Moondream VQA.
...
impl VentralPipeline {
    ...
    #[cfg(feature = "perception")]
    fn try_new_real(_routing: RoutingStrategy) -> Option<Self> {
        match real::RealVentralPipeline::new() {
            Ok(_real_pipeline) => {
                tracing::info!("Ventral pipeline: using real SigLIP backend");
                // TODO: Store real pipeline and dispatch to it in process().
                // For now, real backend validates but we still use stub processing
                // until the VentralPipeline enum refactor (Phase 4).
                None
            }
            Err(e) => {
    ...
    /// derived from the crop content. The resulting 16,384D ContinuousHV
    /// is valid for GWT injection.
    pub fn process(&self, request: &FoveationRequest) -> FoveationResult {
        let start = std::time::Instant::now();

        let (semantic_hv, content, confidence) = match self.routing {
            RoutingStrategy::AlwaysOcr => self.stub_ocr(request),
            RoutingStrategy::AlwaysCaption => self.stub_caption(request),
            RoutingStrategy::AlwaysEmbed => self.stub_embed(request),
            RoutingStrategy::Full => self.stub_full(request),
            RoutingStrategy::Auto => self.stub_auto(request),
        };
    ...
    fn stub_auto(&self, request: &FoveationRequest) -> (ContinuousHV, RecognizedContent, f32) {
        // Simple heuristic: high contrast → "text", low contrast → "object"
        let contrast = self.pixel_contrast(&request.crop_pixels);
        if contrast > 100.0 {
            self.stub_ocr(request)
        } else {
            self.stub_embed(request)
        }
    }

    /// Stub OCR: returns text content based on pixel hash.
    fn stub_ocr(&self, request: &FoveationRequest) -> (ContinuousHV, RecognizedContent, f32) {
        let hash = self.pixel_hash(&request.crop_pixels);
        let hv = self.jl_projector.project_hash(hash);
    ...
    fn stub_embed(&self, request: &FoveationRequest) -> (ContinuousHV, RecognizedContent, f32) {
        let hash = self.pixel_hash(&request.crop_pixels);
        let hv = self.jl_projector.project_hash(hash);
        let content = RecognizedContent::Object {
            label: format!("stub_object_{:04x}", hash & 0xFFFF),
            embedding: vec![0.0; 8], // Minimal placeholder
        };
        (hv, content, 0.4)
    }

    /// Stub caption: returns VQA caption based on pixel hash.
    fn stub_caption(&self, request: &FoveationRequest) -> (ContinuousHV, RecognizedContent, f32) {
        let hash = self.pixel_hash(&request.crop_pixels);
        let hv = self.jl_projector.project_hash(hash);
    ...
    fn stub_full(&self, request: &FoveationRequest) -> (ContinuousHV, RecognizedContent, f32) {
        // In full mode, pick the most informative result.
        // For the stub, just use auto.
        self.stub_auto(request)
    }

    /// Simple pixel contrast measure (standard deviation proxy).
    fn pixel_contrast(&self, pixels: &[u8]) -> f32 {
        if pixels.is_empty() {
            return 0.0;
        }
    ...
    fn pixel_hash(&self, pixels: &[u8]) -> u64 {
        let mut hash: u64 = 5381;
        // Sample every 16th pixel for speed (crops can be large)
        let step = 16.max(1);
...
impl StubJLProjector {
    ...
    pub(crate) fn project_embedding(&self, embedding: &[f32]) -> ContinuousHV {
        ...
        for i in 0..self.dim {
            ...
            for (j, &e) in embedding.iter().enumerate() {
                // xorshift64 per (row, col) pair
                state ^= (j as u64).wrapping_mul(0x517CC1B727220A95);
                state ^= state << 13;
                state ^= state >> 7;
                state ^= state << 17;
                // Rademacher: sign = +1 or -1
    ...
    ///
    /// Uses a simple xorshift-like PRNG seeded by the hash to generate
    /// a pseudo-random vector, then normalizes it.
...
mod real {
    ...
    impl RealVentralPipeline {
                ...
                if let Some(image) = Self::pixels_to_image(request) {
                    match self.vision.embed_image(&image) {
                        Ok(embedding) => {
                            ...
                            // JL projection: 768D → 16,384D via seeded Rademacher matrix
        ...
        /// Convert raw crop pixels to a DynamicImage (grayscale).
        ...
        /// Fallback when image conversion or model fails.
...
mod tests {
    ...
    fn test_ventral_pipeline_auto_returns_valid_hv() {
        let pipeline = VentralPipeline::new(RoutingStrategy::Auto);
    ...
    fn test_ventral_pipeline_ocr_route() {
        let pipeline = VentralPipeline::new(RoutingStrategy::AlwaysOcr);
    ...
    fn test_ventral_pipeline_embed_route() {
        let pipeline = VentralPipeline::new(RoutingStrategy::AlwaysEmbed);
    ...
    fn test_ventral_pipeline_caption_route() {
        let pipeline = VentralPipeline::new(RoutingStrategy::AlwaysCaption);
    ...
    fn test_ventral_pipeline_full_route() {
        let pipeline = VentralPipeline::new(RoutingStrategy::Full);
    ...
    fn test_deterministic_output() {
        ...
        // Same pixels → same semantic HV (deterministic hash)
    ...
    fn test_processing_time_recorded() {
        ...
        // Processing time should be non-negative (could be 0 for very fast stub)
    ...
    fn test_auto_routes_high_contrast_to_text() {
        let pipeline = VentralPipeline::new(RoutingStrategy::Auto);
        // High contrast: alternating 0 and 255
        let high_contrast: Vec<u8> = (0..64).map(|i| if i % 2 == 0 { 0 } else { 255 }).collect();
        let req = make_request(40, high_contrast);
        let result = pipeline.process(&req);

        // High contrast → should route to OCR (text)
        match &result.content {
            RecognizedContent::Text(_) => {} // Expected
    ...
    fn test_auto_routes_low_contrast_to_embed() {
        let pipeline = VentralPipeline::new(RoutingStrategy::Auto);
        // Low contrast: all same value
        let low_contrast = vec![128u8; 64];
        let req = make_request(41, low_contrast);
        let result = pipeline.process(&req);

        // Low contrast → should route to embed (object)
        match &result.content {
            RecognizedContent::Object { .. } => {} // Expected
    ...
    fn test_pixel_contrast_zero_for_uniform() {
        let pipeline = VentralPipeline::new(RoutingStrategy::Auto);
        let c = pipeline.pixel_contrast(&vec![128; 100]);
    ...
    fn test_pixel_contrast_high_for_alternating() {
        ...
        let c = pipeline.pixel_contrast(&pixels);
