...
fn bench_binius_hdc_xor() -> BenchResult {
    ...
    println!(
        "  AND constraints: {} (should be 0 for pure XOR)",
        and_constraints
    );
    println!(
        "  MUL constraints: {} (should be 0 for pure XOR)",
        mul_constraints
    );
    println!(
        "  Total non-linear constraints: {}",
...
fn bench_winterfell_hdc_xor() -> BenchResult {
    ...
    println!(
        "  Estimated constraints: {} (bit decompose + XOR + recompose)",
        constraint_count
    );
...
fn main() {
    ...
    println!(
        "║ Constraints          │ {:>14} │ {:>18} ║",
        binius_result.constraint_count, winterfell_result.constraint_count
    );
    println!(
        "║ Prover time (ms)     │ {:>14.1} │ {:>18.0} ║",
        binius_result.prove_time_ms, winterfell_result.prove_time_ms
    );
    println!(
        "║ Verifier time (ms)   │ {:>14.3} │ {:>18.1} ║",
        binius_result.verify_time_ms, winterfell_result.verify_time_ms
    );
    println!(
        "║ Proof size (KB)      │ {:>14.1} │ {:>18.1} ║",
    ...
    if winterfell_result.constraint_count > 0 {
        let constraint_ratio = winterfell_result.constraint_count as f64
            / binius_result.constraint_count.max(1) as f64;
        let prove_ratio = winterfell_result.prove_time_ms / binius_result.prove_time_ms.max(0.001);

        println!();
        println!(
            "Key finding: Binius uses {}x fewer constraints for HDC XOR",
            if binius_result.constraint_count == 0 {
                "∞".to_string()
            } else {
                format!("{:.0}", constraint_ratio)
            }
        );
        println!(
    ...
    println!("╠══════════════════════════════════════════════════════════════╣");
    println!(
        "║ 3 vecs × 16Kbit  │ {:>10} │ {:>10.1} │ {:>10.1}  ║",
        bundle_3x256.and_constraints,
        bundle_3x256.prove_time_ms,
        bundle_3x256.proof_size_bytes as f64 / 1024.0
    );
    println!(
        "║ 8 vecs × 16Kbit  │ {:>10} │ {:>10.1} │ {:>10.1}  ║",
        bundle_8x256.and_constraints,
        bundle_8x256.prove_time_ms,
        bundle_8x256.proof_size_bytes as f64 / 1024.0
    );
    println!("╚══════════════════════════════════════════════════════════════╝");
    ...
    println!(
        ...
        hamming.and_constraints,
        hamming.dim_words * 64
    );
    ...
    println!(
        "║  8N × 10T        │ {:>11} │ {:>10.1} │ {:>10.1} ║",
        cfc_small.and_constraints,
        cfc_small.prove_time_ms,
        cfc_small.proof_size_bytes as f64 / 1024.0
    );
    println!(
        "║ 64N × 100T       │ {:>11} │ {:>10.1} │ {:>10.1} ║",
        cfc_medium.and_constraints,
        cfc_medium.prove_time_ms,
        cfc_medium.proof_size_bytes as f64 / 1024.0
    );
    println!("╚══════════════════════════════════════════════════════════════╝");
    ...
    println!(
        "    {} AND constraints (vs ~7,360 without sigmoid)",
        sig_result.and_constraints
    );
    ...
    println!("╠══════════════════════════════════════════════════════════════╣");
    println!(
        "║  3 × 4Kbit    │ {:>10} │ {:>10.1} │ {:>10.1}       ║",
        fl_3.and_constraints,
        fl_3.prove_time_ms,
        fl_3.proof_size_bytes as f64 / 1024.0
    );
    println!(
        "║  8 × 4Kbit    │ {:>10} │ {:>10.1} │ {:>10.1}       ║",
        fl_8.and_constraints,
        fl_8.prove_time_ms,
        fl_8.proof_size_bytes as f64 / 1024.0
    );
    println!(
        "║ 16 × 4Kbit    │ {:>10} │ {:>10.1} │ {:>10.1}       ║",
        fl_16.and_constraints,
        fl_16.prove_time_ms,
        fl_16.proof_size_bytes as f64 / 1024.0
    );
    println!("╚══════════════════════════════════════════════════════════════╝");
