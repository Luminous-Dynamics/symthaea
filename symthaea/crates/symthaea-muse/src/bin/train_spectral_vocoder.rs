...
fn main() {
    ...
    println!("  Untrained baseline: avg_error={:.4}, max_error={:.4}", avg_error, max_error);
...
fn generate_training_data() -> Vec<TrainingPair> {
    use symthaea_muse::{compose, MuseConfig, MusicalState, AudioData};
        ...
        for &arousal in &arousal_values {
            for &valence in &valence_values {
                ...
                if samples.len() >= 256 {
                    ...
                    pairs.push(TrainingPair { state, mel_frame: mel });
                }
            }
        }
    }
...
fn evaluate_training_data(pairs: &[TrainingPair]) -> (f32, f32) {
    ...
    for pair in pairs {
        ...
        // L1 error
        let error: f32 = predicted.iter().zip(target.iter())
            .map(|(&p, &t)| (p - t).abs())
            .sum::<f32>() / predicted.len().max(1) as f32;
