...
pub fn score(notes: &[Note], profile: &TasteProfile) -> TasteScore {
    if notes.len() < 10 {
        return TasteScore {
            melodic_movement: 0.0, pitch_diversity: 0.0, interval_balance: 0.0,
            direction_balance: 0.0, dynamic_range: 0.0, pitch_register: 0.0,
            composite: 0.0, details: vec![("Too few notes".into(), 0.0, "Need ≥10".into())],
        };
    }
    ...
    details.push((
        "Repeated notes".into(), repeated_pct,
        format!("{:.0}% (target {:.0}%)", repeated_pct, profile.target_repeated_pct),
    ));

    // 2. Pitch diversity
    let unique = midi_pitches.iter().collect::<std::collections::HashSet<_>>().len();
    ...
    for &p in &midi_pitches { *pitch_counts.entry(p).or_insert(0) += 1; }
    let top_pct = pitch_counts.values().max().copied().unwrap_or(0) as f32 / notes.len() as f32 * 100.0;
    let dominance_score = if top_pct <= profile.max_top_note_pct { 100.0 }
        else { (1.0 - (top_pct - profile.max_top_note_pct) / 50.0).max(0.0) * 100.0 };

    let diversity = (unique_score * 0.5 + dominance_score * 0.5).min(100.0);
    details.push(("Unique pitches".into(), unique as f32, format!("{} (target ≥{})", unique, profile.min_unique_pitches)));
    details.push(("Top note %".into(), top_pct, format!("{:.0}% (target ≤{:.0}%)", top_pct, profile.max_top_note_pct)));

    // 3. Interval balance
    let steps_pct = intervals.iter().filter(|&&iv| iv != 0 && iv.abs() <= 2).count() as f32 / n * 100.0;
    let thirds_pct = intervals.iter().filter(|&&iv| iv.abs() >= 3 && iv.abs() <= 5).count() as f32 / n * 100.0;
    ...
    details.push(("Steps %".into(), steps_pct, format!("{:.0}% (target {:.0}%)", steps_pct, profile.target_steps_pct)));
    details.push(("Thirds %".into(), thirds_pct, format!("{:.0}% (target {:.0}%)", thirds_pct, profile.target_thirds_pct)));
    details.push(("Leaps %".into(), leaps_pct, format!("{:.0}% (target {:.0}%)", leaps_pct, profile.target_leaps_pct)));
    ...
    details.push(("Ascending %".into(), ascending_pct, format!("{:.0}% (target {:.0}%)", ascending_pct, profile.target_ascending_pct)));
    ...
    details.push(("Velocity range".into(), vel_range, format!("{:.0} (target ≥{:.0})", vel_range, profile.min_velocity_range)));

    // 6. Pitch register
    let mean_midi = midi_pitches.iter().map(|&p| p as f32).sum::<f32>() / midi_pitches.len() as f32;
    let pitch_range = *midi_pitches.iter().max().unwrap() as f32 - *midi_pitches.iter().min().unwrap() as f32;
    ...
    details.push(("Mean MIDI".into(), mean_midi, format!("{:.0} (target {:.0})", mean_midi, profile.target_mean_midi)));
    details.push(("Pitch range".into(), pitch_range, format!("{:.0} semi (target {:.0})", pitch_range, profile.target_pitch_range)));
...
pub fn print_report(score: &TasteScore) {
    ...
    println!("╠══════════════════════════════════════════════════════════════╣");
    println!("║  Melodic Movement:  {:5.1}/100  {}║",
        score.melodic_movement, rating(score.melodic_movement));
    println!("║  Pitch Diversity:   {:5.1}/100  {}║",
        score.pitch_diversity, rating(score.pitch_diversity));
    println!("║  Interval Balance:  {:5.1}/100  {}║",
        score.interval_balance, rating(score.interval_balance));
    println!("║  Direction Balance: {:5.1}/100  {}║",
        score.direction_balance, rating(score.direction_balance));
    println!("║  Dynamic Range:     {:5.1}/100  {}║",
        score.dynamic_range, rating(score.dynamic_range));
    println!("║  Pitch Register:    {:5.1}/100  {}║",
        score.pitch_register, rating(score.pitch_register));
    println!("╠══════════════════════════════════════════════════════════════╣");
    println!("║  COMPOSITE:         {:5.1}/100  {}║",
...
mod tests {
    ...
    fn make_scale() -> Vec<Note> {
        ...
        freqs.iter().enumerate().map(|(i, &f)| Note {
            frequency: f, start_time: i as f32 * 0.3, duration: 0.3,
    ...
    fn make_monotone() -> Vec<Note> {
        ...
        (0..20).map(|i| Note {
            frequency: 440.0, start_time: i as f32 * 0.3, duration: 0.3, velocity: 0.7,
        }).collect()
    }
    ...
    fn scale_scores_better_than_monotone() {
        ...
        assert!(scale.composite > mono.composite,
            "scale ({:.1}) should beat monotone ({:.1})", scale.composite, mono.composite);
    }
    ...
    fn monotone_has_zero_movement() {
        ...
        assert!(s.melodic_movement < 5.0, "monotone should have near-zero movement: {:.1}", s.melodic_movement);
    }
    ...
    fn scale_has_good_movement() {
        ...
        assert!(s.melodic_movement > 50.0, "scale should have good movement: {:.1}", s.melodic_movement);
    }
