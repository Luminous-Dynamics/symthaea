...
fn define_scenarios() -> Vec<Scenario> {
    vec![
        // 4 extremes
        Scenario { name: "Excitement/Joy",    arousal: 0.9, valence: 0.8,
            state: make_state(0.9, 0.8, 0.80) },
        Scenario { name: "Panic/Anger",       arousal: 0.9, valence: -0.8,
            state: make_state(0.9, -0.8, 0.30) },
        Scenario { name: "Content/Peace",     arousal: 0.1, valence: 0.8,
            state: make_state(0.1, 0.8, 0.65) },
        Scenario { name: "Sadness/Grief",     arousal: 0.1, valence: -0.8,
            state: make_state(0.1, -0.8, 0.20) },
        // 8 intermediate states at 45-degree intervals
        Scenario { name: "Elation",           arousal: 0.9, valence: 0.0,
            state: make_state(0.9, 0.0, 0.55) },
        Scenario { name: "Serenity",          arousal: 0.1, valence: 0.0,
            state: make_state(0.1, 0.0, 0.50) },
        Scenario { name: "Delight",           arousal: 0.5, valence: 0.8,
            state: make_state(0.5, 0.8, 0.70) },
        Scenario { name: "Melancholy",        arousal: 0.5, valence: -0.8,
            state: make_state(0.5, -0.8, 0.35) },
        Scenario { name: "Tension",           arousal: 0.75, valence: -0.4,
            state: make_state(0.75, -0.4, 0.40) },
        Scenario { name: "Tenderness",        arousal: 0.25, valence: 0.4,
            state: make_state(0.25, 0.4, 0.60) },
        Scenario { name: "Agitation",         arousal: 0.75, valence: 0.4,
            state: make_state(0.75, 0.4, 0.65) },
        Scenario { name: "Despair",           arousal: 0.25, valence: -0.4,
...
fn extract_features(samples: &[f32], sr: u32) -> Features {
    ...
    let harmonic_ratio = if ac0 > 1e-8 { (max_ac / ac0).clamp(0.0, 1.0) } else { 0.0 };
    ...
    for k in 1..dft_size / 2 {
        ...
        if freq < 60.0 || freq > 3000.0 { continue; }
        let mut re = 0.0f32;
        let mut im = 0.0f32;
    ...
    if cs > 0.01 {
        for c in &mut chroma { *c /= cs; }
    }
    // Major key profile correlation (Krumhansl)
    let major_prof: [f32; 12] = [6.35, 2.23, 3.48, 2.33, 4.38, 4.09, 2.52, 5.19, 2.39, 3.66, 2.29, 2.88];
    let minor_prof: [f32; 12] = [6.33, 2.68, 3.52, 5.38, 2.60, 3.53, 2.54, 4.75, 3.98, 2.69, 3.34, 3.17];
    ...
    let active_pcs: Vec<usize> = chroma.iter().enumerate()
        .filter(|(_, &v)| v > 0.05)
        .map(|(i, _)| i)
        .collect();
...
fn pearson_12(chroma: &[f32; 12], profile: &[f32; 12], shift: usize) -> f32 {
    ...
    let mx = sx / 12.0;
    let my = sy / 12.0;
    let mut cov = 0.0f32; let mut vx = 0.0f32; let mut vy = 0.0f32;
    for j in 0..12 {
...
fn residuals(y: &[f32], x1: &[f32], x2: &[f32]) -> Vec<f32> {
    ...
    let mut s11 = 0.0f32; let mut s22 = 0.0f32; let mut s12 = 0.0f32;
    let mut s1y = 0.0f32; let mut s2y = 0.0f32;
    for i in 0..n {
        let d1 = x1[i] - m1;
        let d2 = x2[i] - m2;
        let dy = y[i] - my;
...
fn main() {
    ...
    println!("({} states x {} seeds = {} data points per feature)", n_scenarios, n_seeds, n_total);
    ...
    let feature_cols: Vec<Vec<f32>> = (0..11).map(|fi| {
        all_features.iter().map(|f| f.as_array()[fi]).collect()
    }).collect();

    println!();
    println!("{:<20} | {:>10} | {:>9} | {:>4} | {:>10} | {:>9} | {:>4} | {:>13}",
    ...
    for (fi, name) in FEATURE_NAMES.iter().enumerate() {
        ...
        let sig_a = if p_a < bonferroni as f64 { "***" } else if p_a < 0.05 { " * " } else { "   " };
        let (ci_lo, ci_hi) = fisher_ci_95(r_a, n_total);

        let r_v = pearson_r(&all_valence, col);
        let p_v = p_value_for_r(r_v, n_total);
        let sig_v = if p_v < bonferroni as f64 { "***" } else if p_v < 0.05 { " * " } else { "   " };
    ...
    for (ci, abl_cfg) in ablation_configs.iter().enumerate() {
        print!("  [{}/{}] {:<18} ... ", ci + 1, ablation_configs.len(), abl_cfg.name);
        ...
        let state_discrim = if pairs > 0 { total_dist / pairs as f32 } else { 0.0 };
        ...
        let feat_cols: Vec<Vec<f32>> = (0..11).map(|fi| {
            state_features.iter().map(|f| f.as_array()[fi]).collect()
        }).collect();
        let r_a: f32 = feat_cols.iter()
            .map(|col| pearson_r(&arousal_vals, col).abs())
            .sum::<f32>() / 11.0;
        let r_v: f32 = feat_cols.iter()
    ...
    for res in &ablation_results {
        ...
        println!("{:<18} | {:>14.3} | {:>12.5} | {:>+10.3} | {:>+10.3} | {:>11}",
    ...
    println!("==============================================================================");
    println!("  Analysis complete. {} total data points across {} configurations.",
        n_total + ablation_configs.len() * extreme_indices.len() * 3, 1 + ablation_configs.len());
    println!("==============================================================================");
}
...
fn average_features(features: &[Features]) -> Features {
    ...
    if n < 1.0 { return Features::default(); }
    Features {
