...
fn main() {
    ...
    let tests = vec![
        ...
        ("peaceful_piano", 45.0, MusicalState {
        ...
        ("energetic_flow", 30.0, MusicalState {
        ...
        ("dark_tension", 30.0, MusicalState {
        ...
        ("sacred_stillness", 45.0, MusicalState {
    ...
    for (name, duration, state) in &tests {
        ...
        let spec = hound::WavSpec {
            channels: 2, sample_rate, bits_per_sample: 16,
            sample_format: hound::SampleFormat::Int,
        };
        let mut w = hound::WavWriter::create(&path, spec).expect("wav");
        for pair in &all {
            for &s in pair {
                w.write_sample((s * 32767.0).clamp(-32768.0, 32767.0) as i16).expect("write");
            }
        }
        w.finalize().expect("finalize");

        // Analyze
        let peak = all.iter().flat_map(|p| p.iter()).map(|s| s.abs()).fold(0.0f32, f32::max);
        let rms = (all.iter().flat_map(|p| p.iter()).map(|s| s * s).sum::<f32>() / (all.len() * 2) as f32).sqrt();
        let size_kb = std::fs::metadata(&path).map(|m| m.len() / 1024).unwrap_or(0);

        println!("{} ({}KB) peak={:.3} rms={:.4}", path.display(), size_kb, peak, rms);
    }
