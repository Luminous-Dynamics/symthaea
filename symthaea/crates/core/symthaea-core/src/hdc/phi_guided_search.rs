...
pub struct PhiGuidedOptimizer {
    ...
    /// Φ calculator
    calculator: ConnectivityCalculator,
...
impl PhiGuidedOptimizer {
    ...
    pub fn new(config: PhiOptimizationConfig) -> Self {
        Self {
            config,
            calculator: ConnectivityCalculator::new(),
            step: 0,
            phi_history: Vec::new(),
            best_phi: 0.0,
            rng_state: 42,
    ...
    pub fn optimize_step(&mut self, network: &mut ConsciousnessNetwork) -> OptimizationResult {
        ...
        let current_phi = self.calculator.algebraic_connectivity(&representations);
    ...
    fn compute_edge_gradients(&mut self, network: &mut ConsciousnessNetwork, base_phi: f64) {
        ...
        for edge_idx in 0..network.edges.len() {
            ...
            let representations = network.to_node_representations();
            let phi_minus = self.calculator.algebraic_connectivity(&representations);
    ...
    pub fn optimize(
        ...
        let initial_phi = {
            let representations = network.to_node_representations();
            self.calculator.algebraic_connectivity(&representations)
        };
...
mod tests {
    ...
    fn test_phi_calculation() {
        ...
        let ring_phi = calculator.algebraic_connectivity(&ring_representations);

        // Star for comparison
        InitializationStrategy::Star.apply(&mut network, 0);
        let star_representations = network.to_node_representations();
        let star_phi = calculator.algebraic_connectivity(&star_representations);
