...
mod app {
    ...
    impl Default for Args {
        fn default() -> Self {
            Self {
                ...
                retrieval_fanout: 1,
                split_threshold: 100,
                retrieval_policy: "LeafOnly".to_string(),
    ...
    fn validate_args(args: &Args) -> Result<(), String> {
        ...
        let valid_policies = ["LeafOnly", "ChildFirstParentFallback", "MultiLevel"];
        if !valid_policies.contains(&args.retrieval_policy.as_str()) {
            return Err(format!(
                "--retrieval-policy must be one of: {}",
                valid_policies.join(", ")
            ));
        }
    ...
    fn run_bakeoff(args: &Args) -> BakeoffReport {
        ...
        let policy = match args.retrieval_policy.as_str() {
    ...
    fn run_trial(
        ...
        for example in examples {
            ...
            // Adaptive bundle for primary leaf
            if let Some(node_idx) = pyramid.find_leaf_node_index(leaf_idx) {
                let load = leaf_counts[leaf_idx];
                pyramid.bundle_at_node_adaptive(node_idx, &binding, split_threshold, load);
                leaf_counts[leaf_idx] += 1;
            }

            // Redundancy
            if redundancy_k > 1 {
                let neighbors = leaf_candidates(leaf_idx, config.branching, redundancy_k);
                for &neighbor_idx in neighbors.iter().skip(1) {
                    if let Some(node_idx) = pyramid.find_leaf_node_index(neighbor_idx) {
                        let node = pyramid.nodes[node_idx].clone();
                        pyramid.bundle_at_node(&node, &binding);
                        leaf_counts[neighbor_idx] += 1;
                    }
                }
            }
            ...
            .map(|router| {
                ...
                RouterSummary {
                    ...
                    retrieval_fanout: matching
                        ...
                        .max()
                        .unwrap_or(1),
                    missing_leaf_count: (matching
                        .iter()
                        .map(|trial| trial.missing_leaf_count)
                        .sum::<usize>() as f32
                        / denom) as usize,
