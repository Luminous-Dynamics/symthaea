...
/// Hierarchical level for predictive processing within consciousness (oscillatory-specific)
#[derive(Debug, Clone, Serialize, Deserialize)]
pub(super) struct PredictiveLevel {
...
impl PredictiveLevel {
    pub fn new(name: impl Into<String>, level_idx: usize) -> Self {
...
/// Complete state of predictive consciousness system (oscillatory-specific)
#[derive(Debug, Clone, Serialize, Deserialize)]
pub(super) struct PredictiveConsciousnessState {
    // === Hierarchical Levels ===
    /// Sensory level (raw input processing)
    pub(super) sensory: PredictiveLevel,
    /// Perceptual level (feature integration)
    pub(super) perceptual: PredictiveLevel,
    /// Conceptual level (abstract representations)
    pub(super) conceptual: PredictiveLevel,
    /// Metacognitive level (awareness of awareness)
    pub(super) metacognitive: PredictiveLevel,
...
impl GlobalWorkspaceState {
    ...
    pub fn update_ignition(
        ...
        if self.is_ignited {
            ...
            // Note: Extract values before mutable borrow to avoid E0502
            let winner_info = self.processors.iter()
                .max_by(|a, b| a.competition_strength().partial_cmp(&b.competition_strength()).unwrap())
                .map(|w| (w.processor, w.competition_strength()));

            if let Some((winner_proc, winner_strength)) = winner_info {
                if winner_strength > self.ignition_threshold {
                    // IGNITION! This processor wins access to consciousness
                    self.is_ignited = true;
                    self.broadcasting_processor = Some(winner_proc);
                    self.broadcast_strength = gamma_power * gamma_coherence;
                    self.broadcast_duration = 0.0;
                    ...
                    if let Some(p) = self.processors.iter_mut().find(|p| p.processor == winner_proc) {
...
impl MultiBandState {
    ...
    pub fn advance(&mut self, dt: f64) {
        ...
        // Note: Read theta phase first to avoid simultaneous mutable/immutable borrow
        let theta_phase_cos = self.bands.get(&FrequencyBand::Theta)
            .map(|theta| theta.phase.cos());
        if let (Some(cos_val), Some(gamma)) = (theta_phase_cos, self.bands.get_mut(&FrequencyBand::Gamma)) {
            let theta_phase_factor = (cos_val + 1.0) / 2.0; // 0-1 based on theta phase
        ...
        // Note: Read alpha power first to avoid simultaneous mutable/immutable borrow
        let alpha_power_val = self.bands.get(&FrequencyBand::Alpha).map(|alpha| alpha.power);
        if let (Some(alpha_val), Some(gamma)) = (alpha_power_val, self.bands.get_mut(&FrequencyBand::Gamma)) {
            let suppression = self.alpha_gamma_coupling * alpha_val * 0.2;
...
impl ResonanceDetector {
            ...
            for j in (i + 1)..process_ids.len() {
                ...
                if let (Some(proc_a), Some(proc_b)) = (
                    ...
                    if proc_a.active && proc_b.active {
                        ...
                        // Calculate PLV from history (outside mutable borrow)
                        if let Some(history) = self.phase_histories.get(&key) {
                            if history.len() >= 10 {
                                let plv = Self::compute_plv(history);
                                let mean_diff = Self::compute_mean_phase(history);
                                let plv_threshold = self.plv_threshold;
                                ...
                                if let Some(resonance) = self.resonances.get_mut(&key) {
                                    ...
                                    resonance.sample_count = sample_count;
                                    resonance.is_resonating = plv > plv_threshold;
                                }
                            }
                        }
                    }
    ...
    fn compute_plv(history: &VecDeque<f64>) -> f64 {
        if history.is_empty() {
            return 0.0;
        }
    ...
    fn compute_mean_phase(history: &VecDeque<f64>) -> f64 {
        if history.is_empty() {
            return 0.0;
        }
...
impl AttentionSpotlight {
    ...
    /// Shift attention smoothly to new focus (alias for focus_on with transition)
    pub fn shift_attention(&mut self, position: f64, intensity: f64) {
        self.focus_on(position, intensity);
    }
    ...
    /// Get the intensity of the primary attention focus
    pub fn primary_intensity(&self) -> f64 {
        self.primary_focus.intensity
    }
