    ...
    fn calculate_harmonic_alignment(&self, candidate: &CandidatePrimitive) -> f64 {
        ...
        match candidate.tier {
            ...
            PrimitiveTier::Consciousness => {
                // Consciousness primitives → Integral Wisdom (self-awareness)
                // + Resonant Coherence (integrated experience) + Pan-Sentient Flourishing
                test_field.set_level(
                    FiduciaryHarmonic::IntegralWisdom,
                    test_field.get_level(FiduciaryHarmonic::IntegralWisdom) + 0.18,
                );
                test_field.set_level(
                    FiduciaryHarmonic::ResonantCoherence,
                    test_field.get_level(FiduciaryHarmonic::ResonantCoherence) + 0.12,
                );
                test_field.set_level(
                    FiduciaryHarmonic::PanSentientFlourishing,
                    test_field.get_level(FiduciaryHarmonic::PanSentientFlourishing) + 0.10,
                );
            }
        }
