...
fn test_training_convergence_trajectory() {
    ...
    let (metrics, _, _, _) = train_with_adam(&mut gen, &dataset, &train_cfg, None);
...
fn test_checkpoint_training_resume() {
    ...
    let (metrics_phase1, adam_state, _, _) = train_with_adam(&mut gen, &dataset, &train_cfg, None);
...
fn test_training_gradient_health_after_diverse_training() {
    ...
    let (metrics, adam_state, diag, _) = train_with_adam(&mut gen, &dataset, &train_cfg, None);
