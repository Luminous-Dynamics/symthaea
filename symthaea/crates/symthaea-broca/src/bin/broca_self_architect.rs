...
fn main() -> Result<()> {
    ...
    let diagnostics = generator.substrate_rewriter.monitor_integrity("symthaea-broca")?;
    
    // Identify a warning to resolve (e.g. an unused variable)
    if let Some(warning) = diagnostics.iter().find(|d| d["message"]["level"] == "warning") {
        let message = warning["message"]["message"].as_str().unwrap_or("unknown");
        let file = warning["message"]["spans"][0]["file_name"].as_str().unwrap_or("unknown");
        println!("✨ Integrity Violation detected: '{}' in {}.", message, file);
        ...
        if let Ok(_) = generator.verify_physical_safety("source_refactor", &nucleus) {
             ...
             if let Ok(_) = generator.substrate_rewriter.apply_patch(file, &patched_code) {
                  // 7. Verify the fix
                  println!("\n[Stage 7] Verifying fix through Nix rebuild...");
                  let _ = generator.substrate_rewriter.trigger_rebuild("symthaea-broca");
             }
        }
    ...
    println!("   🌀 Reasoning through safety invariants for {}...", target_logic);
    let monologue = generator.generate_semantic_monologue(&channels, 3)?;
    let nucleus = generator.recursive_fold(&monologue);
    ...
    if let Ok(verified) = generator.formal_bridge.verify_invariant(&proof_goal) {
        if verified {
            println!("   💎 FORMAL PROOF ACHIEVED: '{}' is mathematically sound.", target_logic);
