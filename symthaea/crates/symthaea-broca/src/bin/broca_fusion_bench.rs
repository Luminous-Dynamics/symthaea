...
fn configs() -> Vec<BenchRun> {
    vec![
        BenchRun { name: "baseline (all off)", setup: |_| {} },
        BenchRun { name: "compositional (α=0.15)", setup: |c| {
            c.controller.enable_compositional_logits = true;
        }},
        BenchRun { name: "adaptive alpha", setup: |c| {
            c.controller.enable_compositional_logits = true;
            c.controller.adaptive_compositional_alpha = true;
        }},
        BenchRun { name: "algebraic coherence", setup: |c| {
            c.gating.enable_algebraic_correction = true;
        }},
        BenchRun { name: "adaptive dt", setup: |c| {
            c.controller.enable_adaptive_dt = true;
        }},
        BenchRun { name: "soft veto", setup: |c| {
            c.gating.enable_soft_veto = true;
        }},
        BenchRun { name: "ALL ON", setup: |c| {
            c.controller.enable_compositional_logits = true;
            c.controller.adaptive_compositional_alpha = true;
            c.controller.enable_adaptive_dt = true;
            c.gating.enable_algebraic_correction = true;
...
fn make_dataset(tok: &BpeTokenizer) -> TrainingDataset {
    ...
    let texts = ["I understand", "the answer is", "let me clarify", "perhaps we should"];
    for (i, text) in texts.iter().enumerate() {
        let ch = ThoughtChannels::with_intent(i);
        for _ in 0..3 { ds.push(TrainingPair::new(ch, text.to_string(), tok)); }
    }
    ds
}
...
fn main() {
    ...
    if epochs > 0 {
        ...
        eprintln!("Training done. Final loss: {:.4}", m.last().map(|m| m.avg_loss).unwrap_or(0.0));
    }
    ...
    println!("\n{:<30} {:>8} {:>6} {:>8} {:>6}", "Config", "Perplx", "Eng%", "Coher", "Hall");
    println!("{}", "-".repeat(62));
    for (name, r) in &results {
        println!("{:<30} {:>8.1} {:>.3} {:>+8.4} {:>.3}",
            &name[..name.len().min(30)], r.perplexity, r.english_word_ratio,
            r.avg_coherence, r.hallucination_rate.unwrap_or(0.0));
    }

    if let Some((_, ref base)) = results.first() {
        println!("\nDelta from baseline:");
        for (name, r) in results.iter().skip(1) {
            println!("  {name}: ppl={:+.1} coh={:+.4}",
                r.perplexity - base.perplexity, r.avg_coherence - base.avg_coherence);
        }
    }
}
