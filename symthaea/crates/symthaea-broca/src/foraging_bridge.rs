...
impl ForagingBridge {
    ...
    pub fn forage(&self, query: &str) -> Result<String> {
        #[cfg(feature = "mamba-cpu")]
        {
            ...
            let url = format!("{}/search?q={}&format=json", self.endpoint, urlencoding::encode(query));

            let response: SearxResponse = ureq::get(&url)
            ...
            for res in response.results.iter().take(3) {
                aggregated.push_str(&format!("Source: {}\nContent: {}\n\n", res.url, res.content));
            }

            if aggregated.is_empty() {
                return Err(anyhow::anyhow!("Foraging mission yielded zero empirical hits."));
            }

            println!("✅ Foraging SUCCESS. Retrieved {} characters of knowledge.", aggregated.len());
            Ok(aggregated)
        }
        #[cfg(not(feature = "mamba-cpu"))]
        {
            let _ = query;
            Err(anyhow::anyhow!("Foraging requires mamba-cpu feature (ureq)."))
        }
    }
}
