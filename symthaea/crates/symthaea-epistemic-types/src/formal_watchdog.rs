use crate::global_ledger::{GlobalEpistemicLedger, GlobalClaimStatus};
            ...
            if claim.status == GlobalClaimStatus::Proven {
                if let Some(path) = &claim.formal_proof_path {
                    if !Path::new(path).exists() {
                        violations.push(format!("Proven claim '{}' missing proof file at: {}", claim.name, path));
                    } else {
                        // verify the file compiles in lean 4
                        if !Self::check_lean_compilation(path) {
                             violations.push(format!("Proven claim '{}' failed formal verification at: {}", claim.name, path));
                        }
                    }
                } else {
                    violations.push(format!("Proven claim '{}' has no associated proof path.", claim.name));
                }
            }
        }
        violations
