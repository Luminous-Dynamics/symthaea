...
pub struct QuadrupedEmbodiment { ctrl: QuadrupedController, sim: SimpleQuadrupedSimulator, enc: QuadrupedHdcEncoder, last_p: Option<ContinuousHV>, steps: usize, safety: MotorSafetyLevel, safety_ov: Option<MotorSafetyLevel>, effort: f32, pe: f32, gait: GaitType }
impl QuadrupedEmbodiment {
    pub fn new(g: &GenesisSeed) -> Self { let c = QuadrupedConfig::default(); Self { ctrl: QuadrupedController::new(g, &c), sim: SimpleQuadrupedSimulator::new(), enc: QuadrupedHdcEncoder::new(g, 32), last_p: None, steps: 0, safety: MotorSafetyLevel::Green, safety_ov: None, effort: 0.0, pe: 0.0, gait: GaitType::Trot } }
    ...
    pub fn reset(&mut self) { self.sim.reset(); self.ctrl.reset(); self.enc.reset(); self.last_p = None; self.steps = 0; self.safety = MotorSafetyLevel::Green; self.safety_ov = None; self.effort = 0.0; self.pe = 0.0; self.gait = GaitType::Trot; }
    pub fn safety_level(&self) -> MotorSafetyLevel { self.safety }
    pub fn total_steps(&self) -> usize { self.steps }
    pub fn telemetry(&self) -> EmbodimentTelemetry { EmbodimentTelemetry { total_steps: self.steps as u64, control_effort: self.effort, prediction_error: self.pe, safety_level: format!("{:?}", self.safety), platform: "quadruped".to_string(), num_actuators: NUM_ACTUATORS, epistemic_grounding: grounding_label(GROUNDING_SENSORIMOTOR).to_string(), observation_confidence: grounding_from_prediction_error(self.pe) } }
}
