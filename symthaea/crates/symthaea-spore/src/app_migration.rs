...
pub fn build_mapping_db() -> HashMap<String, Vec<NixAlternative>> {
    ...
    exact(&mut db, "microsoft edge", "microsoft-edge", "Microsoft Edge");
    exact(&mut db, "brave", "brave", "Brave");
    exact(&mut db, "vivaldi", "vivaldi", "Vivaldi");
    exact(&mut db, "opera", "opera", "Opera");
    alt(&mut db, "safari", "firefox", "Firefox", "Safari is macOS-only; Firefox is the closest cross-platform equivalent");

    // ── Office / Productivity ──
    alt(&mut db, "microsoft office", "libreoffice", "LibreOffice", "Full office suite; good .docx/.xlsx compatibility");
    alt(&mut db, "microsoft word", "libreoffice-still", "LibreOffice Writer", "Good .docx compatibility");
    alt(&mut db, "microsoft excel", "libreoffice-still", "LibreOffice Calc", "Good .xlsx compatibility; some macro limitations");
    alt(&mut db, "microsoft powerpoint", "libreoffice-still", "LibreOffice Impress", "Good .pptx compatibility");
    alt(&mut db, "microsoft outlook", "thunderbird", "Thunderbird", "Email client; Exchange via TbSync add-on");
    exact(&mut db, "thunderbird", "thunderbird", "Thunderbird");
    alt(&mut db, "microsoft onenote", "joplin-desktop", "Joplin", "Note-taking with sync; also: obsidian");
    alt(&mut db, "notion", "obsidian", "Obsidian", "Local-first knowledge base; or use Notion web app");
    exact(&mut db, "obsidian", "obsidian", "Obsidian");
    web_alt(&mut db, "google docs", "Web app works natively in any browser");
    web_alt(&mut db, "google sheets", "Web app works natively in any browser");
    web_alt(&mut db, "google slides", "Web app works natively in any browser");

    // ── Development ──
    exact(&mut db, "visual studio code", "vscode", "VS Code");
    exact(&mut db, "vscode", "vscode", "VS Code");
    exact(&mut db, "code", "vscode", "VS Code");
    alt(&mut db, "visual studio", "vscode", "VS Code", "VS Code covers most use cases; .NET via dotnet-sdk");
    ...
    exact(&mut db, "jetbrains intellij idea", "jetbrains.idea-ultimate", "IntelliJ IDEA");
    exact(&mut db, "intellij idea", "jetbrains.idea-ultimate", "IntelliJ IDEA");
    exact(&mut db, "jetbrains pycharm", "jetbrains.pycharm-professional", "PyCharm");
    exact(&mut db, "pycharm", "jetbrains.pycharm-professional", "PyCharm");
    exact(&mut db, "jetbrains webstorm", "jetbrains.webstorm", "WebStorm");
    ...
    exact(&mut db, "jetbrains rustrover", "jetbrains.rust-rover", "RustRover");
    exact(&mut db, "github desktop", "github-desktop", "GitHub Desktop");
    ...
    alt(&mut db, "iterm", "alacritty", "Alacritty", "GPU-accelerated terminal; also: kitty, wezterm");
    ...
    // ── Creative / Design ──
    alt(&mut db, "adobe photoshop", "gimp", "GIMP", "Full image editor; also: krita for painting");
    alt(&mut db, "photoshop", "gimp", "GIMP", "Full image editor; also: krita for painting");
    alt(&mut db, "adobe illustrator", "inkscape", "Inkscape", "Vector graphics editor");
    alt(&mut db, "illustrator", "inkscape", "Inkscape", "Vector graphics editor");
    alt(&mut db, "adobe premiere pro", "kdenlive", "Kdenlive", "Video editor; also: shotcut, davinci-resolve");
    alt(&mut db, "premiere pro", "kdenlive", "Kdenlive", "Video editor; also: shotcut, davinci-resolve");
    alt(&mut db, "adobe after effects", "natron", "Natron", "Compositing; limited compared to AE");
    alt(&mut db, "adobe lightroom", "darktable", "darktable", "RAW photo processor and manager");
    alt(&mut db, "lightroom", "darktable", "darktable", "RAW photo processor and manager");
    alt(&mut db, "adobe indesign", "scribus", "Scribus", "Desktop publishing");
    alt(&mut db, "adobe acrobat", "okular", "Okular", "PDF viewer; editing via LibreOffice Draw");
    alt(&mut db, "adobe xd", "figma-linux", "Figma (Linux)", "UI/UX design; also: penpot (open-source)");
    alt(&mut db, "sketch", "figma-linux", "Figma (Linux)", "UI/UX design; also: penpot (open-source)");
    web_alt(&mut db, "figma", "Figma web app works natively; also figma-linux package");
    ...
    exact(&mut db, "davinci resolve", "davinci-resolve", "DaVinci Resolve");
    exact(&mut db, "kdenlive", "kdenlive", "Kdenlive");
    exact(&mut db, "handbrake", "handbrake", "HandBrake");

    // ── Music / Audio ──
    exact(&mut db, "spotify", "spotify", "Spotify");
    alt(&mut db, "apple music", "spotify", "Spotify", "Or use Apple Music web player");
    alt(&mut db, "itunes", "rhythmbox", "Rhythmbox", "Music library manager; Spotify for streaming");
    exact(&mut db, "vlc", "vlc", "VLC");
    exact(&mut db, "vlc media player", "vlc", "VLC");
    alt(&mut db, "foobar2000", "deadbeef", "DeaDBeeF", "Lightweight audio player");
    exact(&mut db, "ardour", "ardour", "Ardour");
    alt(&mut db, "ableton live", "ardour", "Ardour", "DAW; also: lmms, reaper (via wine)");
    alt(&mut db, "fl studio", "lmms", "LMMS", "Music production; limited compared to FL Studio");
    alt(&mut db, "logic pro", "ardour", "Ardour", "DAW; Logic Pro is macOS-only");
    ...
    exact(&mut db, "microsoft teams", "teams-for-linux", "Teams for Linux");
    no_match(&mut db, "facetime", "No Linux equivalent; use web-based video calls");
    no_match(&mut db, "imessage", "No Linux equivalent; use Signal or other cross-platform messengers");

    // ── File Management / Utilities ──
    exact(&mut db, "7-zip", "p7zip", "7-Zip");
    exact(&mut db, "7zip", "p7zip", "7-Zip");
    alt(&mut db, "winrar", "p7zip", "7-Zip", "Handles RAR and most archive formats");
    exact(&mut db, "filezilla", "filezilla", "FileZilla");
    alt(&mut db, "total commander", "krusader", "Krusader", "Twin-panel file manager; also: midnight commander");
    exact(&mut db, "bitwarden", "bitwarden-desktop", "Bitwarden");
    exact(&mut db, "keepassxc", "keepassxc", "KeePassXC");
    alt(&mut db, "1password", "bitwarden-desktop", "Bitwarden", "Open-source password manager; 1Password has a Linux client too");
    alt(&mut db, "lastpass", "bitwarden-desktop", "Bitwarden", "Open-source password manager");
    exact(&mut db, "syncthing", "syncthing", "Syncthing");
    alt(&mut db, "dropbox", "syncthing", "Syncthing", "P2P file sync; or use rclone for cloud storage");
    alt(&mut db, "google drive", "rclone", "rclone", "Mount Google Drive and 40+ cloud backends");
    alt(&mut db, "onedrive", "rclone", "rclone", "Mount OneDrive; also: onedrive (native client)");
    exact(&mut db, "qbittorrent", "qbittorrent", "qBittorrent");
    alt(&mut db, "utorrent", "qbittorrent", "qBittorrent", "Open-source torrent client");

    // ── System / Security ──
    alt(&mut db, "norton", "clamav", "ClamAV", "Linux has minimal virus risk; ClamAV for scanning");
    alt(&mut db, "mcafee", "clamav", "ClamAV", "Linux has minimal virus risk; ClamAV for scanning");
    alt(&mut db, "windows defender", "clamav", "ClamAV", "Linux has minimal virus risk");
    alt(&mut db, "malwarebytes", "clamav", "ClamAV", "Linux has minimal virus risk");
    alt(&mut db, "ccleaner", "bleachbit", "BleachBit", "System cleaner; NixOS garbage collection handles most of this");
    exact(&mut db, "wireshark", "wireshark", "Wireshark");
    exact(&mut db, "virtualbox", "virtualbox", "VirtualBox");
    exact(&mut db, "vmware", "vmware-workstation", "VMware Workstation");

    // ── Gaming ──
    exact(&mut db, "steam", "steam", "Steam");
    compat(&mut db, "epic games launcher", "heroic", "Heroic Games Launcher", "Open-source Epic/GOG launcher; games via Proton");
    compat(&mut db, "epic games", "heroic", "Heroic Games Launcher", "Open-source Epic/GOG launcher; games via Proton");
    exact(&mut db, "lutris", "lutris", "Lutris");
    compat(&mut db, "battle.net", "lutris", "Lutris", "Run via Wine/Proton through Lutris");
    ...
    alt(&mut db, "parallels", "virt-manager", "virt-manager", "KVM/QEMU frontend; also: gnome-boxes");
    alt(&mut db, "vmware fusion", "virt-manager", "virt-manager", "KVM/QEMU frontend");

    // ── macOS-specific ──
    alt(&mut db, "finder", "nautilus", "GNOME Files", "Default file manager; also: dolphin (KDE), thunar (XFCE)");
    alt(&mut db, "preview", "okular", "Okular", "Document/image viewer");
    alt(&mut db, "keynote", "libreoffice-still", "LibreOffice Impress", "Presentation software");
    alt(&mut db, "numbers", "libreoffice-still", "LibreOffice Calc", "Spreadsheet");
    alt(&mut db, "pages", "libreoffice-still", "LibreOffice Writer", "Word processor");
...
pub fn categorize_app(name: &str) -> &'static str {
    ...
    if lower.contains("chrome") || lower.contains("firefox") || lower.contains("brave")
        || lower.contains("edge") || lower.contains("safari") || lower.contains("opera")
        || lower.contains("vivaldi") || lower.contains("browser")
    {
        "Browsers"
    } else if lower.contains("office") || lower.contains("word") || lower.contains("excel")
        || lower.contains("powerpoint") || lower.contains("outlook") || lower.contains("libre")
        || lower.contains("notion") || lower.contains("obsidian") || lower.contains("onenote")
    {
        "Office & Productivity"
    } else if lower.contains("visual studio") || lower.contains("vscode") || lower.contains("code")
        || lower.contains("jetbrains") || lower.contains("intellij") || lower.contains("pycharm")
        || lower.contains("docker") || lower.contains("git") || lower.contains("terminal")
        || lower.contains("postman") || lower.contains("xcode")
    {
        "Development"
    } else if lower.contains("photoshop") || lower.contains("illustrator") || lower.contains("gimp")
        || lower.contains("inkscape") || lower.contains("blender") || lower.contains("premiere")
        || lower.contains("after effects") || lower.contains("lightroom") || lower.contains("krita")
        || lower.contains("figma") || lower.contains("sketch") || lower.contains("obs")
        || lower.contains("kdenlive") || lower.contains("davinci")
    {
        "Creative & Design"
    } else if lower.contains("spotify") || lower.contains("music") || lower.contains("itunes")
        || lower.contains("audacity") || lower.contains("ardour") || lower.contains("ableton")
        || lower.contains("fl studio") || lower.contains("logic pro") || lower.contains("vlc")
    {
        "Music & Media"
    } else if lower.contains("discord") || lower.contains("slack") || lower.contains("zoom")
        || lower.contains("teams") || lower.contains("telegram") || lower.contains("signal")
        || lower.contains("facetime") || lower.contains("imessage") || lower.contains("element")
    {
        "Communication"
    } else if lower.contains("steam") || lower.contains("epic") || lower.contains("minecraft")
        || lower.contains("battle.net") || lower.contains("lutris") || lower.contains("retroarch")
    {
        "Gaming"
    } else if lower.contains("norton") || lower.contains("mcafee") || lower.contains("defender")
        || lower.contains("wireshark") || lower.contains("virtualbox") || lower.contains("vmware")
        || lower.contains("ccleaner") || lower.contains("7-zip") || lower.contains("winrar")
    {
        "System & Utilities"
    } else {
        "Other"
...
pub fn generate_report(detected_apps: &[DetectedApp]) -> MigrationReport {
    ...
    for app in detected_apps {
        ...
        if alternatives.is_empty() {
            no_match_count += 1;
        } else if alternatives.iter().any(|a| a.match_kind == MatchKind::Exact) {
            exact_count += 1;
        } else {
            alt_count += 1;
        }
...
fn alt(
    ...
    key: &str, pkg: &str, name: &str, note: &str,
) {
    db.entry(key.to_string()).or_default().push(NixAlternative {
        nix_pkg: pkg.to_string(),
        display_name: name.to_string(),
...
mod tests {
    ...
    fn test_mapping_db_size() {
        ...
        assert!(db.len() >= 80, "DB has {} entries, expected >= 80", db.len());
    }
}
