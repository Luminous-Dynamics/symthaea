...
pub struct OrbitalEmbodiment { ctrl: OrbitalController, sim: SimpleOrbitalSimulator, enc: OrbitalHdcEncoder, last_p: Option<ContinuousHV>, steps: usize, safety: MotorSafetyLevel, safety_ov: Option<MotorSafetyLevel>, effort: f32, pe: f32 }
impl OrbitalEmbodiment {
    pub fn new(g: &GenesisSeed) -> Self { let c = OrbitalConfig::default(); Self { ctrl: OrbitalController::new(g, &c), sim: SimpleOrbitalSimulator::new(), enc: OrbitalHdcEncoder::new(g, 32), last_p: None, steps: 0, safety: MotorSafetyLevel::Green, safety_ov: None, effort: 0.0, pe: 0.0 } }
    ...
    pub fn step(&mut self, hv: &ContinuousHV, dt: f32, phi: f64) -> EmbodimentResult {
        let pl = MotorSafetyLevel::from_phi(phi); self.safety = match self.safety_ov { Some(ov) => pl.max(ov), None => pl };
