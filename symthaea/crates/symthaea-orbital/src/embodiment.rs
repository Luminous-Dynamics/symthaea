...
use crate::controller::OrbitalController; use crate::encoder::OrbitalHdcEncoder;
use crate::simulator::{OrbitalPhysicsSimulator, SimpleOrbitalSimulator}; use crate::types::{OrbitalConfig, NUM_ACTUATORS};
...
pub struct OrbitalEmbodiment { ctrl: OrbitalController, sim: SimpleOrbitalSimulator, enc: OrbitalHdcEncoder, last_p: Option<ContinuousHV>, steps: usize, safety: MotorSafetyLevel, safety_ov: Option<MotorSafetyLevel>, moral_safety: Option<MotorSafetyLevel>, effort: f32, pe: f32 }
impl OrbitalEmbodiment {
    pub fn new(g: &GenesisSeed) -> Self { let c = OrbitalConfig::default(); Self { ctrl: OrbitalController::new(g, &c), sim: SimpleOrbitalSimulator::new(), enc: OrbitalHdcEncoder::new(g, 32), last_p: None, steps: 0, safety: MotorSafetyLevel::Green, safety_ov: None, moral_safety: None, effort: 0.0, pe: 0.0 } }
    ...
    pub fn step(&mut self, hv: &ContinuousHV, dt: f32, phi: f64) -> EmbodimentResult {
        ...
        if gain <= 0.3 { cmd.joint_torques = [0.0; NUM_ACTUATORS]; } else { for t in &mut cmd.joint_torques { *t *= gain; } }
        if self.sim.state().comm_window == 0.0 && gain > 0.3 { for t in &mut cmd.joint_torques { *t *= 0.5; } }
    ...
    pub fn reset(&mut self) { self.sim.reset(); self.ctrl.reset(); self.enc.reset(); self.last_p = None; self.steps = 0; self.safety = MotorSafetyLevel::Green; self.safety_ov = None; self.moral_safety = None; self.effort = 0.0; self.pe = 0.0; }
...
impl symthaea_core::embodiment::EmbodimentBridge for OrbitalEmbodiment {
    ...
    fn platform(&self) -> symthaea_core::embodiment::EmbodimentPlatform { symthaea_core::embodiment::EmbodimentPlatform::Orbital }
...
#[cfg(test)] mod tests { use super::*;
    #[test] fn test_step() { let mut b = OrbitalEmbodiment::new(&GenesisSeed::from_phrase("t")); assert!(b.step(&ContinuousHV::random(16384, 42), 0.01, 0.7).success); }
    #[test] fn test_orange_parks() { let mut b = OrbitalEmbodiment::new(&GenesisSeed::from_phrase("t")); let r = b.step(&ContinuousHV::random(16384, 42), 0.01, 0.15); assert!(r.control_effort < 0.01); }
    #[test] fn test_moral_gate_ahimsa_forces_red() {
        ...
        assert_eq!(r.safety_level, MotorSafetyLevel::Red, "ahimsa must force Red regardless of phi");
        assert!(r.control_effort < 0.01, "Red must park, got effort {}", r.control_effort);
    }
}
