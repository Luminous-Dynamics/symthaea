...
use symtropy_math::{Point, Transform, Bivector};
...
async fn main() -> Result<()> {
    ...
    while current_time < SIM_DURATION_SECS {
        ...
        if current_time >= OVERLOAD_TIME && !overload_applied {
            info!("⚠️  [T={:.2}s] INJECTING EXTERNAL OVERLOAD: 20kg load binding joint!", current_time);
        ...
        if gain < 1.0 {
            info!("🛡️  [T={:.2}s] SAFETY GATE ENGAGED: Lumbar Gain throttled to {:.2}x", current_time, gain);
        }
        ...
        // Gather metrics
        if step_count % 30 == 0 {
            let energy = field.entities.get(&torso_handle).map(|e| e.energy.available).unwrap_or(0.0);
            let phi = field.phi(torso_handle);
            info!("[T={:.2}s] Energy: {:.1}J | Phi: {:.3} | Safety Tier: {:?}", 
