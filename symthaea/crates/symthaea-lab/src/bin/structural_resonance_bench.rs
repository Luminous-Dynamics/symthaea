...
async fn main() -> Result<()> {
    ...
    while current_time < SIM_DURATION_SECS {
        ...
        if safety.joint_tiers.iter().any(|t| *t != symtropy_consciousness_physics::safety::SafetyTier::Green) {
            resonance_detected = true;
        }

        // Metrics Gathering
        if step_count % 30 == 0 {
            info!("[T={:.2}s] Head Deflection: {:.3}m | Avg Spine Phi: {:.3}", 
                  current_time, max_head_deflection, field.phi(head_handle));
        }
