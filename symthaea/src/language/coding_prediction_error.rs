    ...
    errors
        ...
        .map(|(idx, error)| {
            ...
            let hint = format!(
                ...
                error.retry_number,
                error.surprise,
                repair_hint_for_prediction_error_category(&error.category),
                error.diagnostic
            );
            (label, hint)
        })
...
fn repair_hint_for_prediction_error_category(category: &str) -> &'static str {
    match category {
        "off_by_one" => {
...
fn test_prediction_errors(
    ...
    diagnostics
        ...
        .map(|diagnostic| {
            let category = categorize_test_failure_diagnostic(&diagnostic);
            text_prediction_error(
...
fn categorize_test_failure_diagnostic(diagnostic: &str) -> &'static str {
...
mod tests {
    ...
    #[test]
    fn test_failure_categories_capture_semantic_shape() {
        assert_eq!(
    ...
    #[test]
    fn semantic_test_failure_hint_is_actionable() {
        let result = ExecutionResult {
            compiled: true,
            compile_errors: Vec::new(),
            tests_passed: 1,
            ...
            simulated: false,
            binary_path: None,
            test_failures: vec![crate::language::code_executor::TestFailure {
                test_name: "tests::off_by_one".to_string(),
                assertion: None,
        ...
        assert_eq!(errors[0].category, "off_by_one");
        assert!(hints[0].1.contains("inclusive/exclusive"));
    }
