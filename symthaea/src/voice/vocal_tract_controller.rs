...
impl VocalTractController {
    ...
    /// Train the output projection on all phoneme targets from a FormantDatabase.
    ///
    /// For each epoch, iterates every phoneme, creates a deterministic cognitive HV
    ...
    pub fn train_on_phoneme_targets(
        &mut self,
        genesis: &GenesisSeed,
        db: &super::formant_targets::FormantDatabase,
        epochs: usize,
    ) -> f32 {
        ...
        let phoneme_hvs: Vec<(&str, ContinuousHV, FormantFrame)> = phonemes
            .iter()
            .filter_map(|name| {
                db.lookup(name).map(|target| {
...
/// Mean squared error between two FormantFrames (formants + scaled energy/voicing).
fn formant_mse(a: &FormantFrame, b: &FormantFrame) -> f32 {
    let diffs = [
        ...
        a.b3 - b.b3,
        a.f0 - b.f0,
        (a.energy - b.energy) * 1000.0,   // Scale to comparable magnitude
        (a.voicing - b.voicing) * 1000.0,
    ];
...
mod tests {
    ...
    fn test_phoneme_training_reduces_loss() {
        ...
        let first_loss = ctrl_baseline.train_on_phoneme_targets(&genesis, &db, 1);

        // Train for more epochs
        let final_loss = ctrl.train_on_phoneme_targets(&genesis, &db, 5);
