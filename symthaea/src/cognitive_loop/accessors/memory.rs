...
impl CognitiveLoopService {
    ...
    pub fn causal_graph(&self) -> Option<&CausalGraph> {
        self.memory.causal_enhancer.as_ref().map(|e| e.current_graph())
    }
    ...
    pub fn causal_stats(&self) -> Option<crate::causal::CausalLoopStats> {
        self.memory.causal_enhancer.as_ref().map(|e| e.stats().clone())
    }
    ...
    pub fn episodic_replay_stats(
        ...
        self.memory.episodic_persistence.replay.as_ref().map(|r| r.stats())
    }

    /// Get the number of stored episodes
    pub fn episodic_replay_count(&self) -> usize {
        self.memory.episodic_persistence
            .replay
            .as_ref()
            .map(|r| r.len())
            .unwrap_or(0)
