...
mod tests {
    ...
    fn test_interplanetary_tier_profile() {
        ...
        assert_eq!(profile.duty_cycle, 1.0); // No duty cycle in space
        assert!(profile.reliability > 0.99);
        assert!(profile.latency_ms >= 2_000); // At least LEO latency
    }
    ...
    fn test_all_with_space_includes_interplanetary() {
        ...
        // Original ALL should be unchanged (backward compatible)
    ...
    fn test_rtlsdr_receive_only() {
        ...
        // TX should always fail (receive-only device)
    ...
    fn test_rtlsdr_bands_comprehensive() {
        // Verify all defined bands are within RTL-SDR range (24 MHz - 1.766 GHz)
        let all_bands = [
            bands::FM_BROADCAST,
            bands::CB_RADIO,
            bands::FRS_GMRS,
