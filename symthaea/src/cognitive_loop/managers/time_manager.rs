...
impl CognitiveSubsystem for TimeManager {
    ...
    fn process(&mut self, snapshot: &CycleSnapshot) -> SubsystemOutput {
        ...
        if self.drift_surprise_ema > DRIFT_SURPRISE_THRESHOLD_PPM {
            let surprise_excess = (self.drift_surprise_ema - DRIFT_SURPRISE_THRESHOLD_PPM) / 100.0;
            output.arousal_delta += (surprise_excess * DRIFT_SURPRISE_NE_GAIN).min(0.05) as f32;
        }
