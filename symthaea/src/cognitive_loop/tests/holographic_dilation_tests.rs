...
mod tests {
    ...
    fn test_surprise_triggers_holographic_dilation_and_thermo_cost() {
        ...
        let initial_dim = service.sensorimotor.vision_sensory.vision_bridge
            .as_ref().unwrap().manifold().hdc_dim();
        ...
        let post_dim = service.sensorimotor.vision_sensory.vision_bridge
            .as_ref().unwrap().manifold().hdc_dim();
        ...
        assert!(post_thermo > 0.05, "Thermodynamic load should have increased (got {post_thermo})");
    }
    ...
    fn test_request_geodesic_triggers_mental_simulation() {
        ...
        {
            let bridge = service.sensorimotor.vision_sensory.vision_bridge.as_mut().unwrap();
            let frame = vec![128u8; 64 * 64 * 3];
            bridge.manifold_mut().observe_frame(&frame, 64, 64, 3, 0.033);
        }
        ...
        for i in 0..33 {
            ...
            let frame = if i % 2 == 0 { frame_a.clone() } else { frame_b.clone() };
        ...
        let bridge = service.sensorimotor.vision_sensory.vision_bridge.as_ref().unwrap();
        let telemetry = bridge.manifold().telemetry();
        
        // The manifold telemetry persists the LAST geodesic path generated
        assert!(!telemetry.last_geodesic_path.is_empty(), "Mental simulation path should be persisted in manifold telemetry");
        assert_eq!(telemetry.last_geodesic_length, 8, "Default geodesic length should be 8");
    ...
    fn test_imagine_future_proactive_simulation() {
        ...
        let movie = service.imagine_future(16).expect("Imagine future should succeed");
        
        assert_eq!(movie.frames.len(), 16);
        assert_eq!(movie.path_length, 16);
        assert!(service.thermodynamic_load() > initial_thermo, "Thermodynamic load should increase after imagination");
