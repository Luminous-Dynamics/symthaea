...
fn assert_metadata_finite(r: &CycleResult, cycle: usize) {
    ...
    // Consciousness
    assert_finite(m.consciousness.consciousness_level, "consciousness_level", cycle);
    assert_finite(
    ...
    assert_finite(m.embodied.embodied_phi_modulation, "embodied_phi_modulation", cycle);
    ...
    // Temporal / phenomenal
    assert_finite(m.temporal.temporal_coherence_score, "temporal_coherence_score", cycle);
    ...
    assert_finite(m.temporal.thermodynamic_entropy, "thermodynamic_entropy", cycle);
    assert_finite(m.temporal.thermodynamic_free_energy, "thermodynamic_free_energy", cycle);
    assert_finite(
    ...
    assert_finite_f32(m.reasoning_chain_confidence, "reasoning_chain_confidence", cycle);
    assert_finite(m.adaptive_reasoning_phi, "adaptive_reasoning_phi", cycle);
    assert_finite(m.meta_reasoning_confidence, "meta_reasoning_confidence", cycle);

    // Attention
    assert_finite_f32(m.attention.attention_schema_focus, "attention_schema_focus", cycle);
    assert_finite_f32(m.attention.psi_attention_avg, "psi_attention_avg", cycle);
    assert_finite_f32(m.attention.phi_attention_weight, "phi_attention_weight", cycle);

    // FEP
    assert_finite(m.fep.predictive_free_energy, "predictive_free_energy", cycle);
    assert_finite(m.fep.predictive_phi_modulation, "predictive_phi_modulation", cycle);
    ...
    assert_finite(m.ethics.value_evaluator_score, "value_evaluator_score", cycle);
    ...
    assert_finite_f32(m.harmonics.harmonies_alignment, "harmonies_alignment", cycle);
    assert_finite(m.harmonics.harmonic_field_coherence, "harmonic_field_coherence", cycle);
    assert_finite(m.harmonics.harmonic_love_resonance, "harmonic_love_resonance", cycle);

    // Quality diagnostics
    assert_finite(m.quality.dissipative_health, "dissipative_health", cycle);
    assert_finite(m.quality.dissipative_entropy_rate, "dissipative_entropy_rate", cycle);
    assert_finite(m.quality.epistemic_phi_eff, "epistemic_phi_eff", cycle);
    assert_finite(m.quality.equation_v2_consciousness, "equation_v2_consciousness", cycle);
    assert_finite_f32(m.quality.hierarchical_ltc_phi, "hierarchical_ltc_phi", cycle);
    assert_finite_f32(m.quality.meta_cognitive_accuracy, "meta_cognitive_accuracy", cycle);
    ...
    assert_finite_f32(m.predictive_behavioral_error, "predictive_behavioral_error", cycle);
    ...
    assert_finite(m.hierarchical_total_free_energy, "hierarchical_total_free_energy", cycle);
    ...
    assert_finite(m.phi_validation_correlation, "phi_validation_correlation", cycle);
    assert_finite(m.pipeline_consciousness, "pipeline_consciousness", cycle);
    assert_finite(m.multimodal_integrated_phi, "multimodal_integrated_phi", cycle);
    assert_finite_f32(m.epistemic_gate_confidence, "epistemic_gate_confidence", cycle);
    assert_finite(m.primitive_validation_phi_gain, "primitive_validation_phi_gain", cycle);
    assert_finite(m.primitive_validation_p_value, "primitive_validation_p_value", cycle);
    ...
    assert_finite_f32(m.perception_mean_coherence, "perception_mean_coherence", cycle);
    assert_finite_f32(m.drive_boredom, "drive_boredom", cycle);
    assert_finite_f32(m.drive_flow_intensity, "drive_flow_intensity", cycle);
    assert_finite_f32(m.drive_exploration_threshold, "drive_exploration_threshold", cycle);
...
fn smoke_test_all_subsystems_1000_cycles() {
    ...
    // ── Assertions ──
    assert_eq!(stats.total_cycles, TOTAL_CYCLES, "All 1000 cycles must complete");
    assert!(
