...
pub(crate) struct ConsciousnessEngineInput<'a> {
    ...
    /// Physiological coherence level [0,1] from CoherenceField.
    /// High coherence boosts unified consciousness (integrated state).
    /// Low coherence dampens it (scattered, stressed state).
    /// Science: McEwen (2007) — allostatic load degrades integration capacity.
    pub coherence_field_integration: f64,
