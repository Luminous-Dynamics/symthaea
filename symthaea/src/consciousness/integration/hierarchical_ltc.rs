...
// Re-use fast_sigmoid from dynamics::ltc (ported from crate)
use crate::dynamics::ltc::fast_sigmoid;

// Compatibility wrapper: SparseMatrix backed by CsrMatrix from dynamics::ltc
use crate::dynamics::ltc::CsrMatrix;

/// Sparse matrix wrapper for hierarchical LTC circuits.
/// Uses CsrMatrix internally but provides the seeded random_sparse constructor
/// needed by the hierarchical architecture.
...
impl SparseMatrix {
    ...
    /// Sparse matrix-vector multiply: output = self * input
    pub fn multiply(&self, input: &[f32], output: &mut [f32]) {
        let result = self.inner.spmv(input);
