...
impl NullModels {
    /// Random spectrum, sorted into an energy-like list.
    ...
    /// Same values as the source spectrum, but jittered and shuffled before sorting.
    /// This preserves rough range while breaking fine structure.
    ...
    /// Smooth non-fractal spectral control.
    ...
    /// Backward-compatible name for older examples.
    ...
    /// Random graph (Erdos-Renyi) with deterministic seed.
    ...
    /// A small hierarchical graph made of dense modules plus sparse bridges.
    ...
    /// A deterministic chain graph useful for box-covering tests.
    ...
    /// Damped alternating oscillator. It has a period-2 component, but should fail persistence.
    ...
    /// Seeded random signal control.
    pub fn random_signal(n: usize, seed: u64) -> Vec<f64> {
        let mut rng = StdRng::seed_from_u64(seed);
        (0..n).map(|_| rng.gen_range(-1.0..1.0)).collect()
    }

    /// Noisy period-2 signal for detector sanity tests.
