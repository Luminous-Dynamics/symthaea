...
impl CognitiveSubsystem for ReasoningManager {
    ...
    fn process(&mut self, snapshot: &CycleSnapshot) -> SubsystemOutput {
        ...
        // ── Resilience-Aware Self-Regulation ──────────────────────────
        // If system self-efficacy is low, throttle exploration and boost caution.
        let self_efficacy = snapshot.metacognitive_self_efficacy;
        if self_efficacy < 0.5 {
            output.lr_modulation *= 0.5;
            output.confidence_delta -= 0.1;
        }
...
mod tests {
    ...
    #[test]
    fn test_reasoning_self_regulation_throttle() {
        let mut rm = ReasoningManager::default();

        // Stable State: Self-efficacy = 0.8
        let snapshot_stable = CycleSnapshot {
            prediction_confidence: 0.9,
            epistemic_confidence: 0.8,
            unified_psi: 0.9,
            metacognitive_self_efficacy: 0.8,
            ..Default::default()
        };
        let output_stable = rm.process(&snapshot_stable);

        // Unstable State: Self-efficacy = 0.2
        let snapshot_unstable = CycleSnapshot {
            prediction_confidence: 0.9,
            epistemic_confidence: 0.8,
            unified_psi: 0.9,
            metacognitive_self_efficacy: 0.2,
            ..Default::default()
        };
        let output_unstable = rm.process(&snapshot_unstable);

        // Assert: Unstable state should trigger learning rate throttle
        assert!(output_unstable.lr_modulation < output_stable.lr_modulation);
        // Assert: Unstable state should increase exploration
        assert!(output_unstable.exploration_delta > output_stable.exploration_delta);
    }
}
