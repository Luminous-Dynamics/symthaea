...
// ============================================================================
// NSM PRIMITIVE GROUNDING FOR ALLEN'S TEMPORAL RELATIONS
// ============================================================================

/// NSM primitive grounding for Allen's 13 temporal relations.
///
/// Each Allen relation is decomposed into Natural Semantic Metalanguage (NSM)
/// primitives, enabling:
/// - Cross-domain temporal reasoning via shared primitive structure
/// - Semantic similarity between temporal relations
/// - Compositional temporal inference grounded in universal concepts
///
/// ## Temporal NSM Primitives Used
///
/// - BEFORE, AFTER: Core temporal ordering
/// - AT_THE_SAME_TIME: Simultaneity/equality
/// - TOUCH: Temporal adjacency (no gap)
/// - INSIDE: Temporal containment
/// - PART: Partial overlap relationships
...
pub(crate) struct AllenRelationPrimitiveGrounding {
    ...
    /// NSM primitives composing this temporal relation
    ...
    /// Degree of overlap: 0.0 (none) to 1.0 (complete)
    pub overlap_degree: f32,

    /// Whether this relation is causal (precedes/meets can be causal)
    ...
    pub(crate) fn for_relation(
        ...
        let (primitives, directionality, overlap_degree, is_causal) = match relation {
            ...
            AllenRelation::Precedes => (
                vec!["NSM_BEFORE", "NSM_NOT", "NSM_TOUCH", "NSM_FOR_SOME_TIME"],
                -1.0,
                0.0,
                true,
            ),
            ...
            AllenRelation::Meets => (
                vec!["NSM_BEFORE", "NSM_TOUCH", "NSM_MOMENT"],
                -1.0,
                0.0,
                true,
            ),
            ...
            AllenRelation::Overlaps => (
                vec!["NSM_BEFORE", "NSM_PART", "NSM_AFTER", "NSM_FOR_SOME_TIME"],
                -0.5,
                0.5,
                true,
            ),
            ...
            AllenRelation::Starts => (
                vec!["NSM_AT_THE_SAME_TIME", "NSM_BEFORE", "NSM_PART"],
                0.0,
                0.5,
                false,
            ),
            ...
            AllenRelation::During => (
                vec!["NSM_INSIDE", "NSM_AFTER", "NSM_BEFORE", "NSM_PART"],
                0.0,
                1.0,
                false,
            ),
            ...
            AllenRelation::Finishes => (
                vec!["NSM_AT_THE_SAME_TIME", "NSM_AFTER", "NSM_PART"],
                0.0,
                0.5,
                false,
            ),
            ...
            AllenRelation::Equals => (
                vec!["NSM_AT_THE_SAME_TIME", "NSM_SAME", "NSM_WHEN"],
                0.0,
                1.0,
                false,
            ),
            ...
            AllenRelation::PrecededBy => (
                vec!["NSM_AFTER", "NSM_NOT", "NSM_TOUCH", "NSM_FOR_SOME_TIME"],
                1.0,
                0.0,
                false,
            ),
            ...
            AllenRelation::MetBy => (
                vec!["NSM_AFTER", "NSM_TOUCH", "NSM_MOMENT"],
                1.0,
                0.0,
                false,
            ),
            ...
            AllenRelation::OverlappedBy => (
                vec!["NSM_AFTER", "NSM_PART", "NSM_BEFORE", "NSM_FOR_SOME_TIME"],
                0.5,
                0.5,
                false,
            ),
            ...
            AllenRelation::StartedBy => (
                vec!["NSM_AT_THE_SAME_TIME", "NSM_AFTER", "NSM_PART", "NSM_OTHER"],
                0.0,
                0.5,
                false,
            ),
            ...
            AllenRelation::Contains => (
                vec!["NSM_BEFORE", "NSM_AFTER", "NSM_NOT", "NSM_INSIDE"],
                0.0,
                1.0,
                false,
            ),
...
impl TemporalInterval {
    ...
    pub fn new(id: impl Into<String>, start: f64, end: f64) -> Result<Self> {
        ...
        let seed = (id_str
            ...
            .wrapping_mul(1000) as f64
            + midpoint * 100.0
            + duration * 10.0) as u64;
...
impl AllenRelation {
    ...
    pub(crate) fn primitive_grounding(
        ...
        AllenRelationPrimitiveGrounding::for_relation(*self, primitive_system)
    }

    /// Check if this relation can represent causal precedence
    ///
    /// Only Precedes, Meets, and Overlaps can represent A causing B,
    /// as the cause must temporally precede or coincide with the effect.
    pub fn is_causal_relation(&self) -> bool {
        matches!(self, Self::Precedes | Self::Meets | Self::Overlaps)
    }

    /// Get relations that are semantically similar (share NSM primitives)
    pub fn semantic_neighbors(&self) -> Vec<Self> {
        match self {
            // Precedes and Meets share BEFORE
            Self::Precedes => vec![Self::Meets, Self::Overlaps],
            Self::Meets => vec![Self::Precedes, Self::Overlaps],

            // Overlaps bridges before and during
            Self::Overlaps => vec![Self::Meets, Self::Starts, Self::During],

            // Starts, During, Finishes share containment semantics
            Self::Starts => vec![Self::Overlaps, Self::During, Self::Equals],
            Self::During => vec![Self::Starts, Self::Finishes],
            Self::Finishes => vec![Self::During, Self::Equals, Self::OverlappedBy],
            ...
            // Inverses mirror their base relations
            Self::PrecededBy => vec![Self::MetBy, Self::OverlappedBy],
            Self::MetBy => vec![Self::PrecededBy, Self::OverlappedBy],
            Self::OverlappedBy => vec![Self::MetBy, Self::Finishes, Self::Contains],
            Self::StartedBy => vec![Self::Equals, Self::Contains],
            Self::Contains => vec![Self::StartedBy, Self::FinishedBy],
            Self::FinishedBy => vec![Self::Contains, Self::Equals],
        }
    }
}
...
impl TemporalReasoner {
    ...
    fn init_composition_table(&mut self) {
        ...
        self.add_composition(
            Precedes,
            During,
            vec![Precedes, Meets, Overlaps, Starts, During],
        );
        self.add_composition(
            Precedes,
            Finishes,
        ...
        self.add_composition(
            Contains,
            Precedes,
            vec![Precedes, Meets, Overlaps, StartedBy, Contains],
        );
    ...
    pub fn compute_relation(&self, a: &TemporalInterval, b: &TemporalInterval) -> AllenRelation {
        ...
        if a_end < b_start - self.config.meet_tolerance {
            ...
            AllenRelation::Precedes
        } else if approx_eq(a_end, b_start)
            || (a_end >= b_start - self.config.meet_tolerance
                && a_end <= b_start + self.config.meet_tolerance
                && a_end < b_end
                && a_start < b_start)
        {
        ...
        // Inverse relations (when A comes "after" B in various ways)
            ...
            AllenRelation::PrecededBy
        } else if approx_eq(b_end, a_start)
            || (b_end >= a_start - self.config.meet_tolerance
                && b_end <= a_start + self.config.meet_tolerance
                && b_end < a_end
                && b_start < a_start)
        {
    ...
    pub fn get_relation(&mut self, id_a: &str, id_b: &str) -> Result<AllenRelation> {
        ...
        self.relations
            .insert((id_b.to_string(), id_a.to_string()), relation.inverse());
    ...
    /// Calculate semantic similarity between two Allen relations via NSM primitives
    ///
    /// This uses the NSM grounding to find how semantically related two temporal
    /// relations are. For example, Precedes and Meets are highly similar (both
    /// involve BEFORE), while Precedes and Contains are less similar.
    pub fn relation_similarity(
        ...
        let g1 = AllenRelationPrimitiveGrounding::for_relation(r1, primitive_system);
        let g2 = AllenRelationPrimitiveGrounding::for_relation(r2, primitive_system);
        g1.similarity(&g2)
    }

    /// Find the Allen relations most similar to a target relation
    pub fn find_similar_relations(
        &self,
        target: AllenRelation,
        primitive_system: &PrimitiveSystem,
        top_k: usize,
    ) -> Vec<(AllenRelation, f32)> {
        let target_grounding =
            AllenRelationPrimitiveGrounding::for_relation(target, primitive_system);

        let mut similarities: Vec<(AllenRelation, f32)> = AllenRelation::all()
            .into_iter()
            .filter(|r| *r != target)
            .map(|r| {
                let grounding = AllenRelationPrimitiveGrounding::for_relation(r, primitive_system);
                (r, target_grounding.similarity(&grounding))
            })
            .collect();
    ...
    pub(crate) fn causal_relations(
        ...
        AllenRelation::all()
            .into_iter()
            .map(|r| AllenRelationPrimitiveGrounding::for_relation(r, primitive_system))
            .filter(|g| g.is_causal_candidate)
            .collect()
    }

    /// Encode a temporal relation using its NSM primitive grounding
    pub fn encode_relation_grounded(
        &self,
        relation: AllenRelation,
        primitive_system: &PrimitiveSystem,
    ) -> BinaryHV {
...
impl ConsciousnessTemporalAnalyzer {
    ...
    pub fn find_potential_causes(
        &mut self,
        target_id: &str,
    ) -> Result<Vec<(String, AllenRelation)>> {
        ...
        for interval in &self.conscious_intervals {
            ...
            let relation = self
                .reasoner
                .get_relation(&interval.interval.id, target_id)?;
    ...
    pub fn find_effects(&mut self, source_id: &str) -> Result<Vec<(String, AllenRelation)>> {
        ...
        for interval in &self.conscious_intervals {
            ...
            let relation = self
                .reasoner
                .get_relation(source_id, &interval.interval.id)?;
        ...
        for start in &sorted {
            ...
            for candidate in &sorted {
                ...
                if let Ok(relation) = self
                    .reasoner
                    .get_relation(&current.interval.id, &candidate.interval.id)
                {
                    match relation {
    ...
    pub fn analyze_continuity(&mut self) -> ContinuityAnalysis {
        ...
        ContinuityAnalysis {
            ...
            average_gap: if gaps.is_empty() {
                ...
                total_gap_time / gaps.len() as f64
            },
...
impl TemporalPrimitiveSystem {
    ...
    /// Create a temporal primitive system with NSM grounding
    ///
    ...
    pub fn with_nsm_grounding(primitive_system: &PrimitiveSystem) -> Self {
        ...
        let all_groundings = AllenRelationPrimitiveGrounding::all_groundings(primitive_system);
        ...
        // Create patterns with NSM-grounded encodings
        let patterns = Self::create_nsm_grounded_patterns(primitive_system);
    ...
    /// Create temporal patterns with NSM grounding
    fn create_nsm_grounded_patterns(primitive_system: &PrimitiveSystem) -> Vec<TemporalPattern> {
        let precedes = AllenRelationPrimitiveGrounding::for_relation(
            AllenRelation::Precedes,
            primitive_system,
        );
        let meets =
            AllenRelationPrimitiveGrounding::for_relation(AllenRelation::Meets, primitive_system);
        let overlaps = AllenRelationPrimitiveGrounding::for_relation(
            AllenRelation::Overlaps,
            primitive_system,
        );
        let contains = AllenRelationPrimitiveGrounding::for_relation(
            AllenRelation::Contains,
            primitive_system,
        );
        let equals =
        ...
        let starts =
            AllenRelationPrimitiveGrounding::for_relation(AllenRelation::Starts, primitive_system);
        let during =
            AllenRelationPrimitiveGrounding::for_relation(AllenRelation::During, primitive_system);
        ...
        vec![
            TemporalPattern {
                ...
                encoding: precedes
                    .primitive_encoding
                    .bind(&precedes.primitive_encoding),
            },
            TemporalPattern {
                name: "Overlap_Chain".to_string(),
                relations: vec![AllenRelation::Overlaps, AllenRelation::Overlaps],
                ...
                encoding: overlaps
                    .primitive_encoding
                    .bind(&overlaps.primitive_encoding),
            },
            TemporalPattern {
                name: "Containment".to_string(),
                relations: vec![AllenRelation::Contains],
    ...
    pub(crate) fn get_nsm_grounding(
        ...
        AllenRelationPrimitiveGrounding::for_relation(relation, primitive_system)
    }

    /// Calculate similarity between two relations using NSM primitives
    pub fn nsm_relation_similarity(
        &self,
        r1: AllenRelation,
        r2: AllenRelation,
        primitive_system: &PrimitiveSystem,
    ...
    /// Display all Allen relations with their NSM groundings
    pub fn display_nsm_groundings(&self, primitive_system: &PrimitiveSystem) -> String {
        let mut output = String::new();
        output.push_str("Allen's 13 Temporal Relations (NSM Grounded)\n");
        output.push_str("═══════════════════════════════════════════\n\n");
        ...
        for relation in AllenRelation::all() {
            ...
            output.push_str(&format!(
                "{:12?} = {}\n",
                relation,
                grounding.semantic_formula()
            ));
            output.push_str(&format!(
                "             dir={:+.1}, overlap={:.1}, causal={}\n\n",
                grounding.directionality, grounding.overlap_degree, grounding.is_causal_candidate
            ));
        }
...
mod tests {
    ...
    #[test]
    fn test_allen_relation_nsm_grounding() {
        use crate::hdc::primitive_system::PrimitiveSystem;

        let primitive_system = PrimitiveSystem::new();

        // Test that all 13 relations get grounded
        let all_groundings = AllenRelationPrimitiveGrounding::all_groundings(&primitive_system);
        assert_eq!(all_groundings.len(), 13);

        // Test Precedes grounding
        let precedes = AllenRelationPrimitiveGrounding::for_relation(
            AllenRelation::Precedes,
            &primitive_system,
        );
        assert!(precedes.nsm_primitives.contains(&"NSM_BEFORE".to_string()));
        assert!(precedes.nsm_primitives.contains(&"NSM_NOT".to_string()));
        assert!(precedes.nsm_primitives.contains(&"NSM_TOUCH".to_string()));
        assert!(precedes.is_causal_candidate);
        assert_eq!(precedes.directionality, -1.0);
        assert_eq!(precedes.overlap_degree, 0.0);

        // Test Meets grounding
        let meets =
            AllenRelationPrimitiveGrounding::for_relation(AllenRelation::Meets, &primitive_system);
        assert!(meets.nsm_primitives.contains(&"NSM_BEFORE".to_string()));
        assert!(meets.nsm_primitives.contains(&"NSM_TOUCH".to_string()));
        assert!(meets.is_causal_candidate);
    ...
    #[test]
    fn test_allen_relation_nsm_similarity() {
        use crate::hdc::primitive_system::PrimitiveSystem;
        ...
        // Precedes and Meets should be similar (both use BEFORE)
        let precedes_meets_sim = precedes.similarity(&meets);

        // Precedes and Equals should be less similar (different temporal semantics)
        let precedes_equals_sim = precedes.similarity(&equals);

        println!("Precedes <-> Meets: {:.3}", precedes_meets_sim);
        println!("Precedes <-> Equals: {:.3}", precedes_equals_sim);

        // Precedes should be more similar to Meets than to Equals
        assert!(
            precedes_meets_sim > precedes_equals_sim,
            "Precedes should be more similar to Meets than Equals"
        );
    }

    #[test]
    fn test_allen_relation_inverse_nsm_symmetry() {
        use crate::hdc::primitive_system::PrimitiveSystem;

        let primitive_system = PrimitiveSystem::new();

        // Test that inverse relations have symmetric directionality
        let precedes = AllenRelationPrimitiveGrounding::for_relation(
            AllenRelation::Precedes,
            &primitive_system,
        );
        let preceded_by = AllenRelationPrimitiveGrounding::for_relation(
            AllenRelation::PrecededBy,
            &primitive_system,
        );

        // Directionality should be opposite
        assert_eq!(precedes.directionality, -1.0);
        assert_eq!(preceded_by.directionality, 1.0);

        // Overlap degree should be the same
        assert_eq!(precedes.overlap_degree, preceded_by.overlap_degree);
    }

    #[test]
    fn test_allen_relation_causal_candidates() {
        use crate::hdc::primitive_system::PrimitiveSystem;

        let primitive_system = PrimitiveSystem::new();
        let reasoner = TemporalReasoner::default();

        let causal = reasoner.causal_relations(&primitive_system);

        // Should have exactly 3 causal candidate relations
        assert_eq!(causal.len(), 3);

        let causal_relations: Vec<_> = causal.iter().map(|g| g.relation).collect();
        assert!(causal_relations.contains(&AllenRelation::Precedes));
        assert!(causal_relations.contains(&AllenRelation::Meets));
        assert!(causal_relations.contains(&AllenRelation::Overlaps));
    }

    #[test]
    fn test_allen_relation_semantic_neighbors() {
        // Precedes and Meets should be neighbors
        let precedes_neighbors = AllenRelation::Precedes.semantic_neighbors();
        assert!(precedes_neighbors.contains(&AllenRelation::Meets));

        // Equals should have Start/Finish related neighbors
        let equals_neighbors = AllenRelation::Equals.semantic_neighbors();
        assert!(equals_neighbors.contains(&AllenRelation::Starts));
        assert!(equals_neighbors.contains(&AllenRelation::Finishes));
    }

    #[test]
    fn test_temporal_primitive_system_nsm_grounding() {
        use crate::hdc::primitive_system::PrimitiveSystem;

        let primitive_system = PrimitiveSystem::new();
        let temporal_system = TemporalPrimitiveSystem::with_nsm_grounding(&primitive_system);

        // All 13 relations should be encoded
        assert_eq!(temporal_system.relation_primitives.len(), 13);

        // Patterns should exist
        assert!(!temporal_system.patterns.is_empty());

        // Test NSM similarity calculation
        let sim = temporal_system.nsm_relation_similarity(
            AllenRelation::Precedes,
            AllenRelation::Meets,
            &primitive_system,
        );
    ...
    #[test]
    fn test_reasoner_find_similar_relations() {
        use crate::hdc::primitive_system::PrimitiveSystem;
        ...
        let similar =
            reasoner.find_similar_relations(AllenRelation::Precedes, &primitive_system, 3);

        assert_eq!(similar.len(), 3);

        // The most similar to Precedes should include Meets (shares BEFORE)
        let relation_names: Vec<_> = similar.iter().map(|(r, _)| *r).collect();
        println!("Top 3 similar to Precedes: {:?}", relation_names);
    }

    #[test]
    fn test_display_nsm_groundings() {
        use crate::hdc::primitive_system::PrimitiveSystem;

        let primitive_system = PrimitiveSystem::new();
        let temporal_system = TemporalPrimitiveSystem::new();

        let display = temporal_system.display_nsm_groundings(&primitive_system);

        // Should contain all relation names
        assert!(display.contains("Precedes"));
        assert!(display.contains("Meets"));
        assert!(display.contains("Equals"));
        assert!(display.contains("Contains"));

        // Should contain NSM primitives
