...
\addplot[blue!70!black, thick, smooth, no markers, domain=0:1000, samples=60]
  {0.9 - 0.7*exp(-x/200)};
\draw[red!50, dashed, thin] (axis cs:0,0.9) -- (axis cs:1000,0.9)
  node[right, font=\tiny, red!50!black] {0.9};
\end{axis}
% --- Anxious (top-right) ---
\begin{axis}[
  name=anxious,
...
\addplot[blue!70!black, thick, smooth, no markers, domain=0:1000, samples=80]
  {0.5 + 0.2*sin(deg(x/40)) * exp(-x/800)};
\draw[red!50, dashed, thin] (axis cs:0,0.5) -- (axis cs:1000,0.5)
  node[right, font=\tiny, red!50!black] {0.5};
\end{axis}
...
\addplot[blue!70!black, thick, smooth, no markers, domain=0:1000, samples=60]
  {0.2 + 0.5*exp(-x/150)};
\draw[red!50, dashed, thin] (axis cs:0,0.2) -- (axis cs:1000,0.2)
  node[right, font=\tiny, red!50!black] {0.2};
\end{axis}
...
\addplot[blue!70!black, thick, smooth, no markers, domain=0:1000, samples=100]
  {0.3 + 0.25*sin(deg(x/25)) * exp(-x/600) + 0.1*sin(deg(x/7))};
\draw[red!50, dashed, thin] (axis cs:0,0.3) -- (axis cs:1000,0.3)
  node[right, font=\tiny, red!50!black] {0.3};
\end{axis}
\end{tikzpicture}
...
attachment ($\sim$0.9); inconsistent caregiving produces anxious oscillation
($\sim$0.5); unresponsive caregiving decays to avoidant ($\sim$0.2);
frightening caregiving produces chaotic disorganized patterns ($\sim$0.3).
Dashed lines indicate equilibrium attractors.}
...
\end{axis}
\end{tikzpicture}
\caption{\textbf{Heterozygosity decay under neutral evolution.} Theoretical
decay $H(t) = H(0)(1 - 1/2N_e)^t$ with $H(0) = 0.7$ for three effective
population sizes (Eq.~\ref{eq:het_decay}). At $N_e = 50$ (Franklin's
\cite{franklin1980evolutionary} minimum), heterozygosity drops below the
critical threshold within $\sim$35 generations. Active management via
HDC-enhanced breeding (Section~\ref{sec:population}) slows this decline.}
\label{fig:het_decay}
\end{figure}
