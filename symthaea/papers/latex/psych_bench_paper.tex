...
\paragraph{Cognitive Profile.} Figure~\ref{fig:radar} shows the 8-domain cognitive profile generated from the full battery. The \hlb{} achieves a grand mean z-score of $+0.745$ across 16 cognitive domains, with 13/16 domains above the human mean and 0 meaningfully below. Social cognition achieves the highest domain z-score ($+3.11$), driven by similarity-based Theory-of-Mind tests where holographic pattern matching maps naturally to the task structure. Language ($z = +2.88$) and SustainedAttention ($z = +2.50$) also perform strongly, reflecting improvements in sequential processing and vigilance. Reasoning ($z = +0.17$) and Executive ($z = -0.03$) domains show human-typical performance, while Inhibition ($z = -0.15$) and Metacognition ($z = -0.10$) are the weakest domains, though both remain within the human-average band.
...
\emph{Domain-specific strengths and weaknesses.} The cognitive profile reveals a broadly above-human profile (grand mean $z = +0.745$, 13/16 domains above human mean). Social cognition achieves the highest z-score ($+3.11$), which may reflect that the Theory-of-Mind benchmarks (false belief, hinting, Reading the Mind in the Eyes) rely on similarity-based discrimination that maps naturally to HDC operations, potentially inflating scores relative to genuine social-cognitive competence. Language ($z = +2.88$) and SustainedAttention ($z = +2.50$) also score well above human norms. The weakest domains---Inhibition ($z = -0.15$) and Metacognition ($z = -0.10$)---remain within the human-average band ($|z| < 1$). These asymmetries are informative---they demonstrate that the profile genuinely captures architectural biases---but they warrant caution when comparing across domains with different measurement characteristics.
