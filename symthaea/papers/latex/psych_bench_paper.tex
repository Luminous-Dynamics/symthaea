...
\begin{abstract}
\noindent
We present \psychbench{}, a psychological benchmark suite comprising 98 benchmarks across 20 cognitive domains for evaluating Holographic Liquid Brain (\hlb{}) architectures---systems combining Hyperdimensional Computing, Liquid Time-Constant networks, Integrated Information Theory, and Active Inference. Existing AI evaluation frameworks lack psychometric infrastructure (HELM), cover narrow domains (ARC), omit normative human comparison (BIG-Bench), or ignore neuromodulatory effects (CogBench). \psychbench{} addresses all four gaps with: (1)~implementations of established paradigms (Stroop, WCST, N-back, and 89 others) with psychometric assessment infrastructure including ICC reliability, speed-accuracy tradeoff curves, and adaptive staircases; (2)~normative anchoring against 90+ published human baselines enabling z-score and clinical-band interpretation; (3)~a 17-benchmark neuromodulator evaluation framework testing dose-response curves, tolerance/withdrawal dynamics, behavioral knockouts, and multi-transmitter synergy; and (4)~a 14-indicator consciousness evaluation derived from six theories. All stimuli share a unified holographic encoding via five HDC adapter types, enabling cross-modal comparison. Demonstration on the \symthaea{} \hlb{} reveals a grand mean z-score of $+0.961$ across 20 cognitive domains (15/20 above human mean, 0 below), with particular strengths in Speech ($z = +3.93$), Consciousness ($z = +3.90$), Motor ($z = +3.20$), Language ($z = +2.88$), and SustainedAttention ($z = +2.50$), and the psychometric analysis exposing important architectural properties: a gradient of reliability from Excellent to Poor (median ICC $= 0.23$, with 5 benchmarks reaching Moderate or better), selective sensitivity to working-memory and encoding-noise ablation, and accuracy-pressure tradeoffs in 4 of 8 tested benchmarks. Implemented in 15,000 lines of Rust with 862 unit tests, \psychbench{} is open-source and fully deterministic.
\end{abstract}
...
\psychbench{} addresses all four gaps simultaneously. We present a comprehensive cognitive battery of 98 benchmarks spanning 20 domains, designed specifically for evaluating \hlb{} and related neuroscience-inspired architectures. The suite is organized around five design principles: psychometric fidelity to source paradigms, unified holographic stimulus encoding, normative anchoring against published baselines, systematic ablation transparency, and self-contained reporting.

Our contributions are:
\begin{enumerate}[nosep]
    \item A 98-benchmark cognitive battery covering 8 profile domains (executive function, memory, attention, social cognition, motor learning, language, metacognition, reasoning) plus specialized sub-batteries for affect, creativity, inhibition, consciousness, speech, substrate independence, and game-theoretic social cognition.
...
\paragraph{Normative anchoring.} Every benchmark has at least one associated human baseline from the published literature, stored as a mean $\pm$ SD pair with citation. The \texttt{BaselineCollection} contains 90+ such entries spanning all 20 domains. Agent performance is converted to z-scores ($z = (x_{\text{agent}} - \mu_{\text{human}}) / \sigma_{\text{human}}$, negated for error metrics where lower is better) and mapped to percentiles via the normal cumulative distribution function. Clinical interpretation bands follow Cicchetti's~\citep{cicchetti1994guidelines} guidelines: Superior ($z > 2.0$), Above Average ($1.0 < z \leq 2.0$), Average ($-1.0 \leq z \leq 1.0$), Below Average ($-2.0 \leq z < -1.0$), and Impaired ($z < -2.0$).
...
\psychbench{} contains 98 benchmarks organized into 20 domains (Figure~\ref{fig:domain_matrix}). We describe each domain below; Table~\ref{tab:inventory} provides the complete inventory with paradigm sources and human baselines.
...
\begin{table}[p]
\centering
\caption{Complete benchmark inventory (representative selection). Each entry lists the domain, benchmark name, source paradigm, primary metric, and human baseline where available. Full inventory with all 98 benchmarks available in the software documentation.}
\label{tab:inventory}
\small
\begin{tabular}{@{}llllr@{}}
\toprule
...
Table~\ref{tab:psychometrics} reveals a range of reliability classes across the 19 benchmarks, with a median ICC of $0.23$. Using 30 seed families (subjects) with per-subject variation in working memory capacity ($K = 4$--$9$), HDC dimensionality ($d = 256$--$1024$), and encoding noise ($\sigma = 0.0$--$0.30$) creates meaningful between-subject variance. Two benchmarks achieve Excellent reliability: Digit Span (ICC $= 1.00$, perfectly determined by WM capacity) and Calibration (ICC $= 0.93$). Two reach Good reliability: WCST ($0.87$) and Stop Signal ($0.84$). SRTT reaches Moderate reliability ($0.70$), reflecting that motor sequence learning is sensitive to the per-subject noise/dimension parameters. The remaining 14 benchmarks fall in the Poor class, including 12 with positive ICCs (Flanker $0.29$, ARC-Fluid $0.28$, Two-Step $0.27$, Reward Learning $0.26$, N-back $0.23$, Visual Search $0.20$, Stroop $0.11$, Change Detection $0.03$) and 4 with negative ICCs (Reversal Learning $-0.37$, Lexical Decision $-0.27$, BART $-0.13$, IGT $-0.11$), indicating that within-subject variance exceeds between-subject variance for these benchmarks. The Limitations section discusses the implications of these reliability patterns for interpreting \psychbench{} as a psychometric instrument versus an engineering test suite.
...
\paragraph{Cognitive Profile.} Figure~\ref{fig:radar} shows the 8-domain cognitive profile generated from the full battery. The \hlb{} achieves a grand mean z-score of $+0.961$ across 20 cognitive domains, with 15/20 domains above the human mean and 5 below. Speech achieves the highest domain z-score ($+3.93$), followed by Consciousness ($z = +3.90$), Motor ($z = +3.20$), Language ($z = +2.88$), and SustainedAttention ($z = +2.50$). Social cognition ($z = +1.56$) remains strong, driven by similarity-based Theory-of-Mind tests and game-theoretic benchmarks where holographic pattern matching maps naturally to the task structure. Reasoning ($z = +0.17$) and Executive ($z = -0.03$) domains show human-typical performance. The five negative domains---Binding ($z = -0.83$), Attention ($z = -0.41$), Metacognition ($z = -0.31$), Inhibition ($z = -0.15$), and Executive ($z = -0.03$)---all remain within the human-average band ($|z| < 1$).
...
\midrule
\psychbench{} & \checkmark & \checkmark & \checkmark & \checkmark & 20 & Rust \\
...
\emph{Domain-specific strengths and weaknesses.} The cognitive profile reveals a broadly above-human profile (grand mean $z = +0.961$, 15/20 domains above human mean). Speech achieves the highest z-score ($+3.93$), followed by Consciousness ($+3.90$) and Motor ($+3.20$), which may reflect that these benchmarks rely on signal-processing and pattern-discrimination tasks that map naturally to HDC operations. Language ($z = +2.88$) and SustainedAttention ($z = +2.50$) also score well above human norms. Five domains fall below the human mean---Binding ($z = -0.83$), Attention ($z = -0.41$), Metacognition ($z = -0.31$), Inhibition ($z = -0.15$), and Executive ($z = -0.03$)---though all remain within the human-average band ($|z| < 1$). These asymmetries are informative---they demonstrate that the profile genuinely captures architectural biases---but they warrant caution when comparing across domains with different measurement characteristics.
...
\psychbench{} provides the first comprehensive cognitive benchmark suite designed for Holographic Liquid Brain architectures. With 98 benchmarks across 20 domains, psychometric assessment infrastructure, normative human comparison against 90+ published baselines, a 17-benchmark neuromodulator evaluation framework, and 14-indicator consciousness evaluation, the suite enables multi-dimensional characterization of neuroscience-inspired cognitive architectures. The demonstration on the \symthaea{} \hlb{} reveals both the suite's strengths and its current limitations: the architecture produces a characteristic cognitive profile with a grand mean z-score of $+0.961$ (15/20 domains above human mean) and particular strengths in Speech ($z = +3.93$), Consciousness ($z = +3.90$), Motor ($z = +3.20$), Language ($z = +2.88$), and SustainedAttention ($z = +2.50$) that is interpretable and reproducible, but the psychometric validation reveals a gradient of reliability (median ICC $= 0.23$, ranging from Excellent to Poor), modest ablation sensitivity, and some remaining normative scale mismatches requiring calibration. These findings are themselves informative---they identify precisely where the architecture and the evaluation framework need improvement. Implemented in 15,000 lines of Rust with 862 unit tests and full determinism, \psychbench{} is open-source and designed for iterative refinement by the growing community of researchers building brain-inspired cognitive architectures.
...
\psychbench{} is open-source under the MIT License. The complete source code, including all 98 benchmark implementations, harness infrastructure, and data generation scripts, is available at:
