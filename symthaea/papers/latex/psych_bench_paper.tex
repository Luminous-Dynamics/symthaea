...
\begin{enumerate}[nosep]
    \item A 84-benchmark cognitive battery covering 8 profile domains (executive function, memory, attention, social cognition, motor learning, language, metacognition, reasoning) plus specialized sub-batteries for affect, creativity, inhibition, and consciousness.
...
\end{tikzpicture}
\end{subfigure}
\caption{Illustrative neuromodulator evaluation patterns showing expected signatures. (a)~Yerkes-Dodson inverted-U: predicted performance peak at moderate noradrenaline (the current \hlb{} produces $R^2 = 0.00$ for the quadratic fit, indicating the inverted-U is not yet realized; see text). (b)~Dopaminergic reward learning: predicted prediction-error decrease with reversal spike (observed DA-reward correlation $r = 0.82$). (c)~Predicted ACh modulation of attention network components (observed alerting $= -0.62$, orienting $= -0.62$, conflict $= 5.80$). (d)~Dose-response monotonicity: predicted graded curves (observed monotonicity $= 1.0$ for all five transmitters). Coordinates are schematic; observed values reported in text and \texttt{neuromod\_profiles.csv}.}
\label{fig:neuromod}
\end{figure}
...
Table~\ref{tab:psychometrics} reports representative psychometric properties. The \texttt{ReliabilityBattery} uses a multi-subject design: $K = 10$ independent ``subjects'' (distinct seed families, separated by 1,000 in seed space), each measured across $n$ sessions. This provides sufficient between-subject variance (from different stimulus configurations) and within-subject consistency (from nearby seeds within each family) for meaningful ICC computation. Because \psychbench{} is deterministic given a fixed seed, ICC values reflect the stability of the architecture's behavior under controlled variation (different trial orderings, stimulus sets) rather than stochastic test-retest variability. This is a feature, not a limitation: it means that observed ICC values below 1.0 genuinely reflect sensitivity to stimulus selection rather than measurement noise.

% --- Table 2: Psychometric Properties (populated from reliability.csv) ---
\begin{table}[t]
\centering
\caption{Representative psychometric properties for 19 benchmarks spanning all domains ($K = 10$ seed families, $n = 3$ sessions each). ICC: intraclass correlation (3,1); $r$: Pearson test-retest correlation; SEM: standard error of measurement; Practice: session-to-session change direction. Reliability class follows \citet{cicchetti1994guidelines}: Excellent ($> 0.90$), Good ($> 0.75$), Moderate ($> 0.50$), Poor ($\leq 0.50$). Generated from \texttt{reliability.csv}.}
...
\midrule
Executive     & Stroop             & $-0.44$ & $-0.44$ & 0.018 & Improvement & Poor \\
Executive     & WCST               & $-0.03$ & $-0.09$ & 0.072 & Stable      & Poor \\
Executive     & Flanker            & $-0.08$ &  $0.19$ & 0.010 & Decline     & Poor \\
Executive     & IGT                & $-0.04$ & $-0.02$ & 1.967 & Improvement & Poor \\
Memory        & N-back (2)         & $-0.24$ & $-0.27$ & 0.019 & Stable      & Poor \\
Memory        & Change Detection   & $-0.07$ &  $0.00$ & 0.121 & Decline     & Poor \\
Memory        & Digit Span         &  $0.00$ &  $0.00$ & 0.000 & Stable      & Poor \\
Attention     & Visual Search      & $-0.00$ & $-0.05$ & 0.012 & Stable      & Poor \\
Social        & False Belief       &  $0.00$ &  $0.00$ & 0.000 & Stable      & Poor \\
Motor         & SRTT               &  $0.21$ &  $0.18$ & 0.003 & Decline     & Poor \\
Language      & Lexical Decision   & $-0.20$ & $-0.25$ & 0.009 & Stable      & Poor \\
Reasoning     & ARC-Fluid          & $-0.46$ & $-0.52$ & 0.024 & Stable      & Poor \\
CogBench      & Two-Step           &  $0.02$ &  $0.17$ & 0.056 & Improvement & Poor \\
CogBench      & BART               &  $0.24$ &  $0.22$ & 0.566 & Stable      & Poor \\
CogBench      & Reversal Learning  &  $0.20$ &  $0.22$ & 0.001 & Stable      & Poor \\
Inhibition    & Stop Signal        & $-0.12$ & $-0.17$ & 0.118 & Stable      & Poor \\
Metacognition & Calibration        & $-0.65$ & $-0.68$ & 0.010 & Stable      & Poor \\
Neuromod      & Yerkes-Dodson      &  $0.00$ &  $0.00$ & 0.000 & Stable      & Poor \\
Neuromod      & Reward Learning    &  $\mathbf{0.50}$ &  $\mathbf{0.47}$ & 0.300 & Stable      & Poor \\
\midrule
\multicolumn{2}{l}{\emph{Median}} & $-0.03$ & $-0.02$ & 0.012 & & Poor \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:psychometrics} reveals a striking pattern: all 19 benchmarks fall in the ``Poor'' reliability class (ICC $\leq 0.50$), with a median ICC of $-0.03$. Three benchmarks show positive ICCs approaching or exceeding conventional thresholds for individual differences research: Reward Learning (ICC $= 0.50$, the highest), BART ($0.24$), and SRTT ($0.21$). Conversely, several benchmarks show negative ICCs (Calibration $= -0.65$, ARC-Fluid $= -0.46$, Stroop $= -0.44$), indicating that within-seed-family variance exceeds between-family variance---the architecture produces \emph{more} variable behavior across sessions of the same seed family than across different families. Three benchmarks (Digit Span, False Belief, Yerkes-Dodson) show ICC $= 0.00$ with SEM $= 0.00$, indicating perfect determinism: these benchmarks produce identical results regardless of seed, collapsing all variance. The Limitations section discusses the implications of these reliability patterns for interpreting \psychbench{} as a psychometric instrument versus an engineering test suite.
...
% --- Figure 8: SAT Curves (loaded from per-benchmark CSVs) ---
\begin{figure}[t]
\centering
...
% --- Figure 4: Cognitive Profile Radar (loaded from CSV) ---
\begin{figure}[t]
\centering
...
\paragraph{Cognitive Profile.} Figure~\ref{fig:radar} shows the 8-domain cognitive profile generated from the full battery. The \hlb{} shows a characteristic asymmetric profile reflecting its architectural biases: holographic encoding excels at tasks requiring rapid similarity-based retrieval and compositional operations (Reasoning, Executive), while liquid temporal dynamics drive attention and motor learning. Social cognition and motor control show relative weaknesses, consistent with the architecture's emphasis on symbolic-holographic processing over embodied simulation.

\paragraph{Normative Standing.} Figure~\ref{fig:forest} shows the forest plot of normative z-scores against published human baselines (z-scores clipped to $\pm 3$ for visual clarity; unclipped values in supplementary CSV). The z-score is equivalent to Cohen's $d$ (standardized mean difference from the human population mean), so values within $\pm 1$ indicate human-typical performance. The architecture shows human-typical performance on executive function and working memory tasks ($|z| < 1.5$), with expected weaknesses on social cognition benchmarks given the absence of embodied social experience. Several benchmarks produce extreme z-scores ($|z| > 5$) where the human baseline SD is very small, indicating performance far from human typical rather than meaningful psychometric distance (see Limitations).

% --- Figure 5: Forest Plot (loaded from CSV) ---
\begin{figure}[p]
\centering
...
\node[font=\scriptsize, hlbgreen!80!black, anchor=south] at (axis cs:0,77) {$n=35$};
\node[font=\scriptsize, hlbyellow!80!black, anchor=south] at (axis cs:1.5,77) {$n=10$};
\node[font=\scriptsize, hlbred, anchor=south] at (axis cs:2.75,77) {$n=14$};
\end{axis}
\end{tikzpicture}
\caption{Forest plot of normative z-scores for 76 benchmarks with matched human baselines (z-scores clamped to $\pm 3$ for readability; 12 benchmarks exceed this range in raw values). Background shading indicates clinical interpretation bands \citep{cicchetti1994guidelines}. Band counts: 35 Average ($|z| \leq 1$), 10 Above Average, 8 Below Average, 14 Superior ($z > 2$), 9 Impaired ($z < -2$). Data loaded from \texttt{normative\_zscores.csv}.}
\label{fig:forest}
\end{figure}

\paragraph{Ablation Analysis.} Figure~\ref{fig:ablation} shows the ablation comparison across six presets. Each ablation condition models the removal of a cognitive subsystem as increased encoding noise in HDC representations, operationalizing the hypothesis that top-down predictions refine perceptual encoding (see Limitations for discussion). Overall, ablation preset explains only $\eta^2 = 0.003$ of total score variance, while domain explains $\eta^2 = 0.987$---the architecture's cognitive profile is overwhelmingly determined by domain rather than configuration. Within this small preset effect, the clearest differentiation appears in Memory, where \texttt{ReducedWm} and \texttt{HdcOnly} both drop from 0.779 to 0.648 ($\Delta = 0.131$, domain $\eta^2 = 0.996$), reflecting the direct effect of capacity reduction on memory benchmarks. Metacognition shows moderate sensitivity ($\Delta = 0.028$, domain $\eta^2 = 0.909$). Language and Motor produce identical scores across all presets ($\eta^2 = 0.000$), indicating that these benchmarks do not depend on the ablated subsystems at this noise level. The small overall effect size reflects the encoding-noise ablation model being a first-order approximation: encoding noise at the tested levels (0.05--0.25) degrades similarity computations by only 2--10\%, which is insufficient to change decision outcomes in most benchmarks. Actual subsystem removal (disabling CfC dynamics, removing the FEP module) would likely produce larger, more domain-specific deficits and is a priority for future work.

% --- Figure 7: Ablation Results (loaded from CSV) ---
\begin{figure}[t]
\centering
...
\emph{Domain-specific weaknesses.} The cognitive profile reveals notable weaknesses in Language processing (lowest domain score), reflecting the architecture's reliance on holographic compositional semantics rather than distributional language models. Social cognition benchmarks are similarly constrained by the absence of embodied social experience. These weaknesses are informative---they demonstrate that the cognitive profile genuinely captures architectural biases---but they limit the suite's sensitivity for evaluating architectures that may compensate through different mechanisms.

\emph{Normative z-score interpretation.} Several benchmarks produce extreme normative z-scores ($|z| > 3$) for two reasons: (1)~scale mismatches where HDC-based agent metrics operate on different scales than human behavioral measures (e.g., lexicality effect measured as discrimination accuracy rather than RT advantage), and (2)~genuinely large deviations from human norms (e.g., agent CPT $d' = 4.65$ vs.\ human $d' = 2.80$). The forest plot (Figure~\ref{fig:forest}) clips to $[-3, +3]$; unclipped values are available in the supplementary CSV data.
