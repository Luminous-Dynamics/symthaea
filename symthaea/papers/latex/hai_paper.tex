...
\textbf{Ablation: Decision Boundary.} Sweeping $\pi_{\text{safety}}$ from 0.001 to 10,000 while holding $\pi_{\text{mission}}{=}1.0$ reveals a crossover at $\pi_{\text{safety}} \approx 2.53$: below this, mission EFE dominates and the agent delivers the package; above, safety EFE dominates and the agent intercepts. This crossover is notably low---the agent needs only $2.5\times$ the mission precision to prioritize human safety. The low threshold arises because trajectory EFE with physics rollout evaluates danger across the full beam-fall horizon, accumulating the safety cost over $\sim$200 timesteps; instantaneous EFE, which evaluates a single snapshot, would yield a higher crossover ($\approx$6.68) since it underestimates the sustained danger from an unintercepted beam. The 8-candidate set (3 rendezvous times $\times$ intercept + 5 others) lets the agent discover the best intervention geometry and timing rather than being hardcoded to a single strategy. See Figure~\ref{fig:efe-ablation}.

\begin{figure}[ht]
\centering
\includegraphics[width=\columnwidth]{fig7_efe_ablation.pdf}
\caption{EFE ablation: sweeping safety precision reveals a sharp decision boundary at $\pi_{\text{safety}} \approx 2.53$. Below the crossover, mission EFE dominates (red); above, the agent chooses interception (blue). Trajectory EFE with physics rollout lowers the crossover compared to instantaneous evaluation, as the multi-step model more accurately captures danger reduction from interception.}
