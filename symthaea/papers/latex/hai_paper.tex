...
\begin{flushleft}
{\Large\textbf\newline{Hyperdimensional Active Inference: Free Energy Principle in Vector Symbolic Architectures}}
\newline
\\
...
Following Butlin et al.'s framework~\cite{butlin2023consciousness}, we evaluated Symthaea against 14 consciousness indicators drawn from 6 leading theories of consciousness.  Table~\ref{tab:butlin} summarizes the results: 12 indicators are \textsc{Present}, 2 are \textsc{Partial}, and 0 are \textsc{Absent}.
...
The eight motor command types provide an interpretable action vocabulary for cognitive systems: attention, learning rate, and exploration as \emph{cognitive actions}, with direct mapping from variational inference to executable commands. The full cognitive loop integrates surprise-driven exploration, prefrontal executive control (capacity-7 buffer), social coherence (Theory of Mind), reasoning engine feedback, counterfactual dream replay~\citep{diekelmann2010memory}, and adaptive urgency throttling (Critical/Normal/Cruise tiers), operating at ${\sim}4$~Hz sustained throughput.

\textbf{Closed feedback loops.} 16 subsystems form closed loops where previous-cycle outputs modulate current-cycle processing, including prefrontal veto~\citep{miller2001integrative}, predictive self-model confidence~\citep{clark2013whatever}, attention schema~\citep{graziano2013consciousness}, GWT broadcast~\citep{baars1988cognitive}, embodied phi modulation~\citep{damasio1999feeling}, resonance frequency modulation of CfC time constants~\citep{buzsaki2006rhythms}, MCE consciousness-level learning rate boost~\citep{dehaene2014consciousness}, and cross-modal binding. Single-module ablation shows $\leq 5\%$ degradation; multi-module synergy stays within $30\%$ of baseline with $<2\%$ computational overhead.
