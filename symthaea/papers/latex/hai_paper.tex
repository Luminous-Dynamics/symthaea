...
\midrule
Avg vowel error (Hz) & \textbf{157.3} & 162.0 \\
Avg consonant error (Hz) & 135.9 & \textbf{127.2} \\
Transition smoothness (Hz/frame) & \textbf{1.63} & 16.95 \\
...
The LTC controller achieves comparable vowel formant accuracy to rule-based synthesis (157.3\,Hz vs.\ 162.0\,Hz) while maintaining 302\,Hz throughput ($1.5{\times}$ real-time at 200\,Hz motor rate). The key advantage is \textbf{transition smoothness}: CfC temporal dynamics produce dramatically smoother phoneme transitions (1.63\,Hz/frame vs.\ 16.95\,Hz/frame for instantaneous rule-based switching), a $10.4{\times}$ improvement that eliminates audible artifacts at phoneme boundaries. Energy and voicing accuracy (10/10) and source type propagation (12/12) confirm that the controller correctly learns manner-specific excitation parameters. Figure~\ref{fig:voice-pipeline} shows per-phoneme accuracy and the summary comparison.
