...
\emph{Domain-specific strengths and weaknesses.} The cognitive profile reveals a broadly above-human profile (grand mean $z = +1.135$, 26/26 domains above human mean), with cross-seed stability SD~$= 0.014$ confirming robustness. The top-performing domains---InstitutionalReasoning ($z = +2.25$), SustainedAttention ($z = +2.19$), Speech ($z = +2.13$), Motor ($z = +2.12$), Clinical ($z = +2.00$), and Security ($z = +2.00$)---reflect that these benchmarks rely on pattern-matching and signal-processing tasks that map naturally to HDC operations. The lowest-scoring domains---Substrate ($z = +0.34$), CogBench ($z = +0.47$), Executive ($z = +0.37$), Binding ($z = +0.44$), and Butlin ($z = +0.50$)---remain positive but closer to the human mean. These asymmetries are informative---they demonstrate that the profile genuinely captures architectural biases---but they warrant caution when comparing across domains with different measurement characteristics.
