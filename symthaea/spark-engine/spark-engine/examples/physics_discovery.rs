...
fn main() {
    ...
    println!("To bridge the {:.0} order gap, we would need ONE of:", report.gap_analysis.gap_orders);
    println!("  • Temperature: {:.2e} K (hotter than the Sun's core)",
             report.gap_analysis.required_temperature_k);
    println!("  • Screening: {:.0} eV ({}× higher than Raiola measured)",
             report.gap_analysis.required_screening_ev,
             report.gap_analysis.required_screening_ev / 310.0);
    println!("  • Phonon modes: {} coherent modes (physical limit ~5-10)",
    ...
    for (i, q) in report.open_questions.iter().enumerate() {
        ...
        println!("   Key experiment: {}\n", q.discriminating_experiments.first().unwrap_or(&"None".to_string()));
    }
    ...
    println!("The ~{:.0} order-of-magnitude gap between Gamow theory and LCF observations",
