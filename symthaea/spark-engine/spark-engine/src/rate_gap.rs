...
impl RateGapCalculator {
    ...
    fn breakdown_gap(
        ...
        // Unexplained factor
        let explained_enhancement = screening_enhancement * phonon_enhancement * hot_spot_enhancement;
    ...
    fn assess_gap(
        ...
        match classification {
            ...
            GapClassification::Significant => {
                recommendations.push("Verify neutron energy spectrum (2.45 MeV for D-D)".to_string());
    ...
    pub fn sensitivity_analysis(base_conditions: &ExperimentalConditions) -> SensitivityAnalysis {
        let base_rate = {
            let n_d = base_conditions.host_material.d_density_full() * base_conditions.loading_ratio;
            let ue = base_conditions.host_material.screening_ev();
            let gamow = crate::physics::GamowIntegration::dd_rate(
                base_conditions.temperature_k,
                ue,
                0,
            );
        ...
        let rate_loading_plus = {
            let n_d = base_conditions.host_material.d_density_full() * (base_conditions.loading_ratio + 0.1);
            let ue = base_conditions.host_material.screening_ev();
            let gamow = crate::physics::GamowIntegration::dd_rate(
                base_conditions.temperature_k,
                ue,
