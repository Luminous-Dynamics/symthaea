...
use spark_engine::literature::{LiteratureDatabase, HostMaterial as LitMaterial, TriggerMethod};
...
fn cmd_analyze(args: &[String]) {
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--help" | "-h" => {
                ...
                println!("    --trigger, -t <T>    Trigger: xray, gamma, acoustic, elec [default: xray]");
    ...
    println!("  • Predicted rate:   {:.2e} n/s", analysis.gamow_predicted_rate);
    println!("  • Gap factor:       {:.2e}", analysis.gap_factor);
    println!("  • Gap orders:       {:.1}", analysis.gap_orders);
    println!("  • Classification:   {:?}", analysis.assessment.classification);
    println!();
    println!("Gap Breakdown:");
    println!("  • Screening:        {:.2e}×", analysis.gap_breakdown.screening_enhancement);
    println!("  • Phonons:          {:.2e}×", analysis.gap_breakdown.phonon_enhancement);
    println!("  • Hot spots:        {:.2e}×", analysis.gap_breakdown.hot_spot_enhancement);
    println!("  • Unexplained:      {:.2e}× ({:.0} orders)",
        analysis.gap_breakdown.unexplained_factor,
        analysis.gap_breakdown.unexplained_orders);
    println!();
...
fn cmd_literature(args: &[String]) {
    ...
    let filtered: Vec<_> = observations.into_iter()
    ...
    for obs in filtered {
        ...
        if !obs.effects.is_empty() {
            ...
            for effect in &obs.effects {
                match effect {
                    spark_engine::literature::ObservedEffect::Neutrons { rate_per_s, energy_mev } => {
                        println!("│   • Neutrons: {:.0} n/s, {} MeV",
                            rate_per_s,
                            energy_mev.map(|e| format!("{:.2}", e)).unwrap_or("?".to_string()));
                    }
        ...
        println!("│ Notes: {}", obs.notes.chars().take(55).collect::<String>());
        println!("└─────────────────────────────────────────────────────────────┘");
        println!();
    }
}
...
fn cmd_hypotheses(_args: &[String]) {
    ...
    println!("   • Gap from observed:  {:.2e}×", comparison.hot_spot.gap_factor);
    println!("   • Required T:         {:.2e} K", comparison.hot_spot.required_temperature_k);
    println!("   • Plausible:          {}", if comparison.hot_spot.plausible { "YES" } else { "NO" });
    println!();

    println!("2. PHONON CASCADE (coherent phonon energy addition)");
    println!("   • Predicted (3 modes): {:.2e} n/s", comparison.phonon_cascade.predicted_rate_n3);
    println!("   • Gap from observed:   {:.2e}×", comparison.phonon_cascade.gap_factor);
    ...
    println!("   • Gap from observed:  {:.2e}×", comparison.super_screening.gap_factor);
...
fn cmd_experiments(args: &[String]) {
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--help" | "-h" => {
                ...
                println!();
                println!("OPTIONS:");
                println!("    --phase, -p <N>   Show specific phase: 1/validation, 2/mechanism, 3/optimization");
                return;
            }
            _ => {}
        }
    ...
    for (i, phase) in phases_to_show.iter().enumerate() {
        ...
        println!("┌─────────────────────────────────────────────────────────────┐");
        println!("│ PHASE {}: {}                                      ", phase_num, phase.name.to_uppercase());
        println!("├─────────────────────────────────────────────────────────────┤");
        println!("│ Goal: {}", phase.goal);
        println!("│ Budget: ${:.0}K  Duration: {} months", phase.cost_usd / 1000.0, phase.duration_months);
        println!("├─────────────────────────────────────────────────────────────┤");
        println!("│ EXPERIMENTS:");
        for exp in &phase.experiments {
            println!("│   [{:.0}%] {} - ${:.0}K", exp.priority * 100.0, exp.name, exp.estimated_cost_usd / 1000.0);
        }
        println!("├─────────────────────────────────────────────────────────────┤");
...
fn cmd_gamow(args: &[String]) {
    ...
    println!("  • Q = {:.2e}", q);
    println!("  • Energy gain: {}", if achievable { "POSSIBLE" } else { "NOT ACHIEVABLE" });
}
