...
| Ramanujan conjecture suite (Tier B: Z3 formal) | Internal, polynomial subset | **4/5 eligible invariants formally proven via Z3 UNSAT with persistent `.smt2` witnesses**: harmonic oscillator, Kepler angular momentum, Hénon-Heiles (scaled), Mystery ODE. Lotka-Volterra out of QF_NRA (transcendental). | Strongest formal claim in Symthaea: any SMT-LIB2 solver re-verifies each witness independently. `papers/ramanujan/proofs/*.smt2`, `papers/ramanujan/reproduce.sh --verify-proofs`. |
