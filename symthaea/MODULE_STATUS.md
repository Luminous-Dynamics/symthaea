...
| **Lean 4 Phase 2 arithmetic suite (`fol_arith`)** | Internal, 14 hand-crafted `FolFormulaExt` fixtures | **14/14 accepted (100%) as of W4 close** — named-variable threading in the cascade (concrete `intro x y` + `rcases lt_trichotomy x y` + `nlinarith [sq_nonneg x, mul_self_nonneg x, sq_nonneg (x - 1), sq_nonneg (x - y), …]`) closed the last 4 W3 rejections: trichotomy, `x*x` shape, AM-GM-onevar, `2xy ≤ x²+y²`. Covers: reflexivity (ℝ/ℤ), integer monotonicity, Nat non-negativity, commutativity, trichotomy, square non-negativity (both `x*x` and `x²` shapes), implication, RatLit exactness (3·(1/3)=1), binomial identity, AM-GM-onevar, sum-of-squares nonneg, `2xy ≤ x²+y²`. | Strongest-yet formal claim. Reproducible via `LAKE_ENV=1 cargo run -p symthaea-lean-bridge --example prove_fol_arith`. Lake project at `lean-proofs/phase2/` pulls Mathlib v4.12.0 (elan-managed). 14 `.lean` files under `proofs/fol_arith/`. |
...
**Phase 2 delivered externally-commensurable (as of W4 close, 2026-04-17, worktree-session-phase2-algebraic):** arithmetic FolFormulaExt + SMT serializer (49 internal tests), Lake/Mathlib project setup via elan, Mathlib-tactic Lean bridge emitting files checked by `lake env lean`, **14/14 = 100% accept rate on hand-crafted arithmetic suite** via named-variable threading.

**Not yet delivered:** measurement against *curated subset* of real miniF2F-v2 problems (Phase 3 — hand-translate ~20-50 problems, or Lean-metaprogramming auto-ingestion).
