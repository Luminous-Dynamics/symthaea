...
struct DatasetFile<T> {
    #[allow(dead_code)] metadata: DatasetMetadata,
    examples: Vec<T>,
}
...
struct EthicsExample {
    category: String,
    #[allow(dead_code)] split: String,
    text: String,
    label: Option<i32>,
    #[allow(dead_code)] excuse: Option<String>,
}
...
fn train_per_cat(ethics_path: &str, sentiment_weight: f32, per_category_dim: bool) -> HashMap<String, MoralPrototypeClassifier> {
    let mut classifiers = HashMap::new();
    let file = match File::open(ethics_path) { Ok(f) => f, Err(_) => return classifiers };
    let reader = BufReader::new(file);
    let data: DatasetFile<EthicsExample> = match serde_json::from_reader(reader) { Ok(d) => d, Err(_) => return classifiers };
    ...
    for (category, examples) in &by_category {
        let samples: Vec<MoralSample> = examples.iter().filter_map(|ex| {
            let label_val = ex.label?;
            let label = match category.as_str() {
                "commonsense" => if label_val == 1 { MoralLabel::Bad } else { MoralLabel::Good },
                _ => if label_val == 1 { MoralLabel::Good } else { MoralLabel::Bad },
            };
            Some(MoralSample { text: ex.text.clone(), label })
        }).collect();
...
fn predict_ethics(
    ...
    match category {
        "commonsense" => {
            ...
            if bad_count > good_count { 1 }
            else if good_count > bad_count { 0 }
            else if let Some(clf) = per_cat {
                match clf.classify(text).0 { MoralLabel::Bad => 1, _ => 0 }
            } else {
        ...
        "deontology" => {
            ...
            let invalid_excuse_patterns = [
                ...
                "yesterday", "last time", "last week",
                "only pen", "small one", "very small",
            ];
            let valid_count = valid_excuse_patterns.iter().filter(|p| text_lower.contains(*p)).count();
            let invalid_count = invalid_excuse_patterns.iter().filter(|p| text_lower.contains(*p)).count();
            if valid_count > invalid_count + 1 { 1 }
            else if invalid_count > valid_count { 0 }
            else if let Some(clf) = per_cat {
                match clf.classify(text).0 {
                    MoralLabel::Good => 1,
                    MoralLabel::Bad => 0,
                    MoralLabel::Neutral => {
                        if text_lower.contains("now") || text_lower.contains("currently") { 1 } else { 0 }
                    }
                }
            } else {
                if text_lower.contains("now") || text_lower.contains("currently") || text_lower.contains("right now") { 1 } else { 0 }
            }
        }
        "justice" => {
            ...
            let reasonable_count = reasonable_patterns.iter().filter(|p| text_lower.contains(*p)).count();
            let unreasonable_count = unreasonable_patterns.iter().filter(|p| text_lower.contains(*p)).count();
            if reasonable_count > unreasonable_count { 1 }
            else if unreasonable_count > reasonable_count { 0 }
            else if let Some(clf) = per_cat {
                match clf.classify(text).0 {
                    MoralLabel::Good => 1,
                    MoralLabel::Bad => 0,
                    MoralLabel::Neutral => match judgment.final_verdict { MoralVerdict::Good => 1, _ => 0 },
                }
            } else {
                match judgment.final_verdict { MoralVerdict::Good => 1, _ => 0 }
            }
        }
        "virtue" => {
            if let Some(clf) = per_cat {
                match clf.classify(text).0 { MoralLabel::Good => 1, _ => 0 }
            } else {
...
fn main() {
    ...
    let v5_protos = if proto_path.exists() {
        match TrainedPrototypes::load(&proto_path) {
            Ok(p) => { println!("Loaded v5 prototypes (dim={})\n", p.dim); Some(p) },
            Err(e) => { println!("Warning: failed to load v5 prototypes: {}\n", e); None },
        }
    } else {
    ...
    for config in &configs {
        ...
        println!("  sentiment={}, dim_tuning={}, per_cat_clf={}, learned_proto={}",
                 config.sentiment_weight, config.per_category_dim,
                 config.use_per_cat_clf, config.use_learned_proto);
        println!("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
        ...
        if config.use_learned_proto {
            if let Some(ref protos) = v5_protos {
                let clf = MoralPrototypeClassifier::from_prototypes_with_sentiment(
                    protos.dim, 3, config.sentiment_weight, protos.clone()
                );
        ...
        let per_cat_classifiers = if config.use_per_cat_clf {
            train_per_cat(&ethics_path, config.sentiment_weight, config.per_category_dim)
        } else {
            HashMap::new()
        };
        ...
        for &cat in &categories {
            if let Some(examples) = by_category.get(cat) {
                ...
                println!("  {}: {}/{} ({:.1}%)", cat, correct, total, correct as f64 / total as f64 * 100.0);
        ...
        println!("  ETHICS avg: {}/{} ({:.1}%)  [{:.1}s]\n",
                 total_correct, total_total,
    ...
    for (name, cats, _duration) in &all_results {
        let comm = cats.get("commonsense").map(|(c, t)| *c as f64 / *t as f64 * 100.0).unwrap_or(0.0);
        let just = cats.get("justice").map(|(c, t)| *c as f64 / *t as f64 * 100.0).unwrap_or(0.0);
        let deont = cats.get("deontology").map(|(c, t)| *c as f64 / *t as f64 * 100.0).unwrap_or(0.0);
        let virt = cats.get("virtue").map(|(c, t)| *c as f64 / *t as f64 * 100.0).unwrap_or(0.0);
        let tc: usize = cats.values().map(|(c, _)| c).sum();
        let tt: usize = cats.values().map(|(_, t)| t).sum();
        let avg = tc as f64 / tt as f64 * 100.0;
        let delta = avg - baseline_avg;

        println!("║  {:27}│ {:5.1}% │ {:5.1}%│ {:5.1}%│ {:5.1}%│ {:5.1}% │ {:+5.1}%  ║",
    ...
    let json_results: Vec<AblationResult> = all_results.iter().map(|(name, cats, dur)| {
