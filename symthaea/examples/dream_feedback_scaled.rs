...
fn main() -> Result<()> {
    #[cfg(not(feature = "magi_loop"))]
    {
        ...
        println!("Run with: cargo run --example dream_feedback_scaled --features magi_loop --release");
        return Ok(());
    }
...
fn run_experiment() -> Result<()> {
    ...
    let dream_briers: Vec<f64> = all_results.iter()
        .filter_map(|r| r.dream_brier)
        .collect();
    let baseline_briers: Vec<f64> = all_results.iter()
    ...
    println!("Mean Improvement: {:.4} ({:.1}%)", mean_improvement, (mean_improvement / mean_baseline) * 100.0);

    // Cross-trial consistency
    let improvements: Vec<f64> = all_results.iter()
    ...
    println!("\nConsistency:");
    println!("  All trials show improvement: {}", if all_positive { "YES" } else { "NO" });
    println!("  Improvement std dev: {:.4}", std_improvement);
    println!("  Coefficient of variation: {:.1}%", (std_improvement / mean_improvement.abs()) * 100.0);
    ...
    println!("\nPooled Analysis (n={} dream, n={} baseline):", total_dream_n, total_baseline_n);
    println!("  Pooled Dream Brier: {:.4}", pooled_dream);
    println!("  Pooled Baseline Brier: {:.4}", pooled_baseline);
    println!("  Pooled Improvement: {:.4} ({:.1}%)", pooled_improvement, (pooled_improvement / pooled_baseline) * 100.0);

    // Bootstrap CI on pooled data
    let (ci_lower, ci_upper) = pooled_bootstrap_ci(total_dream_n, total_baseline_n, pooled_dream, pooled_baseline);

    println!("\n  95% CI for improvement: [{:.4}, {:.4}]", ci_lower, ci_upper);

    if ci_lower > 0.0 {
        println!("\n✓ DREAM FEEDBACK SIGNIFICANTLY IMPROVES CALIBRATION");
        println!("  95% CI excludes zero");
        println!("  Effect size: {:.1}% improvement in Brier score", (pooled_improvement / pooled_baseline) * 100.0);
...
fn run_single_trial(seed: u64) -> Result<TrialResult> {
    ...
    for (i, context) in contexts.iter().enumerate() {
        ...
        if phi_improvement > 0.03 { // Lower threshold for more insights
    ...
    println!("    {} insights, {} priors", insights_created, bridge.num_priors());
    ...
    if let (Some(d), Some(b)) = (stats.dream_informed_brier(), stats.baseline_brier()) {
        println!("    Dream: {:.4} (n={})", d, stats.dream_informed_predictions);
        println!("    Base:  {:.4} (n={})", b, stats.baseline_predictions);
        println!("    Improvement: {:.4} ({:.1}%)", b - d, ((b - d) / b) * 100.0);
    }
...
#[cfg(feature = "magi_loop")]
fn pooled_bootstrap_ci(n_dream: usize, n_baseline: usize, dream_mean: f64, baseline_mean: f64) -> (f64, f64) {
