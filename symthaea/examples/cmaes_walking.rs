...
fn run() {
    ...
    println!("  Pop: {pop_size}, Gens: {n_generations}, Steps: {eval_steps}, Target: {target_speed} m/s");
    println!();
    ...
    for gen in 0..n_generations {
        ...
        for p in 0..pop_size {
            ...
            let (fitness, speed) = evaluate_walking(
                &candidate, n_weights, &genesis, &config, &pd_gains,
                eval_steps, dt, target_speed,
            );
        ...
        if gen % 5 == 0 || gen == n_generations - 1 {
            ...
            println!("{:>5} {:>10.4} {:>10.4} {:>10.4} {:>7.4} {:>7.3} {:>7.3}",
                gen, best_fitness, mean_fitness, best_ever_fitness, sigma, gain, best_speed);
            std::io::stdout().flush().unwrap_or(());
        }
    }
    ...
    let (pd_fit, pd_speed) = evaluate_walking(
        &vec![0.0; n_params], n_weights, &genesis, &config, &pd_gains,
        1000, dt, target_speed,
    );
    let (best_fit, best_speed) = evaluate_walking(
        &best_ever_params, n_weights, &genesis, &config, &pd_gains,
        1000, dt, target_speed,
    );

    println!("  PD alone:      fitness={:.4}, speed={:.4} m/s", pd_fit, pd_speed);
    println!("  PD + learned:  fitness={:.4}, speed={:.4} m/s (gain={:.3})", best_fit, best_speed, best_gain);

    if best_fit > pd_fit {
        let improvement = (best_fit - pd_fit) / pd_fit.abs().max(0.001) * 100.0;
        println!("  *** LEARNED CORRECTION IMPROVES WALKING BY {:.1}% ***", improvement);
    } else {
...
fn evaluate_walking(
    ...
    for step in 0..eval_steps {
        ...
        let head_ok = if s.head_height >= 1.0 { 1.0 } else { (s.head_height / 1.0).max(0.0) };
