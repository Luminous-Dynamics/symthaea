...
impl Genome {
    ...
    fn crossover(a: &Self, b: &Self, rng: &mut ChaCha8Rng) -> Self {
        Self {
            softmin_necessary: if rng.gen_bool(0.5) { a.softmin_necessary } else { b.softmin_necessary },
            softmin_amplifier: if rng.gen_bool(0.5) { a.softmin_amplifier } else { b.softmin_amplifier },
            necessary_weight: if rng.gen_bool(0.5) { a.necessary_weight } else { b.necessary_weight },
            amplifier_ceiling: if rng.gen_bool(0.5) { a.amplifier_ceiling } else { b.amplifier_ceiling },
            first_order_floor: if rng.gen_bool(0.5) { a.first_order_floor } else { b.first_order_floor },
            hot_is_necessary: if rng.gen_bool(0.5) { a.hot_is_necessary } else { b.hot_is_necessary },
            attention_is_necessary: if rng.gen_bool(0.5) { a.attention_is_necessary } else { b.attention_is_necessary },
        }
    }
}
...
fn make_state(
    phi: f64, binding: f64, workspace: f64, hot: f64,
    attention: f64, recurrence: f64, efficacy: f64, knowledge: f64,
...
fn compute_c(genome: &Genome, s: &ConsciousnessStateV2) -> f64 {
    ...
    if genome.hot_is_necessary { necessary.push(hot); } else { amplifiers.push(hot); }
    if genome.attention_is_necessary { necessary.push(attention); } else { amplifiers.push(attention); }

    // Softmin of necessary
    let exp_n = genome.softmin_necessary;
    let nec = if necessary.is_empty() { 0.0 } else {
        let sum: f64 = necessary.iter().map(|&x| (-(x * exp_n)).exp()).sum();
        -(sum / necessary.len() as f64).ln() / exp_n
    };

    // Amplifier mean
    let amp = if amplifiers.is_empty() { 1.0 } else {
...
fn evaluate_fitness(genome: &Genome) -> f64 {
    ...
    if hi_phi > lo_phi { tests_passed += 1; }
    ...
    if hi_hot > lo_hot { tests_passed += 1; }
...
fn main() {
    ...
    println!("║  Pop: {}  Gen: {}  Elite: {:.0}%  Mutation: {:.0}%                      ║",
        POPULATION_SIZE, NUM_GENERATIONS, ELITE_FRACTION * 100.0, MUTATION_RATE * 100.0);
    println!("╚══════════════════════════════════════════════════════════════════╝\n");
    ...
    let incumbent = Genome {
        softmin_necessary: 5.0, softmin_amplifier: 2.0,
        necessary_weight: 0.7, amplifier_ceiling: 1.5,
        first_order_floor: 0.3, hot_is_necessary: false,
    ...
    for (i, (g, f)) in pop.iter().take(5).enumerate() {
        println!("  #{}: f={:.4} nec={:.1} amp={:.1} w={:.2} ceil={:.2} floor={:.2} HOT={} ATT={}",
            i + 1, f, g.softmin_necessary, g.softmin_amplifier,
            g.necessary_weight, g.amplifier_ceiling, g.first_order_floor,
    ...
    let test_states = [
        ("High all", make_state(0.8, 0.8, 0.8, 0.8, 0.8, 0.8, 0.8, 0.8)),
        ("Low Phi", make_state(0.05, 0.8, 0.8, 0.8, 0.8, 0.8, 0.8, 0.8)),
        ("No workspace", make_state(0.8, 0.8, 0.01, 0.8, 0.8, 0.8, 0.8, 0.8)),
        ("First-order", make_state(0.9, 0.9, 0.9, 0.05, 0.5, 0.5, 0.5, 0.5)),
    ];

    for (label, state) in &test_states {
        let evolved_c = compute_c(&best.0, state);
        let actual = eq.compute(state);
        println!("  {:<15} evolved={:.4}  actual={:.4}",
