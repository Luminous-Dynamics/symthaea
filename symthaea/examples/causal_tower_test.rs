...
fn main() {
    ...
    for (name, acc) in &methods {
        ...
        println!("  {:22} {:5.1}%      {:+5.1}%      -{:4.1}%      {}",
                 name, acc * 100.0, delta_random, gap_to_100, marker);
    }
    ...
    if tower_acc > majority_acc {
        ...
        println!("\n  Gap: {:.1}% (Tower: {:.1}% vs Majority: {:.1}%)",
                 ...
                 tower_acc * 100.0, majority_acc * 100.0);
    }
    ...
    for (i, pair) in pairs.iter().enumerate() {
        ...
        let is_correct = match (&predicted, actual) {
            (symthaea::benchmarks::CausalDirection::Forward,
             symthaea::benchmarks::CausalDirection::Forward) => true,
            (symthaea::benchmarks::CausalDirection::Backward,
             symthaea::benchmarks::CausalDirection::Backward) => true,
            _ => false,
        };
    ...
    println!("  Correct: {} ({:.1}%)", right_cases.len(), right_cases.len() as f64 / pairs.len() as f64 * 100.0);
    println!("  Wrong: {} ({:.1}%)", wrong_cases.len(), wrong_cases.len() as f64 / pairs.len() as f64 * 100.0);
    ...
    if !wrong_cases.is_empty() {
        ...
        for (idx, conf, p_fwd) in sorted_wrong.iter().take(5) {
            println!("    Pair {}: confidence={:.2}, p_forward={:.2}", idx + 1, conf, p_fwd);
        }
