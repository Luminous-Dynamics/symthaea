...
use symthaea::soul::{WeaverActor, DailyState, ConceptDiscovery};
...
fn main() {
    ...
    println!("    Redness:    [{:.2}, {:.2}, {:.2}]", red_square[0], red_square[1], red_square[2]);
    println!("    Squareness: [{:.2}, {:.2}, {:.2}, {:.2}]", red_square[8], red_square[9], red_square[10], red_square[11]);
    println!();
    ...
    let daily_state = DailyState {
        ...
        peak_consciousness: consciousness_history.iter().copied().fold(0.0_f64, f64::max) as f32,
        narrative: format!(
            ...
            NUM_EXPOSURES,
            concepts_discovered.len(),
            consciousness_history.iter().copied().fold(0.0_f64, f64::max) * 100.0
        ),
    };
    ...
    if !world_model.pending_concepts.is_empty() {
        ...
        if let Some(first_concept) = world_model.pending_concepts.first() {
            ...
            match naming_ceremony.name_concept(&mut world_model, &uid, "RedSquare", Some(&mut weaver)) {
                Ok(record) => {
    ...
    let surprise_decreased = final_surprise < initial_surprise ||
        surprise_history.windows(2).filter(|w| w[1] <= w[0]).count() > surprise_history.len() / 2;

    let peak_consciousness = consciousness_history.iter().copied().fold(0.0_f64, f64::max);

    println!("  Metric                    | Value          | Status");
    println!("  ──────────────────────────┼────────────────┼─────────────");
    println!("  Total Exposures           | {:>14} | OK", NUM_EXPOSURES);
    println!("  Initial Surprise          | {:>14.4} | -", initial_surprise);
    println!("  Final Surprise            | {:>14.4} | {}",
             final_surprise,
             if surprise_decreased { "LEARNING" } else { "STATIC" });
    ...
    println!("  Concepts Crystallized     | {:>14} | {}",
             concepts_discovered.len(),
             if !concepts_discovered.is_empty() { "EMERGENT" } else { "WAITING" });
    ...
    if success {
        ...
        if !concepts_discovered.is_empty() {
            println!("║   Concepts discovered: {:>3}                                    ║", concepts_discovered.len());
        }
        println!("╚════════════════════════════════════════════════════════════════╝");
    } else {
        println!("╔════════════════════════════════════════════════════════════════╗");
