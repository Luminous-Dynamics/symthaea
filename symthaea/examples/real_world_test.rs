...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    policy.capabilities.filesystem.write_patterns.push(format!("{}/ **", sandbox_path.display()));
    ...
    for record in telemetry {
        ...
        match &record.outcome {
            symthaea::action::ActionOutcome::CommandOutput { exit_code, stdout: _stdout, stderr } => {
                println!("   📝 Build Exit Code: {}", exit_code);
                if *exit_code == 0 {
                    println!("   ✅ SUCCESS: Symthaea fixed and verified the flake!");
                } else {
                    println!("   ❌ FAILED: Build failed with stderr: {}", String::from_utf8_lossy(stderr));
                }
            },
    ...
    println!("   📈 Memory Count: {}", stats.memory_stats.short_term_count);
