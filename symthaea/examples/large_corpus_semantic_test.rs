...
fn get_large_corpus() -> Vec<(&'static str, SemanticCategory)> {
    vec![
        ...
        // ═══════════════════════════════════════════════════════════════════
        ("Justice requires fairness and equality for all citizens", SemanticCategory::AbstractPhilosophy),
        ("Democracy is built on representation and voting rights", SemanticCategory::AbstractPhilosophy),
        ("Freedom means the ability to choose without coercion", SemanticCategory::AbstractPhilosophy),
        ("Truth is correspondence between statement and reality", SemanticCategory::AbstractPhilosophy),
        ("Morality guides our understanding of right and wrong", SemanticCategory::AbstractPhilosophy),
        ("Ethics examines the principles of good conduct", SemanticCategory::AbstractPhilosophy),
        ("Wisdom comes from experience and reflection", SemanticCategory::AbstractPhilosophy),
        ("Beauty exists in the eye of the beholder", SemanticCategory::AbstractPhilosophy),
        ("Consciousness is the awareness of one's own existence", SemanticCategory::AbstractPhilosophy),
        ("Liberty must be balanced with responsibility", SemanticCategory::AbstractPhilosophy),

        // ═══════════════════════════════════════════════════════════════════
        // CAUSAL RELATIONS (10 sentences)
        // ═══════════════════════════════════════════════════════════════════
        ("Smoking causes lung cancer and respiratory disease", SemanticCategory::CausalRelations),
        ("Rain makes the ground wet and nourishes plants", SemanticCategory::CausalRelations),
        ("Exercise leads to improved health and fitness", SemanticCategory::CausalRelations),
        ("Heat causes water to boil at 100 degrees Celsius", SemanticCategory::CausalRelations),
        ("Lack of sleep results in decreased cognitive function", SemanticCategory::CausalRelations),
        ("Deforestation contributes to climate change", SemanticCategory::CausalRelations),
        ("Education leads to better employment opportunities", SemanticCategory::CausalRelations),
        ("Stress triggers the release of cortisol hormones", SemanticCategory::CausalRelations),
        ("Overeating results in weight gain over time", SemanticCategory::CausalRelations),
        ("Pollution causes environmental degradation", SemanticCategory::CausalRelations),

        // ═══════════════════════════════════════════════════════════════════
        // TEMPORAL EVENTS (10 sentences)
        // ═══════════════════════════════════════════════════════════════════
        ("Yesterday I went to the store to buy groceries", SemanticCategory::TemporalEvents),
        ("Tomorrow the sun will rise in the east", SemanticCategory::TemporalEvents),
        ("Last week we finished the project ahead of schedule", SemanticCategory::TemporalEvents),
        ("In the future technology will advance rapidly", SemanticCategory::TemporalEvents),
        ("The meeting started at three o'clock sharp", SemanticCategory::TemporalEvents),
        ("By next year we will have completed the renovation", SemanticCategory::TemporalEvents),
        ("The ancient Romans built roads two thousand years ago", SemanticCategory::TemporalEvents),
        ("Currently the weather is mild and pleasant", SemanticCategory::TemporalEvents),
        ("Soon the leaves will change color for autumn", SemanticCategory::TemporalEvents),
        ("Previously this land was covered by forest", SemanticCategory::TemporalEvents),

        // ═══════════════════════════════════════════════════════════════════
        // EMOTIONAL STATES (10 sentences)
        // ═══════════════════════════════════════════════════════════════════
        ("The news made me feel deeply sad and heartbroken", SemanticCategory::EmotionalStates),
        ("Joy filled my heart when I saw her smiling face", SemanticCategory::EmotionalStates),
        ("Anger consumed him after the unexpected betrayal", SemanticCategory::EmotionalStates),
        ("Peace washed over me during the meditation session", SemanticCategory::EmotionalStates),
        ("Fear gripped her as she heard the strange noise", SemanticCategory::EmotionalStates),
        ("Excitement bubbled up before the big announcement", SemanticCategory::EmotionalStates),
        ("Grief overwhelmed the family after the loss", SemanticCategory::EmotionalStates),
        ("Contentment settled in as the day came to an end", SemanticCategory::EmotionalStates),
        ("Anxiety plagued him before the important exam", SemanticCategory::EmotionalStates),
        ("Love blossomed between them over the summer months", SemanticCategory::EmotionalStates),

        // ═══════════════════════════════════════════════════════════════════
        // SCIENTIFIC FACTS (10 sentences)
        // ═══════════════════════════════════════════════════════════════════
        ("Electrons orbit the atomic nucleus in shells", SemanticCategory::ScientificFacts),
        ("DNA carries genetic information in all living cells", SemanticCategory::ScientificFacts),
        ("Gravity attracts massive objects toward each other", SemanticCategory::ScientificFacts),
        ("Photosynthesis converts light energy into chemical energy", SemanticCategory::ScientificFacts),
        ("The speed of light is approximately 300000 kilometers per second", SemanticCategory::ScientificFacts),
        ("Mitochondria are the powerhouses of the cell", SemanticCategory::ScientificFacts),
        ("Water molecules consist of two hydrogen and one oxygen atom", SemanticCategory::ScientificFacts),
        ("Evolution occurs through natural selection over generations", SemanticCategory::ScientificFacts),
        ("Neurons transmit electrical signals in the brain", SemanticCategory::ScientificFacts),
        ("The Earth orbits the Sun once every 365 days", SemanticCategory::ScientificFacts),

        // ═══════════════════════════════════════════════════════════════════
        // SPATIAL RELATIONS (10 sentences)
        // ═══════════════════════════════════════════════════════════════════
        ("Paris is the capital city of France in Europe", SemanticCategory::SpatialRelations),
        ("The cat sat on the mat beside the fireplace", SemanticCategory::SpatialRelations),
        ("Mount Everest is the tallest mountain on Earth", SemanticCategory::SpatialRelations),
        ("The book is located on the top shelf of the bookcase", SemanticCategory::SpatialRelations),
        ("Australia is a continent in the southern hemisphere", SemanticCategory::SpatialRelations),
        ("The river flows through the valley between the mountains", SemanticCategory::SpatialRelations),
        ("The restaurant is located across the street from the park", SemanticCategory::SpatialRelations),
        ("Japan is an island nation in the Pacific Ocean", SemanticCategory::SpatialRelations),
        ("The airplane flew above the clouds at high altitude", SemanticCategory::SpatialRelations),
        ("The treasure was buried beneath the old oak tree", SemanticCategory::SpatialRelations),

        // ═══════════════════════════════════════════════════════════════════
        // FINANCIAL DOMAIN (10 sentences)
        // ═══════════════════════════════════════════════════════════════════
        ("The bank approved my mortgage application yesterday", SemanticCategory::FinancialDomain),
        ("Stock prices fluctuated wildly during the market crash", SemanticCategory::FinancialDomain),
        ("Interest rates affect borrowing costs for consumers", SemanticCategory::FinancialDomain),
        ("She deposited her paycheck into the savings account", SemanticCategory::FinancialDomain),
        ("The company declared bankruptcy after years of losses", SemanticCategory::FinancialDomain),
        ("Inflation erodes the purchasing power of currency", SemanticCategory::FinancialDomain),
        ("Investors diversify portfolios to reduce risk", SemanticCategory::FinancialDomain),
        ("The credit card debt accumulated substantial interest charges", SemanticCategory::FinancialDomain),
        ("Economic growth depends on consumer spending patterns", SemanticCategory::FinancialDomain),
        ("The hedge fund manager earned millions in bonuses", SemanticCategory::FinancialDomain),

        // ═══════════════════════════════════════════════════════════════════
        // NATURE/ENVIRONMENT (10 sentences)
        // ═══════════════════════════════════════════════════════════════════
        ("The river bank was covered with wildflowers in spring", SemanticCategory::NatureEnvironment),
        ("Birds migrate south when winter approaches each year", SemanticCategory::NatureEnvironment),
        ("The forest ecosystem supports diverse wildlife species", SemanticCategory::NatureEnvironment),
        ("Coral reefs are dying due to ocean acidification", SemanticCategory::NatureEnvironment),
        ("Bees pollinate flowers and are essential for agriculture", SemanticCategory::NatureEnvironment),
        ("The old oak tree provided shade for generations", SemanticCategory::NatureEnvironment),
        ("Wolves hunt in packs across the wilderness", SemanticCategory::NatureEnvironment),
        ("The wetlands serve as a habitat for migratory birds", SemanticCategory::NatureEnvironment),
        ("Mushrooms decompose organic matter in the forest floor", SemanticCategory::NatureEnvironment),
        ("The butterfly emerged from its chrysalis in spring", SemanticCategory::NatureEnvironment),

        // ═══════════════════════════════════════════════════════════════════
        // TECHNOLOGY/COMPUTING (10 sentences)
        // ═══════════════════════════════════════════════════════════════════
        ("The computer virus corrupted all the system files", SemanticCategory::TechnologyComputing),
        ("Cloud computing enables scalable infrastructure deployment", SemanticCategory::TechnologyComputing),
        ("The algorithm optimized the search results efficiently", SemanticCategory::TechnologyComputing),
        ("Machine learning models require large training datasets", SemanticCategory::TechnologyComputing),
        ("The firewall blocked unauthorized network access attempts", SemanticCategory::TechnologyComputing),
        ("Encryption protects sensitive data from hackers", SemanticCategory::TechnologyComputing),
        ("The database stores millions of customer records", SemanticCategory::TechnologyComputing),
        ("Artificial intelligence powers modern recommendation systems", SemanticCategory::TechnologyComputing),
        ("The software update fixed several critical bugs", SemanticCategory::TechnologyComputing),
        ("Quantum computers will revolutionize cryptography", SemanticCategory::TechnologyComputing),

        // ═══════════════════════════════════════════════════════════════════
        // HEALTH/MEDICINE (10 sentences)
        // ═══════════════════════════════════════════════════════════════════
        ("The flu virus spreads rapidly during winter months", SemanticCategory::HealthMedicine),
        ("Antibiotics treat bacterial infections but not viruses", SemanticCategory::HealthMedicine),
        ("Regular exercise improves cardiovascular health", SemanticCategory::HealthMedicine),
        ("The vaccine provides immunity against the disease", SemanticCategory::HealthMedicine),
        ("Blood pressure should be monitored regularly", SemanticCategory::HealthMedicine),
        ("The surgeon performed the operation successfully", SemanticCategory::HealthMedicine),
        ("Mental health is as important as physical health", SemanticCategory::HealthMedicine),
        ("The patient recovered fully after the treatment", SemanticCategory::HealthMedicine),
        ("Diabetes requires careful management of blood sugar", SemanticCategory::HealthMedicine),
        ("The nurse administered the medication as prescribed", SemanticCategory::HealthMedicine),

        // ═══════════════════════════════════════════════════════════════════
        // SOCIAL/INTERPERSONAL (10 sentences)
        // ═══════════════════════════════════════════════════════════════════
        ("Friendship requires trust and mutual respect", SemanticCategory::SocialInterpersonal),
        ("The family gathered for the holiday celebration", SemanticCategory::SocialInterpersonal),
        ("Communication is the foundation of healthy relationships", SemanticCategory::SocialInterpersonal),
        ("The community organized a fundraiser for charity", SemanticCategory::SocialInterpersonal),
        ("Teamwork enables groups to achieve common goals", SemanticCategory::SocialInterpersonal),
        ("The neighbors helped each other during the storm", SemanticCategory::SocialInterpersonal),
        ("Social media connects people across great distances", SemanticCategory::SocialInterpersonal),
        ("The mentor guided the young professional through challenges", SemanticCategory::SocialInterpersonal),
        ("Conflict resolution requires empathy and patience", SemanticCategory::SocialInterpersonal),
        ("Cultural traditions are passed down through generations", SemanticCategory::SocialInterpersonal),
        ...
        ("I walked along the river bank watching the sunset", SemanticCategory::AmbiguousHomonyms),  // Nature meaning
        ("The investment bank collapsed during the financial crisis", SemanticCategory::AmbiguousHomonyms),  // Financial meaning

        // "Virus" homonyms - biological vs computer
        ("The biological virus mutated and spread rapidly", SemanticCategory::AmbiguousHomonyms),  // Medical meaning
        ("The malware virus infected thousands of computers", SemanticCategory::AmbiguousHomonyms),  // Tech meaning

        // "Cell" homonyms - biological vs prison vs phone
        ("The red blood cell carries oxygen through the body", SemanticCategory::AmbiguousHomonyms),  // Biology
        ("The prisoner sat alone in his prison cell", SemanticCategory::AmbiguousHomonyms),  // Prison

        // "Spring" homonyms - season vs water source vs coil
        ("Spring brings flowers and warmer weather", SemanticCategory::AmbiguousHomonyms),  // Season
        ("The natural spring provided fresh water", SemanticCategory::AmbiguousHomonyms),  // Water source

        // "Mouse" homonyms - animal vs computer device
        ("The small gray mouse scurried across the floor", SemanticCategory::AmbiguousHomonyms),  // Animal
        ("Click the computer mouse to select the icon", SemanticCategory::AmbiguousHomonyms),  // Device

        // "Bat" homonyms - animal vs sports equipment
        ("The vampire bat flew through the night sky", SemanticCategory::AmbiguousHomonyms),  // Animal
        ("He swung the baseball bat with great force", SemanticCategory::AmbiguousHomonyms),  // Sports
    ]
}

/// Get pairs of sentences that should cluster together (near-synonyms)
fn get_synonym_pairs() -> Vec<((&'static str, SemanticCategory), (&'static str, SemanticCategory))> {
    vec![
...
fn get_homonym_pairs() -> Vec<((&'static str, SemanticCategory), (&'static str, SemanticCategory))> {
    vec![
...
fn generate_enhanced_mock_embedding(text: &str, category: SemanticCategory) -> Vec<f32> {
    ...
    // Financial keywords
    let financial_keywords = ["bank", "money", "invest", "mortgage", "stock", "credit", "debt", "interest"];
    let financial_score: f32 = financial_keywords.iter()
        .filter(|kw| text_lower.contains(*kw))
        .count() as f32 / financial_keywords.len() as f32;

    // Nature keywords
    let nature_keywords = ["river", "forest", "tree", "bird", "flower", "animal", "wildlife", "ecosystem"];
    let nature_score: f32 = nature_keywords.iter()
        .filter(|kw| text_lower.contains(*kw))
        .count() as f32 / nature_keywords.len() as f32;

    // Technology keywords
    let tech_keywords = ["computer", "software", "algorithm", "database", "network", "digital", "code", "programming"];
    let tech_score: f32 = tech_keywords.iter()
        .filter(|kw| text_lower.contains(*kw))
        .count() as f32 / tech_keywords.len() as f32;

    // Medical keywords
    let medical_keywords = ["virus", "disease", "treatment", "patient", "doctor", "medicine", "health", "symptom"];
    let medical_score: f32 = medical_keywords.iter()
        .filter(|kw| text_lower.contains(*kw))
        .count() as f32 / medical_keywords.len() as f32;
...
fn compute_pairwise_similarities(
    ...
    for cat_a in categories {
        for cat_b in categories {
            stats.insert((*cat_a, *cat_b), PairwiseStats {
...
fn compute_clustering_metrics(
    ...
    let intra_avg = if intra_count > 0 { (intra_sum / intra_count as f64) as f32 } else { 0.0 };
    let inter_avg = if inter_count > 0 { (inter_sum / inter_count as f64) as f32 } else { 0.0 };
...
fn find_confused_pairs(
    ...
    confused.sort_by(|a, b| b.similarity.partial_cmp(&a.similarity).unwrap_or(std::cmp::Ordering::Equal));
    confused
}

/// Find pairs within same category with low similarity (possible misclassifications)
fn find_intra_category_outliers(
    results: &[SentenceResult],
    threshold: f32,
) -> Vec<ConfusionEntry> {
    ...
    outliers.sort_by(|a, b| a.similarity.partial_cmp(&b.similarity).unwrap_or(std::cmp::Ordering::Equal));
    outliers
}
    ...
    for cat in categories {
        ...
        if let Some(entry) = stats.get(&(*cat, *cat)) {
            println!(
                ...
                n,
                entry.avg(),
                if entry.min_similarity < f32::MAX { entry.min_similarity } else { 0.0 },
                if entry.max_similarity > f32::MIN { entry.max_similarity } else { 0.0 },
...
fn main() -> Result<()> {
    ...
    let category_counts: HashMap<SemanticCategory, usize> = corpus
        .iter()
        .fold(HashMap::new(), |mut acc, (_, cat)| {
    ...
    for cat in &categories {
        println!("    {:12}: {}", cat.name(), category_counts.get(cat).unwrap_or(&0));
    }
    println!();
    ...
    println!("  CLUSTERING SCORE (intra - inter): {:.4}", clustering_score);
    println!();
    ...
    println!("Most Confused Pairs (similarity > {:.3}):", confusion_threshold);
    println!();
    ...
    if confused_pairs.len() > 10 {
        println!("      ... and {} more confused pairs", confused_pairs.len() - 10);
    }
    ...
    println!("Intra-Category Outliers (similarity < {:.3}):", outlier_threshold);
    println!();
    ...
    for (i, ((text_a, cat_a), (text_b, cat_b))) in homonym_pairs.iter().enumerate() {
        ...
        let result_a = results.iter().find(|r| r.text.contains(&text_a[..30.min(text_a.len())]));
        let result_b = results.iter().find(|r| r.text.contains(&text_b[..30.min(text_b.len())]));
        ...
        if let (Some(ra), Some(rb)) = (result_a, result_b) {
            ...
            println!("  {}. {} vs {} (sim={:.4}): {}", i + 1, cat_a.short_name(), cat_b.short_name(), sim, status);
            println!("     A: {:.50}...", text_a);
            println!("     B: {:.50}...", text_b);
            println!();
        }
    ...
    for (i, ((text_a, cat_a), (text_b, _cat_b))) in synonym_pairs.iter().enumerate() {
        ...
        if let (Some(ra), Some(rb)) = (result_a, result_b) {
            ...
            println!("  {}. {} (sim={:.4}): {}", i + 1, cat_a.short_name(), sim, status);
            println!("     A: {:.50}...", text_a);
            println!("     B: {:.50}...", text_b);
            println!();
        }
    ...
    println!("Analyzing {} ambiguous sentences for internal structure:", ambiguous_results.len());
    println!();
    ...
    println!();
    println!("  Confusion Analysis:");
    println!("    Confused pairs:       {} (threshold: {:.3})", confused_pairs.len(), confusion_threshold);
    println!("    Intra-cat outliers:   {} (threshold: {:.3})", outliers.len(), outlier_threshold);
    println!();
...
mod tests {
    ...
    fn test_corpus_size() {
        let corpus = get_large_corpus();
        assert!(corpus.len() >= 100, "Corpus should have at least 100 sentences, got {}", corpus.len());
    }
    ...
    fn test_mock_embedding_category_separation() {
        ...
        assert!(sim < 0.9, "Different categories should have lower similarity, got {}", sim);
    }
    ...
    fn test_clustering_score_positive() {
        ...
        assert!(score > 0.0, "Clustering score should be positive with mock embeddings");
    }
}
