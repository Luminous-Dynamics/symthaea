...
fn main() {
    ...
    let old_config = MuseConfig {
        duration_secs: 4.0, max_notes: 16, output_format: OutputFormat::Mono16,
    ...
    let new_config = MuseConfig {
        duration_secs: 4.0, max_notes: 16, melody_mode: MelodyMode::Neural,
        ..Default::default()
    };
    ...
    println!("  Old (4 partial, mono): {} samples, RMS={:.4}", old_comp.audio.len(), old_rms);
    println!("  New (8 partial, stereo): {} samples, RMS={:.4}", new_comp.audio.len(), new_rms);
    check(&mut pass, &mut fail, "Old produces audio", old_rms > 0.001);
    check(&mut pass, &mut fail, "New produces audio", new_rms > 0.001);
    check(&mut pass, &mut fail, "New is stereo", matches!(new_comp.audio, AudioData::StereoF32(_)));
    println!("  Notes: old={}, new={}\n", old_comp.notes.len(), new_comp.notes.len());

    // ─── 2. Output Format Matrix ────────────────────────────────────────────
    println!("━━━ 2. Output Format Matrix ━━━");
    for (fmt, name) in [(OutputFormat::Mono16, "Mono16"), (OutputFormat::MonoF32, "MonoF32"), (OutputFormat::StereoF32, "StereoF32")] {
        let cfg = MuseConfig { duration_secs: 2.0, max_notes: 8, output_format: fmt, ..Default::default() };
        ...
        println!("  {name:12} — {} samples, RMS={:.4}", comp.audio.len(), audio_rms(&comp.audio));
    }

    // Neural mode
    let neural_cfg = MuseConfig { duration_secs: 2.0, max_notes: 8, melody_mode: MelodyMode::Neural, ..Default::default() };
    let neural = compose(&neural_cfg, &MusicalState::default(), 42);
    check(&mut pass, &mut fail, "Neural mode produces audio", !neural.audio.is_empty());
    println!("  Neural — {} notes, {} samples\n", neural.notes.len(), neural.audio.len());

    // ─── 3. Consciousness-Coupled Variation ─────────────────────────────────
    println!("━━━ 3. Consciousness-Coupled Variation ━━━");
    let calm = MusicalState { consciousness_level: 0.9, arousal: 0.1, serotonin: 0.8, ..Default::default() };
    let intense = MusicalState { consciousness_level: 0.2, arousal: 0.9, noradrenaline: 0.8, dopamine: 0.9, ..Default::default() };
    let cfg = MuseConfig { duration_secs: 3.0, max_notes: 12, ..Default::default() };
    ...
    println!("  Calm:    {} notes, RMS={:.4}", calm_comp.notes.len(), calm_rms);
    println!("  Intense: {} notes, RMS={:.4}", intense_comp.notes.len(), intense_rms);
    check(&mut pass, &mut fail, "Different states → different note counts", calm_comp.notes.len() != intense_comp.notes.len());
    println!();
    ...
    println!("━━━ 4. Streaming Pipeline ━━━");
    {
        ...
        let mut synth = StreamingSynth::new(MuseConfig { duration_secs: 4.0, max_notes: 16, ..Default::default() }, 44100);
        synth.update_state(&MusicalState { consciousness_level: 0.8, ..Default::default() });
        let mut total_rms = 0.0f32;
        let mut chunks = 0;
        for _ in 0..100 {
            let chunk = synth.render_chunk();
            let rms: f32 = (chunk.iter().map(|s| s[0]*s[0] + s[1]*s[1]).sum::<f32>() / chunk.len() as f32).sqrt();
            total_rms += rms;
            chunks += 1;
            for s in &chunk {
        ...
        check(&mut pass, &mut fail, "Feedback loop extracts features", features.spectral_centroid > 0.0 || features.rms_energy > 0.0);
    }
    println!();
    ...
    println!("━━━ 5. Module Smoke Tests ━━━");
    {
        ...
        check(&mut pass, &mut fail, "Wavetable produces signal", s.abs() > 0.0 || osc.phase > 0.0);

        // Sidechain
        use symthaea_muse::sidechain::DuckingMatrix;
        let mut matrix = DuckingMatrix::default_matrix(44100);
        let roles = [symthaea_muse::voice::VoiceRole::Lead, symthaea_muse::voice::VoiceRole::Bass];
        let mut bufs = vec![vec![0.5f32; 100], vec![0.3f32; 100]];
        matrix.apply(&mut bufs, &roles, 100);
        check(&mut pass, &mut fail, "Sidechain ducks bass", bufs[1].iter().sum::<f32>() / 100.0 < 0.3);
        ...
        check(&mut pass, &mut fail, "Consciousness reverb produces output", l.abs() > 0.0 || r.abs() > 0.0);

        // Binaural
        use symthaea_muse::binaural::BinauralConsciousnessRenderer;
        let mut bin = BinauralConsciousnessRenderer::new(2, 44100);
        bin.update_state(&MusicalState { consciousness_level: 0.8, ..Default::default() });
        let out = bin.render(&[vec![0.5; 256], vec![0.3; 256]]);
        check(&mut pass, &mut fail, "Binaural renders stereo", out.iter().any(|s| s[0].abs() > 0.01));
        ...
        check(&mut pass, &mut fail, "Quantum stereo decorrelates", (l - r).abs() > 0.001);
        ...
        check(&mut pass, &mut fail, "Phi optimizer modulates state", s.prediction_error > 0.1);

        // Audio feedback
        use symthaea_muse::audio_feedback::AudioFeedbackEncoder;
        let mut fb = AudioFeedbackEncoder::new();
        let chunk: Vec<[f32; 2]> = (0..1024).map(|i| {
            let s = (i as f32 * 0.1).sin() * 0.5;
            [s, s]
        }).collect();
        let features = fb.extract(&chunk, 44100);
        check(&mut pass, &mut fail, "Audio feedback extracts features", features.rms_energy > 0.0);
        ...
        check(&mut pass, &mut fail, "Timbre space returns partials", !partials.is_empty());

        // Collaborative
        use symthaea_muse::collaborative::{CollaborativeComposer, BlendStrategy, HarmonyContribution};
        let mut composer = CollaborativeComposer::new(BlendStrategy::PhiWeighted);
        composer.contribute(HarmonyContribution { participant_id: 1, harmonies: [0.8; 8], phi: 0.9, cycle: 0 });
        composer.contribute(HarmonyContribution { participant_id: 2, harmonies: [0.3; 8], phi: 0.3, cycle: 0 });
        let collective = composer.blend();
        check(&mut pass, &mut fail, "Collaborative blends participants", collective.consciousness_level != 0.5);
        ...
        check(&mut pass, &mut fail, "Spectral vocoder renders audio", chunk.iter().any(|s| s[0].abs() > 0.0001));
    }
    println!();
...
fn check(pass: &mut u32, fail: &mut u32, name: &str, ok: bool) {
    if ok { println!("  [PASS] {name}"); *pass += 1; }
