...
fn compute_correlation(x: &[f64], y: &[f64]) -> f64 {
    ...
    if x_std == 0.0 || y_std == 0.0 { return 0.0; }
...
fn compute_nonlinearity(x: &[f64], y: &[f64]) -> f64 {
    ...
    let mut sorted_pairs: Vec<(f64, f64)> = x.iter().zip(residuals.iter())
        .map(|(xi, ri)| (*xi, *ri))
        .collect();
    ...
    if sorted_residuals.len() < 3 { return 0.0; }
...
fn compute_noise_ratio(x: &[f64], y: &[f64]) -> f64 {
    if x.len() < 3 { return 1.0; }

    let x_mean = compute_mean(x);
    let y_mean = compute_mean(y);
    let y_var = compute_std(y, y_mean).powi(2);

    if y_var == 0.0 { return 0.0; }
    ...
    if ss_xx == 0.0 { return 1.0; }

    let slope = ss_xy / ss_xx;
    let intercept = y_mean - slope * x_mean;

    let residual_var: f64 = x.iter().zip(y.iter())
...
fn main() {
    ...
    for pair in pairs {
        ...
        if results[0] { reci_correct += 1; }
        if results[1] { igci_correct += 1; }
        if results[2] { anm_correct += 1; }
        if results[3] { info_correct += 1; }
        if results[4] { majority_correct += 1; }
    ...
    let n = pairs.len();
    println!("  {:15} {:3}/{} ({:.1}%)", "RECI", reci_correct, n, reci_correct as f64 / n as f64 * 100.0);
    println!("  {:15} {:3}/{} ({:.1}%)", "IGCI", igci_correct, n, igci_correct as f64 / n as f64 * 100.0);
    println!("  {:15} {:3}/{} ({:.1}%)", "ANM", anm_correct, n, anm_correct as f64 / n as f64 * 100.0);
    println!("  {:15} {:3}/{} ({:.1}%)", "Info-Theoretic", info_correct, n, info_correct as f64 / n as f64 * 100.0);
    println!("  {:15} {:3}/{} ({:.1}%)", "Majority Voting", majority_correct, n, majority_correct as f64 / n as f64 * 100.0);

    // ERROR CASE ANALYSIS
    println!("\n┌──────────────────────────────────────────────────────────────────────────┐");
    println!("│ ERROR CASES ({} pairs where Majority Voting fails)                       │", error_cases.len());
    println!("└──────────────────────────────────────────────────────────────────────────┘\n");
    ...
    for (pair_id, meta, results) in &error_cases {
        ...
        if results[0] { rescuers.push("RECI"); rescue_counts[0] += 1; }
        if results[1] { rescuers.push("IGCI"); rescue_counts[1] += 1; }
        if results[2] { rescuers.push("ANM"); rescue_counts[2] += 1; }
        if results[3] { rescuers.push("Info"); rescue_counts[3] += 1; }

        println!("  Pair {:>4}: n={:>5}, corr={:>6.2}, nonlin={:.3}, noise={:.3}  Rescuers: {:?}",
            pair_id, meta.n_samples, meta.correlation, meta.nonlinearity_score, meta.noise_ratio, rescuers);
    }

    println!("\n  RESCUE POTENTIAL:");
    println!("  {:15} rescues {}/{} error cases", "RECI", rescue_counts[0], error_cases.len());
    println!("  {:15} rescues {}/{} error cases", "IGCI", rescue_counts[1], error_cases.len());
    println!("  {:15} rescues {}/{} error cases", "ANM", rescue_counts[2], error_cases.len());
    println!("  {:15} rescues {}/{} error cases", "Info-Theoretic", rescue_counts[3], error_cases.len());
    ...
    println!("  Unrecoverable (all methods wrong):  {}", unrecoverable_pairs.len());
    ...
    let avg_correct_n: f64 = correct_cases.iter().map(|(_, m, _)| m.n_samples as f64).sum::<f64>() / correct_cases.len().max(1) as f64;
    let avg_error_n: f64 = error_cases.iter().map(|(_, m, _)| m.n_samples as f64).sum::<f64>() / error_cases.len().max(1) as f64;

    let avg_correct_corr: f64 = correct_cases.iter().map(|(_, m, _)| m.correlation.abs()).sum::<f64>() / correct_cases.len().max(1) as f64;
    let avg_error_corr: f64 = error_cases.iter().map(|(_, m, _)| m.correlation.abs()).sum::<f64>() / error_cases.len().max(1) as f64;

    let avg_correct_nonlin: f64 = correct_cases.iter().map(|(_, m, _)| m.nonlinearity_score).sum::<f64>() / correct_cases.len().max(1) as f64;
    let avg_error_nonlin: f64 = error_cases.iter().map(|(_, m, _)| m.nonlinearity_score).sum::<f64>() / error_cases.len().max(1) as f64;

    let avg_correct_noise: f64 = correct_cases.iter().map(|(_, m, _)| m.noise_ratio).sum::<f64>() / correct_cases.len().max(1) as f64;
    let avg_error_noise: f64 = error_cases.iter().map(|(_, m, _)| m.noise_ratio).sum::<f64>() / error_cases.len().max(1) as f64;

    println!("  Feature            Correct Cases    Error Cases    Difference");
    println!("  ─────────────────────────────────────────────────────────────────");
    println!("  Sample size        {:8.1}         {:8.1}       {:+.1}", avg_correct_n, avg_error_n, avg_error_n - avg_correct_n);
    println!("  |Correlation|      {:8.3}         {:8.3}       {:+.3}", avg_correct_corr, avg_error_corr, avg_error_corr - avg_correct_corr);
    println!("  Nonlinearity       {:8.3}         {:8.3}       {:+.3}", avg_correct_nonlin, avg_error_nonlin, avg_error_nonlin - avg_correct_nonlin);
    println!("  Noise ratio        {:8.3}         {:8.3}       {:+.3}", avg_correct_noise, avg_error_noise, avg_error_noise - avg_correct_noise);
    ...
    println!("  Oracle accuracy (perfect method selection): {}/{} ({:.1}%)",
        oracle_correct, n, oracle_correct as f64 / n as f64 * 100.0);
    println!("  Current best (Majority Voting):             {}/{} ({:.1}%)",
        majority_correct, n, majority_correct as f64 / n as f64 * 100.0);
    ...
    if !unrecoverable_pairs.is_empty() {
        ...
        for (pair_id, meta) in &unrecoverable_pairs {
            println!("  Pair {:>4}: n={:>5}, corr={:>6.2}, nonlin={:.3}, noise={:.3}",
                pair_id, meta.n_samples, meta.correlation, meta.nonlinearity_score, meta.noise_ratio);
        }
    }
    ...
    for (pair_id, meta, results) in &error_cases {
        ...
        if votes_against_majority >= 2 {
            println!("  Pair {:>4}: {}/{} methods DISAGREE with majority (rescuable)",
