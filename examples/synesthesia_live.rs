...
fn main() {
    ...
    loop {
        ...
        let bass_amp = if is_drop {
            0.8
        } else {
            (song_time * 0.2_f32).sin().abs() * 0.5
        };
        ...
        if (t as u64) % (1000 / RENDER_FPS) == 0 {
            ...
            let vibe = if phi > 0.15 {
                // Lowered threshold for stability
                if is_drop {
                    Vibe::Majesty
                } else if root_act > 0.3 {
                    Vibe::DeepFlow
                } else {
                    Vibe::Joy
                }
            ...
            println!(
                "  ║  Φ (Integration): \x1b[36m{:<40}\x1b[0m {:0.3} ║",
                "█".repeat(phi_len),
                phi
            );
            println!(
                "  ║  ROOT (Structure): \x1b[34m{:<40}\x1b[0m {:0.2} ║",
                "▓".repeat((root_act * 40.0) as usize),
                root_act
            );
            println!(
                "  ║  LEAF (Detail):    \x1b[32m{:<40}\x1b[0m {:0.2} ║",
                "░".repeat((leaf_act * 40.0) as usize),
                leaf_act
            );
            ...
            println!(
                "  ║  > RATE:   {:0.2}x (Speech Speed)                             ║",
                pacing.rate
            );
            println!(
                "  ║  > PAUSE:  {:0.2}s (Breath Gap)                               ║",
            ...
            println!("\n  EVENT LOG:");
            if is_drop {
                println!("  > \x1b[31mTHE DROP (Maintaining Coherence...)\x1b[0m");
            } else {
                println!("  > \x1b[32mRESONATING WITH INPUT\x1b[0m");
            }
        }
        if t > 30_000.0 {
            break;
