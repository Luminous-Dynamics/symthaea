...
//! PARADOX-A0R-F0 research-only, label-blind Plane-F transport worker.
...
const REQUEST_SCHEMA: &str = "PARADOX-A0R-F0-WORKER-REQUEST-V1";
const RESPONSE_SCHEMA: &str = "PARADOX-A0R-F0-WORKER-RESPONSE-V1";
...
const FORBIDDEN_KEY_FRAGMENTS: &[&str] = &[
    "label", "condition", "expected_response", "oracle", "capability_atom",
    "score", "split", "probe", "prediction", "preprocessing", "semantic_fixture",
];
...
fn run() -> Result<(), String> {
    ...
    for (line_index, line) in stdin.lock().lines().enumerate() {
        ...
        stdout.flush().map_err(|e| format!("STDOUT_FLUSH_FAILURE:{e}"))?;
    }
    Ok(())
}
    ...
    match value {
        serde_json::Value::Object(map) => {
            for (key, child) in map {
                ...
                if FORBIDDEN_KEY_FRAGMENTS.iter().any(|x| lower.contains(x)) {
...
fn parse_request(value: &serde_json::Value) -> Result<Request, String> {
    let obj = value.as_object().ok_or_else(|| "REQUEST_MUST_BE_OBJECT".to_string())?;
    let allowed: BTreeSet<&str> = ALLOWED_REQUEST_KEYS.iter().copied().collect();
    if obj.keys().any(|key| !allowed.contains(key.as_str())) {
        return Err("REQUEST_KEY_FORBIDDEN".into());
    }
    if ALLOWED_REQUEST_KEYS.iter().any(|key| !obj.contains_key(*key)) {
        return Err("REQUEST_KEY_MISSING".into());
    }
    ...
    let agent_visible_events = obj["agent_visible_events"]
        .as_array()
    ...
    if agent_visible_events.is_empty() {
        return Err("AGENT_VISIBLE_EVENTS_EMPTY".into());
    }
    let measurement_cycle_index = require_u64(obj, "measurement_cycle_index")? as usize;
    if measurement_cycle_index >= agent_visible_events.len() {
        return Err("MEASUREMENT_CYCLE_OUT_OF_RANGE".into());
    }

    Ok(Request {
        opaque_measurement_id: require_str(obj, "opaque_measurement_id")?.to_owned(),
        opaque_base_fixture_id: require_str(obj, "opaque_base_fixture_id")?.to_owned(),
        opaque_transform_id: require_str(obj, "opaque_transform_id")?.to_owned(),
        technical_replicate_index: require_u64(obj, "technical_replicate_index")?,
...
fn require_str<'a>(
    obj: &'a serde_json::Map<String, serde_json::Value>, key: &str
) -> Result<&'a str, String> {
    obj.get(key).and_then(serde_json::Value::as_str)
        .ok_or_else(|| format!("REQUEST_STRING_REQUIRED:{key}"))
}
fn require_u64(
    obj: &serde_json::Map<String, serde_json::Value>, key: &str
) -> Result<u64, String> {
...
fn require_opaque_id(
    ...
    if value.is_empty() || value.len() > 128 {
        return Err(format!("REQUEST_OPAQUE_ID_INVALID:{key}"));
    }
    Ok(value.to_owned())
}
fn require_hex_id(
    obj: &serde_json::Map<String, serde_json::Value>, key: &str, len: usize
) -> Result<String, String> {
    let value = require_str(obj, key)?;
...
fn execute_request(request: &Request) -> Result<serde_json::Value, String> {
    ...
    let mut service = CognitiveLoopService::new(frozen_config())
        .map_err(|e| format!("SERVICE_CONSTRUCTION_FAILURE:{e}"))?;
    ...
    let recurrent_nonfinite_count = result.output.iter().filter(|v| !v.is_finite()).count();
    let thought_nonfinite_count =
        result.thought_vector.iter().filter(|v| !v.is_finite()).count();
    let recurrent_all_zero = result.output.iter().all(|v| v.to_bits() == 0);
    ...
    let validity = if invalidity_reasons.is_empty() {
    ...
    let feature_bundle_sha256 = hash_frames(
        b"PARADOX-A0R-F0-FEATURE-BUNDLE-V1",
        &[
    ...
    Ok(serde_json::json!({
        ...
        "raw_feature_bundle": {
            ...
            "sealed_feature_bundle_sha256": feature_bundle_sha256.as_str(),
        },
        "scientific_payload": {
...
fn canonical_scientific_payload(
    ...
    push_frame(&mut out, "measurement_cycle_index",
    ...
    push_frame(&mut out, "recurrent_nonfinite_count",
    ...
    push_frame(&mut out, "thought_nonfinite_count",
               &(thought_nonfinite_count as u64).to_le_bytes());
    push_frame(&mut out, "wisdom_byte_length",
               &(wisdom_byte_length as u64).to_le_bytes());
    push_frame(&mut out, "wisdom_sha256", wisdom_sha256.as_bytes());
    push_frame(&mut out, "feature_bundle_sha256", feature_bundle_sha256.as_bytes());
    ...
    push_frame(&mut out, "measurement_validity", validity.as_bytes());
...
fn canonical_provenance_envelope(
    ...
    push_frame(&mut out, "technical_replicate_index",
