...
//! PARADOX-A0R-F0 V3 research-only, label-blind Plane-F transport worker.
...
const REQUEST_SCHEMA: &str = "PARADOX-A0R-F0-WORKER-REQUEST-V3";
...
fn run() -> Result<(), String> {
    ...
    for (line_index, line) in stdin.lock().lines().enumerate() {
        ...
        // Typed serde decoding + deny_unknown_fields deliberately happens in one
        // step. Duplicate known fields are rejected by the derived struct visitor;
        // unknown/semantic keys are rejected before any cognitive service exists.
...
fn execute_request(request: &Request) -> Result<serde_json::Value, String> {
    ...
    // Provenance-only randomness is created only after all scientific bytes and
    // the scientific commitment have been sealed.
