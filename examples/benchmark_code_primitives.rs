...
use symthaea::consciousness::code_primitives::{
    CodePrimitiveRouter, CodePrimitiveExecutor, CodeOperation,
};
use symthaea::meta::self_analysis::SelfAnalyzer;
use symthaea::meta::code_health::CodeHealthScanner;
use symthaea::language::code_parser::{CodeEntity, EntityKind, ParsedCode, Span, CodeParser};
...
fn benchmark_primitive_router() {
    ...
    for op in &operations {
        ...
        println!("  {:?}: {} ns/op ({} primitives)",
            op, per_op, router.select_primitives(*op).len());
    }

    let avg_per_op = total_time.as_nanos() / (iterations * operations.len() as u128);
    println!("\n📊 Average primitive selection: {} ns/op", avg_per_op);
    println!("   Total operations tested: {}", operations.len() * iterations as usize);
}
...
fn benchmark_phi_scores() {
    ...
    for (name, op) in &operations {
        ...
        println!("  {:12} Φ={:.4} |{}{}| {} primitives",
            name, result.phi, bar, empty, result.primitives.len());
    }
    ...
    let highest = phi_scores.iter().max_by(|a, b| a.1.partial_cmp(b.1).unwrap());
    let lowest = phi_scores.iter().min_by(|a, b| a.1.partial_cmp(b.1).unwrap());
...
fn benchmark_task_detection() {
    ...
    let test_cases = [
        ...
        ("Refactor the parse_input function to be more efficient", true),
        ("What is 2 + 2?", false),
    ...
    println!("\n📊 Detection accuracy: {:.1}% ({}/{})",
        accuracy, correct, test_cases.len());
}
...
fn benchmark_code_generator() {
    ...
    let intents = [
        ("Rust function", CodeIntent::Create {
            ...
            spec: CodeSpec::new("rust", "binary_search", "Binary search in a sorted array")
                .with_signature("fn binary_search<T: Ord>(arr: &[T], target: &T) -> Option<usize>")
                .with_epistemic(EpistemicStatus::Certain),
        }),
        ("Python class", CodeIntent::Create {
        ...
        ("Debug request", CodeIntent::Debug {
            target: CodeTarget::new("parse_config", EntityKind::Function)
                .with_path("src/config.rs"),
            symptoms: vec!["Panic on empty input".to_string(), "Wrong default values".to_string()],
        }),
    ];
    ...
    for (name, intent) in &intents {
        ...
        println!("    Source preview: {}...",
            result.source.lines().next().unwrap_or("[empty]").chars().take(50).collect::<String>());
        println!();
    }
}
...
fn benchmark_self_analysis() {
    ...
    let modules = [
        ("src/consciousness/code_primitives.rs", vec!["CodePrimitiveRouter", "CodePrimitiveExecutor", "execute"], vec!["CodeExecutionResult"]),
        ("src/hdc/code_encoder.rs", vec!["encode_entity", "encode_function", "encode_module"], vec!["CodeHDEncoder"]),
        ("src/language/code_generator.rs", vec!["generate", "generate_create", "generate_modify"], vec!["CodeGenerator", "GeneratedCode"]),
        ("src/meta/self_analysis.rs", vec!["introspect_module", "consciousness_map", "operation_phi_scores"], vec!["SelfAnalyzer", "ModuleInsight"]),
        ("src/cognitive_loop/routing.rs", vec!["route", "detect"], vec!["ThalamicRouter", "CodeTaskDetector"]),
    ];
    ...
    println!("  Indexed {} modules in {:?}", model.module_count, index_time);
    println!("  Functions: {}, Types: {}", model.function_count, model.type_count);
    ...
    if let Some(least) = insights.first() {
        println!("    Least integrated: {} (Φ={:.4})", least.path.display(), least.integration);
    }
    if let Some(most) = insights.last() {
        println!("    Most integrated:  {} (Φ={:.4})", most.path.display(), most.integration);
    }
}
...
fn make_mock_parsed(funcs: &[&str], types: &[&str]) -> ParsedCode {
    ...
    for f in funcs {
        parsed.entities.push(CodeEntity::new(EntityKind::Function, *f, span.clone()));
    }
    for t in types {
        parsed.entities.push(CodeEntity::new(EntityKind::Struct, *t, span.clone()));
    }
    parsed
}
...
fn benchmark_cross_tier() {
    ...
    for (name, op) in &operations {
        ...
        println!("    {:12} Code Φ={:.4}, Cross Φ={:.4}, Combined Φ={:.4} ({:?})",
    ...
    let consciousness_phis: Vec<f32> = operations.iter()
        .map(|(_, op)| executor.execute_with_consciousness(*op).combined_phi)
        .collect();
    let metacog_phis: Vec<f32> = operations.iter()
        .map(|(_, op)| executor.execute_with_metacognitive(*op).combined_phi)
        .collect();

    let avg_consciousness = consciousness_phis.iter().sum::<f32>() / consciousness_phis.len() as f32;
...
fn benchmark_code_health() {
    ...
    let test_cases = [
        ("Well-structured module", vec![
            ("parse_config", "Config"),
            ("validate_config", "Config"),
            ("load_config", "Config"),
            ("save_config", "Config"),
        ]),
        ("Mixed cohesion module", vec![
            ("parse_json", "JsonData"),
        ...
        ("Complex module", vec![
            ("init", "State"), ("process", "State"), ("validate", "State"),
            ("transform", "Output"), ("serialize", "Bytes"), ("compress", "Bytes"),
            ("encrypt", "Encrypted"), ("send", "Response"), ("log", "LogEntry"),
            ("cleanup", "Result"),
        ]),
    ];
    ...
    for (name, entities) in &test_cases {
        ...
        if !health.suggestions.is_empty() {
            println!("    Suggestions: {:?}", &health.suggestions[..health.suggestions.len().min(2)]);
        }
        println!();
    }
...
fn make_test_parsed(entities: &[(&str, &str)]) -> ParsedCode {
    ...
    for (func_name, type_name) in entities {
        parsed.entities.push(CodeEntity::new(EntityKind::Function, *func_name, span.clone()));
        parsed.entities.push(CodeEntity::new(EntityKind::Struct, *type_name, span.clone()));
    }
    parsed
}
...
fn benchmark_real_parsing() {
    ...
    for file_path in &test_files {
        ...
        match std::fs::read_to_string(full_path) {
            Ok(source) => {
                ...
                match parser.parse(&source) {
                    Ok(parsed) => {
                        ...
                        let funcs = parsed.entities.iter()
                            .filter(|e| matches!(e.kind, EntityKind::Function))
                            .count();
                        let types = parsed.entities.iter()
                            .filter(|e| matches!(e.kind, EntityKind::Struct | EntityKind::Enum | EntityKind::Trait))
                            .count();

                        println!("  {}:", file_path);
                        println!("    {} LOC, {} entities ({} funcs, {} types)", loc, entities, funcs, types);
                        println!("    Parse: {:?}, Encode: {:?}", parse_time, encode_time);
                        println!("    HDC dim: {}, norm: {:.4}", module_hv.dim(), module_hv.norm());
                        println!();
                    }
                    Err(e) => {
            ...
            Err(_) => {
                println!("  {}: File not found (running from wrong directory?)", file_path);
            }
        }
    }
    ...
    match parser.parse_with_primitives(test_source, 16384) {
        Ok(result) => {
            ...
            println!("    Primitives used: {:?}",
                result.primitive_result.primitives.iter()
