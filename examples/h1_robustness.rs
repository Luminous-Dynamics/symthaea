...
fn run_robustness_analysis() -> Result<()> {
    ...
    let qualia_hvs: Vec<_> = qualia_concepts.iter()
        .map(|c| probe.concept_to_hv(c).unwrap())
        .collect();
    let comp_hvs: Vec<_> = computation_concepts.iter()
        .map(|c| probe.concept_to_hv(c).unwrap())
        .collect();
    ...
    let param_configs = vec![
        ("default", TopologyConfig { min_persistence: 0.05, max_scale: 1.0, num_scales: 20, detect_cycles: true, detect_voids: true }),
        ("loose", TopologyConfig { min_persistence: 0.01, max_scale: 1.0, num_scales: 20, detect_cycles: true, detect_voids: true }),
        ("strict", TopologyConfig { min_persistence: 0.10, max_scale: 1.0, num_scales: 20, detect_cycles: true, detect_voids: true }),
        ("fine", TopologyConfig { min_persistence: 0.05, max_scale: 1.0, num_scales: 50, detect_cycles: true, detect_voids: true }),
        ("coarse", TopologyConfig { min_persistence: 0.05, max_scale: 1.0, num_scales: 10, detect_cycles: true, detect_voids: true }),
    ];
    ...
    for (name, config) in &param_configs {
        ...
        println!("{:8}: Qualia={:.4}, Comp={:.4}, Diff={:.4}, d={:.3}",
                 name, mean(&qualia_unity), mean(&comp_unity), diff, cohens_d);

        param_results.push((name.to_string(), mean(&qualia_unity), mean(&comp_unity), diff, cohens_d));
    }
    ...
    for (i, concept) in qualia_concepts.iter().enumerate() {
        writeln!(csv_file, "\"{}\",phenomenal,{:.4}", concept, qualia_unity[i])?;
    }
    for (i, concept) in computation_concepts.iter().enumerate() {
        writeln!(csv_file, "\"{}\",computational,{:.4}", concept, comp_unity[i])?;
    }
    ...
    let qualia_med = qualia_unity.iter().filter(|&&x| x >= 0.5 && x < 0.9).count();
    ...
    println!("  Phenomenal:    High={}, Med={}, Low={}", qualia_high, qualia_med, qualia_low);
    println!("  Computational: High={}, Med={}, Low={}", comp_high, comp_med, comp_low);
    ...
    let pooled_std = ((std(&qualia_unity).powi(2) + std(&comp_unity).powi(2)) / 2.0).sqrt();
    let cohens_d = if pooled_std > 0.0 { observed_diff / pooled_std } else { 0.0 };
    ...
    println!("  Range: [{:.4}, {:.4}]",
             ...
             qualia_unity.iter().cloned().fold(f64::NEG_INFINITY, f64::max));
    ...
    println!("  Min d: {:.3}", all_d.iter().cloned().fold(f64::INFINITY, f64::min));
    println!("  Max d: {:.3}", all_d.iter().cloned().fold(f64::NEG_INFINITY, f64::max));
