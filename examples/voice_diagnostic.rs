...
fn main() {
    ...
    if let Some(f) = frames.first() {
        println!("First frame: f0={:.1} f1={:.1} f2={:.1} f3={:.1} energy={:.3} voicing={:.1}",
            f.f0, f.f1, f.f2, f.f3, f.energy, f.voicing);
    }
    if let Some(f) = frames.get(frames.len() / 2) {
        println!("Mid frame: f0={:.1} f1={:.1} f2={:.1} f3={:.1} energy={:.3} voicing={:.1}",
            f.f0, f.f1, f.f2, f.f3, f.energy, f.voicing);
    }
    ...
    println!("Non-zero samples: {} ({:.1}%)", non_zero, non_zero as f32 / samples.len() as f32 * 100.0);
