...
fn run_experiment() -> Result<()> {
    ...
    println!("\nStatistical Tests:");
    println!("  Phen+Phen vs Phen+Func: p = {:.4}{}", p_pp_vs_pf, if p_pp_vs_pf < 0.05 { " *" } else { "" });
    println!("  Phen+Phen vs Func+Func: p = {:.4}{}", p_pp_vs_ff, if p_pp_vs_ff < 0.05 { " *" } else { "" });

    // Interpretation
    println!("\nInterpretation:");
    if mean_pp > mean_pf && p_pp_vs_pf < 0.05 {
        println!("  ✓ PREDICTION CONFIRMED: Cross-class binding compresses LESS");
        println!("    Phen+Phen compression ({:+.4}) > Phen+Func ({:+.4})", mean_pp, mean_pf);
        ...
        println!("    But difference not statistically significant (p={:.4})", p_pp_vs_pf);
    } else {
    ...
    println!("────────────────────────────────────────");
    println!("  Phen+Phen: {:.4} ± {:.4}", mean(&phen_indices), std_dev(&phen_indices));
    println!("  Func+Func: {:.4} ± {:.4}", mean(&func_indices), std_dev(&func_indices));
    println!("  Phen+Func: {:.4} ± {:.4}", mean(&cross_indices), std_dev(&cross_indices));
    ...
    let overall_accuracy = (phen_correct + func_correct) as f64 / (phen_indices.len() + func_indices.len()) as f64;

    println!("  Phen+Phen classification: {}/{} ({:.1}%)", phen_correct, phen_indices.len(), phen_accuracy * 100.0);
    println!("  Func+Func classification: {}/{} ({:.1}%)", func_correct, func_indices.len(), func_accuracy * 100.0);
    ...
    for i in 0..phen_hvs.len().min(20) {
        ...
        for j in 0..phen_hvs.len().min(20) {
            if i != j {
                scores.push(compute_binding_compression(&phen_hvs[i], &phen_hvs[j], &topology_config));
            }
        }
    ...
    for i in 0..func_hvs.len().min(20) {
        ...
        for j in 0..func_hvs.len().min(20) {
            if i != j {
                scores.push(compute_binding_compression(&func_hvs[i], &func_hvs[j], &topology_config));
            }
        }
    ...
    if overall_accuracy > 0.65 {
        println!("  ✓ VALID: Can distinguish phenomenal from functional pairs ({:.1}%)", overall_accuracy * 100.0);
    } else {
        println!("  ◐ WEAK: Classification accuracy below threshold ({:.1}%)", overall_accuracy * 100.0);
    }
