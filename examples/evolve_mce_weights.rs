...
impl Genome {
    ...
    fn mutate(&self, rng: &mut ChaCha8Rng) -> Self {
        ...
        for _ in 0..n_mutate {
            ...
            vals[idx] = (vals[idx] + rng.gen_range(-MUTATION_SIGMA..MUTATION_SIGMA)).clamp(0.01, 0.30);
        }
...
fn evaluate_fitness(genome: &Genome) -> f64 {
    ...
    let results: Vec<f64> = profiles.iter()
        .map(|(_, input, _)| {
    ...
    let mut disc = 0u32;
    if results[0] > results[2] { disc += 1; } // fully > text
    if results[2] > results[1] { disc += 1; } // text > cold_start
    if results[3] > results[5] { disc += 1; } // flow > dreaming
    if results[4] < results[1] { disc += 1; } // unconscious < cold_start
    if results[5] > results[4] { disc += 1; } // dreaming > unconscious
    if results[0] > 0.7 { disc += 1; }        // fully > 0.7
    ...
    for gen in 1..=NUM_GENERATIONS {
        ...
        if gen % 10 == 0 || gen == NUM_GENERATIONS {
            ...
            println!("Gen {:2}: best={:.4}  mean={:.4}",
                gen, pop[0].1, pop.iter().map(|p| p.1).sum::<f64>() / POPULATION_SIZE as f64);
        }
    }
