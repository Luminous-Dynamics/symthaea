...
async fn main() {
    ...
    handle.input(symthaea::mind::MindInput::new(
    ...
    println!("   -> Current Thought Dimension: {}", state.current_thought.dim());
    ...
    sleep(Duration::from_millis(100)).await;
    let state = handle.snapshot().await;
    println!("   -> Thought Amplitude in Ultra: {:.4}", state.current_thought.norm());
