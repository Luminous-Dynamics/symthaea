...
// Commercial licensing: see COMMERCIAL_LICENSE.md at repository root//! Layer-wise Topology Analysis - Full Corpus
//!
//! Validates the Layer 18 effect using all 100 concepts (50 phenomenal, 50 functional)
//! with statistical significance testing at each layer.
//!
//! ## Usage
//!
//! ```bash
//! cargo run --example layer_topology_full_corpus --features neural-bridge --release
//! ```
...
fn run_analysis() -> Result<()> {
    println!("\n");
    println!("================================================================");
    println!("   LAYER-WISE TOPOLOGY ANALYSIS - FULL CORPUS");
    println!("   Validating Layer 18 Effect with Statistical Testing");
    println!("================================================================\n");

    // Load concept corpora from JSON files
    let phenomenal_corpus = load_corpus("data/consciousness_probe/phenomenal_concepts.json")?;
    let functional_corpus = load_corpus("data/consciousness_probe/functional_concepts.json")?;

    println!("Loaded corpora:");
    ...
    for (i, layer_idx) in layers_to_analyze.iter().enumerate() {
        // Compute unity scores for all concepts at this layer
        ...
        let sig = if p_value < 0.05 { "*" } else { " " };

        println!(
            "{:5} │ {:10.4} │ {:10.4} │ {:+8.4} │ {:+9.4} │ {:9.4} │  {}",
    ...
    // Find the layer with maximum positive difference
    ...
    for (layer, _, _, _, d, _) in &layer_results {
        ...
        println!("  Layer {:2}: d = {:+.3} ({})", layer, d, interpretation);
    }
...
fn load_corpus(path: &str) -> Result<Vec<String>> {
    ...
    // Try direct "concepts" array first (current format)
    ...
    // Fall back to nested "categories" format
    if concepts.is_empty() {
        if let Some(categories) = json["categories"].as_array() {
            for category in categories {
                if let Some(items) = category["concepts"].as_array() {
                    for item in items {
                        if let Some(text) = item["text"].as_str() {
                            concepts.push(text.to_string());
                        }
