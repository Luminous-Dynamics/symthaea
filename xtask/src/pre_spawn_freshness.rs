...
const RUNTIME_SCHEMA: &str = "symthaea.cargo-runtime-binding.v1";
const MATERIALIZATION_SCHEMA: &str = "symthaea.repository-materialization.v1";
const TOOL_ATTESTATION_SCHEMA: &str = "symthaea.tool-executable-attestation.v1";
...
/// Commit a positive pre-spawn freshness observation for one exact runtime subject.
///
/// This function deliberately does not spawn a process and does not grant authority.
/// The future trusted adapter must populate `observation` from an immediate OS/sandbox
/// re-observation after effect admission and before process spawn.
pub(crate) fn build_receipt(
    ...
    validate_upstream_shapes(&runtime, &materialization, &tool_attestation)?;
    normalize_digest(
...
fn validate_upstream_shapes(
    runtime: &CargoRuntimeBindingReceipt,
    materialization: &RepositoryMaterializationReceipt,
    tool_attestation: &ToolExecutableAttestationReceipt,
) -> anyhow::Result<()> {
    if runtime.schema != RUNTIME_SCHEMA {
        bail!("unexpected runtime-binding schema: {}", runtime.schema);
    }
    if materialization.schema != MATERIALIZATION_SCHEMA {
        bail!("unexpected materialization schema: {}", materialization.schema);
    }
    if tool_attestation.schema != TOOL_ATTESTATION_SCHEMA {
        bail!("unexpected tool-attestation schema: {}", tool_attestation.schema);
    }
...
fn validate_cross_links(
    ...
    if tool_attestation.cargo.reported_version != runtime.context.toolchain.cargo_version
        || tool_attestation.rustc.reported_version != runtime.context.toolchain.rustc_version
        || tool_attestation.cargo.reported_host_triple != runtime.context.toolchain.host_triple
        || tool_attestation.rustc.reported_host_triple != runtime.context.toolchain.host_triple
    {
        bail!("tool executable attestation reports a toolchain inconsistent with runtime binding");
    }
...
mod tests {
    ...
    fn materialization() -> RepositoryMaterializationReceipt {
        RepositoryMaterializationReceipt {
            ...
            schema: MATERIALIZATION_SCHEMA.into(),
    ...
    fn tools() -> ToolExecutableAttestationReceipt {
        ToolExecutableAttestationReceipt {
            ...
            schema: TOOL_ATTESTATION_SCHEMA.into(),
            context_id: digest('3'),
            invocation_id: digest('4'),
    ...
    #[test]
    fn toolchain_report_substitution_rejects_before_freshness() {
        let mut substituted = tools();
        substituted.rustc.reported_version = "rustc 1.95.0".into();
        assert!(expected_projection(&runtime(), &materialization(), &substituted).is_err());
    }
