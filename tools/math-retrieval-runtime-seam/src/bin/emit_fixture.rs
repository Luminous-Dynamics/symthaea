...
//! Deterministic MATH-RET-RUNTIME-001B single-index fixture emitter.
//!
//! This binary exercises the typed Rust execution seam. It emits the frozen
//! MATH-RET-TRACE-001A JSON shape plus a small payload plan. A stdlib Python
//! adapter content-addresses the exact trace/payload bytes and builds the final
//! MATH-RET-PAYLOAD-001A document.
...
use symthaea_math_retrieval_runtime_seam::{
    ...
    ControlBinding, ControlTransform, GraphIdentity, InMemoryEvidenceSink, PayloadAuditBatch,
    PayloadRole, QualifiedRetrievalBackend, QualifiedRetrievalRequest, RetrievalBudget,
    RetrievalError, RetrievalExecutor, RetrievalTraceReceipt, Sha256Digest, SingleIndexExecution,
    StopReason, TraceRetrieval,
};
...
fn parse_usize(m: &BTreeMap<String, String>, key: &str) -> usize {
    required(m, key).parse().unwrap_or_else(|_| panic!("{key}: invalid usize"))
}

fn parse_u64(m: &BTreeMap<String, String>, key: &str) -> u64 {
    required(m, key).parse().unwrap_or_else(|_| panic!("{key}: invalid u64"))
}

fn parse_u32(m: &BTreeMap<String, String>, key: &str) -> u32 {
    required(m, key).parse().unwrap_or_else(|_| panic!("{key}: invalid u32"))
}
...
fn load_config(path: &Path) -> BTreeMap<String, String> {
    let text = fs::read_to_string(path).expect("read adapter config");
    ...
    for (line_no, line) in text.lines().enumerate() {
        ...
        assert!(out.insert(key.clone(), value).is_none(), "duplicate config key: {key}");
    }
    out
}
...
fn serialize_trace(t: &RetrievalTraceReceipt) -> String {
    let retrieval = match &t.retrieval {
        ...
        TraceRetrieval::Fusion { .. } => panic!("001B fixture intentionally targets SingleIndex arm S"),
    };
    ...
    let controls = t
        ...
        .map(|x| {
            ...
            if let Some(d) = &x.control_artifact_sha256 {
                s.push_str(&format!(",\"control_artifact_sha256\":{}", digest_json(d)));
            }
            s.push('}');
            s
        })
...
fn main() {
    ...
    let config_path = PathBuf::from(args.next().expect("usage: emit_fixture <config> <out-dir>"));
    let out_dir = PathBuf::from(args.next().expect("usage: emit_fixture <config> <out-dir>"));
    assert!(args.next().is_none(), "unexpected extra arguments");

    let c = load_config(&config_path);
    fs::create_dir_all(&out_dir).expect("create adapter output directory");

    let ranked = vec![fixed_digest(0xa1), fixed_digest(0xa2), fixed_digest(0xa3)];
    ...
    let mut materializer = FixtureMaterializer {
        payloads: BTreeMap::from([
            (ranked[0].clone(), b"alpha theorem context\n".to_vec()),
            (ranked[1].clone(), b"beta lemma context\n".to_vec()),
            (ranked[2].clone(), b"gamma definition context\n".to_vec()),
        ]),
    };
    let mut sink = InMemoryEvidenceSink::default();

    let request = QualifiedRetrievalRequest {
        trace_id: "math-ret-runtime-001b-fixture-trace".into(),
        audit_id: "math-ret-runtime-001b-fixture-audit".into(),
    ...
    let outcome = RetrievalExecutor::execute(&mut backend, &mut materializer, &mut sink, request)
        .expect("typed runtime fixture must execute");
