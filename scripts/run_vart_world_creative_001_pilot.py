...
def copy_contract(src: Path, dst: Path) -> str:
    if not src.is_file():
        raise PilotError(f"missing contract input: {src}")
...
def validate_cells(cells: Any) -> list[dict[str, Any]]:
    ...
    for cell in cells:
        ...
        if cell["revision_index"] < 0:
            raise PilotError(f"{cell['cell_id']}: revision_index must be nonnegative")
    return cells
...
def tree_closure(root: Path, *, excluded_relpaths: set[str]) -> tuple[str, list[dict[str, str]]]:
    ...
    closure = sha256_bytes(canonical_json_bytes(entries))
...
def main() -> int:
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="verify source/contracts and resolve commands without executing the runtime",
    )
    ...
    write_json(pilot_root / "confirmatory_freeze.json", freeze)
    ...
    verifier_path_value = cfg.get("verifier_path", "scripts/verify_vart_world_creative_001.py")
    verifier_path = resolve_path(verifier_path_value, base=base)
