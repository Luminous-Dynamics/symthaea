...
#
# 2026-07-27 follow-up fix: the working directory this script cloned the
# standalone repo into (STANDALONE_REPO) used to be a single fixed /tmp path,
# reused across invocations and reset via `git reset --hard` under a flock for
# speed. In practice, a concurrent session on this monorepo's shared box ran
# `rm -rf` on that fixed path by hand (to "unstick" what looked like a hung
# run) WHILE another invocation was actively `cd`ed into it running a
# multi-minute nix-based `cargo check` -- flock only serializes invocations of
# this script against each other, it does nothing to stop an external `rm -rf`
# on the path one of them is using. Confirmed live: a ~2-hour export run's
# cargo metadata/fmt/check/hdc-zkp-bench checks ALL failed with an identical
# "No such file or directory" error, entirely because of this, not any defect
# in the exported content. Fixed by making STANDALONE_REPO a unique `mktemp -d`
# per invocation -- there is no longer a shared mutable path to race on, so
# this class of collision is structurally impossible regardless of what any
# other session does by hand. Slower (always a fresh clone, no cached-clone
# reuse) but correct; LOCK_FILE is kept only as an advisory throttle on
# simultaneous full nix rebuilds, not for correctness.
#
# Usage:
...
# Unique per invocation (2026-07-27, fixing a real concurrency incident): this
# used to be a single fixed /tmp path shared across every invocation, reset
# via `git reset --hard` at the start of each run under a flock. In practice,
# concurrent sessions on this monorepo's shared box repeatedly ran `rm -rf` on
# that fixed path by hand (to "unstick" a run they thought was hung) WHILE
# another invocation was actively `cd`ed into it running a multi-minute nix-
# based `cargo check` -- the flock only serializes invocations of this
# script, it does nothing to stop an external `rm -rf` on the path it's using.
# Confirmed live: a ~2-hour export run's cargo metadata/fmt/check/hdc-zkp-bench
# checks ALL failed with the identical "No such file or directory" error,
# because a second session's `rm -rf /tmp/symthaea-standalone-export` fired
# mid-run. A unique directory per invocation makes this class of collision
# structurally impossible -- there is no shared mutable path left to race on,
# so LOCK_FILE below no longer needs to protect it (kept only as a coarse,
# advisory throttle on simultaneous full nix rebuilds under the same heavy
# concurrent-session load, not for correctness).
STANDALONE_REPO="$(mktemp -d /tmp/symthaea-standalone-export-XXXXXX)"
trap 'rm -rf "${STANDALONE_REPO}"' EXIT
...
# Replaces the earlier single-directory trap -- must clean up BOTH temp dirs,
# not just this one, or the STANDALONE_REPO trap set above gets clobbered.
trap 'rm -rf "${EXPORT_TMPDIR}" "${STANDALONE_REPO}"' EXIT
...
#
# STANDALONE_REPO is a fresh mktemp directory every invocation (see top of
# file), so there is no "existing clone to update" branch anymore -- always a
# clean clone. Slower than the old reuse-a-cached-clone design, but immune to
# the concurrency incident that design caused.
...
RSYNC_EXCLUDE=(
    ...
    --exclude='.next/'
    --exclude='dist/'
    # --- Export-bloat gates (added 2026-07-28 after auditing PR #31: the
    # exported tree carried 262MB of tracked content, ~54MB of which was
    # crates/domains/symthaea-muse/UI_mocks/ChatGPT*.png screenshot dumps
    # (design ideation, not source), plus a checked-in compiled ELF release
    # binary under symthaea-scan/releases/. Neither belongs in a flagship
    # standalone repo's source tree. Archive formats are excluded
    # defense-in-depth even though none made it into that particular export.
    --exclude='UI_mocks/'
    --exclude='*.tar.gz'
    --exclude='*.tar'
    --exclude='*.zip'
    --exclude='*.7z'
    --exclude='*.rar'
    --exclude='releases/*-linux-gnu'
    --exclude='releases/*-apple-darwin'
    --exclude='releases/*-pc-windows*'
)
# NOTE: --dry-run does NOT skip writing to STANDALONE_REPO. That directory is
# a disposable, unique-per-invocation /tmp scratch clone -- writing to it (and
# running cargo checks against it) carries zero risk to the real public repo.
# The only step --dry-run actually gates is the final
