...
EXPECTED_TOP_LEVEL_KEYS = {
    ...
    "destination_repository_status",
    "qualification_transfer_policy",
    "host_pin",
    "recovery_source",
    "artifacts",
    "findings",
    "required_pre_extraction_repairs",
}
...
EXPECTED_REPAIR_KEYS = {"id", "status", "statement"}
...
EXPECTED_REPAIRS = {f"REPAIR-{n:03d}" for n in range(1, 6)}
...
def validate(source: dict[str, Any], migration: dict[str, Any], parity: dict[str, Any]) -> list[str]:
    errors: list[str] = []
    require_exact_keys(source, EXPECTED_TOP_LEVEL_KEYS, "top-level source audit", errors)
    ...
    if source.get("$schema") != "./spore-product-source-lineage-v1.schema.json":
        errors.append("$schema must remain bound to the shipped v1 schema")
    ...
    for index, item in enumerate(artifacts):
        if not isinstance(item, dict):
            ...
            require_exact_keys(item, EXPECTED_ARTIFACT_KEYS, f"artifact[{index}]", errors)
    by_id = {
    ...
    for index, item in enumerate(findings):
        ...
        expected_keys = {"id", "severity", "status", "statement"}
        if finding_id in {"SRC-003", "SRC-004", "SRC-005"}:
            expected_keys.add("tracking_issue")
        require_exact_keys(item, expected_keys, f"finding[{index}]", errors)
    finding_by_id = {
    ...
    if not isinstance(src004_statement, str) or not all(
        ...
        for phrase in (
            ...
            "locally observed machine facts",
            "deserializable data",
        )
    ):
        errors.append(
    ...
    src005 = finding_by_id.get("SRC-005", {})
    if src005.get("tracking_issue") != "Luminous-Dynamics/luminous-dynamics#61":
        errors.append("SRC-005 must remain bound to exact lifecycle evidence issue #61")
    src005_statement = src005.get("statement", "")
    if not isinstance(src005_statement, str) or not all(
        phrase in src005_statement
        for phrase in (
            "CleanShutdownMarker",
            "boot_id",
            "prepared boot counter",
            "previously committed active prepared boot subject",
        )
    ):
        errors.append("SRC-005 must retain the generation-only lifecycle evidence defect")
    ...
    for index, item in enumerate(repairs):
        if not isinstance(item, dict):
            errors.append(f"repair[{index}] must be object")
        else:
            require_exact_keys(item, EXPECTED_REPAIR_KEYS, f"repair[{index}]", errors)
    repair_by_id = {
    ...
    if not isinstance(repair002_statement, str) or not all(
        ...
        for phrase in (
            ...
            "locally observed kernel/boot filesystem facts",
            "caller-selected authority values",
            "non-deserializable capabilities",
            "serialized reports",
    ...
    repair005 = repair_by_id.get("REPAIR-005", {})
    repair005_statement = repair005.get("statement", "")
    if not isinstance(repair005_statement, str) or not all(
        phrase in repair005_statement
        for phrase in (
            "shutdown, reboot, suspend, and hibernate",
            "exact locally committed prepared boot subject",
            "cannot mint or select boot_id or counter authority",
            "legacy lifecycle markers",
            "never fabricate exact-subject provenance",
        )
    ):
        errors.append("REPAIR-005 must preserve exact lifecycle subject authority and legacy provenance")
...
def validate_files() -> list[str]:
    load(SOURCE_SCHEMA_PATH)  # Parse the shipped schema; semantic enforcement is dependency-free above.
...
def main() -> int:
    ...
    print("pre_extraction_repairs=5")
