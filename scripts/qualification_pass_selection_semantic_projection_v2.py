...
import qualification_input_closure_v1 as closure_v1
...
SCHEMA = "symthaea.qualification-pass-selection-semantic-projection.v2"
SOURCE_SCHEMA = "symthaea.qualification-pass-selection.v2"
ID_DOMAIN = "symthaea.qualification-pass-selection-semantic-projection-id.v2"
_SHA256_ID = re.compile(r"^sha256:[0-9a-f]{64}$")
NON_CLAIMS = sorted([
    "does not reinterpret or replace the historical PassSelectionV2 identity",
    "does not establish that historical InputClosureId semantics equal the reconstructed normative input closure",
    "does not migrate AttemptHistoryId, QualificationEnvironmentId, or attempt-subject identities",
...
class SemanticProjectionV2Error(train.TrainManifestError):
    pass


def _require_sha256_id(value: Any, *, where: str) -> str:
    if not isinstance(value, str) or _SHA256_ID.fullmatch(value) is None:
        raise SemanticProjectionV2Error(f"{where}: expected sha256:<64 lowercase hex>")
    return value


def _require_exact_keys(value: Any, required: set[str], *, where: str) -> dict[str, Any]:
    if not isinstance(value, dict):
        raise SemanticProjectionV2Error(f"{where}: expected object")
    keys = set(value)
    missing = sorted(required - keys)
    unknown = sorted(keys - required)
    if missing:
        raise SemanticProjectionV2Error(f"{where}: missing fields: {missing}")
    if unknown:
        raise SemanticProjectionV2Error(f"{where}: unknown fields: {unknown}")
    return value
...
def project_selection(*, selection: Any, subject: Any, profile: Any, recipes: list[Any]) -> dict[str, Any]:
    ...
    if selection["schema"] != SOURCE_SCHEMA:
        raise SemanticProjectionV2Error(f"pass selection V2.schema: expected {SOURCE_SCHEMA!r}")
    ...
    if selection_v2._selection_id(source_payload) != declared_selection_id:
        raise SemanticProjectionV2Error(
    ...
    if not isinstance(selected, list) or not selected:
        raise SemanticProjectionV2Error(
            "pass selection V2.selected_recipe_attempts: expected non-empty array"
        )

    observed_legacy_refs: list[str] = []
    ...
    for index, item in enumerate(selected):
        ...
        if recipe is None:
            raise SemanticProjectionV2Error(
                "pass selection V2 selected legacy recipe is not resolved by exact recipe "
                f"preimages: {legacy_ref}"
            )
        observed_legacy_refs.append(legacy_ref)
        mappings.append({
            "legacy_recipe_ref": legacy_ref,
            "recipe_id": _require_sha256_id(
                recipe["recipe_id"], where=f"resolved recipe {legacy_ref}.recipe_id"
            ),
    ...
    normative_closure = closure_v1.build_from_subject(normalized_subject)
    ...
    normalized = {
        ...
        "qualification_input_closure_id_v1": _require_sha256_id(
            normative_closure["input_closure_id"],
            where="qualification input closure.input_closure_id",
        ),
