...
R1 amendment: StateApplicationReceipt binds both a stable operation_id and the
exact operand that actually crossed the Phase-C application boundary. This is
needed because several production paths cast integrated f64 values to f32.

Measurement-only. This script does not execute cognition and grants no authority.
"""
...
def canonical_value(value: dict) -> bytes:
    ...
    encoded = bytearray(u8(VALUE_TAG[kind]))
    raw = value.get("value")
    if kind == "F64_BITS":
        encoded += u64(raw)
    elif kind == "F32_BITS":
...
def application_bytes(record: dict) -> bytes:
    ...
    if source != "FLAG" and source_flag != 0:
...
def self_test() -> None:
    ...
    # Application semantic indices are load-bearing.
    ...
    # Operation identity is also canonical.
    ...
    # Presentation format is not canonical evidence.
    ...
    # Timing is deliberately external to canonical receipt bytes.
...
def main() -> int:
    ...
    if args.self_test:
        self_test()
        print("SPINE-000B receipt commitment oracle R1 self-test: PASS")
