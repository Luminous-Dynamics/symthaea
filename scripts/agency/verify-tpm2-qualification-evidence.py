...
def verify_lock_evidence(files: Dict[str, bytes]) -> None:
    ...
    before_hash, before_path = parse_sha_line(files["CARGO_LOCK_BEFORE_SHA256.txt"], "CARGO_LOCK_BEFORE_SHA256.txt")
    after_hash, after_path = parse_sha_line(files["CARGO_LOCK_CANDIDATE_SHA256.txt"], "CARGO_LOCK_CANDIDATE_SHA256.txt")
...
def verify_flake_evidence(files: Dict[str, bytes]) -> dict:
    ...
    flake_hash, flake_path = parse_sha_line(files["FLAKE_LOCK_SHA256.txt"], "FLAKE_LOCK_SHA256.txt")
    ...
    return {"nixpkgs_locked": retained, "flake_lock_sha256": flake_hash, "rust_toolchain_sha256": toolchain_hash}
...
def verify_status(files: Dict[str, bytes]) -> Tuple[str, str]:
    ...
    if (
        result["result"] != "PASS" or result["last_phase"] != "complete"
        or result["exit_code"] != 0 or result["cargo_lock_stale"] is not False
    ):
...
def verify_tpm_evidence(files: Dict[str, bytes]) -> dict:
    ...
    quote_path = one_line(files["QUOTE_WRAPPER_PATH.txt"], "QUOTE_WRAPPER_PATH.txt")
    check_path = one_line(files["CHECKQUOTE_WRAPPER_PATH.txt"], "CHECKQUOTE_WRAPPER_PATH.txt")
    if not store.startswith("/nix/store/"):
        raise EvidenceError("TPM verifier store is not beneath /nix/store")
    ...
    quote_hash, quote_hash_path = parse_sha_line(files["QUOTE_WRAPPER_SHA256.txt"], "QUOTE_WRAPPER_SHA256.txt")
    check_hash, check_hash_path = parse_sha_line(files["CHECKQUOTE_WRAPPER_SHA256.txt"], "CHECKQUOTE_WRAPPER_SHA256.txt")
    ...
    approved = nonzero_hex64(one_line(files["APPROVED_PCR_PROFILE.txt"], "APPROVED_PCR_PROFILE.txt"), "approved PCR profile")
...
def archive_expected_hash(args: argparse.Namespace, archive: Path) -> Tuple[str, str]:
    if args.expected_archive_sha256:
        ...
        if not HEX64.fullmatch(value):
            raise EvidenceError("--expected-archive-sha256 must be 64 lowercase hex characters")
        return value, "caller"
...
def parse_args(argv: Iterable[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--release", action="store_true", help="require independently supplied archive/head/tree bindings")
...
def main(argv: Iterable[str] = sys.argv[1:]) -> int:
    ...
    try:
        if args.release and not (args.expected_archive_sha256 and args.expected_head and args.expected_tree):
            raise EvidenceError("--release requires --expected-archive-sha256, --expected-head, and --expected-tree")
        ...
        return 0
    except (EvidenceError, OSError, UnicodeDecodeError, tarfile.TarError) as exc:
