#!/usr/bin/env python3
"""EPI-AUTH-FLOW-001 R5R3 deterministic authority-flow discovery.

Measurement-only audit of production Rust paths through which strong epistemic
cube/status values can be produced, converted, transported, scalarized, or used
to control Broca/language behavior.
...
REQUIRED_WITNESSES: dict[str, dict[str, tuple[str, ...]]] = {
    ...
    "broca_cube_conditioning": {
        "crates/domains/symthaea-broca/src/encoder.rs": (
            "e_tier_e4", "n_tier_n3", "m_tier_m3", "pub fn set_epistemic_cube",
            "pub fn epistemic_ordinal",
        ),
    },
    "typed_cube_to_broca": {
    ...
    "hdc_statistical_producer": {
        "crates/core/symthaea-core/src/hdc/statistical_retrieval.rs": (
            "EmpiricalTier::from_z_score", "E3CryptographicallyProven", "E4PubliclyReproducible",
        ),
    },
...
MIN_COUNTS = {
    ...
    "broca_epistemic_gate_surface": 2,
...
def main() -> int:
    ...
    print("schema=epi-auth-flow-001-r5r3-discovery-v1")
    print("authority_scope=measurement-only-cube-certainty-ordinal-broca-authority-flow")
