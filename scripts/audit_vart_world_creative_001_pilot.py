...
def _cell_from_plan_entry(entry: Any) -> dict[str, Any]:
    ...
    for key in ("cell_id", "trial_id", "policy", "fixture", "seed", "revision_index", "paired_block_id"):
        require(key in cell, "PILOT_AUDIT_PLAN_INVALID", f"{cell.get('cell_id', '?')}: missing {key}")
    return cell
...
def verify_paired_design(cells: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for cell in cells:
        ...
        require(isinstance(block, str) and block, "PILOT_AUDIT_PLAN_INVALID", "empty paired_block_id")
...
def verify_pilot(root: Path) -> dict[str, Any]:
    ...
    require(receipt.get("scientific_efficacy_claims_authorized") is False,
            "PILOT_AUDIT_AUTHORITY_VIOLATION", "scientific efficacy authorization")
    require(receipt.get("confirmatory_execution_authorized") is False,
            "PILOT_AUDIT_AUTHORITY_VIOLATION", "confirmatory execution authorization")
    require(receipt.get("claim_authorized") is False,
            "PILOT_AUDIT_AUTHORITY_VIOLATION", "claim authorization")
    ...
    require(isinstance(expected_closure, str) and len(expected_closure) == 64,
            "PILOT_AUDIT_RECEIPT_INVALID", "pilot_evidence_closure_sha256")
    actual_closure = tree_closure(root, excluded_relpaths={RECEIPT_REL})
    require(actual_closure == expected_closure,
            "PILOT_AUDIT_CLOSURE_MISMATCH", f"{actual_closure} != {expected_closure}")

    plan_cells_raw = plan.get("cells")
    require(isinstance(plan_cells_raw, list) and plan_cells_raw,
            "PILOT_AUDIT_PLAN_INVALID", "resolved plan cells")
    cells = [_cell_from_plan_entry(entry) for entry in plan_cells_raw]

    receipt_cells = receipt.get("cells")
    require(isinstance(receipt_cells, list), "PILOT_AUDIT_RECEIPT_INVALID", "receipt cells")
    receipt_ids = [item.get("cell_id") for item in receipt_cells if isinstance(item, dict)]
    plan_ids = [cell["cell_id"] for cell in cells]
    require(receipt_ids == plan_ids,
            "PILOT_AUDIT_PLAN_RECEIPT_MISMATCH", f"{receipt_ids} != {plan_ids}")
    ...
    for key in ("head", "tree", "parent_v05a_head", "parent_v05a_tree"):
        value = source.get(key)
        require(isinstance(value, str) and len(value) == 40,
                "PILOT_AUDIT_RECEIPT_INVALID", f"source.{key}")
    ...
    return {
        ...
        "bounded_statement": (
            "This audit establishes sealed pilot integrity and paired-design coherence only; "
            "it does not establish scientific efficacy or authorize confirmatory execution."
        ),
    }
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Post-pilot integrity and paired-design auditor for VART-WORLD-CREATIVE-001"
    )
