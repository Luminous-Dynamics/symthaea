...
def selftest(policy_path:Path):
 ...
 with tempfile.TemporaryDirectory(prefix='ll009ag-') as td:
  ...
  q=receipt({'schema_version':'q.v1','value':1})
  z=receipt({'schema_version':'z.v1','upstream':q['receipt_sha256']})
  wj(er/'q.json',q);wj(er/'z.json',z)
  mv={'schema_version':M,'study_id':'synthetic','artifacts':[
   {'id':'q','path':'q.json','requirement':'required','dependencies':[],'dependency_bindings':[],'self_hash_mode':'canonical_receipt_sha256'},
   {'id':'z','path':'z.json','requirement':'required','dependencies':['q'],'dependency_bindings':[{'dependency_id':'q','field_path':['upstream'],'identity':'receipt_sha256'}],'self_hash_mode':'canonical_receipt_sha256'},
   {'id':'diag','path':'diag.json','requirement':'optional_diagnostic','dependencies':['q'],'dependency_bindings':[{'dependency_id':'q','field_path':['q_file_sha256'],'identity':'sha256'}],'self_hash_mode':'canonical_receipt_sha256'},
  ...
  wrong_up='2'*64;wj(er/'z.json',receipt({'schema_version':'z.v1','upstream':wrong_up}));expect('valid child with wrong upstream binding',lambda:freeze(pre,pp,repo,head,rt,mp,er));(er/'z.json').write_bytes(oldz)
  ...
  diag=receipt({'schema_version':'diag.v1','q_file_sha256':hf(er/'q.json')});wj(er/'diag.json',diag)
  ...
  bad_diag=receipt({'schema_version':'diag.v1','q_file_sha256':'3'*64});wj(er/'diag.json',bad_diag);expect('optional diagnostic wrong dependency binding',lambda:freeze(pre,pp,repo,head,rt,mp,er));(er/'diag.json').unlink()
  ...
  missing_binding={'schema_version':M,'study_id':'synthetic','artifacts':[{'id':'q','path':'q.json','requirement':'required','dependencies':[],'dependency_bindings':[],'self_hash_mode':'canonical_receipt_sha256'},{'id':'z','path':'z.json','requirement':'required','dependencies':['q'],'dependency_bindings':[],'self_hash_mode':'canonical_receipt_sha256'}]};expect('missing dependency binding',lambda:vm(missing_binding,'synthetic'))
  cyc={'schema_version':M,'study_id':'synthetic','artifacts':[
   {'id':'q','path':'q.json','requirement':'required','dependencies':['z'],'dependency_bindings':[{'dependency_id':'z','field_path':['upstream'],'identity':'receipt_sha256'}],'self_hash_mode':'canonical_receipt_sha256'},
   {'id':'z','path':'z.json','requirement':'required','dependencies':['q'],'dependency_bindings':[{'dependency_id':'q','field_path':['upstream'],'identity':'receipt_sha256'}],'self_hash_mode':'canonical_receipt_sha256'}]};expect('dependency cycle',lambda:vm(cyc,'synthetic'))
