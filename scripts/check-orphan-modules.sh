...
if [[ $summary -eq 1 ]]; then
  echo ""
  echo "orphan modules: $orphans   stranded subtrees: $stranded"
  exit 0
fi

if [[ $orphans -ne 0 || $stranded -ne 0 ]]; then
  echo ""
...
echo "OK: no orphan modules in ${#scope[@]} crate(s) checked, $quarantined quarantined."
