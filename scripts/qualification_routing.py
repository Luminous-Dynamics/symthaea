...
def normalize_decision(value:Any,*,verify_declared_id:bool=True,require_id:bool=False)->dict[str,Any]:
    ...
    subject_mod._require_exact_keys(value,{"schema","subject_id","source_commit","base_commit","merge_base","changed_paths_sha256","router_recipe_id","disposition","required_profile_ids","reason"},{"decision_id"},where="routing decision")
    ...
    out={
        ...
        "source_commit":subject_mod._require_object_id(value["source_commit"],where="routing decision.source_commit"),
        "base_commit":subject_mod._require_object_id(value["base_commit"],where="routing decision.base_commit"),
        "merge_base":subject_mod._require_object_id(value["merge_base"],where="routing decision.merge_base"),
...
def validate_against_subject(decision:Any,subject:Any)->None:
    ...
    if route["subject_id"]!=source["subject_id"] or route["source_commit"]!=source["source_commit"]:
