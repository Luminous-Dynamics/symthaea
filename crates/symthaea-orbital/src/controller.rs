...
use crate::types::{OrbitalCommand, OrbitalConfig, NUM_ACTUATORS};
const HDC_DIM: usize = symthaea_core::hdc::HDC_DIMENSION;
pub struct OrbitalController { network: HdcLtcUnifiedNetwork, weights: Vec<f32>, bias: [f32; NUM_ACTUATORS] }
impl OrbitalController {
    pub fn new(g: &GenesisSeed, c: &OrbitalConfig) -> Self {
        let nc = UnifiedConfig { tau_base: 1.0/c.physics_hz as f32, backbone_tau: 0.5, dimension: HDC_DIM, learning_rate: c.learning_rate, ..UnifiedConfig::default() };
    ...
    pub fn forward(&mut self, hv: &ContinuousHV, dt: f32) -> OrbitalCommand {
        ...
        OrbitalCommand { joint_torques: t }
    }
    pub fn reset(&mut self) { self.network.reset(); }
}
#[cfg(test)] mod tests { use super::*; #[test] fn test_fwd() { let mut c = OrbitalController::new(&GenesisSeed::from_phrase("t"), &OrbitalConfig::default()); let cmd = c.forward(&ContinuousHV::random(HDC_DIM, 42), 0.01); assert!(cmd.joint_torques.iter().all(|t| t.is_finite())); } }
