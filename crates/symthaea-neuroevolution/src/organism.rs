...
impl NeuralOrganism {
    ...
    pub fn spawn(id: u64, genome: NeuralGenome, genesis: &GenesisSeed, generation: u32) -> Self {
        ...
        let network = HdcLtcUnifiedNetwork::from_genesis(
            phenotype.network_config.clone(),
            genesis,
        );
    ...
    pub fn evaluate_step(&mut self, input: &ContinuousHV, dt: f32) -> StepResult {
        ...
        // Project CfC output (16,384D) → FEP observation (32D) via strided sampling
        let obs_values: Vec<f64> = (0..FEP_STATE_DIM)
            .map(|i| {
                let idx = i * (output.dim() / FEP_STATE_DIM);
    ...
    pub fn reset_evaluation(&mut self) {
        ...
        // Re-instantiate network from genome
        let phenotype = self.genome.decode();
        let genesis = GenesisSeed::from_phrase(&format!("organism_{}_reset", self.id));
        self.network =
            HdcLtcUnifiedNetwork::from_genesis(phenotype.network_config, &genesis);
    }
}
...
mod tests {
    ...
    fn test_spawn_valid() {
        ...
        let org = NeuralOrganism::spawn(1, genome, &test_genesis(), 0);
        assert_eq!(org.id, 1);
    ...
    fn test_evaluate_multiple_steps() {
        ...
        let mut org = NeuralOrganism::spawn(1, genome, &test_genesis(), 0);
        for i in 0..10 {
            let input = ContinuousHV::random(16_384, 100 + i);
    ...
    fn test_compute_fitness_with_data() {
        ...
        let mut org = NeuralOrganism::spawn(1, genome, &test_genesis(), 0);
        for i in 0..5 {
            let input = ContinuousHV::random(16_384, 200 + i);
            org.evaluate_step(&input, 0.05);
        }
    ...
    fn test_crossover_inheritance() {
        ...
        let parent1 = NeuralOrganism::spawn(1, g1, &genesis, 0);
        let parent2 = NeuralOrganism::spawn(2, g2, &genesis, 0);
    ...
    fn test_replicate() {
        let genome = NeuralGenome::random(42);
        let genesis = test_genesis();
        let parent = NeuralOrganism::spawn(1, genome, &genesis, 0);
    ...
    fn test_organism_deterministic() {
        let genome = NeuralGenome::random(42);
        let genesis = test_genesis();
        let mut org1 = NeuralOrganism::spawn(1, genome.clone(), &genesis, 0);
        let mut org2 = NeuralOrganism::spawn(1, genome, &genesis, 0);
        let input = ContinuousHV::random(16_384, 99);
