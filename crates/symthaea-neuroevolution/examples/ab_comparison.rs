...
fn main() {
    ...
    println!("  Evolved {} generations in {:.1}s", result.generations_completed, evo_time.as_secs_f64());
    println!("  Best fitness: {:.6}", result.best_fitness);

    let evolved_phenotype = result.best_genome.decode();
    println!("  Evolved config:");
    println!("    tau_base:     {:.4}", evolved_phenotype.neuron_config.tau_base);
    println!("    backbone_tau: {:.4}", evolved_phenotype.neuron_config.backbone_tau);
    println!("    LR:           {:.6}", evolved_phenotype.neuron_config.learning_rate);
    println!("    momentum:     {:.4}", evolved_phenotype.neuron_config.momentum);
    println!("    gating:       {:.4}", evolved_phenotype.neuron_config.gating_steepness);
    println!("    layers:       {:?}", evolved_phenotype.network_config.layer_sizes);
    println!("    activation:   {:?}", evolved_phenotype.neuron_config.activation);
    ...
    println!("  Composite:       {:+.6}   {:+.6}   {:+.6}",
        fitness_default.composite, fitness_evolved.composite,
        fitness_evolved.composite - fitness_default.composite);
    println!("  Free Energy:     {:+.6}   {:+.6}   {:+.6}",
        fitness_default.free_energy, fitness_evolved.free_energy,
        fitness_evolved.free_energy - fitness_default.free_energy);
    println!("  Phi:             {:+.6}   {:+.6}   {:+.6}",
        fitness_default.phi, fitness_evolved.phi,
        fitness_evolved.phi - fitness_default.phi);
    println!("  Pred Accuracy:   {:+.6}   {:+.6}   {:+.6}",
        fitness_default.prediction_accuracy, fitness_evolved.prediction_accuracy,
        fitness_evolved.prediction_accuracy - fitness_default.prediction_accuracy);
    println!("  Efficiency:      {:+.6}   {:+.6}   {:+.6}",
        fitness_default.energy_efficiency, fitness_evolved.energy_efficiency,
    ...
    if delta > 0.01 {
        println!("\n  RESULT: Evolved config OUTPERFORMS default by {:.4}", delta);
    } else if delta < -0.01 {
    ...
    println!("  tau_base:        {:.4}         {:.4}",
        default_phenotype.neuron_config.tau_base, evolved_phenotype.neuron_config.tau_base);
    println!("  backbone_tau:    {:.4}         {:.4}",
        default_phenotype.neuron_config.backbone_tau, evolved_phenotype.neuron_config.backbone_tau);
    println!("  learning_rate:   {:.6}       {:.6}",
        default_phenotype.neuron_config.learning_rate, evolved_phenotype.neuron_config.learning_rate);
    println!("  gating:          {:.4}         {:.4}",
        default_phenotype.neuron_config.gating_steepness, evolved_phenotype.neuron_config.gating_steepness);
    println!("  layers:          {:?}  {:?}",
