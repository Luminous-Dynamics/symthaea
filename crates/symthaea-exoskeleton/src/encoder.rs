...
const RANGES: [[f32; 2]; NUM_STATE_CHANNELS] = [
    [-2.0,2.0],[0.0,2.5],[-0.5,0.5],[-2.0,2.0],[0.0,2.5],[-0.5,0.5],
    [-10.0,10.0],[-10.0,10.0],[-10.0,10.0],[-10.0,10.0],[-10.0,10.0],[-10.0,10.0],
    [-100.0,100.0],[-100.0,100.0],[-100.0,100.0],[-100.0,100.0],[-100.0,100.0],[-100.0,100.0],
    [-50.0,50.0],[-50.0,50.0],[-50.0,50.0],[-50.0,50.0],[-50.0,50.0],[-50.0,50.0],
    [-0.15,0.15],[-0.15,0.15],[0.0,2000.0],[0.0,1.0],
];

pub struct ExoskeletonHdcEncoder {
    base: Vec<ContinuousHV>, levels: Vec<ContinuousHV>,
    prev: Option<[f32; NUM_STATE_CHANNELS]>, num_levels: usize,
}

impl ExoskeletonHdcEncoder {
    pub fn new(genesis: &GenesisSeed, num_levels: usize) -> Self {
        Self {
            base: (0..NUM_STATE_CHANNELS).map(|i| ContinuousHV::from_genesis(genesis, &format!("exo::ch::{i}"), DIM)).collect(),
            levels: (0..num_levels).map(|l| ContinuousHV::from_genesis(genesis, &format!("exo::lv::{l}"), DIM)).collect(),
            prev: None, num_levels,
        }
    }
    pub fn encode(&mut self, state: &ExoskeletonState) -> ContinuousHV {
        ...
        for i in 0..NUM_STATE_CHANNELS {
            let [lo, hi] = RANGES[i]; let r = hi - lo;
            let n = if r.abs() < 1e-10 { 0.5 } else { ((ch[i] - lo) / r).clamp(0.0, 1.0) };
