...
#[derive(Debug, Clone)]
pub struct EpisodeMetrics { pub mean_effort: f32, pub steps_survived: usize, pub fell: bool }

pub struct ExoskeletonTrainer {
    controller: ExoskeletonController, encoder: ExoskeletonHdcEncoder,
    fep: ActiveInferenceExoAgent, simulator: SimpleExoskeletonSimulator, config: ExoskeletonConfig,
}

impl ExoskeletonTrainer {
    pub fn new(config: ExoskeletonConfig) -> Self {
        let g = GenesisSeed::from_phrase("exo_train");
        Self { controller: ExoskeletonController::new(&g, &config), encoder: ExoskeletonHdcEncoder::new(&g, 32),
            fep: ActiveInferenceExoAgent::new(), simulator: SimpleExoskeletonSimulator::new(), config }
    }
    pub fn run_episode(&mut self) -> EpisodeMetrics {
        ...
        for step in 0..self.config.steps_per_episode {
            ...
            if !self.simulator.state().is_finite() {
                return EpisodeMetrics { mean_effort: total_e / (step+1) as f32, steps_survived: step, fell: true };
            }
        }
        let n = self.config.steps_per_episode;
        EpisodeMetrics { mean_effort: total_e / n as f32, steps_survived: n, fell: false }
    }
}
...
mod tests {
    ...
    #[test]
    fn test_episode() {
        let mut c = ExoskeletonConfig::default(); c.steps_per_episode = 200;
        let mut t = ExoskeletonTrainer::new(c);
        let m = t.run_episode();
        assert!(!m.fell); assert_eq!(m.steps_survived, 200);
    }
}
