    ...
    match &comp.audio {
        AudioData::I16(samples) => {
            let spec = hound::WavSpec {
                channels: 1, sample_rate: comp.sample_rate,
        ...
        AudioData::StereoF32(samples) => {
            let spec = hound::WavSpec {
                channels: 2, sample_rate: comp.sample_rate,
                bits_per_sample: 32, sample_format: hound::SampleFormat::Float,
            };
            let mut w = hound::WavWriter::create(path, spec).map_err(|e| e.to_string())?;
            for p in samples { w.write_sample(p[0]).map_err(|e| e.to_string())?; w.write_sample(p[1]).map_err(|e| e.to_string())?; }
            w.finalize().map_err(|e| e.to_string())?;
        }
    }
    Ok(())
