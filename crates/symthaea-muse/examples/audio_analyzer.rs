...
fn analyze_wav(path: &Path) {
    ...
    let crossings: u32 = left.windows(2).map(|w| if (w[0] > 0.0) != (w[1] > 0.0) { 1 } else { 0 }).sum();
    let zcr = crossings as f32 / duration;

    // === Silence Analysis ===
    let silence_threshold = 0.005; // -46 dBFS
    let silent_samples = left.iter().filter(|&&s| s.abs() < silence_threshold).count();
    ...
    let ioi_cv = if ioi_mean > 0.0 { ioi_sd / ioi_mean } else { 0.0 };
    ...
    println!("  {}", name);
    println!("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
    println!("  Duration:       {:.1}s | {}Hz {}ch {}bit", duration, spec.sample_rate, channels, spec.bits_per_sample);
    ...
    // Crackling verdict
    let crackle_verdict = if crackle_count == 0 { "CLEAN" }
        else if crackle_rate < 0.5 { "MINOR" }
        else if crackle_rate < 5.0 { "NOTICEABLE" }
        else { "SEVERE" };
    println!("  Crackling:      {} events ({:.1}/sec) — {}", crackle_count, crackle_rate, crackle_verdict);

    println!("  Silence:        {:.1}%", silence_pct);
    println!("  Stereo Width:   {:.3} (0=mono, 1=wide)", stereo_width);
    println!("  Dynamic Range:  {:.1}x", dyn_range);
    println!();
    println!("  Note Onsets:    {} ({:.2}/sec)", onsets.len(), note_density);
    println!("  IOI Mean:       {:.0}ms", ioi_mean * 1000.0);
    println!("  IOI SD:         {:.0}ms", ioi_sd * 1000.0);
    println!("  IOI CV:         {:.3} (timing variability)", ioi_cv);
    println!("  Percussion:     {} events ({:.0}% of onsets)", percussion_events,
        if !onsets.is_empty() { percussion_events as f32 / onsets.len() as f32 * 100.0 } else { 0.0 });
    println!("  ZCR:            {:.0} Hz (brightness proxy)", zcr);

    // Benchmarks
    println!();
    println!("  ── Benchmarks ──");
    if rms_db < -28.0 { println!("  ⚠ RMS too quiet ({:.0} dBFS, target -20 to -14)", rms_db); }
    else if rms_db > -10.0 { println!("  ⚠ RMS too loud ({:.0} dBFS, may clip)", rms_db); }
    else { println!("  ✓ RMS level OK ({:.0} dBFS)", rms_db); }

    if crackle_count > 0 { println!("  ⚠ Crackling detected ({} events)", crackle_count); }
    else { println!("  ✓ No crackling"); }

    if stereo_width < 0.15 { println!("  ⚠ Nearly mono (width {:.3}, target >0.25)", stereo_width); }
    else if stereo_width > 0.25 { println!("  ✓ Good stereo width ({:.3})", stereo_width); }
    else { println!("  ~ Moderate stereo width ({:.3})", stereo_width); }
    ...
    if ioi_cv < 0.1 && note_density > 0.5 {
        println!("  ⚠ Very uniform timing (CV={:.3}, may sound mechanical)", ioi_cv);
    } else if ioi_cv > 0.3 {
