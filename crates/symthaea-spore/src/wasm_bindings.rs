...
impl SporeEngine {
    ...
    // ======================================================================
    // Canvas: Living Topology SVG
    // ======================================================================

    /// Render a living topology SVG from a CognitiveSnapshot JSON.
    ///
    /// Pass a JS object matching the CognitiveSnapshot schema. Returns a
    /// self-contained animated SVG string suitable for innerHTML injection.
    #[cfg(feature = "canvas")]
    pub fn render_canvas(&self, snapshot: JsValue) -> Result<String, JsError> {
        let snap: symthaea_canvas::CognitiveSnapshot =
            serde_wasm_bindgen::from_value(snapshot)
                .map_err(|e| JsError::new(&format!("Invalid snapshot: {e}")))?;
        Ok(symthaea_canvas::render_snapshot(&snap))
    }

    /// Render a living topology SVG from the current engine state.
    ///
    /// Builds a CognitiveSnapshot from the last cycle's consciousness level,
    /// neuromodulators, and topology, then renders it to SVG.
    #[cfg(feature = "canvas")]
    pub fn render_canvas_current(&self) -> String {
        let snap = symthaea_canvas::CognitiveSnapshot {
            consciousness_level: self.inner.consciousness_level() as f64,
            prediction_error: 0.0,
            living_mind_vitality: self.inner.consciousness_level() as f64 * 0.8,
            ...
            thought_vector: self.inner.get_output_hv().iter().take(32).copied().collect(),
            cycle_count: self.inner.cycle_count(),
        };
        symthaea_canvas::render_snapshot(&snap)
    }
}
