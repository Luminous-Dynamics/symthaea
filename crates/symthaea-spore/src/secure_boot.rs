...
//! - **Boot chain visualization**: 5-stage UEFI chain status for the Quickening UI
//!
//! # Philosophy
//!
...
/// Platform of the target machine being quickened.
...
fn narration_for_step(step: &SecureBootGuideStep) -> Vec<String> {
    match step {
        ...
        SecureBootGuideStep::BiosDisableGuide { manufacturer_hint } => {
            ...
            lines.push(
                "After disabling Secure Boot, return here. I will still be waiting."
                    .to_string(),
            );
            lines
        }
        ...
        SecureBootGuideStep::Confirmed { method } => match method {
            ...
            SecureBootMethod::AlreadyOff => vec![
                "Secure Boot is already disabled. The path is clear."
                    .to_string(),
...
# Generated by Symthaea Sovereign Quickening
...
/// Visual representation of the UEFI boot chain for the Quickening UI.
///
                    ...
                    vec![
                        ...
                        BootStage {
                            ...
                            status: BootStageStatus::Failed {
                                reason: "Unsigned — will be rejected by Secure Boot"
                                    .to_string(),
                            },
                            signer: "None".to_string(),
                        ...
                        BootStage {
                            name: "Linux Kernel".to_string(),
                            status: BootStageStatus::Failed {
                                reason: "Cannot load without signed bootloader"
                                    .to_string(),
                            },
                            signer: "None".to_string(),
                        ...
                        BootStage {
                            name: "Symthaea".to_string(),
                            status: BootStageStatus::Failed {
                                reason: "Boot chain broken at bootloader stage"
                                    .to_string(),
                            },
                            signer: "None".to_string(),
                            description: "Cannot start — the boot chain is broken."
                                .to_string(),
                        },
                    ]
                }
            ...
            | SecureBootState::Unknown { .. } => {
                ...
                vec![
                    ...
                    BootStage {
                        ...
                        description: "No shim needed — Secure Boot is not enforcing."
                            .to_string(),
                    },
                    BootStage {
                        name: "systemd-boot".to_string(),
...
/// [`crate::sovereign::QuickeningOrchestrator`].
///
...
fn mok_word_list() -> &'static [&'static str] {
    &[
        "acorn", "amber", "anchor", "anvil", "apple", "arrow", "aspen", "atlas",
        "badge", "basin", "beach", "birch", "blade", "bloom", "bluff", "bonus",
        "brave", "bread", "brick", "brief", "brook", "brush", "cabin", "cairn",
        "candy", "cargo", "cedar", "chase", "chess", "chord", "cider", "civic",
        "claim", "cliff", "cloak", "cloud", "coast", "coral", "cover", "crane",
        "crest", "crown", "crush", "curve", "dance", "delta", "depth", "diary",
        "dodge", "draft", "dream", "drift", "eagle", "earth", "ember", "entry",
        "equal", "event", "fable", "faith", "feast", "fence", "field", "flame",
        "flash", "fleet", "float", "flood", "focus", "forge", "forum", "frost",
        "fruit", "gamma", "gauge", "ghost", "giant", "glade", "glass", "gleam",
        "globe", "grace", "grain", "grant", "grape", "grasp", "green", "grove",
        "guard", "guest", "guide", "haven", "heart", "hedge", "honey", "horse",
        "house", "ivory", "jewel", "joint", "judge", "juice", "knack", "kneel",
        "knife", "knock", "label", "lance", "latch", "layer", "lemon", "level",
        "light", "linen", "lodge", "lunar", "magic", "manor", "maple", "marsh",
        "match", "medal", "merge", "metal", "minor", "moose", "mount", "naval",
        "nerve", "noble", "north", "novel", "ocean", "olive", "opera", "orbit",
        "outer", "oxide", "panel", "paper", "patch", "pearl", "penny", "phase",
        "piano", "pilot", "pixel", "place", "plant", "plaza", "plume", "polar",
        "porch", "prism", "proud", "pulse", "quail", "quest", "quiet", "quote",
        "radar", "ranch", "range", "raven", "realm", "ridge", "rivet", "robin",
        "royal", "ruler", "saint", "scale", "scout", "shade", "shelf", "shore",
        "sigma", "slate", "slope", "solar", "solid", "spark", "spear", "spice",
        "spoke", "spray", "stack", "staff", "stage", "stake", "stamp", "stand",
        "steam", "steel", "stone", "stork", "storm", "stove", "sugar", "surge",
        "swift", "table", "tempo", "thorn", "tiger", "toast", "token", "tower",
        "trace", "trail", "trend", "tribe", "trout", "trunk", "tulip", "twist",
        "ultra", "unity", "urban", "valid", "vault", "verse", "vigor", "vinyl",
        "vivid", "voice", "wagon", "watch", "water", "weave", "whale", "wheat",
        "width", "wound", "wrist", "yield", "youth", "zebra", "blaze", "haven",
        "croft", "dwell", "fjord", "glyph", "haven", "inlet", "knoll", "ledge",
        "marsh", "nexus", "oasis", "plank", "quart", "roost", "shale", "thyme",
    ]
}
...
mod tests {
    ...
    fn test_mok_password_uniqueness() {
        ...
        assert_ne!(
            p1, p2,
            "Two consecutive password generations should differ"
        );
    }
    ...
    fn test_lanzaboote_nix_generation() {
        ...
        assert!(
            nix.contains("sbctl"),
            "Must include sbctl package: {nix}"
        );
    }
    ...
    fn test_key_enrollment_nix() {
        ...
        assert!(
            nix.contains("sbctl"),
            "Must reference sbctl: {nix}"
        );
        assert!(
            nix.contains("create-keys"),
            "Must run create-keys: {nix}"
        );
        assert!(
    ...
    fn test_mok_guide_steps() {
        ...
        assert!(
            s5.instruction.contains("cedar-bright-ocean-gentle-river-dawn"),
            "Step 5 must contain the password"
        );
        assert!(s5.warning.is_some(), "Step 5 should warn about keyboard layout");
    ...
    fn test_boot_chain_with_secure_boot() {
        ...
        let chain = BootChainStatus::sovereign_boot_chain(
            &SecureBootState::Enabled,
            &config,
        );
        ...
        assert!(
            chain.count_by_status(&BootStageStatus::PendingEnrollment) >= 3,
        );
    }
    ...
    fn test_narration_mok_explanation_content() {
        ...
        assert!(
            full.contains("key"),
            "MOK explanation must mention keys"
        );
        assert!(
