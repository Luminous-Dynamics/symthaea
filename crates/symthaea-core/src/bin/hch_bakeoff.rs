...
mod app {
    ...
    impl Default for Args {
        fn default() -> Self {
            Self {
                ...
                redundancy_k: 1,
                retrieval_fanout: 1,
                split_threshold: 100, // Default: split when leaf has 100 items
                retrieval_policy: "LeafOnly".into(),
                out: PathBuf::from("reports/hch_v010.json"),
            }
        }
    }
        ...
        I: IntoIterator<Item = String>,
    {
        ...
        while let Some(arg) = iter.next() {
            match arg.as_str() {
                ...
                "--split-threshold" => {
                    args.split_threshold = parse_next(&mut iter, "--split-threshold")?
                }
                "--retrieval-policy" => {
                    args.retrieval_policy = next_value(&mut iter, "--retrieval-policy")?
                }
    ...
    fn validate_args(args: &Args) -> Result<(), String> {
        ...
        match args.retrieval_policy.as_str() {
            "LeafOnly" | "ChildFirstParentFallback" | "MultiLevel" => {}
            _ => return Err(format!("unknown retrieval policy: {}", args.retrieval_policy)),
        }
    ...
    fn print_usage() {
        eprintln!(
            "usage: hch_bakeoff [--objects N] [--seeds N] [--dim N] [--branching N] \\
             [--abstain F] [--shortcuts N] [--redundancy-k N] [--retrieval-fanout N] \\
             [--split-threshold N] [--retrieval-policy POLICY] [--out PATH]"
        );
        eprintln!("\npolicies: LeafOnly, ChildFirstParentFallback, MultiLevel");
    }
    ...
    fn run_bakeoff(args: &Args) -> BakeoffReport {
        ...
        let retrieval_policy = match args.retrieval_policy.as_str() {
            "LeafOnly" => RetrievalPolicy::LeafOnly,
            "ChildFirstParentFallback" => RetrievalPolicy::ChildFirstParentFallback,
            "MultiLevel" => RetrievalPolicy::MultiLevel {
                fanout: args.retrieval_fanout,
            },
            _ => unreachable!(),
        };
        ...
        BakeoffReport {
            ...
            retrieval_policy: args.retrieval_policy.clone(),
            diagnosis,
            results,
        }
    }
    ...
    fn run_trial(
        ...
        for example in examples {
            ...
            // Primary routing with adaptive splitting
            let leaf_idx = router.route_and_record(&example.role, &zero, config.branching);
            let node_idx = pyramid.find_leaf_node_index(leaf_idx).unwrap_or(0);
            
            let final_node_idx = pyramid.bundle_at_node_adaptive(
                node_idx,
                &binding,
                split_threshold,
                leaf_counts[leaf_idx],
            );
            leaf_counts[leaf_idx] += 1;
            
            // Redundancy (static for now, doesn't trigger extra splitting on neighbors)
            if redundancy_k > 1 {
                let target_leaves = leaf_candidates(leaf_idx, config.branching, redundancy_k);
                for target_leaf in target_leaves {
                    if target_leaf == leaf_idx { continue; }
                    if let Some(target_node_idx) = pyramid.find_leaf_node_index(target_leaf) {
                        if let Some(target_node) = pyramid.nodes.get(target_node_idx).cloned() {
                            pyramid.bundle_at_node(&target_node, &binding);
                        }
                    }
                }
            }
            
            stored.push((example.role.clone(), value_idx, final_node_idx));
        }
    ...
    fn run_oracle_trial(
        ...
        for (idx, example) in examples.iter().enumerate() {
            ...
            leaf_counts[leaf_idx] += 1;
            let node_idx = pyramid.find_leaf_node_index(leaf_idx).unwrap_or(0);
            let node = pyramid.nodes[node_idx].clone();
            pyramid.bundle_at_node(&node, &binding);
            
            stored.push((example.role.clone(), value_idx, node_idx));
        }
    ...
    fn score_trial(
        ...
        for (role, correct_idx, node_idx) in stored {
            let report = pyramid.retrieve_with_policy(*node_idx, role, codebook, retrieval_policy);
            
            missing_leaf_count += report.missing_leaf_count;
            searched_nodes_total += report.fanout;
            searched_nodes_max = searched_nodes_max.max(report.fanout);

            let sims = report.results;
            if sims.len() < 2 {
                if !sims.is_empty() {
                    if sims[0].0 == *correct_idx { hits1 += 1; }
                    if sims.iter().take(3).any(|(idx, _)| *idx == *correct_idx) { hits3 += 1; }
                }
                abstained += 1;
                continue;
            }
            ...
            .map(|router| {
                ...
                RouterSummary {
                    ...
                    retrieval_fanout: matching
                        ...
                        .max()
                        .unwrap_or(1),
                    missing_leaf_count: (matching.iter().map(|trial| trial.missing_leaf_count).sum::<usize>() as f32 / denom) as usize,
    ...
    fn to_csv(report: &BakeoffReport) -> String {
        let mut csv = String::from(
            "router,top1,top3,mean_margin,abstention_rate,answered_accuracy,load_entropy,max_leaf_load,mean_leaf_load,latency_ms,latency_per_query_ms,split_count,redundancy_k,retrieval_fanout,missing_leaf_count,searched_nodes_mean,searched_nodes_max,logical_storage_multiplier,physical_storage_multiplier,top1_per_logical_storage,top1_per_fanout,answered_accuracy_per_latency_ms,oracle_gap_top1,oracle_gap_margin\n",
        );
    ...
    fn print_summary(report: &BakeoffReport) {
        ...
        println!(
            "objects={} seeds={} dim={} branching={} leaf_dim={} tau={:.3} redundancy_k={} retrieval_fanout={} split_threshold={} retrieval_policy={}",
            report.objects,
            report.seeds,
            report.dim,
            report.branching,
        ...
        for result in &report.results {
            println!(
                "{:20} {:>6.1}% {:>6.1}% {:>8.4} {:>7.1}% {:>7.1}% {:>8.2} {:>6} {:>7} {:>7.2} {:>7.2} {:>8.1}ms",
                result.router,
                result.top1 * 100.0,
                result.top3 * 100.0,
                result.mean_margin,
    ...
    mod tests {
        ...
        fn parse_cost_flags_are_serialized_into_report() {
            ...
            assert_eq!(report.version, "rhn-v0.10.1");
            assert_eq!(report.redundancy_k, 2);
            assert_eq!(report.retrieval_fanout, 3);
            assert_eq!(report.shortcuts, 4);
            assert_eq!(report.split_threshold, 50);
            assert_eq!(report.retrieval_policy, "ChildFirstParentFallback");
