...
pub fn generate_gcode(
    ...
    for (layer_idx, layer) in layers.iter().enumerate() {
        ...
        commands.push(GCodeCommand::Comment(format!("Layer {} z={:.3}", layer_idx, layer.z)));
...
mod tests {
    ...
    fn single_layer_has_z_move() {
        ...
        assert!(
            has_command(&program, |c| matches!(c, GCodeCommand::G0 { z: Some(z), .. } if (*z - 0.2).abs() < 1e-4)),
    ...
    fn gcode_command_display_format() {
        ...
        assert_eq!(
            GCodeCommand::Comment("hello".into()).to_string(),
            "; hello"
        );
    }
