...
fn wide_target_gets_independently_verified_robust_witness() {
    ...
    match solve_robust_single_integrator_analytic(&model, &query).unwrap() {
        RobustReachabilityResult1D::RobustFeasible {
            ...
            assert_eq!(replayed.model_identity(), model.identity());
            assert_eq!(replayed.query_identity(), query.identity());
        }
...
fn target_too_narrow_certificate_independently_replays() {
    ...
    match solve_robust_single_integrator_analytic(&model, &query).unwrap() {
        RobustReachabilityResult1D::CertifiedNotRobust { certificate, .. } => {
            assert!(
                certificate.requirement().required_lower()
                    > certificate.requirement().required_upper()
            );
...
fn theorem_uses_plant_time_and_has_no_cognitive_cadence_input() {
    ...
    assert_eq!(
        disturbance.time_domain(),
        symthaea_core::kinodynamic_reachability::DynamicsTimeDomain::PlantModelSeconds
    );
...
#[test]
fn outward_envelope_exposes_inward_nearest_rounding() {
    let model = model();
    let query = query(-1.0, 1.0, 0.0, 0.1, 0.3);
    let envelope = fixed_control_terminal_envelope(&model, &query, 0.0).unwrap();

    assert_eq!(0.1_f64.mul_add(0.3, 0.0), 0.03);
    assert!(envelope.upper() > 0.03);
}

#[test]
fn nonzero_power_of_two_underflow_is_not_treated_as_exact_zero() {
    let model = model();
    let tiny = f64::MIN_POSITIVE;
    let query = query(-1.0, 1.0, 0.0, 0.0, tiny);
    let envelope = fixed_control_terminal_envelope(&model, &query, tiny).unwrap();

    assert_eq!(tiny * tiny, 0.0);
    assert!(envelope.lower() < 0.0);
    assert!(envelope.upper() > 0.0);
}
...
#[test]
fn sub_guard_target_width_separation_returns_unknown() {
    let model = model();
    let tolerance = RobustReachabilityPolicy1D::reference_v1().absolute_control_tolerance();
    let epsilon = 0.1 * tolerance;
    let query = query(0.0, 1.0, 0.0, 1.0 + epsilon, 1.0);
...
#[test]
fn disturbance_widening_preserves_feasible_unknown_certified_order() {
    let model = model();
    let tolerance = RobustReachabilityPolicy1D::reference_v1().absolute_control_tolerance();
    let epsilon = 0.1 * tolerance;
    let feasible = query(-0.5, 0.5, -0.24, 0.24, 2.0);
    let ambiguous = query(-0.5, 0.5, -0.25 - epsilon, 0.25 + epsilon, 2.0);
    let impossible = query(-0.5, 0.5, -0.3, 0.3, 2.0);

    assert!(matches!(
        solve_robust_single_integrator_analytic(&model, &feasible).unwrap(),
        RobustReachabilityResult1D::RobustFeasible { .. }
    ));
    assert!(matches!(
        solve_robust_single_integrator_analytic(&model, &ambiguous).unwrap(),
        RobustReachabilityResult1D::Unknown {
            reason: UnknownReason::NumericalFailure,
            ..
        }
    ...
    assert!(matches!(
        solve_robust_single_integrator_analytic(&model, &impossible).unwrap(),
        RobustReachabilityResult1D::CertifiedNotRobust { .. }
    ));
}

#[test]
fn positive_witness_is_subject_bound_and_rejects_cross_query_replay() {
    let model = model();
    let original = query(-0.5, 0.5, -0.25, 0.25, 2.0);
    let different = query(-0.6, 0.6, -0.25, 0.25, 2.0);

    let witness = match solve_robust_single_integrator_analytic(&model, &original).unwrap() {
        RobustReachabilityResult1D::RobustFeasible { witness, .. } => witness,
        other => panic!("expected robust witness, got {other:?}"),
    };

    assert!(matches!(
        verify_robust_witness(&model, &different, &witness),
        Err(RobustReachabilityError::InvalidWitness { .. })
    ));
}

#[test]
fn negative_certificate_is_subject_bound_and_rejects_cross_query_replay() {
    let model = model();
    let original = query(-0.1, 0.1, -0.25, 0.25, 2.0);
    let different = query(-0.1, 0.1, -0.3, 0.3, 2.0);

    let certificate = match solve_robust_single_integrator_analytic(&model, &original).unwrap() {
        RobustReachabilityResult1D::CertifiedNotRobust { certificate, .. } => certificate,
        other => panic!("expected negative certificate, got {other:?}"),
    };

    assert!(matches!(
        verify_not_robust_certificate(&model, &different, &certificate),
        Err(RobustReachabilityError::InvalidCertificate { .. })
    ));
}

#[test]
fn positive_and_negative_verifiers_have_distinct_authority_identities() {
    let model = model();
    let feasible = query(-0.5, 0.5, -0.25, 0.25, 2.0);
    let impossible = query(-0.1, 0.1, -0.25, 0.25, 2.0);

    let positive_identity = match solve_robust_single_integrator_analytic(&model, &feasible).unwrap() {
        RobustReachabilityResult1D::RobustFeasible { verification, .. } => {
            verification.verifier_identity()
        }
        other => panic!("expected positive receipt, got {other:?}"),
    };
    let negative_identity = match solve_robust_single_integrator_analytic(&model, &impossible).unwrap() {
        RobustReachabilityResult1D::CertifiedNotRobust { verification, .. } => {
            verification.verifier_identity()
        }
        other => panic!("expected negative receipt, got {other:?}"),
    };

    assert_ne!(positive_identity, negative_identity);
    assert_ne!(positive_identity, [0_u8; 32]);
    assert_ne!(negative_identity, [0_u8; 32]);
}
