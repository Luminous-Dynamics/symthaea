...
fn rel_001_continuous_coordinate_permutation_preserves_binding_and_cosine_geometry() {
    ...
    let after = reverse_continuous_coordinates(&a)
        .similarity(&reverse_continuous_coordinates(&b));
    assert!(
...
fn rel_003_binary_xor_automorphism_does_not_imply_hamming_isometry() {
    ...
    assert_eq!(
        binary_xor_linear_shear(&zero)
            .hamming_distance(&binary_xor_linear_shear(&one_bit)),
        2
    );
    assert_ne!(
