...
impl PrototypeIndex {
    pub fn from_assignments(
        ...
        for (leaf_idx, vector) in assignments {
            ...
            let target = leaf_keys[*leaf_idx].values.as_mut_slice();
        ...
        for (idx, key) in leaf_keys.iter_mut().enumerate() {
            match mode {
                ...
                BundleMode::Mean => {
                    if counts[idx] > 0 {
                        for v in key.values.as_mut_slice() {
                            *v /= counts[idx] as f32;
                        }
                    }
                }
...
fn normalize_hv(hv: &mut ContinuousHV) {
    ...
    if norm > 1e-8 {
        for v in hv.values.as_mut_slice() {
            *v /= norm;
        }
    }
}
...
pub struct RhnRetrievalReport {
    ...
    /// Number of requested leaf indices that could not be resolved to a node.
...
impl PyramidCantorVector {
    ...
    pub fn find_node(&self, level: usize, index: usize) -> Option<&CantorNode> {
        self.find_node_index(level, index)
            .and_then(|idx| self.nodes.get(idx))
    }

    /// Get the flat node index by level and index.
    pub fn find_node_index(&self, level: usize, index: usize) -> Option<usize> {
        self.nodes
            .iter()
            .position(|n| n.level == level && n.index == index)
    }

    /// Get the flat node index for a leaf node (Level 1).
    pub fn find_leaf_node_index(&self, leaf_idx: usize) -> Option<usize> {
        self.find_node_index(1, leaf_idx)
    }
    ...
    pub fn retrieve_multi_leaf(
        ...
        for leaf_idx in candidates {
            if let Some(node_idx) = self.find_leaf_node_index(leaf_idx) {
                if let Some(node) = self.nodes.get(node_idx) {
                    ...
                    for (idx, cand) in codebook.iter().enumerate() {
                        best_by_codebook[idx] =
                            best_by_codebook[idx].max(recovered.similarity(cand));
                    }
                }
            }
        }
    ...
    pub fn node_data_mut(&mut self, node: &CantorNode) -> &mut [f32] {
        &mut self.data.values.as_mut_slice()[node.range.clone()]
    }
    ...
    pub fn broadcast_up(&mut self, node_idx: usize) {
        ...
        let target = &mut self.data.values.as_mut_slice()[parent_range];
        for i in 0..parent_len {
    ...
    pub fn modulate_down(&mut self, node_idx: usize) {
        ...
        for &child_idx in &children_indices {
            ...
            let target = &mut self.data.values.as_mut_slice()[child_range];
...
#[cfg(test)]
mod index_hygiene_tests {
    use super::*;
    use crate::hdc::unified_hv::ContinuousHV;

    fn test_config() -> CantorHdcConfig {
        CantorHdcConfig {
            total_dim: 16,
            levels: 2,
            branching: 4,
    ...
    #[test]
    fn test_indexing_hygiene() {
        let config = test_config();
        let pyramid = PyramidCantorVector::new(config, None);
        assert!(pyramid.find_leaf_node_index(0).is_some());
        assert!(pyramid.find_leaf_node_index(99).is_none());
    }

    #[test]
    fn test_missing_leaf_reporting() {
        let config = test_config();
        let pyramid = PyramidCantorVector::new(config, None);
        let role = ContinuousHV::zero(config.leaf_dim);
        let codebook = vec![ContinuousHV::zero(config.leaf_dim)];
        let policy = RetrievalPolicy::LeafOnly;

        let report = pyramid.retrieve_with_policy(999, &role, &codebook, policy);
        assert_eq!(report.missing_leaf_count, 1);
        assert_eq!(report.fanout, 0);
    }
}
