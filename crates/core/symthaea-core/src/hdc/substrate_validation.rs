...
///
/// This authority-bearing aggregate is intentionally export-only through Serde.
/// Generic deserialization is deliberately unavailable: a future evidence import
/// path must be explicit, provenance-bearing, independently qualified, and must
/// apply a defined evidence-transition policy rather than reconstructing curated
/// authority state from arbitrary serialized bytes.
