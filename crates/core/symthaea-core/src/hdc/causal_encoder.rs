...
impl CausalSpace {
    ...
    /// Remove all causal links where the given variable is an effect.
    ///
    /// This implements Pearl's graph surgery: do(X=x) removes all
    ...
    pub fn remove_effects_of(&mut self, variable: &BinaryHV) -> usize {
        ...
        let mut i = 0;
        while i < self.causal_links.len() {
            if self.causal_links[i].effect.similarity(variable) > 0.9 {
                self.causal_links.remove(i);
                self.temporal_order.remove(i);
                removed += 1;
            } else {
                i += 1;
            }
        ...
        // Rebuild indices if any links were removed
        if removed > 0 {
