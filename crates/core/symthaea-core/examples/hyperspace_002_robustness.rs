...
use symthaea_core::continuous_reachability::{
    ...
    EuclideanPlanningProblem, validate_euclidean_path,
};
...
use symthaea_core::sampling_reachability::{RrtConfig, SamplingReachabilityResult, seeded_rrt};
...
fn main() -> Result<(), Box<dyn Error>> {
    let (subject, primary_hyperspace_001_head, output) = parse_args()?;
    ...
    println!(
        "HYPERSPACE-002 robustness verdict: {}",
...
fn parse_args() -> Result<(String, String, PathBuf), Box<dyn Error>> {
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            ...
            "--primary-hyperspace-001-head" => primary = args.next(),
            "--output" => output = args.next().map(PathBuf::from),
            other => return Err(format!("unknown argument: {other}").into()),
        }
    }
    let subject = validate_sha(subject.ok_or("--subject is required")?, "--subject")?;
    let primary = validate_sha(
        primary.ok_or("--primary-hyperspace-001-head is required")?,
        "--primary-hyperspace-001-head",
    )?;
    Ok((subject, primary, output.ok_or("--output is required")?))
}
...
fn run_study() -> Result<Value, Box<dyn Error>> {
    ...
    let canonical_witness = evaluator_reference_escape_path(
        &feasible_canonical_problem,
        &feasible_canonical,
        0.25,
    )?;
    ...
    for external_w_axis in 0..4 {
        let feasible_oracle =
            PermutationOracle::from_external_w_axis(feasible_canonical.clone(), external_w_axis)?;
        let feasible_problem = permuted_problem(&feasible_canonical_problem, &feasible_oracle)?;
        let transformed_witness: Vec<Vec<f64>> = canonical_witness
            .iter()
            .map(|state| feasible_oracle.to_external(state))
            .collect::<Result<_, _>>()?;
        let witness_validation = match validate_euclidean_path(
            &feasible_problem,
            &feasible_oracle,
            &transformed_witness,
        )? {
            ...
            other => {
                return Err(format!(
                    "axis {external_w_axis} transformed evaluator witness did not replay valid: {other:?}"
                )
                .into());
            }
        };
        ...
        let blocked_oracle =
            PermutationOracle::from_external_w_axis(blocked_canonical.clone(), external_w_axis)?;
        ...
        let axis_pass = successes >= SUCCESS_GATE_PER_AXIS
            && quality_failures == 0
...
fn run_escape_seed(
    ...
    oracle: &PermutationOracle,
    canonical_problem: &EuclideanPlanningProblem,
    canonical_oracle: &FiniteWShellOracle,
    ...
    match seeded_rrt(problem, oracle, config)? {
        SamplingReachabilityResult::Feasible {
            ...
            let replay = match validate_euclidean_path(problem, oracle, &path)? {
                ...
                other => {
                    return Err(format!(
                        "seed {seed} feasible candidate contradicted independent replay: {other:?}"
                    )
                    .into());
                }
            };
            if replay.path_identity() != validation.path_identity() {
                return Err(format!("seed {seed} replay identity mismatch").into());
            }

            let max_abs_w = path
                .iter()
                .map(|state| state[external_w_axis].abs())
            ...
            let canonical_path: Vec<Vec<f64>> = path
                ...
                .collect::<Result<_, _>>()?;
            let projection =
                qualify_projection_trap(canonical_problem, canonical_oracle, &canonical_path);
