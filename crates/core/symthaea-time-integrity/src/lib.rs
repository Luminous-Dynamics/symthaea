...
impl TimeIntegrityReceipt {
    ...
    ///
    /// Strict comparison requires explicit continuity-epoch provenance in
    /// addition to continuous operation and finite uncertainty. A producer must
    /// not upgrade an epoch-less timestamp merely by setting `Continuous`.
    pub fn supports_bounded_comparison(&self) -> bool {
        self.clock_epoch.is_some()
...
impl fmt::Display for TimeComparisonError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::MissingClockEpoch { side } => write!(
                f,
                "{side:?} timestamp has no explicit continuity epoch for strict bounded comparison"
            ),
...
fn ensure_explicit_epoch(
    receipt: &TimeIntegrityReceipt,
    side: ComparisonSide,
) -> Result<(), TimeComparisonError> {
    if receipt.clock_epoch.is_none() {
        return Err(TimeComparisonError::MissingClockEpoch { side });
    }
    Ok(())
}
...
///
/// This is deliberately the weak comparison path. Two epoch-less receipts in
/// the same declared domain may be compared here, but the result carries no
/// strict continuity-across-reset guarantee. Use [`bounded_separation_window_us`]
/// for physical/safety-sensitive claims that require explicit epoch provenance,
/// continuous operation, and finite timestamp error bounds.
...
///
/// Both receipts must provide explicit continuity epochs, refer to the same
/// declared clock domain and epoch, claim continuous operation, and provide
/// finite timestamp error bounds. The returned interval is safe under the
/// supplied claims:
///
...
pub fn bounded_separation_window_us(
    ...
    ensure_explicit_epoch(left, ComparisonSide::Left)?;
    ensure_explicit_epoch(right, ComparisonSide::Right)?;
    ensure_declared_comparable(left, right)?;
...
mod tests {
    ...
    #[test]
    fn declared_comparison_can_remain_weak_without_epoch_provenance() {
        let left = TimeIntegrityReceipt::declared(domain());
        let right = TimeIntegrityReceipt::declared(domain());
        assert_eq!(declared_separation_us(100, &left, 125, &right).unwrap(), 25);
    }
    ...
    #[test]
    fn bounded_capability_requires_explicit_epoch() {
        let receipt = TimeIntegrityReceipt::declared(domain())
            .with_continuity(ContinuityStatus::Continuous)
            .with_uncertainty(TimeUncertainty::bounded(5));
        assert!(!receipt.supports_bounded_comparison());
        assert!(receipt.with_epoch(epoch()).supports_bounded_comparison());
    }

    #[test]
    fn bounded_comparison_rejects_missing_epoch_before_timing_math() {
        let left = TimeIntegrityReceipt::declared(domain())
            .with_continuity(ContinuityStatus::Continuous)
            .with_uncertainty(TimeUncertainty::bounded(5));
        let right = bounded(5);
        ...
        assert_eq!(
            error,
            TimeComparisonError::MissingClockEpoch {
                side: ComparisonSide::Left,
            }
        );
    }
