...
//! before the first observation. It *can* prove a lower bound: if the same
//! desired value has been observed continuously since time T, then it has been
//! present for at least `now - T`. That lower bound may prove persistent drift
//! once it exceeds a convergence window, but it can never prove that a drift is
//! recent enough to be called `Converging`.
...
pub struct HistoricalStateAssessment {
    ...
    /// Lower bound only. The desired value may have become effective before the
    /// first snapshot in the continuous run.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub continuously_observed_desired_age_lower_bound_ms: Option<u64>,
    /// True only when persistence is proved either by the exact temporal
    /// assessor or by a continuous-observation lower bound that already exceeds
...
pub fn assess_state_dimension_with_history(
    ...
    let continuity = desired_state_continuity(history, subject, dimension, at_unix_ms)?;
    let lower_bound = continuity
        .as_ref()
    ...
    let lower_bound_proves_persistence = current.status == TemporalStateStatus::DriftAgeUnknown
        && lower_bound.is_some_and(|age| age > policy.convergence_window_ms);
    ...
    Ok(HistoricalStateAssessment {
        ...
        continuously_observed_desired_age_lower_bound_ms: lower_bound,
...
pub fn desired_state_continuity(
    ...
    let latest_assessment = assess_state_dimension(
        &latest.assertions,
        subject,
        dimension,
        latest.collected_at_unix_ms,
...
mod tests {
    ...
    #[test]
    fn continuous_history_can_prove_persistent_drift_lower_bound() {
    ...
    #[test]
    fn recent_first_observation_cannot_prove_convergence_or_persistence() {
        let history = history(vec![
