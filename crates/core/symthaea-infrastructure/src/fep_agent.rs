...
/// previous `obs_dim: 8` against a 28-channel state silently underused it).
...
impl ActiveInferenceInfrastructureAgent {
    ...
    pub fn tick(&mut self, state: &InfrastructureState) -> InfrastructureFepResult {
        ...
        let observation = Observation {
            ...
            modality: "infrastructure".to_string(),
        };
...
mod tests {
    ...
    fn test_fep_free_energy_is_not_a_raw_l2_norm() {
        let mut a = ActiveInferenceInfrastructureAgent::new();
        let state = InfrastructureState::home();
    ...
    fn test_fep_perceiving_changes_agent_belief() {
        ...
        let mut hot_state = InfrastructureState::home();
