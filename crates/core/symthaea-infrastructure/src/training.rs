...
pub struct InfrastructureTrainer {
    ...
    fep: ActiveInferenceInfrastructureAgent,
    simulator: SimpleInfrastructureSimulator,
    config: InfrastructureConfig,
}

impl InfrastructureTrainer {
    pub fn new(config: InfrastructureConfig) -> Self {
        let g = GenesisSeed::from_phrase(&config.genesis_phrase);
        Self {
            controller: InfrastructureController::new(&g, &config),
            encoder: InfrastructureHdcEncoder::new(&g, 32),
            fep: ActiveInferenceInfrastructureAgent::new(),
