...
impl PCAlgorithm {
    ...
    ///
    /// # Panics
    /// Panics when `alpha` is not finite or is outside the open interval `(0, 1)`.
    pub fn with_alpha(alpha: f64) -> Self {
        Self::try_with_alpha(alpha)
            .expect("PC significance level alpha must be finite and in (0, 1)")
    }

    /// Create with a validated custom significance level.
    pub fn try_with_alpha(alpha: f64) -> Result<Self, &'static str> {
    ...
    fn validate_alpha(alpha: f64) -> Result<(), &'static str> {
        if !alpha.is_finite() || alpha <= 0.0 || alpha >= 1.0 {
            return Err("PC significance level alpha must be finite and in (0, 1)");
        }
        Ok(())
    }
    ...
    fn is_independent(&self, data: &ObservationalData, x: usize, y: usize, z: &[usize]) -> bool {
        ...
        // Two-tailed test against the standard normal using the configured alpha.
        let critical_value = standard_normal_quantile(1.0 - self.alpha / 2.0);
...
///
/// Kept private to the causal core so this crate does not invert the workspace
/// dependency direction by depending on the domain-level statistics crate.
...
mod tests {
    ...
    #[test]
    fn test_pc_algorithm_rejects_invalid_alpha() {
        for alpha in [
            f64::NAN,
            f64::NEG_INFINITY,
            f64::INFINITY,
            ...
            assert!(PCAlgorithm::try_with_alpha(alpha).is_err(), "alpha={alpha}");
        }
    }

    #[test]
    fn test_standard_normal_quantile_landmarks() {
        assert!((standard_normal_quantile(0.975) - 1.959_963_98).abs() < 1e-6);
        assert!((standard_normal_quantile(0.995) - 2.575_829_30).abs() < 1e-6);
    }
