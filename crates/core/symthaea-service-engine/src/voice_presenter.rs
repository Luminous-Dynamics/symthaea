...
pub enum VoicePresentationDisposition {
    ...
    SuppressedCancelled,
    /// Cancellation was observed by an interruptible renderer after registration.
...
/// returned lease remains alive across the blocking renderer call, so another
/// thread can target the exact active generation. The presentation token is also
/// rechecked after registration and continuously probed by the renderer, closing
/// the stop-before-start race that a transient backend `speaking` flag alone cannot
/// solve.
pub fn present_interruptible_voice_outcome<R: CancellableVoiceRenderer>(
    ...
    {
        CancellableRenderOutcome::Completed => Ok(VoicePresentationDisposition::Presented),
        CancellableRenderOutcome::Cancelled => {
            Ok(VoicePresentationDisposition::CancelledDuringPresentation)
        }
    }
}
