...
//! Deterministic metadata registry for Symthaea extensions.
//!
//! This crate owns no executable plugin objects and performs no loading. It
//! indexes validated manifests by extension ID and semantic capability so a
//! later host/router can select among providers without coupling discovery to
//! runtime policy.
...
use symthaea_extension_core::{
    CapabilityId, ExtensionId, ExtensionManifest, ManifestProblem,
};
