...
fn rel_005a_execution_only_v3() {
    ...
    let held_out_c_direct = estimated_ac
        .as_ref()
        .map(|transform| frame.translate_foreign_hypervector(&held_out_a, transform));
    let direct_vs_sequential_transport_error = match (&held_out_c_direct, &held_out_c_sequential) {
    ...
    let transported_unit = estimated_ab
        .as_ref()
        .map(|transform| frame.translate_foreign_hypervector(&ContinuousHV::ones(8), transform));
    ...
    let empty_anchor_result_is_none = frame.compute_xenobot_graft_transform(&[], &[]).is_none();
    ...
    let shuffled_held_out_max = shuffled_held_out
        .as_ref()
        .and_then(|actual| max_abs_error(actual, &expected_held_out_b));
    let shuffled_mask_dispersion = shuffled_transform
        .as_ref()
        .and_then(|transform| mask_dispersion(&implied_masks(&anchors_b, &shuffled_a), transform));
    ...
    let corrupted_transform = frame.compute_xenobot_graft_transform(&corrupted_targets, &anchors_a);
    ...
    let corrupted_mask_dispersion = corrupted_transform.as_ref().and_then(|transform| {
        mask_dispersion(&implied_masks(&corrupted_targets, &anchors_a), transform)
    });
    ...
    let permuted_targets = anchors_a
        ...
        .collect::<Vec<_>>();
    let permutation_estimate = frame.compute_xenobot_graft_transform(&permuted_targets, &anchors_a);
