...
//!
//! This is deliberately not presented as the final paper-quality readout. It is
//! a dependency-free nearest-centroid classifier that can be held identical
//! across diagonal/contextual HLS ablations. A stronger ridge/gradient head
//! should be evaluated separately rather than hidden inside the recurrence.
...
/// Shared nearest-centroid diagnostic readout.
#[derive(Debug, Clone)]
...
impl TrackingPrototypeReadout {
    ...
    pub fn observe(
        ...
        // Normalize once so examples contribute by direction rather than by
        // reservoir magnitude. This keeps the diagnostic readout simple and
        // reduces accidental advantages from state-norm differences.
    ...
    pub fn entity_observations(&self) -> u64 {
        self.entities.observations()
    }

    pub fn location_observations(&self) -> u64 {
        self.locations.observations()
    }
...
mod tests {
    ...
    #[test]
    fn query_kind_selects_answer_domain() {
        let config = StateTrackingBenchmarkConfig {
            entities: 2,
            locations: 2,
            ..StateTrackingBenchmarkConfig::default()
        };
        let mut readout = TrackingPrototypeReadout::from_benchmark_config(64, &config).unwrap();
        let feature = ContinuousHV::new_random(64, 9);
        readout
            .observe(&feature, TrackingAnswer::Entity(1))
            .unwrap();
        readout
            .observe(&feature, TrackingAnswer::Location(0))
            .unwrap();

        assert_eq!(
            readout
                .predict(&feature, TrackingQueryKind::ObjectOwner { object: 0 })
                .unwrap(),
            TrackingAnswer::Entity(1)
        );
        assert_eq!(
            readout
                .predict(&feature, TrackingQueryKind::ObjectLocation { object: 0 })
                .unwrap(),
            TrackingAnswer::Location(0)
        );
    }
}
