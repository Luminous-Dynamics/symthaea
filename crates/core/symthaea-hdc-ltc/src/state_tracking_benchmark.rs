...
impl Default for StateTrackingBenchmarkConfig {
    fn default() -> Self { Self { entities: 32, objects: 64, locations: 16, events: 10_000, query_every: 10, min_dt: 1e-3, max_dt: 1e2, historical_query_rate: 0.5, seed: 42 } }
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub enum StateTrackingBenchmarkError {
    EmptyDomain(&'static str), TooManyIds(&'static str), InvalidQueryCadence, InvalidTimeRange,
    InvalidHistoricalRate, EventIndexOutOfRange { index: usize, len: usize },
    PredictionCountMismatch { expected: usize, actual: usize },
}
impl fmt::Display for StateTrackingBenchmarkError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::EmptyDomain(n) => write!(f, "benchmark domain {n} must be non-empty"),
            Self::TooManyIds(n) => write!(f, "benchmark domain {n} exceeds u16 id capacity"),
...
pub struct TrackingScore {
    pub correct: usize, pub total: usize, pub current_correct: usize, pub current_total: usize,
    pub historical_correct: usize, pub historical_total: usize, pub entity_location_correct: usize,
    pub entity_location_total: usize, pub object_owner_correct: usize, pub object_owner_total: usize,
    pub object_location_correct: usize, pub object_location_total: usize,
}
impl TrackingScore {
...
pub struct StateTrackingBenchmark {
    ...
    #[serde(default)] pub initialization_events: usize,
...
impl StateTrackingBenchmark {
    pub fn generate(config: StateTrackingBenchmarkConfig) -> Result<Self, StateTrackingBenchmarkError> {
        validate_config(&config)?; let mut rng = XorShift64::new(config.seed);
        let initial_entity_locations = (0..config.entities).map(|_| rng.index(config.locations) as LocationId).collect::<Vec<_>>();
        let initial_object_owners = (0..config.objects).map(|_| rng.index(config.entities) as EntityId).collect::<Vec<_>>();
        let mut bootstrap = Vec::with_capacity(config.entities + config.objects);
        for (entity, &to) in initial_entity_locations.iter().enumerate() { bootstrap.push(TrackingEventKind::MoveEntity { entity: entity as EntityId, to }); }
        for (object, &to) in initial_object_owners.iter().enumerate() { bootstrap.push(TrackingEventKind::TransferObject { object: object as ObjectId, to }); }
        rng.shuffle(&mut bootstrap); let initialization_events = bootstrap.len();
        let mut events = Vec::with_capacity(initialization_events + config.events); let mut time = 0.0;
        for kind in bootstrap { time += sample_log_uniform(&mut rng, config.min_dt, config.max_dt); events.push(TrackingEvent { time, kind }); }
        for _ in 0..config.events {
            time += sample_log_uniform(&mut rng, config.min_dt, config.max_dt);
            let kind = if rng.next_bool() { TrackingEventKind::MoveEntity { entity: rng.index(config.entities) as EntityId, to: rng.index(config.locations) as LocationId } }
                else { TrackingEventKind::TransferObject { object: rng.index(config.objects) as ObjectId, to: rng.index(config.entities) as EntityId } };
            events.push(TrackingEvent { time, kind });
        }
        let mut benchmark = Self { config, events, queries: Vec::new(), initialization_events, initial_entity_locations, initial_object_owners };
        benchmark.queries = benchmark.generate_queries(&mut rng)?; Ok(benchmark)
    }
    ...
    pub fn oracle_answer(&self, as_of_event: usize, kind: &TrackingQueryKind) -> Result<TrackingAnswer, StateTrackingBenchmarkError> {
        if as_of_event >= self.events.len() { return Err(StateTrackingBenchmarkError::EventIndexOutOfRange { index: as_of_event, len: self.events.len() }); }
    ...
    pub fn score(&self, predictions: &[TrackingAnswer]) -> Result<TrackingScore, StateTrackingBenchmarkError> {
        if predictions.len() != self.queries.len() { return Err(StateTrackingBenchmarkError::PredictionCountMismatch { expected: self.queries.len(), actual: predictions.len() }); }
        let mut s = TrackingScore { correct:0,total:self.queries.len(),current_correct:0,current_total:0,historical_correct:0,historical_total:0,entity_location_correct:0,entity_location_total:0,object_owner_correct:0,object_owner_total:0,object_location_correct:0,object_location_total:0 };
        for (p,q) in predictions.iter().zip(&self.queries) { let h=(*p==q.expected) as usize; s.correct+=h; if q.is_historical(){s.historical_total+=1;s.historical_correct+=h}else{s.current_total+=1;s.current_correct+=h}; match q.kind { TrackingQueryKind::EntityLocation{..}=>{s.entity_location_total+=1;s.entity_location_correct+=h},TrackingQueryKind::ObjectOwner{..}=>{s.object_owner_total+=1;s.object_owner_correct+=h},TrackingQueryKind::ObjectLocation{..}=>{s.object_location_total+=1;s.object_location_correct+=h} } }
        Ok(s)
    }
    fn generate_queries(&self, rng:&mut XorShift64)->Result<Vec<TrackingQuery>,StateTrackingBenchmarkError>{
        let mut q=Vec::new(); let Some(first)=self.first_fully_observed_event() else{return Ok(q)}; let start=self.initialization_events+self.config.query_every-1; if start>=self.events.len(){return Ok(q)};
        for asked in (start..self.events.len()).step_by(self.config.query_every){ let e=rng.index(self.config.entities) as EntityId; let o=rng.index(self.config.objects) as ObjectId; self.push_query(&mut q,asked,asked,TrackingQueryKind::EntityLocation{entity:e})?; self.push_query(&mut q,asked,asked,TrackingQueryKind::ObjectOwner{object:o})?; self.push_query(&mut q,asked,asked,TrackingQueryKind::ObjectLocation{object:o})?; if asked>first&&rng.next_f64()<self.config.historical_query_rate{let as_of=first+rng.index(asked-first);let o=rng.index(self.config.objects) as ObjectId;self.push_query(&mut q,asked,as_of,TrackingQueryKind::ObjectLocation{object:o})?;} }
        Ok(q)
    }
