...
fn validate_config(c:&StateTrackingBenchmarkConfig)->Result<(),StateTrackingBenchmarkError>{for(n,v)in[("entities",c.entities),("objects",c.objects),("locations",c.locations),("events",c.events)]{if v==0{return Err(StateTrackingBenchmarkError::EmptyDomain(n))}}for(n,v)in[("entities",c.entities),("objects",c.objects),("locations",c.locations)]{if v>u16::MAX as usize+1{return Err(StateTrackingBenchmarkError::TooManyIds(n))}}
if c.query_every==0{return Err(StateTrackingBenchmarkError::InvalidQueryCadence)}
if !c.min_dt.is_finite()||!c.max_dt.is_finite()||c.min_dt<=0.0||c.max_dt<c.min_dt{return Err(StateTrackingBenchmarkError::InvalidTimeRange)}
if !c.historical_query_rate.is_finite()||!(0.0..=1.0).contains(&c.historical_query_rate){return Err(StateTrackingBenchmarkError::InvalidHistoricalRate)}Ok(())}
