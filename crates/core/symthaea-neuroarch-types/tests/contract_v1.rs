...
use symthaea_neuroarch_types::{
    ...
    EdgeDirection, EdgeId, EdgeTransform, InputMergePolicy, RecurrenceKind, SemanticChannel,
    TOPOLOGY_SCHEMA_VERSION, TimescaleClass, TopologyDescriptor, TopologyError,
};
...
fn canonical_v1_bytes_match_golden() {
    let expected = decode_hex(concat!(
        ...
        "00000100000061028000000000000000020000000000000001070000006669787475726501000200",
        "00000100000062028000000000000000020000000000000001070000006669787475726501000300",
        "00000100000063028000000000000000020000000000000001070000006669787475726501000200",
...
fn rejects_invalid_schema_shape_and_timescale() {
    ...
    let mut overflow = topology();
    overflow.circuits[0].state_dimension = u64::MAX;
    overflow.circuits[0].unit_count = 2;
    assert_eq!(overflow.validate(), Err(TopologyError::DimensionOverflow));
