...
//! Dependency-free SHA-256 primitives for evidence content identity.
//!
//! This is the neutral-core extraction of the SHA-256 implementation that
//! already exists in `symthaea-fabrication-kernel::crypto_digest`. It deliberately
//! provides hashing and digest representation only: a digest authenticates bytes
//! only when the bytes are independently admitted/recomputed and the resulting
//! identity is anchored by the appropriate evidence/trust boundary.
...
/// Cryptographic SHA-256 digest bytes.
...
mod tests {
    ...
    fn digest_hex_round_trip() {
        let digest = sha256(b"evidence-plane");
    ...
    #[test]
    fn chunk_boundaries_around_padding_match_one_shot() {
