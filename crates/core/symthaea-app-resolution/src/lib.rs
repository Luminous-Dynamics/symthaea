...
//! corpus without allowing hash iteration order or ambiguous fuzzy candidates to
//! manufacture a canonical application identity.
//!
//! Resolution order:
//!
...
//!
//! `Ambiguous != Resolved` is the core invariant.
...
impl AppResolver {
    ...
    /// Resolve one observed display name or package identifier.
    ///
...
mod tests {
    ...
    fn candidate_classification_preserves_ambiguity() {
        ...
        candidates.insert(0);
        candidates.insert(1);
        let result = resolver.classify("synthetic-collision", &candidates, ResolutionBasis::UniqueSubstring);
        match result {
    ...
    fn compatibility_helper_refuses_ambiguity_and_unknown() {
        ...
        assert!(resolver.resolve_unique("definitely-not-a-real-application-xyz").is_none());
    }
