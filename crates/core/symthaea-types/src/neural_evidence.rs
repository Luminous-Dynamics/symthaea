    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::MissingModel { authority } => {
                write!(
                    f,
                    "{authority:?} evidence requires an explicit model identifier"
                )
            }
            Self::MissingModelVersion { authority } => {
                write!(
                    f,
                    "{authority:?} evidence requires an explicit model version"
                )
            }
            Self::MissingDataset => {
            ...
            Self::MissingModality { authority } => {
                write!(
                    f,
                    "{authority:?} evidence requires an explicit neural modality"
                )
            }
            Self::MissingSourceDigest { authority } => {
                write!(
                    f,
                    "{authority:?} evidence requires an immutable source digest"
                )
            }
...
mod tests {
    ...
    const BLAKE_C: &str = "blake3:cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc";
    ...
    fn valid_multi_hop_coordinate_lineage_survives_roundtrip() {
        ...
        assert_eq!(
            restored.provenance().native_coordinate_system(),
            CoordinateSystem::FsAverage5
        );
        assert_eq!(
            restored.provenance().coordinate_system(),
            CoordinateSystem::Symthaea12
        );
