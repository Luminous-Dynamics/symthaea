...
//!
//! End-to-end tests for the nixward CLI command parsing,
...
fn test_cli_no_args_is_repl() {
    let cli = Cli::parse_from(["nixward"]);
...
fn test_cli_natural_language_multiword() {
    let cli = Cli::parse_from(["nixward", "why", "did", "my", "rebuild", "fail"]);
    assert!(cli.has_natural_input());
...
fn test_cli_search_with_limit() {
    let cli = Cli::parse_from(["nixward", "search", "firefox", "-n", "5"]);
...
fn test_cli_search_options_flag() {
    let cli = Cli::parse_from(["nixward", "search", "enable", "--options"]);
...
fn test_cli_rebuild_modes() {
    for (args, expected_mode) in [
        (vec!["nixward", "rebuild", "switch"], RebuildMode::Switch),
        (vec!["nixward", "rebuild", "test"], RebuildMode::Test),
        (vec!["nixward", "rebuild", "boot"], RebuildMode::Boot),
    ] {
...
fn test_cli_rebuild_with_flake() {
    let cli = Cli::parse_from(["nixward", "rebuild", "switch", "--flake", ".#myhost"]);
...
fn test_cli_rollback_no_generation() {
    let cli = Cli::parse_from(["nixward", "rollback"]);
...
fn test_cli_rollback_with_generation() {
    let cli = Cli::parse_from(["nixward", "rollback", "42"]);
    if let Some(Command::Rollback { generation }) = cli.command {
...
fn test_cli_observe_domains() {
    for (arg, expected) in [
        ...
        let cli = Cli::parse_from(["nixward", "observe", arg]);
...
fn test_cli_observe_no_domain() {
    let cli = Cli::parse_from(["nixward", "observe"]);
...
#[test]
fn test_cli_doctor() {
    let cli = Cli::parse_from(["nixward", "doctor"]);
...
fn test_cli_gc_modes() {
    ...
    let cli = Cli::parse_from(["nixward", "gc", "--aggressive", "--older-than", "30"]);
    if let Some(Command::Gc {
        analyze,
        aggressive,
        older_than,
...
fn test_cli_service_subcommands() {
    let cli = Cli::parse_from(["nixward", "service", "status", "nginx"]);
    ...
    let cli = Cli::parse_from(["nixward", "service", "start", "postgresql"]);
    if let Some(Command::Service {
    ...
    let cli = Cli::parse_from(["nixward", "service", "stop", "docker"]);
    if let Some(Command::Service {
    ...
    let cli = Cli::parse_from(["nixward", "service", "failed"]);
    assert!(matches!(
        cli.command,
...
fn test_cli_flake_subcommands() {
    let cli = Cli::parse_from(["nixward", "flake", "check"]);
    assert!(matches!(
        cli.command,
        Some(Command::Flake {
    ...
    let cli = Cli::parse_from(["nixward", "flake", "update", "nixpkgs"]);
    ...
    let cli = Cli::parse_from(["nixward", "flake", "show"]);
    assert!(matches!(
        cli.command,
        Some(Command::Flake {
    ...
    let cli = Cli::parse_from(["nixward", "flake", "info"]);
    assert!(matches!(
        cli.command,
        Some(Command::Flake {
...
fn test_cli_phi_override() {
    let cli = Cli::parse_from(["nixward", "--phi", "0.95", "rebuild"]);
    assert_eq!(cli.phi, Some(0.95));
}

#[test]
fn test_cli_verbose_levels() {
    let cli = Cli::parse_from(["nixward", "-v", "3", "doctor"]);
...
fn test_cli_output_formats() {
    for (fmt, expected) in [
        ...
        let cli = Cli::parse_from(["nixward", "--format", fmt, "doctor"]);
...
fn test_hdc_world_model_drift() {
    use nixward::mind::HdcWorldModel;
...
fn test_system_state_encoder_roundtrip() {
    use nixward::encoding::SystemStateSnapshot;
    use nixward::encoding::{NixCodebook, SystemStateEncoder};
...
fn test_journal_anomaly_detector_integration() {
    use nixward::mind::JournalAnomalyDetector;
...
fn test_executor_safety_levels() {
    use nixward::action::executor::NixOSExecutor;
