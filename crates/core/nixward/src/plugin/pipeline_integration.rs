...
//!
//! Provides the types and traits needed to wire nixward's active inference
//! engine into the main Symthaea `ConsciousPipeline`. Since nixward cannot
...
/// Symthaea pipeline. Used by the `nixward` CLI and TUI.
...
mod tests {
    ...
    fn test_causal_graph_save_load_roundtrip() {
        ...
        let dir = std::env::temp_dir().join("nixward_test_roundtrip");
    ...
    fn test_checkpoint_survives_restart() {
        let dir = std::env::temp_dir().join("nixward_test_checkpoint");
        std::fs::create_dir_all(&dir).unwrap();
