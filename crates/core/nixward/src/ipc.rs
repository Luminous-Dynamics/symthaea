...
//!
//! Default path: `$XDG_DATA_HOME/nixward/daemon_state.json`
//! (overridable via `NIXWARD_STATE_DIR`).
...
pub struct DaemonSnapshot {
    ...
    /// Last watchdog verdict (written by `nixward watch`, read by daemon/TUI).
    #[serde(default)]
...
///
/// Default location: `$XDG_CONFIG_HOME/nixward/daemon.json`
/// (overridable via `NIXWARD_CONFIG` environment variable).
#[derive(Debug, Clone, Serialize, Deserialize)]
    ...
    pub fn load(path: &Path) -> Self {
        match std::fs::read_to_string(path) {
            Ok(json) => match serde_json::from_str(&json) {
                ...
                Err(e) => {
                    eprintln!(
                        "nixward: failed to parse config {}: {}, using defaults",
                        path.display(),
                        e
                    );
                    Self::default()
...
pub fn default_config_path() -> PathBuf {
    if let Some(config_path) = std::env::var_os("NIXWARD_CONFIG") {
        ...
        PathBuf::from(home).join(".config/nixward/daemon.json")
    } else {
        PathBuf::from("/etc/nixward/daemon.json")
    }
}

/// Default path for the daemon snapshot file.
pub fn default_snapshot_path() -> PathBuf {
    if let Some(state_dir) = std::env::var_os("NIXWARD_STATE_DIR") {
        ...
        PathBuf::from(home)
            .join(".local/share/nixward")
            .join("daemon_state.json")
    } else {
        PathBuf::from("/var/lib/nixward/daemon_state.json")
    }
}
...
mod tests {
    ...
    fn test_write_and_read() {
        let snap = sample_snapshot();
        let dir = std::env::temp_dir().join("nixward-ipc-test");
    ...
    fn test_read_nonexistent() {
        let result = DaemonSnapshot::read_from(Path::new("/tmp/nonexistent-nixward.json"));
    ...
    fn test_config_save_and_load() {
        let dir = std::env::temp_dir().join("nixward-config-test");
    ...
    fn test_config_load_nonexistent() {
        let config = DaemonConfig::load(Path::new("/tmp/nonexistent-nixward-config.json"));
    ...
    fn test_config_load_invalid_json() {
        let dir = std::env::temp_dir().join("nixward-config-invalid");
        let _ = std::fs::create_dir_all(&dir);
        let path = dir.join("daemon.json");
