...
pub struct SovereignConfigGenerator {
    ...
    // Seeded with real causal patterns in `new()`, but the reason_about_*
    // methods below are still pure hardcoded matches -- not yet consulting
    // these. Kept rather than removed since they're the intended
    // architecture for this generator's reasoning.
    #[allow(dead_code)]
...
impl Default for SovereignConfigGenerator {
    fn default() -> Self {
        Self::new()
    }
}
...
impl SovereignConfigGenerator {
    ...
    fn check_conflicts(&self, options: &[(String, String)]) -> Vec<String> {
        ...
        for (cause, effect, relationship) in &patterns {
            if *relationship == "conflicts"
                && option_set.contains_key(cause)
                && option_set.contains_key(effect)
            {
    ...
    fn render_sovereign_config(&self, options: &[(String, String)], packages: &[String]) -> String {
        let mut lines = vec![
            "# Generated by Symthaea — Sovereign Inoculation".to_string(),
            "# Each option was chosen by reasoning about your hardware,".to_string(),
            "# your existing setup, and NixOS best practices.".to_string(),
            "{ config, pkgs, ... }:".to_string(),
            "{".to_string(),
        ];
