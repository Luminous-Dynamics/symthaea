...
/// Tarjan's strongly-connected-components algorithm over a discovered
/// causal subgraph, keyed by variable name.
///
/// `analyze_root_causes` uses this to tell a genuine cycle (mutually
/// "conflicts" options, or any cyclic relationship the graph might learn
/// live via `discover_structure`/`observe_outcome`) apart from a simple
/// chain: every node inside a fully-explored cycle has an incoming edge
/// from its neighbor, so a naive "no incoming edges = root cause" check
/// can never fire for any of them. Collapsing each SCC to one component
/// first restores that check's validity, since the condensation of any
/// directed graph is always a DAG.
    ...
    fn strongconnect(&mut self, v: &str) {
            ...
            for w in neighbors.clone() {
                if !self.indices.contains_key(&w) {
                    ...
                    if w_low < self.lowlink[v] {
                        self.lowlink.insert(v.to_string(), w_low);
                    }
                } else if self.on_stack.contains(&w) {
                    let w_index = self.indices[&w];
                    if w_index < self.lowlink[v] {
                        self.lowlink.insert(v.to_string(), w_index);
                    }
                }
            }
        }
        ...
        if self.lowlink[v] == self.indices[v] {
            loop {
                ...
                self.component_of.insert(w.clone(), self.component_counter);
                if w == v {
                    break;
                }
            }
...
/// Compute a strongly-connected-component id per variable name across a
/// set of directed edges. Two variables share an id iff they're mutually
/// reachable within `edges` (i.e. part of the same cycle).
fn compute_scc(edges: &[CausalEdge]) -> HashMap<String, usize> {
...
impl NixCausalGraph {
    ...
    pub fn analyze_root_causes(&self, symptom: &str) -> RootCauseAnalysis {
        ...
        // Collapse cycles (e.g. mutually "conflicts" options) into single
        // components before the root-cause check, so a cycle member isn't
        // spuriously disqualified just because its only incoming edge comes
        // from its own cycle-neighbor. A component only counts as "having
        // an incoming edge" if some edge crosses in from a *different*
        // component -- this generalizes the old chain-collapsing behavior
        // (report the ultimate upstream cause, not an intermediate) to also
        // cover cycles: if real external causes feed into the cycle, those
        // are correctly reported as the deeper roots instead of the cycle
        // members; if nothing external feeds in, the cycle members
        // themselves are the roots and are now reported as co-equal causes.
        let scc_id = compute_scc(&causal_chain);

        let mut component_has_external_incoming: HashMap<usize, bool> = HashMap::new();
        for edge in &causal_chain {
            let from_id = scc_id.get(&edge.from);
            let to_id = scc_id.get(&edge.to);
            if let (Some(from_id), Some(to_id)) = (from_id, to_id)
                && from_id != to_id
            {
                component_has_external_incoming.insert(*to_id, true);
            }
        }

        let mut root_causes = Vec::new();
        for edge in &causal_chain {
            // The symptom itself can never be its own root cause, even if
            // it's a member of the cycle being analyzed.
            if edge.from == symptom {
                continue;
            }
            let Some(from_id) = scc_id.get(&edge.from) else {
                continue;
            };
            let is_root = !component_has_external_incoming
                .get(from_id)
                .copied()
                .unwrap_or(false);
            if !is_root {
            ...
            let cycle_peers: Vec<&str> = scc_id
                .iter()
                .filter(|(name, id)| *id == from_id && name.as_str() != edge.from)
                .map(|(name, _)| name.as_str())
                .collect();

            let explanation = if cycle_peers.is_empty() {
                format!(
        ...
        // A variable can be `from` on more than one discovered edge into the
        // same component (e.g. a cycle member reached via two different
        // paths); report it once, keeping the highest-confidence instance.
        let mut seen = HashSet::new();
        root_causes.retain(|rc| seen.insert(rc.variable.clone()));
