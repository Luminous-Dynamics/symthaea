...
pub fn generate_completions(shell: Shell) {
    ...
    clap_complete::generate(shell, &mut cmd, "nixward", &mut std::io::stdout());
}
...
mod tests {
    ...
    fn test_bash_completions_generate() {
        ...
        let mut cmd = Cli::command();
        let mut buf = Vec::new();
        clap_complete::generate(Shell::Bash, &mut cmd, "nixward", &mut buf);
        assert!(!buf.is_empty());
    }
    ...
    fn test_zsh_completions_generate() {
        let mut cmd = Cli::command();
        let mut buf = Vec::new();
        clap_complete::generate(Shell::Zsh, &mut cmd, "nixward", &mut buf);
        assert!(!buf.is_empty());
    }
    ...
    fn test_fish_completions_generate() {
        let mut cmd = Cli::command();
        let mut buf = Vec::new();
        clap_complete::generate(Shell::Fish, &mut cmd, "nixward", &mut buf);
        assert!(!buf.is_empty());
    }
}
