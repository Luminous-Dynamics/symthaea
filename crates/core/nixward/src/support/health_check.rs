...
impl HealthAssessor {
    ...
    pub fn check_services(&self, snapshot: &SystemStateSnapshot) -> HealthCheck {
        ...
        // Systemd `@`-instanced template units (e.g.
        // `drkonqi-coredump-processor@1393-....service`, `backup@daily.service`)
        // are ephemeral, per-event job instances -- not persistent services.
        // Once instantiated they commonly linger in "failed" state
        // indefinitely (systemd doesn't auto-clean the failed list), which
        // previously made routine crash-report/backup/log-processing noise
        // read as identical in severity to an actual persistent service
        // being down -- verified live: 13 accumulated
        // drkonqi-coredump-processor@ instances alone pushed overall health
        // to CRITICAL with a "consider rollback" recommendation, which is
        // nonsensical advice for stale crash-handler bookkeeping. Weight
        // them separately: they inform, but don't by themselves drive
        // CRITICAL or a rollback recommendation. See
        // SYMTHAEA_NIXOS_MANAGEMENT_IMPROVEMENT_PLAN_2026-07-26.md "Doctor
        // severity/confidence calibration".
        let (failed_persistent, failed_instanced): (Vec<&str>, Vec<&str>) =
            failed.iter().partition(|name| !name.contains('@'));

        let (status, message, recs) = if failed_persistent.len() > self.max_failed_services + 2 {
            (
                HealthStatus::Critical,
                format!(
                    "{} persistent service(s) failed: {}{}",
                    failed_persistent.len(),
                    failed_persistent.join(", "),
                    if failed_instanced.is_empty() {
                        String::new()
                    } else {
                        format!(
                            " (plus {} ephemeral instanced-unit failure(s), likely routine)",
                            failed_instanced.len()
                        )
                    }
                ),
                vec![
                    format!("Check logs: journalctl -u {}", failed_persistent[0]),
                    "Review recent changes: nixos-rebuild list-generations".to_string(),
                    "Consider rollback: nixos-rebuild switch --rollback".to_string(),
                ],
            )
        } else if !failed_persistent.is_empty() {
            (
                HealthStatus::Warning,
                format!(
                    "{} persistent service(s) failed: {}",
        ...
        } else if !failed_instanced.is_empty() {
            (
                HealthStatus::Warning,
                format!(
                    "{} ephemeral instanced-unit failure(s), usually routine (not a live \
                     regression): {}",
                    failed_instanced.len(),
                    failed_instanced.join(", ")
                ),
                vec![
                    "These are typically one-shot @-instanced jobs (crash processors, backup \
                     runs, etc.) that systemd doesn't auto-clean from the failed list -- \
                     investigate only if the count keeps growing; otherwise clear with: \
...
mod tests {
    ...
    /// Characterization test mined from a real, documented incident (not a
    /// synthetic fixture): CLAUDE.md records the shared mail-conductor
    /// process reaching "22.6GB RSS (69% of a 31GB machine) within 5
    /// minutes of a reboot" before it was caught and fixed with a
    /// systemd MemoryHigh/MemoryMax guardrail. Tier 1 of the test-corpus
    /// plan (SYMTHAEA_NIXOS_MANAGEMENT_IMPROVEMENT_PLAN_2026-07-26.md)
    /// asks: would Doctor's default thresholds have caught this?
    ///
    /// Honest answer, asserted here rather than silently glossed over:
    /// **no.** 69% used is below the default 80% memory_warn_pct, so
    /// overall-memory checks alone would have reported Healthy while this
    /// was actively happening. This isn't a bug in check_memory (69% of a
    /// machine's RAM isn't inherently alarming in general) -- it's a real,
    /// disclosed coverage gap: check_memory only sees aggregate system
    /// memory, with no per-service/per-process breakdown, so it structurally
    /// cannot distinguish "several services are using memory normally" from
    /// "one specific service just started ballooning" until the aggregate
    /// crosses a fixed threshold regardless of trend or attribution. Left
    /// as a documented, known limitation rather than force-fixed.
    #[test]
    fn test_mail_conductor_memory_incident_below_default_warn_threshold() {
        let assessor = HealthAssessor::default();
        let hw = make_hardware(0.0, 69.0);
        let check = assessor.check_memory(&hw);

        assert_eq!(
            check.status,
            HealthStatus::Healthy,
            "documents a real coverage gap: this real historical incident (69% RAM used) \
             would NOT have been flagged by check_memory's default 80% warn threshold -- \
             aggregate-only memory checks can't see a single service ballooning until the \
             whole-system aggregate crosses a fixed line"
        );
    }
    ...
    #[test]
    fn test_many_instanced_failures_alone_are_not_critical() {
        // Regression test: 13 accumulated @-instanced crash-processor
        // failures (a real live observation on this host) used to push
        // overall health to CRITICAL with a "consider rollback"
        // recommendation -- nonsensical for stale, self-accumulating
        // crash-handler bookkeeping rather than a live regression.
        let assessor = HealthAssessor::default();
        let snapshot = SystemStateSnapshot {
            services: (0..13)
                .map(|i| {
                    (
                        format!("drkonqi-coredump-processor@{i}-123.service"),
                        ServiceState::Failed,
                    )
                })
                .collect(),
        ...
        assert_eq!(
            check.status,
            HealthStatus::Warning,
            "many instanced-only failures should warn, not go CRITICAL"
        );
        assert!(
            !check.recommendations.iter().any(|r| r.contains("rollback")),
            "instanced-only failures should never recommend a system rollback, got: {:?}",
            check.recommendations
        );
    }

    #[test]
    fn test_mixed_persistent_and_instanced_failures_severity_driven_by_persistent() {
        let assessor = HealthAssessor::default();
        let snapshot = make_snapshot(
            vec![
                ("nginx.service", ServiceState::Failed),
                (
                    "drkonqi-coredump-processor@1-1.service",
                    ServiceState::Failed,
                ),
                (
                    "drkonqi-coredump-processor@2-2.service",
                    ServiceState::Failed,
                ),
            ],
            None,
        ...
        // Only one persistent failure -- below the >2 critical threshold --
        // so this should warn, driven by nginx alone, with the instanced
        // units not counted toward the persistent-service threshold.
        assert_eq!(check.status, HealthStatus::Warning);
        assert!(check.message.contains("nginx.service"));
        assert!(!check.message.contains("drkonqi"));
    }
