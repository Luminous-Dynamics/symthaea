...
//! HDC + Active Inference benchmarks for nixward.

use criterion::{Criterion, black_box, criterion_group, criterion_main};

use nixward::encoding::{NixCodebook, OptionEncoder, UserInputEncoder};
use nixward::mind::{NixActiveInference, causal_graph::NixCausalGraph};
...
fn bench_causal_query(c: &mut Criterion) {
    ...
    for (cause, effect, _) in nixward::mind::causal_graph::NixOSCausalPatterns::known_patterns() {
...
fn bench_semantic_search(c: &mut Criterion) {
    use nixward::encoding::search_options;
