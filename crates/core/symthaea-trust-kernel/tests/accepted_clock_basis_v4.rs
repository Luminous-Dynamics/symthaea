...
use symthaea_trust_kernel::{
    ...
    ClockBootstrapClaimV2, ClockEvaluationPolicyV3, ClockObservation, ClockObservationVerifier,
    ClockQuorumPolicyRevisionV1, ClockViolation, DetachedSignature, KeyLifecycleStatus,
    KeyTrustRecord, KeyUsage, Sha256Digest, SignatureAlgorithm, TrustSnapshot,
    accept_bootstrap_clock_basis_v4, derive_bootstrap_clock_evaluation_permit_v3,
...
const BASIS_ID: &str = "77728f18e60bb095bcc744657bd1c2faa15f86e85853d363ebb4145d8a4ee3de";
...
fn evaluation_policy(quorum: &ClockQuorumPolicyRevisionV1) -> ClockEvaluationPolicyV3 {
    ClockEvaluationPolicyV3::new(repeated_hex('b'), quorum, 2_000, 2, true).unwrap()
}

fn permit_for(snapshot: &TrustSnapshot) -> symthaea_trust_kernel::ClockEvaluationPermitV3 {
    ...
    .unwrap();
    let authority = verify_clock_bootstrap_authority(&claim, &evidence, &BootstrapVerifier)
        .unwrap();
    derive_bootstrap_clock_evaluation_permit_v3(
        &authority,
        &claim,
        &evaluation,
        &quorum,
...
#[test]
fn frozen_v4_basis_matches_independent_reference() {
    ...
    let basis = accept_bootstrap_clock_basis_v4(
        &permit,
...
#[test]
fn candidate_threshold_comes_from_bound_quorum_policy_not_eligible_pool_minimum() {
    let snapshot = standard_snapshot();
    let permit = permit_for(&snapshot);
    ...
    let verifier = ObservationVerifier::accepting();
    let error = accept_bootstrap_clock_basis_v4(&permit, &one, &snapshot, &verifier)
        .unwrap_err();
    match error {
...
fn quorum_uncertainty_limit_cannot_be_weakened_at_acceptance_time() {
    ...
    let verifier = ObservationVerifier::accepting();
    let error = accept_bootstrap_clock_basis_v4(&permit, &observations, &snapshot, &verifier)
        .unwrap_err();
    match error {
        AcceptedClockBasisError::QuorumVerificationFailed(violations) => {
            assert!(violations.iter().any(|violation| matches!(
...
fn trust_snapshot_substitution_fails_before_signature_verification() {
    ...
    let error = accept_bootstrap_clock_basis_v4(
        &permit,
        &standard_observations(),
        &substituted,
        &verifier,
