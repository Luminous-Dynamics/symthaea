...
const CLOCK_CONTINUITY_DIGEST_DOMAIN: &[u8] = b"symthaea.fabrication.clock-continuity-digest.v1\0";
...
pub fn verify_clock_continuity(
    ...
    let successor_sources = successor.source_ids.iter().cloned().collect::<BTreeSet<_>>();
    ...
    let successor_algorithms = successor.algorithms.iter().cloned().collect::<BTreeSet<_>>();
...
fn digest_clock_continuity_fields(
    ...
    Ok(domain_hash(CLOCK_CONTINUITY_DIGEST_DOMAIN, &bytes))
}
