...
//! Authority-bound quorum semantics and whole-envelope clock evaluation permits.
//!
//! V3 separates two distinct policies:
//! - the exact candidate-window quorum/uncertainty/consensus theorem; and
//! - the whole-envelope key-pool/transition theorem.
//!
//! A private permit retains the validated quorum policy itself so downstream
//! clock admission never needs a caller-supplied `ClockQuorumPolicy`.
...
pub const CLOCK_EVALUATION_POLICY_SCHEMA: &str = "symthaea.trust.clock-evaluation-policy.v3";
pub const CLOCK_BOOTSTRAP_ANCHOR_SCHEMA: &str = "symthaea.trust.clock-bootstrap-anchor.v2";
pub const CLOCK_EVALUATION_PERMIT_SCHEMA: &str = "symthaea.trust.clock-evaluation-permit.v3";

const CLOCK_QUORUM_POLICY_DOMAIN: &[u8] = b"symthaea.trust.clock-quorum-policy.v1\0";
const CLOCK_EVALUATION_POLICY_DOMAIN: &[u8] = b"symthaea.trust.clock-evaluation-policy.v3\0";
const CLOCK_BOOTSTRAP_ANCHOR_DOMAIN: &[u8] = b"symthaea.trust.clock-bootstrap-anchor.v2\0";
const CLOCK_EVALUATION_PERMIT_DOMAIN: &[u8] = b"symthaea.trust.clock-evaluation-permit.v3\0";
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct ClockEvaluationPolicyV3 {
    schema: String,
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub struct ClockEvaluationPolicyIdV3(Sha256Digest);

#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub struct ClockBootstrapAnchorIdV2(Sha256Digest);

#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub struct ClockEvaluationPermitIdV3(Sha256Digest);
...
#[derive(Debug, Clone)]
#[must_use]
pub struct ClockEvaluationPermitV3 {
    id: ClockEvaluationPermitIdV3,
    basis_id: ClockBootstrapAnchorIdV2,
    evaluation_policy_id: ClockEvaluationPolicyIdV3,
...
impl ClockQuorumPolicyRevisionV1 {
    ...
    pub fn id(&self) -> ClockQuorumPolicyRevisionIdV1 {
        ClockQuorumPolicyRevisionIdV1(self.id)
    }

    pub fn minimum_distinct_sources(&self) -> usize {
    ...
    pub fn maximum_uncertainty_ms(&self) -> u64 {
        self.maximum_uncertainty_ms
    }

    pub fn maximum_consensus_width_ms(&self) -> u64 {
        self.maximum_consensus_width_ms
    }
...
impl ClockEvaluationPolicyV3 {
    pub fn new(
        ...
        if max_transition_ms == 0 || minimum_eligible_clock_authority_keys < 2 {
    ...
    pub fn id(&self) -> ClockEvaluationPolicyIdV3 {
        ClockEvaluationPolicyIdV3(self.id)
    }
    ...
    pub fn clock_quorum_policy_id(&self) -> ClockQuorumPolicyRevisionIdV1 {
        ClockQuorumPolicyRevisionIdV1(self.clock_quorum_policy_id)
    }
    ...
    pub fn minimum_eligible_clock_authority_keys(&self) -> usize {
        self.minimum_eligible_clock_authority_keys
    }

    pub fn require_eligible_algorithm_diversity(&self) -> bool {
        self.require_eligible_algorithm_diversity
    }
}
...
digest_id_impl!(ClockEvaluationPolicyIdV3);
digest_id_impl!(ClockBootstrapAnchorIdV2);
digest_id_impl!(ClockEvaluationPermitIdV3);
...
impl ClockEvaluationPermitV3 {
    pub fn id(&self) -> ClockEvaluationPermitIdV3 {
        self.id
    }
    ...
    pub fn evaluation_policy_id(&self) -> ClockEvaluationPolicyIdV3 {
    ...
    pub fn eligible_clock_keys(&self) -> &[ClockAuthorityKeyV1] {
        &self.eligible_clock_keys
    }

    pub fn clock_quorum_policy(&self) -> &ClockQuorumPolicyRevisionV1 {
        &self.clock_quorum_policy
    }

    pub fn runtime_quorum_policy(&self) -> Result<ClockQuorumPolicy, ClockEvaluationPermitError> {
        self.clock_quorum_policy.to_runtime_policy()
    }
}

pub fn derive_bootstrap_clock_evaluation_permit_v3(
    authority: &VerifiedClockBootstrapAuthorityV2,
    claim: &ClockBootstrapClaimV2,
    evaluation_policy: &ClockEvaluationPolicyV3,
    clock_quorum_policy: &ClockQuorumPolicyRevisionV1,
    trust_snapshot: &TrustSnapshot,
) -> Result<ClockEvaluationPermitV3, ClockEvaluationPermitError> {
    claim
        .validate()
    ...
    if claim.clock_evaluation_policy_id() != evaluation_policy.id().as_digest() {
        return Err(ClockEvaluationPermitError::BootstrapPolicyMismatch);
    }
    if evaluation_policy.clock_quorum_policy_id() != clock_quorum_policy.id() {
        return Err(ClockEvaluationPermitError::QuorumPolicyMismatch);
    }
    ...
    let evaluation_upper_unix_ms = claim
        ...
        .checked_add(evaluation_policy.max_transition_ms())
    ...
    let permit_id = ClockEvaluationPermitIdV3(compute_permit_id(
        basis_id,
        evaluation_policy.id(),
        clock_quorum_policy.id(),
        snapshot_digest,
        evaluation_lower_unix_ms,
        evaluation_upper_unix_ms,
    ...
    Ok(ClockEvaluationPermitV3 {
        id: permit_id,
        basis_id,
        evaluation_policy_id: evaluation_policy.id(),
        clock_quorum_policy_id: clock_quorum_policy.id(),
        trust_snapshot_digest: snapshot_digest,
        evaluation_lower_unix_ms,
        evaluation_upper_unix_ms,
        minimum_eligible_clock_authority_keys: evaluation_policy
            .minimum_eligible_clock_authority_keys(),
        require_eligible_algorithm_diversity: evaluation_policy
            .require_eligible_algorithm_diversity(),
        eligible_clock_keys,
        clock_quorum_policy: clock_quorum_policy.clone(),
    })
}
...
fn eligible_clock_authority_keys_for_envelope(
    ...
    lower_unix_ms: u64,
    upper_unix_ms: u64,
    policy: &ClockEvaluationPolicyV3,
    ...
    for key in &trust_snapshot.keys {
        ...
        eligible.push(ClockAuthorityKeyV1 {
...
fn compute_quorum_policy_id(
    policy: &ClockQuorumPolicyRevisionV1,
) -> Result<Sha256Digest, ClockEvaluationPermitError> {
    let bytes = canonical_json_bytes([
        (
            "maximum_consensus_width_ms",
            Value::from(policy.maximum_consensus_width_ms),
        ),
        ("maximum_observations", Value::from(policy.maximum_observations as u64)),
        (
            "maximum_uncertainty_ms",
            Value::from(policy.maximum_uncertainty_ms),
        ),
        (
            "minimum_distinct_sources",
            Value::from(policy.minimum_distinct_sources as u64),
        ),
        (
            "require_algorithm_diversity",
...
fn compute_evaluation_policy_id(
    policy: &ClockEvaluationPolicyV3,
) -> Result<Sha256Digest, ClockEvaluationPermitError> {
    let bytes = canonical_json_bytes([
        (
            "clock_quorum_policy_id",
            Value::String(policy.clock_quorum_policy_id.to_hex()),
        ),
        ("max_transition_ms", Value::from(policy.max_transition_ms)),
        (
            "minimum_eligible_clock_authority_keys",
            Value::from(policy.minimum_eligible_clock_authority_keys as u64),
        ),
        (
            "policy_record_digest",
        ...
        (
            "require_eligible_algorithm_diversity",
            Value::Bool(policy.require_eligible_algorithm_diversity),
        ),
        ("schema", Value::String(policy.schema.clone())),
    ])?;
