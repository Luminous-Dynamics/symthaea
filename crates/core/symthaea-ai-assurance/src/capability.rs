...
//!
//! The model is intentionally small: a trusted host owns an [`AuthorityRoot`]
//! and issues scoped capabilities to less-trusted components. Capabilities are
//! affine Rust values (neither `Copy` nor `Clone`) and delegation can only
//! preserve or narrow scope and expiry.
...
/// Immutable metadata shared by reusable and one-shot grants.
#[derive(Debug, Clone, PartialEq, Eq)]
...
impl<K: CapabilityKind> Capability<K> {
    ...
    pub fn validate_at(&self, now: SystemTime) -> Result<(), GrantError> {
        if self.metadata.is_valid_at(now) {
            Ok(())
        } else {
    ...
    pub fn delegate(
        ...
        if !self.metadata.scope.contains(&scope) {
            return Err(GrantError::ScopeWidening {
                parent: self.metadata.scope.clone(),
                requested: scope,
            });
        }

        if let Some(parent_expiry) = self.metadata.expires_at {
            match expires_at {
        ...
        Ok(Self {
            metadata: GrantMetadata {
                ...
                parent_grant_id: Some(self.metadata.grant_id),
                issuer: self.metadata.subject,
                subject,
                scope,
                expires_at,
                delegation_depth,
...
/// One-shot authority consumed by an authorization transition.
///
/// There is intentionally no conversion from this type into reusable
/// [`Capability`] and no `Clone` implementation.
#[derive(Debug)]
...
impl<K: CapabilityKind> OneShotCapability<K> {
    ...
    pub(crate) fn into_metadata(self) -> GrantMetadata {
        self.metadata
    }
}
...
impl AuthorityRoot {
    ...
    /// Mint authority that can authorize exactly one transition through the
    /// safe API because the grant value is consumed.
...
pub enum GrantError {
    ...
    ScopeWidening { parent: Scope, requested: Scope },
...
mod tests {
    ...
    fn delegation_can_narrow_but_not_widen_scope() {
        ...
        let narrowed = grant
            ...
            .unwrap();
        assert_eq!(narrowed.metadata().parent_grant_id(), Some(grant.metadata().grant_id()));
        assert_eq!(narrowed.metadata().issuer(), holder);

        let widened = grant.delegate(child, Scope::namespace_root("workspace").unwrap(), None, SystemTime::now());
    ...
    fn finite_parent_expiry_cannot_be_removed_or_extended() {
        ...
        let root = AuthorityRoot::new(PrincipalId::new());
        let grant = root.issue::<Write>(PrincipalId::new(), scope(&["scratch"]), Some(parent_expiry));
    ...
    fn expired_grant_fails_closed() {
        ...
        assert!(matches!(grant.validate_at(now), Err(GrantError::Expired { .. })));
    }
