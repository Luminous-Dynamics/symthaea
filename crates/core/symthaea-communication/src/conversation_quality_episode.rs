...
impl LatencyFindingV1 {
    pub fn new(
        ...
        let valid = match budget_finding {
            LatencyBudgetFindingV1::NotEvaluated => true,
            LatencyBudgetFindingV1::NotEstablished => latency_ns.is_none(),
            LatencyBudgetFindingV1::WithinBudget | LatencyBudgetFindingV1::Exceeded => {
                latency_ns.is_some()
            }
        };
        if !valid {
            return Err(ConversationQualityEpisodeErrorV1::InconsistentLatencyEvidence);
        }
        Ok(Self {
            latency_ns,
            budget_finding,
...
impl ConversationQualityEpisodeReceiptV1 {
    pub fn new(
        ...
        let voice_ref = manifest
            .components
            .get(&ConversationComponentKindV1::FullDuplexVoice)
            .expect("validated manifest contains voice component");
        let correction_ref = manifest
            .components
            .get(&ConversationComponentKindV1::Correction)
            .expect("validated manifest contains correction component");
        let persona_ref = manifest
            .components
            .get(&ConversationComponentKindV1::PersonaContinuity)
            .expect("validated manifest contains persona component");
    ...
    pub fn validate(
        ...
        if expected.receipt_commitment != self.receipt_commitment {
