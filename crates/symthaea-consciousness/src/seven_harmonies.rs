...
impl EncodedHarmony {
    ...
    pub fn new(harmony: Harmony, _encoder: &mut SemanticEncoder) -> Self {
        // Use word-wise bundling for semantic similarity
        // This captures word overlap between action descriptions and harmony keywords
        let encoding = Self::encode_text_wordwise(harmony.description());

        // Encode anti-patterns using word-wise bundling
        let anti_patterns: Vec<HV16> = harmony.anti_patterns()
            .iter()
            .map(|pattern| Self::encode_text_wordwise(pattern))
            .collect();
    ...
    /// Encode text using word-level bundling for semantic similarity
    ///
    /// This method creates an HV16 by bundling (superposition) of word vectors.
    /// Each word gets a deterministic random vector, and the bundle is the
    /// majority vote across all word vectors. This captures semantic overlap:
    /// phrases sharing words will have proportionally higher similarity.
    ///
    /// Uses BLAKE3 hashing for word-to-seed mapping to ensure determinism.
    pub fn encode_text_wordwise(text: &str) -> HV16 {
        use blake3::Hasher;
        ...
        // Accumulator for majority vote
        let mut accum = vec![0i32; HV16::DIM];
        ...
        for word in &words {
            ...
            let word_hv = HV16::random(seed);

            // Add to accumulator (bipolar: bit=1 -> +1, bit=0 -> -1)
            for byte_idx in 0..HV16::BYTES {
                for bit_idx in 0..8 {
                    let dim_idx = byte_idx * 8 + bit_idx;
                    if dim_idx < HV16::DIM {
                        let bit = (word_hv.0[byte_idx] >> bit_idx) & 1;
                        accum[dim_idx] += if bit == 1 { 1 } else { -1 };
                    }
                }
            }
        }
        ...
        for (dim_idx, &sum) in accum.iter().enumerate() {
            if sum > 0 {
    ...
    ///
    /// Uses normalized similarity where 0.5 (random baseline) = 0 alignment.
    /// Only counts anti-pattern penalties for significant matches (> 0.55).
    pub fn alignment(&self, action_encoding: &HV16) -> f32 {
        // Raw similarity to harmony (0.0-1.0 where 0.5 is random)
        let harmony_sim = action_encoding.similarity(&self.encoding);

        // Normalize to [-1, +1] where 0 = random baseline
        let positive = (harmony_sim - 0.5) * 2.0;

        // Check for anti-pattern matches above random threshold
        let anti_threshold = 0.55;  // Only penalize significant matches
        let negative: f32 = self.anti_patterns.iter()
            .map(|anti| {
                let sim = action_encoding.similarity(anti);
                if sim > anti_threshold {
                    (sim - 0.5) * 2.0  // Normalize to [0, +1] for significant matches
                } else {
                    0.0  // Ignore random noise
                }
            })
            .max_by(|a, b| a.partial_cmp(b).unwrap())
            .unwrap_or(0.0);

        // Net alignment: positive contribution minus anti-pattern penalty
...
impl SevenHarmonies {
    ...
    pub fn evaluate_action(&mut self, action_description: &str) -> AlignmentResult {
        ...
        // Encode the action using word-wise bundling for semantic similarity
        let action_encoding = EncodedHarmony::encode_text_wordwise(action_description);
...
mod tests {
    ...
    fn test_positive_alignment() {
        ...
        // This phrase contains "compassion" and "care" which match PanSentientFlourishing
        let result = harmonies.evaluate_action(
            "help the user understand their options with compassion and care"
        );

        // Should align positively with Pan-Sentient Flourishing (shares words: care, compassion)
        let pan_sentient = result.harmonies.iter()
            .find(|h| matches!(h.harmony, Harmony::PanSentientFlourishing));
        assert!(pan_sentient.map(|h| h.alignment > 0.0).unwrap_or(false),
               "PanSentientFlourishing should have positive alignment");

        // The specific harmony should be above baseline
        assert!(pan_sentient.unwrap().alignment > 0.0);
    }
    ...
    #[test]
    fn test_wordwise_similarity() {
        // Test that word-level bundling produces meaningful similarities

        // Same text should have similarity 1.0
        let a1 = EncodedHarmony::encode_text_wordwise("care compassion love");
        let a2 = EncodedHarmony::encode_text_wordwise("care compassion love");
        let same_sim = a1.similarity(&a2);
        assert!((same_sim - 1.0).abs() < 0.01, "Same text should have sim ~1.0");

        // Overlapping words should have higher similarity
        let b1 = EncodedHarmony::encode_text_wordwise("care compassion love");
        let b2 = EncodedHarmony::encode_text_wordwise("care compassion help");
        let overlap_sim = b1.similarity(&b2);
        assert!(overlap_sim > 0.6, "Overlapping words should have sim > 0.6");

        // Different words should have ~0.5 similarity (random baseline)
        let c1 = EncodedHarmony::encode_text_wordwise("apple banana cherry");
        let c2 = EncodedHarmony::encode_text_wordwise("dog cat mouse");
        let diff_sim = c1.similarity(&c2);
        assert!(diff_sim > 0.45 && diff_sim < 0.55, "Different words should have sim ~0.5");
    }
