...
use symtropy_math::{Bivector, HyperBox, Point, Transform};
...
async fn main() -> Result<()> {
    ...
    info!(
        "🌱 Ground platform established (Handle: {:?})",
        ground_handle
    );
    ...
    info!(
        "🤖 Humanoid Spawned: {} DOF | Mass: {}kg | Dimension: {}D",
        n_actuators,
    ...
    info!(
        "⏳ Running {}s simulation with Ground Impacts...",
        SIM_DURATION_SECS
    );
    ...
    while current_time < SIM_DURATION_SECS {
        ...
        if current_time >= SHOVE_TIME && !shove_applied {
            info!(
                "💥 [T={:.2}s] INJECTING EXTERNAL SHOVE: 500N Force Vector",
                current_time
            );
        ...
        if step_count % 30 == 0 {
            ...
            info!(
                "[T={:.2}s] System Energy: {:.2}J | Total Loss: {:.2}J",
    ...
    info!(
        "   🌡️  Total Energy Consumed (Work): {:.2} Joules",
        ledger.lifetime_energy
    );
    info!(
        "   🔥 Total Dissipation (Heat): {:.2} Joules",
        ledger.energy_out
    );
    info!(
        "   📉 Energy Conservation Error: {:.6}%",
        ledger.lifetime_error_rate() * 100.0
    );

    if ledger.lifetime_error_rate() < 0.2 {
        // Wider tolerance for complex collisions
        info!("   ✅ Determinism & Conservation Check PASSED.");
    } else {
        warn!(
            "   ⚠️  Conservation Check FAILED: Energy drift {:.2}%",
            ledger.lifetime_error_rate() * 100.0
        );
    }
