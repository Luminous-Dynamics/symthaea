...
pub fn generate(
    ...
    let configuration_nix = build_configuration_nix(hw, choices, &nix_pkgs, &gpu_vendor, &desktop_lower, &resolved_custom);
...
fn write_encryption(out: &mut String, choices: &UserChoices) {
    ...
    if choices.fido2_unlock || choices.tpm2_unlock {
        writeln!(out, "  boot.initrd.systemd.enable = true;  # required for FIDO2/TPM2 unlock at boot").unwrap();
    }
...
fn write_desktop(out: &mut String, desktop: &str, choices: &UserChoices) {
    match desktop {
        "gnome" => {
            ...
            writeln!(out, "  services.xserver.desktopManager.gnome.enable = true;").unwrap();
        }
        "kde" => {
        ...
        "cosmic" => {
            ...
            writeln!(out, "  services.displayManager.cosmic-greeter.enable = true;").unwrap();
        }
        _ => {
    ...
    if choices.auto_login {
        let name = if choices.username.is_empty() { "user" } else { &choices.username };
        match desktop {
            "gnome" | "kde" | "xfce" | "cosmic" => {
                writeln!(out, "  services.displayManager.autoLogin.enable = true;").unwrap();
                writeln!(out, "  services.displayManager.autoLogin.user = \"{}\";", name).unwrap();
            }
            _ => {
                writeln!(out, "  # Auto-login not applicable for {} (no display manager)", desktop).unwrap();
            }
        }
    }
    writeln!(out).unwrap();
...
fn write_packages(out: &mut String, nix_pkgs: &[&str], custom: &[String]) {
    writeln!(out, "  # Package names verified against nixpkgs {}", crate::CURRENT_VERIFIED_CHANNEL).unwrap();
    writeln!(out, "  # If a package fails to build, search: nix search nixpkgs <name>").unwrap();
...
fn write_btrfs_maintenance(out: &mut String) {
    writeln!(out, "  # Automatic btrfs snapshots & scrub (NixForHumanity)").unwrap();
...
fn write_zfs_config(out: &mut String) {
    ...
    writeln!(out, "  networking.hostId = \"deadbeef\"; # Replace with: head -c 8 /etc/machine-id\n").unwrap();
}
...
fn write_chromebook_config(out: &mut String) {
    ...
    writeln!(out, "  # Chromebook keyboard: consider remapping Search key to Super or Ctrl").unwrap();
    writeln!(out, "  # services.xserver.xkb.options = \"chromebook:search_overlay\";\n").unwrap();
}
...
fn write_swap(out: &mut String, swap_gb: u32) {
    if swap_gb > 0 {
        ...
        writeln!(out, "  zramSwap = {{ enable = true; algorithm = \"zstd\"; }};\n").unwrap();
    }
}
...
fn write_kernel(out: &mut String, kernel: &str) {
    match kernel {
        ...
        "hardened" => writeln!(out, "  boot.kernelPackages = pkgs.linuxPackages_hardened;\n").unwrap(),
...
fn write_symthaea(out: &mut String) {
    ...
    writeln!(out, "  # Natural language NixOS management + consciousness metrics").unwrap();
...
fn build_flake_nix(choices: &UserChoices) -> String {
    ...
    if choices.home_manager {
        writeln!(out, "    home-manager = {{").unwrap();
        writeln!(out, "      url = \"github:nix-community/home-manager/release-25.05\";").unwrap();
        writeln!(out, "      inputs.nixpkgs.follows = \"nixpkgs\";").unwrap();
        writeln!(out, "    }};").unwrap();
    }
    if choices.symthaea_edition || choices.mycelix_edition {
        writeln!(out, "    # Future: Symthaea/Mycelix flake inputs will be added here").unwrap();
        writeln!(out, "    # when published as standalone flakes. For now, services are").unwrap();
        writeln!(out, "    # configured via system packages in configuration.nix.").unwrap();
    }
    writeln!(out, "  }};").unwrap();
    writeln!(out).unwrap();
    ...
    if choices.secure_boot { output_args.push("lanzaboote"); }
    if choices.home_manager { output_args.push("home-manager"); }
...
mod tests {
    ...
    fn validate_valid_config() {
        ...
        assert!(errors.is_empty(), "Generated config should be valid, got: {:?}", errors);
    }
    ...
    fn validate_missing_hardware_import() {
        ...
        let errors = validate_nix_syntax(nix);
        assert!(errors.iter().any(|e| e.contains("hardware-configuration.nix")));
    }
  ...
  environment.systemPackages = [ pkgs.vim ];
        ...
        let errors = validate_nix_syntax(nix);
        assert!(errors.is_empty(), "Valid nix should have no errors, got: {:?}", errors);
    }
    ...
    fn fido2_config_included() {
        ...
        let result = generate(&test_hw(), &c, &[]);
        let nix = &result.configuration_nix;
        assert!(nix.contains("boot.initrd.systemd.enable = true"), "FIDO2 requires systemd initrd");
        assert!(nix.contains("fido2-device=auto"), "FIDO2 cryptTabExtraOpts should be present");
    }
    ...
    fn zfs_config_included() {
        ...
        let result = generate(&test_hw(), &c, &[]);
        let nix = &result.configuration_nix;
        assert!(nix.contains("boot.supportedFilesystems = [ \"zfs\" ]"), "ZFS support should be enabled");
        assert!(nix.contains("services.zfs.autoScrub.enable = true"), "ZFS auto-scrub should be enabled");
        assert!(nix.contains("services.zfs.trim.enable = true"), "ZFS trim should be enabled");
        assert!(nix.contains("networking.hostId"), "ZFS requires networking.hostId");
        // Should NOT have btrfs maintenance
        assert!(!nix.contains("services.btrfs.autoScrub"), "ZFS config should not include btrfs scrub");
    }
    ...
    fn chromebook_config() {
        ...
        let result = generate(&hw, &test_choices(), &[]);
        let nix = &result.configuration_nix;
        assert!(nix.contains("services.xserver.libinput.enable = true"), "Chromebook should enable libinput");
    ...
    fn shell_zsh_config() {
        ...
        let result = generate(&test_hw(), &c, &[]);
        let nix = &result.configuration_nix;
        assert!(nix.contains("programs.zsh.enable = true"), "Zsh should be enabled");
        assert!(nix.contains("users.users.tristan.shell = pkgs.zsh"), "User shell should be set to zsh");
    }
    ...
    fn shell_fish_config() {
        ...
        let result = generate(&test_hw(), &c, &[]);
        let nix = &result.configuration_nix;
        assert!(nix.contains("programs.fish.enable = true"), "Fish should be enabled");
        assert!(nix.contains("users.users.tristan.shell = pkgs.fish"), "User shell should be set to fish");
    }
    ...
    fn bluetooth_config() {
        ...
        let result = generate(&test_hw(), &c, &[]);
        let nix = &result.configuration_nix;
        assert!(nix.contains("hardware.bluetooth.enable = true"), "Bluetooth should be enabled");
        assert!(nix.contains("hardware.bluetooth.powerOnBoot = true"), "Bluetooth powerOnBoot should be set");
    }
    ...
    fn printing_config() {
        ...
        let result = generate(&test_hw(), &c, &[]);
        let nix = &result.configuration_nix;
        assert!(nix.contains("services.printing.enable = true"), "Printing should be enabled");
    }
    ...
    fn laptop_config() {
        ...
        let result = generate(&test_hw(), &c, &[]);
        let nix = &result.configuration_nix;
        assert!(nix.contains("services.thermald.enable = true"), "Thermald should be enabled for laptop");
        assert!(nix.contains("services.power-profiles-daemon.enable = true"), "Power profiles should be enabled");
        assert!(nix.contains("services.logind.lidSwitch = \"suspend\""), "Lid switch should suspend");
    }
    ...
    fn kernel_zen_config() {
        ...
        let result = generate(&test_hw(), &c, &[]);
        let nix = &result.configuration_nix;
        assert!(nix.contains("boot.kernelPackages = pkgs.linuxPackages_zen"), "Zen kernel should be configured");
    }
    ...
    fn kernel_hardened_config() {
        ...
        let result = generate(&test_hw(), &c, &[]);
        let nix = &result.configuration_nix;
        assert!(nix.contains("boot.kernelPackages = pkgs.linuxPackages_hardened"), "Hardened kernel should be configured");
    }
    ...
    fn kernel_lts_config() {
        ...
        let result = generate(&test_hw(), &c, &[]);
        let nix = &result.configuration_nix;
        assert!(nix.contains("boot.kernelPackages = pkgs.linuxPackages;"), "LTS kernel should be configured");
    }
    ...
    fn swap_zram_config() {
        ...
        let result = generate(&test_hw(), &c, &[]);
        let nix = &result.configuration_nix;
        assert!(nix.contains("zramSwap"), "zram swap should be enabled when swap_gb > 0");
    }
    ...
    fn home_manager_flake() {
        ...
        let result = generate(&test_hw(), &c, &[]);
        let flake = &result.flake_nix;
        assert!(flake.contains("home-manager"), "Flake should include home-manager input");
        assert!(flake.contains("home-manager.nixosModules.home-manager"), "Flake should include home-manager module");
        let nix = &result.configuration_nix;
        assert!(nix.contains("home-manager.useGlobalPkgs = true"), "Config should enable useGlobalPkgs");
        assert!(nix.contains("home-manager.useUserPackages = true"), "Config should enable useUserPackages");
        assert!(nix.contains("home-manager.users.tristan"), "Config should have user home-manager block");
        assert!(nix.contains("programs.starship.enable = true"), "Config should enable starship");
        assert!(nix.contains("oh-my-zsh"), "Zsh shell should get oh-my-zsh in home-manager");
    }
    ...
    fn home_manager_without_shell() {
        ...
        let result = generate(&test_hw(), &c, &[]);
        let nix = &result.configuration_nix;
        assert!(nix.contains("home-manager.users.tristan"), "Home-manager block should exist");
    ...
    fn default_kernel_no_override() {
        ...
        let result = generate(&test_hw(), &c, &[]);
        let nix = &result.configuration_nix;
        assert!(!nix.contains("boot.kernelPackages = pkgs.linux"), "Default kernel should not override kernelPackages");
    }
    ...
    fn symthaea_edition_config() {
        ...
        let result = generate(&test_hw(), &c, &[]);
        let nix = &result.configuration_nix;
        assert!(nix.contains("Symthaea Consciousness Engine"), "Should have Symthaea section comment");
        assert!(nix.contains("services.ollama"), "Symthaea should enable Ollama");
        assert!(nix.contains("host = \"127.0.0.1\""), "Ollama should bind to localhost");
        assert!(nix.contains("port = 11434"), "Ollama should use port 11434");
        assert!(nix.contains("ollama"), "Ollama package should be in systemPackages");
        let flake = &result.flake_nix;
        assert!(flake.contains("Future: Symthaea/Mycelix flake inputs"), "Flake should have future input comment");
        let errors = validate_nix_syntax(nix);
        assert!(errors.is_empty(), "Symthaea config should be valid Nix, got: {:?}", errors);
    }
    ...
    fn mycelix_edition_config() {
        ...
        let result = generate(&test_hw(), &c, &[]);
        let nix = &result.configuration_nix;
        assert!(nix.contains("Mycelix Sovereign Network"), "Should have Mycelix section comment");
        assert!(nix.contains("services.avahi"), "Mycelix should enable Avahi");
        assert!(nix.contains("nssmdns4 = true"), "Avahi should enable mDNS");
        assert!(nix.contains("publish"), "Avahi should enable publishing");
        let flake = &result.flake_nix;
        assert!(flake.contains("Future: Symthaea/Mycelix flake inputs"), "Flake should have future input comment");
        let errors = validate_nix_syntax(nix);
        assert!(errors.is_empty(), "Mycelix config should be valid Nix, got: {:?}", errors);
    }
    ...
    fn auto_login_gnome() {
        ...
        let result = generate(&test_hw(), &c, &[]);
        let nix = &result.configuration_nix;
        assert!(nix.contains("services.displayManager.autoLogin.enable = true"), "Auto-login should be enabled");
        assert!(nix.contains("services.displayManager.autoLogin.user = \"tristan\""), "Auto-login user should match");
        let errors = validate_nix_syntax(nix);
        assert!(errors.is_empty(), "Auto-login config should be valid Nix, got: {:?}", errors);
    }
    ...
    fn auto_login_kde() {
        ...
        let result = generate(&test_hw(), &c, &[]);
        let nix = &result.configuration_nix;
        assert!(nix.contains("services.displayManager.autoLogin.enable = true"), "Auto-login should work for KDE");
    }
    ...
    fn auto_login_not_for_tiling() {
        ...
        let result = generate(&test_hw(), &c, &[]);
        let nix = &result.configuration_nix;
        assert!(!nix.contains("autoLogin.enable = true"), "Tiling WMs should not get auto-login");
        assert!(nix.contains("Auto-login not applicable"), "Should have explanatory comment");
    }
    ...
    fn extra_users_config() {
        ...
        let result = generate(&test_hw(), &c, &[]);
        let nix = &result.configuration_nix;
        assert!(nix.contains("users.users.alice"), "Extra user alice should be present");
        assert!(nix.contains("users.users.bob"), "Extra user bob should be present");
        assert!(nix.contains("users.users.tristan"), "Primary user should still be present");
        // Extra users should not have wheel group
        let alice_section = nix.split("users.users.alice").nth(1).unwrap_or("");
        assert!(!alice_section.split('}').next().unwrap_or("").contains("wheel"), "Extra users should not have wheel");
        let errors = validate_nix_syntax(nix);
        assert!(errors.is_empty(), "Extra users config should be valid Nix, got: {:?}", errors);
    }
    ...
    fn extra_users_dedup_primary() {
        ...
        assert_eq!(count, 1, "Primary user should not be duplicated in extra users");
        assert!(nix.contains("users.users.alice"), "Non-duplicate extra user should be present");
    }
    ...
    fn luks_plus_tpm2_plus_fido2() {
        ...
        c.fido2_unlock = true;
        let result = generate(&test_hw(), &c, &[]);
        assert!(result.configuration_nix.contains("boot.initrd.systemd.enable = true"));
    ...
    fn home_manager_with_fish() {
        ...
        c.shell = "fish".into();
        let result = generate(&test_hw(), &c, &[]);
        assert!(result.configuration_nix.contains("programs.fish.enable = true"));
    }
    ...
    fn all_options_together() {
        ...
        let errors = validate_nix_syntax(&result.configuration_nix);
        assert!(errors.is_empty(), "Kitchen sink config has syntax errors: {:?}", errors);
        let flake_errors = validate_nix_syntax(&result.flake_nix);
        assert!(flake_errors.is_empty(), "Kitchen sink flake has syntax errors: {:?}", flake_errors);
    ...
    fn all_desktop_environments() {
        for de in &["gnome", "kde", "hyprland", "sway", "xfce", "cosmic", "none"] {
            ...
            let result = generate(&test_hw(), &c, &[]);
            let errors = validate_nix_syntax(&result.configuration_nix);
            assert!(errors.is_empty(), "DE '{}' generates invalid Nix: {:?}", de, errors);
        }
    }
    ...
    fn all_kernels() {
        for kernel in &["default", "zen", "lts", "hardened"] {
            ...
            let result = generate(&test_hw(), &c, &[]);
            let errors = validate_nix_syntax(&result.configuration_nix);
            assert!(errors.is_empty(), "Kernel '{}' generates invalid Nix: {:?}", kernel, errors);
        }
    }
    ...
    fn all_gpu_vendors() {
        for gpu in &["nvidia", "amd", "intel", "unknown"] {
            ...
            let result = generate(&hw, &test_choices(), &[]);
            let errors = validate_nix_syntax(&result.configuration_nix);
            assert!(errors.is_empty(), "GPU '{}' generates invalid Nix: {:?}", gpu, errors);
        }
    }
