...
fn test_daemon_ipc_snapshot_roundtrip() {
    let dir = std::env::temp_dir().join("nix-mind-ipc-roundtrip");
...
fn test_daemon_config_roundtrip() {
    let dir = std::env::temp_dir().join("nix-mind-config-roundtrip");
...
fn test_cognitive_to_ipc_pipeline() {
    ...
    let dir = std::env::temp_dir().join("nix-mind-cognitive-ipc");
    let _ = std::fs::create_dir_all(&dir);
    let path = dir.join("daemon_state.json");
