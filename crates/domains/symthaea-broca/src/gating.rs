...
impl EpistemicCubeGate {
    ...
    /// Apply code-specific hallucination suppression based on epistemic signals.
    ///
    /// When certainty is low (E-axis), penalize tokens that look like
    /// imaginary attribute paths or non-existent identifiers.
    pub fn apply_strict_code_gate(
        ...
        if e > 1 {
            // High certainty — trust the model's creative flow
            return;
        }

        // Low certainty — apply strict gating
        // 1. Identify "hallucination-prone" tokens (long Nix attribute paths, etc.)
        // For now, we penalize any token that is a NIX_TOKEN but NOT a "safe" keyword
        ...
        for (id, logit) in logits.iter_mut().enumerate() {
            ...
            if tokenizer.is_nix_token(id as u32) {
                let token_str = tokenizer.token_str(id as u32);
                // Safe keywords (always allowed)
                let is_safe =
                    ["let", "in", "inherit", "import", "with", "rec"].contains(&token_str);

                if !is_safe {
                    // Penalize proportionally to lack of certainty
                    let penalty = match e {
                        0 => 5.0, // Totally unsure -> heavy penalty
                        1 => 2.0, // Slightly unsure -> moderate penalty
                        _ => 0.0,
                    };
                    *logit -= penalty;
                }
