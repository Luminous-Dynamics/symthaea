...
impl BpeTokenizer {
    ...
    pub fn add_hcl_tokens(&mut self) -> usize {
        let mut added = 0;
        for &token in HCL_TOKENS {
            if !self.token_to_id.contains_key(token) {
                self.add_token(token);
                added += 1;
            }
    ...
    pub fn add_compose_tokens(&mut self) -> usize {
        let mut added = 0;
    ...
    /// Build a tokenizer targeted at substrate distillation: ASCII + common
    /// English + substrate-specific tokens, **no CODE_TOKENS**.
    pub fn for_substrate_distillation(substrate: &str) -> Self {
        ...
        match substrate.to_lowercase().as_str() {
            "nix" => t.add_nix_tokens(),
            "hcl" | "terraform" => t.add_hcl_tokens(),
            "compose" | "docker-compose" | "yaml" => t.add_compose_tokens(),
            _ => 0,
        };
        t
    }

    /// Build a tokenizer targeted at Nix-only distillation. (Deprecated: use for_substrate_distillation)
    pub fn for_nix_distillation() -> Self {
        Self::for_substrate_distillation("nix")
    }
