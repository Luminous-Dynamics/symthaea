...
//! manufacturing. It binds exact external process/lifecycle subjects to non-scalar design
//! relations.
//!
//! ```text
//! process subject referenced
//! != process resolved
...
pub enum ManufacturingLifecycleBridgeError {
    ...
    #[error("invalid exact external subject reference: {0}")]
...
fn ref_id(reference: &ExactLifecycleSubjectRefV1) -> Result<String, ManufacturingLifecycleBridgeError> {
    reference
        .ref_id()
...
pub struct ProcessLifecycleRelationV1 {
    ...
    /// Exact evidence, assessment, strategy, or route subject defining the relation.
...
impl ProcessLifecycleRelationV1 {
    ...
    fn identity_key(&self) -> Result<String, ManufacturingLifecycleBridgeError> {
        self.validate()?;
        Ok(format!("{}:{}", self.kind.tag(), ref_id(&self.subject)?))
    }
}
...
pub struct ManufacturingLifecycleBridgeV1 {
    ...
    /// Exact process-definition subject owned by the manufacturing-process layer.
    /// A well-formed reference does not prove the process exists or is admitted.
    pub process_definition: ExactLifecycleSubjectRefV1,
...
impl ManufacturingLifecycleBridgeV1 {
    ...
    pub fn bridge_id(&self) -> Result<String, ManufacturingLifecycleBridgeError> {
        ...
        hash_field(&mut hasher, &ref_id(&self.process_definition)?);
        if let Some(reference) = &self.process_pack_profile {
            hash_field(&mut hasher, &ref_id(reference)?);
        } else {
            hash_field(&mut hasher, "no-process-pack-profile");
        }
        hash_field(&mut hasher, &ref_id(&self.lifecycle_design_profile)?);
        for key in keyed {
...
mod tests {
    ...
    fn relation(
        kind: ProcessLifecycleRelationKindV1,
        id: &str,
        fill: char,
    ...
    fn bridge() -> ManufacturingLifecycleBridgeV1 {
        ManufacturingLifecycleBridgeV1 {
            ...
            process_definition: subject(
                "symthaea.manufacturing.process-definition",
                "cold-spray-repair-v1",
                '9',
            ),
            process_pack_profile: Some(subject("mfg.pack", "advanced:cold-spray", 'a')),
            lifecycle_design_profile: subject("symthaea.lifecycle.design-profile", "profile-1", 'b'),
            relations: vec![
                relation(
                    ProcessLifecycleRelationKindV1::EnablesCandidateRepairRoute,
                    "repair-route",
                    'c',
                ),
                relation(
                    ProcessLifecycleRelationKindV1::RequiresEnergyAssessment,
                    "energy-inventory",
                    'd',
                ),
            ],
    ...
    #[test]
    fn exact_process_subject_change_changes_bridge_identity() {
        let a = bridge();
        let mut b = a.clone();
        b.process_definition.content_blake3 = digest('8');
    ...
    fn exotic_unknown_can_remain_explicitly_unresolved() {
        let value = ManufacturingLifecycleBridgeV1 {
            ...
            process_definition: subject(
                "symthaea.manufacturing.process-definition",
                "containerless-research-v1",
                '0',
            ),
            process_pack_profile: Some(subject("mfg.pack", "research:containerless", '1')),
            lifecycle_design_profile: subject("symthaea.lifecycle.design-profile", "research-profile", '2'),
