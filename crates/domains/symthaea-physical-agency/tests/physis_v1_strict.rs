...
#[test]
fn multiple_simulation_obligations_require_typed_bindings_before_solver_execution() {
    let selected = cryptographic_selected();
    let prepared = prepare_confirmatory_simulation(
        &selected,
        request("physis-v1-multiple-simulation-obligations"),
        claim(),
    )
    .unwrap();
    let mut safety = open_safety_case();
    safety.add_obligation(ProofObligation::new(
        "a second free-form simulation claim must not reuse the confirmatory receipt",
        EvidenceKind::Simulation,
    ));

    assert!(matches!(
        preregister_confirmatory_safety(prepared, &safety),
        Err(ConfirmatoryContractError::MultipleSimulationObligationsRequireTypedBindings(2))
    ));
}
