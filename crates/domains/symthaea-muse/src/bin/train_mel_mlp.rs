...
//!
//! ```sh
//! cargo run --release -p symthaea-muse --bin train_mel_mlp -- \
...
use symthaea_muse::mel_mlp::{DecoderConfig, MelDecoder};
...
fn parse_args() -> Args {
    ...
    if args.len() < 2 {
        eprintln!(
            "Usage: train_mel_mlp <pairs_dir> <ckpt_path> [--epochs N] [--hidden N] [--lr F] [--max-files N]"
        );
        std::process::exit(1);
    }
