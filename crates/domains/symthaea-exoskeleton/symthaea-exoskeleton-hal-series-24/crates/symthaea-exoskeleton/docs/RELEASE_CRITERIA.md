...
## Series 24 closure gates

- Passivity energy-tank evidence passes under high-power and depleted-energy
  cases.
- Redundant sensor disagreement removes authority and is never silently
  averaged away.
- Adaptive assistance remains shadow-only, bounded to derating, and rejects any
  candidate with a safety violation.
- Hardware arming requires a fresh consent/fit/emergency-release session permit.
- Release provenance, signer policy, hardware binding, and anti-rollback checks
  pass before the HAL transport permit is created.
- Long-horizon runtime, thermal, cycle, fault, and energy-throughput thresholds
  produce deterministic derating, maintenance, and shutdown decisions.
