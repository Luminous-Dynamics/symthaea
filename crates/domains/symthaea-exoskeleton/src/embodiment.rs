...
impl ExoskeletonEmbodiment {
    ...
    pub fn step(&mut self, thought_hv: &ContinuousHV, dt: f32, phi: f64) -> EmbodimentResult {
        ...
        if gain == 0.0 {
            cmd.stiffness_gain = 0.0;
            cmd.damping_gain = 0.0;
        }
    ...
    pub fn telemetry(&self) -> EmbodimentTelemetry {
        EmbodimentTelemetry {
            ...
            safety_level: self.current_safety,
            platform: "exoskeleton".to_string(),
            num_actuators: NUM_ACTUATORS,
...
impl symthaea_core::embodiment::EmbodimentBridge for ExoskeletonEmbodiment {
    ...
    fn platform(&self) -> symthaea_core::embodiment::EmbodimentPlatform {
        symthaea_core::embodiment::EmbodimentPlatform::Exoskeleton
    }
...
mod tests {
    ...
    fn test_extended() {
        ...
        for i in 0..500 {
            let phi = 0.3 + 0.4 * ((i as f64 * 0.1).sin());
            assert!(b.step(&hv, 0.005, phi).success);
        }
    }
    #[test]
    fn test_moral_gate_ahimsa_zeros_torques() {
        ...
        assert_eq!(
            ...
            "ahimsa must force backdrivable Red regardless of phi"
        );
        assert!(
