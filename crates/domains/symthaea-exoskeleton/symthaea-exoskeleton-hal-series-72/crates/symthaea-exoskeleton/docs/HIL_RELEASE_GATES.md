...
## Provenance and endurance additions

- Verify the exact release manifest and approved signature chain before arming.
- Demonstrate rollback refusal after persisting a newer security epoch/build.
- Expire the deployment permit during a HIL run and prove electrical disablement.
- Run the intended maximum continuous duty cycle plus margin while retaining
  timing, thermal, energy-throughput, and direction-reversal counters.
- Prove that maintenance-required and shutdown-required states cannot be cleared
  by a process restart alone.
