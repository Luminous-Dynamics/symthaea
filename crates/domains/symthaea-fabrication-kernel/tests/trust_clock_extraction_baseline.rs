...
//! Compatibility baseline for extracting fabrication trust/clock primitives into
//! a reusable trust kernel.
//!
//! These vectors freeze the existing fabrication wire identity and authority
//! semantics before any package move or schema generalization. Passing this test
//! after extraction means compatibility with this baseline; it does not grant
//! engineering, fabrication, deployment, or actuation authority.

use std::collections::BTreeSet;

use symthaea_fabrication_kernel::attestation::{DetachedSignature, SignatureAlgorithm};
use symthaea_fabrication_kernel::clock::{
    CLOCK_OBSERVATION_SCHEMA, ClockEpochTracker, ClockObservation, ClockObservationVerifier,
    ClockQuorumPolicy, ClockTrackingError, digest_clock_observation, verify_clock_quorum,
};
use symthaea_fabrication_kernel::clock_continuity::{
    ClockContinuityPolicy, digest_clock_continuity, verify_clock_continuity,
};
use symthaea_fabrication_kernel::trust::{
    KeyEligibility, KeyLifecycleStatus, KeyTrustRecord, KeyUsage, TrustSnapshot,
    TrustSnapshotTracker, TrustSnapshotTrackingError, canonical_trust_snapshot_bytes,
...
fn snapshot() -> TrustSnapshot {
    // Deliberately supply the keys in non-canonical order. The constructor must
    // canonicalize them before identity is derived.
    TrustSnapshot::new(
        7,
        1_000,
        2_000,
...
#[test]
fn trust_snapshot_wire_identity_is_frozen_before_extraction() {
...
#[test]
fn trust_snapshot_tracker_semantics_are_frozen_before_extraction() {
...
#[test]
fn clock_quorum_and_continuity_vectors_are_frozen_before_extraction() {
    let snapshot = snapshot();
    let policy = ClockQuorumPolicy::default();
    let verifier = AcceptAllSignatures;

    let observations_42 = epoch_42();
    let obs_a_42 = observations_42
        .iter()
        .find(|observation| observation.source_id == "source-a")
        .unwrap();
    let obs_b_42 = observations_42
        .iter()
        .find(|observation| observation.source_id == "source-b")
        .unwrap();
    assert_eq!(digest_clock_observation(obs_a_42).unwrap().to_hex(), OBS_A_E42);
    assert_eq!(digest_clock_observation(obs_b_42).unwrap().to_hex(), OBS_B_E42);
    ...
    // Observation input order must not affect the verified window identity.
    let mut reversed = observations_42.clone();
    reversed.reverse();
    let reordered = verify_clock_quorum(&reversed, &policy, &snapshot, 1_500, &verifier).unwrap();
