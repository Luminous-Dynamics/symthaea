...
//! Verify the secret-free Series 22 hardware, time, transport, and archive bundle.
//!
//! Usage:
//!   cargo run -p symthaea-vocal-tract --example checkpoint_series22_hardware_time_archive_verifier -- \
//!     series22-public-bundle.postcard 1784548800

use std::{env, fs, process};

use symthaea_vocal_tract::{
    CheckpointOperationalTrustMetrics, CheckpointOperationalTrustRequirements,
    MAX_CHECKPOINT_PUBLIC_ARTIFACT_BYTES, apply_series22_public_verifiability,
    assemble_checkpoint_operational_trust_evidence,
    decode_checkpoint_series22_public_verification_bundle,
};

fn run() -> Result<(), String> {
    let mut arguments = env::args().skip(1);
    let bundle_path = arguments.next().ok_or_else(|| {
        "expected Series 22 public bundle path and verification Unix timestamp".to_owned()
    })?;
    ...
    let bundle = decode_checkpoint_series22_public_verification_bundle(&encoded)
        .map_err(|error| format!("bundle decode failed: {error}"))?;
    let summary = bundle
        .verify(verification_time)
        .map_err(|error| format!("Series 22 public verification failed: {error}"))?;

    let mut metrics = CheckpointOperationalTrustMetrics::default();
    apply_series22_public_verifiability(&mut metrics, &summary)
        .map_err(|error| format!("evidence application failed: {error}"))?;
    let report = assemble_checkpoint_operational_trust_evidence(
        metrics,
        CheckpointOperationalTrustRequirements::series_22_delta(),
    );
    ...
    if !report.passed() {
        return Err("Series 22 hardware/time/archive gates did not pass".to_owned());
    }
    println!(
        "verified {} HSM attestations, {} time authorities, {} archives, and {} transport paths",
        summary.hardware_signing().valid_attestations,
        summary.trusted_time().valid_statements,
        summary.gossip_archive().unique_archives,
        summary.gossip_transport().unique_networks,
    );
    Ok(())
}
