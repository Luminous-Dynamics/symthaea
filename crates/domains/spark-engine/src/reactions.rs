...
impl ReactionCalculator {
    ...
    pub fn calculate(reaction: FusionReaction, temperature_kev: f64) -> ReactionResult {
        ...
        let q_estimate = if loss_power > 0.0 {
            fusion_power / loss_power
        } else {
            0.0
        };
    ...
    fn sigma_v_dd(t_kev: f64) -> f64 {
        ...
        let sigma_v = c[0] * theta.sqrt() * (-3.0 * xi).exp() / (mc2 * t_kev.powi(3))
            * (c[1]
                + t_kev * (c[2] + t_kev * (c[3] + t_kev * (c[4] + t_kev * (c[5] + t_kev * c[6])))));
...
impl MuonCatalyzedFusion {
    ...
    pub fn energy_analysis(&self) -> MuonCatalysisAnalysis {
        ...
        let breakeven_fusions =
            (self.muon_production_cost_mev / self.energy_per_fusion_mev).ceil() as u32;

        let required_sticking_reduction = if q_factor < 1.0 { 1.0 / q_factor } else { 1.0 };
        ...
        MuonCatalysisAnalysis {
            ...
            assessment: if q_factor > 1.0 {
                ...
                format!(
                    "Close to breakeven - need {:.0}× more fusions/muon",
                    1.0 / q_factor
                )
            } else {
                format!(
                    "Far from breakeven - need {:.0}× improvement",
                    1.0 / q_factor
                )
            },
        }
...
impl FusionComparison {
    ...
    pub fn build() -> Self {
        ...
        let recommendation = "D-T magnetic confinement (tokamak) remains the most \
            ...
            but face severe physics barriers."
            .to_string();
