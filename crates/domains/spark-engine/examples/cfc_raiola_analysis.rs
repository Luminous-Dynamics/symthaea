...
fn main() {
    ...
    println!(
        "Loaded {} screening measurements from literature\n",
        loader.screening_data.len()
    );

    // Show a sample of the synthetic dynamics
    if let Some(pd_measurement) = loader
        .screening_data
        .iter()
        .find(|m| m.host_material == "Pd")
    {
        ...
        println!(
            "  Enhancement: {:.1}× over adiabatic limit",
            pd_measurement.enhancement_ratio
        );
        println!("\n  Time scales:");
        println!(
            "    τ_electron:  {:.2e} s (electronic response)",
            dynamics.time_scales.tau_electron
        );
        println!(
            "    τ_phonon:    {:.2e} s (lattice response)",
            dynamics.time_scales.tau_phonon
        );
        println!(
            "    τ_screening: {:.2e} s (screening buildup)",
            dynamics.time_scales.tau_screening
        );
        println!(
            "    τ_fusion:    {:.2e} s (nuclear time scale)",
            dynamics.time_scales.tau_fusion
        );
    ...
    println!(
        "Running CfC analysis on all {} materials...\n",
        loader.screening_data.len()
    );
    ...
    sorted_results.sort_by(|a, b| {
        b.enhancement_ratio
            .partial_cmp(&a.enhancement_ratio)
            .unwrap()
    });
    ...
    for r in sorted_results.iter().take(3) {
        println!(
            ...
            r.material,
            r.enhancement_ratio,
            r.dominant_time_scale,
            r.temporal_features.phonon_correlation
        );
    }
    ...
    println!(
        "   Correlation r = {:.2} suggests temporal dynamics matter.",
    ...
    println!("═══════════════════════════════════════════════════════════════════════");
    println!(
        "  Analysis complete. CfC learned {} temporal features per material.",
        report
            .material_results
            .first()
            .map(|r| r.learned_tau.len())
            .unwrap_or(0)
    );
    println!("═══════════════════════════════════════════════════════════════════════");
}
