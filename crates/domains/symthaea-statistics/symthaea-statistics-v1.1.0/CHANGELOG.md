...
- Added multivariate energy distance and seeded equality-in-distribution tests.
- Added distance covariance/correlation and seeded nonlinear-dependence tests.
- Added Gaussian-kernel MMD with explicit biased/unbiased conventions and pooled median bandwidths.
- Added one-factor Euclidean PERMANOVA with deterministic label permutations.
- Added tie-corrected Kruskal-Wallis inference and Holm-adjusted Dunn comparisons.
- Added fixed- and generalized-DerSimonian-Laird random-effects meta-regression with moderator prediction.
- Added oracle-approximating covariance shrinkage and stable shrunk Mahalanobis distances.
- Added bounded-memory P² online quantiles for long non-mergeable streams.
- Added normal-inverse-gamma conjugate inference for unknown Gaussian mean and variance.
- Added multiclass Brier/log scores, top-k accuracy, ranked probability scores, and top-label calibration.
- Added Sidak, Hochberg, Benjamini-Yekutieli, and Storey q-value corrections.
- Added independent NumPy/SciPy/scikit-learn/statsmodels references, adversarial contracts, and explicit quadratic-work bounds.
