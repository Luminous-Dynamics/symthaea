...
- Energy distance uses the finite-sample V-statistic. Distance covariance and correlation use doubly centered Euclidean distance matrices and the biased finite-sample convention. Permutation p-values assume exchangeable pairings/labels under the null.
- Gaussian-kernel MMD reports squared discrepancy. The unbiased estimator can be negative in finite samples; the biased estimator is clamped only against roundoff below zero. The median-distance bandwidth heuristic is explicit and excludes exact duplicate distances.
- PERMANOVA uses squared Euclidean distances and a one-factor pseudo-F decomposition. It does not implement strata, repeated-measures restrictions, or non-Euclidean corrections.
- Kruskal-Wallis and Dunn statistics use average ranks and a shared tie correction. Dunn p-values use the asymptotic normal reference and Holm adjustment.
- Random-effects meta-regression uses a generalized DerSimonian-Laird moment estimator and normal/Wald coefficient intervals. Moderator selection, ecological bias, and uncertainty in estimated within-study standard errors remain outside the model.
- OAS covariance uses the maximum-likelihood empirical covariance and a spherical target. It regularizes inversion but does not repair nonrepresentative observations or nonlinear geometry.
- P² online quantiles retain five markers per probability, are order-sensitive in finite streams, and are not mergeable. They are approximation tools rather than substitutes for retained empirical distributions when exact quantiles are required.
- Normal-inverse-gamma inference assumes conditionally Gaussian observations and conjugate prior structure. Posterior mean-variance expectations require shape greater than one.
- Multiclass top-label ECE/MCE depend on equal-width bins and can hide class-conditional miscalibration. Proper scoring rules and class-specific inspection should accompany them.
- Sidak/Hochberg require independence or suitable positive dependence for their advertised family-wise guarantees. Benjamini-Yekutieli is valid under arbitrary dependence but conservative. Storey q-values condition on a caller-declared lambda and its estimated null fraction.
- Pairwise distance and kernel matrices are bounded at 25,000,000 elements; larger requests fail with `ComputationTooLarge`.
