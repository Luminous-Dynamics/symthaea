...
//! Final CPU visual-fidelity pass for Spore Boot Ecology.
//!
//! This wrapper adds deterministic membrane detail, bounded caustic arcs, and a
//! reusable quarter-resolution bloom workspace on top of the holographic ecology.
//! It intentionally introduces no GPU, window-system, font, network, or cognitive
//! runtime dependency into early boot.
...
const BLOOM_SCALE: usize = 4;
const BLOOM_THRESHOLD: f32 = 118.0;
const SHELL_NODES: usize = 16;
...
impl FidelityField {
    fn new(width: u32, height: u32, genome: &BootGenome) -> Self {
        ...
        let mut cell_offsets = [0.0; SHELL_NODES];
        for (index, value) in cell_offsets.iter_mut().enumerate() {
            *value = genome.seed[(index + 8) % genome.seed.len()] as f32 / 255.0;
        }
    ...
    fn draw_membrane(&self, buffer: &mut [u32], state: EcologyFrameState, gain: f32) {
        ...
        draw_ellipse(
            ...
            radius * 0.86,
            time * 0.035,
            primary.with_opacity((0.15 * gain).clamp(0.0, 0.18)),
        );
        draw_ellipse(
            buffer,
            self.width,
        ...
        let mut nodes = [Point { x: 0.0, y: 0.0 }; SHELL_NODES];
        for (index, node) in nodes.iter_mut().enumerate() {
            let base = std::f32::consts::TAU * index as f32 / SHELL_NODES as f32;
            let angle = base + (self.cell_offsets[index] - 0.5) * 0.24 + time * 0.012;
            let radial = radius
                * (0.72 + self.cell_offsets[(index + 5) % SHELL_NODES] * 0.20);
            *node = Point {
        ...
        let mesh = primary.with_opacity((0.075 * gain).clamp(0.0, 0.10));
        for index in 0..SHELL_NODES {
            draw_line_alpha(
                buffer,
                self.width,
                self.height,
                nodes[index],
                nodes[(index + 1) % SHELL_NODES],
                mesh,
            );
            if index % 2 == 0 {
                draw_line_alpha(
                    ...
                    self.height,
                    nodes[index],
                    nodes[(index + 5) % SHELL_NODES],
                    mesh.with_opacity((0.045 * gain).clamp(0.0, 0.07)),
                );
            }
        }
    ...
    fn draw_caustics(&self, buffer: &mut [u32], state: EcologyFrameState, gain: f32) {
        ...
        for index in 0..3 {
            ...
            draw_arc(
                ...
                base.with_opacity((base_alpha * gain).clamp(0.0, 0.065)),
            );
        }
    }
}
...
impl BloomWorkspace {
    fn new(width: usize, height: usize) -> Self {
        let low_width = width.div_ceil(BLOOM_SCALE).max(1);
        let low_height = height.div_ceil(BLOOM_SCALE).max(1);
    ...
    fn extract(&mut self, buffer: &[u32], width: usize, height: usize) {
        ...
        for low_y in 0..self.low_height {
            for low_x in 0..self.low_width {
                let start_x = low_x * BLOOM_SCALE;
                let start_y = low_y * BLOOM_SCALE;
                let end_x = (start_x + BLOOM_SCALE).min(width);
                let end_y = (start_y + BLOOM_SCALE).min(height);
                    ...
                    for x in start_x..end_x {
                        ...
                        if peak > BLOOM_THRESHOLD {
                            let knee = ((peak - BLOOM_THRESHOLD) / (255.0 - BLOOM_THRESHOLD))
                                .clamp(0.0, 1.0);
                            for channel in 0..3 {
                                accum[channel] += rgb[channel] * knee;
                            }
                        }
                        samples += 1;
                    }
    ...
    fn composite(&self, buffer: &mut [u32], width: usize, height: usize, strength: f32) {
        for y in 0..height {
            let low_y = (y / BLOOM_SCALE).min(self.low_height - 1);
            for x in 0..width {
                let low_x = (x / BLOOM_SCALE).min(self.low_width - 1);
...
fn screen_add(dst: f32, glow: f32) -> u8 {
    ...
    (255.0 - (255.0 - dst) * (1.0 - glow / 255.0))
        .round()
...
mod tests {
    ...
    #[test]
    fn bloom_brightens_without_darkening() {
    ...
    #[test]
    fn bloom_workspace_is_quarter_resolution_per_axis() {
        let workspace = BloomWorkspace::new(1920, 1080);
        assert_eq!(workspace.low_width, 480);
        assert_eq!(workspace.low_height, 270);
    }

    #[test]
    fn fidelity_changes_holographic_frame() {
        let genome = genome();
