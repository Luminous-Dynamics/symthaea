...
impl IbmiFoundationV1 {
    pub fn validate(&self, pack: &LegacyComputingPackV1) -> Result<(), LegacyIbmiErrorV1> {
        if self.schema_version != LEGACY_IBMI_FOUNDATION_SCHEMA_V1 {
            return Err(LegacyIbmiErrorV1::UnsupportedSchema(self.schema_version.clone()));
        }
        ...
        for mechanism in &self.mechanisms {
                ...
                || mechanism.failure_modes.is_empty()
            {
                return Err(LegacyIbmiErrorV1::InvalidField(format!(
                    "IBM i mechanism {:?} requires source claims, evidence and failure modes",
                    mechanism.kind
                )));
            }
...
fn register_ibmi_sources(
    ...
    let definitions = [
        ("ibm:ibmi-object-authority", "ibm:ibmi-object-authority@7.6", "IBM i object and library authority", "IBM i 7.6 object authority", "https://www.ibm.com/docs/en/i/7.6.0?topic=e-edit-object-authority"),
        ("ibm:ibmi-work-management", "ibm:ibmi-work-management@7.6", "IBM i work management and job queues", "IBM i 7.6 work management", "https://www.ibm.com/support/pages/active-job-queue-analysis-qsys2jobqueueinfo"),
        ("ibm:ibmi-db2", "ibm:ibmi-db2@7.6", "Db2 for IBM i", "Db2 for IBM i 7.6", "https://www.ibm.com/support/pages/db2-ibm-i"),
        ("ibm:ibmi-db2-distributed", "ibm:ibmi-db2-distributed@7.6", "Db2 for i distributed relational database support", "IBM i 7.6 Db2 distributed relational database support", "https://www.ibm.com/docs/en/i/7.6.0?topic=sql-db2-i-distributed-relational-database-support"),
        ("ibm:ibmi-authority-collection", "ibm:ibmi-authority-collection@7.6", "IBM i authority collection", "IBM i 7.6 authority collection", "https://www.ibm.com/docs/en/i/7.6.0?topic=collection-authority-interfaces"),
        ("ibm:ibmi-save-restore", "ibm:ibmi-save-restore@7.6", "IBM i save and restore", "IBM i 7.6 save and restore", "https://www.ibm.com/support/pages/node/7181664"),
        ("ibm:ibmi-tcpip-connectivity", "ibm:ibmi-tcpip-connectivity@7.6", "IBM TCP/IP Connectivity for i", "IBM i 7.6 TCP/IP connectivity", "https://www.ibm.com/docs/en/i/7.6.0?topic=c-software-requirements-2"),
        ("ibm:ibmi-cl-overview", "ibm:ibmi-cl-overview@7.6", "IBM i Control Language", "IBM i 7.6 Control Language", "https://www.ibm.com/docs/en/i/7.6.0"),
    ];
...
fn register_ibmi_claims(pack: &mut LegacyComputingPackV1) -> Result<(), LegacyIbmiErrorV1> {
    let claims = [
        claim("legacy:ibmi:object-library-namespace", "ibm:ibmi-object-authority@7.6", "IBM i resources are typed system objects addressed through libraries and library-list resolution; object authority views expose object name, containing library, owner, type, primary group, authorized users, and authorization-list association.", "Edit Object Authority", SupportCategory::Software),
        claim("legacy:ibmi:subsystem-job-queue", "ibm:ibmi-work-management@7.6", "IBM i batch work is mediated by job queues and subsystem descriptions; queue/subsystem limits can explain waiting work without implying application failure.", "Active job queue analysis", SupportCategory::Software),
        claim("legacy:ibmi:db2-integrated", "ibm:ibmi-db2@7.6", "Db2 for i is integrated with IBM i and Power Systems rather than managed as a separately installed database engine; IBM i storage and operating-system facilities are part of the database operating model.", "Db2 for IBM i", SupportCategory::Software),
        claim("legacy:ibmi:db2-distributed-connectivity", "ibm:ibmi-db2-distributed@7.6", "Db2 for i supports distributed relational database connections and package operations, so local database health and remote relational-database connectivity are distinct diagnostic dimensions.", "Db2 for i distributed relational database support", SupportCategory::Network),
        claim("legacy:ibmi:object-authority-model", "ibm:ibmi-object-authority@7.6", "IBM i object access is governed by object-specific authorities, ownership, profiles/groups, authorization lists and special authorities; broad special authority is not equivalent to least-privilege diagnosis.", "Edit Object Authority", SupportCategory::Security),
        claim("legacy:ibmi:authority-collection-observation", "ibm:ibmi-authority-collection@7.6", "IBM i authority collection can record authority-check information for users and objects and can be inspected through commands, APIs, and QSYS2 views without granting broader object authority.", "Authority collection interfaces", SupportCategory::Security),
        claim("legacy:ibmi:save-restore-semantics", "ibm:ibmi-save-restore@7.6", "IBM i backup and recovery behavior depends on object type, save scope, media/save-file state, access paths and object-specific save semantics; successful save completion does not by itself prove a complete restore path.", "IBM i 7.6 backup and restore enhancements", SupportCategory::Software),
        claim("legacy:ibmi:tcpip-connectivity", "ibm:ibmi-tcpip-connectivity@7.6", "IBM TCP/IP Connectivity for i is a platform component used by networked IBM i services; interface, route, service-job and application health must be distinguished during network diagnosis.", "Software requirements", SupportCategory::Network),
        claim("legacy:ibmi:cl-control-surface", "ibm:ibmi-cl-overview@7.6", "IBM i Control Language is a primary system administration and programming control surface; command availability or syntax does not establish authority, safety, or applicability to the active object/job context.", "Control Language", SupportCategory::Software),
    ];
    for claim in claims {
...
fn seed_mechanisms() -> Vec<IbmiMechanismModelV1> {
    ...
    vec![
        mechanism(M::ObjectLibraryNamespace, LegacyKnowledgeAreaV1::SoftwareLifecycle, "Object and library namespace", "Typed objects live in libraries and job/thread library-list context affects unqualified resolution.", &[], &["legacy:ibmi:object-library-namespace"], &[
        ...
        mechanism(M::JobSubsystemWorkManagement, LegacyKnowledgeAreaV1::WorkloadAndJobs, "Jobs, job queues, and subsystems", "Submitted work can wait on job queues until a subsystem selects it; queue/subsystem state and limits are separate from application execution state.", &[], &["legacy:ibmi:subsystem-job-queue"], &[
            ...
            ("queue-not-selected", "A valid submitted job remains queued because the queue/subsystem is held, inactive or constrained.", &["job remains queued without application execution evidence", "queue/subsystem state explains non-selection"], &["restarting the application before confirming the job ever became active"]),
            ("subsystem-capacity", "Work waits because active-job or priority constraints are reached.", &["active jobs are at configured limits", "queued work becomes eligible as capacity changes"], &["raising limits globally without workload-impact review"]),
        ]),
        mechanism(M::Db2ForI, LegacyKnowledgeAreaV1::Storage, "Db2 for i", "Db2 for i is integrated into IBM i, while local database state, locks/journaling and distributed relational connectivity remain distinct diagnostic layers.", &[M::ObjectLibraryNamespace, M::JobSubsystemWorkManagement], &["legacy:ibmi:db2-integrated", "legacy:ibmi:db2-distributed-connectivity"], &[
        ...
        mechanism(M::ObjectAuthority, LegacyKnowledgeAreaV1::IdentityAndSecurity, "Object authority and special authorities", "Object access depends on object authorities, ownership, profiles/groups, authorization lists and special authorities; authority collection can provide evidence without broadening privilege.", &[M::ObjectLibraryNamespace], &["legacy:ibmi:object-authority-model", "legacy:ibmi:authority-collection-observation"], &[
        ...
        mechanism(M::SaveRestore, LegacyKnowledgeAreaV1::AvailabilityAndRecovery, "Save and restore", "Recoverability depends on what objects and dependencies were actually saved, the target/IASP context and restore semantics—not just a successful save completion message.", &[M::ObjectLibraryNamespace, M::Db2ForI], &["legacy:ibmi:save-restore-semantics"], &[
        ...
        mechanism(M::TcpIpNetworking, LegacyKnowledgeAreaV1::Networking, "TCP/IP networking and service jobs", "Interface/route reachability, TCP transport and IBM i server/application job health are separate diagnostic layers.", &[M::JobSubsystemWorkManagement], &["legacy:ibmi:tcpip-connectivity"], &[
        ...
        mechanism(M::ControlLanguage, LegacyKnowledgeAreaV1::SystemLifecycle, "Control Language", "CL is a principal IBM i control/programming surface, but a known command is only a syntax/capability fact—not authorization or safe-change approval.", &[M::ObjectLibraryNamespace, M::ObjectAuthority], &["legacy:ibmi:cl-control-surface"], &[
...
impl fmt::Display for LegacyIbmiErrorV1 {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::ProcedureIdentityConflict(id) => write!(f, "IBM i procedure identity conflict for {id}"),
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn coverage_promotion_is_monotonic_and_real_base_tags_are_retired() {
        let mut pack = seed_legacy_computing_pack_v1(1_800_000_000_000).unwrap();
        assert_eq!(
            pack.profile(LegacyPlatformV1::IbmI)
                .unwrap()
        ...
        assert!(!profile.gap_tags.contains("object-library-model"));
        assert!(!profile.gap_tags.contains("db2-for-i"));
        assert!(profile
            .gap_tags
    ...
    fn every_mechanism_is_source_bound_and_discriminative() {
        let mut pack = seed_legacy_computing_pack_v1(1_800_000_000_000).unwrap();
        let foundation = enrich_legacy_ibmi_foundation_v1(&mut pack, 1_800_000_000_100).unwrap();
    ...
    fn db2_local_state_and_distributed_connectivity_stay_distinct() {
        ...
        let db2 = foundation
            .mechanisms
            .iter()
            .find(|mechanism| mechanism.kind == IbmiMechanismKindV1::Db2ForI)
            .unwrap();
        assert!(db2.failure_modes.iter().any(|failure| failure.id == "local-object-not-network"));
        assert!(db2.failure_modes.iter().any(|failure| failure.id == "distributed-connectivity"));
    }
}
