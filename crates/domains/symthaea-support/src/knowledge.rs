...
use crate::knowledge_source::{
    KnowledgeOriginV1, KnowledgeProviderIdV1, KnowledgeQueryPurposeV1, KnowledgeRecordIdV1,
    KnowledgeShareabilityV1, KnowledgeSourceErrorV1, SupportKnowledgeHitV1,
    SupportKnowledgeQueryV1, SupportKnowledgeSourceV1,
};
use crate::types::*;

const LOCAL_SUPPORT_PROVIDER_ID: &str = "symthaea-support/local-articles";
...
impl SupportKnowledgeSourceV1 for KnowledgeManager {
    fn search_support_knowledge(
        ...
        query.validate()?;
        let provider_id = KnowledgeProviderIdV1::try_new(LOCAL_SUPPORT_PROVIDER_ID)?;
        ...
        for article in &self.articles {
            ...
            hits.push(SupportKnowledgeHitV1 {
                ...
                source_id: KnowledgeRecordIdV1::try_new(format!(
                    "support-article:{}",
                    article.id
                ))?,
...
mod tests {
    ...
    fn graduation_medium_quality_defers() {
        let km = KnowledgeManager::new();
        assert!(matches!(
            km.evaluate_for_graduation(0.4, 3, 1),
    ...
    fn legacy_search_returns_results_sorted_by_phi() {
        ...
        km.add_article(
            "1".into(),
            "DNS Troubleshooting".into(),
            SupportCategory::Network,
            0.3,
        );
        km.add_article(
            "2".into(),
            "DNS Configuration".into(),
            SupportCategory::Network,
            0.8,
        );
        km.add_article(
    ...
    fn promotion_review_excludes_local_only_but_local_reasoning_can_use_it() {
        ...
        assert!(
            local
                .iter()
                .all(|h| h.provider_id.as_str() == LOCAL_SUPPORT_PROVIDER_ID)
        );
        assert!(
            local
                .iter()
