...
//! transition evidence object != verifier admission authority
...
/// Opaque capability required to apply a state transition.
///
/// There is intentionally no public constructor. A later verifier/evaluator
/// module may gain crate-internal issuance authority after its receipts and
/// policy rules are qualified. Merely constructing `FindingTransitionEvidence`
/// is therefore insufficient to mutate a finding state.
#[derive(Debug)]
pub struct TransitionAdmissionToken {
    _sealed: (),
}

/// Opaque capability required to create a pre-specified finding at genesis.
///
/// This prevents callers from laundering exploratory work into a pre-specified
/// status by supplying an arbitrary preregistration digest.
#[derive(Debug)]
pub struct PreRegistrationAdmissionToken {
    _sealed: (),
}
...
///
/// Fields are private by design. External consumers may create only the
/// low-authority `Recorded + Exploratory + Unassessed` state directly. Higher
/// states require an opaque verifier admission capability.
#[derive(Debug, Clone, PartialEq, Eq)]
...
impl FindingState {
    ...
    /// analysis. The opaque token prevents ordinary callers from self-asserting
    /// preregistration authority.
    pub fn recorded_pre_specified(
        _admission: &PreRegistrationAdmissionToken,
        finding_receipt_digest: Digest32,
    ...
    /// Apply an already-verified transition.
    ///
    /// The opaque admission token is intentionally impossible for external
    /// callers to construct in this tranche. Transition verification/admission
    /// will be exposed only after the evaluator layer is separately qualified.
    pub fn apply_verified_transition(
        &self,
        _admission: &TransitionAdmissionToken,
        transition: &FindingTransitionEvidence,
        next_state_digest: Digest32,
    ) -> Result<Self, TransitionError> {
        self.apply_admitted_transition(transition, next_state_digest)
    }

    fn apply_admitted_transition(
        &self,
        transition: &FindingTransitionEvidence,
        next_state_digest: Digest32,
    ) -> Result<Self, TransitionError> {
...
/// Evidence-shaped description of a requested/evaluated state transition.
///
/// This type is intentionally constructible and therefore **not** itself an
/// authority capability. Only `TransitionAdmissionToken` grants mutation access.
#[derive(Debug, Clone, PartialEq, Eq)]
...
mod tests {
    ...
    fn admission() -> TransitionAdmissionToken {
        TransitionAdmissionToken { _sealed: () }
    }

    fn preregistration_admission() -> PreRegistrationAdmissionToken {
        PreRegistrationAdmissionToken { _sealed: () }
    }
    ...
    #[test]
    fn public_verified_api_requires_opaque_admission_capability() {
        let initial = state();
        let transition = event(
            initial.state_digest(),
            TransitionAxis::Authority,
        ...
        assert_eq!(
            initial
                .apply_verified_transition(&token, &transition, d(3))
                .unwrap()
                .authority(),
            AnalyticAuthority::Normalized
        );
    }
    ...
    #[test]
    fn pre_specified_mode_requires_genesis_receipt_and_admission() {
        let token = preregistration_admission();
        let initial = FindingState::recorded_pre_specified(&token, d(1), d(7), d(2), 100);
        assert_eq!(
