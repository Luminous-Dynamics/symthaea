...
fn context_and_outcome_for(sub_index: u64, schedule: StreamSchedule) -> (ContextLabel, f64) {
    let is_a = sub_index.is_multiple_of(2);
    match schedule {
...
pub fn generate_bound_stream(total: u64, schedule: StreamSchedule) -> Vec<Tuple> {
    ...
    for i in 0..total {
        if i % 2 == 0 {
            ...
            let label = if control_index.is_multiple_of(2) {
                ContextLabel::A
            } else {
                ContextLabel::B
            };
            let action = if control_index.is_multiple_of(2) {
                ACTION_FORAGE
            } else {
                ACTION_REST
            };
...
pub fn generate_shuffled_stream(total: u64, rng_state: &mut u64) -> Vec<Tuple> {
    ...
    for i in 0..total {
        if i % 2 == 0 {
            ...
            let label = if xorshift(rng_state).is_multiple_of(2) {
                ContextLabel::A
            } else {
                ContextLabel::B
            };
