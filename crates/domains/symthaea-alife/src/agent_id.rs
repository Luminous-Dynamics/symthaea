...
impl AgentId {
    ...
    /// irrelevant). Never emitted by [`AgentIdAllocator::next`], so it can never collide with a
    /// real allocated id.
...
/// Monotonic allocator — every call to [`AgentIdAllocator::next`] returns a fresh id, never
...
mod tests {
    ...
    fn unallocated_sentinel_never_collides_with_a_real_allocation() {
        let mut alloc = AgentIdAllocator::new();
        for _ in 0..10_000 {
            assert_ne!(alloc.next(), AgentId::UNALLOCATED);
        }
    }
}
