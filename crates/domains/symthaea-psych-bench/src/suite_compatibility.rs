    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::MissingObservedMember(id) => {
                write!(f, "manifest included member is absent from observed suite: {id}")
            }
            Self::UnexpectedObservedMember(id) => {
                write!(f, "observed suite contains undeclared member: {id}")
            }
            Self::ExcludedMemberObserved(id) => {
                write!(f, "observed suite contains explicitly excluded member: {id}")
            }
            Self::ManifestDigestMismatch => {
                write!(f, "compatibility receipt manifest digest does not match manifest")
            }
            ...
            Self::CodeSubjectMismatch => {
                write!(f, "compatibility receipt code subject does not match code binding")
            }
...
impl BenchmarkSuiteCompatibilityReceipt {
    ...
    pub fn bind(
        ...
        if manifest.ordering != SuiteOrderingSemantics::OrderIndependent {
            return Err(BenchmarkSuiteCompatibilityError::
                OrderedManifestRequiresSequenceReceipt);
        }
    ...
    pub fn validate_against(
        ...
        if self.included_member_count != expected_count {
            return Err(BenchmarkSuiteCompatibilityError::IncludedMemberCountMismatch {
...
mod tests {
    ...
    use crate::suite_manifest::{
        BenchmarkSuiteMember, ClaimClassBinding, BENCHMARK_SUITE_MANIFEST_SCHEMA_VERSION,
    };
    ...
    fn exact_order_independent_population_binds_all_three_identities() {
        ...
        let receipt = BenchmarkSuiteCompatibilityReceipt::bind(
            &manifest,
            &observed,
            &binding,
        )
    ...
    fn ordered_manifest_is_refused_without_sequence_authority() {
        ...
        assert_eq!(
            ...
            Err(BenchmarkSuiteCompatibilityError::
                OrderedManifestRequiresSequenceReceipt)
        );
    }
    ...
    fn serialized_receipt_revalidates_and_every_identity_is_bound() {
        ...
        let observed = observed(&["A", "B"]);
        let binding = binding(&manifest);
        let original = BenchmarkSuiteCompatibilityReceipt::bind(
            &manifest,
            &observed,
            &binding,
        )
