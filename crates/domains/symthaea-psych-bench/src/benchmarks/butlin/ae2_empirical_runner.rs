...
/// Evaluate one named `Expectation` against `counters` via
/// `symthaea_evidence_plane::check_integrity` — the shared evidence-plane
/// integrity check, used here as a single-name convenience wrapper so each
/// hook-fired-fraction gate below reads as a declared expectation rather
/// than an inline float comparison. Behavior-preserving: `MustExceed(0.9)`/
/// `MustBeBelow(0.1)` use the same strict `>`/`<` semantics this module's
/// original inline checks did.
fn expectation_holds(counters: &EvidenceCounters, name: &str, expectation: Expectation) -> bool {
    let mut declared = HashMap::new();
    declared.insert(name.to_string(), expectation);
    check_integrity(&declared, counters).is_ok()
}
...
pub fn run_ae2_empirical() -> Ae2EmpiricalRun {
    ...
    // The three hook-fired-fraction gates above, evaluated via the shared
    // `symthaea-evidence-plane` contract (`EvidenceCounters` + `Expectation`
    // + `check_integrity`) instead of inline float comparisons. Thresholds
    // and strict `>`/`<` semantics are unchanged from the original checks.
    let mut hook_counters = EvidenceCounters::new();
    hook_counters.record("baseline_hook_fired_fraction", baseline_hook_fired_fraction);
    hook_counters.record("target_hook_fired_fraction", target_hook_fired_fraction);
    hook_counters.record("sham_lever_fired_fraction", sham_lever_fired_fraction);

    let intervention_applied = expectation_holds(
        &hook_counters,
    ...
    let sham_lever_fired = expectation_holds(
        &hook_counters,
