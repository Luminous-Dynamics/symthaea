...
// `pub(crate)`, not private: `qualification_design.rs`'s feature-gated drift
// test (`qualification_design_matches_ablation_specs`) reads this to confirm
// the static campaign design hasn't silently gone stale against real
// changes here.
pub(crate) fn ablation_specs() -> Vec<AblationSpec> {
    vec![
        AblationSpec {
