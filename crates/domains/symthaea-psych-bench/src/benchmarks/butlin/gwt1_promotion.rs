...
pub struct Gwt1DirectPromotionDecisionV1 {
    pub policy: String,
    /// What the supplied envelope resolution reported before this promotion
    /// layer performed its own consistency check.
    pub reported_qualification_outcome: Gwt1QualificationOutcomeV1,
    /// Fail-closed qualification outcome actually used for promotion.
...
/// produce `CausallySupported` or `FunctionallySupported`. Any malformed or
/// internally inconsistent resolution is demoted to `Inconclusive` before the
/// evidence-tier mapping occurs.
...
mod tests {
    ...
    fn artifact_failure_cannot_be_wrapped_in_qualified() {
        ...
        assert_eq!(
            decision.input_status,
            Gwt1PromotionInputStatusV1::ArtifactIntegrityFailure
        );
        assert_eq!(
            decision.qualification_outcome,
            Gwt1QualificationOutcomeV1::Inconclusive
        );
    ...
    fn outer_inner_outcome_mismatch_fails_closed() {
        ...
        let decision = promote_direct_gwt1_v1(&malformed);
        assert_eq!(decision.input_status, Gwt1PromotionInputStatusV1::OutcomeMismatch);
        assert_eq!(decision.evidence_outcome, EvidenceOutcome::Inconclusive);
    }
