...
//! `UalRuntimeQualification::run_qual::manipulation_applied`, which fails the whole
...
use super::report::{
    ...
    UalRuntimeQualification, UalStaticQualification, combine_schedule_reports,
};
    ...
    fn schedule_report(
        ...
        let qualification = UalRuntimeQualification {
            static_qual: UalStaticQualification {
                // Sham invariant (no motor/timing side channel to perturb)
                // is a structural guarantee pinned by
                // `sham_unrelated_noise_does_not_change_outcome` — a design
                // fact, not a per-run measurement, per the second
                // claim-integrity pass's static/runtime split.
                control_design_valid: true,
                // Structural: `softmax_choice`'s signature has no
                // ground-truth parameter to leak, pinned by
                // `leakage_choice_fn_has_no_ground_truth_access`.
                ...
                // P1's scalar delta-rule mechanism has no open construct-
                // validity question (unlike P2/P4a's HDC retrieval).
                construct_validity_passed: true,
            },
            run_qual: UalRunQualification {
                manipulation_applied,
                // Honest scope: this is an internal-responsiveness
                // observation this run (did the learned value move at all),
                // not an independently-verified positive control in the
                // classical sense — see the second claim-integrity pass.
                positive_control_observed: any_value_crossed,
                // No genuine negative-control manipulation exists in this
                // probe's own trial function to re-derive per run (see
                // `sham_unrelated_noise_does_not_change_outcome`'s doc
                // comment: the sham lever has literally no parameter to
                // wire in) — this reflects that structural fact, not a
                // fresh per-run measurement.
                negative_control_observed: true,
                signal_usable,
                schedule_executed_as_declared: manipulation_applied,
            },
        };
