...
//!
//! 1b. **Second claim-integrity pass (2026-07-27, same day)**: a follow-up
//!    review correctly pointed out that "B retrieves A over C" is a
//!    relational-*identity* finding, not by itself second-order
//!    *conditioning* — ranking A above C shows the memory contains a B→A
//!    relation, not that B acquired functional value or changed behavior.
//!    `behavioral_expression` (the field `Demonstrated` is gated on) now
//!    requires the **conjunction** of two independently-gated sub-results:
//!    (a) relational-identity retrieval above chance, AND (b) a genuine
//!    choice-mechanism test — `b_inferred_value` (A's value if retrieval
//!    favored A, else C's) fed through the same `softmax_choice` primitive
//!    P1 uses, forced against neutral D — showing the transferred value
//!    actually changes a decision, not just a similarity number. Only when
//!    both hold does this probe's result support "second-order value
//!    conditioning"; retrieval alone is reported as such in the notes, not
//!    silently promoted.
//!
...
use super::common::{generate_near_chance_hv, next_seed, softmax_choice};
use super::report::{
...
struct P2TrialOutcome {
    ...
    memory_norm: f32,
    /// B's inferred value: A's value if retrieval favored A over C, else
    /// C's value. This is a genuine scalar estimate (not just a similarity
    /// margin), fed into an actual choice mechanism below — the second
    /// claim-integrity pass's fix for "retrieval alone is not conditioning".
    b_inferred_value: f64,
    /// Result of a forced choice between B (valued at `b_inferred_value`)
    /// and neutral D (fixed at 0.5), via the same `softmax_choice` primitive
    /// P1 uses. `true` = chose B. This is the genuine behavioral/action
    /// readout: does the transferred value actually change a decision, not
    /// just a similarity number.
    chose_b_over_d: bool,
}
...
/// Phase 6 (P2 re-evaluation, `SYMTHAEA_UAL_P2_PHASE6_RESULT_2026-07-28.md`):
/// practical-significance floor for the choice-rate (value-transfer)
/// criterion. Not an arbitrary number — reuses this file's own existing
/// positive-control bar for the structurally analogous reward-probability
/// value (`positive_control_observed = mean_a_value > 0.6` below), rather
/// than importing a different probe's margin. A CI-lower-bound-excludes-0.5
/// criterion alone is not sufficient (a large enough n can make a trivially
/// small effect "significant" without it being practically meaningful) —
/// this floor requires a real, meaningful margin above chance.
const VALUE_TRANSFER_PRACTICAL_FLOOR: f64 = 0.60;

/// Phase 6 preregistered confirmatory sample size, derived BEFORE running
/// anything at this scale and never adjusted afterward. One-sided power
/// analysis (alpha=0.05, power=80%) for detecting whether the true
/// choice-rate clears `VALUE_TRANSFER_PRACTICAL_FLOOR`, using the existing
/// frozen n=40 pilot estimate (p1=0.625,
/// `SYMTHAEA_UAL_FROZEN_EVIDENCE_2026-07-27.md`) purely as the planning
/// estimate of effect size, not as evidence for the final decision:
///
///   n = ((z_a*sqrt(p0*(1-p0)) + z_b*sqrt(p1*(1-p1))) / (p1-p0))^2
///     where p0=0.60, p1=0.625, z_0.05=1.6449, z_0.20=0.8416
///     = ((1.6449*sqrt(0.24) + 0.8416*sqrt(0.234375)) / 0.025)^2
///     ~= 2356, rounded up to 2400 for a small safety margin.
const PHASE6_CONFIRMATORY_N: usize = 2400;
...
impl P2SecondOrder {
    ...
    fn run_trial(
        ...
        let mut dyn_rng_choice =
            config.trial_seed("ual", "p2_choice", run_idx) ^ 0x9E3779B97F4A7C15;

        if matches!(rung, BaselineRung::GraphPropagation) {
            let b_inferred_value = a_value;
            let chose_b_over_d =
                softmax_choice(&[b_inferred_value, d_value], 0.3, &mut dyn_rng_choice) == 0;
        ...
        // B's inferred value: whichever of A/C the retrieval favored. A
        // genuine scalar estimate, not just a similarity margin — fed into
        // the choice mechanism below.
        let b_inferred_value = if neighbor_is_a_over_c {
            a_value
        } else {
            c_value
        };
    ...
    pub fn ual_report(&self, config: &BenchmarkConfig, n: usize) -> UalProbeReport {
        ...
        let mut report = combine_schedule_reports("UAL-P2", &blocked, &interleaved);
        // Mandatory, always-attached disclosure (a follow-up review's point:
        // a schedule note buried only in the module doc is too easy to miss
        // when a report gets forwarded on its own). Reproduction across
        // Blocked/Interleaved for THIS mechanism is proven algebraically
        // trivial (`schedule_produces_algebraically_identical_normalized_memory`)
        // — it is not a meaningful robustness demonstration, unlike P1/P4a
        // where the schedule genuinely could have mattered.
        report = report.with_note(
            "SCHEDULE CAVEAT: P2's accumulator is proven algebraically schedule-invariant \
             for this mechanism (bind_hv is the same fixed vector every pairing step) — \
             'replicated across schedules' here means the two orderings are algebraically \
             equivalent for this learner, not that a meaningful manipulation was \
             successfully varied and survived. Contrast with P1/P4a, whose schedules \
             genuinely can (and do) change the outcome.",
        );
        report
    }
    ...
    fn schedule_report(
        ...
        let mut chose_b = Vec::with_capacity(n);
        ...
        for i in 0..n {
            ...
            chose_b.push(if o.chose_b_over_d { 1.0 } else { 0.0 });
        ...
        let choice_metric =
            MetricValue::from_samples_bootstrap(&chose_b, config.seed ^ 0x5EED_C401);

        // Sub-result 1: relational-identity retrieval above chance.
        let relational_identity_observed = retrieval_metric.ci_lower > 0.5;
        // Sub-result 2: genuine behavioral/action readout — a forced choice
        // between B and neutral D actually favors B above chance, not just
        // a similarity margin. Second claim-integrity pass's fix: retrieval
        // alone does not establish "second-order conditioning" (a follow-up
        // review's point) — both sub-results are required for
        // `Demonstrated`.
        //
        // Phase 6 (P2 re-evaluation): a bare CI-excludes-chance check is not
        // sufficient — a large enough n could make a trivially small effect
        // "significant" without it being practically meaningful. Requires
        // the CI lower bound to clear `VALUE_TRANSFER_PRACTICAL_FLOOR`
        // instead of bare chance (0.5).
        let value_transfer_observed = choice_metric.ci_lower > VALUE_TRANSFER_PRACTICAL_FLOOR;
        let value_transfer_above_chance_only =
            !value_transfer_observed && choice_metric.ci_lower > 0.5;
        let behavioral_expression = if relational_identity_observed && value_transfer_observed {
            Presence::Observed
        } else {
        ...
        // Honest scope: an internal-responsiveness observation this run
        // (did A's own value move at all), not an independently-verified
        // positive control in the classical sense.
        let positive_control_observed = mean_a_value > 0.6;
        ...
        let schedule_executed_as_declared =
            format!("{blocked_steps:?}") != format!("{interleaved_steps:?}");

        let qualification = UalRuntimeQualification {
            static_qual: UalStaticQualification {
                // D's own value never moves (never conditioned, never
                // paired) — structural guarantee by construction (d_value
                // is a literal constant in `run_trial`), not re-derived per
                // run.
                ...
                // The retrieval mechanism (`.bind(&b_hv)`, not textbook
                // unbinding) is characterized, not unresolved — the audit
                // (`hdc_binding_properties.rs`) established it works via a
                // real, understood, directionally-correct self-squared-term
                // bias. Unlike P4a, no open *interpretation* question
                // remains for what P2's retrieval-identity criterion means.
            ...
            run_qual: UalRunQualification {
                ...
                signal_usable: n > 0
                    && retrieval_metric.mean.is_finite()
                    && choice_metric.mean.is_finite(),
        ...
        .with_note(format!(
            "relational-identity(A over C) rate={:.4} [{:.4},{:.4}]; value-transfer(choose B over D) rate={:.4} [{:.4},{:.4}] under {:?}; mean(sim_A - sim_D)={:.4}; mean A_value={:.4}",
            retrieval_metric.mean, retrieval_metric.ci_lower, retrieval_metric.ci_upper,
            choice_metric.mean, choice_metric.ci_lower, choice_metric.ci_upper,
            schedule, mean_margin_ad, mean_a_value
        ));
        if relational_identity_observed && !value_transfer_observed {
            report = report.with_note(
                "relational retrieval observed WITHOUT value-transfer/behavioral expression — \
                 this supports only 'P2 relational retrieval', not 'second-order value conditioning'",
            );
        }
        if value_transfer_above_chance_only {
            report = report.with_note(format!(
                "choice-rate CI lower bound ({:.4}) is statistically distinguishable from chance \
                 (0.5) but does not clear the practical-significance floor ({:.2}) — reported as \
                 NotDemonstrated on the practical criterion, not silently merged with a clean null",
                choice_metric.ci_lower, VALUE_TRANSFER_PRACTICAL_FLOOR
            ));
        }
        report
    }
...
mod tests {
    ...
    /// **Calibration pilot, NOT the live confirmatory verdict** (reframed for
    /// Phase 6, `SYMTHAEA_UAL_P2_PHASE6_RESULT_2026-07-28.md`). This is the
    /// frozen n=40/dim=512 pilot from the second claim-integrity pass:
    /// relational retrieval is essentially perfect (rate=1.0), but the real
    /// forced choice between B and neutral D lands at a point estimate of
    /// ~0.625 with a bootstrap CI of roughly [0.45, 0.75] — far too wide to
    /// be a real confirmatory claim at n=40, and its lower bound sits well
    /// below both chance (0.5) and the Phase 6 practical floor
    /// (`VALUE_TRANSFER_PRACTICAL_FLOOR`=0.60). This pilot's point estimate
    /// (0.625) is exactly what Phase 6's power analysis used to derive
    /// `PHASE6_CONFIRMATORY_N` — see the properly-powered confirmatory test
    /// below (`phase6_confirmatory_value_transfer_result`) for the actual
    /// answer to "is there a real, practically meaningful effect".
    #[test]
    fn value_transfer_criterion_is_not_yet_met_relational_retrieval_alone_is() {
        let cfg = config();
        let report = P2SecondOrder.ual_report(&cfg, 40);
        assert_eq!(
            report.functional_outcome,
            FunctionalOutcome::NotDemonstrated,
            "expected the stricter conjunctive criterion to correctly withhold \
             Demonstrated pending a real value-transfer signal; got {:?}",
            report.functional_outcome
        );
        assert!(
            report
                .notes
                .iter()
                .any(|n| n.contains("relational retrieval observed WITHOUT value-transfer")),
            "the NotDemonstrated verdict must explicitly distinguish retrieval from \
             conditioning, not just report a bare negative"
        );
    }

    /// Phase 6 calibration/holdout partition: the confirmatory config's seed
    /// must produce a fully disjoint random stream from the existing n=40
    /// pilot's default seed (42) -- guaranteed by `trial_seed`'s
    /// multiplicative hash over `config.seed`, verified directly rather than
    /// assumed.
    #[test]
    fn phase6_confirmatory_seed_is_disjoint_from_pilot_seed() {
        let pilot_cfg = config();
        let confirmatory_cfg = BenchmarkConfig {
            seed: pilot_cfg.seed ^ 0xFEED_C0DE,
            ..config()
        };
        assert_ne!(pilot_cfg.seed, confirmatory_cfg.seed);
        for run_idx in 0..PHASE6_CONFIRMATORY_N.min(50) {
            for tag in ["p2_stim_a", "p2_stim_b", "p2_stim_c", "p2_stim_d"] {
                let pilot_seed = pilot_cfg.trial_seed("ual", tag, run_idx);
                let confirmatory_seed = confirmatory_cfg.trial_seed("ual", tag, run_idx);
                assert_ne!(
                    pilot_seed, confirmatory_seed,
                    "pilot and confirmatory seed streams must never collide (tag={tag}, run_idx={run_idx})"
                );
            }
        }
    }

    /// **Phase 6 (P2 re-evaluation) confirmatory result — real outcome:
    /// `Demonstrated`.** `SYMTHAEA_UAL_P2_PHASE6_RESULT_2026-07-28.md`
    /// records the full preregistration: practical floor
    /// (`VALUE_TRANSFER_PRACTICAL_FLOOR`=0.60) and sample size
    /// (`PHASE6_CONFIRMATORY_N`=2400, from a one-sided power analysis using
    /// the frozen n=40 pilot estimate) were both fixed BEFORE this test was
    /// run, using a confirmatory seed (`42 ^ 0xFEED_C0DE`) fully disjoint
    /// from the pilot's seed (see
    /// `phase6_confirmatory_seed_is_disjoint_from_pilot_seed`).
    ///
    /// The real result, both schedules: relational-identity rate=1.0000
    /// [1.0000,1.0000]; value-transfer (choose B over D) rate=0.6579
    /// [0.6383,0.6763] — CI lower bound clears the 0.60 floor with a
    /// comfortable margin (not a borderline pass), and closely replicates
    /// the n=40 pilot's point estimate (0.625 -> 0.6579, same direction and
    /// magnitude, now with a CI ~14x narrower). This is the first
    /// `Demonstrated` verdict anywhere in the UAL track, reached via a
    /// properly-powered preregistered test, not via loosening any threshold
    /// after the fact. Per the UAL design doc's disclosure requirement, this
    /// licenses at most "second-order value conditioning demonstrated for
    /// this benchmark-local candidate mechanism" -- NOT "UAL demonstrated"
    /// and NOT any claim about live Symthaea.
    #[test]
    fn phase6_confirmatory_value_transfer_result() {
        let cfg = BenchmarkConfig {
            seed: 42 ^ 0xFEED_C0DE,
            dimension: 512,
            ..Default::default()
        };
        let report = P2SecondOrder.ual_report(&cfg, PHASE6_CONFIRMATORY_N);
        assert_eq!(
            report.functional_outcome,
            FunctionalOutcome::Demonstrated,
            "Phase 6 confirmatory result at n={PHASE6_CONFIRMATORY_N}: {:?}; notes={:?}",
            report.functional_outcome,
            report.notes
        );
        assert_eq!(report.behavioral_expression, Presence::Observed);
        assert!(
            report
                .notes
                .iter()
                .any(|n| n.contains("both schedules independently reached Demonstrated")),
            "expected both Blocked and Interleaved to independently qualify, not one \
             arm carrying the other; notes={:?}",
            report.notes
        );
    }
}
