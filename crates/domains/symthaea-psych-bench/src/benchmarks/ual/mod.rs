...
//!   a shared HDC value-integration memory. **Superseded by
//!   `p4a_compositional_generalization`** (2026-07-28) after a
//!   construct-validity gap was found — kept as historical record, its own
//!   `Inconclusive` verdict remains valid and unchanged.
//! - **P4a-v2** (`p4a_compositional_generalization`): the P4a redesign —
//!   a genuinely relational "matching" rule with deterministic, known-in-
//!   advance correct answers per held-out item, and a 5-rung baseline
//!   ladder including a shuffled-relation control, modeled on
//!   `benchmarks::executive::ravens`'s proven rule-extraction-and-scoring
//!   pattern.
//!
...
pub mod p1_live_diagnostic;
pub mod p1_reversal;
pub mod p2_second_order;
pub mod p4a_compositional_generalization;
pub mod p4a_recombination;
pub mod report;

#[cfg(feature = "symthaea-backend")]
pub use p1_live_diagnostic::P1LiveReversal;
pub use p1_reversal::P1Reversal;
pub use p2_second_order::P2SecondOrder;
pub use p4a_compositional_generalization::P4aCompositionalGeneralization;
pub use p4a_recombination::P4aRecombination;
pub use report::{
    FunctionalOutcome, Presence, ScheduleStatus, SystemUnderTest, UalProbeReport,
    UalRunQualification, UalRuntimeQualification, UalSchedule, UalStaticQualification,
};
