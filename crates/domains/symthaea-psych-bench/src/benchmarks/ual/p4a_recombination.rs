    ...
    fn schedule_report(
        ...
        let qualification = UalRuntimeQualification {
            static_qual: UalStaticQualification {
                ...
                // structural guarantee, pinned by
                // `value_table_fails_on_held_out_compounds`.
                ...
                // Covered by `marginal_mean_baseline_passes_by_construction`
                // and `static_binding_geometry_only_no_value_semantics`.
                baseline_design_valid: true,
                // **Deliberately false — this is the fix for a real
                // contradiction a follow-up review caught**: this probe's
                // report was previously typed `Demonstrated` in the SAME
                // breath as a prose note saying "do not cite as evidence".
                // A typed result a downstream tool can act on must not
                // contradict its own prose caveat. `hdc_binding_properties.rs`
                // established that `.bind(compound_hv)` (not
                // `.bind(&compound_hv.inverse())`) does not perform textbook
                // VSA unbinding here — whether WZ/YX's readouts reflect
                // genuine compositional generalization or a geometric/
                // self-product artifact of this crate's real
                // `ContinuousHV::random` distribution is unresolved. Until
                // that's settled, this field forces the whole report to
                // `Inconclusive` regardless of how clean the behavioral
                // numbers look — see module doc's PROTOTYPE caveat.
                construct_validity_passed: false,
            },
            run_qual: UalRunQualification {
                // Training trials always execute (no analog of P1's
                // hazard-based non-manipulation); real check that memory
                // formation happened.
                manipulation_applied: true,
                positive_control_observed: positive_control_passed,
                negative_control_observed: true,
                signal_usable: n > 0 && diff_metric.mean.is_finite(),
                schedule_executed_as_declared: schedule_valid,
            },
        };
        ...
        .with_note(format!(
            "mean(WZ_readout - YZ_readout)={:.4} [{:.4},{:.4}] under {:?}; mean internal margin diff={:.4}; PROTOTYPE: binding semantics under audit, see module doc — this probe's functional outcome is forced Inconclusive via construct_validity_passed=false until resolved",
...
mod tests {
    ...
    /// **Second claim-integrity pass regression**: a follow-up review caught
    /// a real contradiction — this probe's report was typed `Demonstrated`
    /// in the same breath as a prose note saying "do not cite as evidence".
    /// Pinned here as an explicit, intentional invariant (not an incidental
    /// side effect of the qualification refactor): as long as
    /// `construct_validity_passed=false` for P4a, the report must ALWAYS be
    /// `Inconclusive`, regardless of how clean the underlying WZ/YZ
    /// discrimination looks. This test should start failing the moment
    /// someone flips that field to `true` — which should only happen once
    /// the HDC binding/unbinding audit's open question is actually resolved.
    #[test]
    fn functional_outcome_is_forced_inconclusive_pending_construct_validity_resolution() {
        let cfg = config();
        let report = P4aRecombination.ual_report(&cfg, 40);
        assert_eq!(
            report.functional_outcome,
            FunctionalOutcome::Inconclusive,
            "P4a must report Inconclusive while construct_validity_passed=false, \
             even though the underlying WZ>YZ effect is real and reproducible"
        );
        assert!(
            report
                .notes
            ...
            "the Inconclusive verdict must cite construct_validity_passed as the reason"
        );
    }
}
