...
/// Validation result for one exact frozen model. Pass/fail is derived only from the precommitted gate.
...
/// Identity-only permit proving an exact frozen model passed its precommitted validation gate.
