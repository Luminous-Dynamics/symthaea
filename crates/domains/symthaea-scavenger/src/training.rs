...
use crate::fep_agent::ActiveInferenceScavengerAgent;
...
pub struct ScavengerTrainer {
    controller: ScavengerController,
    encoder: ScavengerHdcEncoder,
    fep: ActiveInferenceScavengerAgent,
    simulator: SimpleScavengerSimulator,
    config: ScavengerConfig,
}

impl ScavengerTrainer {
    pub fn new(config: ScavengerConfig) -> Self {
        let g = GenesisSeed::from_phrase(&config.genesis_phrase);
        Self {
            controller: ScavengerController::new(&g, &config),
            encoder: ScavengerHdcEncoder::new(&g, 32),
            fep: ActiveInferenceScavengerAgent::new(),
            simulator: SimpleScavengerSimulator::new(&g),
            config,
        }
    }
...
mod tests {
    ...
    #[test]
    fn test_episode() {
        let mut c = ScavengerConfig::default();
        c.steps_per_episode = 200;
        let mut t = ScavengerTrainer::new(c);
        let m = t.run_episode();
        assert!(!m.diverged);
