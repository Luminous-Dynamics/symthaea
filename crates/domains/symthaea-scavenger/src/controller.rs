...
use crate::types::{NUM_ACTUATORS, ScavengerCommand, ScavengerConfig};
...
pub struct ScavengerController {
...
impl ScavengerController {
    pub fn new(g: &GenesisSeed, c: &ScavengerConfig) -> Self {
        ...
        let wh = ContinuousHV::from_genesis(g, "scavenger::out_w", NUM_ACTUATORS * HDC_DIM);
    ...
    pub fn forward(&mut self, hv: &ContinuousHV, dt: f32) -> ScavengerCommand {
    ...
    /// makes `ScavengerTrainer` actually train (Tier 2 of
    /// SYMTHAEA_UNAUDITED_PLATFORMS_REVIEW_2026-07-07.md).
    pub fn train_step(&mut self, target: &ScavengerCommand) -> f32 {
...
mod tests {
    ...
    fn test_fwd() {
        let mut c =
            ScavengerController::new(&GenesisSeed::from_phrase("t"), &ScavengerConfig::default());
