...
id_type!(
    SectorId,
    "stock-flow sector id must be non-empty and bounded"
);
id_type!(
    InstrumentId,
    "stock-flow instrument id must be non-empty and bounded"
);
id_type!(
    PositionId,
    "stock-flow position id must be non-empty and bounded"
);
...
mod tests {
    ...
    // The helper mirrors the five explicit stock-flow coordinates of the
    // production type. Keeping them visible makes synthetic fixtures auditable.
    ...
    fn equity_financing_and_real_capital_are_distinct_records() {
        let system = StockFlowSystem {
            ...
            real_capital_formation: vec![
                RealCapitalFormation::new(sector("firm"), RealCapitalCategory::Machinery, 100.0)
                    .unwrap(),
            ],
            ..Default::default()
        };
    ...
    fn writeoff_is_explicit_and_does_not_disappear() {
        let system = StockFlowSystem {
            positions: vec![position(
                "loan", "bank", "firm", 100.0, 0.0, 0.0, 25.0, 75.0,
            )],
            ..Default::default()
        };
        let report = system.validate(ClosureProfile::default()).unwrap();
