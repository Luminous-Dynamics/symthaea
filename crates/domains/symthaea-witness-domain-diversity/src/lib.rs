...
//! Administrative-domain diversity verification for independently witnessed continuity.
//!
//! `symthaea-mycelix-continuity-witness` proves that distinct Holochain agent keys retained
//! and observed the exact Xenia checkpoint. This crate asks the deliberately separate question:
//! do enough of those keys belong to independently governed/control domains?
//!
...
//! subject binding, schema/type, purpose and validity window. Symthaea then re-checks the
//! compact verified view and counts canonical `control_domain_id`s rather than labels.
//!
//! The result is evidence, never authority, consent, personhood, or an execution permit.
...
use symthaea_mycelix_continuity_witness::IndependentlyWitnessedCheckpoint;
use thiserror::Error;
...
/// Purpose bound into the verified credential or presentation.
...
///
/// There are intentionally no defaults: deployments must choose thresholds explicitly.
...
pub struct WitnessDiversityPolicy {
    ...
    /// If true, every base witness key must have an acceptable verified domain binding.
...
/// root therefore share the same `control_domain_id` and count only once.
...
/// Boundary to Mycelix Identity or another trusted VC verification implementation.
///
/// Implementations MUST, before returning `Some(binding)`:
/// - cryptographically verify the credential/presentation;
...
/// - verify the asserted domain and canonical control root;
...
mod tests {
    ...
    use std::fmt;
    use symthaea_mycelix_continuity_witness::mycelix_checkpoint_digest;
    ...
    fn checkpoint(witnesses: &[&str]) -> IndependentlyWitnessedCheckpoint {
        IndependentlyWitnessedCheckpoint {
            ...
            checkpoint_digest: mycelix_checkpoint_digest(b"checkpoint"),
    ...
    fn distinct_control_roots_pass() {
        ...
        let resolver = MockResolver {
            values: BTreeMap::from([
                ("uhCAk-a".into(), binding("uhCAk-a", "council:technical", "operator:alpha", "did:issuer:a")),
                ("uhCAk-b".into(), binding("uhCAk-b", "org:archive", "operator:beta", "did:issuer:b")),
            ]),
            fail_for: None,
        };
    ...
    fn different_labels_under_same_control_root_count_once() {
        ...
        let resolver = MockResolver {
            values: BTreeMap::from([
                ("uhCAk-a".into(), binding("uhCAk-a", "council:technical", "operator:alpha", "did:issuer:a")),
                ("uhCAk-b".into(), binding("uhCAk-b", "org:archive", "operator:alpha", "did:issuer:b")),
            ]),
            fail_for: None,
        };
        assert!(matches!(
    ...
    fn issuer_diversity_is_independent_policy_axis() {
        ...
        let resolver = MockResolver {
            values: BTreeMap::from([
                ("uhCAk-a".into(), binding("uhCAk-a", "domain:a", "root:a", "did:issuer:shared")),
                ("uhCAk-b".into(), binding("uhCAk-b", "domain:b", "root:b", "did:issuer:shared")),
            ]),
            fail_for: None,
        };
        assert!(matches!(
    ...
    #[test]
    fn unbound_witness_fails_when_complete_binding_is_required() {
        let cp = checkpoint(&["uhCAk-a", "uhCAk-b"]);
        let resolver = MockResolver {
            values: BTreeMap::from([(
                "uhCAk-a".into(),
    ...
    #[test]
    fn schema_substitution_is_rejected() {
        let cp = checkpoint(&["uhCAk-a"]);
        let mut value = binding("uhCAk-a", "domain:a", "root:a", "did:issuer:a");
        value.schema_id = "mycelix:schema:governance:other:v1".into();
        let resolver = MockResolver {
            values: BTreeMap::from([("uhCAk-a".into(), value)]),
            fail_for: None,
        };
        assert!(matches!(
            verify_witness_domain_diversity(&cp, policy(1, 1), &resolver),
            Err(WitnessDiversityError::CredentialSchemaMismatch)
        ));
    }
