...
//! Build, rotate, audit, and query publication checkpoint witness policies.
...
use symthaea_music_theory::{
    CalibrationPublicationCatalogCheckpoint, CalibrationPublicationCheckpointWitnessPolicy,
    CalibrationPublicationWitnessPolicyEpoch, CalibrationPublicationWitnessPolicyLedger,
    CalibrationPublicationWitnessPolicyRotationPayload,
    CalibrationPublicationWitnessPolicyRotationSet,
    CalibrationSignedPublicationWitnessPolicyRotation, CalibrationSignerIdentity,
    ...
    build_calibration_publication_witness_policy_genesis,
    build_calibration_publication_witness_policy_rotation_set,
    build_calibration_signed_publication_witness_policy_rotation,
    plan_calibration_publication_witness_policy_rotation,
...
fn plan(arguments: Arguments) -> Result<(), Box<dyn Error>> {
    let ledger: CalibrationPublicationWitnessPolicyLedger =
    ...
    write_json(
        ...
        &RotationPlanExport { epoch, payload },
    )
}

fn wrap(arguments: Arguments) -> Result<(), Box<dyn Error>> {
    let payload: CalibrationPublicationWitnessPolicyRotationPayload =
        read_json(&required_path(arguments.payload, "--payload")?)?;
    let signature = std::fs::read(required_path(arguments.signature, "--signature")?)?;
    let envelope = build_calibration_signed_publication_witness_policy_rotation(
        payload,
...
fn rotation_set(arguments: Arguments) -> Result<(), Box<dyn Error>> {
    let payload: CalibrationPublicationWitnessPolicyRotationPayload =
        read_json(&required_path(arguments.payload, "--payload")?)?;
    let outgoing_policy: CalibrationPublicationCheckpointWitnessPolicy = read_json(
        &required_path(arguments.outgoing_policy, "--outgoing-policy")?,
    )?;
    let incoming_policy: CalibrationPublicationCheckpointWitnessPolicy = read_json(
        &required_path(arguments.incoming_policy, "--incoming-policy")?,
    )?;
    let outgoing = read_many::<CalibrationSignedPublicationWitnessPolicyRotation>(
        &arguments.outgoing_statements,
    )?;
    let incoming = read_many::<CalibrationSignedPublicationWitnessPolicyRotation>(
...
fn append(arguments: Arguments) -> Result<(), Box<dyn Error>> {
    let mut ledger: CalibrationPublicationWitnessPolicyLedger =
        read_json(&required_path(arguments.ledger, "--ledger")?)?;
    let epoch: CalibrationPublicationWitnessPolicyEpoch =
        read_json(&required_path(arguments.policy_epoch, "--policy-epoch")?)?;
    let set: CalibrationPublicationWitnessPolicyRotationSet =
        read_json(&required_path(arguments.rotation_set, "--rotation-set")?)?;
    let verifier = verifier(&arguments)?;
    append_calibration_publication_witness_policy_rotation(&mut ledger, epoch, set, &verifier)?;
    write_json(arguments.write.as_deref(), &ledger)
}
...
fn verify(arguments: Arguments) -> Result<(), Box<dyn Error>> {
    let ledger: CalibrationPublicationWitnessPolicyLedger =
        read_json(&required_path(arguments.ledger, "--ledger")?)?;
    let verifier = verifier(&arguments)?;
    let report = verify_calibration_publication_witness_policy_ledger(&ledger, &verifier);
    write_json(arguments.write.as_deref(), &report)?;
    if !report.accepted() {
        std::process::exit(2);
    }
...
fn active(arguments: Arguments) -> Result<(), Box<dyn Error>> {
    ...
    let checkpoint: CalibrationPublicationCatalogCheckpoint =
        read_json(&required_path(arguments.checkpoint, "--checkpoint")?)?;
    let epoch = active_calibration_publication_witness_policy_epoch(&ledger, &checkpoint)
        .ok_or_else(|| invalid_input("no witness-policy epoch is active at this checkpoint"))?;
    ...
    fn parse() -> Result<Self, Box<dyn Error>> {
        ...
        while let Some(argument) = values.next() {
            match argument.as_str() {
                ...
                "--policy-epoch" => {
                    result.policy_epoch = Some(PathBuf::from(next_value(&mut values, &argument)?))
                }
                "--payload" => {
...
fn print_help() {
    eprintln!("evidence_publication_witness_policy <command> [options]");
    ...
    eprintln!(
        "  append --ledger FILE --policy-epoch FILE --rotation-set FILE --verifier PROGRAM [--verifier-arg ARG]... [--write FILE]"
    );
