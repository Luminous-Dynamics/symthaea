...
///
/// `pub` (not just crate-private) so the Phase 2.2C-i architecture-isolation ablation
/// (`examples/evolutionary_rescue_common.rs`) can construct synthetic second-channel raw values
/// in the same domain `normalize_trait` expects, without duplicating these numbers.
pub const TRAIT_FLOOR: f64 = 0.15;
pub const TRAIT_CEILING: f64 = 0.6;
