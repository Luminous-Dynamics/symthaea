...
use symthaea_futures_core::{
    AbstentionReason, ForecastDistribution, ForecastOutput, Horizon, OutcomeRegion, OutcomeSpaceId,
    TrajectoryGenerator,
};
...
fn interval_atoms_distribution(
    ...
    // Merge equal ticks into one branch, summing their weights.
    //
    // Ensemble members frequently agree on an extinction tick, which used to emit several
    // identical `[t, t]` point atoms. `ForecastDistribution` now rejects duplicate/overlapping
    // regions, because at the *boundary* a repeated region is indistinguishable from a
    // double-counting bug. Here the domain meaning is unambiguous — repeated agreement means
    // mass concentrates at `t` — so canonicalizing is correct rather than an error.
    //
    // This is numerically identical to the previous behaviour: the scorer's `probability_of`
    // already summed every branch matching a region, so merging changes no score.
    let mut merged: Vec<(f64, f64)> = Vec::new();
    for &t in atom_ticks {
        match merged.iter_mut().find(|(loc, _)| *loc == t) {
            Some((_, w)) => *w += weight,
            None => merged.push((t, weight)),
        }
    }

    // Renormalize away accumulated f64 summation drift, so a large ensemble can't miss
    // MASS_TOLERANCE by a few ULPs.
    ...
    let branches = merged
        ...
        .map(|(t, w)| {
            (
                w / total,
                OutcomeRegion::interval(t, t).expect("tick is finite and low == high"),
                Vec::new(),
            )
        })
        .collect();
    ...
    ForecastDistribution::try_from_raw(
        issued_at_tick,
        horizon,
        OutcomeSpaceId(TIME_TO_EXTINCTION_CONDITIONAL.to_string()),
        branches,
        0.0,
    )
    .expect("merged, renormalized point atoms are valid by construction")
}
...
mod tests {
    ...
    fn generated_forecast_is_scoreable_by_the_calibration_crate() {
        ...
        let score = BrierScore
            .score(&forecast, &OutcomeRegion::Boolean(true))
    ...
    fn extract_interval_point(output: &ForecastOutput) -> f64 {
        ...
        let OutcomeRegion::Interval(iv) = dist.branches()[0].outcome else {
            panic!("expected an Interval outcome");
        };
        assert_eq!(iv.low(), iv.high(), "expected a single-atom point forecast");
        iv.low()
    }
    ...
    fn extract_atom_ticks(output: &ForecastOutput) -> Vec<f64> {
        ...
        dist.branches()
            .iter()
            .map(|b| {
                let OutcomeRegion::Interval(iv) = b.outcome else {
                    panic!("expected an Interval outcome");
                };
                assert_eq!(iv.low(), iv.high(), "expected a single-atom branch");
                iv.low()
            })
            .collect()
    }
    ...
    fn ensemble_matches_uncensored_point_forecast_on_a_noiseless_decline() {
        ...
        // This assertion changed shape when `interval_atoms_distribution` began merging equal
        // point atoms (a `ForecastDistribution` now rejects duplicate/overlapping regions). It
        // previously read `assert_eq!(atoms.len(), 50)` — one branch per bootstrap member, each
        // checked to equal the point forecast.
        //
        // That was asserting an implementation detail. A distribution has mass over regions, not
        // a member count, and "all 50 resamples agree" is expressed more directly as a single
        // atom carrying the entire mass. The check below is strictly stronger on the property
        // the test actually claims: it pins the total mass, which the old form never did.
        //
        // Coverage genuinely lost: this test can no longer witness that 50 resamples were drawn
        // rather than one. `ensemble_expresses_real_spread_under_noise` is what covers the
        // ensemble actually drawing multiple distinct members.
        assert_eq!(
            atoms.len(),
            1,
            "the zero-noise limit must collapse to exactly one atom, got {atoms:?}"
        );
        assert!(
            (atoms[0] - point_predicted).abs() < 1e-9,
            "expected the collapsed atom to match the point forecast ({point_predicted}), got {}",
            atoms[0]
        );
        ...
        let mass: f64 = dist.branches().iter().map(|b| b.probability.get()).sum();
        assert!(
            (mass - 1.0).abs() < 1e-9,
            "the single collapsed atom must carry the whole mass, got {mass}"
        );
        assert_eq!(dist.unsupported_mass().get(), 0.0);
    }
    ...
    fn ensemble_expresses_real_spread_under_noise() {
        ...
        let total_probability: f64 = dist.branches().iter().map(|b| b.probability.get()).sum();
        assert!(
    ...
    fn ensemble_forecast_is_scoreable_via_crps_and_a_correct_atom_beats_a_confident_wrong_point() {
        ...
        let score = Crps
            ...
            .expect("CRPS shape mismatch on a validated forecast")
            .get();
    ...
    fn time_to_extinction_forecast_is_scoreable_via_crps() {
        ...
        let actual = OutcomeRegion::interval(2.0, 2.0).unwrap();
        let score = Crps
            .score(&forecast, &actual)
            .expect("CRPS shape mismatch on a validated forecast")
            .get();
