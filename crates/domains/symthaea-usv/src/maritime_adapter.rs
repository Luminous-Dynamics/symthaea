...
/// Project USV-local state into the shared maritime substrate.
pub fn to_maritime_state(
    ...
    let (latitude_deg, longitude_deg, navigation_quality) = match navigation_fix {
        Some(fix) => (Some(fix.latitude_deg), Some(fix.longitude_deg), fix.quality),
...
mod tests {
    ...
    fn absent_external_fix_becomes_dead_reckoning_not_fake_coordinates() {
        let maritime = to_maritime_state("usv-1", 42, &state(), None);
