    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::ProjectionNotComplete(status) => {
                write!(f, "electrical utility projection is not complete: {status:?}")
            }
            Self::MissingProjectionField(field) => {
                write!(f, "complete electrical utility projection is missing: {field}")
            }
            ...
            Self::DuplicateUnresolvedReason(reason) => {
                write!(f, "duplicate unresolved utility projection reason: {reason:?}")
            }
        }
    }
}
...
fn validate_complete_projection(
    ...
    let electrical_energy_j = projection
        .electrical_energy_j
        .ok_or(UtilityBindingError::MissingProjectionField(
            "electrical_energy_j",
        ))?;
    let peak_power_w = projection
        .peak_electrical_power_w
        .ok_or(UtilityBindingError::MissingProjectionField(
            "peak_electrical_power_w",
        ))?;
    let process_time_s = projection
        .process_time_s
        .ok_or(UtilityBindingError::MissingProjectionField("process_time_s"))?;
...
mod tests {
    ...
    fn exact_demand_context_and_identity_are_preserved() {
        ...
        assert_eq!(bound.gross_energy_j, projection.electrical_energy_j.unwrap());
        assert_eq!(
            bound.batch_duration_s,
            projection.process_time_s.unwrap()
        );
        assert_eq!(
            bound.peak_power_w,
