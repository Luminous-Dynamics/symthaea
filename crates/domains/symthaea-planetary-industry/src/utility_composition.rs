...
mod tests {
    use super::*;
    use crate::{
        DurationRangeS, EnergyRangeJ, FractionRange, MassRangeKg, MaterialGrade,
        OutputDisposition, PhysicalForm, PowerRangeW, ProcessInput, ProcessInputRole,
        ProcessOutput, ProcessOutputRole, UtilityDemand, UtilityProjectionReason,
        UtilityProjectionStatus,
    };
    ...
    fn valid_process_is_validated_projected_and_bound_losslessly() {
        ...
        assert_eq!(bound.gross_energy_j, EnergyRangeJ::new(90.0, 110.0).unwrap());
        assert_eq!(bound.peak_power_w, PowerRangeW::new(20.0, 25.0).unwrap());
        assert_eq!(bound.batch_duration_s, DurationRangeS::new(9.0, 11.0).unwrap());
    ...
    fn detached_stale_projection_cannot_influence_preferred_path() {
        ...
        assert_eq!(stale.electrical_energy_j, Some(EnergyRangeJ::new(90.0, 110.0).unwrap()));
        ...
        let bound =
            compose_subject_bound_electrical_accounting_case(&changed, &baseline_context()).unwrap();
        assert_eq!(bound.gross_energy_j, EnergyRangeJ::new(140.0, 160.0).unwrap());
        assert_eq!(bound.peak_power_w, PowerRangeW::new(30.0, 35.0).unwrap());
        assert_eq!(bound.batch_duration_s, DurationRangeS::new(12.0, 14.0).unwrap());
    ...
    fn incomplete_electrical_basis_fails_through_fresh_projection() {
        let mut process = baseline_process();
        process.utilities.retain(|utility| {
            !matches!(utility, UtilityDemand::ElectricalEnergy(_))
        });
        assert_eq!(
            compose_subject_bound_electrical_accounting_case(&process, &baseline_context()),
    ...
    fn unresolved_thermal_and_cooling_semantics_survive_composition() {
        ...
        let bound =
            compose_subject_bound_electrical_accounting_case(&process, &baseline_context()).unwrap();
        assert_eq!(
    ...
    fn explicit_context_is_preserved_without_feasibility_evaluation() {
        ...
        assert_eq!(case.storage_discharge_power_w, context.storage_discharge_power_w);
        assert_eq!(
