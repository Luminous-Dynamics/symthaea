...
fn finite_difference3(a: f64, b: f64, c: f64, label: &'static str) -> Result<f64, OntologyError> {
    let value = a - b - c;
    if value.is_finite() {
        Ok(value)
    } else {
...
pub fn screen_thermal_approach(
    ...
    Ok(ThermalApproachReport {
        ...
        required_source_temperature_k,
        basis: case.basis,
        supports_finite_driving_force_claim: case.basis == ThermalApproachBasis::FiniteDrivingForce
            && case.minimum_approach_k.min.value() > 0.0,
    })
}
