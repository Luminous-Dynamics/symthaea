...
use symthaea_assurance_linux_tpm_ima_pcr_anchor::{
    LINUX_TPM_IMA_PCR_ANCHOR_POLICY_SCHEMA_V1, LinuxTpmImaPcrAnchorDisposition,
    LinuxTpmImaPcrAnchorIssue, LinuxTpmImaPcrAnchorPolicy, qualify_linux_tpm_ima_pcr_anchor,
};
...
use symthaea_assurance_tpm2_checkquote_adapter::{
    CHECKQUOTE_POLICY_SCHEMA_V1, CheckquoteExecutionRequest, CheckquoteExecutor,
    RawTpm2QuoteBundle, Sha256PcrValue, ToolExecution, Tpm2CheckquotePolicy,
    Tpm2QuoteQualification, verify_tpm2_quote,
};
...
use symthaea_linux_ima_replay::{
    IMA_REPLAY_POLICY_SCHEMA_V1, ImaReplayPolicy, ImaReplayQualification, RequiredImaMeasurement,
    SupportedImaTemplate, verify_canonical_ima_sha256,
};
...
fn fixture(quote_matches_ima: bool) -> Fixture {
    ...
    let quote_pcr = if quote_matches_ima {
        ima_final
    } else {
        [0x77; 32]
    };
...
fn exact_quote_possession_and_ima_replay_mint_opaque_pcr_anchor() {
    ...
    assert_eq!(
        result.verified().pcr_value(),
        fixture.ima.verified().final_pcr()
    );
    assert_eq!(
