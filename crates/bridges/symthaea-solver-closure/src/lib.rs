    ...
    pub fn validate(&self) -> Result<(), ClosureError> {
        ...
        for constraint in &self.ambient_constraints {
            ...
            if !constraints.insert(constraint.as_str()) {
                return Err(ClosureError::DuplicateAmbientConstraint(constraint.clone()));
            }
        }
...
mod tests {
    ...
    fn unresolved_dependency_blocks_finalization() {
        ...
        assert!(matches!(
            result,
            Err(ClosureError::UnresolvedDependencies(_))
        ));
    }
    ...
    fn unknown_parent_is_rejected() {
        ...
        assert!(matches!(
            closure.validate(),
            Err(ClosureError::UnknownParent { .. })
        ));
    }

    #[test]
    fn dependency_cycle_is_rejected() {
        let a = SolverInputArtifact::new("a", SolverInputRole::Referenced, d("a")).with_parent("b");
        let b = SolverInputArtifact::new("b", SolverInputRole::Referenced, d("b")).with_parent("a");
        ...
        assert!(matches!(
            closure.validate(),
            Err(ClosureError::DependencyCycle(_))
        ));
    }
    ...
    fn primary_and_solver_cardinality_are_enforced() {
        ...
        no_primary
            .artifacts
            .retain(|artifact| artifact.role != SolverInputRole::Primary);
