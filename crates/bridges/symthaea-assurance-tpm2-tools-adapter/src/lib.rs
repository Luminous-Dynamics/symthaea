...
pub use parser::{Tpm2NvPublicEvidence, parse_nv_public};
#[cfg(target_os = "linux")]
...
pub fn read_tpm2_nv_counter(
    ...
    if !policy.validate() {
        return Err(Tpm2AdapterError::InvalidPolicy);
    }
    verify_tool(
        executor,
        &policy.nvreadpublic_path,
        &policy.expected_nvreadpublic_blake3,
    )?;
    verify_tool(
        executor,
        &policy.nvread_path,
        &policy.expected_nvread_blake3,
    )?;
    ...
    let read_run = executor.execute(
        ...
        &[
            ...
            policy.tcti.clone(),
            "-C".into(),
            policy.read_hierarchy.command_value(policy.nv_index),
            "-s".into(),
            COUNTER_BYTES.to_string(),
            handle,
        ],
    )?;
    clean_success(&read_run)?;
    if read_run.stdout.len() != COUNTER_BYTES {
        return Err(Tpm2AdapterError::UnexpectedCounterReadLength(
            read_run.stdout.len(),
        ));
    }
    let mut bytes = [0_u8; COUNTER_BYTES];
    bytes.copy_from_slice(&read_run.stdout);
    // TPM integer wire values are marshalled most-significant-octet first.
    ...
    observation
        ...
        .ok_or(Tpm2AdapterError::InvalidObservation)
}
...
pub fn bind_observation_to_checkpoint(
    ...
    if !observation.validate(policy) {
        return Err(Tpm2AdapterError::InvalidObservation);
    }
    ...
    checkpoint
        ...
        .ok_or(Tpm2AdapterError::CheckpointValidationFailed)
}
...
fn clean_success(run: &ToolExecution) -> Result<(), Tpm2AdapterError> {
    if run.exit_code != Some(0) {
        return Err(Tpm2AdapterError::ToolFailed);
    }
    if !run.stderr.is_empty() {
        return Err(Tpm2AdapterError::ToolEmittedStderr);
    }
    Ok(())
}
