...
pub fn frozen_witness_protocol(frozen_at_unix_ms: i64) -> Result<FrozenProtocol> {
    ...
    .with_success_criterion(
        "A finite canonical Futures Laboratory score is produced when the baseline emits a distribution.",
    )
    .map_err(|error| WitnessError::Protocol(error.to_string()))?;
    ...
    .map_err(|error| WitnessError::Protocol(error.to_string()))?
    .with_success_criterion(
        "A finite canonical Futures Laboratory score is produced when minimum history is satisfied; otherwise NotComputed must retain the typed abstention.",
    )
    .map_err(|error| WitnessError::Protocol(error.to_string()))?;
    ...
    let outcome = match report.brier_score {
        ...
        None => MetricOutcome::NotComputed {
            reason: match report.abstention_reason() {
                Some(reason) => format!(
                    "forecaster abstained: {}",
                    abstention_reason_label(reason)
                ),
...
fn output_kind_label(output: &ForecastOutput) -> String {
    match output {
        ...
        ForecastOutput::Abstain(reason) => {
            format!("abstain:{}", abstention_reason_label(*reason))
        }
    }
}
