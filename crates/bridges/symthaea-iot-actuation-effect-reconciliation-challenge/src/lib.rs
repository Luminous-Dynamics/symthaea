...
pub const EFFECT_RECONCILIATION_CHALLENGE_WIRE_MAGIC: &[u8] =
    b"SYMTHAEA-IOT-RECON-CHALLENGE-V1\0";
...
///
/// Fields are private and production construction requires a live rollback-protected journal
/// wrapper, OS entropy and the guard-local wall clock. The only wire representation exported by
/// this crate is [`Self::canonical_bytes`]; there is deliberately no serde/bincode surface that
/// could create a second signed representation.
...
impl EffectReconciliationChallengeV1 {
    ...
    /// Domain-separated commitment to the exact canonical wire bytes.
    ///
    /// This deliberately hashes the exported wire representation rather than independently hashing
    /// fields a second time. A device-class verifier can therefore bind its signature to either the
    /// canonical bytes or this digest without risking representation drift.
    ...
    /// Explicit cross-language wire encoding for the reconciliation challenge.
    ///
    /// Layout, in order:
    ///
    /// ```text
    ...
    /// nonce:[u8;32]
    /// journal_generation:u64 BE
    /// journal_digest:[u8;32]
    /// correlation_digest:[u8;32]
    /// command_digest:[u8;32]
    /// envelope_digest:[u8;32]
    /// composition_digest:[u8;32]
    /// device:u32-length + UTF-8
    /// operation:u32-length + UTF-8
    /// executor:u32-length + UTF-8
    /// sequence:u64 BE
    /// adapter_id:u32-length + UTF-8
    /// source_state:u8 (0 Prepared, 1 AdapterAcknowledged, 2 AdapterIndeterminate)
    ///   if state=1: adapter_evidence_digest:[u8;32]
    /// attempt_common_fenced_at_unix_ms:u64 BE
    /// attempt_wall_valid_until_unix_ms:u64 BE
    /// issued_at_unix_ms:u64 BE
    /// expires_at_unix_ms:u64 BE
    /// ```
    pub fn canonical_bytes(
        ...
        push_u64(&mut out, self.journal_generation);
        push_digest(&mut out, self.journal_digest);
        push_digest(&mut out, self.correlation_digest);
        push_digest(&mut out, self.command_digest);
        push_digest(&mut out, self.envelope_digest);
        push_digest(&mut out, self.composition_digest);
        ...
        push_string(&mut out, &self.adapter_id)?;
        match self.source_state {
            ReconciliationSourceStateV1::Prepared => out.push(0),
            ReconciliationSourceStateV1::AdapterAcknowledged {
                adapter_evidence_digest,
            } => {
                out.push(1);
                push_digest(&mut out, adapter_evidence_digest);
            }
            ReconciliationSourceStateV1::AdapterIndeterminate => out.push(2),
        }
        push_u64(&mut out, self.attempt_common_fenced_at_unix_ms);
        push_u64(&mut out, self.attempt_wall_valid_until_unix_ms);
        push_u64(&mut out, self.issued_at_unix_ms);
...
fn push_string(
    out: &mut Vec<u8>,
    value: &str,
) -> Result<(), EffectReconciliationChallengeValidationError> {
    let len = u32::try_from(value.len())
        .map_err(|_| EffectReconciliationChallengeValidationError::EncodingLengthOverflow)?;
...
pub enum EffectReconciliationChallengeValidationError {
    ...
    #[error("effect-reconciliation challenge canonical encoding length overflow")]
