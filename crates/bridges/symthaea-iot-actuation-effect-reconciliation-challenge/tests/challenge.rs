...
use symthaea_iot_actuation_effect_reconciliation_challenge::{
    EFFECT_RECONCILIATION_CHALLENGE_WIRE_MAGIC, EffectReconciliationChallengeIssueError,
    ReconciliationSourceStateV1, issue_effect_reconciliation_challenge,
};
...
fn prepared_attempt_issues_fresh_nonce_bound_challenges_for_exact_protected_head() {
    ...
    assert_eq!(first.operation().0.as_str(), "qualification.effect");
    assert_eq!(first.executor().0.as_str(), "qualification:executor");
    ...
    let first_wire = first.canonical_bytes().unwrap();
    let first_wire_again = first.canonical_bytes().unwrap();
    let second_wire = second.canonical_bytes().unwrap();
    assert!(first_wire.starts_with(EFFECT_RECONCILIATION_CHALLENGE_WIRE_MAGIC));
    assert_eq!(first_wire, first_wire_again);
...
fn acknowledged_attempt_challenge_retains_exact_adapter_evidence_without_claiming_realization() {
    ...
    assert!(challenge
        ...
        .unwrap()
        .windows(32)
        .any(|window| window.iter().all(|byte| *byte == 0xE7)));
