...
//! current authority context, and complete relevant negative-authority fact set.
...
/// not contain the epoch, context, negative facts, or source frontier witnesses
...
impl VerifiedAuthorityStateV2 {
    ...
    pub fn authority_context(&self) -> &AuthorityContextRef {
        &self.authority_context
    }
    ...
    pub fn evaluation_input(
        ...
        Ok(AuthorityEvaluationInput {
            ...
            current_authority_context: self.authority_context.clone(),
            use_state,
        })
    }
}
...
pub fn snapshot_digest_v2(
    ...
    validate_context(authority_context)?;
    ...
    t.string(&authority_context.namespace.0)?;
    t.fixed(&authority_context.digest.0);
    t.u32(canonical.len())?;
...
fn validate_statement_shape(statement: &AuthorityStateStatementV2) -> Result<(), AuthorityStateError> {
    ...
    validate_context(&statement.authority_context)
}
...
pub enum AuthorityStateError {
    ...
    #[error("fresh authority-state witnesses disagree on frontier, sequence, epoch, context, or negatives")]
    StateDisagreement,
