...
fn snapshot_identity_changes_with_current_context() {
    ...
    let a = snapshot_digest_v2(
        ...
        1,
        AuthorityEpoch(7),
        &AuthorityContextRef::new("swarm-controller", digest(2)),
        &[],
    )
    .unwrap();
    let b = snapshot_digest_v2(
        ...
        1,
        AuthorityEpoch(7),
        &AuthorityContextRef::new("swarm-controller", digest(3)),
        &[],
    )
    .unwrap();
    assert_ne!(a, b);
...
fn matching_verified_state_feeds_pure_evaluator() {
    ...
    let statements = vec![
        signed_statement(&grant, wire, &key1, [3; 16], context.clone(), vec![]),
        signed_statement(&grant, wire, &key2, [4; 16], context, vec![]),
    ];
...
fn verified_context_rotation_denies_old_grant() {
    ...
    let statements = vec![
        signed_statement(&grant, wire, &key1, [3; 16], rotated.clone(), vec![]),
        signed_statement(&grant, wire, &key2, [4; 16], rotated, vec![]),
    ];
...
#[test]
fn witnesses_must_agree_on_current_context() {
    let grant = root_grant();
    let time = verified_time(grant.digest().0);
    let (policy, key1, key2) = state_policy();
    let challenge = PendingAuthorityStateChallengeV2::new(&policy, &grant, &time).unwrap();
    ...
    let statements = vec![
        ...
        signed_statement(
            ...
            &key2,
            [4; 16],
            AuthorityContextRef::new("swarm-controller", digest(10)),
            vec![],
        ),
    ];
