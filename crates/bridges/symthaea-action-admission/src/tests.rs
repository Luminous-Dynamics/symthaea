...
use symthaea_action_frontier::{
    establish_grant_frontier_v2, CheckpointCasStoreV2, FrontierV2Error,
};
use symthaea_action_runtime::{AttemptId, EffectIntentId, ReservationState};
...
#[derive(Default)]
struct DriftAfterReservationCasStore {
    state: Option<CheckpointHeadV2>,
    cas_count: u8,
    drift_next_read: bool,
}

impl CheckpointCasStoreV2 for DriftAfterReservationCasStore {
    type Error = CasConflict;

    fn current_head(&mut self) -> Result<Option<CheckpointHeadV2>, Self::Error> {
        if self.drift_next_read {
            self.drift_next_read = false;
            let current = self.state.ok_or(CasConflict)?;
    ...
    fn compare_and_swap(
        ...
        self.cas_count = self.cas_count.saturating_add(1);
        if self.cas_count == 2 {
            self.drift_next_read = true;
        }
        Ok(next)
    }
}
...
fn absent_current_authority_context_cannot_allocate_or_persist_a_use() {
    ...
    let (_, mut frontier) =
        establish_grant_frontier_v2(&grant, &account, SharedCasStore::default()).unwrap();
    let genesis_head = frontier.expected_head();
    ...
    assert_eq!(frontier.expected_head(), genesis_head);
}
...
#[test]
fn frontier_grant_substitution_fails_before_account_mutation() {
    let grant = root_grant();
    let time = verified_time(grant.digest().0);
    let state = verified_state(
        &grant,
    ...
    assert!(matches!(
        error,
        AdmissionFrontierV2Error::Admission(AdmissionV2Error::FrontierGrantMismatch)
    ));
    assert_eq!(account.authority_use_state().unwrap(), GrantUseState::default());
}

#[test]
fn post_cas_frontier_drift_returns_no_authorized_token_and_contains_writer() {
    let grant = root_grant();
    let time = verified_time(grant.digest().0);
    let state = verified_state(
        &grant,
    ...
    let (_, mut frontier) = establish_grant_frontier_v2(
        &grant,
        &account,
        DriftAfterReservationCasStore::default(),
    )
    .unwrap();
    let genesis_head = frontier.expected_head();
    ...
    assert!(matches!(
        error,
        AdmissionFrontierV2Error::Frontier(FrontierV2Error::StoreFrontierChanged)
    ));
