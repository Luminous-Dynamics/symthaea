...
//! and the durable action frontier. It deliberately does not execute effects.
//!
//! ```text
//! CapabilityGrant v2
//! + VerifiedAuthorityStateV2
//! + VerifiedAuthorityTime
//! + exact current GrantAccountV2 / CasFrontierV2
...
/// account from the trusted frontier before retrying.
    ...
    S: CheckpointCasStoreV2,
{
    ...
    let successor = GrantAccountCheckpointV2::successor(frontier.current_checkpoint(), grant, account)?;
    let persisted = frontier.persist_new_reservation(grant, successor)?;
    ...
    S: CheckpointCasStoreV2,
{
    ...
    let armed = frontier.arm_outcome_unknown(grant, persisted, successor)?;
...
where
    S: CheckpointCasStoreV2,
{
    if account.snapshot() != frontier.current_checkpoint().snapshot().clone() {
...
pub enum AdmissionV2Error {
    ...
    #[error("in-memory grant account does not equal the exact persisted frontier snapshot")]
