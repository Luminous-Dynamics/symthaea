...
- bit-exact equality between every ordered record's `experimental_gap_ev` and the corresponding value in the dataset truth map;
- exact full truth-keyset equality with the plan's complete source candidate universe.

That ordered-record/truth-map equality check is important because `MatbenchGapDataset` is a public struct: truth restriction does not assume the value necessarily came from the official parser merely because its type name says Matbench.
...
The second replay is the authority for which frozen rankings were measured: it re-evaluates the exact runs from the comparison freeze rather than trusting nested measurement fields in isolation.
