...
use symthaea_sim_worker_containment::install_worker_containment;

fn main() {
    if let Err(error) = install_worker_containment() {
        eprintln!("failed to install worker containment: {error}");
        std::process::exit(77);
    }
