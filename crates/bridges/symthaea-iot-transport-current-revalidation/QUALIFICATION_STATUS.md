...
- the exact signed `(attestor_id,key_id)` is resolved only after fixed current verification, and the selected current key-record digest is retained;
- the revalidated proof's exclusive validity deadline is the minimum of receipt expiry, selected key expiry and current trust-snapshot expiry;
- natural key expiry and natural trust-snapshot expiry are independently covered by regressions even when the trust head itself has not changed;
- evidence accepted historically through a permissive signature provider cannot survive the fixed current verifier;
