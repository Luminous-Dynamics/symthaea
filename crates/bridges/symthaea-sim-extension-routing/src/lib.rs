...
use std::fmt::Debug;
use symthaea_extension_admission::AdmissionCurrentnessSource;
...
impl LazySimulationRegistry {
    ...
    /// Verify process-local authority and live currentness, route, instantiate
    /// only the winner, then delegate result/provenance validation to the
    /// existing `SimulationRegistry` contract.
    ///
    /// The entire scoped admission set must belong to the authority used to
    /// construct this registry and every member must remain current at this exact
    /// use site. No backend factory is invoked before those checks succeed.
    pub fn run(
        &self,
        ...
        let admissions = self
            .authority_scope
            .recheck_set(admissions, currentness)
