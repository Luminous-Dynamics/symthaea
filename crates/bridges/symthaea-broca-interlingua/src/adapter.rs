...
const DEFAULT_GROUNDING_ID: &str = "internal:structured-thought-redacted-export/v1";

#[derive(Clone, Debug, PartialEq, Eq)]
pub struct StructuredThoughtScipPolicy {
    /// Privacy default: raw user input is not exported and therefore does not
    /// affect the outbound semantic hash.
    ...
    /// Bound the amount of working-memory material exposed to a peer.
...
/// Trusted controls compiled exclusively from typed internal state.
...
impl BrocaRendererPolicy {
    ...
    /// The output contains no user-controlled strings.
...
impl StructuredThoughtScipAdapter {
    pub fn graph(
        ...
        let grounding_id = policy
            .grounding_id
            .as_deref()
            .unwrap_or(DEFAULT_GROUNDING_ID);
        if grounding_id.trim().is_empty() {
            return Err(BrocaScipError::InvalidPolicy(
        ...
        let mut nodes = vec![ConceptNode {
            ...
            grounded_by: vec![grounding_id.into()],
            confidence,
        }];
    ...
    pub fn compile_for_text_peer(
        ...
        let envelope = CognitiveEnvelope::from_graph(graph, thought_confidence(thought)?, provenance)?;
        let mut fallback =
...
mod tests {
    ...
    #[test]
    fn code_bearing_thought_stays_on_existing_code_path() {
        let mut thought = thought();
        ...
        assert_eq!(
            StructuredThoughtScipAdapter::graph(
                &thought,
                &StructuredThoughtScipPolicy::default()
            ),
