...
#[cfg(not(unix))]
compile_error!("symthaea-process-file-io-launcher currently requires Unix");
...
fn main() {
    ...
    let mut input = File::open(&stdin_path)
    ...
    let output = OpenOptions::new()
        ...
        .create_new(true)
        .open(&stdout_path)
        .unwrap_or_else(|error| fail(73, &format!("failed to create stdout file: {error}")));
...
fn fail(code: i32, message: &str) -> ! {
    eprintln!("symthaea-process-file-io-launcher: {message}");
