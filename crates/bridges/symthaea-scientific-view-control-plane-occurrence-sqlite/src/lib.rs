        ...
        match self.resolve_operation(&self.binding, proposed.operation_id())? {
            ControlPlaneStoreOperationResolutionV1::Found(record)
                if record.occurrence() == proposed
                    && record.store_reference() == store_reference =>
            {
    ...
    match read_metadata(connection)? {
        ...
        None => {
            ...
            let frontier_count: i64 =
                connection.query_row("SELECT COUNT(*) FROM control_plane_frontier", [], |row| {
                    row.get(0)
                })?;
            if occurrence_count != 0 || frontier_count != 0 {
                return Err(
...
pub enum SqliteControlPlaneOccurrenceStoreError {
    ...
    Occurrence(
        #[from] symthaea_scientific_view_control_plane_occurrence::ControlPlaneOccurrenceError,
    ),
    ...
    #[error(
        "SQLite metadata is absent but durable store state already exists: occurrences={occurrence_count}, frontier_rows={frontier_count}"
    )]
