...
fn genesis_candidate() -> CandidateScientificViewControlPlaneTransitionV1 {
    ...
    let binding = ScientificViewDeploymentBindingV1::new(
        ...
        &profile,
        vec![
            AuthoritySourceBindingV1::new(Role::ResearchSemanticHead, "research/store-a", 7, c(21))
                .unwrap(),
...
fn atomically_new_path_is_the_only_first_bootstrap_case() {
    ...
    let store =
        SqliteControlPlaneOccurrenceStoreV1::open(&path, occurrence_binding(), c(81)).unwrap();
    drop(store);
    ...
    let frontier_count: i64 = connection
        .query_row("SELECT COUNT(*) FROM control_plane_frontier", [], |row| {
            row.get(0)
        })
        .unwrap();
    assert_eq!(journal_mode.to_ascii_lowercase(), "wal");
    assert_eq!(
        (metadata_count, occurrence_count, frontier_count),
        (1, 0, 0)
    );
