...
use symthaea_scientific_view_control_plane_occurrence::{
    attempt_observed_control_plane_commit, ControlPlaneCommitPlanV1,
    ...
    ObservedControlPlaneCommitOutcomeV1,
};
...
fn genesis_successor_and_old_operation_survive_reopen() {
    ...
    let plan2 = ControlPlaneCommitPlanV1::prepare(binding.clone(), &c2, Some(&occurrence1)).unwrap();
    ...
    let reopened =
        SqliteControlPlaneOccurrenceStoreV1::open(&path, binding.clone(), c(81)).unwrap();
    assert_eq!(
        reopened.load_frontier(&binding).unwrap().unwrap().occurrence(),
        &occurrence2
    );
    assert!(matches!(
...
fn operation_absence_and_frontier_come_from_one_validated_snapshot() {
    ...
    let plan2 =
        ControlPlaneCommitPlanV1::prepare(binding.clone(), &uncommitted, Some(&occurrence1)).unwrap();
...
fn copied_database_cannot_be_reopened_under_another_store_epoch() {
    ...
    let store = SqliteControlPlaneOccurrenceStoreV1::open(&path, occurrence_binding(7), c(84))
        .unwrap();
    drop(store);
