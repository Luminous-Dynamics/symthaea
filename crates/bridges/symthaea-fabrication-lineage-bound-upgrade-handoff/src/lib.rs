...
//! Lineage-bound upgrade handoff authority rooted in the globally current finalized predecessor.
//!
//! The underlying hardened handoff crate correctly validates endpoint transitions, policy authority,
//! threshold governance and trusted interval time, but its portable plan still accepts a caller-
//! supplied predecessor endpoint, rollback target and evidence checkpoint. This bridge removes those
//! degrees of freedom from the hardened next-upgrade path.

#![deny(unsafe_code)]

use serde::Serialize;
use symthaea_fabrication_global_upgrade_head::{
    FinalizedUpgradePredecessorRootIdV1, FinalizedUpgradePredecessorRootV1,
};
...
/// Prepared next-upgrade handoff whose predecessor provenance is already frozen to one globally
/// current finalized-upgrade root. The underlying threshold ceremony still signs the exact ordinary
/// handoff prepared payload; this wrapper proves that payload could only be prepared from this root
/// in the exact same authenticated governance view and clock interval.
#[derive(Debug, Clone)]
#[must_use]
pub struct PreparedLineageBoundUpgradeHandoffV1 {
    id: PreparedLineageBoundUpgradeHandoffIdV1,
    predecessor_root_id: FinalizedUpgradePredecessorRootIdV1,
...
impl PreparedLineageBoundUpgradeHandoffV1 {
    pub fn id(&self) -> PreparedLineageBoundUpgradeHandoffIdV1 {
        self.id
    }
    pub fn predecessor_root_id(&self) -> FinalizedUpgradePredecessorRootIdV1 {
        self.predecessor_root_id
    }
    pub fn inner_prepared_id(&self) -> PreparedClockGovernedUpgradeHandoffIdV1 {
        self.inner_prepared.id()
    }
    pub fn signing_payload_digest(&self) -> Sha256Digest {
        self.inner_prepared.signing_payload_digest()
    }
    pub fn plan(&self) -> &ClockGovernedUpgradeHandoffPlanV1 {
        self.inner_prepared.plan()
    }
    pub fn plan_digest(&self) -> Sha256Digest {
        self.inner_prepared.plan_digest()
    }
...
/// Opaque authorized handoff that preserves the exact ordinary handoff authorization while adding
/// non-forgeable provenance to the globally current finalized predecessor root.
#[derive(Debug, Clone)]
#[must_use]
...
impl LineageBoundClockGovernedUpgradeHandoffV1 {
    pub fn id(&self) -> LineageBoundClockGovernedUpgradeHandoffIdV1 {
        self.id
    }
    pub fn prepared_id(&self) -> PreparedLineageBoundUpgradeHandoffIdV1 {
        self.prepared_id
    }
    pub fn predecessor_root_id(&self) -> FinalizedUpgradePredecessorRootIdV1 {
        self.predecessor_root_id
    ...
    pub fn inner_handoff_id(&self) -> ClockGovernedUpgradeHandoffIdV1 {
        self.inner_handoff.id()
    }
    pub fn plan(&self) -> &ClockGovernedUpgradeHandoffPlanV1 {
        self.inner_handoff.plan()
    }
    pub fn plan_digest(&self) -> Sha256Digest {
        self.inner_handoff.plan_digest()
    }
    pub fn current_head_id(&self) -> CurrentFinalizedUpgradeHeadIdV1 {
        self.current_head_id
    }
...
/// Build the next portable handoff plan with predecessor-critical fields derived from the opaque
/// globally-current finalized predecessor root rather than accepted from the caller.
#[allow(clippy::too_many_arguments)]
pub fn build_lineage_bound_upgrade_handoff_plan_v1(
    predecessor_root: &FinalizedUpgradePredecessorRootV1,
...
/// Prepare the exact handoff in the same authenticated governance view and trusted clock interval
/// that established the predecessor root as globally current.
#[allow(clippy::too_many_arguments)]
pub fn prepare_lineage_bound_upgrade_handoff_v1(
    ...
    if plan.predecessor != *predecessor_root.endpoint() {
        violations.push(LineageBoundUpgradeHandoffError::PredecessorMismatch);
    }
    if plan.rollback_target_digest != predecessor_root.rollback_target_digest() {
        violations.push(LineageBoundUpgradeHandoffError::RollbackTargetMismatch);
    }
    if plan.evidence_checkpoint_digest != predecessor_root.evidence_checkpoint_digest() {
        violations.push(LineageBoundUpgradeHandoffError::EvidenceCheckpointMismatch);
    }
    ...
    if predecessor_root.current_head_id() != current_head.id()
        ...
        || predecessor_root.transparency_log_digest()
            != current_head.current_transparency_log_digest()
    ...
    if let Err(error) = trust_snapshot.validate() {
        violations.push(LineageBoundUpgradeHandoffError::TrustSnapshotInvalid(format!(
            "{error:?}"
        )));
    }
    ...
    if trust_snapshot_digest != registry_head.trust_snapshot_digest() {
        violations.push(LineageBoundUpgradeHandoffError::TrustSnapshotMismatch);
    }

    if let Err(error) = containment_state.validate() {
        violations.push(LineageBoundUpgradeHandoffError::ContainmentStateInvalid(format!(
            "{error:?}"
        )));
    }
    ...
    if containment_state_digest != governance_view.containment_state_digest() {
        violations.push(LineageBoundUpgradeHandoffError::ContainmentStateMismatch);
    }
    if compromise_tracker_digest != governance_view.compromise_tracker_digest() {
        violations.push(LineageBoundUpgradeHandoffError::CompromiseTrackerMismatch);
    }
    if containment_state.generation != governance_view.containment_generation() {
        violations.push(LineageBoundUpgradeHandoffError::ContainmentGenerationMismatch);
    }
    ...
    if operational_basis.id() != predecessor_root.operational_basis_id()
        ...
        || operational_basis.id() != current_head.current_operational_basis_id()
    {
    ...
    let clock = match derive_clock_governance_evaluation_envelope_v1(operational_basis) {
        Ok(value) => value,
        Err(error) => {
            violations.push(LineageBoundUpgradeHandoffError::Clock(error));
            return Err(violations);
        }
    };
    if clock.id() != predecessor_root.clock_envelope_id()
        || clock.id() != governance_view.observation_clock_envelope_id()
        || clock.id() != current_head.current_clock_envelope_id()
    {
    ...
    let commitment = PreparedCommitment {
        schema: PREPARED_LINEAGE_BOUND_UPGRADE_HANDOFF_SCHEMA,
        predecessor_root_id: predecessor_root.id().to_hex(),
    ...
    let id = PreparedLineageBoundUpgradeHandoffIdV1(
        hash_serializable(PREPARED_DOMAIN, &commitment).map_err(|error| vec![error])?,
    );

    Ok(PreparedLineageBoundUpgradeHandoffV1 {
        id,
        predecessor_root_id: predecessor_root.id(),
        inner_prepared,
...
/// Authorize the lineage-bound handoff using the same interval-qualified threshold ceremony that
/// authorizes the underlying exact handoff payload, then wrap that authority with predecessor-root
/// provenance. The inner handoff is intentionally not exposed as a public object getter.
pub fn authorize_lineage_bound_upgrade_handoff_v1(
    ...
    let inner_handoff = authorize_clock_governed_upgrade_handoff_v1(prepared.inner_prepared, ceremony)
        .map_err(|error| LineageBoundUpgradeHandoffError::InnerAuthorization(format!(
            "{error:?}"
        )))?;

    let commitment = AuthorizedCommitment {
        schema: LINEAGE_BOUND_UPGRADE_HANDOFF_SCHEMA,
        prepared_id: prepared_id.to_hex(),
        predecessor_root_id: predecessor_root_id.to_hex(),
        inner_handoff_id: inner_handoff.id().to_hex(),
        plan_digest: inner_handoff.plan_digest().to_hex(),
        current_head_id: current_head_id.to_hex(),
        governance_view_id: governance_view_id.to_hex(),
        registry_head_id: registry_head_id.to_hex(),
        predecessor_endpoint_digest: predecessor_endpoint_digest.to_hex(),
        predecessor_finalization_sequence,
        predecessor_checkpoint_digest: predecessor_checkpoint_digest.to_hex(),
    ...
    let id = LineageBoundClockGovernedUpgradeHandoffIdV1(hash_serializable(
        AUTHORIZED_DOMAIN,
        &commitment,
    )?);
...
fn hash_serializable<T: Serialize + ?Sized>(
    domain: &[u8],
    value: &T,
) -> Result<Sha256Digest, LineageBoundUpgradeHandoffError> {
    let bytes = serde_json::to_vec(value)
        .map_err(|error| LineageBoundUpgradeHandoffError::Encoding(error.to_string()))?;
