...
use symthaea_sim_worker_filesystem::install_worker_filesystem_containment;

fn main() {
    if let Err(error) = install_worker_filesystem_containment() {
        eprintln!("failed to install worker filesystem containment: {error}");
        std::process::exit(78);
    }
