...
fn main() {
    ...
    if d_ok && n_ok && v_ok {
        println!("PASS (d_model={}, n_layer={}, vocab={})", wrapper.d_model(), wrapper.n_layer(), wrapper.vocab_size());
        passed += 1;
    } else {
        println!("FAIL (d_model={}, n_layer={}, vocab={})", wrapper.d_model(), wrapper.n_layer(), wrapper.vocab_size());
        failed += 1;
    }
    ...
    let same = logits_pre.iter().zip(logits_post.iter())
    ...
    let differs = logits_pre.iter().zip(logits_diff.iter())
    ...
    match generate_tokens(&mut wrapper, "The meaning of life is", 10) {
        Ok((ids, text)) => {
            ...
            if ids.len() == 10 && all_ascii {
                println!("PASS ({:.0}ms, {:?})", start.elapsed().as_secs_f32() * 1000.0, text.trim());
                passed += 1;
            } else {
...
fn generate_tokens(wrapper: &mut MambaWrapper, prompt: &str, count: usize) -> Result<(Vec<u32>, String), String> {
    let prompt_ids = wrapper.encode(prompt).map_err(|e| format!("encode: {e}"))?;

    let mut last_logits = vec![];
    for &token_id in &prompt_ids {
        last_logits = wrapper.forward_one_token(token_id).map_err(|e| format!("forward: {e}"))?;
    }
    ...
    for _ in 0..count {
        ...
        last_logits = wrapper.forward_one_token(next_id).map_err(|e| format!("forward: {e}"))?;
    }

    let text = wrapper.decode(&generated).map_err(|e| format!("decode: {e}"))?;
    Ok((generated, text))
}
...
fn run_e2e_pipeline(wrapper: &mut MambaWrapper) -> Result<String, String> {
    ...
    // 5. Generate text
    let (_, text) = generate_tokens(wrapper, "", 5)
        .or_else(|_| {
            // If empty prompt fails, try with BOS
            let eos_id = wrapper.eos_token_id();
            let mut logits = wrapper.forward_one_token(eos_id)
            ...
            for _ in 0..5 {
                let next = logits.iter()
                    ...
                    .unwrap_or(0);
                ids.push(next);
                logits = wrapper.forward_one_token(next)
                    .map_err(|e| format!("forward: {e}"))?;
            }
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--help" | "-h" => {
                ...
                eprintln!();
                eprintln!("Options:");
                eprintln!("  --model, -m MODEL   HuggingFace model ID (default: state-spaces/mamba-130m)");
                process::exit(0);
            }
            _ => {}
        }
