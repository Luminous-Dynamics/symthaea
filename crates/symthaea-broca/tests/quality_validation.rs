...
use symthaea_broca::gating::{
    time_pressure_adjusted_max_tokens, CANONICAL_HEDGING_WORDS, CANONICAL_INFORMAL_WORDS,
};
...
// ===========================================================================
// 16. New Channel (v3) Integration: time_pressure reduces generation length
// ===========================================================================

#[test]
fn test_time_pressure_reduces_generation_length() {
    let genesis = quality_genesis();
    let mut config = quality_config();
    config.gating.base_max_tokens = 64;
    // Use the default time_pressure_max_reduction (0.5 = 50% reduction at max pressure)

    // Generate with relaxed time pressure (0.0)
    let mut gen_relaxed = BrocaGenerator::new(&genesis, config.clone());
    let mut ch_relaxed = ThoughtChannels::with_intent(1);
    ch_relaxed.set_context(0.0, 0.5, 0.5, 0.5); // time_pressure=0 (relaxed)
    let result_relaxed = gen_relaxed.generate(&ch_relaxed);

    // Generate with urgent time pressure (1.0)
    let mut gen_urgent = BrocaGenerator::new(&genesis, config.clone());
    let mut ch_urgent = ThoughtChannels::with_intent(1);
    ch_urgent.set_context(1.0, 0.5, 0.5, 0.5); // time_pressure=1 (urgent)
    let result_urgent = gen_urgent.generate(&ch_urgent);

    // Verify the max_tokens ceiling is actually lower under time pressure.
    // With base=64 and max_reduction=0.5, time_pressure=1.0 gives max_tokens=32.
    let max_relaxed = time_pressure_adjusted_max_tokens(
        64,
        0.0,
        config.gating.time_pressure_max_reduction,
    );
    let max_urgent = time_pressure_adjusted_max_tokens(
        64,
        1.0,
        config.gating.time_pressure_max_reduction,
    );
    assert!(
        max_urgent < max_relaxed,
        "Urgent max tokens ({max_urgent}) should be less than relaxed ({max_relaxed})"
    );

    // The urgent generation should not exceed the reduced ceiling.
    assert!(
        result_urgent.num_tokens <= max_urgent,
        "Urgent generation ({} tokens) should respect reduced ceiling ({max_urgent})",
        result_urgent.num_tokens,
    );

    // The relaxed generation can use up to the full ceiling.
    assert!(
        result_relaxed.num_tokens <= max_relaxed,
        "Relaxed generation ({} tokens) should respect full ceiling ({max_relaxed})",
        result_relaxed.num_tokens,
    );

    eprintln!(
        "Time pressure: relaxed={} tokens (max {}), urgent={} tokens (max {})",
        result_relaxed.num_tokens, max_relaxed, result_urgent.num_tokens, max_urgent,
    );
}

// ===========================================================================
// 17. New Channel (v3) Integration: domain_familiarity reduces hedging
// ===========================================================================

#[test]
fn test_domain_familiarity_reduces_hedging() {
    // This test validates at the gating level that domain_familiarity reduces
    // hedging boost magnitudes. The generator's `apply()` defaults to
    // familiarity=0.0, so we test `apply_with_familiarity()` directly.
    ...
    let gate = symthaea_broca::gating::EpistemicGate::new(&tokenizer, &gating_config);

    // Resolve hedging token IDs from the tokenizer
    let hedging_ids: Vec<u32> = CANONICAL_HEDGING_WORDS
        .iter()
        .filter_map(|w| {
    ...
    // Skip test if no hedging words resolve (tokenizer may not have them)
    if hedging_ids.is_empty() {
        eprintln!("No hedging tokens resolved in minimal tokenizer; skipping detail check");
        return;
    }
    ...
    let mut logits_novel = vec![0.0f32; vocab_size];
    gate.apply_with_familiarity(&mut logits_novel, 3.0, 0.0); // Unknown + novel

    // Expert domain (familiarity=1): reduced hedging boost
    let mut logits_expert = vec![0.0f32; vocab_size];
    gate.apply_with_familiarity(&mut logits_expert, 3.0, 1.0); // Unknown + expert

    // Sum hedging logits for novel vs expert
    let sum_novel: f32 = hedging_ids.iter().map(|&id| logits_novel[id as usize]).sum();
    let sum_expert: f32 = hedging_ids.iter().map(|&id| logits_expert[id as usize]).sum();

    assert!(
        sum_novel > sum_expert,
        "Novel domain should have higher hedging boost sum ({sum_novel:.3}) \
         than expert domain ({sum_expert:.3})"
    );

    // Expert domain with scale=1.0 should produce zero hedging boost
    // (familiarity_scale = 1.0 - 1.0 * 1.0 = 0.0)
    assert!(
        sum_expert.abs() < 1e-6,
        "Expert domain with full hedging scale should have ~0 hedging boost, got {sum_expert:.6}"
    );

    eprintln!(
        "Domain familiarity: novel hedging sum={sum_novel:.3}, expert hedging sum={sum_expert:.3}"
    );
}

// ===========================================================================
// 18. New Channel (v3) Integration: social_context affects formality
// ===========================================================================

#[test]
fn test_social_context_affects_formality() {
    // social_context amplifies the informal word penalty when warmth is low.
    // We test that formal context (social_context=1.0) applies a stronger penalty
    // to informal words than intimate context (social_context=0.0).
    let tokenizer = BpeTokenizer::default_minimal();
    let gating_config = GatingConfig {
        low_warmth_threshold: 0.5,
        warmth_penalty_multiplier: -5.0,
        social_context_formality_scale: 1.0,
        ..Default::default()
    };

    let modulator = symthaea_broca::gating::EmotionalModulator::new(&tokenizer, &gating_config);

    // Resolve informal token IDs
    ...
    if informal_ids.is_empty() {
        eprintln!("No informal tokens resolved in minimal tokenizer; skipping detail check");
        return;
    }

    let vocab_size = tokenizer.vocab_size();

    // Low warmth + intimate context (social_context=0.0)
    let mut logits_intimate = vec![0.0f32; vocab_size];
    let mut ch_intimate = ThoughtChannels::with_intent(1);
    ch_intimate.set_emotion(0.0, 0.3, 0.1); // low warmth
    ch_intimate.set_context(0.0, 0.5, 0.0, 0.5); // intimate
    modulator.apply(&mut logits_intimate, &ch_intimate, 15);

    // Low warmth + formal context (social_context=1.0)
    let mut logits_formal = vec![0.0f32; vocab_size];
    let mut ch_formal = ThoughtChannels::with_intent(1);
    ch_formal.set_emotion(0.0, 0.3, 0.1); // same low warmth
    ch_formal.set_context(0.0, 0.5, 1.0, 0.5); // formal
    modulator.apply(&mut logits_formal, &ch_formal, 15);

    // Sum the penalty on informal tokens
    let penalty_intimate: f32 = informal_ids
        .iter()
        .map(|&id| logits_intimate[id as usize])
        .sum();
    let penalty_formal: f32 = informal_ids
        .iter()
        .map(|&id| logits_formal[id as usize])
        .sum();

    // Both should be negative (penalties), but formal should be more negative
    // (stronger suppression of informal language)
    assert!(
        penalty_formal < penalty_intimate,
        "Formal context should apply stronger informal penalty ({penalty_formal:.3}) \
         than intimate ({penalty_intimate:.3})"
    );

    eprintln!(
        "Social context formality: intimate penalty={penalty_intimate:.3}, formal penalty={penalty_formal:.3}"
    );
}

// ===========================================================================
// 19. Veto Loop Prevention Stress Test
// ===========================================================================

#[test]
fn test_veto_loop_prevention_stress() {
    let genesis = quality_genesis();
    let mut config = quality_config();
    // Enable all mechanisms that could cause veto loops
    config.enable_coherence_feedback = true;
    config.enable_semantic_veto = true;
    config.gating.max_vetoes = 1;
    config.gating.veto_refractory = 4;
    config.gating.base_max_tokens = 200; // Large budget to stress the loop
    config.gating.veto_threshold = 0.5; // Aggressive veto threshold to increase chances

    // Generate with 200+ token budget, max_vetoes=1
    let mut gen = BrocaGenerator::new(&genesis, config);
    let channels = ThoughtChannels::with_intent(1); // Answer intent

    // This should terminate. The key assertion is that generate() returns
    // (no infinite loop). If max_vetoes=1, at most 1 veto + hesitation is inserted,
    // then generation continues without further vetoes until max_tokens.
    let result = gen.generate(&channels);

    // Verify termination and reasonable output
    assert!(
        result.num_tokens <= 200,
        "Generation should respect max_tokens=200 ceiling, got {}",
        result.num_tokens,
    );
    assert!(
        result.num_tokens == result.token_ids.len(),
        "num_tokens ({}) should match token_ids.len() ({})",
        result.num_tokens,
        result.token_ids.len(),
    );
    assert!(
        result.final_coherence.is_finite(),
        "Coherence should remain finite under stress: {}",
        result.final_coherence,
    );

    // Count veto markers in the text (each veto inserts "-- wait, " by default)
    let veto_count = result.text.matches("-- wait,").count();
    assert!(
        veto_count <= 1,
        "With max_vetoes=1, at most 1 veto should fire, got {veto_count}"
    );

    eprintln!(
        "Veto stress: {} tokens, veto_triggered={}, veto_count_in_text={}, coherence={:.3}, \
         eos={}, text_len={}",
        result.num_tokens,
        result.veto_triggered,
        veto_count,
