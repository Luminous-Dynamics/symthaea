...
use crate::fep_agent::ActiveInferenceClimeAgent;
use crate::simulator::{ClimePhysicsSimulator, SimpleClimeSimulator};
use crate::types::ClimeConfig;
...
pub struct ClimeTrainer {
    controller: ClimeController,
    encoder: ClimeHdcEncoder,
    fep: ActiveInferenceClimeAgent,
    simulator: SimpleClimeSimulator,
    config: ClimeConfig,
}

impl ClimeTrainer {
    pub fn new(config: ClimeConfig) -> Self {
        let g = GenesisSeed::from_phrase(&config.genesis_phrase);
        Self {
            controller: ClimeController::new(&g, &config),
            encoder: ClimeHdcEncoder::new(&g, 32),
            fep: ActiveInferenceClimeAgent::new(),
...
mod tests {
    ...
    #[test]
    fn test_episode() {
        let mut c = ClimeConfig::default();
        c.steps_per_episode = 200;
        let mut t = ClimeTrainer::new(c);
        let m = t.run_episode();
        assert!(!m.diverged);
