...
use crate::consciousness_scaling::{
    compute_network_scores, effective_dimensionality, network_correlation_matrix, NetworkScores,
    N_THEORIES, THEORY_NAMES,
};
        ...
        Topology::SmallWorld { rewire_prob } => {
            ...
            for i in 0..n {
                ...
                if prob < rewire_prob {
                    seed = seed.wrapping_mul(6364136223846793005).wrapping_add(1);
                    let target =
                        ((seed >> 33) as f64 / (1u64 << 31) as f64 * n as f64) as usize % n;
                    if target != i {
...
pub fn topology_scores(n: usize, topology: Topology, n_configs: usize) -> Vec<NetworkScores> {
    let dt = 0.1;
    let warmup = 2000;
    let measure = 3000;
    ...
    (0..n_configs)
        .map(|c| {
            ...
            let means: Vec<f64> = voltage_traces
                .iter()
                .map(|tr| tr.iter().sum::<f64>() / tr.len() as f64)
                .collect();
            let gm = means.iter().sum::<f64>() / means.len() as f64;
            let complexity =
                means.iter().map(|m| (m - gm).powi(2)).sum::<f64>() / means.len() as f64;
            ...
            let entropy: f64 = bins
                ...
                .filter(|&&b| b > 0)
                .map(|&b| {
                    let p = b as f64 / n as f64;
                    -p * p.ln()
                })
                .sum();
            let max_ent = (n_bins.min(n) as f64).ln();
            let entropy_norm = if max_ent > 0.0 {
                entropy / max_ent
            } else {
                0.0
            };

            let dissipative: f64 = voltage_traces
                .iter()
                .map(|tr| {
            ...
            for chunk in sync_trace.chunks(window) {
                ...
                decoherence +=
                    chunk.iter().map(|s| (s - m).powi(2)).sum::<f64>() / chunk.len() as f64;
                n_w += 1;
            }
