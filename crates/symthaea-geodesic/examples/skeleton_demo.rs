...
"#;
    ...
    println!("\nPDG: {} nodes, {} edges", pdg.nodes.len(), pdg.edges.len());
    ...
    println!("  χ  = {} (Euler characteristic)", fingerprint.euler_characteristic);
    ...
    let result = active_inference_synthesize(
        &betti,
        &["iterate", "accumulate sum"],
        &manifold,
        None,
        5,
    );
    ...
    println!("Skeleton before filling ({} unfilled slots):", skeleton.unfilled_count());
    ...
    fn fill_all(c: &mut SkeletonCombinator, fills: &mut std::collections::VecDeque<&str>) {
        match c {
            ...
            SkeletonCombinator::Iterate { init, condition, body } => {
                if !init.is_filled() { if let Some(f) = fills.pop_front() { init.fill(f); } }
                if !condition.is_filled() { if let Some(f) = fills.pop_front() { condition.fill(f); } }
                fill_all(body, fills);
            }
            SkeletonCombinator::Reduce { operation, initial, collection } => {
                if !operation.is_filled() { if let Some(f) = fills.pop_front() { operation.fill(f); } }
                if !initial.is_filled() { if let Some(f) = fills.pop_front() { initial.fill(f); } }
                if !collection.is_filled() { if let Some(f) = fills.pop_front() { collection.fill(f); } }
            }
            SkeletonCombinator::Leaf(slot) => {
                if !slot.is_filled() { if let Some(f) = fills.pop_front() { slot.fill(f); } }
            }
            _ => {}
        }
    }
    ...
    println!("After filling ({} unfilled slots):", skeleton.unfilled_count());
    ...
    if let Some(code) = skeleton.emit_rust(1) {
        ...
        println!("Could not emit — {} slots still unfilled", skeleton.unfilled_count());
    }
...
fn demo(title: &str, betti: &BettiNumbers, hints: &[&str]) {
    println!("{}", "=".repeat(60));
    println!("=== {} ===", title);
    println!("Target: β₀={}, β₁={}, β₂={}", betti.beta_0, betti.beta_1, betti.beta_2);
    println!("Hints: {:?}\n", hints);
