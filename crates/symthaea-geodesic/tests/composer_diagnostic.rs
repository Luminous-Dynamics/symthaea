...
fn diagnostic_subtree_matching() {
    ...
    let mut results: Vec<(String, f32, f32)> = memory.nearest_k(&target_iterate.encode(), 30)
        .iter()
        .map(|(entry, _)| {
    ...
    for (name, structural, name_based) in results.iter().take(10) {
        ...
        println!("{:<25} {:>10.4} {:>10.4}{}", name, structural, name_based, marker);
    }
    ...
    let name_sim = find_max_iterate.encode().similarity(&target_iterate.encode());
    ...
    println!("  Improvement:                 {:+.4}", structural_sim - name_sim);
    ...
    let loop_a = ProgramNode::iterate(
        ProgramNode::atom("x"), ProgramNode::atom("y"), ProgramNode::atom("z"));
    let loop_b = ProgramNode::iterate(
        ProgramNode::atom("p"), ProgramNode::atom("q"), ProgramNode::atom("r"));
    let slot_sim = encode_structural(&loop_a).similarity(&encode_structural(&loop_b));
    println!("\n  Iterate(x,y,z) vs Iterate(p,q,r) structural: {:.4}", slot_sim);
