...
mod tests {
    ...
    fn deep_model_accuracy_by_region() {
        ...
        eprintln!("\n{} MODEL ACCURACY BY NUCLEAR REGION {}", "=".repeat(20), "=".repeat(20));
        ...
        for entry in &ame {
            ...
            let bin = if a < 40 { &mut light_errors }
                else if a < 120 { &mut medium_errors }
                else if a < 210 { &mut heavy_errors }
                else { &mut actinide_errors };
            bin.push(err);

            // Check if near magic number
            let near_magic = magic_n.iter().any(|&m| (n as i32 - m as i32).unsigned_abs() <= 2)
                || magic_z.iter().any(|&m| (z as i32 - m as i32).unsigned_abs() <= 2);
            if near_magic {
        ...
        for (name, errs) in [
            ...
            eprintln!("{:<20} {:>6} {:>10.3} {:>10.3} {:>10.3}",
                name, errs.len(), mean, rms, max);
        }
    }
    ...
    fn deep_shell_evolution() {
        ...
        eprintln!("\n{} SHELL EVOLUTION FAR FROM STABILITY {}", "=".repeat(20), "=".repeat(20));
        ...
        for z in (30..=60).step_by(2) {
            let s2n_80 = pred.predict(z, 80).binding_energy
                - pred.predict(z, 78).binding_energy;
            let s2n_82 = pred.predict(z, 82).binding_energy
                - pred.predict(z, 80).binding_energy;
            let s2n_84 = pred.predict(z, 84).binding_energy
                - pred.predict(z, 82).binding_energy;
            let gap = s2n_82 - s2n_84;
            eprintln!("{:>4} {:>10.3} {:>10.3} {:>10.3} {:>10.3}", z, s2n_80, s2n_82, s2n_84, gap);
        }
        ...
        for z in (20..=42).step_by(2) {
            let s2n_48 = pred.predict(z, 48).binding_energy
                - pred.predict(z, 46).binding_energy;
            let s2n_50 = pred.predict(z, 50).binding_energy
                - pred.predict(z, 48).binding_energy;
            let s2n_52 = pred.predict(z, 52).binding_energy
                - pred.predict(z, 50).binding_energy;
            let gap = s2n_50 - s2n_52;
            eprintln!("{:>4} {:>10.3} {:>10.3} {:>10.3} {:>10.3}", z, s2n_48, s2n_50, s2n_52, gap);
        }
        ...
        for z in (20..=32).step_by(2) {
            let s2n_38 = pred.predict(z, 38).binding_energy
                - pred.predict(z, 36).binding_energy;
            let s2n_40 = pred.predict(z, 40).binding_energy
                - pred.predict(z, 38).binding_energy;
            let s2n_42 = pred.predict(z, 42).binding_energy
                - pred.predict(z, 40).binding_energy;
            let gap = s2n_40 - s2n_42;
            eprintln!("{:>4} {:>10.3} {:>10.3} {:>10.3} {:>10.3}", z, s2n_38, s2n_40, s2n_42, gap);
        }
    }
    ...
    fn deep_alpha_landscape() {
        ...
        eprintln!("\n{} SUPERHEAVY ALPHA DECAY LANDSCAPE {}", "=".repeat(20), "=".repeat(20));
        ...
        eprintln!("Estimated half-life: {:.2e} s ({:.2e} yr)",
            t_sec, t_sec / 3.156e7);
    }
    ...
    fn deep_terra_incognita() {
        ...
        eprintln!("\n{} TERRA INCOGNITA: UNMEASURED PREDICTIONS {}", "=".repeat(20), "=".repeat(20));

        // Build set of measured nuclei
        use std::collections::HashSet;
        let measured: HashSet<(u16, u16)> = ame.iter()
        ...
        for z in 20..=100u16 {
            for n in 20..=170u16 {
                if measured.contains(&(z, n)) { continue; }
                ...
                if p.uncertainty < 0.5 {
                    ...
                    if sn > 0.0 && ba > 6.0 {
                        unmeasured_predictions.push((z, n, a, p.binding_energy, p.uncertainty, sn, ba));
                    }
                }
            }
        }
        ...
        for &(z, n, a, be, sigma, sn, ba) in unmeasured_predictions.iter().take(count) {
            eprintln!("{:>4} {:>4} {:>4} {:>10.3} {:>8.3} {:>8.3} {:>8.4}",
                z, n, a, be, sigma, sn, ba);
        }

        eprintln!("\nTotal unmeasured predictions with σ < 0.5 MeV: {}", unmeasured_predictions.len());

        // How many are in r-process path? (Z < 50, N > 60)
        let rprocess_relevant: Vec<_> = unmeasured_predictions.iter()
            .filter(|&&(z, n, _, _, _, _, _)| z < 50 && n > 60)
            .collect();
        eprintln!("Of which, r-process relevant (Z<50, N>60): {}", rprocess_relevant.len());
        ...
        if !rprocess_relevant.is_empty() {
            ...
            for &&(z, n, a, be, sigma, sn, _ba) in rprocess_relevant.iter().take(15) {
                eprintln!("  Z={:>3} N={:>3} A={:>3}: BE={:.1} ± {:.2} MeV, S_n={:.2} MeV",
                    z, n, a, be, sigma, sn);
            }
        }
    }
    ...
    fn deep_fission_barriers() {
        ...
        eprintln!("\n{} FISSION BARRIER LANDSCAPE {}", "=".repeat(20), "=".repeat(20));
        ...
        for z in (104..=120).step_by(2) {
            for n in [170, 174, 178, 182, 184, 186, 190] {
                ...
                let survivable = if barrier > 4.0 { "YES" }
                    else if barrier > 1.0 { "marginal" }
                    else { "no" };

                eprintln!("{:>4} {:>4} {:>4} {:>10.2} {:>10.4} {:>10.1} {:>12} (Bf≈{:.1})",
    ...
    fn deep_anomalies() {
        ...
        eprintln!("\n{} BINDING ENERGY ANOMALIES {}", "=".repeat(20), "=".repeat(20));
        ...
        eprintln!("{:>4} {:>4} {:>4} {:>10} {:>10} {:>10} {:>10}",
            "Z", "N", "A", "Pred(MeV)", "Exp(MeV)", "Err(MeV)", "Err/A");
        eprintln!("{}", "-".repeat(56));

        for &(z, n, pred_be, exp_be, err) in residuals.iter().take(20) {
            let a = z + n;
            eprintln!("{:>4} {:>4} {:>4} {:>10.3} {:>10.3} {:>10.3} {:>10.4}",
                z, n, a, pred_be, exp_be, err, err / a as f64);
        }

        // Statistics by shell region
        eprintln!("\n--- Residual Statistics by Region ---");

        let near_doubly_magic: Vec<f64> = residuals.iter()
            .filter(|&&(z, n, _, _, _)| {
                let dm = [(8,8), (20,20), (28,28), (50,50), (82,82), (82,126), (50,82)];
                dm.iter().any(|&(mz, mn)| {
        ...
        let deformed: Vec<f64> = residuals.iter()
            .filter(|&&(z, n, _, _, _)| {
        ...
        let actinide: Vec<f64> = residuals.iter()
        ...
        for (name, errs) in [
            ...
            eprintln!("  {:<25} n={:>4} RMS={:.3} MeV, bias={:+.3} MeV",
                name, errs.len(), rms, bias);
        }
    }
    ...
    fn deep_wigner_energy() {
        ...
        eprintln!("\n{} WIGNER ENERGY (np PAIRING AT N=Z) {}", "=".repeat(20), "=".repeat(20));
        ...
        for z in (10..=50).step_by(2) {
            ...
            let be_nm2 = if n >= 2 { pred.predict(z, n - 2).binding_energy } else { continue };

            // Wigner energy ≈ BE(N=Z) - 0.5 * [BE(N=Z+2) + BE(N=Z-2)]
            let wigner = be_nz - 0.5 * (be_np2 + be_nm2);

            eprintln!("{:>4} {:>4} {:>10.3} {:>10.3} {:>10.3} {:>10.3}",
