...
const ATOMIC_MASS: [f64; 104] = [
    ...
    121.760, 127.60,  126.904, 131.293, 132.905, 137.327, 138.905, 140.116, 140.908, 144.242, // 51-60
    145.0,   150.36,  151.964, 157.25,  158.925, 162.500, 164.930, 167.259, 168.934, 173.045, // 61-70
...
fn d_electrons(z: u8) -> u8 {
    match z {
        21 => 1, 22 => 2, 23 => 3, 24 => 5, 25 => 5, 26 => 6, 27 => 7, 28 => 8, 29 => 10, 30 => 10,
        39 => 1, 40 => 2, 41 => 4, 42 => 5, 43 => 5, 44 => 7, 45 => 8, 46 => 10, 47 => 10, 48 => 10,
        72 => 2, 73 => 3, 74 => 4, 75 => 5, 76 => 6, 77 => 7, 78 => 9, 79 => 10, 80 => 10,
...
mod tests {
    ...
    fn test_periodic_trends() {
        ...
        assert!(li[2] < be[2], "Li EN ({}) should be < Be EN ({})", li[2], be[2]);
        assert!(be[2] < b[2], "Be EN ({}) should be < B EN ({})", be[2], b[2]);
        ...
        // dim 3 = IE / 25.0
        assert!(li[3] < f[3], "Li IE ({}) should be < F IE ({})", li[3], f[3]);

        // Atomic radius should decrease left-to-right (dim 5)
        assert!(li[5] > f[5], "Li radius ({}) should be > F radius ({})", li[5], f[5]);

        eprintln!("=== Periodic Trends (Period 2) ===");
        eprintln!("EN:     Li={:.3} Be={:.3} B={:.3} C={:.3} N={:.3} O={:.3} F={:.3}",
            li[2], be[2], b[2], c[2], n[2], o[2], f[2]);
        eprintln!("IE:     Li={:.3} Be={:.3} B={:.3} C={:.3} N={:.3} O={:.3} F={:.3}",
            li[3], be[3], b[3], c[3], n[3], o[3], f[3]);
        eprintln!("Radius: Li={:.3} Be={:.3} B={:.3} C={:.3} N={:.3} O={:.3} F={:.3}",
