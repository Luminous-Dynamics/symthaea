...
fn test_timestamp_age_calculation() {
    ...
    assert!(age >= 3599 && age <= 3601, "Age {} not within expected range", age);
}
...
fn test_timestamp_future_age_negative() {
    ...
    let age = ts.age_seconds();
    assert!(age < 0, "Future timestamp should have negative age, got {}", age);
}
...
fn test_timestamp_microsecond_precision() {
    ...
    assert!(diff.abs() < 2, "Timestamp lost precision: {} microseconds", diff);
}
...
fn test_unit_vector_normalized_diagonal() {
    ...
    assert!(is_unit_vector(&[1.0/sqrt3, 1.0/sqrt3, 1.0/sqrt3], 0.001));
}
...
fn test_trust_level_weight_bounds() {
    ...
    for level in [
        ...
        assert!(weight >= 0.0 && weight <= 1.0,
                "Weight {} for {:?} out of bounds", weight, level);
    }
}
...
fn test_mycelix_error_messages() {
    let errors = vec![
        ...
        (MycelixError::InvalidTle { reason: "bad checksum".to_string() }, "bad checksum"),
        (MycelixError::TleChecksumMismatch { line: 1, expected: 5, actual: 3 }, "checksum"),
        (MycelixError::EntryNotFound { entry_type: "Bounty".to_string(), hash: "abc123".to_string() }, "Bounty"),
        (MycelixError::Unauthorized { operation: "delete".to_string(), reason: "not owner".to_string() }, "delete"),
        (MycelixError::DuplicateEntry { entry_type: "Object".to_string(), identifier: "25544".to_string() }, "Duplicate"),
    ];

    for (error, expected_substring) in errors {
        let message = error.message();
        assert!(message.contains(expected_substring),
                "Error message '{}' should contain '{}'", message, expected_substring);
    }
}
...
fn test_mycelix_error_codes_are_screaming_snake_case() {
    let errors = vec![
        ...
        MycelixError::DuplicateEntry { entry_type: String::new(), identifier: String::new() },
    ];
...
fn test_alert_priority_ordering() {
    ...
    for i in 0..priorities.len()-1 {
        assert!(priorities[i] < priorities[i+1],
                "{:?} should be less than {:?}", priorities[i], priorities[i+1]);
    }
}
...
fn test_conjunction_alert_is_critical() {
    let make_assessment = |risk_level: RiskLevel| {
    ...
    let negligible_alert = ConjunctionAlert::new_conjunction(&make_assessment(RiskLevel::Negligible));
...
fn test_conjunction_alert_recommendations() {
    ...
    assert!(emergency_alert.recommendation.as_ref().unwrap().contains("IMMEDIATE"));
