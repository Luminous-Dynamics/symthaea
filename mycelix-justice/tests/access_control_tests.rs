...
mod party_access_tests {
    ...
    fn test_complainant_can_access_own_case() {
        ...
        assert!(access.can_view, "Complainant should be able to view own case");
        assert!(access.can_submit_evidence, "Complainant should be able to submit evidence");
        assert!(access.can_update, "Complainant should be able to update own case");
    }
    ...
    fn test_respondent_can_access_case() {
        ...
        assert!(access.can_submit_evidence, "Respondent should be able to submit evidence");
        assert!(!access.can_withdraw, "Respondent should not be able to withdraw case");
    }
    ...
    fn test_third_party_cannot_access_private_case() {
        ...
        assert!(!access.can_view, "Third party should not be able to view private case");
        assert!(!access.can_submit_evidence, "Third party should not be able to submit evidence");
    }
    ...
    fn test_witness_has_limited_access() {
        ...
        assert!(access.can_submit_evidence, "Witness should be able to submit testimony");
        assert!(!access.can_update, "Witness should not be able to update case");
    }
    ...
    fn test_expert_has_specialized_access() {
        ...
        assert!(access.can_submit_evidence, "Expert should be able to submit expert opinion");
        assert!(!access.can_update, "Expert should not be able to update case");
    }
    ...
    fn test_complainant_can_withdraw_case() {
        ...
        let access = check_case_access("case-001", "did:mycelix:alice");
        assert!(access.can_withdraw, "Complainant should be able to withdraw case");
    }
...
mod arbitrator_access_tests {
    ...
    fn test_assigned_arbitrator_can_access_case() {
        ...
        assert!(access.can_view, "Assigned arbitrator should be able to view case");
        assert!(access.can_view_all_evidence, "Arbitrator should be able to view all evidence");
    }
    ...
    fn test_unassigned_arbitrator_cannot_access_case() {
        ...
        assert!(!access.can_view, "Unassigned arbitrator should not be able to view case");
    }
    ...
    fn test_arbitrator_can_render_decision() {
        ...
        let access = check_case_access("case-001", "did:mycelix:arb1");
        assert!(access.can_render_decision, "Arbitrator should be able to render decision");
    }
    ...
    fn test_party_cannot_render_decision() {
        ...
        assert!(!alice_access.can_render_decision, "Complainant should not be able to render decision");
        assert!(!bob_access.can_render_decision, "Respondent should not be able to render decision");
    }
    ...
    fn test_recused_arbitrator_loses_access() {
        ...
        let access = check_case_access("case-001", "did:mycelix:arb1");
        assert!(!access.can_view, "Recused arbitrator should lose access to case");
        assert!(!access.can_render_decision, "Recused arbitrator should not render decisions");
    }
    ...
    fn test_arbitrator_panel_all_have_access() {
        ...
        for arb in &["did:mycelix:arb1", "did:mycelix:arb2", "did:mycelix:arb3"] {
            ...
            assert!(access.can_render_decision, "All panel arbitrators should be able to vote");
        }
    }
}
...
mod evidence_access_tests {
    ...
    fn test_parties_can_view_public_evidence() {
        ...
        assert!(alice_access.can_view, "Complainant should view public evidence");
        assert!(bob_access.can_view, "Respondent should view public evidence");
    }
    ...
    fn test_adjudicators_only_evidence() {
        ...
        create_case(&case);
        assign_arbitrator("case-001", "did:mycelix:arb1");
        create_evidence("evidence-001", "case-001", EvidenceVisibility::AdjudicatorsOnly);

        let arb_access = check_evidence_access("evidence-001", "did:mycelix:arb1");
        let alice_access = check_evidence_access("evidence-001", "did:mycelix:alice");
        let bob_access = check_evidence_access("evidence-001", "did:mycelix:bob");

        assert!(arb_access.can_view, "Arbitrator should view adjudicators-only evidence");
        assert!(!alice_access.can_view, "Complainant should not view adjudicators-only evidence");
        assert!(!bob_access.can_view, "Respondent should not view adjudicators-only evidence");
    }
    ...
    fn test_restricted_evidence_access() {
        ...
        assert!(alice_access.can_view, "Alice should view restricted evidence");
        assert!(!bob_access.can_view, "Bob should not view restricted evidence");
    }
    ...
    fn test_sealed_evidence_no_access() {
        ...
        assert!(!arb_access.can_view, "Arbitrator should not view sealed evidence without order");
        assert!(!alice_access.can_view, "Party should not view sealed evidence");
    }
    ...
    fn test_evidence_submitter_can_always_view_own() {
        let case = create_test_case("case-001", "did:mycelix:alice", "did:mycelix:bob");
        create_case(&case);
        create_evidence_by_submitter("evidence-001", "case-001", "did:mycelix:alice", EvidenceVisibility::AdjudicatorsOnly);

        // Even though it's adjudicators-only, submitter should see their own evidence
        let alice_access = check_evidence_access("evidence-001", "did:mycelix:alice");
        assert!(alice_access.can_view, "Submitter should always be able to view own evidence");
    }
}
...
mod appeal_access_tests {
    ...
    fn test_losing_party_can_file_appeal() {
        ...
        assert!(bob_access.can_file_appeal, "Losing party should be able to file appeal");
    }
    ...
    fn test_winning_party_cannot_file_appeal() {
        ...
        assert!(!alice_access.can_file_appeal, "Winning party should not be able to file appeal");
    }
    ...
    fn test_either_party_can_appeal_split_decision() {
        ...
        assert!(alice_access.can_file_appeal, "Either party can appeal split decision");
        assert!(bob_access.can_file_appeal, "Either party can appeal split decision");
    }
    ...
    fn test_third_party_cannot_file_appeal() {
        ...
        assert!(!charlie_access.can_file_appeal, "Third party should not be able to file appeal");
    }
    ...
    fn test_cannot_appeal_after_finalization() {
        ...
        let bob_access = check_appeal_access("decision-001", "did:mycelix:bob");
        assert!(!bob_access.can_file_appeal, "Cannot appeal finalized decision");
    }
}
...
mod enforcement_access_tests {
    ...
    fn test_system_enforcer_can_execute() {
        ...
        assert!(access.can_execute, "Assigned enforcer should be able to execute");
        assert!(access.can_update_status, "Assigned enforcer should be able to update status");
    }
    ...
    fn test_parties_cannot_execute_enforcement() {
        ...
        assert!(!alice_access.can_execute, "Party should not be able to execute enforcement");
        assert!(!bob_access.can_execute, "Party should not be able to execute enforcement");
    }
    ...
    fn test_parties_can_view_enforcement_status() {
        ...
        assert!(alice_access.can_view, "Party should be able to view enforcement status");
        assert!(bob_access.can_view, "Party should be able to view enforcement status");
    }
    ...
    fn test_target_can_contest_enforcement() {
        ...
        let bob_access = check_enforcement_access("enforcement-001", "did:mycelix:bob");
        assert!(bob_access.can_contest, "Enforcement target should be able to contest");
    }
    ...
    fn test_non_target_cannot_contest_enforcement() {
        ...
        let alice_access = check_enforcement_access("enforcement-001", "did:mycelix:alice");
        assert!(!alice_access.can_contest, "Non-target should not be able to contest enforcement");
    }
}
...
mod restorative_circle_access_tests {
    ...
    fn test_facilitator_has_full_control() {
        ...
        assert!(access.can_record_session, "Facilitator should be able to record sessions");
        assert!(access.can_add_agreement, "Facilitator should be able to add agreements");
        assert!(access.can_complete, "Facilitator should be able to complete circle");
    }
    ...
    fn test_participants_have_limited_access() {
        ...
        assert!(!access.can_record_session, "Participant should not record sessions");
        assert!(!access.can_complete, "Participant should not complete circle");
    }
    ...
    fn test_consent_required_for_participation() {
        ...
        assert!(!before_access.can_fully_participate, "Should not fully participate before consent");

        // After consent
        record_consent("circle-001", "did:mycelix:alice");
        let after_access = check_circle_participation("circle-001", "did:mycelix:alice");
        assert!(after_access.can_fully_participate, "Should fully participate after consent");
    }
}
...
fn create_evidence(id: &str, case_id: &str, visibility: EvidenceVisibility) {
    EVIDENCE.lock().unwrap().insert(id.to_string(), TestEvidence {
        id: id.to_string(),
...
fn create_evidence_by_submitter(id: &str, case_id: &str, submitter: &str, visibility: EvidenceVisibility) {
    EVIDENCE.lock().unwrap().insert(id.to_string(), TestEvidence {
        id: id.to_string(),
        case_id: case_id.to_string(),
        visibility,
        ...
        if let Some(case) = case_store.get(&evidence.case_id) {
            ...
            match &evidence.visibility {
                ...
                EvidenceVisibility::AdjudicatorsOnly => {
                    EvidenceAccess { can_view: is_arbitrator }
                }
                EvidenceVisibility::Restricted(allowed) => {
                    EvidenceAccess { can_view: allowed.contains(&accessor.to_string()) }
                }
...
fn render_decision(case_id: &str, decision_id: &str, outcome: DecisionOutcome) {
    DECISIONS.lock().unwrap().insert(decision_id.to_string(), TestDecision {
...
fn check_appeal_access(decision_id: &str, accessor: &str) -> AppealAccess {
    ...
    if let Some(decision) = decision_store.get(decision_id) {
        if decision.finalized {
            return AppealAccess { can_file_appeal: false };
        }
        ...
        if let Some(case) = case_store.get(&decision.case_id) {
            ...
            AppealAccess { can_file_appeal: can_appeal }
        } else {
            AppealAccess { can_file_appeal: false }
        }
    } else {
        AppealAccess { can_file_appeal: false }
    }
...
fn create_enforcement(id: &str, decision_id: &str) {
    ENFORCEMENTS.lock().unwrap().insert(id.to_string(), TestEnforcement {
        id: id.to_string(),
...
fn create_circle(id: &str, case_id: &str, facilitator: &str) {
    CIRCLES.lock().unwrap().insert(id.to_string(), TestCircle {
        id: id.to_string(),
        case_id: case_id.to_string(),
...
fn check_circle_access(circle_id: &str, accessor: &str) -> CircleAccess {
    ...
    if let Some(circle) = store.get(circle_id) {
        ...
        let is_participant = circle.participants.iter().any(|(did, _, _)| did == accessor);
...
fn check_circle_participation(circle_id: &str, accessor: &str) -> CircleParticipationAccess {
    let store = CIRCLES.lock().unwrap();
    if let Some(circle) = store.get(circle_id) {
        if let Some((_, _, consented)) = circle.participants.iter().find(|(did, _, _)| did == accessor) {
            CircleParticipationAccess { can_fully_participate: *consented }
        } else {
            CircleParticipationAccess { can_fully_participate: false }
        }
    } else {
        CircleParticipationAccess { can_fully_participate: false }
    }
