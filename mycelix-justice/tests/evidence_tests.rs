...
mod evidence_submission_tests {
    ...
    fn test_submit_multiple_evidence_items() {
        let evidence1 = create_test_evidence("ev-001", "case-001", "did:mycelix:alice", EvidenceType::Document, "Doc 1");
        let evidence2 = create_test_evidence("ev-002", "case-001", "did:mycelix:alice", EvidenceType::Transaction, "Transaction record");
        let evidence3 = create_test_evidence("ev-003", "case-001", "did:mycelix:bob", EvidenceType::Testimony, "Witness testimony");
    ...
    fn test_submit_evidence_with_all_types() {
        // Document
        let doc = create_test_evidence("ev-doc", "case-001", "did:mycelix:alice", EvidenceType::Document, "Document");
        assert!(simulate_submit_evidence(doc).is_ok());

        // Transaction
        let txn = create_test_evidence("ev-txn", "case-001", "did:mycelix:alice", EvidenceType::Transaction, "Transaction");
        assert!(simulate_submit_evidence(txn).is_ok());

        // Communication
        let comm = create_test_evidence("ev-comm", "case-001", "did:mycelix:alice", EvidenceType::Communication, "Chat logs");
        assert!(simulate_submit_evidence(comm).is_ok());

        // Testimony
        let testimony = create_test_evidence("ev-test", "case-001", "did:mycelix:witness", EvidenceType::Testimony, "Witness statement");
        assert!(simulate_submit_evidence(testimony).is_ok());

        // Expert Opinion
        let expert = create_test_evidence("ev-expert", "case-001", "did:mycelix:expert", EvidenceType::ExpertOpinion, "Expert analysis");
        assert!(simulate_submit_evidence(expert).is_ok());

        // Media
        let media = create_test_evidence("ev-media", "case-001", "did:mycelix:alice", EvidenceType::Media, "Photos");
...
mod chain_of_custody_tests {
    ...
    fn test_evidence_has_initial_custody_event() {
        ...
        let submitted = result.unwrap();
        assert!(!submitted.custody.is_empty(), "Evidence should have initial custody event");
    ...
    fn test_record_evidence_access() {
        let evidence = create_test_evidence("evidence-001", "case-001", "did:mycelix:alice", EvidenceType::Document, "Doc");
        simulate_submit_evidence(evidence).unwrap();
        ...
        assert!(updated.custody.iter().any(|c| c.action == CustodyAction::Accessed && c.actor == "did:mycelix:arbitrator1"));
    }
    ...
    fn test_record_evidence_copy() {
        ...
        assert!(result.is_ok());
        let updated = result.unwrap();
        assert!(updated.custody.iter().any(|c| c.action == CustodyAction::Copied));
    }
    ...
    fn test_record_evidence_challenged() {
        ...
        assert!(result.is_ok());
        let updated = result.unwrap();
        assert!(updated.custody.iter().any(|c| c.action == CustodyAction::Challenged));
    }
    ...
    fn test_seal_evidence() {
        ...
        assert!(sealed.custody.iter().any(|c| c.action == CustodyAction::Sealed));
    }
    ...
    fn test_sealed_evidence_cannot_be_modified() {
        ...
        assert!(result.is_err(), "Should not be able to modify sealed evidence");
    }
    ...
    fn test_custody_chain_integrity() {
        ...
        simulate_record_custody_event("evidence-001", CustodyAction::Accessed, "did:mycelix:arb1", None).unwrap();
        simulate_record_custody_event("evidence-001", CustodyAction::Verified, "did:mycelix:verifier", None).unwrap();
        simulate_record_custody_event("evidence-001", CustodyAction::Accessed, "did:mycelix:arb2", None).unwrap();
...
mod evidence_verification_tests {
    ...
    fn test_dispute_evidence() {
        ...
        let dispute = EvidenceDisputeInput {
            ...
            reason: "The document has been tampered with - metadata shows edits after filing date".to_string(),
        };
    ...
    fn test_disputed_evidence_updates_verification_status() {
        ...
        assert_eq!(updated_evidence.verification.status, VerificationStatus::Disputed);
    }
}
...
mod evidence_visibility_tests {
    ...
    fn test_evidence_visibility_all_parties() {
        let mut evidence = create_test_evidence("evidence-001", "case-001", "did:mycelix:alice", EvidenceType::Document, "Doc");
        ...
        assert!(can_access_evidence("evidence-001", "did:mycelix:arbitrator1"));
    }

    #[test]
    fn test_evidence_visibility_adjudicators_only() {
        let mut evidence = create_test_evidence("evidence-001", "case-001", "did:mycelix:alice", EvidenceType::Document, "Sensitive doc");
    ...
    fn test_evidence_visibility_restricted() {
        let mut evidence = create_test_evidence("evidence-001", "case-001", "did:mycelix:alice", EvidenceType::Document, "Restricted doc");
        evidence.visibility = EvidenceVisibility::Restricted {
            parties: vec!["did:mycelix:alice".to_string(), "did:mycelix:arbitrator1".to_string()],
        };
    ...
    fn test_evidence_visibility_sealed() {
        let mut evidence = create_test_evidence("evidence-001", "case-001", "did:mycelix:alice", EvidenceType::Document, "Sealed doc");
...
mod evidence_integrity_tests {
    ...
    fn test_evidence_requires_valid_hash_format() {
        ...
        let result = simulate_submit_evidence(evidence);
        assert!(result.is_err(), "Evidence with empty hash should be rejected");
    }
    ...
    fn test_verify_evidence_hash_matches_content() {
        ...
        let result = verify_evidence_integrity("evidence-001", "QmYwAPJzv5CZsnA625s3Xf2nemtYgPpHdWEz79ojWnPbdG");
        assert!(result.is_ok());
...
fn create_test_evidence(id: &str, case_id: &str, submitter: &str, evidence_type: EvidenceType, description: &str) -> TestEvidence {
    TestEvidence {
        id: id.to_string(),
...
fn simulate_submit_evidence(mut evidence: TestEvidence) -> Result<TestEvidence, String> {
    ...
    EVIDENCE_STORE.lock().unwrap().insert(evidence.id.clone(), evidence.clone());
    Ok(evidence)
}
...
fn simulate_record_custody_event(evidence_id: &str, action: CustodyAction, actor: &str, notes: Option<String>) -> Result<TestEvidence, String> {
...
fn simulate_verify_evidence(input: EvidenceVerificationInput) -> Result<TestEvidenceVerification, String> {
    let verification = TestEvidenceVerification {
        id: format!("ver-{}-{}", input.evidence_id, VERIFICATION_STORE.lock().unwrap().len()),
        evidence_id: input.evidence_id.clone(),
        verifier: input.verifier,
        status: input.status,
        notes: input.notes,
    };
    VERIFICATION_STORE.lock().unwrap().push(verification.clone());
    Ok(verification)
}

fn simulate_dispute_evidence(input: EvidenceDisputeInput) -> Result<TestEvidenceDispute, String> {
    let dispute = TestEvidenceDispute {
        id: format!("disp-{}-{}", input.evidence_id, DISPUTE_STORE.lock().unwrap().len()),
        evidence_id: input.evidence_id.clone(),
...
fn simulate_get_evidence_verifications(evidence_id: &str) -> Vec<TestEvidenceVerification> {
    VERIFICATION_STORE.lock().unwrap()
        .iter()
