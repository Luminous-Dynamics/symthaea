...
mod case_validation_tests {
    ...
    fn test_case_requires_description() {
        ...
        assert!(result.is_err(), "Case with empty description should be invalid");
        assert_eq!(
            result.unwrap_err(),
    ...
    fn test_case_requires_complainant_did() {
        ...
        assert!(result.is_err(), "Case with non-DID complainant should be invalid");
        assert_eq!(
            result.unwrap_err(),
            "Case parties must be DIDs"
        );
    }
    ...
    fn test_case_requires_respondent_did() {
        ...
        let result = validate_case_parties(&case);
        assert!(result.is_err(), "Case with non-DID respondent should be invalid");
        assert_eq!(
            result.unwrap_err(),
            "Case parties must be DIDs"
        );
    ...
    fn test_case_whitespace_only_title_invalid() {
        ...
        let result = validate_case_title(&case);
        assert!(result.is_err(), "Case with whitespace-only title should be invalid");
    }
...
mod evidence_validation_tests {
    ...
    fn test_evidence_requires_submitter_did() {
        ...
        assert!(result.is_err(), "Evidence with non-DID submitter should be invalid");
        assert_eq!(
            result.unwrap_err(),
            "Evidence submitter must be a DID"
        );
    }
    ...
    fn test_evidence_requires_description() {
        ...
        assert!(result.is_err(), "Evidence with empty description should be invalid");
        assert_eq!(
            result.unwrap_err(),
    ...
    fn test_evidence_requires_content_hash() {
        ...
        assert!(result.is_err(), "Evidence with empty content hash should be invalid");
        assert_eq!(
            result.unwrap_err(),
            "Evidence content hash required"
        );
    }
    ...
    fn test_evidence_whitespace_only_description_invalid() {
        ...
        let result = validate_evidence_description(&evidence);
        assert!(result.is_err(), "Evidence with whitespace-only description should be invalid");
    }
}
...
mod mediation_validation_tests {
    ...
    fn test_mediation_requires_mediator_did() {
        ...
        assert!(result.is_err(), "Mediation with non-DID mediator should be invalid");
        assert_eq!(
            result.unwrap_err(),
            "Mediator must be a DID"
        );
    }
...
mod arbitration_validation_tests {
    ...
    fn test_arbitration_requires_odd_number_of_arbitrators() {
        ...
        assert!(result.is_err(), "Arbitration with even number of arbitrators should be invalid");
        assert_eq!(
            result.unwrap_err(),
    ...
    fn test_arbitration_requires_arbitrator_dids() {
        ...
        assert!(result.is_err(), "Arbitration with non-DID arbitrator should be invalid");
        assert_eq!(
            result.unwrap_err(),
    ...
    fn test_arbitration_zero_arbitrators_invalid() {
        ...
        assert!(result.is_err(), "Arbitration with zero arbitrators should be invalid");
    }
}
...
mod decision_validation_tests {
    ...
    fn test_decision_requires_reasoning() {
        let decision = MockDecision {
            ...
            votes: vec![MockVote { arbitrator: "did:mycelix:arb1".to_string(), choice: "ForComplainant".to_string() }],
        };

        let result = validate_decision_reasoning(&decision);
        assert!(result.is_err(), "Decision with empty reasoning should be invalid");
        assert_eq!(
            result.unwrap_err(),
    ...
    fn test_valid_decision_passes_validation() {
        let decision = MockDecision {
            ...
            votes: vec![
                MockVote { arbitrator: "did:mycelix:arb1".to_string(), choice: "ForComplainant".to_string() },
                MockVote { arbitrator: "did:mycelix:arb2".to_string(), choice: "ForComplainant".to_string() },
                MockVote { arbitrator: "did:mycelix:arb3".to_string(), choice: "ForRespondent".to_string() },
            ],
        };
...
mod appeal_validation_tests {
    ...
    fn test_appeal_requires_appellant_did() {
        ...
        assert!(result.is_err(), "Appeal with non-DID appellant should be invalid");
        assert_eq!(
            result.unwrap_err(),
            "Appellant must be a DID"
        );
    }
    ...
    fn test_appeal_requires_argument() {
        ...
        assert!(result.is_err(), "Appeal with empty argument should be invalid");
        assert_eq!(
            result.unwrap_err(),
            "Appeal argument required"
        );
    }
...
mod enforcement_validation_tests {
    ...
    fn test_enforcement_requires_enforcer_did() {
        ...
        assert!(result.is_err(), "Enforcement with non-DID enforcer should be invalid");
        assert_eq!(
            result.unwrap_err(),
            "Enforcer must be a DID"
        );
    }
...
mod restorative_circle_validation_tests {
    ...
    fn test_circle_requires_facilitator_did() {
        let circle = MockRestorativeCircle {
            ...
            participants: vec!["did:mycelix:alice".to_string(), "did:mycelix:bob".to_string()],
        };

        let result = validate_circle_facilitator(&circle);
        assert!(result.is_err(), "Circle with non-DID facilitator should be invalid");
        assert_eq!(
            result.unwrap_err(),
            "Facilitator must be a DID"
        );
    }
    ...
    fn test_circle_requires_participants() {
        ...
        assert!(result.is_err(), "Circle with no participants should be invalid");
        assert_eq!(
            result.unwrap_err(),
