...
mod circle_creation_tests {
    ...
    fn test_circle_creates_case_link() {
        let circle = create_test_circle("circle-001", "case-001", "did:mycelix:facilitator", create_default_participants());
    ...
    fn test_circle_creates_facilitator_link() {
        let circle = create_test_circle("circle-001", "case-001", "did:mycelix:jane_facilitator", create_default_participants());
        simulate_create_circle(circle).unwrap();
    ...
    fn test_circle_with_all_roles() {
        ...
        let circle = create_test_circle("circle-001", "case-001", "did:mycelix:facilitator", participants);
        let result = simulate_create_circle(circle);
    ...
    fn test_circle_requires_participants() {
        let circle = create_test_circle("circle-001", "case-001", "did:mycelix:facilitator", vec![]);

        let result = simulate_create_circle(circle);
        assert!(result.is_err(), "Circle with no participants should be rejected");
    }
}
...
mod consent_tests {
    ...
    fn test_record_participant_consent() {
        ...
        let alice = updated.participants.iter().find(|p| p.did == "did:mycelix:alice").unwrap();
...
mod session_tests {
    ...
    fn test_record_session() {
        ...
        let session = CircleSession {
            ...
            next_steps: vec!["Review community impact".to_string(), "Prepare apology statement".to_string()],
        };

        let result = simulate_record_session("circle-001", session, vec!["did:mycelix:alice".to_string(), "did:mycelix:bob".to_string()]);
        assert!(result.is_ok());
    ...
    fn test_record_multiple_sessions() {
        ...
        simulate_record_session("circle-001", CircleSession {
            session_number: 1,
            summary: "Session 1".to_string(),
            next_steps: vec![],
        }, vec!["did:mycelix:alice".to_string()]).unwrap();

        simulate_record_session("circle-001", CircleSession {
            session_number: 2,
            summary: "Session 2".to_string(),
            next_steps: vec![],
        }, vec!["did:mycelix:alice".to_string(), "did:mycelix:bob".to_string()]).unwrap();
    ...
    fn test_session_tracks_attendance() {
        ...
        simulate_record_session("circle-001", CircleSession {
            ...
            next_steps: vec![],
        }, vec!["did:mycelix:alice".to_string()]).unwrap(); // Only Alice attended

        let updated = simulate_get_circle("circle-001").unwrap();
        let alice = updated.participants.iter().find(|p| p.did == "did:mycelix:alice").unwrap();
        let bob = updated.participants.iter().find(|p| p.did == "did:mycelix:bob").unwrap();
    ...
    fn test_session_in_forming_status_rejected() {
        ...
        let result = simulate_record_session("circle-001", session, vec!["did:mycelix:alice".to_string()]);
        assert!(result.is_err(), "Should not be able to record session while circle is forming");
    }
}
...
mod agreement_tests {
    ...
    fn test_add_agreement() {
        ...
        let result = simulate_add_agreement("circle-001", "Bob will make a public apology to Alice within 7 days");
        assert!(result.is_ok());

        let updated = result.unwrap();
        assert!(updated.agreements.contains(&"Bob will make a public apology to Alice within 7 days".to_string()));
    }
...
mod status_transition_tests {
    ...
    fn test_get_circles_by_status() {
        simulate_create_circle(create_test_circle("c-001", "case-001", "did:mycelix:f", create_default_participants())).unwrap();
        simulate_create_circle(create_test_circle("c-002", "case-002", "did:mycelix:f", create_default_participants())).unwrap();
        simulate_create_circle(create_test_circle("c-003", "case-003", "did:mycelix:f", create_default_participants())).unwrap();
...
mod completion_tests {
    ...
    fn test_complete_circle() {
        ...
        let result = simulate_complete_circle("circle-001", vec!["Final agreement: All terms fulfilled".to_string()]);
        assert!(result.is_ok());

        let completed = result.unwrap();
        assert_eq!(completed.status, CircleStatus::Completed);
        assert!(completed.agreements.contains(&"Final agreement: All terms fulfilled".to_string()));
    }
    ...
    fn test_complete_circle_adds_final_agreements() {
        ...
        let result = simulate_complete_circle("circle-001", vec![
            "Final term 1".to_string(),
            "Final term 2".to_string(),
        ]);
    ...
    fn test_completed_circle_cannot_add_sessions() {
        ...
        let result = simulate_record_session("circle-001", session, vec!["did:mycelix:alice".to_string()]);
        assert!(result.is_err(), "Should not be able to add sessions to completed circle");
    }
}
...
mod query_tests {
    ...
    fn test_get_facilitator_circles() {
        simulate_create_circle(create_test_circle("c-001", "case-001", "did:mycelix:jane", create_default_participants())).unwrap();
        simulate_create_circle(create_test_circle("c-002", "case-002", "did:mycelix:jane", create_default_participants())).unwrap();
        simulate_create_circle(create_test_circle("c-003", "case-003", "did:mycelix:john", create_default_participants())).unwrap();
    ...
    fn test_get_case_circle() {
        simulate_create_circle(create_test_circle("circle-001", "case-001", "did:mycelix:f", create_default_participants())).unwrap();
...
fn create_test_circle(id: &str, case_id: &str, facilitator: &str, participants: Vec<CircleParticipant>) -> TestRestorativeCircle {
...
fn simulate_create_circle(circle: TestRestorativeCircle) -> Result<TestRestorativeCircle, String> {
    ...
    CIRCLE_STORE.lock().unwrap().insert(circle.id.clone(), circle.clone());
    Ok(circle)
}
...
fn simulate_get_case_circle(case_id: &str) -> Option<TestRestorativeCircle> {
    CIRCLE_STORE.lock().unwrap().values().find(|c| c.case_id == case_id).cloned()
}

fn simulate_get_facilitator_circles(facilitator_did: &str) -> Vec<TestRestorativeCircle> {
    CIRCLE_STORE.lock().unwrap()
        .values()
...
fn simulate_record_consent(circle_id: &str, participant_did: &str) -> Result<TestRestorativeCircle, String> {
...
fn simulate_record_session(circle_id: &str, session: CircleSession, attendees: Vec<String>) -> Result<TestRestorativeCircle, String> {
    let mut store = CIRCLE_STORE.lock().unwrap();
    if let Some(circle) = store.get_mut(circle_id) {
...
fn simulate_add_agreement(circle_id: &str, agreement: &str) -> Result<TestRestorativeCircle, String> {
    let mut store = CIRCLE_STORE.lock().unwrap();
    if let Some(circle) = store.get_mut(circle_id) {
...
fn simulate_update_circle_status(circle_id: &str, new_status: CircleStatus) -> Result<TestRestorativeCircle, String> {
    let mut store = CIRCLE_STORE.lock().unwrap();
    if let Some(circle) = store.get_mut(circle_id) {
...
fn simulate_complete_circle(circle_id: &str, final_agreements: Vec<String>) -> Result<TestRestorativeCircle, String> {
    let mut store = CIRCLE_STORE.lock().unwrap();
    if let Some(circle) = store.get_mut(circle_id) {
