...
mod enforcement_creation_tests {
    ...
    fn test_enforcement_creates_decision_link() {
        let enforcement = create_test_enforcement("enforcement-001", "decision-001", 0, "did:mycelix:enforcer");
        simulate_create_enforcement(enforcement).unwrap();

        let decision_enforcements = simulate_get_decision_enforcement("decision-001");
        assert!(decision_enforcements.iter().any(|e| e.id == "enforcement-001"));
    }
    ...
    fn test_create_multiple_enforcements_for_decision() {
        ...
        let enforcement1 = create_test_enforcement("enforcement-001", "decision-001", 0, "did:mycelix:enforcer");
        let enforcement2 = create_test_enforcement("enforcement-002", "decision-001", 1, "did:mycelix:enforcer");
        let enforcement3 = create_test_enforcement("enforcement-003", "decision-001", 2, "did:mycelix:enforcer");
...
mod enforcement_status_tests {
    ...
    fn test_status_pending_to_in_progress() {
        ...
        let result = simulate_update_enforcement_status("enforcement-001", EnforcementStatus::InProgress);
        assert!(result.is_ok());
    ...
    fn test_status_in_progress_to_partially_completed() {
        let enforcement = create_test_enforcement("enforcement-001", "decision-001", 0, "did:mycelix:enforcer");
        simulate_create_enforcement(enforcement).unwrap();
        simulate_update_enforcement_status("enforcement-001", EnforcementStatus::InProgress).unwrap();

        let result = simulate_update_enforcement_status("enforcement-001", EnforcementStatus::PartiallyCompleted);
        assert!(result.is_ok());
        assert_eq!(result.unwrap().status, EnforcementStatus::PartiallyCompleted);
    }
    ...
    fn test_status_to_completed() {
        ...
        let result = simulate_update_enforcement_status("enforcement-001", EnforcementStatus::Completed);
        assert!(result.is_ok());
    ...
    fn test_status_to_failed() {
        ...
        let result = simulate_update_enforcement_status("enforcement-001", EnforcementStatus::Failed);
        assert!(result.is_ok());
    ...
    fn test_status_to_contested() {
        ...
        let result = simulate_update_enforcement_status("enforcement-001", EnforcementStatus::Contested);
        assert!(result.is_ok());
    ...
    fn test_get_pending_enforcements() {
        simulate_create_enforcement(create_test_enforcement("e-001", "d-001", 0, "did:mycelix:e")).unwrap();
        simulate_create_enforcement(create_test_enforcement("e-002", "d-002", 0, "did:mycelix:e")).unwrap();
        simulate_create_enforcement(create_test_enforcement("e-003", "d-003", 0, "did:mycelix:e")).unwrap();
...
mod enforcement_action_tests {
    ...
    fn test_record_multiple_actions() {
        ...
        simulate_record_action("enforcement-001", EnforcementAction {
            action_type: EnforcementActionType::Notification,
            target_happ: None,
            target_entry: None,
...
mod enforcement_completion_tests {
    ...
    fn test_complete_enforcement_with_final_action() {
        ...
        let completed = result.unwrap();
        assert!(completed.actions.iter().any(|a| a.result.contains("Final completion")));
    }
    ...
    fn test_mark_enforcement_failed() {
        ...
        let result = simulate_mark_enforcement_failed("enforcement-001", "Unable to locate respondent's assets for seizure");
        assert!(result.is_ok());

        let failed = result.unwrap();
        assert_eq!(failed.status, EnforcementStatus::Failed);
        assert!(failed.actions.iter().any(|a| a.action_type == EnforcementActionType::ManualRequired));
    }
    ...
    fn test_completed_enforcement_cannot_add_actions() {
        ...
        let result = simulate_record_action("enforcement-001", action);
        assert!(result.is_err(), "Should not be able to add actions to completed enforcement");
    }
}
...
mod cross_happ_enforcement_tests {
    ...
    fn test_execute_cross_happ_action() {
        ...
        assert!(updated.actions.iter().any(|a| a.action_type == EnforcementActionType::CrossHappAction));
    }
    ...
    fn test_multiple_cross_happ_actions() {
        ...
        simulate_execute_cross_happ_action("enforcement-001", "mycelix-wallet", None, "Funds seized").unwrap();
        simulate_execute_cross_happ_action("enforcement-001", "mycelix-trust", None, "Reputation penalty applied").unwrap();
        simulate_execute_cross_happ_action("enforcement-001", "mycelix-marketplace", None, "Account suspended").unwrap();

        let updated = simulate_get_enforcement("enforcement-001").unwrap();
        let cross_happ_actions: Vec<_> = updated.actions.iter()
...
mod enforcement_query_tests {
    ...
    fn test_get_decision_enforcement() {
        simulate_create_enforcement(create_test_enforcement("e-001", "decision-001", 0, "did:mycelix:e")).unwrap();
        simulate_create_enforcement(create_test_enforcement("e-002", "decision-001", 1, "did:mycelix:e")).unwrap();
        simulate_create_enforcement(create_test_enforcement("e-003", "decision-002", 0, "did:mycelix:e")).unwrap();
...
fn create_test_enforcement(id: &str, decision_id: &str, remedy_index: u32, enforcer: &str) -> TestEnforcement {
    TestEnforcement {
        id: id.to_string(),
...
fn simulate_create_enforcement(enforcement: TestEnforcement) -> Result<TestEnforcement, String> {
    ...
    ENFORCEMENT_STORE.lock().unwrap().insert(enforcement.id.clone(), enforcement.clone());
    Ok(enforcement)
}
...
fn simulate_update_enforcement_status(id: &str, new_status: EnforcementStatus) -> Result<TestEnforcement, String> {
...
fn simulate_complete_enforcement(id: &str, final_action: Option<EnforcementAction>) -> Result<TestEnforcement, String> {
    let mut store = ENFORCEMENT_STORE.lock().unwrap();
    if let Some(enforcement) = store.get_mut(id) {
...
fn simulate_get_pending_enforcements() -> Vec<TestEnforcement> {
    ENFORCEMENT_STORE.lock().unwrap()
        .values()
        .filter(|e| e.status == EnforcementStatus::Pending || e.status == EnforcementStatus::InProgress)
        .cloned()
        .collect()
}
...
fn simulate_execute_cross_happ_action(id: &str, target_happ: &str, target_entry: Option<String>, result: &str) -> Result<TestEnforcement, String> {
    let action = EnforcementAction {
