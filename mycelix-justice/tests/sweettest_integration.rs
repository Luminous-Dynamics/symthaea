//! # Mycelix Justice - Sweettest Integration Tests
//!
...
mod complaint_tests {
    ...
    async fn test_file_complaint() {
        ...
        let record: Record = conductor
            .call(&cell.zome("complaints"), "file_complaint", complaint.clone())
            .await;
        ...
        assert_eq!(filed.complainant, "did:mycelix:alice", "Complainant must match");
        ...
        assert_eq!(filed.complaint_type, ComplaintType::ContractDispute, "Type must match");
    }
    ...
    async fn test_get_complaints_by_party() {
        ...
        assert!(!complaints.is_empty(), "Should find complaints for complainant");
        ...
        assert!(!respondent_complaints.is_empty(), "Should find complaints for respondent");
    }
    ...
    async fn test_update_complaint_status() {
        ...
        let updated_record: Record = conductor
            .call(&cell.zome("complaints"), "update_complaint_status", update_input)
            .await;

        let updated: Complaint = decode_entry(&updated_record).expect("Failed to decode");
        assert_eq!(updated.status, ComplaintStatus::UnderReview, "Status should be UnderReview");
    }
    ...
    async fn test_submit_response_to_complaint() {
        ...
        assert_eq!(response.complaint_id, "complaint-resp-001", "Complaint ID must match");
        assert_eq!(response.respondent, "did:mycelix:hank", "Respondent must match");
...
mod evidence_tests {
    ...
    async fn test_submit_evidence() {
        ...
        assert_eq!(submitted.complaint_id, "complaint-001", "Complaint ID must match");
        assert_eq!(submitted.submitter, "did:mycelix:alice", "Submitter must match");
        assert_eq!(submitted.evidence_type, EvidenceType::Document, "Type must match");
        assert_eq!(submitted.content_hash, "QmHash123abc", "Content hash must match");
    }
    ...
    async fn test_get_complaint_evidence() {
        ...
        assert!(complaint_evidence.len() >= 3, "Should have at least 3 evidence items");
    }
    ...
    async fn test_verify_evidence() {
        ...
        assert_eq!(verification.evidence_id, "evidence-verify-001", "Evidence ID must match");
        assert_eq!(verification.status, VerificationStatus::Verified, "Status should be Verified");
    }
    ...
    async fn test_dispute_evidence() {
        ...
        assert_eq!(dispute.evidence_id, "evidence-dispute-001", "Evidence ID must match");
    ...
    async fn test_get_evidence_by_submitter() {
        ...
        assert!(!by_submitter.is_empty(), "Should find evidence by submitter");
    }
}
...
mod arbitration_tests {
    ...
    async fn test_create_arbitration_panel() {
        ...
        assert_eq!(created.status, ArbitrationStatus::PanelFormation, "Status must be PanelFormation");
        assert_eq!(created.selection_method, ArbitratorSelection::Random, "Selection method must match");
    }
    ...
    async fn test_update_arbitration_status() {
        ...
        let updated_record: Record = conductor
            .call(&cell.zome("arbitration"), "update_arbitration_status", update_input)
            .await;

        let updated: Arbitration = decode_entry(&updated_record).expect("Failed to decode");
        assert_eq!(updated.status, ArbitrationStatus::EvidenceReview, "Status should be EvidenceReview");
    }
    ...
    async fn test_render_decision() {
        ...
        let now = Timestamp::now();
        let appeal_deadline = Timestamp::from_micros(now.as_micros() as i64 + 30 * 24 * 3600 * 1_000_000);
        ...
        assert_eq!(rendered.outcome, DecisionOutcome::ForComplainant, "Outcome must match");
        ...
        assert_eq!(rendered.remedies[0].remedy_type, RemedyType::Compensation, "Remedy type must match");
    }
    ...
    async fn test_file_appeal() {
        ...
        let appeal = Appeal {
            ...
            argument: "New evidence has surfaced that was not available during arbitration.".to_string(),
        ...
        assert_eq!(filed.appellant, "did:mycelix:respondent-appeal", "Appellant must match");
...
mod lifecycle_tests {
    ...
    async fn test_complete_justice_lifecycle() {
        ...
        assert_eq!(filed.status, ComplaintStatus::Open, "Complaint should be Open");
        ...
        assert_eq!(created_arb.arbitrators.len(), 3, "Panel must have 3 members");
        ...
        assert_eq!(rendered.outcome, DecisionOutcome::ForComplainant, "Outcome should be for complainant");
        ...
        let resolved_record: Record = conductor
            .call(&cell.zome("complaints"), "update_complaint_status", resolve_input)
            .await;

        let resolved: Complaint = decode_entry(&resolved_record).expect("Failed to decode");
        assert_eq!(resolved.status, ComplaintStatus::Resolved, "Complaint should be Resolved");
    }
}
