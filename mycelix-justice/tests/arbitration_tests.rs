...
mod arbitration_creation_tests {
    ...
    fn test_create_arbitration_panel() {
        ...
        assert!(result.is_ok(), "Creating valid arbitration panel should succeed");
    }
    ...
    fn test_arbitration_creates_case_link() {
        let arbitration = create_test_arbitration(
            "arb-001",
            "case-001",
            vec![create_arbitrator("did:mycelix:arb1", ArbitratorRole::Primary)],
        );
    ...
    fn test_arbitration_selection_methods() {
        ...
        let arb_expert = create_test_arbitration_with_selection(
            "arb-expert",
            "case-004",
            ArbitratorSelection::ExpertiseBased { domain: "smart_contracts".to_string() },
        );
    ...
    fn test_arbitration_with_alternates() {
        ...
        let alternates: Vec<_> = created.arbitrators.iter()
...
mod arbitrator_response_tests {
    ...
    fn test_arbitrator_accepts() {
        ...
        let arb1 = updated.arbitrators.iter().find(|a| a.did == "did:mycelix:arb1").unwrap();
    ...
    fn test_arbitrator_recuses() {
        ...
        assert!(result.is_ok());
        let updated = result.unwrap();
        let arb2 = updated.arbitrators.iter().find(|a| a.did == "did:mycelix:arb2").unwrap();
    ...
    fn test_all_arbitrators_accept_moves_to_evidence_review() {
        ...
        simulate_record_arbitrator_response("arb-001", "did:mycelix:arb1", true, false, None).unwrap();
        simulate_record_arbitrator_response("arb-001", "did:mycelix:arb2", true, false, None).unwrap();
        simulate_record_arbitrator_response("arb-001", "did:mycelix:arb3", true, false, None).unwrap();
...
mod arbitration_status_tests {
    ...
    fn test_status_panel_formation_to_evidence_review() {
        ...
        let result = simulate_update_arbitration_status("arb-001", ArbitrationStatus::EvidenceReview);
        assert!(result.is_ok());
    ...
    fn test_status_deliberation_to_decision_drafting() {
        ...
        let result = simulate_update_arbitration_status("arb-001", ArbitrationStatus::DecisionDrafting);
        assert!(result.is_ok());
    ...
    fn test_status_to_decision_rendered() {
        ...
        let result = simulate_update_arbitration_status("arb-001", ArbitrationStatus::DecisionRendered);
        assert!(result.is_ok());
    ...
    fn test_get_arbitrations_by_status() {
        simulate_create_arbitration(create_test_arbitration("arb-001", "case-001", vec![create_arbitrator("did:mycelix:arb1", ArbitratorRole::Primary)])).unwrap();
        simulate_create_arbitration(create_test_arbitration("arb-002", "case-002", vec![create_arbitrator("did:mycelix:arb2", ArbitratorRole::Primary)])).unwrap();
...
mod decision_tests {
    ...
    fn test_render_decision() {
        let decision = create_test_decision(
            ...
            vec![
                ArbitratorVote { arbitrator: "did:mycelix:arb1".to_string(), vote: VoteChoice::ForComplainant },
                ArbitratorVote { arbitrator: "did:mycelix:arb2".to_string(), vote: VoteChoice::ForComplainant },
                ArbitratorVote { arbitrator: "did:mycelix:arb3".to_string(), vote: VoteChoice::ForRespondent },
            ],
        );
    ...
    fn test_decision_creates_case_link() {
        let decision = create_test_decision(
            ...
            DecisionType::MeritsDecision,
            DecisionOutcome::ForComplainant,
            vec![ArbitratorVote { arbitrator: "did:mycelix:arb1".to_string(), vote: VoteChoice::ForComplainant }],
        );
    ...
    fn test_decision_types() {
        // Merits decision
        let merits = create_test_decision("d-merits", "case-001", "arb-001", DecisionType::MeritsDecision, DecisionOutcome::ForComplainant, create_default_votes());
        assert!(simulate_render_decision(merits).is_ok());

        // Interim decision
        let interim = create_test_decision("d-interim", "case-002", "arb-002", DecisionType::InterimDecision, DecisionOutcome::ForComplainant, create_default_votes());
        assert!(simulate_render_decision(interim).is_ok());

        // Default decision
        let default = create_test_decision("d-default", "case-003", "arb-003", DecisionType::DefaultDecision, DecisionOutcome::ForComplainant, create_default_votes());
        assert!(simulate_render_decision(default).is_ok());

        // Consent decision
        let consent = create_test_decision("d-consent", "case-004", "arb-004", DecisionType::ConsentDecision, DecisionOutcome::Settled, create_default_votes());
        assert!(simulate_render_decision(consent).is_ok());

        // Dismissal
        let dismissal = create_test_decision("d-dismiss", "case-005", "arb-005", DecisionType::Dismissal, DecisionOutcome::Dismissed, create_default_votes());
    ...
    fn test_decision_outcomes() {
        // For complainant
        let for_comp = create_test_decision("d-1", "case-001", "arb-001", DecisionType::MeritsDecision, DecisionOutcome::ForComplainant, create_default_votes());
        assert_eq!(simulate_render_decision(for_comp).unwrap().outcome, DecisionOutcome::ForComplainant);

        // For respondent
        let for_resp = create_test_decision("d-2", "case-002", "arb-002", DecisionType::MeritsDecision, DecisionOutcome::ForRespondent, create_default_votes());
        assert_eq!(simulate_render_decision(for_resp).unwrap().outcome, DecisionOutcome::ForRespondent);

        // Split decision
        let split = create_test_decision("d-3", "case-003", "arb-003", DecisionType::MeritsDecision, DecisionOutcome::SplitDecision, create_default_votes());
        assert_eq!(simulate_render_decision(split).unwrap().outcome, DecisionOutcome::SplitDecision);
    }
    ...
    fn test_decision_with_dissenting_opinions() {
        ...
        decision.dissents = vec![
            DissentingOpinion {
                arbitrator: "did:mycelix:arb3".to_string(),
                opinion: "I disagree with the majority because the evidence was insufficient...".to_string(),
            },
        ];
    ...
    fn test_finalize_decision() {
        let decision = create_test_decision("decision-001", "case-001", "arb-001", DecisionType::MeritsDecision, DecisionOutcome::ForComplainant, create_default_votes());
        simulate_render_decision(decision).unwrap();
    ...
    fn test_cannot_appeal_finalized_decision() {
        ...
        let appeal = create_test_appeal("appeal-001", "case-001", "decision-001", "did:mycelix:bob");
        let result = simulate_file_appeal(appeal);
        assert!(result.is_err(), "Should not be able to appeal finalized decision");
    }
}
...
mod appeal_tests {
    ...
    #[test]
    fn test_file_appeal() {
        let decision = create_test_decision("decision-001", "case-001", "arb-001", DecisionType::MeritsDecision, DecisionOutcome::ForRespondent, create_default_votes());
        simulate_render_decision(decision).unwrap();

        let appeal = create_test_appeal("appeal-001", "case-001", "decision-001", "did:mycelix:alice");
        let result = simulate_file_appeal(appeal);
    ...
    fn test_appeal_grounds() {
        ...
        let mut appeal = create_test_appeal("appeal-001", "case-001", "decision-001", "did:mycelix:alice");
        appeal.grounds = vec![
    ...
    fn test_appeal_within_deadline() {
        let mut decision = create_test_decision("decision-001", "case-001", "arb-001", DecisionType::MeritsDecision, DecisionOutcome::ForRespondent, create_default_votes());
    ...
    fn test_appeal_number_tracking() {
        ...
        let appeal1 = create_test_appeal("appeal-001", "case-001", "decision-001", "did:mycelix:alice");
        ...
        let mut appeal2 = create_test_appeal("appeal-002", "case-001", "decision-001", "did:mycelix:alice");
...
impl MockTimestamp {
    fn now() -> Self {
        MockTimestamp { micros: 1706000000000000 }
    }

    fn from_days_from_now(days: i64) -> Self {
        MockTimestamp { micros: 1706000000000000 + (days * 24 * 3600 * 1_000_000) }
    }
}
...
fn create_test_arbitration(id: &str, case_id: &str, arbitrators: Vec<Arbitrator>) -> TestArbitration {
    TestArbitration {
        id: id.to_string(),
...
fn create_test_arbitration_with_selection(id: &str, case_id: &str, selection: ArbitratorSelection) -> TestArbitration {
    TestArbitration {
        id: id.to_string(),
        case_id: case_id.to_string(),
        arbitrators: vec![create_arbitrator("did:mycelix:arb1", ArbitratorRole::Primary)],
        selection_method: selection,
        status: ArbitrationStatus::PanelFormation,
    }
}

fn create_test_decision(id: &str, case_id: &str, arb_id: &str, decision_type: DecisionType, outcome: DecisionOutcome, votes: Vec<ArbitratorVote>) -> TestDecision {
    TestDecision {
        id: id.to_string(),
        case_id: case_id.to_string(),
...
fn simulate_create_arbitration(arbitration: TestArbitration) -> Result<TestArbitration, String> {
    ...
    ARB_STORE.lock().unwrap().insert(arbitration.id.clone(), arbitration.clone());
    Ok(arbitration)
}
...
fn simulate_get_case_arbitration(case_id: &str) -> Option<TestArbitration> {
    ARB_STORE.lock().unwrap().values().find(|a| a.case_id == case_id).cloned()
}

fn simulate_get_arbitrator_cases(arbitrator_did: &str) -> Vec<TestArbitration> {
    ARB_STORE.lock().unwrap()
        .values()
...
fn simulate_record_arbitrator_response(arb_id: &str, arbitrator_did: &str, accepted: bool, recused: bool, reason: Option<String>) -> Result<TestArbitration, String> {
...
fn simulate_update_arbitration_status(arb_id: &str, new_status: ArbitrationStatus) -> Result<TestArbitration, String> {
    let mut store = ARB_STORE.lock().unwrap();
    if let Some(arb) = store.get_mut(arb_id) {
...
fn simulate_render_decision(decision: TestDecision) -> Result<TestDecision, String> {
    ...
    DECISION_STORE.lock().unwrap().insert(decision.id.clone(), decision.clone());
    Ok(decision)
}
...
fn simulate_file_appeal(appeal: TestAppeal) -> Result<TestAppeal, String> {
    ...
    APPEAL_STORE.lock().unwrap().insert(appeal.id.clone(), appeal.clone());
    Ok(appeal)
}
...
fn simulate_update_appeal_status(appeal_id: &str, new_status: AppealStatus) -> Result<TestAppeal, String> {
