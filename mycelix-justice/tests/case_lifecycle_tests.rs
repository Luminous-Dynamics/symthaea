...
mod case_filing_tests {
    ...
    fn test_file_case_creates_case_record() {
        let case = create_test_case("case-001", "Contract Dispute", "did:mycelix:alice", "did:mycelix:bob");
    ...
    fn test_file_case_creates_complainant_link() {
        let case = create_test_case("case-001", "Property Dispute", "did:mycelix:alice", "did:mycelix:bob");

        let result = simulate_file_case(case.clone());
        assert!(result.is_ok());

        // Verify complainant can retrieve the case
        let complainant_cases = simulate_get_cases_for_party(&case.complainant, PartyType::Complainant);
        assert!(complainant_cases.iter().any(|c| c.id == "case-001"));
    }

    #[test]
    fn test_file_case_creates_respondent_link() {
        let case = create_test_case("case-001", "Financial Dispute", "did:mycelix:alice", "did:mycelix:bob");

        let result = simulate_file_case(case.clone());
        assert!(result.is_ok());

        // Verify respondent can retrieve the case
        let respondent_cases = simulate_get_cases_for_party(&case.respondent, PartyType::Respondent);
        assert!(respondent_cases.iter().any(|c| c.id == "case-001"));
    }

    #[test]
    fn test_file_case_with_context() {
        let mut case = create_test_case("case-001", "Marketplace Dispute", "did:mycelix:alice", "did:mycelix:bob");
        ...
        assert_eq!(filed_case.context.as_ref().unwrap().happ, Some("mycelix-marketplace".to_string()));
    }

    #[test]
    fn test_file_case_with_severity_levels() {
        // Minor case
        let mut minor_case = create_test_case("case-minor", "Minor Issue", "did:mycelix:alice", "did:mycelix:bob");
        minor_case.severity = CaseSeverity::Minor;
        assert!(simulate_file_case(minor_case).is_ok());

        // Moderate case
        let mut moderate_case = create_test_case("case-moderate", "Moderate Issue", "did:mycelix:alice", "did:mycelix:bob");
        moderate_case.severity = CaseSeverity::Moderate;
        assert!(simulate_file_case(moderate_case).is_ok());

        // Serious case
        let mut serious_case = create_test_case("case-serious", "Serious Issue", "did:mycelix:alice", "did:mycelix:bob");
        serious_case.severity = CaseSeverity::Serious;
        assert!(simulate_file_case(serious_case).is_ok());

        // Critical case
        let mut critical_case = create_test_case("case-critical", "Critical Issue", "did:mycelix:alice", "did:mycelix:bob");
...
mod case_phase_transition_tests {
    ...
    fn test_phase_transition_filed_to_negotiation() {
        ...
        assert!(result.is_ok(), "Transition from Filed to Negotiation should succeed");
    ...
    fn test_phase_transition_negotiation_to_mediation() {
        ...
        assert!(result.is_ok(), "Transition from Negotiation to Mediation should succeed");
    ...
    fn test_phase_transition_mediation_to_arbitration() {
        ...
        assert!(result.is_ok(), "Transition from Mediation to Arbitration should succeed");
    ...
    fn test_phase_transition_arbitration_to_appeal() {
        ...
        assert!(result.is_ok(), "Transition from Arbitration to Appeal should succeed");
    ...
    fn test_phase_transition_with_deadline() {
        ...
        let result = simulate_update_phase_with_deadline(&case.id, CasePhase::Negotiation, Some(deadline));
        assert!(result.is_ok());
    ...
    fn test_direct_filing_to_arbitration_for_serious_cases() {
        ...
        let result = simulate_update_phase(&case.id, CasePhase::Arbitration);
        assert!(result.is_ok(), "Serious cases should be able to go directly to arbitration");
    }
}
...
mod case_status_tests {
    ...
    fn test_cannot_modify_closed_case() {
        ...
        assert!(result.is_err(), "Should not be able to reactivate a closed case");
    }
}
...
mod party_management_tests {
    ...
    fn test_add_witness_to_case() {
        ...
        let updated_case = result.unwrap();
        assert!(updated_case.parties.iter().any(|p| p.did == "did:mycelix:witness1" && p.role == PartyRole::Witness));
    }
    ...
    fn test_add_expert_to_case() {
        ...
        let updated_case = result.unwrap();
        assert!(updated_case.parties.iter().any(|p| p.did == "did:mycelix:expert1" && p.role == PartyRole::Expert));
    }
    ...
    fn test_add_intervenor_to_case() {
        ...
        let updated_case = result.unwrap();
        assert!(updated_case.parties.iter().any(|p| p.did == "did:mycelix:intervenor1" && p.role == PartyRole::Intervenor));
    }
    ...
    fn test_add_affected_party_to_case() {
        ...
        let updated_case = result.unwrap();
        assert!(updated_case.parties.iter().any(|p| p.did == "did:mycelix:affected1" && p.role == PartyRole::Affected));
    }
    ...
    fn test_party_added_with_timestamp() {
        ...
        let updated_case = result.unwrap();
        let witness = updated_case.parties.iter().find(|p| p.did == "did:mycelix:witness1").unwrap();
...
mod case_query_tests {
    ...
    fn test_get_my_cases_as_complainant() {
        let case1 = create_test_case("case-001", "Dispute 1", "did:mycelix:alice", "did:mycelix:bob");
        let case2 = create_test_case("case-002", "Dispute 2", "did:mycelix:alice", "did:mycelix:charlie");
    ...
    fn test_get_my_cases_as_respondent() {
        let case = create_test_case("case-001", "Dispute", "did:mycelix:alice", "did:mycelix:bob");
    ...
    fn test_get_all_cases() {
        simulate_file_case(create_test_case("case-001", "Dispute 1", "did:mycelix:alice", "did:mycelix:bob")).unwrap();
        simulate_file_case(create_test_case("case-002", "Dispute 2", "did:mycelix:charlie", "did:mycelix:dave")).unwrap();
        simulate_file_case(create_test_case("case-003", "Dispute 3", "did:mycelix:eve", "did:mycelix:frank")).unwrap();
    ...
    fn test_get_cases_avoids_duplicates() {
        ...
        let case1 = create_test_case("case-001", "Dispute 1", "did:mycelix:alice", "did:mycelix:bob");
        let case2 = create_test_case("case-002", "Dispute 2", "did:mycelix:bob", "did:mycelix:alice");
...
fn create_filed_case(id: &str) -> TestCase {
    ...
    CASE_STORE.lock().unwrap().insert(id.to_string(), case.clone());
    case
}
...
fn simulate_file_case(case: TestCase) -> Result<TestCase, String> {
    ...
    CASE_STORE.lock().unwrap().insert(case.id.clone(), case.clone());
    Ok(case)
}
...
fn simulate_update_phase_with_deadline(case_id: &str, new_phase: CasePhase, deadline: Option<MockTimestamp>) -> Result<TestCase, String> {
    let mut store = CASE_STORE.lock().unwrap();
    if let Some(case) = store.get_mut(case_id) {
        case.phase = new_phase;
