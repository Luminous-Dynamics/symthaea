...
pub fn file_complaint(complaint: Complaint) -> ExternResult<Record> {
    if complaint.complainant.is_empty() || complaint.complainant.len() > 256 {
        return Err(wasm_error!(WasmErrorInner::Guest("Complainant must be 1-256 characters".into())));
    }
    if complaint.respondent.is_empty() || complaint.respondent.len() > 256 {
        return Err(wasm_error!(WasmErrorInner::Guest("Respondent must be 1-256 characters".into())));
    }
    ...
    if complaint.evidence_ids.len() > 100 {
        return Err(wasm_error!(WasmErrorInner::Guest("Maximum 100 evidence items".into())));
    }
...
pub fn get_complaint(complaint_id: String) -> ExternResult<Option<Record>> {
    ...
    let filter = ChainQueryFilter::new()
        .entry_type(EntryType::App(AppEntryDef::try_from(UnitEntryTypes::Complaint)?))
...
pub fn get_complaints_by_party(party_did: String) -> ExternResult<Vec<Record>> {
    if party_did.is_empty() || party_did.len() > 256 {
        return Err(wasm_error!(WasmErrorInner::Guest("Party DID must be 1-256 characters".into())));
    }
    let mut complaints = Vec::new();

    let complainant_links = get_links(
        LinkQuery::try_new(anchor_hash(&format!("complainant:{}", party_did))?, LinkTypes::ComplainantToComplaint)?,
        GetStrategy::default(),
    )?;
    complaints.extend(records_from_links(complainant_links)?);

    let respondent_links = get_links(
        LinkQuery::try_new(anchor_hash(&format!("respondent:{}", party_did))?, LinkTypes::RespondentToComplaint)?,
        GetStrategy::default(),
    )?;
    ...
    for record in query(filter)? {
        if let Some(complaint) = record.entry().to_app_option::<Complaint>().ok().flatten() {
            if complaint.id == input.complaint_id {
                ...
                let action_hash = update_entry(record.action_address().clone(), &EntryTypes::Complaint(updated))?;
                return get(action_hash, GetOptions::default())?
                    .ok_or(wasm_error!(WasmErrorInner::Guest("Not found".into())));
            }
        }
    }
    Err(wasm_error!(WasmErrorInner::Guest("Complaint not found".into())))
}
...
pub fn submit_response(input: SubmitResponseInput) -> ExternResult<Record> {
    ...
    let response = Response {
        id: format!("response:{}:{}", input.complaint_id, sys_time()?.as_micros()),
...
pub fn get_open_complaints(_: ()) -> ExternResult<Vec<Record>> {
    ...
    for record in query(filter)? {
        if let Some(complaint) = record.entry().to_app_option::<Complaint>().ok().flatten() {
            if matches!(complaint.status, ComplaintStatus::Open | ComplaintStatus::UnderReview) {
...
pub fn get_complaint_responses(complaint_id: String) -> ExternResult<Vec<Record>> {
    ...
    let links = get_links(
        LinkQuery::try_new(anchor_hash(&format!("complaint:{}", complaint_id))?, LinkTypes::ComplaintToResponse)?,
        GetStrategy::default(),
    )?;
