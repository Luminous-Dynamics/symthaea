...
pub fn initiate_arbitration(input: InitiateArbitrationInput) -> ExternResult<Record> {
    ...
    create_link(hash_entry(&input.complaint_id)?, action_hash.clone(), LinkTypes::ComplaintToArbitration, ())?;
    get(action_hash, GetOptions::default())?.ok_or(wasm_error!(WasmErrorInner::Guest("Not found".into())))
}

#[derive(Serialize, Deserialize, Debug)]
pub struct InitiateArbitrationInput { pub complaint_id: String, pub jurors: Vec<String> }
...
pub fn issue_judgment(input: IssueJudgmentInput) -> ExternResult<Record> {
    ...
    create_link(hash_entry(&input.arbitration_id)?, action_hash.clone(), LinkTypes::ArbitrationToJudgment, ())?;
    get(action_hash, GetOptions::default())?.ok_or(wasm_error!(WasmErrorInner::Guest("Not found".into())))
}
...
pub fn get_arbitration(arbitration_id: String) -> ExternResult<Option<Record>> {
    let filter = ChainQueryFilter::new()
        .entry_type(EntryType::App(AppEntryDef::try_from(UnitEntryTypes::Arbitration)?))
...
pub fn get_arbitration_for_complaint(complaint_id: String) -> ExternResult<Vec<Record>> {
    ...
    for link in get_links(GetLinksInputBuilder::try_new(hash_entry(&complaint_id)?, LinkTypes::ComplaintToArbitration)?.build())? {
...
pub fn get_judgment(arbitration_id: String) -> ExternResult<Vec<Record>> {
    ...
    for link in get_links(GetLinksInputBuilder::try_new(hash_entry(&arbitration_id)?, LinkTypes::ArbitrationToJudgment)?.build())? {
        if let Some(record) = get(ActionHash::try_from(link.target).map_err(|_| wasm_error!(WasmErrorInner::Guest("Invalid".into())))?, GetOptions::default())? {
    ...
    for record in query(filter)? {
        if let Some(arb) = record.entry().to_app_option::<Arbitration>().ok().flatten() {
            if arb.id == input.arbitration_id {
                let updated = Arbitration { status: input.new_status, ..arb };
                let action_hash = update_entry(record.action_address().clone(), &EntryTypes::Arbitration(updated))?;
                return get(action_hash, GetOptions::default())?.ok_or(wasm_error!(WasmErrorInner::Guest("Not found".into())));
            }
        }
    }
    Err(wasm_error!(WasmErrorInner::Guest("Arbitration not found".into())))
}
        ...
        if let Some(arb) = record.entry().to_app_option::<Arbitration>().ok().flatten() {
            if arb.id == input.arbitration_id {
                let new_deadline = Timestamp::from_micros(
                    arb.deadline.as_micros() as i64 + (input.extension_days as i64 * 24 * 3600 * 1_000_000)
                );
                let updated = Arbitration { deadline: new_deadline, ..arb };
                let action_hash = update_entry(record.action_address().clone(), &EntryTypes::Arbitration(updated))?;
                return get(action_hash, GetOptions::default())?.ok_or(wasm_error!(WasmErrorInner::Guest("Not found".into())));
            }
        }
    ...
    for record in query(filter)? {
        if let Some(mut arb) = record.entry().to_app_option::<Arbitration>().ok().flatten() {
            if arb.id == input.arbitration_id {
                ...
                let action_hash = update_entry(record.action_address().clone(), &EntryTypes::Arbitration(arb))?;
                return get(action_hash, GetOptions::default())?.ok_or(wasm_error!(WasmErrorInner::Guest("Not found".into())));
            }
        }
    }
...
pub fn submit_juror_vote(input: JurorVoteInput) -> ExternResult<Record> {
    let vote = JurorVote {
        id: format!("vote:{}:{}:{}", input.arbitration_id, input.juror_id, sys_time()?.as_micros()),
        arbitration_id: input.arbitration_id.clone(),
    ...
    create_link(hash_entry(&input.arbitration_id)?, action_hash.clone(), LinkTypes::ArbitrationToVote, ())?;
    get(action_hash, GetOptions::default())?.ok_or(wasm_error!(WasmErrorInner::Guest("Not found".into())))
}
...
pub fn get_arbitration_votes(arbitration_id: String) -> ExternResult<Vec<Record>> {
    ...
    for link in get_links(GetLinksInputBuilder::try_new(hash_entry(&arbitration_id)?, LinkTypes::ArbitrationToVote)?.build())? {
        if let Some(record) = get(ActionHash::try_from(link.target).map_err(|_| wasm_error!(WasmErrorInner::Guest("Invalid".into())))?, GetOptions::default())? {
            votes.push(record);
        }
    }
