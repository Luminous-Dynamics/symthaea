...
pub fn validate(op: Op) -> ExternResult<ValidateCallbackResult> {
    match op.flattened::<EntryTypes, LinkTypes>()? {
        FlatOp::StoreEntry(store_entry) => match store_entry {
            OpEntry::CreateEntry { app_entry, action: _ } => validate_create_entry(app_entry),
            OpEntry::UpdateEntry { app_entry, action: _, original_action_hash: _, original_entry_hash: _ } => {
                validate_create_entry(app_entry)
            }
            _ => Ok(ValidateCallbackResult::Valid),
        },
        FlatOp::StoreRecord(store_record) => match store_record {
            OpRecord::CreateEntry { app_entry, action: _ } => validate_create_entry(app_entry),
            OpRecord::UpdateEntry { app_entry, action: _, original_action_hash: _, original_entry_hash: _ } => {
                validate_create_entry(app_entry)
            }
            _ => Ok(ValidateCallbackResult::Valid),
        },
    ...
    match app_entry {
        EntryTypes::CrossHappDispute(dispute) => {
            if !dispute.complainant_did.starts_with("did:mycelix:") {
                return Ok(ValidateCallbackResult::Invalid("Complainant must have valid DID".into()));
            }
            if !dispute.respondent_did.starts_with("did:mycelix:") {
                return Ok(ValidateCallbackResult::Invalid("Respondent must have valid DID".into()));
            }
