...
pub fn create_enforcement_order(input: CreateEnforcementInput) -> ExternResult<Record> {
    ...
    create_link(hash_entry(&input.judgment_id)?, action_hash.clone(), LinkTypes::JudgmentToEnforcement, ())?;
    create_link(hash_entry(&input.target)?, action_hash.clone(), LinkTypes::TargetToEnforcement, ())?;
    get(action_hash, GetOptions::default())?.ok_or(wasm_error!(WasmErrorInner::Guest("Not found".into())))
}

#[derive(Serialize, Deserialize, Debug)]
pub struct CreateEnforcementInput { pub judgment_id: String, pub target: String, pub enforcement_type: EnforcementType }
...
pub fn execute_enforcement(order_id: String) -> ExternResult<Record> {
    ...
    let filter = ChainQueryFilter::new().entry_type(EntryType::App(AppEntryDef::try_from(UnitEntryTypes::EnforcementOrder)?)).include_entries(true);
    for record in query(filter)? {
        if let Some(order) = record.entry().to_app_option::<EnforcementOrder>().ok().flatten() {
            if order.id == order_id {
                let now = sys_time()?;
                let updated = EnforcementOrder { status: EnforcementStatus::Executed, executed: Some(now), ..order };
                let action_hash = update_entry(record.action_address().clone(), &EntryTypes::EnforcementOrder(updated))?;
...
pub fn get_enforcement_order(order_id: String) -> ExternResult<Option<Record>> {
    let filter = ChainQueryFilter::new()
        .entry_type(EntryType::App(AppEntryDef::try_from(UnitEntryTypes::EnforcementOrder)?))
...
pub fn get_orders_for_target(target: String) -> ExternResult<Vec<Record>> {
    let mut orders = Vec::new();
    for link in get_links(GetLinksInputBuilder::try_new(hash_entry(&target)?, LinkTypes::TargetToEnforcement)?.build())? {
...
pub fn appeal_enforcement(input: AppealEnforcementInput) -> ExternResult<Record> {
    ...
    create_link(hash_entry(&input.order_id)?, action_hash.clone(), LinkTypes::OrderToAppeal, ())?;
    ...
    for record in query(filter)? {
        if let Some(order) = record.entry().to_app_option::<EnforcementOrder>().ok().flatten() {
            if order.id == input.order_id {
                let updated = EnforcementOrder { status: EnforcementStatus::Appealed, ..order };
                update_entry(record.action_address().clone(), &EntryTypes::EnforcementOrder(updated))?;
                break;
            }
        }
    }
...
pub fn suspend_enforcement(order_id: String) -> ExternResult<Record> {
    ...
    for record in query(filter)? {
        if let Some(order) = record.entry().to_app_option::<EnforcementOrder>().ok().flatten() {
            if order.id == order_id {
                let updated = EnforcementOrder { status: EnforcementStatus::Suspended, ..order };
                let action_hash = update_entry(record.action_address().clone(), &EntryTypes::EnforcementOrder(updated))?;
                return get(action_hash, GetOptions::default())?.ok_or(wasm_error!(WasmErrorInner::Guest("Not found".into())));
            }
        }
...
pub fn get_order_appeals(order_id: String) -> ExternResult<Vec<Record>> {
    ...
    for link in get_links(GetLinksInputBuilder::try_new(hash_entry(&order_id)?, LinkTypes::OrderToAppeal)?.build())? {
        if let Some(record) = get(ActionHash::try_from(link.target).map_err(|_| wasm_error!(WasmErrorInner::Guest("Invalid".into())))?, GetOptions::default())? {
            appeals.push(record);
        }
    }
