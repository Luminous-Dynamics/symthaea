...
//! Holochain conductor context for the Civic Leptos app.
//!
//! Role-aware: calls can target "governance" or "finance" roles
//! within the unified hApp.
...
impl HolochainCtx {
    /// Call a zome function on the specified role (e.g. "governance" or "finance").
    pub async fn call_zome<I: Serialize, O: DeserializeOwned>(
        ...
        let transport = match transport.as_ref() {
            ...
            None => {
                return Err(format!(
                    "Mock mode: {role}.{zome}.{fn_name} — no conductor connected"
                ));
            }
        };
...
pub fn HolochainProvider(children: Children) -> impl IntoView {
    ...
    spawn_local(async move {
        ...
        web_sys::console::log_1(
            &format!("[Civic] Connecting to conductor at {url}…").into(),
        );
        ...
        match ws_transport.connect(config).await {
            Ok(()) => {
                web_sys::console::log_1(&"[Civic] Connected to conductor!".into());
                *transport_for_connect.borrow_mut() = Some(ws_transport);
                set_status.set(ConnectionStatus::Connected);
            }
            Err(e) => {
                web_sys::console::log_1(
                    &format!("[Civic] Could not connect: {e}. Running in mock mode.").into(),
                );
